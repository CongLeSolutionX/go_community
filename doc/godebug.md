...
Additionally, [os.Lstat](/pkg/os#Lstat) starts setting the [os.ModeIrregular](/pkg/os#ModeIrregular)
type mode bit for all reparse tags on Windows, except for those that are explicitly supported by the
[os](/pkg/os) package, which, since Go 1.23, doesn't include mount points. This helps to
identify files that need special handling outside of the [os](/pkg/os) package.
[os.Readlink](/pkg/os#Readlink) still supports reading mount points, but no longer tries to return
a canonical path for mounted volumes, which was only partially
working.
