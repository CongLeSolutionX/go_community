...
A [*version*](#glos-version) is an immutable snapshot of a module, which may be
...
this standard into canonical versions. This may result in a
[*pseudo-version*](#glos-pseudo-version), a pre-release version that encodes a
revision identifier (such as a Git commit hash) and a timestamp from a version
control system. For example, the command `go get -d golang.org/x/net@daa7c041`
will convert the commit hash `daa7c041` into the pseudo-version
`v0.0.0-20191109021931-daa7c04131f5`. Canonical versions are required outside
the main module, and the `go` command will report an error if a non-canonical
version like `master` appears in a `go.mod` file.
...
a release with compatible changes, the minor version must be incremented, and
the patch version must be set to 0.
...
required module version from a `require` statement in a `go.mod` file (subject
to `replace` and `exclude` statements in the main module's `go.mod` file.
...
**pre-release version:** A version with a hyphen followed by a series of
...
**pseudo-version:** A [pre-release version](#glos-pre-release-version) that
encodes a revision identifier (such as a Git commit hash) and a timestamp from a
version control system. For example, `v0.0.0-20191109021931-daa7c04131f5`. Used
for [compatibility with non-module repositories](#non-module-compat) and in
other situations when a tagged version is not available.
