...
The new `go` `telemetry` command displays the telemetry
collection mode. With a mode argument: `local`, `on`, or `off`, it sets the collection mode.
`go` `env` now lists two values: the `GOTELEMETRY` value, which
