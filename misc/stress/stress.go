...
var (
	flagP       = flag.Int("p", runtime.NumCPU(), "number of parallel processes")
	flagTimeout = flag.Duration("timeout", 10*time.Minute, "timeout for an individual process")
	flagKill    = flag.Bool("kill", true, "kill timed out processes, or just print pid (to attach with gdb)")
	flagFailure = flag.String("failure", "", "regexp of the sought for failure, if not empty all non-matching failures will be ignored")
)
...
func main() {
	...
	if *flagFailure != "" {
		failureRe = regexp.MustCompile(*flagFailure)
	}
	res := make(chan []byte)
	for i := 0; i < *flagP; i++ {
		go func() {
