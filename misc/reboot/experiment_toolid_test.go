...
func TestExperimentToolID(t *testing.T) {
	...
	if err := ioutil.WriteFile(filepath.Join(goroot, "VERSION"), []byte("go1.13"), 0666); err != nil {
		t.Fatal(err)
	}
	...
	wantVersion := []byte(`compile version go1.13`)
	...
	// Verify compiler version string.
	gotVersion = bytes.TrimSpace(runCmd(t, gorootSrc, env, goCmdPath, "tool", "compile", "-V=full"))
	wantVersion = []byte(`compile version go1.13 X:fieldtrack,framepointer`)
	if !bytes.Equal(gotVersion, wantVersion) {
