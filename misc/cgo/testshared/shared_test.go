...
func testMain(m *testing.M) (int, error) {
	...
	modRoot := filepath.Join(scratchDir, "src", "testshared")
	...
	err = filepath.Walk("testdata", func(path string, info os.FileInfo, err error) error {
		scratchPath := modRoot + strings.TrimPrefix(path, "testdata")
		if info.IsDir() {
			if path == "testdata" {
				return nil
			}
...
func TestABIChecking(t *testing.T) {
	...
	resetFileStamps()
	appendFile(t, "./depBase/dep.go", "func ABIBreak() {}\n")
	goCmd(t, "install", "-buildmode=shared", "-linkshared", "./depBase")
	...
	resetFileStamps()
	writeFile(t, "./depBase/dep2.go", "package depBase\nfunc noABIBreak() {}\n")
	goCmd(t, "install", "-buildmode=shared", "-linkshared", "./depBase")
