...
func goCmd(t *testing.T, args ...string) {
	...
	if err := c.Run(); err != nil {
		t.Errorf("executing %s failed", strings.Join(c.Args, " "))
	}
}
...
// isNew)
func AssertRebuilt(t *testing.T, path string) {
	if !isNew(path) {
		t.Errorf("%s was not rebuilt", path)
	}
}

// Fail if path has been rebuilt (i.e. is newer than the time stamp used by isNew)
func AssertNotRebuilt(t *testing.T, path string) {
	if isNew(path) {
		t.Errorf("%s was rebuilt", path)
	}
}
...
func TestRebuilding(t *testing.T) {
	...
	goCmd(t, "install", "-linkshared", "exe")
	AssertRebuilt(t, filepath.Join(gopathInstallDir, "dep.a"))
	AssertRebuilt(t, filepath.Join(gopathInstallDir, "libdep.so"))

	// If the .a file is newer than the .so, the .so is rebuilt (but not the .a)
	resetFileStamps()
	touch(filepath.Join(gopathInstallDir, "dep.a"))
	goCmd(t, "install", "-linkshared", "exe")
	AssertNotRebuilt(t, filepath.Join(gopathInstallDir, "dep.a"))
	AssertRebuilt(t, filepath.Join(gopathInstallDir, "libdep.so"))
}
...
func TestABIChecking(t *testing.T) {
	...
	if err == nil {
		t.Error("executing exe did not fail after ABI break")
	}
	scanner := bufio.NewScanner(bytes.NewReader(output))
	foundMsg := false
	for scanner.Scan() {
		if scanner.Text() == "abi mismatch detected between the executable and libdep.so" {
			foundMsg = true
			break
		}
	}
	...
	if !foundMsg {
		t.Error("exe did not fail with expected message")
	}
