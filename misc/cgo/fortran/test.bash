...
case "$FC" in
*gfortran*)
  lib_path=$(dirname $($FC -print-file-name=libgfortran.$libext))
  if [ "$goos" == "darwin" ]; then
    export CGO_LDFLAGS="$CGO_LDFLAGS -L $lib_path"
  else
    export CGO_LDFLAGS="$CGO_LDFLAGS -rpath $lib_path -L $lib_path"
  fi
  ;;
esac
