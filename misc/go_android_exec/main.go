...
// The following platforms lack syscall.Flock
...
func adbRun(args string) (int, error) {
	...
	filter := newExitCodeFilter(os.Stdout, exitStr)
	cmd.Stdout = filter
...
func newExitCodeFilter(w io.Writer, exitStr string) *exitCodeFilter {
	// Build a regexp that matches any suffix of the exit string.
	// We do it this way to avoid assuming anything about the
	...
	return &exitCodeFilter{w: w, exitRe: exitRe}
}
