...
// TODO(gri) need to test uint and uintptr cases as well

func TestSizes(t *testing.T) {
	var u uint
	if want := unsafe.Sizeof(u) * 8; UintSize != want {
		t.Fatalf("UintSize = %d; want %d", UintSize, want)
	}

	var p uintptr
	if want := unsafe.Sizeof(p) * 8; UintptrSize != want {
		t.Fatalf("UintptrSize = %d; want %d", UintptrSize, want)
	}
}
	...
	for i := 0; i < 256; i++ {
		...
		for k := 0; k < 64-8; k++ {
			...
			if x <= 1<<8-1 {
				got := Ones8(uint8(x))
				if got != want {
					t.Fatalf("Ones8(%#016x) == %d; want %d", x, got, want)
				}
			}
			...
			if x <= 1<<16-1 {
				got := Ones16(uint16(x))
				if got != want {
					t.Fatalf("Ones16(%#016x) == %d; want %d", x, got, want)
				}
			}
			...
			if x <= 1<<32-1 {
				got := Ones32(uint32(x))
				if got != want {
					t.Fatalf("Ones32(%#016x) == %d; want %d", x, got, want)
				}
			}
			...
			if x <= 1<<64-1 {
				got := Ones64(uint64(x))
				if got != want {
					t.Fatalf("Ones64(%#016x) == %d; want %d", x, got, want)
				}
			}
		}
	}
	...
	for i := 0; i < 256; i++ {
		...
		for k := 0; k < 64-8; k++ {
			...
			if x <= 1<<8-1 {
				...
				if got != want {
					t.Fatalf("Log8(%#016x) == %d; want %d", x, got, want)
				}
			}
			...
			if x <= 1<<16-1 {
				...
				if got != want {
					t.Fatalf("Log16(%#016x) == %d; want %d", x, got, want)
				}
			}
			...
			if x <= 1<<32-1 {
				...
				if got != want {
					t.Fatalf("Log32(%#016x) == %d; want %d", x, got, want)
				}
			}
			...
			if x <= 1<<64-1 {
				...
				if got != want {
					t.Fatalf("Log64(%#016x) == %d; want %d", x, got, want)
				}
			}
		}
	}
