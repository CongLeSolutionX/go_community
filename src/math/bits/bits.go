...
// UintptrSize is the size of a uinptr in bits.
const UintptrSize = 32 << (^uintptr(0) >> 32 & 1)

// LeadingZerosN returns the number of leading zero bits in x.
// N is one of 8, 16, 32, 64, Ptr; or absent for uint.
...
func LeadingZeros64(x uint64) int   { return nlz(x, 64) }
func LeadingZerosPtr(x uintptr) int { return nlz(uint64(x), UintptrSize) }
...
func TrailingZeros64(x uint64) int   { return ntz(x, 64) }
func TrailingZerosPtr(x uintptr) int { return ntz(uint64(x), UintptrSize) }
...
func OnesPtr(x uintptr) int { return pop(uint64(x)) }
...
func RotateLeftPtr(x uintptr, k uint) uintptr {
	return uintptr(uint(rot(uint64(x), UintptrSize, k%UintptrSize)))
}
...
func RotateRightPtr(x uintptr, k uint) uintptr {
	return uintptr(rot(uint64(x), UintptrSize, UintptrSize-k%UintptrSize))
}
...
func Reverse64(x uint64) uint64    { return rev(x, 64) }
func ReversePtr(x uintptr) uintptr { return uintptr(rev(uint64(x), UintptrSize)) }
...
func ReverseBytes64(x uint64) uint64    { return swap(x, 64) }
func ReverseBytesPtr(x uintptr) uintptr { return uintptr(swap(uint64(x), UintptrSize)) }
...
func Log(x uint) int       { return log(uint64(x), UintSize) }
func Log8(x uint8) int     { return log(uint64(x), 8) }
func Log16(x uint16) int   { return log(uint64(x), 16) }
func Log32(x uint32) int   { return log(uint64(x), 32) }
func Log64(x uint64) int   { return log(x, 64) }
func LogPtr(x uintptr) int { return log(uint64(x), UintptrSize) }
