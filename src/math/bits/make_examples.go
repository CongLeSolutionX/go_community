...
func main() {
	...
	for _, e := range []struct {
		...
		},
		{
			name: "Reverse",
			in:   15,
			out:  [4]interface{}{bits.Reverse8(15), bits.Reverse16(15), bits.Reverse32(15), bits.Reverse64(15)},
		},
		{
			name: "ReverseBytes",
			in:   15,
			out:  [4]interface{}{0, bits.ReverseBytes16(15), bits.ReverseBytes32(15), bits.ReverseBytes64(15)},
		},
		{
			name: "Len",
			in:   8,
		...
		for i, size := range []int{8, 16, 32, 64} {
			if e.name == "ReverseBytes" && size == 8 {
				continue // there's no ReverseBytes8
			}
