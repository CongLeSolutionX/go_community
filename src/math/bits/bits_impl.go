...
import "unsafe" // for size verification only

func init() {
	var u uint
	if UintSize != unsafe.Sizeof(u)*8 {
		panic("incorrect UintSize")
	}

	var p uintptr
	if UintptrSize != unsafe.Sizeof(p)*8 {
		panic("incorrect UintptrSize")
	}
}
