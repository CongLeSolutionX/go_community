...
func TestFloatQuo(t *testing.T) {
	genOperands(t, (*Float).Quo, (*Float).Mul)
}

func genOperands(t *testing.T, op, inv func(z, x, y *Float, mode RoundingMode) *Float) {
	...
	for i := 0; i < 8; i++ {
		...
		// compute accurate x as z inv y
		y := new(Float).SetFloat64(3.14159265358979323e123)
		x := NewFloat(0, y.Precision()+z.Precision(), ToZero)
		inv(x, z, y, ToZero)
		...
		// modes and compare against result of x op y.
		for _, mode := range [...]RoundingMode{ToZero, ToNearest, AwayFromZero} {
			for d := -5; d < 5; d++ {
				prec := uint(preci + d)
				got := op(NewFloat(0, prec, mode), x, y, mode)
				want := round(bits, prec, mode)
				if got.Cmp(want) != 0 {
...
func round(bits []int, prec uint, mode RoundingMode) *Float {
	// determine range
	var min, max int
	for i, b := range bits {
		if i == 0 || b < min {
	...
	if prec >= prec0 {
		return new(Float).SetBits(bits...)
	}
	// prec < prec0

	// determine bit 0, rounding, and sticky bit, and result bits
	...
	// round
	f := new(Float).SetBits(res...) // rounded to zero
