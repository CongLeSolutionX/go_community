...
// addition/subtraction of arguments represented by bit sets with the
// respective floating-point addition/subtraction for a variety of precisions
// and rounding modes.
func TestFloatAdd(t *testing.T) {
	// Each tests entry contains two bit sets representing the argument
	// floating-point numbers x and y. The bit set for x + y is the
	// concatenation of the argument bit sets (see Float.SetBits).
	var tests = []struct {
		x, y []int
	}{
		{[]int{}, []int{}},
		...
		{[]int{0}, []int{0}},
		{[]int{10}, []int{-10}},
		{[]int{100, 10, 1}, []int{0, -1, -2, -10}},
		// TODO(gri) add much more test cases
	}

	for i, test := range tests {
		x := new(Float).SetBits(test.x...)
		y := new(Float).SetBits(test.y...)
		zbits := append(test.x, test.y...)
		z := new(Float).SetBits(zbits...)

		// round accurate z for a variety of precisions and
		// modes and compare against result of x + y.
		for _, mode := range [...]RoundingMode{ToZero, ToNearest, AwayFromZero} {
			for _, prec := range precList {
				// addition
				sum := roundBits(zbits, prec, mode)
				for _, swap := range []bool{false, true} {
					...
					if swap {
						got.Sub(z, y, mode)
						diff = roundBits(test.x, prec, mode)
					} else {
						got.Sub(z, x, mode)
						diff = roundBits(test.y, prec, mode)
					}
					if got.Cmp(diff) != 0 {
						t.Errorf("i = %d, prec = %d, %s:\n\t     %s\n\t-    %s\n\t=    %s\n\twant %s",
							i, prec, mode, x, y, got, diff)
					}
				}
			}
		}
...
func TestFloatQuo(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		for _, mode := range [...]RoundingMode{ToZero, ToNearest, AwayFromZero} {
			for d := -5; d < 5; d++ {
				prec := uint(preci + d)
				got := NewFloat(0, prec, mode).Quo(x, y, mode)
...
func roundBits(x []int, prec uint, mode RoundingMode) *Float {
	...
	// round
	f := new(Float).SetBits(z...) // rounded to zero
	if mode == ToNearest && rbit == 1 && (sbit == 1 || sbit == 0 && bit0 != 0) || mode == AwayFromZero {
		// round away from zero
		f.Round(f, prec, ToZero) // extend precision // TODO(gri) better approach?
		f.Add(f, new(Float).SetBits(int(r)+1), ToZero)
	}
	return f
}
