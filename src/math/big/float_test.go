...
func TestIssue6866(t *testing.T) {
	for _, prec := range precList {
		two := NewFloat(2, prec, ToNearest)
		one := NewFloat(1, prec, ToNearest)
		three := NewFloat(3, prec, ToNearest)
		msix := NewFloat(-6, prec, ToNearest)
		psix := NewFloat(+6, prec, ToNearest)

		p := NewFloat(0, prec, ToNearest)
		z1 := NewFloat(0, prec, ToNearest)
		z2 := NewFloat(0, prec, ToNearest)

		// z1 = 2 + 1.0/3*-6
		p.Quo(one, three, ToNearest)
		p.Mul(p, msix, ToNearest)
		z1.Add(two, p, ToNearest)

		// z2 = 2 - 1.0/3*+6
		p.Quo(one, three, ToNearest)
		p.Mul(p, psix, ToNearest)
		z2.Sub(two, p, ToNearest)
...
func TestFloatQuo(t *testing.T) {
	const prec = 100
	q0 := new(Float).SetBits(prec-11, -1, -10) // q == 10...0.1000000001
	if got := q0.Precision(); got != prec {
		t.Errorf("got prec = %d; want %d", got, prec)
	}

	d := new(Float).SetFloat64(3.14159265358979323e123)
	p := NewFloat(0, d.Precision() + prec, ToNearest)
	p.Mul(d, q0, ToNearest)
	if got := p.Accuracy(); got != exact {
		t.Errorf("got acc = %s; want exact", got)
	}

	q := NewFloat(0, prec, ToNearest).Quo(p, d, ToNearest)
	if q.Cmp(q0) != 0 {
		t.Errorf("prec = %d: %s / %s = %s; want %s", q.Precision(), p, d, q, q0)
	}

	// q = 10...0.1
	q = NewFloat(0, prec-10+1, ToNearest).Quo(p, d, ToNearest)
	want := new(Float).SetBits(prec-11, -1)
	if q.Cmp(want) != 0 {
		t.Errorf("prec = %d: %s / %s = %s; want %s", q.Precision(), p, d, q, want)
	}

	// q = 10...1
	q = NewFloat(0, prec-10, ToNearest).Quo(p, d, ToNearest)
	want = new(Float).SetBits(prec-11, 0)
	if q.Cmp(want) != 0 {
		t.Errorf("prec = %d: %s / %s = %s; want %s", q.Precision(), p, d, q, want)
	}
	...
	// q = 10...0
	q = NewFloat(0, prec-10-1, ToNearest).Quo(p, d, ToNearest)
	want = new(Float).SetBits(prec-11)
	if q.Cmp(want) != 0 {
		t.Errorf("prec = %d: %s / %s = %s; want %s", q.Precision(), p, d, q, want)
	}

	// q = xxx0.0...0, prec = xxx0 + 1, ToNearest => q = xxx0.0
	// q = xxx0.0...1, prec = xxx0 + 1, ToNearest => q = xxx0.0
	// q = xxx0.1...0, prec = xxx0 + 1, ToNearest => q = xxx0.1
	// q = xxx1.1...0, prec = xxx1 + 1, ToNearest => q = xxx1.1
	// q = xxx0.1...1, prec = xxx0 + 1, ToNearest => q = xxx0+1.0

	// q = xxx0.0...0, prec = xxx0 + 0, ToNearest => q = xxx0.
	// q = xxx0.0...1, prec = xxx0 + 0, ToNearest => q = xxx0.
	// q = xxx0.1...0, prec = xxx0 + 0, ToNearest => q = xxx0.
	// q = xxx1.1...0, prec = xxx1 + 0, ToNearest => q = xxx1+1.
	// q = xxx0.1...1, prec = xxx0 + 0, ToNearest => q = xxx0+1.
}

func TestFloatSetBits(t *testing.T) {
	var tests = []struct{
		bits []int
		want string
	}{
