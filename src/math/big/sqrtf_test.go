// Copyright 2017 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package big

import (
	"fmt"
	"math"
	"math/rand"
	"testing"
)

// Check full IEEE-754 compatibility when called on a Float with a
// 53-bit mantissa
func TestSqrtf53(t *testing.T) {
	for i := 0; i < 1e5; i++ {
		r := rand.Float64()
		z := NewFloat(0) // prec defaults to 53

		x64, _ := z.Sqrt(NewFloat(r)).Float64()
		want := math.Sqrt(r)
		if x64 != want {
			t.Fatalf("Sqrt(%g) =\n got %g;\nwant %g", z, x64, want)
		}
	}
}

func TestSqrtf(t *testing.T) {
	for _, test := range []struct {
		x    float64
		want string
	}{
		// 350 decimal digits are enough to give us ~1000 binary digits
		{0.5, "0."},
		{2.0, "1."},
		{3.0, "1."},
		{4.0, "2.0"},
	} {
		for _, prec := range []uint{24, 53, 64, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000} {
			want := new(Float).SetPrec(prec)
			want.Parse(test.want, 10)

			z := new(Float).SetPrec(prec)
			z.Sqrt(NewFloat(test.x))

			if z.Cmp(want) != 0 || want.Acc() != z.Acc() {
				t.Errorf("prec = %d, Sqrt(%v) =\ngot  %g (%s);\nwant %g (%s)",
					prec, test.x, z, z.acc, want, want.acc)
			}
		}
	}
}

func TestSqrtfSpecial(t *testing.T) {
	for _, test := range []struct {
		x    *Float
		want *Float
	}{
		{NewFloat(+0), NewFloat(+0)},
		{NewFloat(-0), NewFloat(-0)},
		{NewFloat(math.Inf(+1)), NewFloat(math.Inf(+1))},
	} {
		z := new(Float).Sqrt(test.x)
		if z.neg != test.want.neg || z.form != test.want.form {
			t.Errorf("Sqrt(%v) = %v (neg: %v); want %v (neg: %v)",
				test.x, z, z.neg, test.want, test.want.neg)
		}
	}

}

// Benchmarks

func BenchmarkSqrtf(b *testing.B) {
	for _, prec := range []uint{53, 1e2, 1e3, 1e4, 1e5} {
		x := NewFloat(2)
		z := new(Float).SetPrec(prec)
		b.Run(fmt.Sprintf("%v", prec), func(b *testing.B) {
			b.ReportAllocs()
			for n := 0; n < b.N; n++ {
				z.Sqrt(x)
			}
		})
	}
}
