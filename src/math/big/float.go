...
// Uint64 truncates x towards zero and returns the resulting unsigned integer.
// The result is Exact if x.IsInt(), x >= 0, and x fits into a uint64.
// The result is (0, Above) for x < 0, and (math.MaxUint64, Below) for
// x > math.MaxUint64.
...
// Int truncates x towards zero and returns the resulting integer
// or nil if x is an infinity. The result is Exact if x.IsInt();
// otherwise it is Below for x > 0, and Above for x < 0.
func (x *Float) Int() (res *Int, acc Accuracy) {
	if debugFloat {
		x.validate()
	}
	// accurracy for inexact results
