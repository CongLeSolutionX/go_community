...
func (z *Float) round(sbit uint) {
	...
	if rbit|sbit != 0 {
		...
		roundUp := false // false means rounding down (truncating); true means rounding up
		switch z.mode {
		case ToNegativeInf:
			roundUp = z.neg
		case ToZero:
			// nothing to do
		case ToNearestEven:
			roundUp = rbit != 0 && (sbit != 0 || z.mant[0]&lsb != 0)
		case ToNearestAway:
			roundUp = rbit != 0
		case AwayFromZero:
			roundUp = true
		...
		// A positive result (!z.neg) is Above the exact result if we rounded up,
		// and it's Below if we rounded down (Exact results require no rounding).
		// For a negative result (z.neg) it's the other way round.
		z.acc = makeAcc(roundUp != z.neg)
