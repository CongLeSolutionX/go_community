...
func (x *Float) Float32() (float32, Accuracy) {
	...
	case finite:
		...
		if e < emin {
			...
			// such that its msb falls immediately to the right of the float32
			// float32 mantissa space. In other words, if the smallest denormal
			// is considered "1.0", for p == 0, the mantissa value m is >= 0.5.
...
func (x *Float) Float64() (float64, Accuracy) {
	...
	case finite:
		...
		if e < emin {
			...
			p = mbits + 1 - emin + int(e)
			// If p == 0, the mantissa of x is shifted so much to the right
			// such that its msb falls immediately to the right of the float64
			// float64 mantissa space. In other words, if the smallest denormal
			// is considered "1.0", for p == 0, the mantissa value m is >= 0.5.
			// If m > 0.5, it is rounded up to 1.0; i.e., the smallest denormal.
			// If m == 0.5, it is rounded down to even, i.e., 0.0.
			// If p < 0, the mantissa value m is <= "0.25" which is never rounded up.
