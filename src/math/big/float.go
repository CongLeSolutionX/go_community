...
// A Float may also be +0 or -0; or NaN (Not-a-Number) for values
// that are not numbers (including infinities).
//
...
// IEEE-754 Inf and NaN values are mapped to NaN, and Float operations that
// would produce an Inf or Nan in IEEE-754 result in a NaN.
//
...
type Float struct {
	mode RoundingMode
	acc  Accuracy
	neg  bool  // ignored for NaNs
	mant nat   // empty for 0 and NaNs
	exp  int32 // 0 for x, nanExp for NaN
	prec uint32
}
...
// Unless x is a zero or NaN, x.mant is normalized such that the msb of
// x.mant == 1 (i.e., the msb is shifted all the way "to the left").
// Thus, if the mantissa has trailing 0 bits or x.prec is not a multiple
// of the the Word size _W, x.mant[0] has trailing zero bits. Zero and NaN
// values have an empty mantissa and a 0 or nanExp exponent, respectively.

const (
	MaxExp  = math.MaxInt32     // largest supported exponent magnitude
	MinExp  = math.MinInt32 + 2 // smallest supported exponent magnitude
	infExp  = math.MinInt64 + 1 // reserved for future use
...
// If prec == 0, the result is ±0 for normalized z with the sign set according
// to z; it is NaN otherwise. If prec > MaxPrec, it is set to MaxPrec.
...
// The result may be 0 for |x| == 0 or NaN.
...
//	Undef.MantExp(mant) = 0, with mant set to Undef
//
...
// NaN is not considered an integer.
...
// IsNaN reports whether x is undefined.
...
// If e < -MaxExp, z becomes ±0; if e > MaxExp, z becomes NaN.
...
// (math.MaxInt64, Below) for x > math.MaxInt64 or NaN.
...
// x and y must not be 0 or an NaN.
...
// while ignoring the signs of x and y. x and y must not be 0 or an NaN.
...
// TODO(gri) think more about NaN comparison. (maybe we cannot use Cmp)
