...
func (z *Float) round(sbit uint) {
	...
	rbit := z.mant.bit(r) & 1    // rounding bit; be safe and ensure its a single bit
	if sbit == 0 {
		// TODO(gri) if rbit != 0 we don't need to compute sbit for some rounding modes (optimization)
		sbit = z.mant.sticky(r)
	}
	sbit &= 1 // be safe and ensure its a single bit
	...
	// determine number of trailing zero bits ntz and
