...
// eConvergent returns the n-th convergent.
func eConvergent(n int) *big.Rat {
	return recurse(0, int64(n))
}

// Use the classic continued fraction for e
//     e = [1; 0, 1, 1, 2, 1, 1,... 2n, 1, 1 ...]
// i.e. for the n-th term, use
//     1       if    n != 1 (mod 3)
//  2(n-1)/3   if    n == 1 (mod 3)
func recurse(n, lim int64) *big.Rat {
	var res *big.Rat
	if n%3 == 1 {
		// Initialize big.Rat from a couple of int64s
	...
	// inverse of the result of recurse.
	r := new(big.Rat).Inv(recurse(n+1, lim))
