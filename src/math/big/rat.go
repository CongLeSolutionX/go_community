...
// infinite recurring decimal, ok == false means affirmative.
func (z *Rat) FloatPrec() (digits int, ok bool) {
	if z.IsInt() {
		return 0, true
	}

	// The following code utilizes a mathematical approach to deduce
	// the initial transient of the fraction z.
	//
	// For a fraction to have a terminating decimal representation
	// (or a non-repeating decimal portion), the denominator must be
	// of the form 2^n * 5^m * K where n and m are both non-negative
	// integers (including 0) and K is an integer (>0) coprime to 10.
	//
	// If the denominator is of the form 2^n * 5^m with K == 1 after simplification,
	// then the fraction is a terminating decimal, on the other hand, if K != 1,
	// the fraction is eventually periodic, but with an initial transient. Note that
	// there is one special case where n and m are both zero, which makes the fraction
	// an infinite recurring decimal with no initial transient, on the basis of Euler's theorem.
	//
