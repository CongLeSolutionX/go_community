...
//
//	x = x**b[0] + x**b[1] + ... x**b[len(b)-1]
//
// A Bits value is normalized if each b[i] occurs at most once. For instance
// Bits{0, 0, 1} is not normalized but represents the same floating-point
// number as Bits{2}, which is normalized. The zero (nil) value of Bits is
// a ready to use Bits value and represents the value 0.
type Bits []int
