...
func (z *Float) exponential(x *Float) *Float {
	...
	for loop := newLoop_("exponential", x, 1, 20); ; {
		term.Set(xN)
...
func newLoop(name string, x *Float, i, itersPerBit uint64) *loop {
	...
	return &loop{
		name:   name,
		x:      x,
		prev:   new(Float).SetMode(mode).SetPrec(prec),
		last:   new(Float).SetMode(mode).SetPrec(prec),
		index_: new(Float),
		i:      i,
	}
}
