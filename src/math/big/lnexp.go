...
func (x *Float) mantExp10(mant *Float) int {
	...
	exp10.Mul(exp10, ln2(mant.Prec()))
	exp10.Quo(exp10, ln10(mant.Prec()))
...
func makeAllocator(sample *Float) allocator { return allocator{uint(sample.prec)} }
func (a allocator) float() *Float           { return new(Float).SetPrec(a.prec) }
...
func (z *Float) exponential(x *Float) *Float {
	...
	// don't destroy x while computing z
	if x == z {
		t := new(Float).Set(x)
		x = t
	}
	...
	const extra = 16 // TODO(gri) what is the right number?
	z.prec += extra
	z.mode = ToZero
	a := makeAllocator(z)
...
func (z *Float) ln(x *Float) *Float {
	...
	// perform computation with extra bits
	const extra = 32 // TODO(gri) what is the right number?
	z.prec += extra
	a := makeAllocator(z)
	...
	case -1:
		// x < 1
		if x == z {
			// don't destroy x during computation
			x = a.float().Set(x)
		}
	case 0:
		// x == 1
...
// TODO(gri) decide on one of the looping constructs
...
func (l *loop) done(z *Float) bool {
	...
	if l.prev.Cmp(l.last) == 0 {
		...
		if l.fixedCount >= l.itersPerBit {
			//println(l.name, l.x.String(), l.i, "iterations", z.prec)
			return true
		}
	} else {
		l.fixedCount = 0
...
var ln2Cache, ln10Cache struct {
	val *Float
	sync.RWMutex
}

func init() {
	ln2Cache.val, _ = new(Float).SetPrec(3000).SetString(ln2Str)
	ln10Cache.val, _ = new(Float).SetPrec(3000).SetString(ln10Str)
}

// TODO(gri) factor code below!

func ln2(prec uint) *Float {
	ln2Cache.RLock()
	if ln2Cache.val.Prec() >= prec {
		v := ln2Cache.val
		ln2Cache.RUnlock()
		return v
	}
...
func ln10(prec uint) *Float {
	ln10Cache.RLock()
	if ln10Cache.val.Prec() >= prec {
		v := ln10Cache.val
		ln10Cache.RUnlock()
		return v
	}
	ln10Cache.RUnlock()
	ln10Cache.Lock()
	ln10Cache.val.SetPrec(prec)
	ln10Cache.val.ln(ten)
	v := ln10Cache.val
	ln10Cache.Unlock()
	return v
}

const ln2Str = "0.6931471805599453094172321214581765680755001343602552541206800094933936219696947156058633269964186875" +
...
const ln10Str = "2.30258509299404568401799145468436420760110148862877297603332790096757260967735248023599720508959829" +
