...
func (x *Float) mantDecimalExp(mant *Float) (dexp int) {
	a := makeAllocator(mant)

	// x = mant * 2**exp
	exp := int64(x.MantExp(mant)) // 0.5 <= mant < 1.0
	positive := true
	if exp < 0 {
		exp = -exp
		positive = false
	...
	// compute decimal exponent fexp = exp * (ln2/ln10)
	fexp := a.float().SetInt64(exp)
	fexp.Mul(fexp, a.ln2())
	fexp.Quo(fexp, a.ln10())
	...
	// back into the mantissa.
	iexp, _ := fexp.Int64()
	// TODO(gri) Have exponentioal deal with negative values
	// correctly, then we don't need to differentiate here.
	if positive {
		dexp = int(iexp)
	} else {
		dexp = -int(iexp)
	...
	frac := fexp.Sub(fexp, a.float().SetInt64(iexp))
...
func makeAllocator(sample *Float) allocator {
	prec := uint(64)
	if sample != nil {
		prec = uint(sample.prec)
	}
	return allocator{prec}
}
...
func (a allocator) ln2() *Float {
	x, _ := a.float().SetString(strLn2)
	return x
}

func (a allocator) ln10() *Float {
	x, _ := a.float().SetString(strLn10)
	return x
}
...
// https://github.com/robpike/ivy, adjusted to make it work in this package.
//
...
func (z *Float) exponential(x *Float) *Float {
	...
	for loop := newLoop("exponential", x, 4); ; {
		term.Set(xN)
	...
	// e**(-x) == 1/(e**x)
	if x.neg {
		// Cannot use floatOne below due to initializtion cycle
		// TODO(gri) fix this
		t := a.float().SetUint64(1)
		z.Quo(t, z)
	}
...
func newLoop(name string, x *Float, itersPerBit uint64) *loop {
	a := makeAllocator(x)
	return &loop{
		name:          name,
		start:         a.float().Set(x),
