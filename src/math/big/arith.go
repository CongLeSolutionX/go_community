...
//	 		If r<q0, We won't enter branch1. And because 0<=rr<d, won't enter branch2. Then we get floor(realQ) without adjusting.
//	 		But when r>=q0, we will enter branch1, so we need enter branch2 next to get the right qq. So next we prove if rr >= q0, then d<= rr+d <B.
...
func getInvert(d1 Word) (inv uint) {
	...
	x1 := ^u
	x0 := uint(_M)
	inv, _ = bits.Div(x1, x0, u) // (_B^2-1)/U-_B = (_B*(_M-U)+_M)/U
	return
}
