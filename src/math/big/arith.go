...
// getInvert return the multiplicative inverse of the divisor.
// inverse = floor(( _B^2 - 1 ) / u1 - _B). u1 = d1 << nlz(d1).
func getInvert(d1 Word) (inv uint) {
	...
	// As u1 = d1 << nlz(d1), 2 * u1 >= _B > qh * ul, we need to We need to subtract one from qh at most twice.
	p := qh * ul
	if r < p {
		qh--
		r += u1
