...
func divWW(x1, x0, y Word, m uint) (q, r Word) {
	s := nlz(y)
	if s != 0 {
		x1 = x1<<s | x0>>(_W-s)
		x0 <<= s
...
// getReciprocal return the reciprocal of the divisor. rec = floor(( _B^2 - 1 ) / u - _B). u = d1 << nlz(d1).
func getReciprocal(d1 Word) (rec uint) {
	u := uint(d1 << nlz(d1))
	x1 := ^u
	x0 := uint(_M)
	rec, _ = bits.Div(x1, x0, u) // (_B^2-1)/U-_B = (_B*(_M-U)+_M)/U
	return
}
