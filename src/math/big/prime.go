...
func (x *Int) ProbablyPrime(n int) bool {
	...
	w := x.abs[0]
	if len(x.abs) == 1 && w < 64 {
		return (uint64(primeBitMask)>>w)&1 != 0
	}
...
func (n nat) probablyPrimeLucas() bool {
	...
	for ; ; p++ {
		if p > 10000 {
			...
			panic("math/big: internal error: cannot find (D/n) = -1 for " + (&Int{abs: n}).String())
		}
		d[0] = p*p - 4
		j := Jacobi(intD, intN)
		if j == -1 {
		...
		if p == 40 {
			...
			// After 5 attempts, take a moment to check if n is indeed a square.
			t1 = t1.sqrt(n)
			t1 = t1.mul(t1, t1)
			if t1.cmp(n) == 0 {
				return false
