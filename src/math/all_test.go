...
func compareCases[F float32 | float64]() []compareTest[F] {
	nan64 := NaN()
	zero, nan, inf := F(0.0), F(nan64), F(Inf(0))

	// construct -NaN using Copysign instead of the expression "-NaN",
	// since that behavior isn't well-defined across architectures.
	negnan := F(Copysign(nan64, -1))
