...
func TestRoundTiesToEven(t *testing.T) {
	for i := 0; i < len(vf); i++ {
		if f := RoundTiesToEven(vf[i]); !alike(round[i], f) {
			t.Errorf("RoundTiesToEven(%g) = %g, want %g", vf[i], f, round[i])
		}
	}
	for i := 0; i < len(vfroundEvenSC); i++ {
		if f := RoundTiesToEven(vfroundEvenSC[i][0]); !alike(vfroundEvenSC[i][1], f) {
			t.Errorf("RoundTiesToEven(%g) = %g, want %g", vfroundEvenSC[i][0], f, vfroundEvenSC[i][1])
		}
	}
}
...
func BenchmarkRoundTiesToEven(b *testing.B) {
	x := 0.0
	for i := 0; i < b.N; i++ {
		x = RoundTiesToEven(roundNeg)
	}
	GlobalF = x
}
