...
func shl(bhi, blo uint64, n uint) (rhi, rlo uint64) {
	rhi = bhi<<n | blo>>(64-n) | blo<<(n-64)
	rlo = blo << n
	return
}

func shr(bhi, blo uint64, n uint) (rhi, rlo uint64) {
	rlo = blo>>n | bhi<<(64-n) | bhi>>(n-64)
	rhi = bhi >> n
	return
}

func shrcompress(hi, lo uint64, n uint) (rhi, rlo uint64) {
	// Performance here is really sensitive to the order/placement
	// of these branches. n == 0 is common enough to be in the fast
	// path. TODO: Perhaps more measurement needs to be done to find
	// the optimal order/placement?
	switch {
	case n == 0:
		return hi, lo
	case n == 64:
		return 0, hi | nonzero(lo)
	case n >= 128:
		return 0, nonzero(hi | lo)
	case n < 64:
		rhi, rlo = shr(hi, lo, n)
		rlo |= nonzero(lo & (1<<n - 1))
	case n < 128:
		rhi, rlo = shr(hi, lo, n)
		rlo |= nonzero(hi&(1<<(n-64)-1) | lo&(1<<n-1))
	}
	return
}

func lz(hi, lo uint64) (l int32) {
	l = int32(bits.LeadingZeros64(hi))
	if l == 64 {
		l += int32(bits.LeadingZeros64(lo))
	}
	return l
}

// split splits f into sign, biased exponent, and mantissa.
// It adds the implicit 1 bit to the mantissa for normal values,
// and normalizes subnormal values.
func split(f float64) (sign uint32, exp int32, mantissa uint64) {
	b := Float64bits(f)
	sign = uint32(b >> 63)
	exp = int32(b>>52) & (1<<11 - 1)
	mantissa = b << 11 >> 11
...
func Fma(x, y, z float64) float64 {
	...
	if f := x/x + y/y; f != f || z == 0.0 {
		return x*y + z
	}
	// Handle non-finite z separately. Evaluating x*y+z with no
	...
	if pe > 1022+bias || pe == 1022+bias && (m+1<<9)>>63 == 1 {
		...
		return Float64frombits(uint64(ps)<<63 | (1<<11-1)<<52)
	}
	if pe < 0 {
		n := uint(-pe)
