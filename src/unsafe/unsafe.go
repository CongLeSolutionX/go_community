...
// SliceData returns a pointer to the underlying array of s.
// If len(s) > 0 SliceData returns &s[0].
// Otherwise, if cap(s) > 0 SliceData returns &s[:1][0].
// Otherwise, if s == nil, SliceData returns nil.
// Otherwise, (s != nil && cap(s) == 0), SliceData returns a non-nil pointer to
// an unspecified element of the underlying array.
...
// If len(str) == 0, StringData may or may not return nil.
