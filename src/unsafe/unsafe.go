...
// Add adds len to ptr, returning the updated pointer: unsafe.Pointer(uintptr(ptr) + uintptr(len)).
// The size argument len must be of integer type or an untyped constant.
// A constant size argument must be representable by a value of type int;
...
// and whose length and capacity are len: (*[len]ArbitraryType)(unsafe.Pointer(ptr))[:].
// The size argument len must be of integer type or an untyped constant.
