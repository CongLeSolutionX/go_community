	...
	for _, ev := range events {
		...
		case EvUserTaskCreate:
			if prevEv, ok := tasks[ev.Args[0]]; ok {
				return fmt.Errorf("task id conflicts (id:%d), %q vs %q", ev, prevEv)
			}
			tasks[ev.Args[0]] = ev
		case EvUserTaskEnd:
			if prevEv, ok := tasks[ev.Args[0]]; ok {
...
const (
	...
	EvUserTaskCreate    = 45 // trace.NewContext [timestamp, internal id, internal parent id, stack, name string]
	EvUserTaskEnd       = 46 // end of task [timestamp, internal id, stack]
	EvUserSpan          = 47 // trace.Attach [timestamp, internal id, mode, stack]
...
var EventDescriptions = [EvCount]struct {
	...
	EvUserTaskCreate:    {"UserTaskCreate", 1010, true, []string{"taskid", "pid", "name id"}, []string{"name"}},
	EvUserTaskEnd:       {"UserTaskEnd", 1010, true, []string{"taskid"}, nil},
	EvUserSpan:          {"UserSpan", 1010, true, []string{"taskid", "mode", "name id"}, []string{"name"}},
	EvUserLog:           {"UserLog", 1010, true, []string{"id", "key id"}, []string{"key", "value"}},
}
