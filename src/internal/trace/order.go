	...
	for ; pending != 0; pending-- {
		...
		for i, b := range batches {
			...
			if g != unordered {
				...
				if curr.seq != noseq && curr.seq != state.seq || curr.status != state.status {
					//println("  consider", ev.Ts, "from", i, "g:", g, "curr:", curr.seq, curr.status, "real:", state.seq, state.status, "NOT READY")
					continue
				}
			}
			//println("  consider", ev.Ts, "from", i, "g:", g, "curr:", curr.seq, curr.status, "next:", next.seq, next.status, "READY")
		...
		if f.g != unordered {
			...
			if f.curr.seq != noseq && f.curr.seq != state.seq || f.curr.status != state.status {
				//println("  event", f.ev.Ts, "from", f.batch, "g:", f.g, "curr:", f.curr.seq, f.curr.status, "real:", state.seq, state.status)
