...
// order merges a set of per-P event batches into a single, consistent stream.
...
// This approach ensures that we form consistent stream even if timestamps are
...
func stateTransition(ev *Event) (g uint64, init, next gState) {
	...
	case EvGoStartLocal:
		...
		ev.Type = EvGoStart // lower to EvGoStart, EvGoStartLocal is merely for ordering
		g = ev.G
...
func transition(gs map[uint64]gState, g uint64, init, next gState) {
	...
	curr := gs[g]
	if init.seq != noseq && init.seq != curr.seq || init.status != curr.status {
