...
func (o *ordering) advance(ev *baseEvent, evt *evTable, m ThreadID, gen uint64) (schedCtx, bool, error) {
	...
	case go122.EvGoDestroySyscall:
		...
		if err := validateCtx(curCtx, event.SchedReqs{Thread: event.MustHave, Proc: event.MustNotHave, Goroutine: event.MustHave}); err != nil {
			return curCtx, false, err
		}
		...
		if state.status != go122.GoSyscall {
			return curCtx, false, fmt.Errorf("%v event for goroutine that's not %v", typ, GoSyscall)
		}
		// This goroutine is exiting itself.
		delete(o.gStates, curCtx.G)
		newCtx.G = NoGoroutine
...
type taskState struct {
	// nameID is the type of the active task.
	nameID stringID
