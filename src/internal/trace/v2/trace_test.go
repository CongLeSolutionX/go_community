...
func TestTraceAnnotations(t *testing.T) {
	testTraceProg(t, "annotations.go", func(t *testing.T, tb, _ []byte) {
		type evDesc struct {
...
func TestTraceCPUProfile(t *testing.T) {
	testTraceProg(t, "cpu-profile.go", func(t *testing.T, tb, stderr []byte) {
...
func TestTraceFutileWakeup(t *testing.T) {
	testTraceProg(t, "futile-wakeup.go", func(t *testing.T, tb, _ []byte) {
...
func TestTraceStacks(t *testing.T) {
	testTraceProg(t, "stacks.go", func(t *testing.T, tb, _ []byte) {
		type frame struct {
			fn   string
			line int
		}
		...
		const mainLine = 21
		want := []evDesc{
			{trace.EventRangeBegin, "GC concurrent mark phase", []frame{
				{"runtime.GC", 0},
				{"main.main", 0},
			}},
...
func testTraceProg(t *testing.T, progName string, extra func(t *testing.T, trace, stderr []byte)) {
	testenv.MustHaveGoRun(t)

	// Check if we're on a builder.
	onBuilder := testenv.Builder() != ""

	testPath := filepath.Join("./testprog", progName)
	testName := progName
