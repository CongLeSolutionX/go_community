...
// flushActiveSpans is called when processing a goroutine end event
// to finalize any active spans in the goroutine.
func (g *GDesc) flushActiveSpans(lastTs, activeGCStartTime int64, trigger *Event) {
...
func GoroutineStats(events []*Event) map[uint64]*GDesc {
	...
	for _, ev := range events {
		...
		case EvGoEnd, EvGoStop:
			...
			g.flushActiveSpans(lastTs, gcStartTime, ev)
	...
	for _, g := range gs {
		...
		g.flushActiveSpans(lastTs, gcStartTime, nil)
