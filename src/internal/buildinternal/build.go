...
func Eval(ctxt *Context, x constraint.Expr, allTags map[string]bool) bool {
...
func matchTag(ctxt *Context, name string, allTags map[string]bool) bool {
	if allTags != nil {
		allTags[name] = true
	}
...
func SaveCgo(ctxt *Context, filename string, di *Package, text string) error {
	for _, line := range strings.Split(text, "\n") {
		...
		case "CFLAGS", "CPPFLAGS", "CXXFLAGS", "FFLAGS", "LDFLAGS":
			...
			makePathsAbsolute(ctxt, args, di.Dir)
		}
...
func makePathsAbsolute(ctxt *Context, args []string, srcDir string) {
	nextPath := false
	for i, arg := range args {
		if nextPath {
...
func matchAuto(ctxt *Context, text string, allTags map[string]bool) bool {
...
func GoodOSArchFile(ctxt *Context, name string, allTags map[string]bool) bool {
