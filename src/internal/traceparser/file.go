...
func (p *Parsed) parseHeader() error {
	...
	n, err := p.r.Read(buf[:])
	if n != 16 || err != nil {
		return fmt.Errorf("hdr read %d (expected 16), err %v", n, err)
	}
	...
	// PJW: we dont'have the special code for versions < 1010 yet
	switch ver {
	case 1005, 1007:
...
func (p *Parsed) scanFile() error {
	...
	for {
		...
		if err != nil {
			msg := fmt.Sprintf("argsAt error %v; off=%d off0=%d %s",
		...
		switch typ {
		case EvBatch:
			if len(args) != 2 { // PJW: what's this about? (version skew)
				err := fmt.Errorf("unexpected: %d not 2, at %d EvBatch", len(args), p.Count)
				return err
			}
			if footerLoc == 0 {
				// EvBatch in footer is just to have a header for stacks
				locp := int64(args[0])
				p.batches = append(p.batches, batch{Off: off0, P: locp,
					Cycles: int64(args[1])})
