...
func (p *Parsed) createEvents(f func(string)) error {
	...
	for i, ev := range sorted {
		...
		// PJW: why isn't this in the loop at line 38
...
// eventList for sorting Events by time
...
// deterministic sorting, but affects semantics when the Ts's are equal
// Sort by timestamp. with equal timestamp by P stably.
func (l eventList) Less(i, j int) bool {
	if l[i].Ts != l[j].Ts {
		return l[i].Ts < l[j].Ts
	}
	//return l[i].P < l[j].P // PJW
	return false
}
