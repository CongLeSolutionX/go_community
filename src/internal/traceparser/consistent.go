...
// This code is copied from runtime/trace. With greater understanding it could be
...
func (p *Parsed) postProcess(events []*Event) error {
	...
	checkRunning := func(pd pdesc, g gdesc, ev *Event, allowG0 bool) error {
		if g.state != gRunning {
			return fmt.Errorf("not gRunning %s %v %v", ev, g, allowG0)
		}
		if pd.g != ev.G {
			return fmt.Errorf("%s is on p, but p is %v", ev, pd)
		}
		if !allowG0 && ev.G == 0 {
			return fmt.Errorf("!allowG0 g 0 for %s", ev)
		}
		return nil
	}
	for i, ev := range events {
		...
		case EvGoUnblock:
			if g.state != gRunning {
				return fmt.Errorf("%d %s is not running at unblock %s", i, ev, g.state)

			}
			if ev.P != TimerP && px.g != ev.G {
				// PJW: do better here.
				log.Printf("hey there %d %s", i, ev)
				for j := 0; j <= i; j++ {
					log.Printf("%d/%d %s", j, i, events[j])
				}
		...
		case EvGoSysCall:
			if err := checkRunning(px, g, ev, false); err != nil {
				log.Printf("%d: %v", i, err) // PJW: for debugging
				log.Printf("known Ps:")
				for k := range ps {
					log.Printf("  %d", k)
				}
		...
		case EvUserRegion:
			// PJW: ev.G == 0 would be useless and wrong. can it happen?
			mode := ev.Args[1]
			spans := activeSpans[ev.G]
