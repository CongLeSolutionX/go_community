...
func (p *Parsed) batchify(b *batch) error {
	...
	for off := b.Off; ; {
		...
		ev2 := RawEvent{typ: typ, off: uint32(off0 - b.Off)}
		var args []uint64
		...
		if typ == EvUserLog {
			...
			if ev2.sarg != "" { // how can this possible happen?
				err = fmt.Errorf("EvUserLog already has a SArg at offset 0x%x", off)
			}
			ev2.sarg = s
		}
		evs = append(evs, ev2)
