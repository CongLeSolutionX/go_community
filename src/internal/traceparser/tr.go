...
// Parsed is the result of parsing a trace file stored in the os's filesystem
type Parsed struct {
	// Set by New
	...
	Version            int         // version of the trace file
	...
	batches            []batch
	timerGoids         map[uint64]bool // which we've seen
	// the following are per Parse
	MinWant, MaxWant int64    // range wanted
	Err              error    // stop processing
...
// some information about the parse in P.
...
func commonInit(fd filebuf.Buf, fname string) (*Parsed, error) {
	...
	// PJW: done with seenArgs
	ans.seenArgs = nil
	// convert the clicks in batches to nanoseconds
	ans.toNanoseconds()
	if ans.MaxTs <= 0 || ans.TicksPerSec <= 0 {
		err := ParseError{P: ans, Err: fmt.Errorf("corrupt trace file, negative time %d/%d",
...
func (p *Parsed) toNanoseconds() {
	...
	for i, ex := range p.batches {
		...
		e := p.batches[i]
		if false && e.Nano < 0 { // might happen once, for earliest event
			log.Fatalf("i:%d e:%v ex %v minTs:%d", i, e, ex, minCycles)
		}
	}
	p.MaxTs = int64(float64(p.maxticks-minCycles) * freq)
}

// argsAt returns the args of an event in the file
// and the offset for the next event.
// EvString returns off, nil, nil, and EvUserLog ignores
// its string argument.
func (p *Parsed) argsAt(off int, check byte) (int, []uint64, error) {
	...
	if typ == EvUserLog {
		// we do not absorb the string argument. Callers must treat EvUserLog specially
	}
	return off, args, nil
}
...
// OSStats reports on the underlying i/o if p was ceated by New().
