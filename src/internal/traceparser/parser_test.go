...
func TestStats(t *testing.T) {
	...
	// but the NaCl client always fails on os.Open. Fortunately, if returns
	// nil, nil from ioutil.ReadDir()
	files, err := ioutil.ReadDir("./testdata")
	if err != nil {
		t.Fatal(err)
	}
	for _, f := range files {
		...
		if err != nil {
			errx := err
			// Temporary code to diagnose NaCl trybot failure
			dir, err := os.Getwd()
			if err != nil {
			...
			if err != nil {
				t.Fatalf("Readirnames failed %v", err)
			}
			t.Logf(". contains %v", names)
			t.Fatal(errx)
		}
		stat := p.OSStats()
