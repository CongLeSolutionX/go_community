...
index2plus:
	...
	ADD      $16, R7, R9  // Check if next 16 bytes past last
	CMP      R9, LASTBYTE // compare with last
	BGE	 index2rem    // carry out simple comparison for the tailend
	MOVD     $0xff00, R21 // Mask for later
	MTVSRD   R21, V25     // Move to Vreg
	VSPLTH   $3, V25, V31 // Splat mask
	VSPLTH   $0, V0, V1   // Splat 1st 2 bytes of sep
...
index2loop:
	...
	BLT     index2loop2     // If < last, continue loop
	CMP     R7, LASTBYTE    // Compare addr+16 against last byte
	BLT     index2rem       // If < 16 carry out simple comparison for the tailend
...
index3plus:
	...
	ADD    $19, R7, R9  // Find bytes for use in this loop
	CMP    R9, LASTBYTE // Compare against last byte
	BGE    index3rem    // Remaining string 2<=len<=16 carry out simple comparison for the tailend
	MOVD   $0xff00, R21 // Set up mask for upcoming loop
	MTVSRD R21, V25     // Move mask to Vreg
	VSPLTH $3, V25, V31 // Splat mask
	VSPLTH $0, V0, V1   // Splat 1st two bytes of sep
...
index3loop:
	...
	BLT      index3loop2  // If within, continue this loop
	CMP      R7, LASTSTR  // Compare against last start byte
	BLT      index3rem2   // Process remainder, carry out simple comparison, skip repeat comparison with LASTSTR
	VSPLTISB $0, V3       // Special case for last 16 bytes
	BR       index3loop   // Continue this loop
...
index4plus:
	...
	ADD  $20, R7, R9  // Check string size to load
	CMP  R9, LASTBYTE // Verify string length
	BGE  index4rem    // If not large enough, process remaining, carry out simple comparison
...
next4:
	...
	BLT     index4loop   // If not, continue loop
	CMP     R7, LASTSTR  // Check remainder
	BLE     index4rem    // Process remainder, carry out simple comparison, skip repeat comparison with LASTSTR
	BR      notfound     // Not found
...
index5plus:
        ...
        // Set up masks for use with VSEL
	// we use the same strategy as for 4 byte separator except that we add one extra byte comparison
        MOVD    $0xff, R21 // Set up mask 0xff000000ff000000...
        SLD     $24, R21
        MTVSRWS R21, V29
...
index6plus:
	...
	SUB	R7, R27, R15 // separator is 7 bytes
	CMP	R15, $16     
	BLT	index7rem    // if remaining string is < 16 bytes process remainder, use simple comparison
...
index2to16tail:
	...
	SUB     R16,R9, R10    // At this point R9 should be either 18 or 17
        LXVB16X (R7)(R10), V9  
	CMP	R10, $1	       // if it is 17 compare that one byte and go to remainder loop 
	BNE	extra2	       // if it is 18 bytes make two comparisons for each of them and then go to remainder loop
	...
	VAND       V1, SEPMASK, V2 // Just compare size of sep
        ...
        VSLDOI     $1, V1, V25, V1 // Shift string left by 1 byte
	VSPLTISB	$0, V25
        BR         index2to16next  // Check the remaining partial strings in index2to16next
extra2:
...
#endif
	RET
found4: ADD $1, R7     // found from unrolled loop at index 3
found3: ADD $1, R7     // found from unrolled loop at index 2
found2: ADD $1, R7     // found from unrolled loop at index 1
