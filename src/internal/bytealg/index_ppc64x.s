...
TEXT ·Index(SB),NOSPLIT|NOFRAME,$0-56
        ...
        MOVD    b_sep+24(FP), R5        // R5 = separator pointer
        MOVD    b_seplen+32(FP), R6     // R6 = separator length
...
TEXT ·IndexString(SB),NOSPLIT|NOFRAME,$0-40
        ...
        MOVD    s_sep+16(FP), R5  // R5 = separator pointer
        MOVD    s_seplen+24(FP), R6 // R6 = separator length
...
loadge16:
	...
	CMP     R10, $16        // I think this is right??
	BGE     sepge16         // Sep crosses 16 byte boundary

	RLDICR  $0, R5, $59, R8 // Adjust addr to 16 byte container
	LXVD2X  (R8)(R0), V0_   // Load 16 byte container (LVX?)
	VPERM   V0, V0, SWAP, V0 // Fix endian order
	SLD     $3, R9         // Set up shift
	MTVSRD  R9, V8_        // Set up shift
	VSLDOI	$8, V8, V8, V8
...
sepge16:
	LXVD2X	(R5)(R0), V0_	// Load 16 bytes of sep
	VPERM	V0, V0, SWAP, V0 // Put in correct order

	VAND    V0, SEPMASK, V0 // mask out separator
	BLE	CR4, index2to16	// Correct?
...
// Loop as long as len(string) > 16
index2loop2:
	LXVD2X	(R7)(R19), V3_	// Load 16 bytes @ stringaddr +1
	VPERM	V3, V3, SWAP, V3 // Fix endian order
index2loop:
	LXVD2X	(R7)(R0), V2_	// Load 16 bytes of string
	VPERM	V2, V2, SWAP, V2 // Fix endian order
...
index3loop2:
	...
	VSPLTISB $0, V10	// Clear V10
	LXVD2X	(R7)(R21), V3_	// Load string[2:18]
	VPERM	V3, V3, SWAP, V3 // Fix endian order
	VSLDOI	$14, V3, V10, V3 // Left justify next 2 bytes
index3loop:
	LXVD2X	(R7)(R0), V2_	// Load string[0:16]
	VPERM	V2, V2, SWAP, V2 // Fix endian order
...
// Set up masks to isolate appropriate 1st byte
// when comparing splatted sep against shifted
// strings.
index4plus:
...
next4:
	...
	LXVD2X	(R7)(R9), V3_	// Load 16 bytes at string+3
	VPERM	V3, V3, SWAP, V3 // Fix endian order
...
index2to16loop:
	LXVD2X	(R7)(R0), V1_	// Load 16 bytes of string
	VPERM	V1, V1, SWAP, V1 // Fix endian order
	//TODO: This could be improved if the string could be checked
...
index2to16tail:
	...
	LXVD2X	(R7)(R0), V1_	// OK to load using R7 addr
	VPERM	V1, V1, SWAP, V1 // Correct endian order
	BR	index2to16next	// Continue on
short:
	RLDICR	$0, R7, $59, R9	// Adjust addr to 16 byte container
	LXVD2X	(R9)(R0), V1_	// Load 16 byte container (LVX?)
	VPERM	V1, V1, SWAP, V1 // Fix endian order
...
index17plus:
	...
	LXVD2X	(R5)(R9), V1_	// Load string bytes (TODO: too long?)
	VPERM	V1, V1, SWAP, V1 // Fix endian order
	VSLO	V1, V9, V1	// Shift left TODO: perm?
	VSPLTISB $0xff, V7	// Splat 1s
	VSPLTISB $0, V27	// Splat 0

index17to32loop:
	LXVD2X	(R7)(R0), V2_	// Load next 16 bytes
	VPERM	V2, V2, SWAP, V2 // Fix endian order
next17:
	LXVD2X	(R7)(R9), V3_	// Load 16 bytes @ R9 offset
	VPERM	V3, V3, SWAP, V3 // Fix endian order
...
index33plus:
	CMP	R6, $48		// Check for next size
	BGT	index49plus
	SUB	$32, R6, R9	// Length over 33
	LXVD2X	(R5)(R16), V1_	// Load next 16
	VPERM	V1, V1, SWAP, V1 // Fix endian order
	SLD	$56, R9, R10	// Shift value for use in VSLO
	MTVSRD	R10, V9_	// Move shift value to V9
	ADD	$16, R5, R15	// Adjust address
	LXVD2X	(R15)(R9), V2_	// Load next 16 including adj bytes
	VPERM	V2, V2, SWAP, V2 // Fix endian order
	VSLO	V2, V9, V2	// Shift over to left justify
	VSPLTISB $0xff, V11	// Splat 1s
	VSPLTISB $0, V27	// Splat 0s

index33to48loop:
	ADD	$16, R7, R15	// Next 16
	LXVD2X	(R7)(R0), V3_	// Load 1st 16
	LXVD2X	(R7)(R16), V4_	// Load 2nd 16
	LXVD2X	(R7)(R9), V5_	// Load remaining 16
	VPERM	V3, V3, SWAP, V3 // Fix endian order
	VPERM   V5, V5, SWAP, V5
	VPERM	V4, V4, SWAP, V4
	VSLO	V5, V9, V5	// Shift to isolate extra bytes

	VCMPEQUB V0, V3, V6	// Compare 1st 16 bytes
	VCMPEQUB V1, V4, V7	// Compare 2nd 16 bytes
	VCMPEQUB V2, V5, V8	// Compare extra bytes over 32
	VAND	V6, V7, V9	// If all equal, all bits set
	VAND	V8, V9, V10	// Same
	VCMPEQUBCC V10, V11, V12 // Check if all equal
	BLT	CR6, found	// All equal
	ADD	$1, R7		// Try next byte
	CMP	R7, LASTSTR	// Compare against last starting byte
	BGT	notfound	// Not found
	BR	index33to48loop	// Continue on

index49plus:
	CMP	R6, $64		// Compare against max
	BGT	index65plus
	SUB	$48, R6, R9	// Remaining length over 48
	SLD	$56, R9, R10	// Justification needed for LXVLL
	MTVSRD	R10, V9_	// Move length
	MOVD	$32, R21	// Set up index for 32
	MOVD	$48, R22	// Set up index for 48
	VSPLTISB $0, V27	// Zero
	ADD	$32, R5, R15	// Set up base for sep
	LXVD2X	(R5)(R0), V0_	// Load 16 bytes sep
	LXVD2X	(R5)(R16), V1_	// Load next 16 bytes sep
	LXVD2X	(R5)(R21), V2_	// Load next 16 bytes sep
	LXVD2X	(R5)(R9), V3_	// Load last bytes in sep
	VPERM	V0, V0, SWAP, V0 // Fix endian order
	VPERM	V1, V1, SWAP, V1
	VPERM	V3, V3, SWAP, V3
	VPERM	V2, V2, SWAP, V2
	VSLO	V3, V9, V3	// Left justify last part
index49to64loop:
	ADD	$32, R7, R15	// Set up base for last part of string
	LXVD2X	(R7)(R0), V4_	// Load 16 bytes of string
	LXVD2X	(R7)(R16), V5_	// Load next 16
	LXVD2X	(R7)(R21), V6_	// Load next 16
	LXVD2X	(R7)(R9), V7_	// Load last bytes of string
	VPERM	V5, V5, SWAP, V5 // Fix endian order
	VPERM	V7, V7, SWAP, V7
	VPERM	V4, V4, SWAP, V2
	VPERM	V6, V6, SWAP, V6
	VSLO	V7, V9, V7	// Left justify last part of string
index49next:
	VCMPEQUD V0, V4, V8	// Compare first 16 bytes
	VCMPEQUD V1, V5, V9	// Compare next 16 bytes
	VCMPEQUD V2, V6, V10	// Compare next 16 bytes
	VCMPEQUD V3, V7, V11	// Compare last bytes
	VAND	V8, V9, V12	// Combine the compare result
	VAND	V10, V11, V13	// Combine the compare result
	VAND	V12, V13, V14	// Combine the combines
	VCMPEQUDCC V14, ONES, V16	// Check if all ones
	BLT	CR6, found	// All equal means found
	ADD	$1, R7		// Bump string pointer by 1 byte
	CMP	R7, LASTSTR	// and try again
	BLE	index49to64loop
notfound:
