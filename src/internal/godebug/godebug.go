...
func (s *Setting) Value() string {
	s.once.Do(func() {
		...
		vp := cache.values[s.name]
		if vp == nil {
			if cache.values == nil {
				cache.values = make(map[string]*atomic.Pointer[string])
			}
			vp = new(atomic.Pointer[string])
			vp.Store(&empty)
			cache.values[s.name] = vp
		}
		s.value = vp
	})
	return *s.value.Load()
...
func update(def, env string) {
	...
	for name, vp := range cache.values {
		if !did[name] {
			vp.Store(&empty)
		}
	}
	...
	for i := end - 1; i >= -1; i-- {
		if i == -1 || s[i] == ',' {
			if eq >= 0 {
				...
				if !did[name] {
					did[name] = true
					vp := cache.values[name]
					if vp == nil {
						vp = new(atomic.Pointer[string])
						cache.values[name] = vp
					}
					vp.Store(&value)
				}
			}
