...
func NewCoverageMetaDataBuilder(pkgpath string, pkgname string, modulepath string, pkclass coverage.PkgClassification) (*CoverageMetaDataBuilder, error) {
	if pkgpath == "" {
		return nil, fmt.Errorf("invalid empty package path")
	}
	if pkclass < coverage.PkgStdlib || pkclass > coverage.PkgNoMod {
