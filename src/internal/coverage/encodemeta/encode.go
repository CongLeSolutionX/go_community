...
func NewCoverageMetaDataBuilder(pkgpath string, modulepath string, pkclass coverage.PkgClassification) (*CoverageMetaDataBuilder, error) {
	if pkgpath == "" {
...
func (b *CoverageMetaDataBuilder) Emit(w io.WriteSeeker) error {
	...
	b.wrUint32(w, 0)
	// Write packagepath and modulepath (string table indices).
	b.wrUint32(w, uint32(b.pkgpath))
	b.wrUint32(w, uint32(b.modpath))
