...
func (m *CoverageMetaFileWriter) Write(finalHash [16]byte, modname string, blobs [][]byte, mode coverage.CounterMode, granularity coverage.CounterGranularity) error {
	mhsz := uint64(unsafe.Sizeof(coverage.MetaFileHeader{}))
	modnameStrIdx := m.stab.Lookup(modname)
	stSize := m.stab.Size()
