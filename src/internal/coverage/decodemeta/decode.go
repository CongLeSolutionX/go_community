...
func NewCoverageMetaDataDecoder(b []byte, readonly bool) *CoverageMetaDataDecoder {
	x := &CoverageMetaDataDecoder{
		r:    slicereader.NewReader(b, readonly),
		size: uint32(len(b)),
		tmp:  make([]byte, 0, 256),
	}
	x.readHeader()
...
func (d *CoverageMetaDataDecoder) readStringTable() {
	...
	d.strtab = make([]string, 0, d.nfiles)
	for idx := uint32(0); idx < d.nfiles; idx++ {
		len := d.r.ReadULEB128()
		d.strtab = append(d.strtab, d.r.ReadString(int64(len)))
	}
}

func (d *CoverageMetaDataDecoder) PackagePath() string {
	return d.strtab[d.pkgpath]
}
...
func (d *CoverageMetaDataDecoder) ReadFunc(fidx uint32, f *coverage.FuncDesc) error {
	...
	f.Srcfile = d.strtab[fileidx]
	f.Funcname = d.strtab[fnameidx]
