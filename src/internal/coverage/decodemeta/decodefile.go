...
func (r *CoverageMetaFileReader) readFileHeader() error {
	...
	// Read string table.
	r.strtab = make([]string, 0, r.strtabents)
	...
	sr := slicereader.NewReader(b, false)
	for idx := uint32(0); idx < r.strtabents; idx++ {
		slen := sr.ReadULEB128()
		r.strtab = append(r.strtab, sr.ReadString(int64(slen)))
	}
