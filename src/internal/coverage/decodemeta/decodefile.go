...
// top-level info (counter mode, Go module name, number of packages)
// and then a separate self-contained meta-data section for each Go
// package.
...
// ModuleName returns the go.mod name for the instrumented program
// that produced this meta-data file.
func (r *CoverageMetaFileReader) ModuleName() string {
	return r.strtab.Get(r.hdr.ModuleName)
}
