...
//
// Note that if the "go build -cover" takes place in the context
// of a multi-module workspace (https://go.dev/ref/mod#workspaces),
// any package contained in a module that is the target of a "use"
// directive in the go.work file will be considered "main module".
...
const (
	...
	PkgNoMod   // package not in go.mod
	PkgAll     // pseudo-classification intended to capture "all" packages
)
...
func PkgClassificationFromString(s string) PkgClassification {
	switch s {
	case "stdlib":
		return PkgStdlib
	case "depmod":
