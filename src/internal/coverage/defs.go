...
// A counter data file is composed of a file header followed by one
// or more "segments" (each segment representing a given run)
// followed by a footer.
//
// string table, an args section, then a series of entries
...
// segment in a counter data file. Following the segment header will
// be the string table and args table, and then (possibly) padding
// bytes to bring the byte size of the preamble up to a multiple of 4.
// Immediately following that will be the counter payloads.
...
// and stores the number of segments.
