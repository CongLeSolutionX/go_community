...
func (v *ctrVis) VisitFuncs(f encodecounter.CounterVisitorFcn) error {
...
func TestCounterDataFileWriterReader(t *testing.T) {
	...
	for kf, flav := range flavors {
		...
		cdfw := encodecounter.NewCoverageDataFileWriter(of, flav)
		if cdfw == nil {
			t.Fatalf("NewCoverageDataFileWriter failed")
		}
		...
		if false {
			cmd := exec.Command("/usr/bin/od", "--address-radix=d", "-t", "x1", cfpath)
			b, _ := cmd.CombinedOutput()
			fmt.Fprintf(os.Stderr, "=-= od -x of counter file %s:\n%s\n",
				cfpath, b)
		}
...
func TestCounterDataAppendSegment(t *testing.T) {
	...
	ctrs := []uint32{}
	q := uint32(0)
	var cdfw *encodecounter.CoverageDataFileWriter
	for idx := 0; idx < numSegments; idx++ {
		...
		if idx == 0 {
			// Perform the encode and write.
			cdfw = encodecounter.NewCoverageDataFileWriter(of, coverage.CtrRaw)
			if cdfw == nil {
				t.Fatalf("NewCoverageDataFileWriter failed")
			}
