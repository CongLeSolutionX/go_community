...
func TestMetaDataEmptyPackage(t *testing.T) {
	...
	p := "empty/package"
	mp := "m"
	b, err := encodemeta.NewCoverageMetaDataBuilder(p, mp, coverage.PkgNonMod)
	if err != nil {
	...
	pmp := dec.ModulePath()
	if pmp != mp {
		t.Errorf("dec.PackagePath(): got %s want %s", pmp, mp)
	}
}
...
func TestMetaDataEncoderDecoder(t *testing.T) {
	...
	pp := "foopkg"
	mp := "barmod"
	pkcl := coverage.PkgMainMod
	b, err := encodemeta.NewCoverageMetaDataBuilder(pp, mp, pkcl)
	if err != nil {
		t.Fatalf("making builder: %v", err)
	}
	...
	gotpkcl := dec.PackageClassification()
	if gotpkcl != pkcl {
		t.Errorf("pkg classification: got %d want %d", gotpkcl, pkcl)
	}
...
func createBlob(t *testing.T, i int) []byte {
	b, err := encodemeta.NewCoverageMetaDataBuilder("foopkg", "", coverage.PkgStdlib)
	if err != nil {
		t.Fatalf("making builder: %v", err)
	}
...
func TestMetaDataFileWriterReader(t *testing.T) {
	...
	err = mfw.Write(finalHash, "myModule", blobs, coverage.CtrModeAtomic, gran)
	if err != nil {
