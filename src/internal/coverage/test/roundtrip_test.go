...
func TestMetaDataEmptyPackage(t *testing.T) {
	...
	pn := "package"
	mp := "m"
	b, err := encodemeta.NewCoverageMetaDataBuilder(p, pn, mp, coverage.PkgNonMod)
	if err != nil {
...
func TestMetaDataEncoderDecoder(t *testing.T) {
	...
	b, err := encodemeta.NewCoverageMetaDataBuilder(pp, pn, mp, pkcl)
	if err != nil {
		t.Fatalf("making builder: %v", err)
	}
...
func createBlob(t *testing.T, i int) []byte {
	nomodule := ""
	b, err := encodemeta.NewCoverageMetaDataBuilder("foo/pkg", "pkg",
		nomodule, coverage.PkgStdlib)
	if err != nil {
		t.Fatalf("making builder: %v", err)
	}
