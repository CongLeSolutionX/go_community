...
func NewCoverageDataFileWriter(cfname string, f *os.File, args []string) *CoverageDataFileWriter {
...
// previously supplied to NewCoverageDataFileWriter. The file not
...
func (cfw *CoverageDataFileWriter) Write(finalHash [16]byte, visitor CounterVisitor) bool {
	return cfw.writeHeader(finalHash) &&
...
func (cfw *CoverageDataFileWriter) writeHeader(finalHash [16]byte) bool {
...
func (cfw *CoverageDataFileWriter) writeCounters(visitor CounterVisitor) bool {
	...
	emitter := func(pkid uint32, funcid uint32, counters []uint32) bool {
		if !wru32(uint32(len(counters))) || !wru32(pkid) || !wru32(funcid) {
			return false
		}
