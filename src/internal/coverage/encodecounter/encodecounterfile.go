...
func NewCoverageDataFileWriter(w io.Writer, flav coverage.CounterFlavor) *CoverageDataFileWriter {
...
type CounterVisitor interface {
	NumFuncs() (int, error)
	VisitFuncs(f CounterVisitorFcn) error
}

// CounterVisitorFcn describes a callback function invoked when writing
// coverage counter data.
type CounterVisitorFcn func(pkid uint32, funcid uint32, counters []uint32) error

// Write writes the contents of the count-data file to the writer
// previously supplied to NewCoverageDataFileWriter. Returns an error
// if something went wrong somewhere with the write.
func (cfw *CoverageDataFileWriter) Write(metaFileHash [16]byte, args map[string]string, visitor CounterVisitor) error {
...
func (cfw *CoverageDataFileWriter) writeSegmentPreamble(args map[string]string, visitor CounterVisitor) error {
	...
	csh.StrTabLen = uint32(len(ws.Payload()))
	...
	csh.ArgsLen = uint32(len(ws.Payload())) - csh.StrTabLen
	...
	if err := cfw.writeBytes(ws.Payload()); err != nil {
		return err
	}
	return nil
}
...
func (cfw *CoverageDataFileWriter) AppendSegment(args map[string]string, visitor CounterVisitor) error {
...
func (cfw *CoverageDataFileWriter) writeHeader(metaFileHash [16]byte) error {
	// Emit file header.
...
func (cfw *CoverageDataFileWriter) writeBytes(b []byte) error {
	if len(b) == 0 {
		return nil
	}
...
func (cfw *CoverageDataFileWriter) writeCounters(visitor CounterVisitor) error {
	// Notes:
...
func (cfw *CoverageDataFileWriter) writeFooter() error {
	cfw.segs++
