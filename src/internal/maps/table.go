...
func (t *table) rehash() {
	...
	// TODO(prattmic): it is unclear how to make rehash in place work with iteration.
	//if t.capacity > abi.SwissMapGroupSlots && t.tombstones() >= t.capacity/3 {
	//	t.rehashInPlace()
	//	return
	//}
	...
	for i := uint64(0); i < t.groups.length; i++ {
		...
		for j := uint32(0); j < abi.SwissMapGroupSlots; j++ {
			...
			default:
				...
				panic("ctrl should be empty or deleted")
			}
		}
	}
