...
func slotSize(typ *abi.SwissMapType) (uintptr, uintptr) {
	...
	elemSize := typ.Elem.Size_
	if elemSize == 0 {
		// slot is a non-zero sized struct that ends in a
		// zero-sized field. We add an extra byte of padding,
		// to ensure that taking the address of the final
		// zero-sized field can't manufacture a pointer to the
		// next object in the heap. See issue 9401.
		//
		// TODO: Specialize the type to avoid this wasted space?
		// struct{} is a common map element.
		elemSize++
	}

	// Align elem size up to key alignment to account for padding before
	// next slot.
	elemSize = alignUp(elemSize, uintptr(typ.Key.Align_))
...
func groupSize(typ *abi.SwissMapType) uintptr {
	...
	return ctrlGroupsSize + abi.SwissMapGroupSlots*slotSize
}
