...
// as for the packages transitively imported by the files or packages in pkgs.
func WriteImportcfg(t testing.TB, dstPath string, packageFiles map[string]string, pkgs ...string) {
	...
	if len(pkgs) > 0 {
		...
		cmd := Command(t, GoToolPath(t), "list", "-export", "-e", "-deps", "-f", `{{if ne .ImportPath "command-line-arguments"}}{{if .Export}}{{.ImportPath}}={{.Export}}{{end}}{{end}}`)
