...
func (ht *HashTrieMap[K, V]) CompareAndDelete(key K, old V) (deleted bool) {
	...
	i, hashShift, slot, n := ht.compare(key, hash)
	if n == nil {
		if i != nil {
			i.mu.Unlock()
		}
...
func (ht *HashTrieMap[K, V]) compare(key K, hash uintptr) (i *indirect[K, V], hashShift uint, slot *atomic.Pointer[node[K, V]], n *node[K, V]) {
	for {
