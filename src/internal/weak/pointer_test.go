...
func TestPointerFinalizer(t *testing.T) {
	...
	runtime.SetFinalizer(bt, func(bt *T) {
		if wt.Strong() == nil {
			t.Errorf("weak pointer went nil before finalizer ran")
		}
		done <- struct{}{}
	})
	// bt is no longer referenced.
	...
	runtime.GC()
	if wt.Strong() == nil {
		t.Errorf("weak pointer went nil after just one GC")
	}

	// Wait for the finalizer to run.
	<-done

	// Finally clean up bt and make sure the weak pointer goes down with it.
	runtime.GC()
	if wt.Strong() != nil {
