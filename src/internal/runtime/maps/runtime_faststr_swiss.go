...
func (m *Map) getWithoutKeySmallFastStr(typ *abi.SwissMapType, key string) unsafe.Pointer {
	...
	if len(key) < 32 {
		// String equality is quick - just check all 8 entries.
		for range abi.SwissMapGroupSlots {
			// Unlike fast{32,64}, check presence before doing equality because
			// presence check is relatively faster.
			if ctrls&(1<<7) == 0 && key == *(*string)(slotKey) {
	...
	// String equality might be expensive. Do a quick check first.
