...
//
// A group holds abi.SwissMapGroupSlots slots (key/elem pairs) plus their control word.
type group struct {
	typ *abi.SwissMapType

	// data points to the group, which has layout:
	//
	// type typ.Group struct {
	// 	ctrls ctrlGroup
	// 	slots [groupSize]slot
	// XXX: TODO: add placeholder word as a slot target for 0-zero
	// key+elem, which slots will be size 0. key() and elem() will
	// otherwise generate a pointer beyond the end of this type.
	// }
	//
	// type slot struct {
	// 	key  typ.Key
...
type groups struct {
	...
	// lengthMask is the number of groups in data minus (note that length
	// must be a power of two). This allows computing i%length quickly
	// using bitwise AND.
	lengthMask uint64
}
