...
func (t *table) getWithKey(typ *abi.SwissMapType, hash uintptr, key unsafe.Pointer) (unsafe.Pointer, unsafe.Pointer, bool) {
	...
	// to call Equal many times. This likelyhood can be analyzed as follows
	...
	for ; ; seq = seq.next() {
		...
		for match != 0 {
			...
			if t.typ.Key.Equal(key, slotKey) {
				...
				// XXX: For indirect key/elem, clear pointer not value
				...
				typedmemclr(t.typ.Elem, slotElem)
...
func (it *Iter) Next() {
	...
	if it.globalDepth != it.m.globalDepth {
		...
		//
		// Multiplication is assosciative across modulo operations,
		// A * (B % C) = (A * B) % (A * C),
	...
	for ; it.dirIdx < it.m.dirLen; {
		...
		for ; it.groupIdx <= it.tab.groups.lengthMask; it.groupIdx++ {
			g := it.tab.groups.group(it.m.typ, (it.groupIdx + it.groupSlotOffset) & it.tab.groups.lengthMask)
