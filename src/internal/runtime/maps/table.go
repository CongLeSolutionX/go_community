...
type table struct {
	...
	// `groups.length*abi.SwissMapGroupSlots`.
	capacity uint64
...
func (t *table) PutSlot(key unsafe.Pointer) unsafe.Pointer {
	...
	for ; ; seq = seq.next() {
		...
		if match != 0 {
			...
			if t.growthLeft != 0 {
				panic("invariant failed: growthLeft is unexpectedly non-zero")
			}
...
func (it *Iter) Next() {
	...
	for ; it.groupIdx < it.groups.length; it.groupIdx++ {
		g := it.groups.group((it.groupIdx + it.offset) % it.groups.length)
