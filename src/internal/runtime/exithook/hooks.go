...
//
// If an exit hook panics, Run will panic but will also have removed the hooks
// that it ran from the list, so that the panic's exit will be able to call Run
// to run the other hooks and exit.
//
// If an exit hook invokes exit in the same goroutine, the goroutine will panic.
//
