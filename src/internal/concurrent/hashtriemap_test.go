...
func testHashTrieMap(t *testing.T, newMap func() *HashTrieMap[string, int]) {
	...
	t.Run("DeleteMultiple", func(t *testing.T) {
		...
		m.Delete(testData[1])
		m.Delete(testData[105])
		m.Delete(testData[6])
		m.Delete(testData[85])
		for i, s := range testData {
...
func dumpMap[K comparable, V any](ht *HashTrieMap[K, V]) {
	dumpNode(ht, &ht.root.node, 0)
}

func dumpNode[K comparable, V any](ht *HashTrieMap[K, V], n *node[K, V], depth int) {
