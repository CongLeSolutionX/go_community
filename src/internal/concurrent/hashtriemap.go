...
type HashTrieMap[K comparable, V any] struct {
	root     *indirect[K, V]
	keyHash  func(unsafe.Pointer, uintptr) uintptr
	keyEqual func(unsafe.Pointer, unsafe.Pointer) bool
	seed     uintptr
}
...
func (ht *HashTrieMap[K, V]) LoadOrStore(key K, value V) (result V, loaded bool) {
	...
	var n *node[K, V]
	for {
		// Find the value or a candidate location for insertion.
		i = ht.root
		hashShift = 8 * goarch.PtrSize
		for hashShift != 0 {
			hashShift -= nChildrenLog2
...
// loadOrStoreLocked completes a LoadOrStore operation on i which must be locked. The caller
// must have confirmed that the target slot in i contains an entry node.
...
func (ht *HashTrieMap[K, V]) Delete(key K) {
	...
	e, ok := n.entry().tryDelete(key, ht.keyEqual)
	if !ok {
...
func (e *entry[K, V]) lookup(key K, equal func(unsafe.Pointer, unsafe.Pointer) bool) (V, bool) {
	for e != nil {
...
// tryDelete tries to removs the key from the overflow chain. Returns the new entry chain
// and whether or not anything was deleted.
//
// tryDelete must be called under the mutex of the indirect node which e is a child of.
func (head *entry[K, V]) tryDelete(key K, equal func(unsafe.Pointer, unsafe.Pointer) bool) (*entry[K, V], bool) {
	if equal(unsafe.Pointer(&head.key), abi.NoEscape(unsafe.Pointer(&key))) {
