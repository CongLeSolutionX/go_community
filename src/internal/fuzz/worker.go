...
// shared memory. It runs for a maxium of 1 minute.
func (w *worker) minimize(ctx context.Context) (retErr error) {
	...
	defer func() {
		...
		w.memMu <- mem
		w.stop()
		// TODO(jayconrod,katiehockman): w.waitErr is often nil by this point,
		// so this isn't always a particularly helpful error message.
		message := fmt.Sprintf("fuzzing process terminated unexpectedly: %v", w.waitErr)
		// Even if minimization was cancelled due to an interrupt, still report
		// the last crasher that we found to the coordinator.
	...
	for rem := time.Until(minimizeDeadline); rem > 0; {
		...
		err := w.client.minimize(ctx, args)
		if err == nil {
