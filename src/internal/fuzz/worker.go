...
func (ws *workerServer) fuzz(ctx context.Context, args fuzzArgs) (resp fuzzResponse) {
	...
	for {
		...
		default:
			...
			newCov := coverage()
			if len(newCov) != len(ws.coverageData) {
				panic(fmt.Sprintf("num edges changed at runtime: %d, expected %d", len(newCov), len(ws.coverageData)))
			}
			for i := range newCov {
				if ws.coverageData[i] == 0 && newCov[i] > ws.coverageData[i] {
					// This run hit a new edge.
