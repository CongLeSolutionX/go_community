...
func (w *worker) runFuzzing() error {
	...
	var inputC chan CorpusEntry     // set non-nil when able to process input.
	fuzzC := make(chan struct{}, 1) // sent when we finish processing an input.
...
func (w *worker) start() (err error) {
	...
	cmd.Dir = w.dir
	cmd.Env = w.env[:len(w.env):len(w.env)]
