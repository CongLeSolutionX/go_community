...
//
// coordinate loops until ctx is canceled or a fatal error is encountered. While
...
func (w *worker) coordinate(ctx context.Context) error {
	...
	if err := w.client.ping(ctx); err != nil {
		w.stop()
		return fmt.Errorf("fuzzing process terminated without fuzzing")
	...
	for {
		...
		case <-w.termC:
			...
			if exitErr, ok := w.waitErr.(*exec.ExitError); ok && exitErr.ExitCode() == workerExitCode {
				return fmt.Errorf("fuzzing process exited unexpectedly due to an internal failure")
				// TODO(jayconrod,katiehockman): record and return stderr.
			}
