	...
	for {
		...
		case result := <-c.resultC:
			...
			if result.crasherMsg != "" {
				...
				doUpdate := true
				cov := c.readCoverage()
				foundNew := expandsCoverage(cov, result.coverageData, doUpdate)
				if foundNew && !c.coverageOnlyRun() {
					...
					if c.covOnlyInputs == 0 {
						...
						// to 0), and unblock inputC to start fuzzing.
...
func newCoordinator(opts CoordinateFuzzingOpts) (*coordinator, error) {
	...
	if len(corpus.entries) == 0 {
		interestingCount = 0 // there won't be a coverage-only run
		var vals []interface{}
	...
	c := &coordinator{
		...
		interestingCount: int64(interestingCount),
	}
...
func (c *coordinator) logStats() {
	...
	if c.coverageOnlyRun() {
		fmt.Fprintf(c.opts.Log, "gathering baseline coverage, elapsed: %.1fs, workers: %d\n", elapsed.Seconds(), c.opts.Parallel)
	} else {
...
func (c *coordinator) nextInput() (fuzzInput, bool) {
	...
	input.doFuzz = true // this input should be fuzzed
...
func (c *coordinator) readCoverage() []byte {
	c.coverageDataMu.Lock()
	defer c.coverageDataMu.Unlock()
	ret := make([]byte, len(c.coverageData))
	copy(ret, c.coverageData)
	return ret
}

func (c *coordinator) writeCoverage(newCoverage []byte) {
	c.coverageDataMu.Lock()
	defer c.coverageDataMu.Unlock()
	c.coverageData = newCoverage
}
