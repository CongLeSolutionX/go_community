			...
			if result.crasherMsg != "" {
				...
				if c.warmupRun() {
					...
					if c.canMinimize() && result.canMinimize && crashMinimizing == nil {
						...
						if opts.CacheDir != "" {
							entryPath, err := writeToCorpus(result.entry.Data, opts.CacheDir)
							if err != nil {
								stop(err)
							}
							result.entry.Data = nil
							result.entry.Path = entryPath
							result.entry.Name = filepath.Base(entryPath)
						}
...
type CorpusEntry = struct {
	...
	// Path is the path of the corpus file, if the entry was loaded from the
	// seed corpus. For entries added with f.Add and entries generated by the
	// mutator, Path is empty and Data is populated.
	Path string
...
func ReadCorpus(dir string, types []reflect.Type) ([]CorpusEntry, error) {
	...
	for _, file := range files {
		...
		corpus = append(corpus, CorpusEntry{Path: filename, Name: file.Name(), Values: vals})
	}
	if len(errs) > 0 {
...
// file's path, or an error if it failed.
