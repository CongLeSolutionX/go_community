	...
	for {
		...
		case result := <-c.resultC:
			...
			if inputC == nil && !stopping {
				// inputC was disabled, but we have more work to do, so re-enable it.
...
type fuzzResult struct {
	...
	// countRequested is the number of values to coordinator asked the worker
...
func (c *coordinator) nextInput() (fuzzInput, bool) {
	...
	if c.countRequested > 0 {
		...
		if n > c.countRequested-c.count-c.countWaiting {
			n = c.countRequested - c.count - c.countWaiting
		}
		c.countWaiting += n
	}
