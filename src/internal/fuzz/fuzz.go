...
func (c *coordinator) nextInput() (fuzzInput, bool) {
	...
	if c.opts.Count > 0 {
		workerCountRequested = c.opts.Count / int64(c.opts.Parallel)
		if c.opts.Count%int64(c.opts.Parallel) > 0 {
			workerCountRequested++
		}
		if workerCountRequested > c.opts.Count-c.count-c.countWaiting {
			workerCountRequested = c.opts.Count - c.count - c.countWaiting
		}
