...
func (c *coordinator) nextInput() (fuzzInput, bool) {
	...
	var workerCountRequested int64
	if c.countRequested > 0 {
		workerCountRequested = c.countRequested / int64(c.parallel)
		if c.countRequested%int64(c.parallel) > 0 {
			workerCountRequested++
		}
		if workerCountRequested > c.countRequested-c.count-c.countWaiting {
			workerCountRequested = c.countRequested - c.count - c.countWaiting
		}
		c.countWaiting += workerCountRequested
	}
	return fuzzInput{entry: e, countRequested: workerCountRequested}, true
}
