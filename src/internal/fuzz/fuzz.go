...
// same environment as the coordinator process.
//
...
// Additional seed values are read from the corpus automatically and should not
...
func CoordinateFuzzing(parallel int, seed [][]byte) error {
	...
	// to GOFUZZCACHE. If the testdata directory is in the module cache, only
	// write to GOFUZZCACHE.
}
