...
// in testdata.
//
// type is the list of types which make up a corpus entry.
//
...
// filesystem. Each type of error is saved, along with all of the files which
// failed with that error. The testing framework uses this to report a command
// to re-run that specific input.
...
func ReadCorpus(dir string, types []reflect.Type) ([]CorpusEntry, error) {
	...
	for _, file := range files {
		...
		if err != nil {
			errs[err] = append(errs[err], filename)
			continue
		}
		if len(vals) != len(types) {
			err := fmt.Errorf("wrong number of values in corpus file")
			errs[err] = append(errs[err], filename)
			continue
		}
		for i := range types {
			if reflect.TypeOf(vals[i]) != types[i] {
				err := fmt.Errorf("mismatched types in corpus file, want %v", types)
				errs[err] = append(errs[err], filename)
				continue
			}
		}
