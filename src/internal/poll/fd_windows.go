...
func (fd *FD) WriteMsg(buf []byte, oob []byte, sa syscall.Sockaddr) (int, int, error) {
	if err := fd.writeLock(); err != nil {
		return 0, 0, err
	}
	defer fd.writeUnlock()
	...
	for len(buf) > 0 {
		...
		ntotal += int(n)
		oobtotal += int(o.msg.Control.Len)
		if err != nil {
			return ntotal, oobtotal, err
		}
		buf = buf[n:]
	}
	return ntotal, oobtotal, nil
}
