...
// pipePool caches pipes to avoid high frequency creations and destructions of pipe buffers,
// the garbage collector will free all pipes in the sync.Pool, thus we need to set up
// the Finalizer for each pipe to close the its file descriptors before the actual GC.
var pipePool = sync.Pool{New: func() interface{} {
	// Discard the error which occurred during the creation of pipe buffer,
	// redirecting the data transmission to the conventional way utilizing read() + write() as a fallback.
	if p, _, err := newPipe(); err == nil {
		// Assign the destroyPipe to each pipe, which will be executed in a separate goroutine
		// when the GC finds this pipe unreachable, then detach the Finalizer from pipe and free it
		// at the next GC time.
...
func getPipe() (*pipe, string, error) {
...
func putPipe(p *pipe) {
	if p.data != 0 {
		// Clear the Finalizer before we destroy this pipe.
...
// newPipe sets up a temporary pipe for a splice operation.
func newPipe() (pp *pipe, sc string, err error) {
	...
	if err := syscall.Pipe2(fds[:], flags); err != nil {
		return nil, "pipe2", err
	}

	pp = new(pipe)
	pp.rfd, pp.wfd = fds[0], fds[1]
