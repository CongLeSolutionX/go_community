...
func checkFileIsNotPartOfNetpoll(f *os.File) error {
	...
	if !found {
		return fmt.Errorf("%v fd=%v: is not found in the log\n", f.Name(), f.Fd())
	}
	if lfd.FD.IsPartOfNetpoll() {
		return fmt.Errorf("%v fd=%v: is part of netpoll, but should not be (logged: net=%v err=%v)\n", f.Name(), f.Fd(), lfd.Net, lfd.Err)
	}
	return nil
}
...
func testSerialFdsAreInitialised(t *testing.T, name string) {
...
func TestSerialFdsAreInitialised(t *testing.T) {
	for _, name := range []string{"COM1", "COM2", "COM3", "COM4"} {
		t.Run(name, func(t *testing.T) {
			testSerialFdsAreInitialised(t, name)
		})
	}
}
