...
// copy_file_range(2). dst and src must refer to regular files.
func CopyFileRange(dst, src *FD, remain int64) (written int64, handled bool, err error) {
	...
	for remain > 0 {
		...
		n, errno := copyFileRange(dst, src, int(max))
		switch errno {
		case syscall.ENOSYS:
...
func copyFileRange(dst, src *FD, max int) (written int64, err error) {
	...
	//
	// Note that in the call to unix.CopyFileRange below, we use NULL
