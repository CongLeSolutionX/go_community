...
func checkPipes(fds [][2]int) bool {
	for _, fd := range fds {
		...
		_, _, errno1 := syscall.Syscall(unix.FcntlSyscall, uintptr(fd[0]), syscall.F_GETFD, 0)
		_, _, errno2 := syscall.Syscall(unix.FcntlSyscall, uintptr(fd[1]), syscall.F_GETFD, 0)
