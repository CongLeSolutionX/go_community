...
func GetDriveType(pathName *uint16) (driveType uint32) {
	r0, _, _ := syscall.Syscall(procGetDriveTypeW.Addr(), 1, uintptr(unsafe.Pointer(pathName)), 0, 0)
	driveType = uint32(r0)
	return
}

func GetVolumePathName(pathName *uint16, volumeName *uint16, bufLen uint32) (err error) {
	r1, _, e1 := syscall.Syscall(procGetVolumePathNameW.Addr(), 3, uintptr(unsafe.Pointer(pathName)), uintptr(unsafe.Pointer(volumeName)), uintptr(bufLen))
	if r1 == 0 {
		if e1 != 0 {
			err = errnoErr(e1)
		} else {
