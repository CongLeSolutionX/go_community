...
func SysctlNameToMIB(name []byte) ([]uint32, error) {
	...
	_, _, err := syscall.Syscall6(syscall.SYS___SYSCTL, uintptr(unsafe.Pointer(&oid[0])), uintptr(len(oid)), uintptr(unsafe.Pointer(&mib[0])), uintptr(unsafe.Pointer(&miblen)), uintptr(unsafe.Pointer(&name[0])), uintptr(len(name)))
	if err != 0 {
		return nil, err
	}
	if miblen == 0 {
