...
func TestSupportSockNonblockCloexec(t *testing.T) {
	...
	if got && (major < 11 || (major == 11 && minor < 4)) {
		t.Fatalf("SupportSockNonblockCloexec is true, but kernel version is older than 11.4, kernel: %d.%d", major, minor)
	}
	if !got && (major > 11 || (major == 11 && minor >= 4)) {
		t.Errorf("SupportSockNonblockCloexec is false, but kernel version is newer than 11.4, kernel: %d.%d", major, minor)
	}
}
