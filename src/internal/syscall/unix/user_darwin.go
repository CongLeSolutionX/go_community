...
func libc_malloc_trampoline()

func Malloc(n uintptr) (unsafe.Pointer, error) {
	p, _, errno := syscall_syscallPtr(abi.FuncPCABI0(libc_malloc_trampoline), n, 0, 0)
	if errno != 0 {
		return nil, errno
	}
...
//go:cgo_import_dynamic libc_realloc realloc "/usr/lib/libSystem.B.dylib"
func libc_realloc_trampoline()

func Realloc(p unsafe.Pointer, n uintptr) (unsafe.Pointer, error) {
	up, _, errno := syscall_syscallPtr(abi.FuncPCABI0(libc_realloc_trampoline), uintptr(p), n, 0)
	if errno != 0 {
		return nil, errno
	}
...
func libc_free_trampoline()

func Free(p unsafe.Pointer) {
	syscall_syscallPtr(abi.FuncPCABI0(libc_free_trampoline), uintptr(p), 0, 0)
}
