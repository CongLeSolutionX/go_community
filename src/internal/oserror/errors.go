...
func IsTimeout(err wrapper) bool {
	for err != nil {
		if err == ErrTimeout {
			return true
		}
		...
		if x, ok := err.(interface{ Is(wrapper) bool }); ok && x.Is(ErrTimeout) {
			return true
		}
		err = err.Unwrap()
	}
...
func IsTemporary(err wrapper) bool {
	for err != nil {
		if err == ErrTemporary {
			return true
		}
		...
		if x, ok := err.(interface{ Is(wrapper) bool }); ok && x.Is(ErrTemporary) {
			return true
		}
		err = err.Unwrap()
	}
