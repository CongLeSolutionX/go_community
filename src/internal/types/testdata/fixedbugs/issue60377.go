...
// The type parameter P is not used in interface Tm.
// Tm is a defined parameterized interface type which
// can be assigned to any other interface with the same
// methods. Therefore we cannot infer a type argument
// in this case because any type would do.

type Tm[P any] interface{ m() }
...
// This is similar to the first example but here Tm is a component
...
// supply one), then to infer the wrong type in other cases (see
// below). Finally, if we decide to accept go.dev/issues/8082,
// the behavior here is correct.

func g5[P any](func(Tm[P])) {}

func _() {
	var f func(Tm[int])
	g5 /* ERROR "cannot infer P" */ (f)
	g5[int](f)
	g5[string](f /* ERROR "cannot use f (variable of type func(Tm[int])) as func(Tm[string]) value in argument to g5[string]" */)
}
...
// Choosing the wrong would cause the second argument to not match.
