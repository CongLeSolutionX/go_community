...
// var _ = variadic(1) // ERR not enough arguments
...
func _() {
	...
	fboth(both)
	fboth(recv /* ERR cannot use */ )
	fboth(send /* ERR cannot use */ )

	frecv(both)
	frecv(recv)
	frecv(send /* ERR cannot use */ )

	fsend(both)
	fsend(recv /* ERR cannot use */)
	fsend(send)
}
...
func _() {
	...
	ffboth(both)
	ffboth(recv /* ERR cannot use */ )
	ffboth(send /* ERR cannot use */ )

	ffrecv(both /* ERR cannot use */ )
	ffrecv(recv)
	ffrecv(send /* ERR cannot use */ )

	ffsend(both /* ERR cannot use */ )
	ffsend(recv /* ERR cannot use */ )
	ffsend(send)
}
...
func _() {
	...
	g3(& /* ERR does not match */ s1, "1", myString("2"), "3")
	_ = s1
...
func h[] /* ERR empty type parameter list */ () {}

func _() {
	h /* ERR cannot index */ [] /* ERR operand */ ()
}

// Generic functions must have a function body.

func _ /* ERR generic function is missing function body */ [P any]()
