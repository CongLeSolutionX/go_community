...
func _[T any](ch T) {
	close(ch /* ERR cannot close non-channel */)
}
...
func _[T C2](ch T) {
	close(ch /* ERR cannot close receive-only channel */)
}
...
func _[T any](x, y T) {
	copy(x /* ERR copy expects slice arguments */ , y)
}

func _[T ~[]byte](x, y T) {
	copy(x, y)
	copy(x, "foo")
	copy("foo" /* ERR expects slice arguments */ , y)
	...
	type myByte byte
	var x3 []myByte
	copy(x3 /* ERR different element types */ , y)
	copy(y /* ERR different element types */ , x3)
}

func _[T ~[]E, E any](x T, y []E) {
	copy(x, y)
	copy(x /* ERR different element types */ , "foo")
}

func _[T ~string](x []byte, y T) {
	copy(x, y)
	copy(y /* ERR expects slice arguments */ , x)
}

func _[T ~[]byte|~string](x T, y []byte) {
	copy(x /* ERR expects slice arguments */ , y)
	copy(y, x)
}
...
func _[T M3](m T) {
	delete(m /* ERR must have identical key types */, "foo")
}
...
func _[
	...
	_ = make /* ERR expects 2 or 3 arguments */ (S1)
	_ = make(S1, 10, 20)
	_ = make /* ERR expects 2 or 3 arguments */ (S1, 10, 20, 30)
	_ = make(S2 /* ERR cannot make S2: no core type */ , 10)
	...
	_ = make(M1)
	_ = make(M1, 10)
	_ = make/* ERR expects 1 or 2 arguments */(M1, 10, 20)
	_ = make(M2 /* ERR cannot make M2: no core type */ )
	...
	_ = make(C1)
	_ = make(C1, 10)
	_ = make/* ERR expects 1 or 2 arguments */(C1, 10, 20)
	_ = make(C2 /* ERR cannot make C2: no core type */ )
	_ = make(C3)
}
...
func _[T comparable]() {
	...
	assert(bb == 8)
	const _ = unsafe /* ERR not constant */ .Alignof(a)
	const _ = unsafe /* ERR not constant */ .Alignof(s)
	...
	assert(mm == 8)
	const _ = unsafe /* ERR not constant */ .Alignof(t)
}
