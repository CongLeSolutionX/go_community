...
// When N <= 0, if no earlier error was seen reading from R,
// Read returns Err. If Err is nil, it returns EOF instead.
...
func (l *LimitedReader) Read(p []byte) (n int, err error) {
	...
	// Cases when reading an extra byte, which is only when isSentinel:
	//   n < l.N:
	...
	//   n > l.N, err == nil:
	//     return n and sentinel, return sentinel if called again
	//   n > l.N, err != nil:
	//     return n and err, return err if called again
	if extraByte {
		if int64(n) == l.N {
			if err != nil {
				l.limitErr = err
