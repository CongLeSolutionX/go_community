...
var emptySubexpIndices = map[string]int{"": 0, "foo": -1}
...
var subexpCases = []subexpCase{
	...
	{`(?P<foo>.*)(?P<bar>(a)b)(?P<foo>.*)a`, 4, []string{"", "foo", "bar", "", "foo"}, map[string]int{"": 0, "foo": 1, "bar": 2, "other": -1}},
}
...
func TestSubexp(t *testing.T) {
	for _, c := range subexpCases {
		...
		if c.indices != nil {
			for subexpName, expected := range c.indices {
				index := re.SubexpIndex(subexpName)
				if index != expected {
					t.Errorf("%q: SubexpIndex(%q) = %d, want %d", c.input, subexpName, index, expected)
				}
			}
		}
	}
