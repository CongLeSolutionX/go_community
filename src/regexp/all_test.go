...
func BenchmarkFindSubmatch(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if string(subs[0]) != expectedSubs {
			b.Fatalf("unexpected match: %q. Expecting %q on s=%q", subs[0], expectedSubs, s)
		}
		if string(subs[1]) != "aab" {
			b.Fatalf("unexpected submatch: %q. Expecting %q on s=%q", subs[1], "aab", s)
		}
	}
}
