...
func Fields(s string) []string {
	...
	if setBits < utf8.RuneSelf { // ASCII fast path
		...
		for i < len(s) && int(asciiSpace>>s[i])&1 != 0 {
			i++
		}
		fieldStart = i
		for i < len(s) {
			if int(asciiSpace>>s[i])&1 == 0 {
				i++
				continue
			}
			a[na] = s[fieldStart:i]
	...
	return fieldsFuncSplit(s, unicode.IsSpace, n)
}
...
func FieldsFunc(s string, f func(rune) bool) []string {
	...
	return fieldsFuncSplit(s, f, n)
}

// fieldsFuncSplit is like FieldsFunc but takes an initial estimated
// guess n for the number of splits.
func fieldsFuncSplit(s string, f func(rune) bool, n int) []string {
	// Now create them.
	a := make([]string, 0, n)
	na := 0
