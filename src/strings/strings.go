...
func FieldsFunc(s string, f func(rune) bool) []string {
	fromIndex := make([]int, 0, 16)
	toIndex := make([]int, 0, 16)
	...
	for i, rune := range s {
		if f(rune) {
			if wasField {
				toIndex = append(toIndex, i)
				wasField = false
			}
		} else {
			if !wasField {
				fromIndex = append(fromIndex, i)
				wasField = true
			}
		}
	}
	if len(fromIndex) > len(toIndex) { // Last field might end at EOF.
		toIndex = append(toIndex, len(s))
	}

	a := make([]string, len(fromIndex))
	for i := range fromIndex {
		a[i] = s[fromIndex[i]:toIndex[i]]
	}
	return a
}
