...
func Repeat(s string, count int) string {
	...
	case ' ', '-', '0', '=', '\t':
		switch {
		case HasPrefix(repeatedSpaces, s) && n < len(repeatedSpaces):
			return repeatedSpaces[:n]
		case HasPrefix(repeatedDashes, s) && n < len(repeatedDashes):
			return repeatedDashes[:n]
		case HasPrefix(repeatedZeroes, s) && n < len(repeatedZeroes):
			return repeatedZeroes[:n]
		case HasPrefix(repeatedEquals, s) && n < len(repeatedEquals):
			return repeatedEquals[:n]
		case HasPrefix(repeatedTabs, s) && n < len(repeatedTabs):
