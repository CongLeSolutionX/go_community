...
type abiStep struct {
	...
	ireg   int     // integer register offset, used if kind == abiStepIntReg or kind == abiStepPointer
	freg   int     // FP register offset, used if kind == abiStepFloatReg
}
...
//
// An abiSeq should be populated by calling its extend method.
type abiSeq struct {
	// steps is the set of instructions.
	//
	// The instructions are grouped together by whole argumennts,
	// with the starting index for the instructions
	// of the i'th Go value available in valueStart.
	//
	// For instance, if this ABI represents 3 arguments
...
// extend extends the abiSeq with a new Go value of type t.
//
// If the reciever was stack-assigned, returns the single
// abiStep describing that translation, and nil otherwise.
func (a *abiSeq) extend(t *rtype) *abiStep {
	pStart := len(a.steps)
...
// extendRcvr extends the abiSeq with a new method call
// reciever according to the interface calling convention.
//
// If the reciever was stack-assigned, returns the single
// abiStep describing that translation, and nil otherwise.
func (a *abiSeq) extendRcvr(rcvr *rtype) *abiStep {
	// The reciever is always one word.
...
func (a *abiSeq) regAssign(t *rtype, offset uintptr) bool {
	...
	case Interface:
		return a.assignIntN(offset, ptrSize, 2, 0b11)
	case Slice:
...
//
// Should not be called directly; use extend instead.
...
type abiDesc struct {
	...
	// for the call. stackArgSize is the amount of space
	// reserved for arguments. retOffset is the offset
	// at which return values begin, and
	// spill is additional space reserved in
	// reflectcall's frame such that if a preemption
	// occurs during the call, arguments in registers
	// may be spilled to the stack.
	stackArgSize, retOffset, spill uintptr
...
func (a *abiDesc) dump() {
	...
	println("ret")
	a.ret.dump()
	println("stackArgSize", a.stackArgSize)
...
func newAbiDesc(t *funcType, rcvr *rtype) abiDesc {
	...
	// the stackArgSize and retOffset.
	stackArgSize := in.stackBytes
