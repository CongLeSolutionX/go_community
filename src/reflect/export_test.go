...
func FuncLayout(t Type, rcvr Type) (frametype Type, argSize, retOffset uintptr, stack []byte, gc []byte, ptrs bool) {
	...
	var s *bitVector
	if rcvr != nil {
		ft, argSize, retOffset, s, _, _, _ = funcLayout((*funcType)(unsafe.Pointer(t.(*rtype))), rcvr.(*rtype))
	} else {
		ft, argSize, retOffset, s, _, _, _ = funcLayout((*funcType)(unsafe.Pointer(t.(*rtype))), nil)
	}
	frametype = ft
