...
func ArrayOf(count int, elem Type) Type {
	...
	// ArrayOf also calls cacheGet/cachePut and thus may modify the sate of the
	// lookupCache mutex.
	slice := SliceOf(elem)
	...
	*array = *prototype
	...
	array.ptrToThis = nil
	zero := ValueOf(make([]byte, array.size))
	array.zero = unsafe.Pointer(((*SliceHeader)(unsafe.Pointer(zero.Pointer()))).Data)
	...
	esize := etyp.Size()
	ealg := etyp.alg
	arrAt := func(ptr unsafe.Pointer, idx int, esize uintptr) unsafe.Pointer {
		return unsafe.Pointer(uintptr(ptr) + uintptr(idx)*esize)
	}
