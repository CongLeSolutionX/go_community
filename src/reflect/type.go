...
func (n name) nameLen() int {
	return int(uint16(*n.data(1, "1-byte"))<<8 | uint16(*n.data(2, "1-byte")))
}

func (n name) tagLen() int {
	if *n.data(0, "1-byte")&(1<<1) == 0 {
		return 0
	}
	off := 3 + n.nameLen()
	return int(uint16(*n.data(off, "1-byte"))<<8 | uint16(*n.data(off+1, "1-byte")))
}
...
func (n name) pkgPath() string {
	if n.bytes == nil || *n.data(0, "1-byte")&(1<<2) == 0 {
		return ""
	}
	off := 3 + n.nameLen()
	if tl := n.tagLen(); tl > 0 {
	...
	var nameOff int32
	copy((*[4]byte)(unsafe.Pointer(&nameOff))[:], (*[4]byte)(unsafe.Pointer(n.data(off, "4-byte")))[:])
...
// For example, if the result of the addition is going to be dereferenced as
// a 1-byte value, then "1-byte" would be a good reason to pass as whySafe.
