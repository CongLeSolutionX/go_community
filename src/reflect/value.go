...
func extractRegParam1(regs *cpu.RegArgState, a abiParam, t *rtype, p *unsafe.Pointer) {
	...
	switch t.Kind() {
	case UnsafePointer, Ptr, Chan, Map, Func:
		*(*uintptr)(v) = uintptr(regs.Ints[a.iReg])
	case Int, Uint:
	...
	default:
		panic("unexpected kind in struct field value extraction")
	}
}
...
func (v Value) call(op string, in []Value) []Value {
	...
	for i, v := range in {
		...
		// TODO(mknyszek): We lost scratch space for assignTo when
		// we moved to the register ABI. We can get it back for
		// stack-allocated args if we precompute what's stack allocated
		// and what isn't.
		...
		if abi := abiIn[i+abiOffset]; !abi.stackAllocated() {
			...
			// the frame so that it can spill into it if
			// TODO(mknyszek): Remove this when we no longer have
			// compiler-generated spill space.
			ri, rf := regsForType(v.typ)
			frameSize = (frameSize + 7) &^ 7
			frameSize += uintptr(ri) * ptrSize
			frameSize += uintptr(rf) * 8
			continue
		}
