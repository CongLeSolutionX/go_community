...
func (v Value) call(op string, in []Value) []Value {
		...
		for _, st := range abi.call.stepsForValue(i + inStart) {
			...
			case abiStepStack:
				...
				if v.flag&flagIndir != 0 {
					...
					// This case must be a pointer type, in which case
					// this copy shouldn't be partial anyway.
	...
	// Call.
	call(frametype, fn, stackArgs, uint32(frametype.size), uint32(abi.retOffset), uint32(frameSize), unsafe.Pointer(&regArgs))
	...
	if nout == 0 {
		...
		for i := 0; i < nout; i++ {
			...
			case UnsafePointer, Ptr, Chan, Map, Func:
				...
				ret[i] = Value{tv.common(), regArgs.ReturnPtrs[steps[0].ireg], flag(t.Kind())}
				continue
			}
			...
			for _, st := range steps {
				...
				case abiStepPointer:
					...
					*((*unsafe.Pointer)(s)) = regArgs.ReturnPtrs[st.ireg]
				case abiStepFloatReg:
					offset := add(s, st.offset, "precomputed value offset")
...
//
// regs contains the argument values passed in registers. outRegs
// will contain the return results passed in registers.
func callReflect(ctxt *makeFuncImpl, frame unsafe.Pointer, retValid *bool, regs abi.RegArgs) (outRegs abi.RegArgs) {
	ftyp := ctxt.ftyp
	f := ctxt.fn

	_, _, abi := funcLayout(t, rcvrtype)
	...
	for i, typ := range ftyp.in() {
		...
		if st := steps[0]; st.kind == abiStepStack {
			if ifaceIndir(typ) {
				...
				if typ.size > 0 {
					typedmemmove(typ, v.ptr, add(ptr, st.stackOffset, "typ.size > 0"))
				}
				v.flag |= flagIndir
			} else {
				v.ptr = *(*unsafe.Pointer)(add(ptr, st.stackOffset, "1-ptr"))
			}
		} else {
			// Handle pointers passed in registers.
			switch typ.Kind() {
			case UnsafePointer, Ptr, Chan, Map, Func:
				...
				v.ptr = regs.ReturnPtrs[steps[0].ireg]
				continue
			}
			...
			for _, st := range steps {
				...
				case abiStepPointer:
					...
					*((*unsafe.Pointer)(s)) = regs.ReturnPtrs[st.ireg]
				case abiStepFloatReg:
					offset := add(v.ptr, st.offset, "precomputed value offset")
	...
	if numOut > 0 {
		...
		stepsLoop:
			for _, st := range abi.call.stepsForValue(i) {
				switch st.kind {
				case abiStepStack:
					// Copy values to the "stack."
					addr := add(stackArgs, st.stkOff, "precomputed stack arg offset")
				...
				case abiStepIntReg, abiStepPointer:
					...
					if v.flag&flagIndir != 0 {
						offset := add(v.ptr, st.offset, "precomputed value offset")
						memmove(unsafe.Pointer(&outRegs.Ints[st.ireg]), offset, st.size)
					} else {
						...
						outRegs.Ints[st.ireg] = uintptr(v.ptr)
					}
				case abiStepFloatReg:
					// Copy values to "float registers."
					if v.flag&flagIndir == 0 {
					...
					offset := add(v.ptr, st.offset, "precomputed value offset")
					memmove(unsafe.Pointer(&outRegs.Floats[st.freg]), offset, st.size)
				default:
					panic("unknown ABI part kind")
				}
			}
...
// regs contains the argument values passed in registers. outRegs
// will contain the return results passed in registers.
func callMethod(ctxt *methodValue, frame unsafe.Pointer, retValid *bool, regs abi.RegArgs) (outRegs abi.RegArgs) {
	rcvr := ctxt.rcvr
	...
	// in a register or on the stack).
	argOffset := uintptr(0)
	if st := abi.call.steps[0]; st.kind == abiStepStack {
	...
	// argOffset is either 0 or ptrSize, and so should be sufficiently
	// aligned for any type today. Let's defensively align to the next
	// stack argument (if there is one) to avoid issues if one day we
	// have types with greater alignments.
	// Skip the first step because it's always going to be for the
	// reciever.
	for _, st := range abi.call.steps[1:] {
	...
	call(frametype, fn, scratch, uint32(frametype.size), uint32(retOffset), uint32(frameSize), unsafe.Pointer(&regs))

	// Copy the result registers to the returned space.
	outRegs = regs
...
// at stackArgs and register arguments laid out in regArgs. regArgs has
// type *abi.RegArgs. frameSize is the total amount of stack space that
// will be reserved by call, so this should include enough space to spill
// register arguments to the stack in case of preemption.
//
// After fn returns, reflectcall copies stackArgsSize-stackRetOffset
// result bytes back into stackArgs+stackRetOffset before returning, for
// any return values passed on the stack. Register-based return values will
// be found in the same regArgs structure.
//
// regArgs must also be prepared with an appropriate ReturnPtrs bitmap
