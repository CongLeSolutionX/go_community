...
// slice between the length and the capacity.
//
// It panics if v's Kind is not Slice or if n is negative or too large to
// allocate the memory.
...
func (v Value) grow(n int) {
	...
	case p.Len+n > p.Cap:
		t := v.typ.Elem().(*rtype)
		*p = growslice(t, *p, p.Len+n)
	}
}

// extendSlice extends a slice by n elements.
//
// Unlike grow, which only modifies the capacity of the slice in place,
// extend returns a new slice header value with the length incremented by
// the number of specified elements.
func (v Value) extendSlice(n int) Value {
	v.mustBeExported()
	v.mustBe(Slice)

	// Shallow copy the receiver to avoid mutating the source slice.
	s := *(*unsafeheader.Slice)(v.ptr)
...
//go:noescape
func growslice(t *rtype, old unsafeheader.Slice, cap int) unsafeheader.Slice
