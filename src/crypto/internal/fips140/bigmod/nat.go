...
// SetUint64 assigns x = y, and returns an error if y >= m.
//
// The output will be resized to the size of m and overwritten.
...
//
// Moduli can leak the exact number of bits needed to store their value, and are
// stored without padding. Their actual value is still kept secret.
type Modulus struct {
