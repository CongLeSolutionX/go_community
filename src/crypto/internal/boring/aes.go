...
// These wrappers allocate outLen on the C stack, and check that it matches the expected
// value, to avoid having to pass a pointer from Go, which would escape to the heap.

int EVP_AEAD_CTX_seal_wrapper(const GO_EVP_AEAD_CTX *ctx, uint8_t *out,
							  size_t exp_out_len, size_t max_out_len,
							  const uint8_t *nonce, size_t nonce_len,
							  const uint8_t *in, size_t in_len,
							  const uint8_t *ad, size_t ad_len) {
	size_t out_len;
	int ok = _goboringcrypto_EVP_AEAD_CTX_seal(ctx, out, &out_len, max_out_len,
		nonce, nonce_len, in, in_len, ad, ad_len);
	if (ok == 0) return ok;
	if (out_len != exp_out_len) return 0;
	return 1;
};
...
int EVP_AEAD_CTX_open_wrapper(const GO_EVP_AEAD_CTX *ctx, uint8_t *out,
							  ...
							  const uint8_t *ad, size_t ad_len) {
	size_t out_len;
	int ok = _goboringcrypto_EVP_AEAD_CTX_open(ctx, out, &out_len, max_out_len,
		nonce, nonce_len, in, in_len, ad, ad_len);
	if (ok == 0) return ok;
	if (out_len != exp_out_len) return 0;
	return 1;
...
func (g *aesGCM) Seal(dst, nonce, plaintext, additionalData []byte) []byte {
	...
	ok := C.EVP_AEAD_CTX_seal_wrapper(
		&g.ctx,
		(*C.uint8_t)(unsafe.Pointer(&dst[n])), outLen, outLen,
