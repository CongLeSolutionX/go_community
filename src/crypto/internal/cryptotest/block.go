...
func testCipher(t *testing.T, cipher func(dst, src []byte), blockSize int) {
	...
	t.Run("Aliasing", func(t *testing.T) {
		...
		rng.Read(buff)
		cipher(expectedOutput[:blockSize], buff[:blockSize])

		// Check that the same output is generated when src=dst alias to the same
		// memory
		cipher(buff[:blockSize], buff[:blockSize])
		if !bytes.Equal(buff[:blockSize], expectedOutput[:blockSize]) {
			t.Errorf("block cipher produced different output when dst = src; got %x, want %x", buff[:blockSize], expectedOutput[:blockSize])
		}
	})
