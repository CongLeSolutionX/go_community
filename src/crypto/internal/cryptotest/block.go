...
func testCipher(t *testing.T, rng io.Reader, cipher func(dst, src []byte), blockSize int) {
	...
	t.Run("OutOfBoundsRead", func(t *testing.T) {
		...
		src = make([]byte, blockSize*3) // We will pad src on either side with data
		rng.Read(src[:blockSize])
		rng.Read(src[blockSize*2:])
		testDst := make([]byte, blockSize)
		cipher(testDst, src[blockSize:blockSize*2])
