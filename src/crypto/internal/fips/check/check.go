...
// put writes x as a big-endian uint64 to b.
// The FIPS object file format uses numbers in this form.
func put(b *[8]byte, x uint64) {
	b[0] = byte(x >> 56)
	b[1] = byte(x >> 48)
	b[2] = byte(x >> 40)
	b[3] = byte(x >> 32)
...
func init() {
	...
	if linkinfo.Magic[0] != 0xff || string(linkinfo.Magic[1:]) != fipsMagic {
		println(string(linkinfo.Magic[:]))
