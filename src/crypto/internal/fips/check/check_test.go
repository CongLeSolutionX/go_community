...
// Note: This file is named aaa_test.go because we need its func init
// to run before the func init in check.go, and init funcs run in source
// order, with the source sorted by file name.
...
func TestInfo(t *testing.T) {
	...
	sect := func(i int, name string, p unsafe.Pointer) {
		s := Linkinfo.Sects[i]
		if !(s.Start <= uintptr(p) && uintptr(p) < s.End) {
