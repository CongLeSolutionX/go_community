...
func NewPrivateKey(N []byte, e uint, d, P, Q, dP, dQ, qInv []byte) (*PrivateKey, error) {
	...
	if dP == nil {
		panic("TODO(filippo): implement CRT computation")
	}
	if dQ == nil {
		panic("TODO(filippo): implement CRT computation")
	}
	var qInvN *bigmod.Nat
	if qInv == nil {
		panic("TODO(filippo): implement CRT computation")
	} else {
...
func NewPrivateKeyWithoutCRT(N []byte, e uint, d []byte) (*PrivateKey, error) {
	n, err := bigmod.NewModulus(N)
	if err != nil {
		return nil, err
	}
...
func encrypt(pub *PublicKey, plaintext []byte) ([]byte, error) {
	...
	return bigmod.NewNat().ExpShortVarTime(m, pub.E, pub.N).Bytes(pub.N), nil
}
...
func decrypt(priv *PrivateKey, ciphertext []byte, check bool) ([]byte, error) {
	...
	if check {
		c1 := bigmod.NewNat().ExpShortVarTime(m, E, N)
		if c1.Equal(c) != 1 {
			return nil, ErrDecryption
		}
	}
