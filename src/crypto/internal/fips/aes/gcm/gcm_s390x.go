...
// (useGHASH) uses the KM instruction to encrypt using AES in counter mode
// (via aes.CTR) and the KIMD instruction for GHASH. The second implementation
// (useGCM) uses the newer KMA instruction which performs both operations (but
// still requires KIMD to hash large nonces).

var useGHASH = cpu.S390X.HasAES && cpu.S390X.HasGHASH
