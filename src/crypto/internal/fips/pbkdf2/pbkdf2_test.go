...
func TestWithHMACSHA1(t *testing.T) {
	// Note: SHA1 is intentionally not in crypto/internal/fips, or a fips.Hash. We use it
	//       here only for test purposes.
	testHash(t, func() fips.Hash { return sha1.New() }, "SHA1", sha1TestVectors)
}
...
func BenchmarkHMACSHA1(b *testing.B) {
	// Note: SHA1 is intentionally not in crypto/internal/fips, or a fips.Hash. We use it
	//       here only for test purposes.
	benchmark(b, func() fips.Hash { return sha1.New() })
}
