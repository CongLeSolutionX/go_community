...
//
// This tool should be built in a FIPS enabled mode, both with/without
// CPU features disabled to provide test coverage for algorithms with/without
// processor algorithm accelerators (PAA). Testing must be done per operating
// environment (OE).
//
// TODO(@cpu): Provide more concrete advice for ^ when stablized.
//
...
//	$> go build ./crypto/internal/acvpwrapper/main.go
//
//	$> # From BoringSSL root:
...
func writeResponse(writer *bufio.Writer, args [][]byte) error {
	...
	// TODO(XXX): perhaps should be deferred to flush cmd?
	return writer.Flush()
}
