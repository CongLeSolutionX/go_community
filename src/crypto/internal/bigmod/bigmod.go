...
func (i *Int) Equal(k *Int) bool {
	return i.nat.cmpEq(k.nat) == 1
}
...
// Mul sets n = x * y, and returns n. All Ints must share the same modulus.
func (n *Int) Mul(x, y *Int) *Int {
	d, a, b, m, m0inv := n.nat, x.nat, y.nat, n.m.nat, n.m.m0inv
	if d == a || d == b { // TODO
	...
	copy(n.nat.limbs, d.limbs)
	return n
}

// Add sets n = a + b, and returns n. All Ints must share the same modulus.
func (n *Int) Add(a, b *Int) *Int {
	x, y, m := a.nat, b.nat, n.m.nat
	if a.nat != n.nat { // TODO
		x = n.nat.assign(yes, a.nat)
	}
...
// Sub sets n = a - b, and returns n. All Ints must share the same modulus.
...
func (i *Int) Exp(e []byte) *Int {
	...
	for _, b := range e {
		for _, j := range []int{4, 0} {
			...
			t1.Mul(i, t0)
			i.nat.assign(not(ctEq(k, 0)), t1.nat)
		}
	}
