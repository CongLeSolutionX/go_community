...
func (x *Nat) shiftIn(y uint, m *Modulus) *Nat {
	...
	for i := _W - 1; i >= 0; i-- {
		...
		// Like in lightReduce, we need the subtraction if either it didn't
		// underflow (meaning 2x + b > m) or if computing 2x + b overflowed
		// (meaning 2x + b > 2^_W*n > m).
...
// lightReduce computes x -= m if and only if x >= m or if "always" is yes.
//
// always is usually a carry that indicates that the operation that produced x
// overflowed its size, meaning its abstract value is > 2^_W*n > m.
//
// Both operands must have the same announced length.
func (x *Nat) lightReduce(always choice, m *Modulus) {
	t := NewNat().set(x)
...
func (x *Nat) Add(y *Nat, m *Modulus) *Nat {
	overflow := x.add(y)
	x.lightReduce(choice(overflow), m)
	return x
}
...
func (x *Nat) montgomeryMul(a *Nat, b *Nat, m *Modulus) *Nat {
	...
	default:
		...
		// if necessary (which as explained in lightReduce can be the case both
		// if x >= m, or if x overflowed).
		//
