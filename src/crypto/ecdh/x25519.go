...
func (c *x25519Curve) NewPrivateKey(key []byte) (*PrivateKey, error) {
	...
	x25519ScalarMult(k.publicKey, k.privateKey, x25519Basepoint[:])
...
var x25519ZeroOutput = make([]byte, x25519SharedSecretSize)
...
func (c *x25519Curve) ECDH(local *PrivateKey, remote *PublicKey) ([]byte, error) {
	...
	if subtle.ConstantTimeCompare(out, x25519ZeroOutput) == 1 {
