...
//go:noescape
func kdsaSig(uint64, *[1720]byte) (uint64, uint64)
...
func preconditions(sv signverify, c elliptic.Curve, bo *bufferoffsets) bool {
	...
	case 521:
		...
		if sv == signing {
			bo.functionCode = 11
			return false //not supporting Sign 521 for now
		} else {
			bo.functionCode = 3
			return true
		}
...
func sign(priv *PrivateKey, csprng *cipher.StreamReader, c elliptic.Curve, e *big.Int) (r, s *big.Int, err error) {
	...
	if preconditions(signing, c, &bo) && e.Sign() != 0 {
		...
		for {
			...
			success, errn := kdsaSig(bo.functionCode, &buffer)
			if errn == 1 {
