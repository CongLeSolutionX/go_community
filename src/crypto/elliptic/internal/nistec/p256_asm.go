...
// P256Point is a P-256 point in Jacobian coordinates (X:Y:Z) where TODO. The
// point at infinity can be represented by any set of coordinates with Z = 0.
...
func NewP256Point() *P256Point {
	return &P256Point{
		x: p256One, y: p256One, z: p256Element{},
	}
}
...
func (p *P256Point) SetBytes(b []byte) (*P256Point, error) {
	// p256Mul operates in a Montgomery domain with R = 2^256 mod p, where p
	// is the underlying field of the curve. Thus rr here is R×R mod p. See
	// comment in Inverse about how this is used.
...
// at an index offset of -1, so 8P is at index 7, P is at 0, and 16P is at 15.
...
// Montgomery multiplication modulo P. Sets res = in1 * in2 * R⁻¹.
//go:noescape
func p256Mul(res, in1, in2 *p256Element)

// Montgomery square modulo P, repeated n times (n >= 1).
//go:noescape
func p256Sqr(res, in *p256Element, n int)

// Montgomery multiplication by 1 (out of the Montgomery domain).
// Sets res = in * R⁻¹, bringing res out of the Montgomery domain.
//go:noescape
func p256FromMont(res, in *p256Element)

// If cond is not 0, sets val = -val modulo P.
//go:noescape
...
// TODO: document and change table type
//go:noescape
...
func P256OrdInverse(k []byte) ([]byte, error) {
	...
	// Montgomery multiplication by one converts a Montgomery value out of the
	// domain.
...
func (p *P256Point) p256BaseMult(scalar *p256OrdElement) {
	...
	p256SelectBase(&t0, p256Precomputed, sel)
	...
	for i := 1; i < 43; i++ {
		...
		index += 6
		sel, sign = boothW6(uint(wvalue))
		p256SelectBase(&t0, p256Precomputed[i*32*8*8:], sel)
