...
var p256B, _ = new(p256Element).SetBytes([]byte{0x5a, 0xc6, 0x35, 0xd8, 0xaa, 0x3a, 0x93, 0xe7, 0xb3, 0xeb, 0xbd, 0x55, 0x76, 0x98, 0x86, 0xbc, 0x65, 0x1d, 0x6, 0xb0, 0xcc, 0x53, 0xb0, 0xf6, 0x3b, 0xce, 0x3c, 0x3e, 0x27, 0xd2, 0x60, 0x4b})
...
func (p *P256Point) SetBytes(b []byte) (*P256Point, error) {
	...
	case len(b) == 1+p256ElementLength && (b[0] == 2 || b[0] == 3):
		x, err := new(p256Element).SetBytes(b[1:])
		if err != nil {
			return nil, err
		}

		// y² = x³ - 3x + b
		y := p256Polynomial(new(p256Element), x)
		if !p256Sqrt(y, y) {
...
func (p *P256Point) bytes(out *[1 + 2*p256ElementLength]byte) []byte {
	...
	x := new(p256Element).Mul(p.x, zinv)
	y := new(p256Element).Mul(p.y, zinv)
...
func p256Sqrt(e, x *p256Element) (isSquare bool) {
	candidate := new(p256Element)
	p256SqrtCandidate(candidate, x)
	square := new(p256Element).Square(candidate)
	if square.Equal(x) != 1 {
		return false
	}
	e.Set(candidate)
...
func p256SqrtCandidate(z, x *p256Element) {
