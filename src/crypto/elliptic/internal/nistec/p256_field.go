...
func (e *p256Element) bytes(out *[p256ElementLen]byte) []byte {
	// TODO: don't use big.Int to convert out of the Montgomery domain :-/
	return p256ToBig(e).FillBytes(out[:])
}
...
// If v is not 66 bytes or it encodes a value higher than 2^256 - 2^224 + 2^192 + 2^96 - 1,
...
func (e *p256Element) SetBytes(v []byte) (*p256Element, error) {
	...
	// TODO: don't use big.Int to convert into the Montgomery domain :-/
	p256FromBig(e, new(big.Int).SetBytes(v))
	return e, nil
}

// mask64Bits returns 0xffffffff if cond is 1, and 0 otherwise.
func mask64Bits(cond int) uint32 { return ^(uint32(cond) - 1) }
...
var p256P, _ = new(big.Int).SetString("115792089210356248762697446949407573530086143415290314195533631308867097853951", 10)

// p256FromBig sets out = R*in.
func p256FromBig(out *p256Element, in *big.Int) {
...
var p256RInverse, _ = new(big.Int).SetString("7fffffff00000001fffffffe8000000100000000ffffffff0000000180000000", 16)

// p256ToBig returns a *big.Int containing the value of in.
func p256ToBig(in *p256Element) *big.Int {
	result, tmp := new(big.Int), new(big.Int)

	result.SetInt64(int64(in[len(in)-1]))
