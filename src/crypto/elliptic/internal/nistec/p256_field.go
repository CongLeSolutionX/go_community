...
func (e *p256Element) bytes(out *[p256ElementLen]byte) []byte {
	...
	// First, we do a Montgomery multiplication (that calculates a * b * 1/R,
	// since both a and b are expected to be multiplied by R, and we don't want
	// the result to be multiplied by R²) by 1 (not in the Montgomery domain),
	// getting (a * R) * 1 * 1/R = a. Then, we need to carry the limb overflows,
