...
func (curve p256Curve) Inverse(k *big.Int) *big.Int {
	...
	p256OrdMul(table[4*0:], x, RR)          // _1
	p256OrdSqr(x, table[4*0:], 1)           // _10
	p256OrdMul(table[4*1:], x, table[4*0:]) // _11
	p256OrdMul(table[4*2:], x, table[4*1:]) // _101
	p256OrdMul(table[4*3:], x, table[4*2:]) // _111
	p256OrdSqr(x, table[4*2:], 1)           // _1010
	p256OrdMul(table[4*4:], table[4*2:], x) // _1111

	p256OrdSqr(table[4*5:], x, 1)
	p256OrdMul(table[4*5:], table[4*5:], table[4*0:]) // _10101
	p256OrdSqr(x, table[4*5:], 1)                     // _101010
	p256OrdMul(table[4*6:], table[4*2:], x)           // _101111
	p256OrdMul(x, table[4*5:], x)                     // _111111 = x6
	p256OrdSqr(t, x, 2)                               // _11111100
	p256OrdMul(t, t, table[4*1:])                     // _11111111 = x8
	...
	sqrs := []uint8{6, 5, 4, 5, 5, 4, 3, 3, 5, 9, 6, 2, 5, 6, 5, 4, 5, 5, 3, 10, 2, 5, 5, 3, 7, 6}
	indx := []uint8{6, 3, 1, 4, 5, 2, 2, 2, 3, 6, 4, 0, 0, 4, 3, 3, 3, 2, 1, 6, 1, 1, 1, 0, 5, 4}

	for i, s := range sqrs {
		p256OrdSqr(x, x, int(s))
		p256OrdMul(x, x, table[4*indx[i]:])
	}
