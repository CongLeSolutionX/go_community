...
func p256FromBig(out *[p256Limbs]uint32, in *big.Int) {
	...
	// represented as a×R mod p. p256Mul calculates (a × b × R⁻¹) mod p. RR is R in
	// the domain, or R×R mod p, thus p256Mul(x, RR) gives x×R, i.e. converts
...
func p256TightReduce(inout *[p256Limbs]uint32) {
	...
	t[8] += b << 29
	p256CopyConditional(inout, &t, b-1)
}
