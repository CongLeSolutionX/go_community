...
func TestRenegotiationExtension(t *testing.T) {
	...
	if len(buf) < 5+4 {
		t.Fatalf("Server return short message of length %d", len(buf))
	}
	...
	// unmarshal expected to be given the handshake header, but
	...
	if !serverHello.secureRenegotiation {
		t.Fatalf("Secure renegotiation extension was not echoed.")
	}
}
