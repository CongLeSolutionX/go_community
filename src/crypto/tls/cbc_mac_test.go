...
func TestCopySecretSlice(t *testing.T) {
	...
	// bytes, respectively.
	for _, l := range []int{20, 32} {
		// CBC-mode ciphers will call copySecretSlice on 64, 128, 192,
		// 256 (small padded plaintexts unconstrained by the maximum
		// padding), and 256+l (the maximum padding is 256).
		//
		// We also use this codepath for RC4, which is unconstrained by
		// block sizes, so additionally test l and l+1 to capture
		// boundary cases.
		for _, tot := range []int{l, l + 1, 64, 128, 192, 256, 256 + l} {
			b := in[:tot]
			for i := 0; i < tot-l; i++ {
