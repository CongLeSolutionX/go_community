...
func (c *Config) getCertificate(clientHello *ClientHelloInfo) (*Certificate, error) {
	...
	if c.NameToCertificate != nil {
		...
		labels[0] = "*"
		if cert, ok := c.NameToCertificate[strings.Join(labels, ".")]; ok {
			return cert, nil
		}
	}
...
func (chi *ClientHelloInfo) SupportsCertificate(c *Certificate) error {
	...
	// supportsStaticRSA returns nil if the certificate and connection support
	// the static RSA key exchange, and unsupported otherwise. The logic for
	// supporting static RSA is completely disjoint from the logic for
	// supporting signed key exchanges, so we just check it as a fallback.
	supportsStaticRSA := func(unsupported error) error {
		...
		rsaCipherSuite := selectCipherSuite(chi.CipherSuites, chi.config.cipherSuites(), func(c *cipherSuite) bool {
	...
	if !supportsECDHE(config, chi.SupportedCurves, chi.SupportedPoints) {
		return supportsStaticRSA(errors.New("client doesn't support ECDHE, can only use legacy RSA key echange"))
	}
	...
	if priv, ok := c.PrivateKey.(crypto.Signer); ok {
		...
		case *ecdsa.PublicKey:
			...
			case elliptic.P521():
				curve = CurveP521
			default:
				return supportsStaticRSA(unsupportedCertificateError(c))
			}
			var curveOk bool
			for _, c := range chi.SupportedCurves {
				...
				for _, cc := range chi.config.curvePreferences() {
					if c != cc {
						continue
					}
					curveOk = true
		...
		case ed25519.PublicKey:
			if vers < VersionTLS12 || len(chi.SignatureSchemes) == 0 {
				return errors.New("connection does not support Ed25519")
			}
			ecdsaCipherSuite = true
		case *rsa.PublicKey:
		default:
	...
	cipherSuite := selectCipherSuite(chi.CipherSuites, chi.config.cipherSuites(), func(c *cipherSuite) bool {
		if c.flags&suiteECDHE == 0 {
			return false
		}
	...
	if cipherSuite == nil {
		return supportsStaticRSA(errors.New("client doesn't support any cipher suites compatible with the certificate"))
	}
