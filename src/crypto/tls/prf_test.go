...
func TestKeysFromPreMasterSecret(t *testing.T) {
	for i, test := range testKeysFromTests {
		...
		contextKeyingMaterial := hex.EncodeToString(ekm([]byte("label"), []byte("context"), 32))
		noContextKeyingMaterial := hex.EncodeToString(ekm([]byte("label"), nil, 32))
		if contextKeyingMaterial != test.contextKeyingMaterial ||
			noContextKeyingMaterial != test.noContextKeyingMaterial {
