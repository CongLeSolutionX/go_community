...
func (hs *serverHandshakeStateTLS13) processClientHello() error {
	...
	if selectedGroup == x25519Kyber768Draft00 {
		ecdhGroup = X25519
		if len(ecdhData) != 32+mlkem768.EncapsulationKeySize {
			c.sendAlert(alertIllegalParameter)
			return errors.New("tls: invalid Kyber client key share")
		}
		ecdhData = ecdhData[:32]
	}
	...
	if selectedGroup == x25519Kyber768Draft00 {
		ciphertext, kyberShared, err := kyberEncapsulate(clientKeyShare.data[32:])
		if err != nil {
			c.sendAlert(alertIllegalParameter)
			return errors.New("tls: invalid Kyber client key share")
		}
...
func (hs *serverHandshakeStateTLS13) doHelloRetryRequest(selectedGroup CurveID) (*keyShare, error) {
	...
	if ks.group != selectedGroup {
		c.sendAlert(alertIllegalParameter)
		return nil, errors.New("tls: client sent invalid key share in second ClientHello")
	}
