...
func (hs *serverHandshakeStateTLS13) handshake() error {
	...
	// waiting for the client second flight, but the application might not
...
func (hs *serverHandshakeStateTLS13) checkForResumption() (usePSK bool, err error) {
	...
	for i, identity := range hs.clientHello.pskIdentities {
		// Don't let cheap ClientHello messages make the server work too much
		// attempting to decrypt session tickets.
		if i > 4 {
			break
		}

		sessionTicket := append([]byte{}, identity.label...)
		plaintext, _ := c.decryptTicket(sessionTicket)
		if plaintext == nil {
			continue
		}
		...
		if !bytes.Equal(hs.clientHello.pskBinders[i], pskBinder) {
