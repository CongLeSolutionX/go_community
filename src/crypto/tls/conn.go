...
func (c *Conn) writeRecordLocked(typ recordType, data []byte) (int, error) {
	if c.quic != nil {
		...
		if err := c.quicWriteCryptoData(c.out.level, data); err != nil {
			return 0, err
		}
		if !c.buffering {
...
func (c *Conn) readHandshakeBytes(n int) error {
	if c.quic != nil {
		for c.hand.Len() < n {
			// Drop the handshake mutex while waiting for the QUICConn
			// to provide us with more handshake bytes, to allow the
			// user to call ConnectionState before the handshake completes.
			closed := false
			c.handshakeMutex.Unlock()
			select {
			case c.quic.readingc <- struct{}{}:
			case c.quic.readc <- struct{}{}:
...
func (c *Conn) handshakeContext(ctx context.Context) (ret error) {
	...
	if c.quic != nil {
		if c.handshakeErr == nil {
			...
			c.quicSetReadSecret(EncryptionLevelApplication, c.cipherSuite, c.in.trafficSecret)
		} else {
			var a AlertError
			c.out.Lock()
			...
			c.handshakeErr = fmt.Errorf("%w%.0w", c.handshakeErr, a)
		}
		close(c.quic.readc)
		close(c.quic.readingc)
	}
