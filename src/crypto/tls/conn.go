...
type Conn struct {
	...
	input     bytes.Reader // application data waiting to be read
...
func (hc *halfConn) encrypt(record, payload []byte, rand io.Reader) ([]byte, error) {
	...
	if explicitNonceLen := hc.explicitNonceLen(); explicitNonceLen > 0 {
		...
		if _, ok := hc.cipher.(cbcMode); !ok && explicitNonceLen < 16 {
...
func (c *Conn) readRecord(want recordType) error {
	...
	case recordTypeApplicationData:
		...
		// To avoid copying the decrypted data, which still lives on the
		// rawInput buffer, split the buffer in two. The decrypted data becomes
		// c.input, while rawInput gets reinitialized so it doesn't own it anymore.
		c.rawInput = *bytes.NewBuffer(c.rawInput.Bytes())
		c.input.Reset(data)
		return nil
...
func (r *atLeastReader) Read(p []byte) (n int, err error) {
	if r.N <= 0 {
		return 0, io.EOF
	}
	n, err = r.R.Read(p)
...
func (c *Conn) readFromUntil(r io.Reader, n int) error {
	...
	// Use atLeastReader and some extra buffer to make possible extra input
	// waiting on the wire available to be peeked at in (*Conn).Read.
...
func (c *Conn) Read(b []byte) (int, error) {
	...
	for emptyRecordCount := 0; emptyRecordCount <= maxConsecutiveEmptyRecords; emptyRecordCount++ {
		...
		n, _ := c.input.Read(b)
		if c.input.Len() == 0 {
			c.input.Reset(nil) // release the buffer
		}
