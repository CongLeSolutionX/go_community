...
func ExampleConfig_verifyPeerCertificate() {
	...
	// validation. This example shows a VerifyPeerCertificate implementation
	...
	config := &tls.Config{
		...
		// VerifyPeerCertificate can't access that value, so it has to be set
	...
	config.VerifyPeerCertificate = func(certificates [][]byte, verifiedChains [][]*x509.Certificate) error {
		// verifiedChains will always be nil when InsecureSkipVerify is true.
		_ = verifiedChains
		...
		opts := x509.VerifyOptions{
			Roots:         config.RootCAs, // or config.ClientCAs on the server side
			DNSName:       config.ServerName,
			Intermediates: x509.NewCertPool(),
			// The default KeyUsages are appropriate for server certificates
			// checked by a client. On the server side, set them to
