	...
	for !extensions.Empty() {
		...
		case extensionServerName:
			...
			for !nameList.Empty() {
				...
				if nameType == 0 && len(m.serverName) == 0 {
		...
		case extensionSupportedCurves:
			...
			if !extData.ReadUint16LengthPrefixed(&curves) ||
				curves.Empty() || !extData.Empty() {
				return false
			}
			for !curves.Empty() {
				var curve uint16
		...
		case extensionSupportedPoints:
			...
			if !ReadUint8LengthPrefixed(&extData, &m.supportedPoints) ||
				len(m.supportedPoints) == 0 || !extData.Empty() {
				return false
			}
		case extensionSessionTicket:
			// RFC 5077, Section 3.2
			m.ticketSupported = true
			m.sessionTicket = []byte(extData)
		case extensionSignatureAlgorithms:
			// RFC 5246, Section 7.4.1.4.1
			var sigAndAlgs cryptobyte.String
			if !extData.ReadUint16LengthPrefixed(&sigAndAlgs) ||
				sigAndAlgs.Empty() || !extData.Empty() {
				return false
			}
