...
func (q *testQUICConn) HandshakeDone() {
	q.done = true
}
...
func runTestQUICConnection(a, b *testQUICConn) error {
...
func TestQUICConnection(t *testing.T) {
	...
	if err := runTestQUICConnection(cli, srv); err != nil {
...
func TestQUICSessionResumption(t *testing.T) {
	...
	if err := runTestQUICConnection(cli2, srv2); err != nil {
	...
	for _, clientErr := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			for {
				...
				gotErr := runTestQUICConnection(cli, srv)
				if gotErr == wantErr {
					continue
				}
...
func TestQUICPostHandshakeKeyUpdate(t *testing.T) {
	...
	keyUpdateBytes := keyUpdate.marshal()
	if err := cli.conn.HandleCryptoData(EncryptionLevelApplication, append([]byte{
		byte(typeKeyUpdate),
