...
func runTestQUICConnection(a, b *testQUICConn, onHandleCryptoData func()) error {
	if err := a.conn.Start(); err != nil {
		return err
	}
	if err := b.conn.Start(); err != nil {
		return err
	}
	idleCount := 0
	for {
...
func TestQUICConnection(t *testing.T) {
	...
	if err := runTestQUICConnection(cli, srv, nil); err != nil {
...
func TestQUICSessionResumption(t *testing.T) {
	...
	if err := runTestQUICConnection(cli2, srv2, nil); err != nil {
	...
	for _, clientErr := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			for {
				...
				gotErr := runTestQUICConnection(cli, srv, nil)
				if gotErr == wantErr {
					continue
				}
...
func TestQUICHandshakeError(t *testing.T) {
	...
	cli := newTestQUICClient(t, clientConfig)
	srv := newTestQUICServer(t, serverConfig)
	if err := runTestQUICConnection(cli, srv, nil); !errors.Is(err, alertBadCertificate) {
...
func TestQUICConnectionState(t *testing.T) {
	...
	if err := runTestQUICConnection(cli, srv, onHandleCryptoData); err != nil {
		t.Fatalf("error during connection handshake: %v", err)
	}
}
