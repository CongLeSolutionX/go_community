...
//
// The RSA operations in this package are implemented using constant-time
// algorithms, except for key generation.
package rsa
...
func decrypt(priv *PrivateKey, ciphertext []byte) ([]byte, error) {
	...
	// m1 = m1 - m2 mod p
	m1.Sub(bigmod.NewInt(P).SetInt(m2))
	// m1 = m1 * Qinv mod p
	m1.Mul(Qinv)
	// m = m1 * q mod N
	m := bigmod.NewInt(N).SetInt(m1).Mul(QmodN)
	// m = m + m2 mod N
	m.Add(bigmod.NewInt(N).SetInt(m2))
...
func decryptAndCheck(priv *PrivateKey, ciphertext []byte) (m []byte, err error) {
	...
	check, _ := encrypt(&priv.PublicKey, m)
