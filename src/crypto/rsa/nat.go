...
func (x *nat) fillBytes(bytes []byte) []byte {
	...
	// If all of the limbs fit in the buffer bytes, we have nothing left to do
	if fittingLimbs >= len(x.limbs) {
		return bytes
	}
	// Because of how we calculated fittingLimbs, only the last remaining limb
...
// add computes x += y, if on == 1, and does nothing otherwise
//
// Both operands must have the same announced length.
//
