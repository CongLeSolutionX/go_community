...
func (x *nat) shiftIn(y uint, m *modulus) *nat {
	...
	for i := _W - 1; i >= 0; i-- {
		...
		var borrow uint
		for i := range x.limbs {
			l := ctSelect(needSubtraction, s.limbs[i], x.limbs[i])

			res := l<<1 + carry
			x.limbs[i] = res & _MASK
			carry = res >> _W

			res = x.limbs[i] - m.nat.limbs[i] - borrow
			s.limbs[i] = res & _MASK
			borrow = res >> _W
		}
...
// montgomeryMul calculates out = a * b / R mod m, with R = 2^(_W * n) and
...
func (d *nat) montgomeryMul(a *nat, b *nat, m *modulus) *nat { // TODO
	...
	for i := 0; i < len(a.limbs); i++ {
		f := ((d.limbs[0] + a.limbs[i]*b.limbs[0]) * m.m0inv) & _MASK
		carry := uint(0)
		for j := 0; j < len(b.limbs); j++ {
			// z = d[j] + a[i] * b[j] + f * m[j] + carry <= 2^(2W+1) - 2^(W+1) + 2^W
			hi, lo := bits.Mul(a.limbs[i], b.limbs[j])
			z_lo, c := bits.Add(d.limbs[j], lo, 0)
