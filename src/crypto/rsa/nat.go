...
// that the quotient cannot fit in a uint. Instead, constant time selection
// should be used to handle these edge cases where the return value is
// undefined.
...
func modulusFromNat(nat *nat) *modulus {
	...
	size := uint(len(m.nat.limbs))
	for size > 0 && m.nat.limbs[size-1] == 0 {
		size--
	}
...
func (x *nat) shiftIn(y uint, m *modulus) *nat {
	...
	q := ctSelect(ctEq(a1, b0), _MASK, ctSelect(ctEq(rawQ, 0), 0, rawQ-1))
...
//
// The output will be resized to the size of me and overwritten.
