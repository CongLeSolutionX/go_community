...
// makes a Bleichenbacher attack significantly more difficult, the protections
// are only effective if the rest of the protocol which uses them are designed
// with these considerations in mind. In particular, if any subsequent
// operations which use the decrypted session key leak any information about the
// key (e.g. if it is a static or random key) then the mitigations become
// useless against a Bleichenbacher attack. This method must be used extremely
// carefully, and typically should only be used when absolutely necessary for
// compatibiltiy with an existing protocol (such as TLS) that is designed with
