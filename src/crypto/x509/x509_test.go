...
const malformedCRLs = `-----BEGIN CERTIFICATE-----
...
func TestIncorrectCRLDistributionEncoding(t *testing.T) {
	crls := []string{"http://epscd.catcert.net/crl/ec-acc.crl", "http://epscd2.catcert.net/crl/ec-acc.crl"}
	block, rest := pem.Decode([]byte(malformedCRLs))
	if len(rest) != 0 {
		t.Fatalf("unexpected trailing data: %s", rest)
	}
	cert, err := ParseCertificate(block.Bytes)
	if err != nil {
		t.Fatalf("failed to parse certificate: %s", err)
	}
	if len(cert.CRLDistributionPoints) != len(crls) {
		t.Fatalf("incorrect CRL DistributionPoint length. Got %v wanted %v", len(cert.CRLDistributionPoints), len(crls))
	}
	for i, crl := range cert.CRLDistributionPoints {
		if crl != crls[i] {
			t.Errorf("incorrect CRL. Got %s wanted %s", crl, crls[i])
		}
	}
}
