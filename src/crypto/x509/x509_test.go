...
func TestMultipleRDN(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("certificate with two elements in an RDN failed to parse: %s", err)
	}

	if expected := "eportal.mss.edus.si"; cert.Subject.CommonName != expected {
		t.Errorf("expected common name of %q, but got %q", expected, cert.Subject.CommonName)
	}

	if expected := "1236484010010"; cert.Subject.SerialNumber != expected {
		t.Errorf("expected serial number of %q, but got %q", expected, cert.Subject.SerialNumber)
	}
}
