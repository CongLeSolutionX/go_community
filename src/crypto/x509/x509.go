...
func signTBS(signed []byte, key crypto.Signer, sigAlg SignatureAlgorithm, rand io.Reader) ([]byte, error) {
	...
	if err := checkSignature(sigAlg, signed, signature, key.Public(), true); err != nil {
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv any) ([]byte, error) {
	...
	signatureAlgorithm, pkixAlgorithm, err := signingParamsForKey(key, template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
	...
	c := tbsCertificate{
		...
		SignatureAlgorithm: pkixAlgorithm,
...
func (c *Certificate) CreateCRL(rand io.Reader, priv any, revokedCerts []pkix.RevokedCertificate, now, expiry time.Time) (crlBytes []byte, err error) {
	...
	signatureAlgorithm, pkixAlgorithm, err := signingParamsForKey(key, 0)
	if err != nil {
		return nil, err
	}
...
func CreateRevocationList(rand io.Reader, template *RevocationList, issuer *Certificate, priv crypto.Signer) ([]byte, error) {
	...
	signatureAlgorithm, pkixAlgorithm, err := signingParamsForKey(priv, template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
