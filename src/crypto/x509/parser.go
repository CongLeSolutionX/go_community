...
func parseName(raw cryptobyte.String) (*pkix.RDNSequence, error) {
	...
	for !inner.Empty() {
		...
		for !set.Empty() {
			...
			if err != nil {
				return nil, fmt.Errorf("x509: invalid AttributeTypeAndValue: %s", err)
			}
...
func parseExtension(der cryptobyte.String) (pkix.Extension, error) {
	...
	if !der.ReadASN1ObjectIdentifier(&ext.Id) {
		return ext, errors.New("x509: malformed OID") // extnID
	}
	if der.PeekASN1Tag(cbasn1.BOOLEAN) {
		if !der.ReadASN1Boolean(&ext.Critical) {
			return ext, errors.New("x509: malformed critical boolean")
		}
	}
	var val cryptobyte.String
	if !der.ReadASN1(&val, cbasn1.OCTET_STRING) {
		return ext, errors.New("x509: malformed value") // extnValue
	}
	ext.Value = val
	return ext, nil
}
...
func parseCertificate(der []byte) (*Certificate, error) {
	...
	cert.Raw = der[:len(der)-len(input)]
	...
	if !tbs.ReadOptionalASN1Integer(&cert.Version, cbasn1.Tag(0).Constructed().ContextSpecific(), 0) {
		return nil, errors.New("x509: malformed version") // bad version
	}
	if cert.Version < 0 {
	...
	cert.Version++
	if cert.Version > 3 {
		return nil, errors.New("x509: invalid version") // bad version
	}

	serial := new(big.Int)
	if !tbs.ReadASN1Integer(serial) {
		return nil, errors.New("x509: malformed serial number") // bad version
	}
	...
	if !inner.ReadASN1(&outerSigAISeq, cbasn1.SEQUENCE) {
	...
	if !spki.ReadASN1BitString(&spk) {
		return nil, errors.New("x509: malformed subjectPublicKey") // bad subjectPublicKey
	}
