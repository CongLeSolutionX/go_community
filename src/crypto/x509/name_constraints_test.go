...
var nameConstraintsTests = []nameConstraintsTest{
	...
	// #50: trailing dots don't confuse DNS matches
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				ok: []string{"dns:example.com."},
		...
		noOpenSSL: true, // OpenSSL doesn't canonicalise domain names before matching
	},

	// #51: trailing dots don't confuse email matches
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				ok: []string{"email:example.com."},
	...
	// #52: URI constraints only cover the host part of the URI
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
	...
	// #54: URIs with IPs and ports are rejected
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				ok: []string{"uri:example.com"},
	...
	// #55: URIs with IPv6 addresses are also rejected
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				ok: []string{"uri:example.com"},
	...
	// #56: URIs with IPv6 addresses with ports are also rejected
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				ok: []string{"uri:example.com"},
	...
	// #57: URI constraints are effective
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				ok: []string{"uri:example.com"},
	...
	// #58: URI constraints are effective
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
	...
	// #59: URI constraints can allow subdomains
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
	...
	// #60: excluding an IPv4-mapped-IPv6 address also excludes the IPv4
	// address.
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				bad: []string{"ip:::ffff:1.2.3.4/96"},
			},
		},
		intermediates: [][]constraintsSpec{
			[]constraintsSpec{
		...
		expectedError: "\"1.2.3.4\" is excluded",
	},

	// #61: excluding an IPv4 address also excludes the mapped IPv6 form of
	// that address.
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{
				bad: []string{"ip:1.2.3.4/32"},
			},
		},
		intermediates: [][]constraintsSpec{
			[]constraintsSpec{
		...
		leaf: leafSpec{
			sans: []string{"ip:::ffff:1.2.3.4"},
		},
		expectedError: "\"1.2.3.4\" is excluded",
	},

	// #62: omitting extended key usage in a CA certificate implies that
	// any usage is ok.
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{},
	...
	// #63: The “any” EKU also means that any usage is ok.
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{},
		},
	...
	// #65: A specified key usage in an intermediate forbids other usages
	...
	// #67: trying to add extra permitted key usages in an intermediate
	...
	// #69: in order to support COMODO chains, SGC key usages permit
	// serverAuth and clientAuth.
	nameConstraintsTest{
		roots: []constraintsSpec{
			constraintsSpec{},
...
func addConstraintsToTemplate(constraints constraintsSpec, template *Certificate) error {
	parse := func(constraints []string) (dnsNames []string, ips []*net.IPNet, emailAddrs []string, uriDomains []string, unknown bool, err error) {
		for _, constraint := range constraints {
			...
			default:
				return nil, nil, nil, nil, false, fmt.Errorf("unknown constraint %q", constraint)
			}
		}

		return dnsNames, ips, emailAddrs, uriDomains, unknown, err
	}

	var err error
	var unknownPermitted, unknownExcluded bool
	template.PermittedDNSDomains, template.PermittedIPRanges, template.PermittedEmailAddresses, template.PermittedURIDomains, unknownPermitted, err = parse(constraints.ok)
	if err != nil {
		return err
	}

	template.ExcludedDNSDomains, template.ExcludedIPRanges, template.ExcludedEmailAddresses, template.ExcludedURIDomains, unknownExcluded, err = parse(constraints.bad)
	if err != nil {
		return err
	}

	if unknownPermitted || unknownExcluded {
		// A custom extension is needed for this, which will cause any
		// other constraints to be ignored.
		appendUnknown := func(contents []byte, tag uint8) []byte {
			contents = append(contents, tag|32 /* constructed */ |0x80 /* context-specific */)
			contents = append(contents, 5 /* length */)
			contents = append(contents, 0x30 /* SEQUENCE */)
			contents = append(contents, 3 /* length */)
			contents = append(contents, 9 /* undefined GeneralName type nine */)
			return append(contents, 1, 1)
		}

		var contents []byte
		if unknownPermitted {
			contents = appendUnknown(contents, 0 /* tag 0 for permitted */)
		}
		if unknownExcluded {
			contents = appendUnknown(contents, 1 /* tag 1 for excluded */)
		}
...
func TestConstraintCases(t *testing.T) {
	...
	for i, test := range nameConstraintsTests {
		...
		for _, name := range test.leaf.sans {
			switch {
			...
			case strings.HasPrefix(name, "uri:"):
				...
				if err != nil {
					t.Fatalf("#%d: cannot parse URI %q: %s", i, name[4:], err)
				}
		...
		var err error
		if template.ExtKeyUsage, template.UnknownExtKeyUsage, err = parseEKUs(test.leaf.ekus); err != nil {
