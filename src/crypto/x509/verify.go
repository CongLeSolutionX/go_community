...
func domainToReverseLabels(domain string) (reverseLabels []string, ok bool) {
	...
	if len(reverseLabels) > 0 && len(reverseLabels[0]) == 0 {
		// An empty label at the end indicates a global domain and is
		// the same as omitting the label.
		reverseLabels = reverseLabels[1:]
	}
		...
		err := forEachSAN(sanExtension, func(tag int, data []byte) error {
			...
			case nameTypeIP:
				...
				if err := c.checkNameConstraints(&count, maxConstraintComparisons, "IP address", ip.String(), ip,
					func(parsedName, constraint interface{}) (bool, error) {
						return constraint.(*net.IPNet).Contains(parsedName.(net.IP)), nil
