...
func TestTemplateLookUp(t *testing.T) {
	...
	if t1.Lookup("foo") != nil {
		t.Error("expected nil return value for undefined template")
	}
	t1.New("bar")
	...
	if t1.Lookup("foo") == nil {
		t.Error("expected non-nil return values for defined template")
	}
}
...
func TestNew(t *testing.T) {
	...
	if t1.common != t2.common {
		t.Error("expected both templates to share common data structure")
	}
	if t1.Tree == nil {
		t.Error("expected non-nil tree")
	}
	if t2.Tree != nil {
		t.Error("expected template to have empty tree")
	}

	containsT1 := false
	for _, tmpl := range t1.Templates() {
		if tmpl == t2 {
			t.Error("Templates() should not return undefined template")
		}
		if tmpl == t1 {
			containsT1 = true
		}
	...
	if !containsT1 {
		t.Error("Templates() should return defined template")
	}
}
