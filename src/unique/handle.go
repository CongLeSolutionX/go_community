...
func Make[T comparable](value T) Handle[T] {
	...
	for {
		...
		// Delete the value so we can insert a new one.
		m.Delete(value)
	}
	...
	if !loaded {
		...
		cleanupFuncs = append(cleanupFuncs, func() {
			...
			m.Enumerate(func(key T, wp weak.Pointer[T]) bool {
				// Don't call Strong, which might revive the pointer.
				if wp.Nil() {
					m.Delete(key)
				}
				return true
...
func init() {
	// Set up a background goroutine to occasionally clean up the map.
	...
	runtime_registerUniqueMapCleanup(func() {
		// Try to wake the cleanup goroutine.
		select {
		case wait <- struct{}{}:
		default:
		}
