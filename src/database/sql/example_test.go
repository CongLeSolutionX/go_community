...
func ExampleDB_Query() error {
	...
	// If the database is being written to, it ensure to check for Close
	// errors that may be returned from the driver. It could the the database
	// encounted an auto-commit error and was forced to rollback changes.
	rerr := rows.Close()
	if rerr != nil {
		return err
	}
...
func ExampleDB_PingContext() {
	...
	// Set the context with a short timeout. Time taken will only
	// include network resolution and travel time typically.
