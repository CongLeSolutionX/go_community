...
//
// The ErrBadConn should only be returned from Validator, SessionResetter, or
// a query method if the connection is already in an invalid (e.g. closed) state.
//
// All Conn implementations should implement the following interfaces:
// Pinger, SessionResetter, and Validator.
//
// If named parameters or context is supported, the driver's Conn should implement:
// ExecerContext, QueryerContext, ConnPrepareContext, and ConnBeginTx.
//
// To support custom data types implement NamedValueChecker. NamedValueChecker also allows
// queries to accept per query options as a parameter by returning ErrRemoveArgument from
// CheckNamedValue.
//
...
// it should implmeent the following interfaces: RowsColumnTypeScanType, RowsColumnTypeDatabaseTypeName,
// RowsColumnTypeLength, RowsColumnTypeNullable, and RowsColumnTypePrecisionScale.
// A given row value may also return another Rows. This often represents a database cursor value.
//
// Before a connection is returned to the connection pool after use, IsValid is called if supported.
// Before a connection is reused for another query, ResetSession is called if supported. If a connection
// is never returned to the connection pool but immediately reused, then ResetSession is called prior to
