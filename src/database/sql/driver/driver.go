...
//
// If a [Conn] implements [Validator], then the [Validator.Isvalid] method is called
// before returning the connection to the connection pool. If an entry in the
// connection pool implements [SessionResetter], then [SessionResetter.ResetSession]
...
// [SessionResetter.ResetSession] is called prior to reuse but [Validator.IsValid]
// is not called.
package driver
