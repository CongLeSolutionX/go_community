...
//
// Most code should use package [database/sql].
//
...
// [ErrRemoveArgument] from [NamedValueChecker.CheckNamedValue].
//
...
//
// Before a connection is returned to the connection pool after use, [Validator.IsValid] is
// called if implemented. Before a connection is reused for another query,
// [SessionResetter.ResetSession] is called if implemented. If a connection is never returned
// to the connection pool but immediately reused, then ResetSession is called prior to
...
type Driver interface {
	...
	// closed), but doing so is unnecessary; the [database/sql] package
...
// that [Connector.Connect] method to obtain each needed connection,
// instead of invoking the [Driver.Open] method for each connection.
...
// and can create any number of equivalent [Conn]s for use
...
// [DriverContext.OpenConnector] method, to allow drivers
// access to context and to avoid repeated parsing of driver
// configuration.
//
// If a Connector implements [io.Closer], [database/sql.DB.Close]
// method will call [io.Closer.Close] and return error (if any).
type Connector interface {
	...
	// mainly to maintain compatibility with the [database/sql.DB.Driver] method
	// on [database/sql.DB].
	Driver() Driver
}
...
//
// If a Conn does not implement Pinger, the sql package's [database/sql.DB.Ping] and
...
// the sql package's [database/sql.DB.Exec] will first prepare a query, execute the statement,
...
//
// If a Conn does not implement [ExecerContext], the sql package's [database/sql.DB.Exec]
...
// the sql package's [database/sql.DB.Query] will first prepare a query, execute the statement,
// and then close the statement.
//
...
//
// If a Conn does not implement QueryerContext, the sql package's [database/sql.DB.Query]
...
type Stmt interface {
	...
	//
	// If NumInput returns >= 0, the [database/sql] package will sanity check
...
// StmtExecContext enhances the [Stmt] interface by providing [Stmt.Exec] with context.
...
// StmtQueryContext enhances the [Stmt] interface by providing [Stmt.Query] with context.
