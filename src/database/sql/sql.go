...
func (db *DB) Close() error {
	...
	db.freeConn = nil
	db.closed = true
	db.connRequestSet.CloseAndRemoveAll()
	db.mu.Unlock()
	for _, fn := range fns {
		err1 := fn()
		if err1 != nil {
...
func (db *DB) maybeOpenNewConnections() {
	numRequests := db.connRequestSet.Len()
	if db.maxOpen > 0 {
...
func (db *DB) conn(ctx context.Context, strategy connReuseStrategy) (*driverConn, error) {
	...
	if db.maxOpen > 0 && db.numOpen >= db.maxOpen {
		...
		delHandle := db.connRequestSet.Add(req)
		db.waitCount++
		db.mu.Unlock()
		...
		case <-ctx.Done():
			...
			db.mu.Lock()
			deleted := db.connRequestSet.Delete(delHandle)
			db.mu.Unlock()
			...
			if !deleted {
				...
				// oustanding, we should probably block on req (in a new
...
func (db *DB) putConnDBLocked(dc *driverConn, err error) bool {
	...
	if req, ok := db.connRequestSet.TakeRandom(); ok {
		if err == nil {
			dc.inUse = true
		}
		req <- connRequest{
...
//
//   - adding an element
//   - removing the just-added element
...
func (s *connRequestSet) Add(v chan connRequest) connRequestDelHandle {
	...
	// under a scannable-threshold. As an optimization, we could device the *int
...
//
// It reports whether it was deleted. (It can return false if a caller
// to TakeRandom took it meanwhile, or upon the second call to Delete)
...
func (s *connRequestSet) deleteIndex(idx int) {
	...
	*(s.s[idx].curIdx) = -1
	...
	s.s[len(s.s)-1] = connRequestAndIndex{} // zero out final elemnt
	s.s = s.s[:len(s.s)-1]
}

// TakeRandom removes and removes a random element from s
