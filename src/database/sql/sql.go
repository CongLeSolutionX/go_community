...
// Remove closed conns from s.css.
// s.mu should be locked.
func (s *Stmt) removeClosedStmtLocked() {
	dbClosed := atomic.LoadUint64(&s.db.numClosed)
	if dbClosed-s.lastNumClosed > 0 {
		s.db.mu.Lock()
		i, j := 0, len(s.css)-1
		for {
			for i < len(s.css) && !s.css[i].dc.dbmuClosed {
				i++
			}
			for i < j && s.css[j].dc.dbmuClosed {
				j--
			}
			if i >= j { // i > j when i == len(s.css).
				break
			}
			s.css[i] = s.css[j]
			i++
		...
		//fmt.Printf("%d connections collected\n", len(s.css)-i)
		s.db.mu.Unlock()
		s.css = s.css[:i]
