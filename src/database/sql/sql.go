...
type DB struct {
	...
	dsn    string
	// numClosed is an atomic counter represents how many connection was closed.
...
// removeClosedStmtLocked removes closed conns in s.css lazily.
// s.mu should be locked.
func (s *Stmt) removeClosedStmtLocked() {
	// Lazy enough for locking db.mu.
	t := len(s.css)/2 + 1
	if t > 10 {
		t = 10
	}
...
func (s *Stmt) connStmt() (ci *driverConn, releaseConn func(error), si driver.Stmt, err error) {
	...
	// Does the connection have stmt already?
	s.mu.Lock()
	for _, v := range s.css {
		if v.dc == dc {
			s.mu.Unlock()
