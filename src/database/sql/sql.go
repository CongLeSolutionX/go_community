...
// IsolationLevel is the transaction isolation level used in TxOpt.
...
// TxOpt holds the transaction options to be used in DB.BeginTx.
type TxOpt struct {
...
// BeginTx is canceled.
//
// The provided TxOpt is optional and may be nil if defaults should be used.
// If a non-default isolation level is used
...
func (db *DB) BeginTx(ctx context.Context, opts *TxOpt) (*Tx, error) {
	var tx *Tx
	var err error
	for i := 0; i < maxBadConnRetries; i++ {
...
func (db *DB) begin(ctx context.Context, opts *TxOpt, strategy connReuseStrategy) (tx *Tx, err error) {
	dc, err := db.conn(ctx, strategy)
	if err != nil {
		return nil, err
	}
