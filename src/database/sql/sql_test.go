...
// waitForOpen checks db.numOpenConns until either it equals want or
// the maxWait time elapses.
func waitForOpen(t *testing.T, db *DB, maxWait time.Duration, want int) {
	var numOpen int
	if !waitCondition(maxWait, 5*time.Millisecond, func() bool {
		numOpen = db.numOpenConns()
		return numOpen == want
	}) {
		t.Fatalf("open conns = %d; want %d", numOpen, want)
	}
}
...
func TestQueryContextWait(t *testing.T) {
	...
	if prepares := numPrepares(t, db) - prepares0; prepares != 1 {
		...
		// executes this test may fail. After adjusting how the context
		// is canceled just log this failure condition.
...
func TestTxContextWait(t *testing.T) {
	...
	// Checked on DB Close.
	waitForOpen(t, db, 5*time.Second, 0)
}
