...
func TestMkDirAllIntermediateTrailingSpace(t *testing.T) {
	...
	if err != nil && !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("MkdirAll(%q) should have failed with fs.ErrNotExist, but did not", path)
	}
	if err != nil && err.(*fs.PathError).Path != path {
		t.Fatalf("MkdirAll(%q) should have failed with (*fs.PathError).Path matched with path, but did not", path)
	}
}
