...
func removeAll(path string) error {
	...
	access := uint32(syscall.FILE_LIST_DIRECTORY)
	sharemode := uint32(syscall.FILE_SHARE_READ | syscall.FILE_SHARE_WRITE | syscall.FILE_SHARE_DELETE)
	attrs := uint32(syscall.FILE_FLAG_BACKUP_SEMANTICS | syscall.FILE_FLAG_OPEN_REPARSE_POINT)
	parent, err := syscall.CreateFile(parentDirW, access, sharemode, nil, syscall.OPEN_EXISTING, attrs, 0) // check if parent exists
	if err != nil {
		if IsNotExist(err) {
...
func removeAllChildsFrom(parent syscall.Handle, base string) (bool, error) {
	...
	for {
		numErr := 0
		names, _, _, readErr := readdir(&dirinfo, reqSize, readdirName)
...
func removeAllFrom(parent syscall.Handle, base string) error {
	...
	// remove its contents.
	// A permission error means that we don't have write permission on
	// the parent directory, but this entry might still be a directory
	// whose contents need to be removed.
	// Otherwise just return the error.
	if err != syscall.ERROR_DIR_NOT_EMPTY { // && !IsPermission(err) {
	...
	var recurseErr error
	for {
		ok, err := removeAllChildsFrom(parent, base)
		if recurseErr == nil {
			recurseErr = err
		}
		if ok {
	...
	return &PathError{Op: "removeAllFrom", Path: base, Err: err}
}
...
func openFdAt(parent syscall.Handle, name string, access uint32) (syscall.Handle, error) {
	namew, err := syscall.UTF16FromString(name)
	if err != nil {
		return 0, err
	}
	ntPath := windows.UnicodeString{
		Buffer:        &namew[0],
		Length:        uint16(len(namew)*2 - 2),
		MaximumLength: uint16(len(namew) * 2),
	}
	defer runtime.KeepAlive(namew)
