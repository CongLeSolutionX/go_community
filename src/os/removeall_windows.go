...
func removeAllChildsFrom(parent syscall.Handle, base string) (bool, error) {
	h, err := openFdAt(parent, base, syscall.FILE_LIST_DIRECTORY, windows.FILE_DIRECTORY_FILE)
	if err != nil {
		if IsNotExist(err) {
			return true, nil
		}
...
func removeAllFrom(parent syscall.Handle, base string) error {
	child, err := openFdAt(parent, base, windows.DELETE|windows.FILE_READ_ATTRIBUTES, 0)
	if err != nil {
		if IsNotExist(err) {
			return nil
		}
...
func openFdAt(parent syscall.Handle, name string, access, options uint32) (syscall.Handle, error) {
	...
	err = windows.NtCreateFile(&h,
		...
		syscall.FILE_SHARE_READ|syscall.FILE_SHARE_WRITE|syscall.FILE_SHARE_DELETE,
		windows.FILE_OPEN,
		options|windows.FILE_SYNCHRONOUS_IO_NONALERT|syscall.FILE_FLAG_OPEN_REPARSE_POINT,
		0,
		0,
	)
	if err != nil {
