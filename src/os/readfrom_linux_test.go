...
func testSpliceFile(t *testing.T, proto string, size, limit int64) {
	dst, src, data, hook := newSpliceFileTest(t, proto, size)
	...
	if hook.called && hook.srcfd != rfd {
		t.Fatalf("wrong source file descriptor: got %d, want %d", hook.srcfd, rfd)
	}
...
// newSpliceFileTest initializes a new test for copy_file_range.
//
...
func newSpliceFileTest(t *testing.T, proto string, size int64) (*File, net.Conn, []byte, *spliceFileHook) {
	t.Helper()
