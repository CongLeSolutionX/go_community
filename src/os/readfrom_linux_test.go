...
func newCopyFileRangeTest(t *testing.T, size int64) (dst, src *File, data []byte, hook *copyFileRangeHook) {
	...
	tmpdir, err := ioutil.TempDir("", "TestCopyFileRange-")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { RemoveAll(tmpdir) })

	src, err = Create(filepath.Join(tmpdir, "src"))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { src.Close() })

	dst, err = Create(filepath.Join(tmpdir, "dst"))
	if err != nil {
		t.Fatal(err)
	}
