...
func current() (*User, error) {
	...
	// and that GetUserName returns the process owner's username
	// when the current thread is impersonating a different user.
	// See https://go.dev/issue/68647.
	var usr *User
...
func runAsProcessOwner(f func() error) error {
	...
	if !isProcessToken {
		...
		defer func() {
			err = windows.ImpersonateLoggedOnUser(prevToken)
			if err != nil {
				// This should not happen, but if it does, it's better to exit the
				// goroutine and terminate the thread than to leave it running with
				// the wrong token.
				impersonateFailed = true
				println("os/user: terminating goroutine: failed to revert to previous token:", err.Error())
				runtime.Goexit()
			}
		}()
	}
...
func getCurrentToken() (t syscall.Token, isProcessToken bool, err error) {
	...
	const access = syscall.TOKEN_QUERY | syscall.TOKEN_DUPLICATE | syscall.TOKEN_IMPERSONATE
	err = windows.OpenThreadToken(thread, access, true, &t)
