...
func lookupUser(username string) (*User, error) {
	...
	initSize, err := userBuffer.initialSize()
	if err != nil {
...
func lookupGroup(groupname string) (*Group, error) {
	...
	initSize, err := groupBuffer.initialSize()
	if err != nil {
...
var (
	userBuffer  = bufferKind{"_SC_GETPW_R_SIZE_MAX", C._SC_GETPW_R_SIZE_MAX}
	groupBuffer = bufferKind{"_SC_GETGR_R_SIZE_MAX", C._SC_GETGR_R_SIZE_MAX}
)

func (k bufferKind) initialSize() (C.size_t, error) {
	sz := C.sysconf(k.sysconf)
	if sz == -1 {
	...
	if !isSizeReasonable(int64(sz)) {
		return 0, fmt.Errorf("unreasonable %s of %d", k.name, sz)
	}
	return C.size_t(sz), nil
}
...
func alloc(sz C.size_t) *memBuffer {
	return &memBuffer{
		ptr:  C.malloc(sz),
		size: sz,
	}
...
func retryWithBuffer(buf *memBuffer, f func() syscall.Errno) error {
	for {
		...
		if !isSizeReasonable(int64(newSize)) {
			return fmt.Errorf("internal buffer exceeds %d bytes", newSize)
		}
		buf.resize(newSize)
	}
}
