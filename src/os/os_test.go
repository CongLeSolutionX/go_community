...
func TestOpenError(t *testing.T) {
	...
	for _, rooted := range []bool{true, false} {
		for _, tt := range openErrorTests {
			...
			if rooted {
				var rel string
				rel, err = filepath.Rel(sfdir, tt.path)
				if err != nil {
					t.Fatal(err)
				}
				name = fmt.Sprintf("Root(%q).OpenFile(%q, %d)", sfdir, rel, tt.mode)
				f, err = r.OpenFile(rel, tt.mode, 0)
			} else {
				name = fmt.Sprintf("OpenFile(%q, %d)", tt.path, tt.mode)
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			testMaybeRooted(t, func(t *testing.T, r *Root) {
				...
				var err error
				if r == nil {
					f, err = OpenFile(filename, O_RDWR|O_CREATE, test.mode)
				} else {
					f, err = r.OpenFile(filename, O_RDWR|O_CREATE, test.mode)
				}
				if err != nil {
					t.Fatal(err)
				}
				...
				if test.mode&0o444 != 0 {
					...
					if err == nil {
						t.Errorf("ReadFile = <nil>; want failure")
					}
				}
				...
				if test.mode&0o222 != 0 {
					...
					if err == nil {
						t.Errorf("WriteFile = <nil>; want failure")
					}
				}
			})
		})
