	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				if !errors.Is(err, ErrClosed) && !strings.Contains(err.Error(), "broken pipe") && !strings.Contains(err.Error(), "pipe is being closed") && !strings.Contains(err.Error(), "i/o on hungup channel") {
					t.Error(err)
				}
				return
			}
