...
func TestRenameFailed(t *testing.T) {
	...
	case *LinkError:
		...
		if err.New != to {
			t.Fatalf("rename %q, %q: err.New: want %q, got %q", from, to, to, err.New)
		}
	case nil:
		t.Fatalf("rename %q, %q: expected error, got nil", from, to)

		// cleanup whatever was placed in "renameto"
		Remove(to)
	default:
		t.Fatalf("rename %q, %q: expected %T, got %T %v", from, to, new(LinkError), err, err)
	}
}
