...
func openRootInRoot(r *Root, name string) (*Root, error) {
	if err := r.root.f.checkValid("openat"); err != nil {
		return nil, err
	}
	f, err := r.OpenFile(name, O_RDONLY, 0)
	if err != nil {
		return nil, err // already a *PathError
	}
	nr, err := newRoot(f)
	if err != nil {
...
func newRoot(f *File) (*Root, error) {
	fi, err := f.Stat()
	if err != nil {
		f.Close()
		return nil, underlyingError(err)
	}
	if !fi.IsDir() {
		f.Close()
		return nil, syscall.ENOTDIR
	}
	return &Root{root{f: f}}, nil
}
...
func doInRoot[T any](r *Root, name string, f func(parent sysfdType, name string) (T, error)) (ret T, err error) {
	...
	if err := r.root.f.pfd.Incref(); err != nil {
		return ret, err
	}
	defer r.root.f.pfd.Decref()

	rootfd := r.root.f.pfd.Sysfd
	dirfd := rootfd
	defer func() {
		if dirfd != rootfd {
			syscall.Close(dirfd)
