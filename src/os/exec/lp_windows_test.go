...
// makePATH returns a string suitable for use as a PATH variable
// containing all of the parent directories of the given files, which are
// assumed relative to root.
func makePATH(root string, files []string) string {
	var paths []string
	for _, f := range files {
...
func installProgs(t *testing.T, root string, files []string) {
	for _, f := range files {
		...
		if strings.ToLower(filepath.Ext(f)) == ".bat" {
...
func installExe(t *testing.T, dstPath string) {
	...
	dst, err := os.OpenFile(dstPath, os.O_CREATE|os.O_EXCL|os.O_WRONLY, 0777)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestLookPathWindows(t *testing.T) {
	...
	for _, tt := range lookPathTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pathVar := makePATH(root, tt.files)
