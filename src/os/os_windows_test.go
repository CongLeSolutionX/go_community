...
func TestUTF16Alloc(t *testing.T) {
	...
	// UTF16ToString has a stack-allocated buffer
	// that avoids allocating when the input's
	// length is 64 or less.
	allowsPerRun(1, func() {
		syscall.UTF16ToString(s[:64])
	})
	allowsPerRun(2, func() {
		syscall.UTF16ToString(s[:])
	})
	allowsPerRun(1, func() {
