...
// replaceDriveWithVolumeID replaces the drive prefix of path
// with the volume ID of the drive. E.g. C:\foo -> \\?\Volume{GUID}\foo.
...
func TestReadlink_SymbolicLink_Directory_Absolute_Drive(t *testing.T) {
	...
	link := filepath.Join(tmpdir, "link")
	target := filepath.Join(tmpdir, "target")
	cmd := testenv.Command(t, "cmd", "/c", "mklink", "/D", link, target)
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("%v: %v\n%s", cmd, err, out)
	}
	testReadlink(t, link, target)
...
func TestReadlink_SymbolicLink_Directory_Absolute_Volume(t *testing.T) {
	...
	target := filepath.Join(tmpdir, "target")
	volTarget := replaceDriveWithVolumeID(t, target)
	cmd := testenv.Command(t, "cmd", "/c", "mklink", "/D", link, volTarget)
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("%v: %v\n%s", cmd, err, out)
	}
	want := volTarget
...
func TestReadlink_SymbolicLink_File_Absolute_Drive(t *testing.T) {
	...
	cmd := testenv.Command(t, "cmd", "/c", "mklink", link, target)
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("%v: %v\n%s", cmd, err, out)
	}
	testReadlink(t, link, target)
...
func TestReadlink_SymbolicLink_File_Absolute_Volume(t *testing.T) {
	...
	cmd := testenv.Command(t, "cmd", "/c", "mklink", link, volTarget)
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("%v: %v\n%s", cmd, err, out)
	}
	want := volTarget
