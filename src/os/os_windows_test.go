...
func testDirLinks(t *testing.T, tests []dirLinkTest) {
	...
	for _, test := range tests {
		...
		if err != nil {
			t.Errorf("%s test failed: %v", test.name, err)
			continue
		}
...
func enableCreateLinkPrivileges() error {
	...
	privStr, err := syscall.UTF16PtrFromString("SeCreateSymbolicLinkPrivilege")
	if err != nil {
		return err
	}
...
func TestDirectorySymbolicLink(t *testing.T) {
	...
	if err := enableCreateLinkPrivileges(); err != nil {
		t.Skip(`failed to enable "SeCreateSymbolicLinkPrivilege"; skipping current test`)
	}
	var tests = []dirLinkTest{
		{
			name: "use_mklink_cmd",
