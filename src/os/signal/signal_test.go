...
func TestAtomicStop(t *testing.T) {
	if os.Getenv("GO_TEST_ATOMIC_STOP") == "" {
		...
		for i := 0; i < tries; i++ {
			go func() {
				// Sleep to give the main goroutine a chance
				// to call Notify before we raise SIGINT.
				// There is no race here, as the test will
				// still pass if the main goroutine fails to
				// call Notify: the program will die with a
				// SIGINT, which is one of the success cases.
				// In the usual case the Sleep will let the
				// main goroutine call Notify, and we will
				// test that the signal is not lost by the
				// call to Stop.
