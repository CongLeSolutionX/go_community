...
func (f *File) readFrom(r io.Reader) (written int64, handled bool, err error) {
	...
	err = rc.Read(func(fd uintptr) bool {
		...
		// Since src is a mocked of poll.FD, it has no src.pd.runtimeCtx, so poll.Splice may
		// return an error with "waiting for unsupported file type" when getting EAGAIN error and then calling
		// poll.pollDesc.waitRead inside, thus we need to wait until r is readable again.
