...
func (f *File) readFrom(r io.Reader) (int64, error) {
	written, handled, err := copyFileRange(f, r)
	if !handled {
...
func copyFileRange(dst *File, r io.Reader) (written int64, handled bool, err error) {
	...
	written, handled, err = pollCopyFileRange(&dst.pfd, &src.pfd, remain)
	if lr != nil {
		lr.N -= written
	}
