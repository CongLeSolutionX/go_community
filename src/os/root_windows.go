...
func openat(dirfd syscall.Handle, name string, flag int, options uint32, perm FileMode) (syscall.Handle, error) {
	...
	err = windows.NtCreateFile(
		&h,
		// Must have FILE_GENERIC_READ to use GetFileInformationByHandle.
		access|windows.FILE_GENERIC_READ,
		objAttrs,
		&ioStatusBlock,
		&allocSize,
	...
	// We need to open files with FILE_GENERIC_READ to use GetFileInformationByHandle.
	// If the user is opening this file write-only, drop the read permission here.
	if flag&(O_RDONLY|O_WRONLY|O_RDWR) == O_WRONLY {
		...
		err = syscall.DuplicateHandle(^syscall.Handle(0), h, ^syscall.Handle(0), &h, access, false, syscall.DUPLICATE_CLOSE_SOURCE)
		if err != nil {
			return syscall.InvalidHandle, err
		}
	}
