...
func trimRightByteFunc(s string, fn func(i uint8) bool) string {
	i := len(s)
	for i > 0 && fn(s[i-1]) {
		i--
	}
...
//
// These are the rules implemented in this function:
//
// - Trailing path separators from the last part are replaced by a single backslash, e.g. ["a", "b///"] -> ["a", `b\`].
// - Dot parts are removed, e.g. ["a", ".", "b"] -> ["a", "b"].
// - Dot-dot parts are removed along with their parent, e.g. ["a", "..", "b"] -> ["b"].
// - A trailing dot is removed unless the resulting part is "..", e.g. ["a.", "b..", "...", "d"] -> ["a", "b.", "...", "d"].
// - Trailing dots and spaces are removed from the last part, e.g. ["a   "] -> ["a"].
// - Empty parts are removed, e.g. ["a", "", "b"] -> ["a", "b"].
// - If the path is empty, it is replaced by a single dot, e.g. [] -> ["."].
func rootCleanParts(parts []string, start int) ([]string, error) {
	// Remove trailing path separators from the part.
	// We will add them back later if necessary.
	var hasSeparator bool
	if len(parts) > 0 {
		last := parts[len(parts)-1]
		parts[len(parts)-1] = trimRightByteFunc(last, IsPathSeparator)
		hasSeparator = len(parts[len(parts)-1]) != len(last)
	}
	...
	for i := start; i < len(parts); i++ {
		...
		case stringslite.HasSuffix(part, "."):
			// Remove one trailing dot unless if the resulting part is "..".
			if part != "..." {
				parts[i] = part[:len(part)-1]
			}
		}
	}
	if hasSeparator && len(parts) > 0 {
		// Add back trailing path separators.
		parts[len(parts)-1] += "\\"
	}
	if len(parts) > 0 {
		// Remove trailing dots and spaces from last component.
		parts[len(parts)-1] = trimRightByteFunc(parts[len(parts)-1], func(i uint8) bool {
			return i == '.' || i == ' '
		})
	}
	// Delete empty parts.
