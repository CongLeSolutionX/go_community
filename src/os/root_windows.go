...
func rootOpenDir(parent syscall.Handle, name string) (syscall.Handle, error) {
	h, err := openat(parent, name, syscall.O_RDONLY|windows.O_DIRECTORY, 0)
	if err == syscall.ERROR_FILE_NOT_FOUND {
		// For consistency: Windows returns ERROR_PATH_NOT_FOUND when
		// any path component before the leaf is not found.
