...
func (test *rootTest) errEndsTest(t *testing.T, err error, format string, args ...any) bool {
	t.Helper()
	if test.wantError {
		if err == nil {
...
func TestRootOpen_File(t *testing.T) {
	...
	for _, test := range rootTestCases {
		test.run(t, func(t *testing.T, target string, root *os.Root) {
			...
			if test.errEndsTest(t, err, "root.Open(%q)", test.open) {
				return
			}
			defer f.Close()
...
func TestRootCreate(t *testing.T) {
	...
	for _, test := range rootTestCases {
		test.run(t, func(t *testing.T, target string, root *os.Root) {
			...
			if test.errEndsTest(t, err, "root.Create(%q)", test.open) {
				return
			}
...
func TestRootOpenRoot(t *testing.T) {
	for _, test := range rootTestCases {
		test.run(t, func(t *testing.T, target string, root *os.Root) {
			...
			if test.errEndsTest(t, err, "root.OpenRoot(%q)", test.open) {
				return
			}
			defer rr.Close()
			f, err := rr.Open("f")
...
var rootConsistencyTestCases = []rootConsistencyTest{{
	...
	open: "target/",
	detailedErrorMismatch: func() bool {
		// Whether Create("file/") returns ENOTDIR or EISDIR varies by platform.
		// When GOOS=js, os.Root.Create consitently returns ENOTDIR.
...
func (test rootConsistencyTest) run(t *testing.T, f func(t *testing.T, path string, r *os.Root) (string, error)) {
	...
	t.Run(test.name, func(t *testing.T) {
		...
		if err1 != nil || err2 != nil {
			...
			if !test.detailedErrorMismatch && e1.Err != e2.Err {
...
func TestRootUseAfterClose(t *testing.T) {
	...
	for _, test := range []struct {
		...
		name: "OpenFile",
		f: func(r *os.Root, filename string) error {
			_, err := r.OpenFile(filename, os.O_RDWR, 0666)
			return err
		},
	}, {
		name: "OpenRoot",
...
func TestRootConcurrentClose(t *testing.T) {
	...
	go func() {
		...
		first := true
		for {
			f, err := r.OpenFile("file", os.O_RDWR|os.O_CREATE, 0666)
			if err != nil {
				ch <- err
				return
			}
