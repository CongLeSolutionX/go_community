...
// If pollable is true, it tries to add the file to the runtime poller.
func newFile(h syscall.Handle, name string, kind string, pollable bool) *File {
	if kind == "file" {
		var m uint32
...
func NewFile(fd uintptr, name string) *File {
	...
	return newFile(h, name, "file", false)
}
...
func openFile(name string, flag int, perm FileMode) (file *File, err error) {
	...
	return newFile(r, name, "file", true), nil
}
...
func Pipe() (r *File, w *File, err error) {
	...
	return newFile(p[0], "|0", "file", true), newFile(p[1], "|1", "file", true), nil
}
