...
func TestGetwdDoesNotPanicWhenPathIsLong(t *testing.T) {
	...
	// Check that syscall.Getwd can return a path longer than 300 characters.
	for i := 0; ; i++ {
		wd, err := syscall.Getwd()
		t.Logf("[iter %2d] getwd(): %q len: %d err: %v", i, wd, len(wd), err)
		if err != nil {
			t.Fatal("getwd:", err)
		}
		if len(wd) > 300 {
		...
		// Use absolute path becase fixLongPath ignores relative paths.
		path := filepath.Join(wd, t.Name())
		// [TODO] We use skip below since not all Windows versions support
