...
// gstringb reads a non-empty byte slice from b, prefixed with a 16-bit length in little-endian order.
// If the length is greater than the number of bytes in b, nil is returned.
//go:nosplit
...
// close the numbered file descriptor, unless it is a member of fds.
//go:nosplit
func closeFdsExcept(n int, fds []int) {
	for _, fd := range fds {
		if n == fd {
			return
		}
