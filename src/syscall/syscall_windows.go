...
func Readlink(path string, buf []byte) (n int, err error) {
	...
	if uintptr(bytesReturned) < unsafe.Sizeof(*rdb) ||
		(rdb.ReparseTag != IO_REPARSE_TAG_SYMLINK && rdb.ReparseTag != IO_REPARSE_TAG_MOUNT_POINT) {
	...
	if rdb.ReparseTag == IO_REPARSE_TAG_SYMLINK {
		data := (*symbolicLinkReparseBuffer)(unsafe.Pointer(&rdb.reparseBuffer.DataBuffer[0]))
		s := UTF16ToString(((*[0xffff]uint16)(unsafe.Pointer(&data.PathBuffer[0])))[data.PrintNameOffset/2 : (data.PrintNameLength-data.PrintNameOffset)/2])
		n = copy(buf, []byte(s))
	} else {
		data := (*mountPointReparseBuffer)(unsafe.Pointer(&rdb.reparseBuffer.DataBuffer[0]))
		s := UTF16ToString(((*[0xffff]uint16)(unsafe.Pointer(&data.PathBuffer[0])))[data.PrintNameOffset/2 : (data.PrintNameLength-data.PrintNameOffset)/2])
		n = copy(buf, []byte(s))
	}
	return n, nil
