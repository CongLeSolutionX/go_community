...
// CopyBytesToGo copies bytes from the Uint8Array src to the byte slice dst. It returns the number of bytes copied,
// which will be the minimum of the lengths of src and dst. CopyBytesToGo panics if src is not an Uint8Array.
func CopyBytesToGo(dst []byte, src Value) int {
	n, ok := copyToGo(dst, src.ref)
	if !ok {
		panic("syscall/js: CopyBytesToGo: expected Uint8Array")
	}
	return n
}
...
// CopyBytesToJS copies bytes from the byte slice src to the Uint8Array dst. It returns the number of bytes copied,
// which will be the minimum of the lengths of src and dst. CopyBytesToJS panics if dst is not an Uint8Array.
func CopyBytesToJS(dst Value, src []byte) int {
	n, ok := copyToJS(dst.ref, src)
	if !ok {
		panic("syscall/js: CopyBytesToJS: expected Uint8Array")
	}
	return n
}
