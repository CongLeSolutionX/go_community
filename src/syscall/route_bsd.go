...
func rsaAlignOf(salen int) int {
	...
	if darwin64Bit {
		...
		// we need to know what's the underlying kernel's
...
func parseSockaddrLink(b []byte) (Sockaddr, error) {
	lla, _, err := parseLinkLayerAdj(b[4:])
	if err != nil {
		return nil, err
	}
	rsa := (*RawSockaddrDatalink)(unsafe.Pointer(&b[0]))
	sa := lla.(*SockaddrDatalink)
	sa.Len = rsa.Len
...
// parseLinkLayerAdj parses b as a datalink socket address in
// conventional BSD kernel form.
func parseLinkLayerAdj(b []byte) (Sockaddr, int, error) {
	...
	rsa := (*rawSockaddrDatalink)(unsafe.Pointer(&b[0]))
	l := rsaAlignOf(int(4 + rsa.Nlen + rsa.Alen + rsa.Slen))
	if len(b) < l {
		return nil, 0, EINVAL
	}
	b = b[4:]
	sa := &SockaddrDatalink{Nlen: rsa.Nlen, Alen: rsa.Alen, Slen: rsa.Slen}
	for i := 0; len(sa.Data) > i && i < int(rsa.Nlen+rsa.Alen+rsa.Slen); i++ {
		sa.Data[i] = int8(b[i])
	}
	return sa, l, nil
}
...
// parseSockaddrInet parses b as an internet socket address in
// conventional BSD kernel form.
func parseNetworkLayerAdj(b []byte, preferredFamily byte) (Sockaddr, error) {
...
func (m *RouteMessage) sockaddr() ([]Sockaddr, error) {
	...
	for i := uint(0); i < RTAX_MAX && len(b) >= minRoutingSockaddrLen; i++ {
		...
		default:
			sa, err := parseNetworkLayerAdj(b, preferredFamily)
			if err != nil {
				return nil, err
			}
			sas = append(sas, sa)
