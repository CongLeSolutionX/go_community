...
func parseSockaddrLink(b []byte) (Sockaddr, error) {
	lla, _, err := parseLinkLayerAdjacency(b[4:])
	if err != nil {
		return nil, err
	}
...
// parseLinkLayerAdjacency parses b as a datalink socket address in
// conventional BSD kernel form.
func parseLinkLayerAdjacency(b []byte) (Sockaddr, int, error) {
	...
	// | Data            (variable) |
	// +----------------------------+
	type linkLayerAdjacency struct {
		Type byte
		Nlen byte
		Alen byte
		Slen byte
	}
	lla := (*linkLayerAdjacency)(unsafe.Pointer(&b[0]))
...
// parseNetworkLayerAdjacency parses b as an internet socket address
// in conventional BSD kernel form.
func parseNetworkLayerAdjacency(b []byte, preferredFamily byte) (Sockaddr, error) {
...
func (m *RouteMessage) sockaddr() ([]Sockaddr, error) {
	...
	for i := uint(0); i < RTAX_MAX && len(b) >= minRoutingSockaddrLen; i++ {
		...
		default:
			sa, err := parseNetworkLayerAdjacency(b, preferredFamily)
			if err != nil {
				return nil, err
			}
			sas = append(sas, sa)
