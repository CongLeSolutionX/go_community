...
func Environ() []string {
	envs, e := GetEnvironmentStrings()
	if e != nil {
		return nil
	}
	defer FreeEnvironmentStrings(envs)
	r := make([]string, 0, 50) // Empty with room to grow.
	envp := uintptr(unsafe.Pointer(envs))
	size := unsafe.Sizeof(*envs)
	for {
		...
		r = append(r, string(utf16.Decode(unsafe.Slice((*uint16)(unsafe.Pointer(envp)), n))))
		envp += size * (uintptr(n) + 1)
	}
	return r
}
