...
func ptracePeek(pid int, addr uintptr, out []byte) (count int, err error) {
	...
	for len(out) > 0 {
		...
		try(ptrace64(PT_READ_BLOCK, int64(pid), int64(addr), bsize, uintptr(unsafe.Pointer(&out[0]))))
		addr += uintptr(bsize)
		n += bsize
		out = out[n:]
	}
...
func ptracePoke(pid int, addr uintptr, data []byte) (count int, err error) {
	...
	for len(data) > 0 {
		...
		try(ptrace64(PT_WRITE_BLOCK, int64(pid), int64(addr), bsize, uintptr(unsafe.Pointer(&data[0]))))
		addr += uintptr(bsize)
		n += bsize
		data = data[n:]
	}
