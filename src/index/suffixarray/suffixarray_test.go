...
func (x *index) Len() int           { return len(x.sa32) + len(x.sa64) }
func (x *index) Less(i, j int) bool { return bytes.Compare(x.at(i), x.at(j)) < 0 }
func (x *index) Swap(i, j int) {
	if x.sa32 != nil {
		x.sa32[i], x.sa32[j] = x.sa32[j], x.sa32[i]
	} else {
		x.sa64[i], x.sa64[j] = x.sa64[j], x.sa64[i]
	}
}

func (a *index) at(i int) []byte {
	if a.sa32 != nil {
		return a.data[a.sa32[i]:]
	} else {
		return a.data[a.sa64[i]:]
	}
}
...
func equal(x, y *Index) bool {
	...
	switch {
	case x.sa32 != nil && y.sa32 != nil:
		for i, j := range x.sa32 {
			if j != y.sa32[i] {
				return false
			}
		}
	case x.sa32 != nil && y.sa64 != nil:
		for i, j := range x.sa32 {
			if int64(j) != y.sa64[i] {
				return false
			}
		}
	case x.sa64 != nil && y.sa32 != nil:
		for i, j := range x.sa64 {
			if j != int64(y.sa32[i]) {
				return false
			}
		}
	case x.sa64 != nil && y.sa64 != nil:
		for i, j := range x.sa64 {
			if j != y.sa64[i] {
				return false
			}
