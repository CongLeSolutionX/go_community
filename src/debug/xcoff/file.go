...
// Information stored about an XCOFF symbol in AUX_CSECT entry
...
// Information stored about an XCOFF symbol in a AUX_FCN entry
...
func NewFile(r io.ReaderAt) (*File, error) {
	...
	sr.Seek(int64(hdrsz)+int64(opthdr), os.SEEK_SET)
	...
	// Read symbol table
	sr.Seek(int64(symptr), os.SEEK_SET)
	...
	for i := 0; i < int(nsyms); i++ {
		...
		if !needAuxFcn {
			sr.Seek(int64(numaux-1)*SYMESZ, os.SEEK_CUR)
		}
		i += numaux
		numaux = 0
		switch f.TargetMachine {
	...
	skip:
		...
		sr.Seek(int64(numaux)*SYMESZ, os.SEEK_CUR)
	}
	...
	for _, sect := range f.Sections {
		...
		sr.Seek(int64(sect.Relptr), os.SEEK_SET)
...
// Library name patern is either path/base/member or base/member
func (f *File) readImportIDs(s *Section) ([]string, error) {
	// Read loader header
	s.sr.Seek(0, os.SEEK_SET)
	var istlen uint32
	var nimpid int32
	var impoff uint64
	switch f.TargetMachine {
	...
	s.sr.Seek(int64(impoff), os.SEEK_SET)
...
func (f *File) ImportedSymbols() ([]ImportedSymbol, error) {
	...
	s.sr.Seek(int64(stoff), os.SEEK_SET)
	st := make([]byte, stlen)
	io.ReadFull(s.sr, st)
	...
	s.sr.Seek(int64(symoff), os.SEEK_SET)
