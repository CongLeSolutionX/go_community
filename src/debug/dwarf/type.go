...
//
// See the the comment on StructField for more info on the interpretation of
...
//
// Some notes on the BitSize/BitOffset/DataBitOffset fields that appear
// in BaseType and StructField below:
//
// These fields encapsulate information about bit size and bit offset
// for data members declared as bit fields in C/C++ struct/union/class
// types.
//
// DWARF also allow for the possibility of base types that have
// non-zero bit size and bit offset, so this information is also
// captured for base types, but it is worth noting that it's not
// possible to trigger this behavior using maintstream languages.
//
// Description of the elements below:
//
// - BitSize is simply the number of bits in the bitfield.
//
// - BitOffset contains the value of the "DW_AT_bit_offset" attribute
//   for the DIE corresponding to the field; this attribute was used in
//   DWARF 3 and prior, deprecated in DWARF 4, and removed in DWARF 5.
//   Most C compilers still emit this attr when DWARF 4 is requested.
//
// - DataBitOffset contains the value of the "DW_AT_data_bit_offset"
//   attribute for the DIE corresponding to the field; this attribute
//   was added in DWARF 4 as an optional alternative to "DW_AT_bit_offset",
//   but not mandated, then made the default for DWARF 5.
//
// These attributes (at the moment) appear _only_ for bitfield data members.
//
...
// } s;
//
// Depending on the compiler vintage and command line options used,
// the compiler may choose to emit DW_AT_bit_offset attributes to
// indicate bit field offsets, or it may use DW_AT_data_bit_offset.
//
// DW_AT_bit_offset contains a count of the bits between the most
// significant bit of the storage unit (in this case, "int") holding
// the bit field to the most significan bit of the bit field.
// DW_AT_bit_offset values will vary depending on the endianness of
// the target (since [confusingly] on little-endian systems the
// representation is still in big-endian terms.
//
// DW_AT_data_bit_offset counts bits from the start of the enclosing
// entity to the start of the bit field in question (meaning that
// these values can be quite large for structs with many fields prior
// to the bitfield). DW_AT_data_bit_offset do not vary depending on
...
//
// For DW_AT_data_bit_offset, expected values would be:
//
//   "j":     32
//   "k":     37
//   "m":     43
//   "n":     48
//
// Here the value 32 for "j" reflects the fact that the bitfield is
// preceded by other data members. Hence DW_AT_data_bit_offset values
// can be quite large for large structs.
//
type StructField struct {
	...
	BitOffset     int64 // see comment on StructField
	DataBitOffset int64 // see comment on StructField
