...
// PCToLine returns the compilation unit Entry and line table
// information corresponding to the PC value. If a correspondence
// cannot be found, it returns nil, nil, nil.
func (d *Data) PCToLine(pc uint64) (*Entry, *LineEntry, error) {
	...
	for {
		...
		highpc := ent.Val(AttrHighpc)
		if highpc, ok := highpc.(uint64); ok && pc >= highpc {
			continue
		}
		if highpc, ok := highpc.(int64); ok && pc >= uint64(int64(lowpc)+highpc) {
			continue
		}
