...
type LineReader struct {
	...
	programOffset Offset // section offset of statement program
...
func (r *LineReader) readPrologue() error {
	...
	prologueLength := Offset(buf.uint32())
...
// PCToLine returns the compile unit Entry and line table information
// corresponding to the PC value.  If a correspondence cannot be
// found, it returns nil, nil, nil.
func (d *Data) PCToLine(pc uint64) (*Entry, *LineEntry, error) {
	var lent LineEntry
	...
	for {
		...
		// Skip this CU if pc is clearly not in its range.
		// TODO: Check AttrRanges
		lowpc, ok := ent.Val(AttrLowpc).(uint64)
		if ok && pc < lowpc {
			continue
		}
		highpc, ok := ent.Val(AttrHighpc).(uint64)
		if ok && pc >= highpc {
			continue
		}

		// Check this CU's line table for pc
		...
		if err := lr.SeekPC(pc, &lent); err == nil {
			return ent, &lent, nil
		} else if err != UnknownPC {
			return nil, nil, err
		}
	}
	return nil, nil, nil
