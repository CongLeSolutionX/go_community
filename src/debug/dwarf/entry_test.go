...
func TestAbstractOrigin(t *testing.T) {
	// Note: this test binary was generated by compiling testdata/leafstack.cpp
	// with "g++ -O -g" from GCC 4.8.4 on Ubuntu 16.04 x86_64.
	d := elfData(t, "testdata/leafstack.elf")
	r := d.Reader()
	e, err := r.Next()
	if err != nil {
		t.Fatal(err)
	...
	i := 0
	for entry, err := r.Next(); entry != nil && err == nil; entry, err = r.Next() {
		if entry.Tag != TagInlinedSubroutine {
			continue
		}
		if i > len(inlined) {
			t.Fatalf("too many inlined subroutines (expected at most %d)", i)
		}

		// Inlined subroutines don't have name attributes
		_, got := entry.Val(AttrName).(string)
		if got {
			t.Errorf("inlined subroutine %d name present, not expected", i)
			break
		}

		// Inlined subroutines should have abstract origins
		origin, err := r.AbstractOrigin(entry)
		if err != nil {
			t.Errorf("inlined subroutine %d AbstractOrigin: %v", i, err)
			continue
		}

		// Origin DIE should indeed have a name
		if got, ok := origin.Val(AttrName).(string); !ok {
			t.Errorf("inlined subroutine %d no name", i)
		} else if got != inlined[i] {
			t.Errorf("inlined subroutine %d name is %s, expected %s", i, got, inlined[i])
		}
		i++
	}

	if i < len(inlined) {
		t.Errorf("saw only %d inlined subroutines, expected %d", i, len(inlined))
	}
}
