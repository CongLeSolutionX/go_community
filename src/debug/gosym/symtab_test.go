...
func TestIssue29551(t *testing.T) {
	tests := []struct {
		...
		{Sym{goVersion: ver118 + 1, Name: "type:.eq.[9]debug/elf.intName"}, ""},
		{Sym{goVersion: ver118 + 1, Name: "type:.hash.debug/elf.ProgHeader"}, ""},
		{Sym{goVersion: ver118 + 1, Name: "type:.eq.runtime._panic"}, ""},
		{Sym{goVersion: ver118 + 1, Name: "type:.hash.struct { runtime.gList; runtime.n int32 }"}, ""},
		{Sym{goVersion: ver118 + 1, Name: "go:(*struct { sync.Mutex; math/big.table [64]math/big"}, ""},
		{Sym{goVersion: ver118 + 1, Name: "go.uber.org/zap/buffer.(*Buffer).AppendString"}, "go.uber.org/zap/buffer"},
	}

	for _, tc := range tests {
		tc := tc
		t.Run(tc.sym.Name, func(t *testing.T) {
			t.Parallel()
