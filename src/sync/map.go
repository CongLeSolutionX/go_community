...
// Map methods do not require external synchronization.
//
...
type Map struct {
	...
	// misses counts the number of Load or Len calls for which the clean map was
	// nil since the last write.
	//
...
// loaded reports whether the value was loaded (false if stored).
...
// If f returns false, all remaining keys and values are skipped.
//
// f must not call any other methods on the Map.
...
func (m *Map) Range(f func(key, value interface{}) (shouldContinue bool)) {
	clean, _ := m.clean.Load().(map[interface{}]interface{})
	if clean == nil {
		m.mu.Lock()
		if m.dirty == nil {
