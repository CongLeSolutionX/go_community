...
func BenchmarkPoolStarvation(b *testing.B) {
	var p Pool
	count := 100
	// Reduce number of putted objects by equal to one goroutine's part
	// of uniform objects distribution. It creates objects starvation
	// that force P-local storage to steal objects from other Ps.
	countStarved := count - (count / runtime.GOMAXPROCS(0))
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
