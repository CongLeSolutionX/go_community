...
// Build toolchain using Go bootstrap (>=1.16).
//
...
// File suffixes that use build tags introduced since Go 1.16.
// These must not be copied into the bootstrap build directory.
// Also ignore test files.
var ignoreSuffixes = []string{
	"_test.s",
	"_test.go",
	// TODO(#47914): Remove below suffixes once all Go builders use Go 1.16 as bootstrap compiler.
	"_arm64.s",
	"_arm64.go",
	"_riscv64.s",
	"_riscv64.go",
...
func bootstrapBuildTools() {
	...
	if goroot_bootstrap == "" {
		goroot_bootstrap = pathf("%s/go1.16", os.Getenv("HOME"))
	}
	...
	// Use -gcflags=-l to disable inlining to workaround bugs in Go 1.4's compiler.
	...
	cmd := []string{
		...
		"install",
		"-gcflags=-l", // TODO(#47914): Remove once all Go builders use Go 1.16 as bootstrap compiler.
