...
func isUnneededSSARewriteFile(srcFile string) bool {
	...
	return strings.ToLower(fileArch) != strings.TrimSuffix(runtime.GOARCH, "le")
}
...
func bootstrapRewriteFile(text, srcFile string) string {
	...
	if isUnneededSSARewriteFile(srcFile) {
		// archCaps is 386, AMD64, ...
		archCaps := strings.TrimSuffix(strings.TrimPrefix(filepath.Base(srcFile), "rewrite"), ".go")
