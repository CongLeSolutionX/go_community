...
func (t *tester) registerTests() {
	...
	for _, cpu := range []string{"1", "2", "4"} {
		...
		t.tests = append(t.tests, distTest{
			...
			fn: func() error {
				...
				cmd.Env = append([]string{"GOMAXPROCS=2"}, os.Environ()...)
				return cmd.Run()
			},
		})
	}
	...
	if t.hasBash() && cgo && t.goos != "windows" {
	...
	if t.hasBash() && t.goos != "nacl" && t.goos != "android" {
		t.registerTest("doc_progs", "../docs/progs", "time", "./run")
	}
	if t.hasBash() && t.goos != "nacl" && t.goos != "android" {
...
func (t *tester) cgoTest() error {
	env := append([]string{"GOTRACEBACK=2"}, os.Environ()...)
...
func (t *tester) testDirTest() error {
	...
	cmd.Env = t.dedupEnv(append(os.Environ(), "GOOS="+t.gohostos, "GOARCH="+t.gohostarch, "GOMAXPROCS="))
	if err := cmd.Run(); err != nil {
		return err
	}
...
func (t *tester) dedupEnv(env []string) []string {
	caseInsensitive := t.gohostos == "windows"
