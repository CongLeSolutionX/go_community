...
func (t *tester) run() {
	...
	t.cgoEnabled, _ = strconv.ParseBool(strings.TrimSpace(parts[0]))
	t.goexe = strings.TrimSpace(parts[1])
	t.gotmpdir = strings.TrimSpace(parts[2])
...
// pkgs is the list of packages to build and run. The caller is responsible for
// adding opts.testFlags, and must call setupCmd on the resulting exec.Cmd to
// set its directory and environment.
	...
	if os.Getenv("GO_BUILDER_NAME") != "" && goos != "android" && !t.iOS() && goos != "plan9" && goos != "js" {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				...
				cmd := (&goTest{
					goCmd: filepath.Join(moved, "bin", "go"),
					pkg:   "fmt",
				}).command(t)
				unsetEnv(cmd, "GOROOT")
...
func (o rtSequential) isRegisterTestOpt() {}
...
func (o rtPreFunc) isRegisterTestOpt() {}
...
func (o rtHostTest) isRegisterTestOpt() {}
...
func (t *tester) registerRaceTests() {
	...
	if t.cgoEnabled {
		...
		// t.registerTest("race:misc/cgo/test", hdr, &goTest{dir: "../misc/cgo/test", race: true, env: []string{"GOTRACEBACK=}})
	}
	if t.extLink() {
