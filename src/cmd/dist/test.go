...
func (t *tester) registerTests() {
	...
	if len(t.runNames) > 0 {
		...
		for _, pkg := range pkgs {
			...
			if strings.HasPrefix(pkg, "vendor/") || strings.HasPrefix(pkg, "cmd/vendor/") {
				// Vendored code has no tests,
				// and we don't care too much about vet errors
				// since we can't modify the code,
				// so skip the tests entirely.
				continue
			}
