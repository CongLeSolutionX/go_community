...
func setup() {
	...
	if strings.HasPrefix(goversion, "release.") || (strings.HasPrefix(goversion, "go") && !strings.Contains(goversion, "beta")) {
		...
		// Only for releases, so that people working in the main branch
		// can still have full paths for stack traces for compiler crashes and the like.
		toolenv = append(toolenv, "-trimpath")
	}
}
