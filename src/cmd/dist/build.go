...
func timelog(op, name string) {
	...
	if timeLogFile == nil {
		...
		if i < 0 {
			log.Fatal("time log %s does not begin with start line", os.Getenv("GOBULDTIMELOGFILE"))
		}
		t, err := time.Parse(time.UnixDate, s[:i])
		if err != nil {
			log.Fatal("cannot parse time log line %q: %v", s, err)
		}
		timeLogStart = t
		timeLogFile = f
	}
...
func showEnv(verb, goCmd string) {
	...
	run("", CheckExit|ShowOutput, "go", "list", "-f={{.Stale}}", "cmd/compile")
...
func checkNotStale(goBinary string, targets ...string) {
	...
	if out != "" {
		fatalf("unexpected stale targets reported by %s for %v:\n%s", goBinary, targets, out)
	}
}
