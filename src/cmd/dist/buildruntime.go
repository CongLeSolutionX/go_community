...
func mkzversion(dir, file string) {
	...
	fmt.Fprintf(&buf, "const DefaultGoroot = %#q\n", goroot_final)
	fmt.Fprintf(&buf, "const TheVersion = %#q\n", findgoversion())
	fmt.Fprintf(&buf, "const Goexperiment = %#q\n", os.Getenv("GOEXPERIMENT"))
...
func mkzbootstrap(file string) {
	...
	fmt.Fprintf(&buf, "const defaultGOROOT = %#q\n", goroot_final)
	fmt.Fprintf(&buf, "const defaultGO386 = %#q\n", go386)
	fmt.Fprintf(&buf, "const defaultGOARM = %#q\n", goarm)
	fmt.Fprintf(&buf, "const defaultGOOS = runtime.GOOS\n")
	fmt.Fprintf(&buf, "const defaultGOARCH = runtime.GOARCH\n")
	fmt.Fprintf(&buf, "const defaultGO_EXTLINK_ENABLED = %#q\n", goextlinkenabled)
