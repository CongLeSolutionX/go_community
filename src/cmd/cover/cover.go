...
func readPackageConfig(path string) error {
	...
	for lineNum, line := range strings.Split(string(data), "\n") {
		...
		case "granularity":
			if value != "perblock" && value != "perfunc" {
				return fmt.Errorf(`%s:%d: tag %q requires perblock/perfunc value`,
					path, lineNum, tag)
			}
		default:
			return fmt.Errorf(`%s:%d: unrecognized pkgcfg tag %q`,
				path, lineNum, tag)
		}
		cfgMap[tag] = value
	}
	if seen != 6 {
		return fmt.Errorf("error reading pkgconfig file %q: got %d values expected %d", path, seen, 6)
	}
	return nil
}
...
func (p *Package) emitMetaData(w io.Writer) {
	...
	fmt.Fprintf(cf, "countergranularity %s\n", cfgMap["granularity"])
