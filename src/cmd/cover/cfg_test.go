...
func writePkgConfig(t *testing.T, outdir, tag, ppath, pname, pkclass string, regonly bool, gran string) string {
	outcfg := filepath.Join(outdir, "outcfg.txt")
	cfgContents := fmt.Sprintf(`
# comment
pkgpath=%s
pkgname=%s
pkgclassification=%s
...
granularity=%s
outconfig=%s
`, ppath, pname, pkclass, regonly, gran, outcfg)
...
func testCoverWithCfg(t *testing.T) {
	...
	for _, scenario := range scenarios {
		...
		ppath := "cfg/a"
		pname := "a"
		pkgclass := scenario.class
		mode := scenario.mode
		gran := scenario.gran
		regonly := scenario.regonly
		incfg = writePkgConfig(t, instdira, tag, ppath, pname,
			pkgclass, regonly, gran)
	...
	writeFile(t, incfg, fmt.Sprintf("blah=foo\n"))
	_, _, errmsg = runPkgCover(t, instdira, tag, incfg, mode,
		pfiles("a"), errExpected)
	want = "unrecognized pkgcfg tag"
	if !strings.Contains(errmsg, want) {
		t.Errorf("'bad config file' test: wanted %s got %s", want, errmsg)
	}

	// Expect errr on empty config file.
	t.Logf("writing empty config")
	writeFile(t, incfg, fmt.Sprintf("\n"))
	_, _, errmsg = runPkgCover(t, instdira, tag, incfg, mode,
		pfiles("a"), errExpected)
	want = "got 0 values expected 6"
	if !strings.Contains(errmsg, want) {
		t.Errorf("'bad config file' test: wanted %s got %s", want, errmsg)
	}
}
