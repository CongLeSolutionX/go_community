...
func TestMain(m *testing.M) {
	...
	os.Setenv("CMDCOVER_TEST_RUN_MAIN", "true")
...
func TestCoverWithToolExec(t *testing.T) {
	...
	// Build wrapper.
	toolexec, err := testenv.BuildToolExec(t, "cover.test", dir)
	if err != nil {
		t.Fatal("failed to build toolexec", err)
	}
	...
	t.Run("WithCfg", func(t *testing.T) {
		testCoverWithCfg(t, toolexecArg)
	})
}
