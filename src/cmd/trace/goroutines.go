...
func init() {
	http.Handle("/goroutines", http.HandlerFunc(httpGoroutines))
	http.Handle("/goroutine", http.HandlerFunc(httpGoroutine))
	http.Handle("/gtrace", http.HandlerFunc(httpGTrace))
}
...
func httpGoroutine(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fmt.Fprintf(w, "failed to parse id parameter '%v': %v\n", r.FormValue("id"), err)
		return
	}
	analyzeGoroutines()
	var glist gdescList
...
{{range $}}
  <tr>
    <td> <a href="/gtrace?id={{.ID}}">{{.ID}}</a> </td>
    <td> {{.TotalTime}} </td>
    <td> {{.ExecTime}} </td>
    <td> {{.IOTime}} </td>
    <td> {{.BlockTime}} </td>
...
// httpGTrace serves a trace for a single goroutine.
func httpGTrace(w http.ResponseWriter, r *http.Request) {
	goid, err := strconv.ParseUint(r.FormValue("id"), 10, 64)
	if err != nil {
		fmt.Fprintf(w, "failed to parse id parameter '%v': %v\n", r.FormValue("id"), err)
		return
	}
	analyzeGoroutines()
	gset := relatedGoroutines(goid)
	g := gs[goid]
	httpTraceImpl(w, r, true, g.StartTime, g.EndTime, goid, gset)
}

// relatedGoroutines finds set of related goroutines that we need to include
// into trace for goroutine goid.
func relatedGoroutines(goid uint64) map[uint64]bool {
