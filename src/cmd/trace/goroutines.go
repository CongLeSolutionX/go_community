...
func init() {
	...
	http.HandleFunc("/gtrace", httpGTrace)
}
...
{{range $}}
  <tr>
    <td> <a href="/gtrace?goid={{.ID}}">{{.ID}}</a> </td>
    <td> {{.TotalTime}} </td>
    <td> {{.ExecTime}} </td>
    <td> {{.IOTime}} </td>
    <td> {{.BlockTime}} </td>
...
func httpGTrace(w http.ResponseWriter, r *http.Request) {
	goid, err := strconv.ParseUint(r.FormValue("goid"), 10, 64)
	if err != nil {
		fmt.Fprintf(w, "failed to parse goid parameter '%v': %v\n", r.FormValue("goid"), err)
		return
	}
	httpTraceImpl(w, r, goid)
}
