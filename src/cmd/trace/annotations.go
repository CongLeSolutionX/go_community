...
func httpUserTask(w http.ResponseWriter, r *http.Request) {
	...
	for _, task := range tasks {
		...
		rawEvents := task.events
		for _, span := range task.spans {
			if span.Start != nil {
				rawEvents = append(rawEvents, span.Start)
			}
		}
		sort.Slice(rawEvents, func(i, j int) bool {
			return rawEvents[i].Ts < rawEvents[j].Ts
		})
...
func analyzeAnnotations() (annotationAnalysisResult, error) {
	...
	for goid, stats := range gs {
		for _, s := range stats.Spans {
			...
			if s.Start != nil && s.Start.Type == trace.EvGoCreate { // skip fake span
				task.events = append(task.events, s.Start)
			} else {
