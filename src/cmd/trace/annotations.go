	...
	for _, ev := range events {
		...
		case trace.EvUserTaskCreate, trace.EvUserTaskEnd, trace.EvUserLog, trace.EvUserSpan:
			...
			if typ == trace.EvUserSpan {
				if err := activeSpans.processEvUserSpan(ev, task); err != nil {
					return annotationAnalysisResult{}, err
				}
			}
		...
		case trace.EvGoEnd:
			...
			activeSpans.processGoroutineEnd(ev)

			// trace.EvGoStop (e.g. select{}) makes the goroutine live forever,
			// so spans are left open forever.
...
type goroutineToSpans map[uint64][]*spanDesc // goroutine id to spans

func (gtos goroutineToSpans) processEvUserSpan(ev *trace.Event, task *taskDesc) error {
	goid := ev.G
	taskid := ev.Args[0]
	spans := gtos[goid]
	...
	case 1: // end
		...
		s := spans[len(spans)-1]
		if s.task.id != taskid {
			return fmt.Errorf("misuse of span is detected: span (task %v, %s) ends while span (task %v, %s) is active", taskid, ev.SArgs[0], s.task.id, s.name)
		}
		s.end = ev
		// TODO(hyangah): this belongs to internal/trace/parser.go
		s.start.Link = ev
		gtos[goid] = spans[:len(spans)-1]
	}
	return nil
}

func (gtos goroutineToSpans) processGoroutineEnd(ev *trace.Event) {
	goid := ev.G
	spans := gtos[goid]
	for _, s := range spans {
		s.end = ev
