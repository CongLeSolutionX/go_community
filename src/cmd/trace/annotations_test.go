...
//   goroutine 2: starts task0, do work in task0.span0
...
func TestAnalyzeAnnotationSimple(t *testing.T) {
	// Run prog0 and capture the execution trace.
	traceProgram(prog0, "TestAnalyzeAnnotationSimple")
...
func TestAnalyzeAnnotationGC(t *testing.T) {
	...
	for _, task := range res.tasks {
		got := task.allOverlappingDuration(res.gcEvents)
		switch task.name {
		case "taskWithGC":
			if got <= 0 || got >= gcTime {
