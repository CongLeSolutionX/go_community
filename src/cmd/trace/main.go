...
func main() {
	...
	switch *pprofFlag {
	case "net":
		if err := pprofIO(os.Stdout); err != nil {
			log.Fatal(err)
		}
		os.Exit(0)
	case "sync":
		if err := pprofBlock(os.Stdout); err != nil {
			log.Fatal(err)
		}
		os.Exit(0)
	case "syscall":
		if err := pprofSyscall(os.Stdout); err != nil {
			log.Fatal(err)
		}
		os.Exit(0)
	case "sched":
		if err := pprofSched(os.Stdout); err != nil {
			log.Fatal(err)
		}
		os.Exit(0)
	case "":
	default:
		log.Fatalf("Unknown pprof type %s", *pprofFlag)
	}
