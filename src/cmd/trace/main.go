...
func main() {
	...
	if err != nil {
		dief("%s\n", fmt.Errorf("failed to read trace file: %w", err))
	}
	defer tracef.Close()
	...
	if err != nil {
		dief("%s\n", fmt.Errorf("failed to stat trace file: %v", err))
	}
	traceSize := fi.Size()
	...
	if *pprofFlag != "" {
		...
		default:
			dief("%s\n", fmt.Errorf("unknown pprof type %s\n", *pprofFlag))
		}
		records, err := f(&http.Request{})
		if err != nil {
			dief("%s\n", fmt.Errorf("failed to generate pprof: %v\n", err))
		}
	...
	switch *debugFlag {
	case 1:
		dief("%s\n", debugProcessedEvents(tracef))
	case 2:
		dief("%s\n", debugRawEvents(tracef))
	}

	ln, err := net.Listen("tcp", *httpFlag)
	if err != nil {
		dief("%s\n", fmt.Errorf("failed to create server socket: %w", err))
	}
