...
func Main(traceFile, httpAddr, pprof string, debug int) error {
	...
	switch debug {
	case 1:
		return debugProcessedEvents(data)
	case 2:
		return debugRawEvents(data)
	}
...
func debugProcessedEvents(traceData []byte) error {
	tr, err := tracev2.NewReader(bytes.NewReader(traceData))
	if err != nil {
		return err
	}
	for {
...
func debugRawEvents(traceData []byte) error {
