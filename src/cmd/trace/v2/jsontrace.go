			...
			case tracev2.ResourceGoroutine:
				...
				if (from == tracev2.GoSyscall && to == tracev2.GoWaiting) ||
					...
					if gs.startRunning.Kind() != tracev2.EventBad {
						...
						// Double check: We should not end up here unless we're
				...
				if !from.Executing() && to == tracev2.GoRunning {
					// Goroutine started running. If this goroutine has a start cause,
					// emit a flow event for it.
					if gs.startCause.Kind() != tracev2.EventBad {
