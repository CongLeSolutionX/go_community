...
// @TODO(fg) is there a less brittle way than matching this range name?
const gcConcurrentMarkPhase = "GC concurrent mark phase"
...
func generateTrace(parsed parsedTrace, c traceviewer.TraceConsumer) error {
	...
	for _, ev := range parsed.events {
		...
		case tracev2.EventRangeBegin:
			if ev.Range().Name == gcConcurrentMarkPhase {
				gcStart = ev
			}
		case tracev2.EventRangeEnd:
			if ev.Range().Name == gcConcurrentMarkPhase {
				te.Slice(traceviewer.Slice{
					Name:     "GC",
					Ts:       ev.Time().Sub(first.Time()),
					Dur:      ev.Time().Sub(gcStart.Time()),
					Proc:     uint64(trace.GCP), // TODO(fg) why is this not the same as ev.Proc()?
					Stack:    te.Stack(eventFrames(gcStart, &scratchFrames)),
