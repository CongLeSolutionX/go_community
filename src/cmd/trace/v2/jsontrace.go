	...
	for _, ev := range parsed.events {
		...
		case tracev2.EventStateTransition:
			...
			case tracev2.ResourceGoroutine:
				...
				if from.Executing() && !to.Executing() && gs.startRunning.Kind() != tracev2.EventBad {
					// Emit a slice event for the time the goroutine was running.
					te.Slice(traceviewer.SliceEvent{
						Name:     gs.label,
						Ts:       gs.startRunning.Time().Sub(first.Time()),
						Dur:      ev.Time().Sub(gs.startRunning.Time()),
				...
				if !from.Executing() && to == tracev2.GoRunning && gs.startCause.Kind() != tracev2.EventBad {
					// Goroutine started running with a start cause. Emit a flow event.
					te.Arrow(traceviewer.ArrowEvent{
						Name:      gs.startCauseName,
						Start:     gs.startCause.Time().Sub(first.Time()),
						End:       ev.Time().Sub(first.Time()),
						FromProc:  uint64(gs.startCauseProc),
						ToProc:    uint64(ev.Proc()),
						FromStack: te.Stack(eventFrames(gs.startCause, &scratchFrames)),
					})
					gs.startCause = tracev2.Event{}
					gs.startCauseName = ""
					gs.startCauseProc = 0
				}
				if to == tracev2.GoWaiting {
					// Goroutine started blocking.
					gs.startBlockReason = st.Reason
				}

				// Handle state tracking for flow events.
				if from == tracev2.GoWaiting {
					gs.startCause = ev
					gs.startCauseName = "unblock"
					gs.startCauseProc = uint64(ev.Proc())
					if strings.Contains(gs.startBlockReason, "network") {
						// Emit an unblock instant event for the "Network" lane.
						te.Instant(traceviewer.InstantEvent{
							Name:  "unblock",
							Ts:    ev.Time().Sub(first.Time()),
							Proc:  trace.NetpollP,
							Stack: te.Stack(eventFrames(ev, &scratchFrames)),
						...
						gs.startBlockReason = ""
						gs.startCauseProc = trace.NetpollP
					}
				}
				if from == tracev2.GoNotExist && to == tracev2.GoRunnable {
					// Goroutine was created.
					gs.startCause = ev
					gs.startCauseName = "go"
					gs.startCauseProc = uint64(ev.Proc())
				}
				if from == tracev2.GoSyscall && to != tracev2.GoRunning {
					// Exiting blocked syscall.
					gs.startCause = ev
					gs.startCauseName = "exit blocked syscall"
					gs.startCauseProc = trace.SyscallP
				...
				// Write down any running-related state we need to track.
				if to == tracev2.GoRunning && gs.startRunning.Kind() == tracev2.EventBad {
					gStates[goID].startRunning = ev
				} else if to == tracev2.GoSyscall && ev.Proc() != tracev2.NoProc {
...
type gState struct {
	...
	startCause     tracev2.Event
	startCauseName string // The name to assign to the flow event.
	startCauseProc uint64
}
