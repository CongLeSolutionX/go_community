...
func JSONTraceHandler(traceData []byte) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		te := traceviewer.NewEmitter(c, 0, time.Duration(start), time.Duration(end))
		defer te.Flush()
				...
				case tracev2.ResourceGoroutine:
					...
					if (from == tracev2.GoSyscall && to == tracev2.GoWaiting) ||
						...
						te.Event(&format.Event{
							...
							TID:      uint64(ev.Proc()),
							Stack:    te.Stack(frames(scratchFrames, gs.running)),
							EndStack: te.Stack(frames(scratchFrames, ev)),
						})
...
func frames(frames []*trace.Frame, ev tracev2.Event) []*trace.Frame {
	n := 0
