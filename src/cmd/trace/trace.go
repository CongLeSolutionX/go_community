...
func generateTrace(params *traceParams, consumer traceviewer.TraceConsumer) error {
	// TODO(fg) remove old mode type
	emitter := traceviewer.NewEmitter(consumer, traceviewer.Mode(params.mode))
	defer emitter.Flush()
	...
	for _, ev := range ctx.parsed.Events {
		...
		case trace.EvHeapGoal:
			emitter.NextGC(time.Duration(ev.Ts), ev.Args[0])
		}
		if setGStateErr != nil {
			return setGStateErr
		}
	...
	emitter.Gomaxprocs(uint64(maxProc))
