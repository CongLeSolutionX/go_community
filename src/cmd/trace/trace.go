...
func generateTrace(params *traceParams, consumer traceviewer.TraceConsumer) error {
	...
	ctx.emitSectionFooter(format.StatsSection, "STATS", 0)

	if ctx.mode&modeTaskOriented != 0 {
		ctx.emitSectionFooter(format.TasksSection, "TASKS", 1)
	}

	if ctx.mode&modeGoroutineOriented != 0 {
		ctx.emitSectionFooter(format.ProcsSection, "G", 2)
	} else {
		ctx.emitSectionFooter(format.ProcsSection, "PROCS", 2)
	}

	ctx.emitFooter(&format.Event{Name: "thread_name", Phase: "M", PID: format.ProcsSection, TID: trace.GCP, Arg: &NameArg{"GC"}})
	ctx.emitFooter(&format.Event{Name: "thread_sort_index", Phase: "M", PID: format.ProcsSection, TID: trace.GCP, Arg: &SortIndexArg{-6}})

	ctx.emitFooter(&format.Event{Name: "thread_name", Phase: "M", PID: format.ProcsSection, TID: trace.NetpollP, Arg: &NameArg{"Network"}})
	ctx.emitFooter(&format.Event{Name: "thread_sort_index", Phase: "M", PID: format.ProcsSection, TID: trace.NetpollP, Arg: &SortIndexArg{-5}})

	ctx.emitFooter(&format.Event{Name: "thread_name", Phase: "M", PID: format.ProcsSection, TID: trace.TimerP, Arg: &NameArg{"Timers"}})
	ctx.emitFooter(&format.Event{Name: "thread_sort_index", Phase: "M", PID: format.ProcsSection, TID: trace.TimerP, Arg: &SortIndexArg{-4}})

	ctx.emitFooter(&format.Event{Name: "thread_name", Phase: "M", PID: format.ProcsSection, TID: trace.SyscallP, Arg: &NameArg{"Syscalls"}})
	ctx.emitFooter(&format.Event{Name: "thread_sort_index", Phase: "M", PID: format.ProcsSection, TID: trace.SyscallP, Arg: &SortIndexArg{-3}})

	// Display rows for Ps if we are in the default trace view mode (not goroutine-oriented presentation)
	if ctx.mode&modeGoroutineOriented == 0 {
		for i := 0; i <= maxProc; i++ {
			ctx.emitFooter(&format.Event{Name: "thread_name", Phase: "M", PID: format.ProcsSection, TID: uint64(i), Arg: &NameArg{fmt.Sprintf("Proc %v", i)}})
			ctx.emitFooter(&format.Event{Name: "thread_sort_index", Phase: "M", PID: format.ProcsSection, TID: uint64(i), Arg: &SortIndexArg{i}})
		}
	}
	...
	if ctx.mode&modeGoroutineOriented != 0 {
		for k, v := range ginfos {
			...
			ctx.emitFooter(&format.Event{Name: "thread_name", Phase: "M", PID: format.ProcsSection, TID: k, Arg: &NameArg{v.name}})
		}
		// Row for the main goroutine (maing)
		ctx.emitFooter(&format.Event{Name: "thread_sort_index", Phase: "M", PID: format.ProcsSection, TID: ctx.maing, Arg: &SortIndexArg{-2}})
...
func (ctx *traceContext) emitSectionFooter(sectionID uint64, name string, priority int) {
	ctx.emitFooter(&format.Event{Name: "process_name", Phase: "M", PID: sectionID, Arg: &NameArg{name}})
	ctx.emitFooter(&format.Event{Name: "process_sort_index", Phase: "M", PID: sectionID, Arg: &SortIndexArg{priority}})
}
...
func (ctx *traceContext) emitHeapCounters(ev *trace.Event) {
	...
	if tsWithinRange(ev.Ts, ctx.startTime, ctx.endTime) {
		ctx.emit(&format.Event{Name: "Heap", Phase: "C", Time: ctx.time(ev), PID: 1, Arg: &heapCountersArg{ctx.heapStats.heapAlloc, diff}})
	}
...
func (ctx *traceContext) emitGoroutineCounters(ev *trace.Event) {
	...
	if tsWithinRange(ev.Ts, ctx.startTime, ctx.endTime) {
		ctx.emit(&format.Event{Name: "Goroutines", Phase: "C", Time: ctx.time(ev), PID: 1, Arg: &goroutineCountersArg{uint64(ctx.gstates[gRunning]), uint64(ctx.gstates[gRunnable]), uint64(ctx.gstates[gWaitingGC])}})
	}
...
func (ctx *traceContext) emitThreadCounters(ev *trace.Event) {
	...
	if tsWithinRange(ev.Ts, ctx.startTime, ctx.endTime) {
		ctx.emit(&format.Event{Name: "Threads", Phase: "C", Time: ctx.time(ev), PID: 1, Arg: &threadCountersArg{
...
func (ctx *traceContext) emitArrow(ev *trace.Event, name string) {
	...
	ctx.arrowSeq++
	ctx.emit(&format.Event{Name: name, Phase: "s", TID: ctx.proc(ev), ID: ctx.arrowSeq, Time: ctx.time(ev), Stack: ctx.stack(ev.Stk), Cname: color})
...
func (ctx *traceContext) buildBranch(parent frameNode, stk []*trace.Frame) int {
	...
	if !ok {
		...
		ctx.consumer.ConsumeViewerFrame(strconv.Itoa(node.id), format.Frame{Name: fmt.Sprintf("%v:%v", frame.Fn, frame.Line), Parent: parent.id})
	}
