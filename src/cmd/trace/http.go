...
// httpTraceImpl serves either whole trace (goid==0) or trace for goid goroutine.
func httpTraceImpl(w http.ResponseWriter, r *http.Request, goid uint64) {
	params := ""
	if goid != 0 {
...
func httpJsonTrace(w http.ResponseWriter, r *http.Request) {
	...
	if goids := r.FormValue("goid"); goids != "" {
		...
		if err != nil {
			fmt.Fprintf(w, "failed to parse goid parameter '%v': %v\n", goids, err)
			return
		}
		analyzeGoroutines()
		g := gs[goid]
		ctx.gtrace = true
		ctx.startTime = g.StartTime
		ctx.endTime = g.EndTime
		ctx.maing = goid
		ctx.gs = relatedGoroutines(goid)
	}
...
func (ctx *traceContext) generateTrace() {
	ctx.w.Write([]byte("{\"traceEvents\": [\n"))
	...
	for _, ev := range events {
		...
		if ctx.gs != nil && ev.P < trace.NetpollP && !ctx.gs[ev.G] {
			continue
		}
	...
	fmt.Fprintf(ctx.w, `{"name": "process_name", "ph": "M", "pid": 0, "args": { "name" : "PROCS" }},`)
	fmt.Fprintf(ctx.w, `{"name": "process_sort_index", "ph": "M", "pid": 0, "args": { "sort_index" : 1 }},%v`, "\n")
	fmt.Fprintf(ctx.w, `{"name": "process_name", "ph": "M", "pid": 1, "args": { "name" : "STATS" }},`)
	fmt.Fprintf(ctx.w, `{"name": "process_sort_index", "ph": "M", "pid": 1, "args": { "sort_index" : 0 }},%v`, "\n")

	fmt.Fprintf(ctx.w, `{"name": "thread_name", "ph": "M", "pid": 0, "tid": %v, "args": { "name" : "network" }},%v`, trace.NetpollP, "\n")
	fmt.Fprintf(ctx.w, `{"name": "thread_sort_index", "ph": "M", "pid": 0, "tid": %v, "args": { "sort_index" : -5 }},%v`, trace.NetpollP, "\n")

	fmt.Fprintf(ctx.w, `{"name": "thread_name", "ph": "M", "pid": 0, "tid": %v, "args": { "name" : "timers" }},%v`, trace.TimerP, "\n")
	fmt.Fprintf(ctx.w, `{"name": "thread_sort_index", "ph": "M", "pid": 0, "tid": %v, "args": { "sort_index" : -4 }},%v`, trace.TimerP, "\n")

	fmt.Fprintf(ctx.w, `{"name": "thread_name", "ph": "M", "pid": 0, "tid": %v, "args": { "name" : "syscalls" }},%v`, trace.SyscallP, "\n")
	fmt.Fprintf(ctx.w, `{"name": "thread_sort_index", "ph": "M", "pid": 0, "tid": %v, "args": { "sort_index" : -3 }},%v`, trace.SyscallP, "\n")
	...
	if ctx.gtrace && ctx.gs != nil {
		for k, v := range gnames {
			...
			fmt.Fprintf(ctx.w, `{"name": "thread_name", "ph": "M", "pid": 0, "tid": %v, "args": { "name" : "%v" }},%v`, k, v, "\n")
		}
		fmt.Fprintf(ctx.w, `{"name": "thread_sort_index", "ph": "M", "pid": 0, "tid": %v, "args": { "sort_index" : -2 }},%v`, ctx.maing, "\n")
		fmt.Fprintf(ctx.w, `{"name": "thread_sort_index", "ph": "M", "pid": 0, "tid": 0, "args": { "sort_index" : -1 }},%v`, "\n")
	}
