...
func (s *ssaState) exprAddr(n *Node) *ssa.Value {
	switch n.Op {
	case ONAME:
		if n.Class == PEXTERN {
			// global variable
	...
	default:
		log.Fatalf("exprAddr: bad op %v", n.Op)
		return nil
	}
}
...
func (s *ssaState) nilCheck(ptr *ssa.Value) {
...
// Starts a new block on return.
func (s *ssaState) boundsCheck(idx, len *ssa.Value) {
