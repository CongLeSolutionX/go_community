...
func cgen_slice(n, res *Node, wb bool) {
	...
	compare := func(n1, n2 *Node) {
		p := Thearch.Ginscmp(OGT, Types[TUINT], n1, n2, -1)
		panics = append(panics, p)
	}
	...
	if i.Op != 0 {
		...
		if same(&i, &j) {
			...
			case OREGISTER:
				if i.Op == OLITERAL {
					ginscon(Thearch.Optoas(OSUB, Types[TUINT]), Mpgetfix(i.Val.U.Xval), &j)
				} else {
					gins(Thearch.Optoas(OSUB, Types[TUINT]), &i, &j)
				}
			}
		}
		...
		if k.Op == 0 {
			...
			case OREGISTER:
				if same(&i, &k) {
					...
					ginscon(Thearch.Optoas(OSUB, Types[TUINT]), Mpgetfix(i.Val.U.Xval), &k)
				} else {
					gins(Thearch.Optoas(OSUB, Types[TUINT]), &i, &k)
				}
			}
		}
	}
	...
	if !adjustBase && !needFullUpdate {
		...
		if i.Op != 0 && adjustBase {
			...
			if Isconst(size, CTINT) {
				...
				p = Thearch.Ginscmp(OEQ, Types[TUINT], size, &tmp, -1)
			}
			var w int64
			if n.Op == OSLICESTR {
