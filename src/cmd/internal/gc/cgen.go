...
func cgen_append(n, res *Node) {
	...
	Thearch.Gins(Thearch.Optoas(OADD, Types[TUINT]), Nodintconst(int64(count(n.List)-1)), &r1)
	var r2 Node
	Regalloc(&r2, Types[TUINT], nil)
	Thearch.Gmove(cap, &r2)
	Thearch.Gins(Thearch.Optoas(OCMP, Types[TUINT]), &r1, &r2)
	p := Gbranch(Thearch.Optoas(OLE, Types[TUINT]), nil, +1)
	// Note: rlen and src are Regrealloc'ed below at the target of the
	// branch we just emitted; do not use for other purposes.
	rlen := r1
	Regfree(&r2)
	Regfree(&src)
