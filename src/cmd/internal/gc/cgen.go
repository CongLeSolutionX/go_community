	...
	case OINDEX:
		if Ctxt.Arch.Thechar == '5' {
			...
			if w == 0 {
				...
				if w&(w-1) == 0 {
					...
					Nodconst(&n1, Types[TUINT32], int64(log2(uint64(w))))
					Thearch.Gmove(&n1, &n4)
					Thearch.Gins(Thearch.Optoas(OLSH, Types[TUINT32]), &n4, &n2)
				} else {
		...
		if Ctxt.Arch.Thechar == '8' {
			...
			if w == 0 {
				...
				if w&(w-1) == 0 {
					// Power of 2.  Use shift.
					Nodconst(&tmp, Types[TUINT32], int64(log2(uint64(w))))
					Thearch.Gins(Thearch.Optoas(OLSH, Types[TUINT32]), &tmp, &n2)
				} else {
					// Not a power of 2.  Use multiply.
