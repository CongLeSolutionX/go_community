...
var zeroLevel Level = Level{}

func intFrom(l Level) int32 {
	neg := l.neg
	pos := l.pos
	sum := neg + pos
	return int32(sum)
...
func levelFrom(i int32) Level {
	i16 := int16(i)
	neg := i16 & (i16 >> 15)  // zero if i16 >= 0
	pos := i16 &^ (i16 >> 15) // zero if i16 < 0
	return Level{neg: neg, pos: pos}
}

func (l1 Level) lt(l2 Level) bool {
	return intFrom(l1) < intFrom(l2)
}

func (l1 Level) le(l2 Level) bool {
	return intFrom(l1) <= intFrom(l2)
}

func (l1 Level) ge(l2 Level) bool {
	return intFrom(l1) >= intFrom(l2)
}

func (l1 Level) gt(l2 Level) bool {
	return intFrom(l1) > intFrom(l2)
}

func inc(l Level, forDeref bool) Level {
	neg := l.neg
	pos := l.pos
	sum := neg + pos
	if sum > MinLevel {
		// only record pending dereferences, not slices/indices
		if forDeref || sum >= 0 {
			return Level{neg: neg, pos: pos + 1}
		}
		return Level{neg: sum + 1, pos: 0}
	}
	return Level{neg: MinLevel}
}

func (l Level) laterDereferenced() bool {
	return l.pos > 0
}

func dec(l Level) Level {
	neg := l.neg
	pos := l.pos
	sum := neg + pos
	// Decrements are normalizing to remove "laterDerefenced-ness"
	if sum > MinLevel {
		sum--
		if sum < 0 {
			return Level{neg: sum}
		}
		return Level{pos: sum}
	}
	return Level{neg: MinLevel}
}
...
var maxEncodedLevel Level = Level{pos: int16(bitsMaskForTag - 1)} // The largest level that can be stored in a tag.
...
func escNoteOutputParamFlow(e uint16, vargen int32, level Level) uint16 {
	...
	if level.lt(zeroLevel) { // was <-1 but how can you do this except w/ a heap allocation?
		return EscHeap
	}
	if level.gt(maxEncodedLevel) {
		// Cannot encode larger values than maxEncodedLevel.
		level = maxEncodedLevel
	}
	encoded := uint16(intFrom(level) + 1)
...
func inToOut(dst, src *Node) bool {
	...
	return dst.Op == ONAME && dst.Class == PPARAMOUT &&
		src.Op == ONAME && src.Class&^PHEAP == PPARAM && src.Curfn == dst.Curfn
}

func escwalk(e *EscState, level Level, dst *Node, src *Node) {
	if src.Walkgen == walkgen && src.Esclevel.le(level) {
		return
	}
	src.Walkgen = walkgen
	src.Esclevel = level
	...
	var leaks bool
	if inToOut(dst, src) && src.Esc != EscScope && src.Esc != EscHeap && dst.Esc != EscHeap {
		// This case handles:
		// 1. return in
		// 2. return &in
		// 3. tmp := in; return &tmp
		// 4. return *in
		if Debug['m'] != 0 {
			Warnl(int(src.Lineno), "leaking param: %v to result %v level=%v", Nconv(src, obj.FmtShort), Sconv(dst.Sym, 0), intFrom(level))
		}
	...
	if dst.Esc == EscHeap &&
		src.Op == ONAME && src.Class&^PHEAP == PPARAM && src.Esc != EscHeap &&
		level.gt(zeroLevel) { // || level == 0 && !ispointy(src.Type)) {
	...
	leaks = level.le(zeroLevel) && dst.Escloopdepth < src.Escloopdepth

	switch src.Op {
	case ONAME:
		if src.Class&^PHEAP == PPARAM && (leaks || dst.Escloopdepth < 0) && src.Esc != EscHeap {
			src.Esc = EscScope
			if Debug['m'] != 0 {
	...
	case OPTRLIT, OADDR:
		if leaks && !level.laterDereferenced() {
			src.Esc = EscHeap
			ee := uint16(EscHeap)
			...
			osrc := outervalue(src.Left)
			if inToOut(dst, osrc) && osrc.Esc != EscScope && osrc.Esc != EscHeap && dst.Esc != EscHeap {
				ee = EscReturn
			}
			addrescapes(src.Left, ee)
			if Debug['m'] != 0 {
				p := src
	...
	case ODOTPTR, OINDEXMAP, OIND:
		// Being very conservative here with recording pending/cancelling dereference.
		newlevel := inc(level, src.Op != OINDEX)
