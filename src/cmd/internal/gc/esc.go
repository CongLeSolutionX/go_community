...
func (l Level) int() int {
	// How does this interact with function summary computation?
	return int(l.value)
}
...
func (l1 Level) min(l2 Level) Level {
	return Level{
		value:   min16(l1.value, l2.value),
		current: min16(l1.current, l2.current),
		max:     min16(l1.max, l2.max)}
}

// net returns the net dereference level at this point,
// which is the sum of max-dereferences-after-copy-suffix
// and the running sum of increments(dereference) and
// decrements(address-of).
func (l Level) net() int {
	suffix := l.max + l.current
	if suffix < 0 {
		suffix = 0
	}
	return int(l.value + suffix)
}
...
func escwalk(e *EscState, level Level, dst *Node, src *Node) {
	...
	var leaks bool
	if funcOutputAndInput(dst, src) && src.Esc != EscScope && src.Esc != EscHeap && dst.Esc != EscHeap {
		// This case handles:
		// 1. return in
		// 2. return &in
		// 3. tmp := in; return &tmp
	...
	leaks = level.net() <= 0 && dst.Escloopdepth < src.Escloopdepth
	...
	case OPTRLIT, OADDR:
		if leaks {
			...
			if funcOutputAndInput(dst, osrc) && osrc.Esc != EscScope && osrc.Esc != EscHeap && dst.Esc != EscHeap {
				ee = EscReturn
			}
