...
type Level struct {
	...
	//
	// current and max are used to compute the maximum-copy-started-suffix-level
	// applied to a sink.  For example:
...
func (l Level) inc() Level {
	...
	return Level{value: satInc8(l.value), current: satInc8(l.current), max: l.max}
}
...
func (l Level) dec() Level {
	...
	return Level{value: l.value - 1, current: l.current - 1, max: l.max}
}

func (l Level) copy() Level {
	prefix := satAdd8(l.max, l.current)
	if prefix < 0 {
		prefix = 0
	}
	return Level{value: l.value, current: 0, max: prefix}
}

func (l1 Level) min(l2 Level) Level {
	return Level{
		value:   min8(l1.value, l2.value),
		current: min8(l1.current, l2.current),
		max:     min8(l1.max, l2.max)}
}
...
func escNoteOutputParamFlow(e uint16, vargen int32, level Level) uint16 {
	...
	if level.int() < 0 { // was <-1 but how can you do this except w/ a heap allocation?
		if level.guaranteedDereference() <= 0 {
			return EscHeap
		}
		return e | EscContentEscapes // At least one deref, thus only content.
	}
...
// funcOutputAndInput reports whether src and dst correspond to input and output parameters of the same function.
...
func escwalk(e *EscState, level Level, dst *Node, src *Node) {
	...
	if src.Walkgen == walkgen {
		...
		// convergence.
		if src.Esclevel == src.Esclevel.min(level) {
			return
		} else {
			level = src.Esclevel.min(level)
		}
	}
	...
	var leaks bool
	if funcOutputAndInput(dst, src) && src.Esc&EscMask != EscScope && src.Esc&EscMask != EscHeap && dst.Esc&EscMask != EscHeap {
		// This case handles:
		// 1. return in
		// 2. return &in
		// 3. tmp := in; return &tmp
	...
	if dst.Esc&EscMask == EscHeap &&
		src.Op == ONAME && src.Class&^PHEAP == PPARAM && src.Esc&EscMask != EscHeap &&
		level.int() > 0 {
		src.Esc = EscContentEscapes | escMax(src.Esc, EscNone)
		if Debug['m'] != 0 {
	...
	switch src.Op {
	case ONAME:
		if src.Class&^PHEAP == PPARAM && (leaks || dst.Escloopdepth < 0) && src.Esc&EscMask != EscHeap {
			if level.guaranteedDereference() > 0 {
				src.Esc = EscContentEscapes | escMax(src.Esc, EscNone)
				if Debug['m'] != 0 {
					if Debug['m'] == 1 {
	...
	case OPTRLIT, OADDR:
		if leaks {
			...
			if funcOutputAndInput(dst, osrc) && osrc.Esc&EscMask != EscScope && osrc.Esc&EscMask != EscHeap && dst.Esc&EscMask != EscHeap {
				ee = EscReturn
			}
