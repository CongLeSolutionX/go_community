...
type EscState struct {
	...
	I2Oescape uint16    // escape value for parameter transmission (EscReturn or EscHeap)
}
...
func escassignfromtag(e *EscState, note *string, dsts *NodeList, src *Node) uint16 {
	...
	if em&EscContentEscapes != 0 {
		escassign(e, &e.theSink, Nod(OIND, src, nil))
	}
...
func escwalk(e *EscState, level int, dst *Node, src *Node) {
	...
	case OCALLMETH, OCALLFUNC, OCALLINTER:
		if src.Escretval != nil {
			if Debug['m'] > 1 {
				fmt.Printf("%v:[%d] %v escwalk replace: %v with %v\n", Ctxt.Line(int(lineno)), e.loopdepth,
					Nconv(src, obj.FmtShort), Nconv(src.Escretval.N, obj.FmtShort))
			}
			src = src.Escretval.N
		}
	}
