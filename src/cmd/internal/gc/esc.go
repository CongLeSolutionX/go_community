...
// symOrNil returns n.Nname.Sym if no nils are encountered along the way.
func symOrNil(n *Node) *Sym {
...
// curfnSymOrNil returns n.Curfn.Nname.Sym if no nils are encountered along the way.
func curfnSymOrNil(n *Node) *Sym {
	return symOrNil(n.Curfn)
}
...
func escAnalyze(all *NodeList, recursive bool) {
	...
	if Debug['m'] != 0 {
		for l := e.noesc; l != nil; l = l.Next {
			if l.N.Esc == EscNone {
				Warnl(int(l.N.Lineno), "%v %v does not escape", curfnSymOrNil(l.N), Nconv(l.N, obj.FmtShort))
			}
		}
	}
}
...
func esc(e *EscState, n *Node, up *Node) {
	...
	if Debug['m'] > 1 {
		fmt.Printf("%v:[%d] %v esc: %v\n", Ctxt.Line(int(lineno)), e.loopdepth, symOrNil(Curfn), n)
	}
	...
	case OAS, OASOP, OASWB:
		if (n.Left.Op == OIND || n.Left.Op == ODOTPTR) && n.Left.Left.Op == ONAME && // dst is ONAME dereference
			...
			if Debug['m'] != 0 {
				Warnl(int(n.Lineno), "%v ignoring self-assignment to %v", curfnSymOrNil(n), Nconv(n.Left, obj.FmtShort))
			}
	...
	case OAPPEND:
		if !n.Isddd {
			...
			escassign(e, &e.theSink, addDereference(slice2)) // lose track of assign to dereference
			if Debug['m'] > 2 {
				Warnl(int(n.Lineno), "%v special treatment of append(slice1, slice2...) %v", curfnSymOrNil(n), Nconv(n, obj.FmtShort))
			}
		}
...
func escassign(e *EscState, dst *Node, src *Node) {
	...
	if Debug['m'] > 1 {
		fmt.Printf("%v:[%d] %v escassign: %v(%v)[%v] = %v(%v)[%v]\n",
			Ctxt.Line(int(lineno)), e.loopdepth, symOrNil(Curfn),
...
func escflood(e *EscState, dst *Node) {
	...
	if Debug['m'] > 1 {
		fmt.Printf("\nescflood:%d: dst %v scope:%v[%d]\n", walkgen, Nconv(dst, obj.FmtShort), curfnSymOrNil(dst), dst.Escloopdepth)
	}
...
func escwalk(e *EscState, level Level, dst *Node, src *Node) {
	...
	if Debug['m'] > 1 {
		fmt.Printf("escwalk: level:%d depth:%d %.*s op=%v %v(%v) scope:%v[%d]\n",
			level, e.pdepth, e.pdepth, "\t\t\t\t\t\t\t\t\t\t", Oconv(int(src.Op), 0), Nconv(src, obj.FmtShort), Jconv(src, obj.FmtShort), curfnSymOrNil(src), src.Escloopdepth)
	}
