 ...
 */
// func _mpsetexp(a *Mpflt, exp int) {
...
func mpgetfltN(a *Mpflt, prec int, bias int) float64 {
	...
	case 53:
		x, _ = a.Val.Float64()
	case 24:
		// This code is not correct if a number a can only be represented
		// as a denormal float32 because in that case, rounding must be
		// done to the actual number of bits present. At the moment we're
		// ok because there appear to be no test cases checking for this.
		// It's also extremely rare to run into a problem in the wild.
		// TODO(gri) replace this with a.Val.Float32() once available
		var t Mpflt
