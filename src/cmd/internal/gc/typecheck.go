...
OpSwitch:
	...
	case OASOP,
		...
		if (op == ODIV || op == OMOD) && Isconst(r, CTINT) {
			if mpcmpfixc_(r.Val.U.Xval, 0) == 0 {
	...
	case OINDEX:
		...
		case TSTRING,
			...
			if Isconst(n.Right, CTINT) {
				x := Mpgetfix_(n.Right.Val.U.Xval)
				if x < 0 {
				...
				} else if Mpcmpfixfix_(n.Right.Val.U.Xval, Maxintval[TINT]) > 0 {
	...
	case OMAKE:
		...
		case TARRAY:
			...
			if Isconst(l, CTINT) && r != nil && Isconst(r, CTINT) && Mpcmpfixfix_(l.Val.U.Xval, r.Val.U.Xval) > 0 {
...
func checksliceconst(lo *Node, hi *Node) int {
	if lo != nil && hi != nil && lo.Op == OLITERAL && hi.Op == OLITERAL && Mpcmpfixfix_(lo.Val.U.Xval, hi.Val.U.Xval) > 0 {
...
func keydup(n *Node, hash []*Node) {
	...
	case CTINT,
		CTRUNE:
		b = uint32(Mpgetfix_(n.Val.U.Xval))
...
func indexdup(n *Node, hash []*Node) {
	...
	var c uint32
	for a := hash[h]; a != nil; a = a.Ntest {
		c = uint32(Mpgetfix_(a.Val.U.Xval))
		if b == c {
...
func checkmake(t *Type, arg string, n *Node) int {
	if n.Op == OLITERAL {
		...
		case CTINT,
			...
			CTCPLX:
			n.Val = toint(n.Val)
			if mpcmpfixc_(n.Val.U.Xval, 0) < 0 {
				Yyerror("negative %s argument in make(%v)", arg, Tconv(t, 0))
				return -1
			}

			if Mpcmpfixfix_(n.Val.U.Xval, Maxintval[TINT]) > 0 {
