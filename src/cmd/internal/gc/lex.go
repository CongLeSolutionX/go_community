...
func Main() {
	...
	if Thearch.Thechar == '6' {
		...
		obj.Flagcount("shared", "generate code that can be linked into a shared library (but assume all go code is in one such library; see -dynlink)", &flag_shared)
		flag.BoolVar(&flag_dynlink, "dynlink", false, "generate code that allows go code to be dynamically linked against go code")
	}
