...
var startCalled, maybeChildCalled bool
...
// do so. It should only be called by cmd/go, and only after Start and MaybeChild
// have already been called.
func MaybeParent() {
	if !startCalled || !maybeChildCalled {
		panic("MaybeParent must be called after Start and MaybeChild")
	}
	telemetry.Start(telemetry.Config{
		Upload:       true,
		TelemetryDir: os.Getenv("TEST_TELEMETRY_DIR"),
