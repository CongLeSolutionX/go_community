...
func PProf(c Completer, interactive **bool) Commands {
	return Commands{
		...
		"callgrind": {c, report.Callgrind, awayFromTTY("callgraph.out", interactive), false, "Outputs a graph in callgrind format"},
		"proto":     {c, report.Proto, awayFromTTY("pb.gz", interactive), false, "Outputs the profile in compressed protobuf format"},

		// Generate report in DOT format and postprocess with dot
		"gif": {c, report.Dot, invokeDot("gif", interactive), false, "Outputs a graph image in GIF format"},
		"pdf": {c, report.Dot, invokeDot("pdf", interactive), false, "Outputs a graph in PDF format"},
		"png": {c, report.Dot, invokeDot("png", interactive), false, "Outputs a graph image in PNG format"},
		"ps":  {c, report.Dot, invokeDot("ps", interactive), false, "Outputs a graph in PS format"},
		...
		"eog":    {c, report.Dot, invokeVisualizer(interactive, invokeDot("svg", nil), "svg", []string{"eog"}), false, "Visualize graph through eog"},
		"evince": {c, report.Dot, invokeVisualizer(interactive, invokeDot("pdf", nil), "pdf", []string{"evince"}), false, "Visualize graph through evince"},
		"gv":     {c, report.Dot, invokeVisualizer(interactive, invokeDot("ps", nil), "ps", []string{"gv --noantialias"}), false, "Visualize graph through gv"},
		...
		"weblist": {c, report.WebList, invokeVisualizer(interactive, awayFromTTY("html", nil), "html", browsers()), true, "Output annotated source in HTML for functions matching regexp or address"},
	}
}
...
// the screen.
func awayFromTTY(format string, interactive **bool) PostProcessor {
	return func(input *bytes.Buffer, output io.Writer, ui plugin.UI) error {
		if interactive != nil && **interactive {
			dir := os.Getenv("PPROF_TMPDIR")
			tempFile, err := tempfile.New(dir, "profile", "."+format)
			if err != nil {
				return err
			}
...
func invokeDot(format string, interactive **bool) PostProcessor {
	divert := awayFromTTY(format, interactive)
	return func(input *bytes.Buffer, output io.Writer, ui plugin.UI) error {
...
func saveSVGToFile() PostProcessor {
	generateSVG := invokeDot("svg", nil)
	divert := awayFromTTY("svg", nil)
	return func(input *bytes.Buffer, output io.Writer, ui plugin.UI) error {
