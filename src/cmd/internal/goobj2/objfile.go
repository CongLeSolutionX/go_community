...
func (r *Reader) Autolib() []ImportedPkg {
	const importSize = stringRefSize + 8
	n := (r.h.Offsets[BlkAutolib+1] - r.h.Offsets[BlkAutolib]) / importSize
	s := make([]ImportedPkg, n)
	off := r.h.Offsets[BlkAutolib]
	for i := range s {
		pkg := r.StringRef(off)
		fingerprint := r.uint64At(off + stringRefSize)
		s[i] = ImportedPkg{pkg, fingerprint}
		off += importSize
	}
	return s
}
