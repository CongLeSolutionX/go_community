...
func TestSplitQuotedFieldsAndUnescape(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "quote_mixed", value: `'a '"b "`, want: []string{"a b "}},
		{name: "escape", value: `\'\"\\\a`, want: []string{`'"\a`}},
		{name: "quote_escape", value: `'a\' b'`, want: []string{"a' b"}},
		{name: "quote_unclosed", value: `'a`, wantErr: "unclosed quote"},
		{name: "escape_unfinished", value: `a\`, wantErr: "unfinished escaping"},
	} {
		t.Run(test.name, func(t *testing.T) {
			got, err := SplitQuotedFieldsAndUnescape(test.value)
			if err != nil {
				if test.wantErr == "" {
...
func TestJoinAndQuoteFields(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			got := JoinAndQuoteFields(test.args)
			if got != test.want {
