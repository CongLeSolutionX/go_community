...
func instinit(ctxt *obj.Link) {
	...
	wasm_pc_f_loop_until = ctxt.Lookup("wasm_pc_f_loop_until")
}
...
func genWasmExportWrapper(s *obj.LSym, appendp func(p *obj.Prog, as obj.As, args ...obj.Addr) *obj.Prog) {
	...
	// In the unwinding case, we call wasm_pc_f_loop_until to handle stack switch and rewinding,
	// until a normal return (non-unwinding) back to this function.
	p = appendp(p, AIf)
	p = appendp(p, AI32Const, retAddr)
	p = appendp(p, AI32Const, constAddr(16))
	p = appendp(p, AI32ShrU)
	p = appendp(p, ACall, obj.Addr{Type: obj.TYPE_MEM, Name: obj.NAME_EXTERN, Sym: wasm_pc_f_loop_until})
	p = appendp(p, AEnd)
