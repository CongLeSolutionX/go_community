...
func preprocess(ctxt *obj.Link, s *obj.LSym, newprog obj.ProgAlloc) {
	...
	for p != nil {
		...
		case obj.ACALL, ACALLNORESUME:
			...
			// jmpdefer removes the frame of deferreturn from the stack,
			// so the WebAssembly function for deferreturn needs to return without RET.
			...
			p = appendp(p, AIf)
			if call.As == ACALLNORESUME && call.To.Sym != sigpanic { // sigpanic never resumes
