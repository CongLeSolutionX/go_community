...
func init() {
	obj.RegisterRegister(MINREG, MAXREG-MINREG, rconv)
...
func preprocess(ctxt *obj.Link, s *obj.LSym, newprog obj.ProgAlloc) {
	...
	if s.Func.Text.From.Sym.Wrapper() {
		...
		p := s.Func.Text
		p = appendp(p, AMOVD, gpanic, regAddr(REG_I0))

		p = appendp(p, AGet, regAddr(REG_I0))
		p = appendp(p, AI64Eqz)
		p = appendp(p, ANot)
		p = appendp(p, AIf)
	...
	if numResumePoints > 0 {
		p := s.Func.Text
		p = appendp(p, ALoop) // entryPointLoop
	...
	for p != nil {
		...
		case obj.AJMP:
			...
			if jmp.To.Type == obj.TYPE_MEM {
				p = appendp(p, ACall, obj.Addr{Type: obj.TYPE_MEM, Sym: jmp.To.Sym})
			} else {
				// (target is on stack)
				p = appendp(p, AI32WrapI64)
		...
		case obj.ACALL, ACALLNORESUME:
			...
			// write return PC_B to Go stack
			p = appendp(p, AMOVD, constAddr(pcAfterCall), obj.Addr{Type: obj.TYPE_MEM, Reg: REG_SP})

			// write return PC_F to Go stack
			p = appendp(p, AGet, regAddr(REG_SP))
			p = appendp(p, AI32Const, obj.Addr{Type: obj.TYPE_BRANCH, Sym: s})
			p = appendp(p, AI32Store16, constAddr(2))
			...
			if call.To.Type == obj.TYPE_MEM {
				p = appendp(p, ACall, obj.Addr{Type: obj.TYPE_MEM, Sym: call.To.Sym})
			} else {
				// (target is on stack)
				p = appendp(p, AI32WrapI64)
				p = appendp(p, AI32Const, constAddr(16))
	...
	for p != nil {
		...
		case AAddrOf:
			...
			case obj.NAME_AUTO, obj.NAME_PARAM:
				...
				p = appendp(p, AGet, regAddr(REG_SP))
				p = appendp(p, AI64ExtendUI32)
				p = appendp(p, AI64Const, constAddr(addrOf.From.Offset))
				p = appendp(p, AI64Add)

			case obj.NAME_EXTERN:
				p = appendp(p, AI64Const, obj.Addr{Type: obj.TYPE_ADDR, Name: obj.NAME_EXTERN, Sym: addrOf.From.Sym})
...
func assemble(ctxt *obj.Link, s *obj.LSym, newprog obj.ProgAlloc) {
	...
	for p := s.Func.Text; p != nil; p = p.Link {
		...
		case AGet:
			...
			case obj.TYPE_REG:
				reg := p.From.Reg
				if reg >= REG_PC_F && reg <= REG_RET3 {
					w.WriteByte(0x23) // get_global
					writeUleb128(w, uint64(reg-REG_PC_F))
					continue
				}
				if reg >= REG_I0 && reg <= REG_F15 {
					w.WriteByte(0x20) // get_local
					writeUleb128(w, uint64(reg-REG_I0))
					continue
				}
				panic("bad reg")
			}
