...
func preprocess(ctxt *obj.Link, s *obj.LSym, newprog obj.ProgAlloc) {
	...
	if wi := s.Func().WasmImport; wi != nil {
		...
		if wi.Module == "gojs" {
			...
			for _, f := range wi.Params {
				// Each call to the load instructions will consume the value of sp on the stack, so
				...
				// Because of our call convention, the paramters are located an additional 8 bytes
		...
		// that our work isn't overwritten below.
		framesize = 0
...

	{
		...
		if len(unwindExitBranches) > 0 {
			...
			appendp(p, AI32Const, constAddr(1))
		}
	}
