...
// Rotate generates a new set of parameters with the rotation amount
// increased by the given value. The selected bits are left unchanged.
func (r RotateParams) Rotate(amount int8) RotateParams {
	r.Amount += amount
	r.Amount &= 63
	return r
}
...
// If the intersection is not contiguous nil will be returned.
