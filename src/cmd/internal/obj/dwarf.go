...
//
// The lines generated here aren't perfect -- with the file numbers not being
// wrong for the package level (compilation unit) level that the linker will
// write the symbol in the binary. The file numbers will be fixed up in the
// linker.
func (ctxt *Link) generateDebugLinesSymbol(s, lines *LSym) {
	...
	for !pcfile.Done && !pcline.Done {
		...
		if int32(file) != pcfile.Value {
			...
			dwarf.Uleb128put(dctxt, lines, int64(pcline.Value))
