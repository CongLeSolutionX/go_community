...
// DWARF section, saving that for the linker. This table just generates the
// state machine part of debug_lines. The full table is generate by the linker.
// It's worth noting that we use the file numbers from the full package
// (not just the function in question) when generating the state machine. We
// do this so we don't have to do a fixup on the indices when writing the full
// section.
func (ctxt *Link) generateDebugLinesSymbol(s, lines *LSym) {
	...
	for i, filename := range s.Func.Pcln.File {
		// TODO: Do we need to do anything special for inlined calls?
		if symbolIndex := ctxt.PosTable.FileIndex(filename); symbolIndex >= 0 {
			fileNums[int32(i)] = int64(symbolIndex)
		} else {
	...
	//   basic_block = false
	// Of these, address will be set at the beginning of every function.
	dctxt.AddUint8(lines, dwarf.DW_LNS_set_file)
	...
	dwarf.Uleb128put(dctxt, lines, 0)
	if is_stmt != 1 {
