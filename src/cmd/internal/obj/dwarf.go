...
func (c dwCtxt) LookupOrCreateSym(die *dwarf.DWDie, name string, st objabi.SymKind) dwarf.Sym {
	...
	ds.Type = st
	ds.Set(AttrDuplicateOK, true) //todo: maybe don't need dupok?
	return ds
}
...
func (ctxt *Link) PopulateDWARFType(typ dwarf.Type) {
	...
	dwarf.NewAttr(die, dwarf.DW_AT_go_runtime_type, dwarf.DW_CLS_GO_TYPEREF, 0, s)
	if typedefdie != nil {
		typeInfo.dwarfInfo[0] = typedefdie.Sym.(*LSym)
		typeInfo.dwarfInfo[1] = die.Sym.(*LSym)
	} else {
		typeInfo.dwarfInfo[0] = die.Sym.(*LSym)
		//todo: empty sym, make it contentaddress
		typeInfo.dwarfInfo[1] = &LSym{
			Type: objabi.SDWARFTYPE,
		}
	}
}
