...
func (ctxt *Link) PredefinedDwarfType() {
	ctxt.DwarfCtxt.enable = true
	ctxt.DwarfCtxt.fixTypes.Uintptr = ctxt.DwarfCtxt.DefGoTypeDefault(ctxt.LookupDwPredefined("uintptr"))
...
func (ctxt *Link) DumpDwarfTypes() {
	...
	lookupPrototype := func(name string) *dwarf.DWDie {
		...
		dwsym.Set(AttrDuplicateOK, true)
		die, _, err := dwarf.NewType(t, dwctxt, dwctxt.fixTypes, prototypeRoot)
		if err != nil {
			ctxt.Diag(err.Error())
			return nil
		}
	...
	dwarf.SynthesizeMapTypes(dwctxt, dwctxt.dwTypesRoot, lookupPrototype, dwctxt.fixTypes.Uintptr, ctxt.Arch.Arch)
