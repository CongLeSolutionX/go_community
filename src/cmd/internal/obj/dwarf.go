...
func (c dwCtxt) AddSectionOffset(s dwarf.Sym, size int, t interface{}, ofs int64) {
	...
	r := &ds.R[len(ds.R)-1]
	// This method is only for attr DW_AT_go_runtime_type in DWARF type dies.
	// We don't expect a type description to be marked reachable only for the dwarf info.
	// So use a weak reference for it.
...
func (c dwCtxt) ReferencePtr(dwtype dwarf.Sym) dwarf.Sym {
	...
	// It is always unnamed ptr type here, it can be dupok.
...
func (ctxt *Link) DelayPopulateDWARFType(typ dwarf.Type, dupok bool) dwarf.Sym {
	if ok := ctxt.delayTypes[typ]; !ok {
		ctxt.delayTypes[typ] = dupok
	}
	return ctxt.Lookup(dwarf.InfoPrefix + typ.Name())
}
...
func (ctxt *Link) DumpDwarfTypes(lookupRuntime func(name string) dwarf.Type, Uintptr dwarf.Type) {
	for typ, dupok := range ctxt.delayTypes {
		ctxt.PopulateDWARFType(typ, dupok)
	}
