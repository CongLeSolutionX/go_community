...
func materializeSImm(newprog obj.ProgAlloc, p *obj.Prog, imm int64, reg int16, overwriteP bool) {
	...
	// single-instruction cases with ADDIW or ORI
	// condition is equivalent to simmFits(imm, totalLowWidth) || imm > 0 && uimmFits(imm, totalLowWidth)
	if -0x800 <= imm && imm <= 0xfff {
		// if simmFits:
		// ADDIW $imm, ZERO, reg
		//
		// else, must have 2048 <= imm <= 4095
		// ORI $imm, ZERO, reg
...
type choppedSImm struct {
	...
	upper int32
	// 51..32 bits of target immediate, to be loaded with CU32ID.
	higher int32
	// 63..52 bits of target immediate, to be loaded with CU52ID.
	top int32
}
