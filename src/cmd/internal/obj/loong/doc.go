...
3. Go uses the MOV family of pseudo-instructions for register moves, memory accesses and constant
loads. Constant loads can only use the native-width form, that is MOV.

  Examples:
    MOV   R4, R5               <=>     or         $r5, $r4, $r0
    MOVB  R4, R5               <=>     ext.w.b    $r5, $r4
    MOVBU R4, R5               <=>     andi       $r5, $r4, 0xff
    MOVH  R4, R5               <=>     ext.w.h    $r5, $r4
    MOVHU R4, R5               <=>     bstrpick.d $r5, $r4, 15, 0
    MOVW  R4, R5               <=>     addi.w     $r5, $r4, 0
    MOVWU R4, R5               <=>     bstrpick.d $r5, $r4, 31, 0
...
There are some special cases to the general rule, though:
