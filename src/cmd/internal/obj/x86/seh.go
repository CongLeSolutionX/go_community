...
func (b *sehbuf) write16(v uint16) {
	b.ctxt.Arch.ByteOrder.PutUint16(b.data[b.off:], v)
	b.off += 2
}
...
func populateSeh(ctxt *obj.Link, s *obj.LSym) {
	...
	buf.write8(uint8(ops[len(ops)-1].Prog.Link.Pc)) // Size of prolog
	buf.write8(nodes)                               // Count of nodes
	if hasfp {
		buf.write8((fpoffset/16)<<4 | sehRegisters[fpreg])
	} else {
		buf.write8(0)
	}
	...
	for i := len(ops) - 1; i >= 0; i-- {
		...
		// instruction.
		buf.write8(uint8(p.Link.Pc))
		switch op {
		case sehOpSaveReg:
			buf.writecode(UWOP_PUSH_NONVOL, sehRegisters[p.From.Reg])
		case sehOpSetFP:
	...
	fn.SehUnwindInfoSym = ctxt.LookupInit("go:sehuw."+symname, func(s *obj.LSym) {
