...
func init() {
	...
	for i := range obj.X86suffixes {
		suffix := newEVEXSuffix()
		parts := strings.Split(obj.X86suffixes[i], ".")
		for j := range parts {
			switch parts[j] {
			case "Z":
				suffix.zeroing = true
...
const (
	_            rlistKind = iota // Unrecognized kind (never set explicitly)
	rlistInvalid                  // Unknown range kind
	rlistXmm                      // [Xa-Xb] range
	rlistYmm                      // [Ya-Yb] range
	rlistZmm                      // [Za-Zb] range
)

// evexRlist is decoded obj.Addr.Offset field for TYPE_REGLIST.
// See EncodeRegisterRange and decodeRegisterRange.
type evexRlist struct {
	kind rlistKind
	lo   int16
	hi   int16
}

// Reg0 returns low register constant.
// Shouldn't be used with lists of invalid kind.
func (list evexRlist) Reg0() int {
	return int((list.kind-rlistXmm)*32) + int(list.lo) + REG_X0
}

// Reg1 returns high register constant.
// Shouldn't be used with lists of invalid kind.
func (list evexRlist) Reg1() int {
	return int((list.kind-rlistXmm)*32) + int(list.hi) + REG_X0
}
...
func EncodeRegisterRange(reg0, reg1 int16) int64 {
	regIsXmm := func(r int16) bool { return r >= REG_X0 && r <= REG_X31 }
	regIsYmm := func(r int16) bool { return r >= REG_Y0 && r <= REG_Y31 }
	regIsZmm := func(r int16) bool { return r >= REG_Z0 && r <= REG_Z31 }

	// TODO(quasilyte): when oclass register case is refactored into
	// lookup table, use it here to get register kind more easily.
	// Helper functions should go away too (they will become redundant).
	kind := rlistInvalid
	switch {
	...
	lo := int64(regIndex(reg0))
	hi := int64(regIndex(reg1))
	list := (lo << 0) +
		(hi << 5) +
		(int64(kind) << 10)
	return list << 16
...
// decodeRegisterRange decodes 64-bit value created by EncodeRegisterRange.
func decodeRegisterRange(list int64) evexRlist {
	list >>= 16 // Skip ARM bits
	return evexRlist{
		lo:   int16((list >> 0) & 0x1F),     // [.........aaaaa]
		hi:   int16((list >> 5) & 0x1F),     // [....bbbbb.....]
		kind: rlistKind((list >> 10) & 0xF), // [kkkk..........]
	}
}
