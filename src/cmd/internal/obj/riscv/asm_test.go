...
func TestLargeJump(t *testing.T) {
	...
	if runtime.GOARCH != "riscv64" {
		t.Skip("Skipping riscv64-only test")
	}
	testenv.MustHaveGoBuild(t)
...
`
	...
	// Build generated files.
	build := testenv.Command(t, testenv.GoToolPath(t), "build")
	build.Dir = dir
...
func genLargeJump(buf *bytes.Buffer) {
	...
	for i := 0; i < 1<<19+1<<9; i++ { // This magic number will generate a negative offset for JALR.
		fmt.Fprintln(buf, "ADD $0, X0, X0")
	}
