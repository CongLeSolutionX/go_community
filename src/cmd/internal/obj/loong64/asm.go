	...
	for {
		...
		for p = prev.Link; p != nil; prev, p = p, p.Link {
			...
			if o.type_ == 6 && p.To.Target() != nil {
				...
				// BFPT, BFPT, -BEQ and -BNE is 21 bits, and the others are 16 bits. The jump
				// target address is to logically shift the immediate value in the instruction
				// code to the left by 2 bits and then sign extend.
				//
				// Note: -BEQ and -BNE here refer to BEQZ and BNEZ, and their corresponding GNU
				// assembly instructions are beqz and bnez.
				bound := int64(1 << (18 - 1))
