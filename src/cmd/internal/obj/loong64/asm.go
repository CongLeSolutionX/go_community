...
var optab = []Optab{
	...
	{AAMSWAPW, C_REG, C_NONE, C_NONE, C_REG, C_REG, 65, 4, 0, 0},
...
var atomicInst = map[obj.As]uint32{
	...
	AAMMAXWDBU: 0x070E0 << 15, // ammax_db.wu
	AAMMAXVDBU: 0x070E1 << 15, // ammax_db.du
	AAMMINWDBU: 0x070E2 << 15, // ammin_db.wu
	AAMMINVDBU: 0x070E3 << 15, // ammin_db.du
}

func IsAtomicInst(as obj.As) bool {
	if _, ok := atomicInst[as]; ok {
		return true
	}
	return false
}
...
func (c *ctxt0) asmout(p *obj.Prog, o *Optab, out []uint32) {
	...
	case 65: // am{swap,add,and,or,xor,max,min}[_db] rd, rk,rj
		// am{max,min}[_db].{wu,du} rd, rk,rj
		// rk: p.From.Reg,  rj: p.To.Reg,  rd: p.RegTo2
		rk := p.From.Reg
		rj := p.To.Reg
		rd := p.RegTo2
		o1 = atomicInst[p.As]
		o1 |= OP_RRR(o1, uint32(rk), uint32(rj), uint32(rd))

		// Note:
		// 1. If the AM* atomic instruction has the same register number as rd and rj,
		// the execution will trigger an Instruction Non-defined Exception.
		//
		// 2. If the AM* atomic instruction has the same register number as rd and rk,
