...
the corresponding PPC64 binary instructions as defined by the PowerPC64
...
is included here; refer to the PPC64 ISA if interested in more detail.
...
4. Loads and stores from memory
  ...
  is a memory reference, then it is a store; when the target is a register, then it
  is a load.
  ...
  loads or stores. When used with an update then the base register is updated by the value
...
4. Compares
  ...
  to be read by a later branch or isel. Within these fields, bits are set
  to indicate less than, greater than, or equal conditions.
...
1. Special register usage in Go asm
  ...
  VSn is used for vector-scalar registers. V and VS overlap: V0 == VS32 (0-31)
