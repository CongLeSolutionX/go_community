...
6. Shift instructions
  ...
  compiler will add appropriate code to compare the shift value to achieve the
  the correct result, but if assembler does not add extra checking.

  Other non-simple shifts can have a mask and start bit to use in the Go assembler
  instruction. These shifts will generate the corresponding PPC64 assembler
  instruction to include the first bit and along with the range of the mask. See
  the ISA for more detail on these. There are a few examples below.
  ...
  In the original set of Go opcodes for PPC64, the shift opcodes did not map directly
  onto the PPC64 opcodes and as a result, the operands were not consistent. The shift
  instructions which require bit ranges are represented as bit masks in the Go assembler,
  but bit ranges and lengths in the PPC64 assembler. Here are a few examples where this
  happens.
  ...
  recommended to use these instead to avoid confusion.
