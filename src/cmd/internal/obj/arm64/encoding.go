...
func (c *ctxt7) encodeElm(p *obj.Prog, bin uint32, ag *obj.Addr, instIdx, oprIdx, elmIdx int, checks map[uint32]uint32) (uint32, bool) {
	...
	case sa_imm__imm5:
		...
		case AZCMPEQ, AZCMPGT, AZCMPGE, AZCMPLT, AZCMPLE, AZCMPNE, AZINDEX:
	...
	case sa_shift__immh_immb:
		...
		var shiftR bool
		if isShiftR(p.As) {
			// The right shift amount is in the range 1 to the destination element width in bits.
			arng, _, _ = DecodeIndex(p.To.Index)
			shiftR = true
		} else if isShiftL(p.As) {
			// The left shift amount is in the range 0 to the source element width in bits minus 1.
	...
	// Note: this element is also used by some SVE instructions and has
...
// Check that the arrangements in the source and destination are consistent
