...
func (c *ctxt7) encodeElm(p *obj.Prog, bin uint32, ag *obj.Addr, instIdx, oprIdx, elmIdx int, checks map[uint32]uint32) (uint32, bool) {
	...
	case Sa_xn_sp__Rn:
		...
		switch p.As {
		case AADDPL, AADDSPL, AADDSVL, AADDVL:
			enc = (reg & 0x1f) << 16
		default:
			enc = (reg & 0x1f) << 5
		}
	case Sa_xn_sp__Xn, Sa_wn_wsp__Rn:
		if reg == REG_R31 { // excluding the ZR register
			c.ctxt.Diag("illegal source register: %v\n", p)
			return 0, false
		}
	...
	case Sa_wn__Rn, Sa_xn_1__Rn, Sa_xn_2__Rn:
		enc = (reg & 0x1f) << 5
	case Sa_xn_1__Rn_and_Rm, Sa_wn_1__Rn_and_Rm, Sa_xs__Rn_and_Rm, Sa_ws__Rn_and_Rm:
	...
	case Sa_ws__Rs:
		...
		if ai.Class != AC_REG && ai.Class != AC_PAIR {
	...
	case Sa_imm__imm5:
		...
		imm5 := offset
		switch InstTab[instIdx].GoOp {
		case AZINDEX:
			// For INDEX  <Zd>.<T>, <R><n>, #<imm>, <imm> is encoded in bits 20-16, <imm> is the first operand.
			// For INDEX  <Zd>.<T>, #<imm>, <R><m>, <imm> is encoded in bits 9-5, <imm> is the second operand.
			// <imm> is the signed immediate operand, in the range -16 to 15.
			if imm5 < -16 || imm5 > 15 {
			...
			if oprIdx == 0 {
				...
				return (uint32(imm5) & 31) << 5, true
			}
		case AZCMPEQ, AZCMPGT, AZCMPGE, AZCMPLT, AZCMPLE, AZCMPNE:
			...
			return (uint32(imm5) & 31) << 16, true
		case ACCMN, ACCMNW, ACCMP, ACCMPW, AZLD1B, AZLD1SB, AZLDFF1B, AZLDFF1SB, AZPRFB, AZST1B:
			// Is a five bit unsigned (positive) immediate.
		case AZLD1H, AZLD1SH, AZLDFF1H, AZLDFF1SH, AZPRFH, AZST1H:
			...
			imm5 = imm5 >> 1
		case AZLD1W, AZLD1SW, AZLDFF1W, AZLDFF1SW, AZPRFW, AZST1W:
			// Is the optional unsigned immediate byte offset,
			...
			imm5 = imm5 >> 2
		case AZLD1D, AZLDFF1D, AZPRFD, AZST1D:
			// Is the optional unsigned immediate byte offset,
	...
	case Sa_imm5__Rt:
		...
		enc = uint32(offset)
	case Sa_imm6__option_2_option_0_S_Rt_2_0:
	...
	case Sa_shift__shift__ASR_LSL_LSR:
		...
		enc = ((shift - SHIFT_LL) & 3) << 22
	case Sa_shift__sh__LSL_0_LSL_12:
		sh := uint32(0)
		if offset&^0xfff == 0 {
			sh = 0
		} else if offset&^0xfff000 == 0 {
	...
	case Sa_op1_Cn_Cm_op2:
		...
		enc = uint32(offset)
	case Sa_at_op__op1_CRm_0_op2: // at_op encoded in op1:CRm<0>:op2, candidate values are truncated.
	...
	case Sa_brb_op__op2: // brb_op encoded in op2, candidate values are truncated.
	...
	case Sa_dc_op__op1_CRm_op2: // dc_op encoded in op1:CRm:op2, candidate values are truncated.
	...
	case Sa_ic_op__op1_CRm_op2: // ic_op encoded in op1:CRm:op2, candidate values are truncated.
		op := SpecialOperand(offset)
	...
	case Sa_tlbi_op__op1_CRn_CRm_op2: // tlbi_op encoded in op1:CRn:CRm:op2, candidate values are truncated.
		op, ok := sysInstTLBIDC[SpecialOperand(p.From.Offset)]
		if !ok || op.cn == 7 {
			c.ctxt.Diag("illegal argument: %v\n", p)
			return 0, false
		...
		enc = uint32(SYSARG4(int(op.op1), int(op.cn), int(op.cm), int(op.op2)))
	case Sa_tlbip_op__op1_CRn_CRm_op2: // tlbip_op encoded in op1:CRn:CRm:op2, candidate values are truncated.
	...
	case Sa_systemreg__o0_op1_CRn_CRm_op2:
		...
		enc = v
	case Sa_pstatefield__op1_op2_CRm: // pstatefield encoded in op1:op2:CRm, candidate values are truncated.
	...
	case Sa_option, Sa_option__CRm:
		bop := SpecialOperand(offset)
		if option, exist := barrierOpEnc[bop]; !exist || e == Sa_option && (bop == SPOP_SSBB || bop == SPOP_PSSBB) {
			c.ctxt.Diag("invalid option: %v\n", p)
			return 0, false
		} else {
			enc = option << 8
		}
	case Sa_targets__op2_2_1__omitted_c_j_jc:
...
var barrierOpEnc = map[SpecialOperand]uint32{
	SPOP_SY:    15,
	SPOP_ST:    14,
	SPOP_LD:    13,
	SPOP_ISH:   11,
