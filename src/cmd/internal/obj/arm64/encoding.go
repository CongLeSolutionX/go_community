...
func (c *ctxt7) isRegOrZR(p *obj.Prog, r int16) {
	if !(r >= REG_R0 && r <= REG_R31) {
		c.ctxt.Diag("illegal register %s: %v\n", rconv(int(r)), p)
	}
}
...
func (c *ctxt7) mem_optional_imm12_unsigned(p *obj.Prog, arg *arg, shift uint) uint32 {
	...
	if vs<<shift != arg.Offset {
		c.ctxt.Diag("odd offset %d: %v\n", arg.Offset, p)
	}
	if vs < 0 || vs >= (1<<12) {
		c.ctxt.Diag("offset %d out of range: %v\n", vs, p)
	}
...
func (c *ctxt7) mem_imm7_signed(p *obj.Prog, arg *arg, shift uint) uint32 {
	...
	if vo < -64<<shift || vo > 63<<shift || vo&(1<<shift-1) != 0 {
		c.ctxt.Diag("invalid offset %d: %v\n", vo, p)
	}
...
func (c *ctxt7) imm5___B_1__H_2__S_4__D_8_index__imm5_imm4__imm4lt30gt_1__imm4lt31gt_2__imm4lt32gt_4__imm4lt3gt_8_1(p *obj.Prog, arg *arg) (uint32, uint32) {
	...
	default:
		c.ctxt.Diag("invalid arrangement %v: %v\n", arrange(arng), p)
	}
	return imm5, imm4
}
...
func (c *ctxt7) immediate_8x8_a_b_c_d_e_f_g_h(p *obj.Prog, imm int64) uint32 {
	...
	for i := 0; i < 8; i++ {
		...
		if tmp == 0xff {
			...
			c.ctxt.Diag("invalid immediate %d: %v\n", imm, p)
			return 0
		}
	}
	return uint32(ret)
...
func (c *ctxt7) size_imm(p *obj.Prog, imm, mul int64) uint32 {
	...
	case mul << 3:
		return 3
	default:
		c.ctxt.Diag("invalid immediate %d, expected %d, %d, %d or %d: %v\n", imm, mul, mul<<1, mul<<2, mul<<3, p)
	}
	return 0
}
...
func (c *ctxt7) sz_Q___2S_00__4S_01__2D_11(p *obj.Prog, arng int) (uint32, uint32) {
	...
	case ARNG_2D:
		sz, Q = 1, 1
	default:
		c.ctxt.Diag("invalid arrangement specifier %s: %v\n", arrange(arng), p)
	}
	return sz, Q
}
...
func (c *ctxt7) encodeArg(p *obj.Prog, arg *arg, atyp argtype) uint32 {
	...
	case arg_Xns_mem_post_fixedimm_1:
		rm := arg.Offset
		if rm != 1 {
			c.ctxt.Diag("invalid post offset %d, expected 1: %v\n", rm, p)
		}
		c.isRegOrRSP(p, arg.Reg)
		return (uint32(arg.Reg) & 0x1f) << 5
	...
	case arg_Xns_mem_post_fixedimm_2:
		rm := arg.Offset
		if rm != 2 {
			c.ctxt.Diag("invalid post offset %d, expected 2: %v\n", rm, p)
		}
		c.isRegOrRSP(p, arg.Reg)
		return (uint32(arg.Reg) & 0x1f) << 5
	...
	case arg_Xns_mem_post_fixedimm_4:
		rm := arg.Offset
		if rm != 4 {
			c.ctxt.Diag("invalid post offset %d, expected 4: %v\n", rm, p)
		}
		c.isRegOrRSP(p, arg.Reg)
		return (uint32(arg.Reg) & 0x1f) << 5
	...
	case arg_Xns_mem_post_fixedimm_8:
		rm := arg.Offset
		if rm != 8 {
			c.ctxt.Diag("invalid post offset %d, expected 8: %v\n", rm, p)
		}
		c.isRegOrRSP(p, arg.Reg)
		return (uint32(arg.Reg) & 0x1f) << 5
	...
	case arg_Xns_mem_post_Xm:
		rm := arg.Index & 0x1f
		if rm == 31 {
			c.ctxt.Diag("invalid register ZR: %v\n", p)
		}
		c.isRegOrRSP(p, arg.Reg)
	...
	case arg_Vt_1_arrangement_B_index__Q_S_size_1:
		at := int((arg.Reg >> 5) & 15)
		if at != ARNG_B {
			c.ctxt.Diag("invalid arrangement specifier %v: %v\n", arrange(at), p)
		}
		index := arg.Index
		if index < 0 || index > 15 {
			c.ctxt.Diag("index %d out of range 0 to 15: %v\n", index, p)
		}
		Q := uint32(index>>3) & 1
	...
	case arg_Vt_1_arrangement_H_index__Q_S_size_1:
		...
		index := arg.Index
		if index < 0 || index > 7 {
			c.ctxt.Diag("index %d out of range 0 to 7: %v\n", index, p)
		}
		Q := uint32(index>>2) & 1
	...
	case arg_Vt_1_arrangement_S_index__Q_S_1:
		...
		index := arg.Index
		if index < 0 || index > 3 {
			c.ctxt.Diag("index %d out of range 0 to 3: %v\n", index, p)
		}
		Q := uint32(index>>1) & 1
	...
	case arg_Vt_1_arrangement_D_index__Q_1:
		...
		index := arg.Index
		if index < 0 || index > 1 {
			c.ctxt.Diag("index %d out of range 0 to 1: %v\n", index, p)
		}
		Q := uint32(index) & 1
	...
	case arg_Xm_shift__LSL_0__LSR_1__ASR_2__0_63:
		...
		amount := (arg.Offset >> 10) & 63
		if amount >= 64 {
			c.ctxt.Diag("shift amount out of range 0 to 64: %v\n", p)
		}
		shift := (arg.Offset >> 22) & 3
		if shift > 2 || shift < 0 {
			c.ctxt.Diag("unsupported shift operator: %v\n", p)
	...
	case arg_immediate_cmode__8_0__16_1:
		...
		if arg.Offset == 16 {
			...
			c.ctxt.Diag("invalid immediate %d, expected 8 or 16: %v\n", arg.Offset, p)
		}
	...
	case arg_immediate_exp_3_pre_4_imm8:
		...
		if rf < 0 {
			c.ctxt.Diag("invalid floating-point immediate %d: %v\n", rf, p)
		}
	...
	case arg_immediate_shift_64_implicit_imm16_hw,
		...
		hw := movcon(imm)
		if hw < 0 || hw > 3 {
			c.ctxt.Diag("invalid constant %d: %v\n", imm, p)
		}
	...
	case arg_immediate_0_7_op1:
		if (uint32(arg.Offset) &^ uint32(0x7)) != 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 7: %v\n", arg.Offset, p)
		}
	...
	case arg_immediate_0_127_CRm_op2:
		if (uint64(arg.Offset) &^ uint64(0x7F)) != 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 127: %v\n", arg.Offset, p)
		}
	...
	case arg_immediate_0_15_CRm, arg_immediate_optional_0_15_CRm:
		if (uint64(arg.Offset) &^ uint64(0xF)) != 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 15: %v\n", arg.Offset, p)
		}
	...
	case arg_immediate_0_65535_imm16, arg_immediate_optional_0_65535_imm16:
		if arg.Offset > 65535 || arg.Offset < 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 65535: %v\n", arg.Offset, p)
		}
	...
	case arg_immediate_MSL__a_b_c_d_e_f_g_h, arg_immediate_OptLSLZero__a_b_c_d_e_f_g_h:
		imm := (arg.Offset)
		if imm < -128 || imm > 255 {
			c.ctxt.Diag("immediate %d out of range -128 to 255: %v\n", imm, p)
		}
		return ((uint32(imm)>>5)&7)<<16 | (uint32(imm)&0x1f)<<5

	case arg_immediate_0_31_imm5:
		v := arg.Offset
		if (uint64(v) &^ uint64(0x1F)) != 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 31: %v\n", v, p)
		}
		return uint32(v) << 16

	case arg_immediate_0_15_nzcv:
		v := arg.Offset
		if (uint64(v) &^ uint64(0xF)) != 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 15: %v\n", v, p)
		}
		return uint32(v)
	...
	case arg_immediate_0_63_b5_b40:
		if (uint64(arg.Offset) &^ uint64(0x3F)) != 0 {
			c.ctxt.Diag("immediate %d out of range 0 to 63: %v\n", arg.Offset, p)
		}
	...
	case arg_immediate_floatzero:
		if arg.Val.(float64) != 0.0 {
			c.ctxt.Diag("immediate must be 0.0: %v\n", p)
		}
		return 0
	...
	case arg_immediate_index_Q_imm4__imm4lt20gt_00__imm4_10:
		...
		if af == ARNG_8B {
			...
			c.ctxt.Diag("invalid arrangement %v, should be B8 or B16: %v\n", arrange(af), p)
		}
		if index < 0 || index > max {
			c.ctxt.Diag("illegal offset %d: %v\n", index, p)
		}
	...
	case arg_prfop_Rt:
		...
		if arg.Type == obj.TYPE_CONST {
			v = uint32(arg.Offset)
			if v > 31 {
				c.ctxt.Diag("illegal prefetch operation %d: %v\n", v, p)
			}
		} else {
			val, ok := prfopfield[SpecialOperand(arg.Offset)]
			if !ok {
				c.ctxt.Diag("illegal prefetch operation %v: %v\n", SPCconv(arg.Offset), p)
			}
			v = val
		}
		return v & 31
	...
	case arg_sysreg_o0_op1_CRn_CRm_op2:
		...
		if v == 0 {
			c.ctxt.Diag("illegal system register %v: %v\n", rconv(int(arg.Reg)), p)
		}
		if (optab[uint32(p.Optab)].skeleton & (v &^ (3 << 19))) != 0 {
			c.ctxt.Diag("register value overlap: %v\n", p)
		}
		if p.As == AMRS && accessFlags&SR_READ == 0 {
			c.ctxt.Diag("system register %v is not readable: %v\n", rconv(int(arg.Reg)), p)
		} else if p.As == AMSR && accessFlags&SR_WRITE == 0 {
			c.ctxt.Diag("system register %v is not writable: %v\n", rconv(int(arg.Reg)), p)
		}
		return v
	...
	case arg_sysop_DC_SYS_CR_system, arg_sysop_TLBI_SYS_CR_system:
		...
		if !ok || (p.As == ATLBI && op.cn != 8) || (p.As == ADC && op.cn != 7) {
			c.ctxt.Diag("illegal argument %v: %v\n", SPCconv(arg.Offset), p)
		}
		if op.hasOperand2 {
			if arg.Index == 0 { // Xt is integrated in arg.Index.
	...
	case arg_pstatefield_op1_op2__SPSel_05__DAIFSet_36__DAIFClr_37:
		...
		if v == 0 {
			c.ctxt.Diag("illegal PSTATE field: %v\n", p)
		}
		return v
	...
	case arg_conditional:
		...
		if cond < SPOP_EQ || cond > SPOP_NV {
			c.ctxt.Diag("invalid condition %v: %v\n", SPCconv(arg.Offset), p)
		} else {
			cond -= SPOP_EQ
		}
		return uint32(cond)
