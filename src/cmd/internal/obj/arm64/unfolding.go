...
func (c *ctxt7) movCon32ToReg(con32 int64, rto int16, pos src.XPos) []inst {
	v := uint32(con32)
	// Rewrite the high 32-bit to be a repetition of the low
	// 32-bit, so that the BITCON test can be shared for both
	// 32-bit and 64-bit. 32-bit ops will zero the high 32-bit
	// of the destination register anyway.
	if isbitcon(uint64(v)<<32 | uint64(v)) {
		// -> MOVW $imm, rto
...
func bitwiseOp(c *ctxt7, p *obj.Prog, cidx, sidx uint16, neg, supportZR, op32 bool, movToReg func(int64, int16, src.XPos) []inst) []inst {
	tc := c.aclass(p, &p.To)
	if tc != C_REG {
		return nil
	}
...
func unfoldAND(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDxxi, ANDxxxs, false, p.To.Reg != REGZERO, false, c.movCon64ToReg)
}

func unfoldANDW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDwwi, ANDwwws, false, p.To.Reg != REGZERO, true, c.movCon32ToReg)
}

func unfoldEOR(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, EORxxi, EORxxxs, false, p.To.Reg != REGZERO, false, c.movCon64ToReg)
}

func unfoldEORW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, EORwwi, EORwwws, false, p.To.Reg != REGZERO, true, c.movCon32ToReg)
}

func unfoldORR(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ORRxxi, ORRxxxs, false, p.To.Reg != REGZERO, false, c.movCon64ToReg)
}

func unfoldORRW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ORRwwi, ORRwwws, false, p.To.Reg != REGZERO, true, c.movCon32ToReg)
}

func unfoldBIC(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDxxi, BICxxxs, true, p.To.Reg != REGZERO, false, c.movCon64ToReg)
}

func unfoldBICW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDwwi, BICwwws, true, p.To.Reg != REGZERO, true, c.movCon32ToReg)
}

func unfoldEON(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, EORxxi, EONxxxs, true, p.To.Reg != REGZERO, false, c.movCon64ToReg)
}

func unfoldEONW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, EORwwi, EONwwws, true, p.To.Reg != REGZERO, true, c.movCon32ToReg)
}

func unfoldORN(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ORRxxi, ORNxxxs, true, p.To.Reg != REGZERO, false, c.movCon64ToReg)
}

func unfoldORNW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ORRwwi, ORNwwws, true, p.To.Reg != REGZERO, true, c.movCon32ToReg)
}

func unfoldANDS(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDSxxi, ANDSxxxs, false, true, false, c.movCon64ToReg)
}

func unfoldANDSW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDSwwi, ANDSwwws, false, true, true, c.movCon32ToReg)
}

func unfoldBICS(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDSxxi, BICSxxxs, true, true, false, c.movCon64ToReg)
}

func unfoldBICSW(c *ctxt7, p *obj.Prog) {
	c.ctab[p] = bitwiseOp(c, p, ANDSwwi, BICSwwws, true, true, true, c.movCon32ToReg)
}
...
func unfoldMOVW(c *ctxt7, p *obj.Prog) {
	...
	case obj.TYPE_CONST:
		...
		} else if isbitcon(vbitcon) && p.To.Reg != REGZERO {
