...
func op2A(p *obj.Prog, idx uint16) []inst {
	return progToInst(p, idx, []arg{addrToOprd(p.To), addrToOprd(p.From)})
}
...
func op2ASO(p *obj.Prog, idx uint16) []inst {
	return progToInst(p, idx, []arg{addrToOprd(p.From), addrToOprd(p.To)})
}
...
func op2D3A(p *obj.Prog, idx uint16) []inst {
	a := []arg{{Reg: p.RegTo2, Type: obj.TYPE_REG}, addrToOprd(p.From), addrToOprd(p.To)}
...
func op2SA(p *obj.Prog, idx uint16) []inst {
	a := []arg{{Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(p.From)}
	return progToInst(p, idx, a)
}
...
func op3A(p *obj.Prog, idx uint16) []inst {
	...
	a := []arg{addrToOprd(p.To), {Reg: r, Type: obj.TYPE_REG}, addrToOprd(p.From)}
	return progToInst(p, idx, a)
}
...
func op3S4A(p *obj.Prog, idx uint16) []inst {
	a := []arg{addrToOprd(p.To), addrToOprd(*p.GetFrom3()), addrToOprd(p.From), {Reg: p.Reg, Type: obj.TYPE_REG}}
	return progToInst(p, idx, a)
}
...
func op4A(p *obj.Prog, idx uint16) []inst {
	a := []arg{addrToOprd(p.To), addrToOprd(*p.GetFrom3()), {Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(p.From)}
	return progToInst(p, idx, a)
}
...
func asmCall(c *ctxt7, p *obj.Prog) {
	...
	case C_SBRA:
		...
		c.ctab[p] = progToInst(p, BLl, []arg{addrToOprd(p.To)})
		p.Mark |= BRANCH26BITS
		if p.To.Sym != nil {
	...
	case C_REG, C_ZOREG:
		...
		c.ctab[p] = progToInst(p, BLRx, []arg{addrToOprd(p.To)})
...
func asmJMP(c *ctxt7, p *obj.Prog) {
	...
	case C_SBRA:
		// B label -> B <label>
		c.ctab[p] = progToInst(p, Bl, []arg{addrToOprd(p.To)})
		p.Mark |= BRANCH26BITS
		if p.To.Sym != nil {
			c.rtab[p] = newRelocation(c.cursym, 4, p.To.Sym, p.To.Offset, objabi.R_CALLARM64)
		}
	case C_REG, C_ZOREG:
		...
		c.ctab[p] = progToInst(p, BRx, []arg{addrToOprd(p.To)})
	}
}

func asmRET(c *ctxt7, p *obj.Prog) {
	// RET -> RET {<Xn>}
	c.ctab[p] = progToInst(p, RETx, []arg{addrToOprd(p.To)})
}
...
func asmBitFieldOps(c *ctxt7, p *obj.Prog) {
	...
	a := []arg{addrToOprd(p.To), {Reg: r, Type: obj.TYPE_REG}, addrToOprd(p.From), addrToOprd(*from3)}
...
func asmMOVD(c *ctxt7, p *obj.Prog) {
	...
	case obj.TYPE_CONST:
		...
		if movcon(v) >= 0 {
			...
			a := []arg{addrToOprd(p.To), {Type: obj.TYPE_CONST, Offset: ^v}}
	...
	case obj.TYPE_MEM:
		...
		case C_TLS_LE:
			...
			c.ctab[p] = progToInst(p, MOVZxis, []arg{addrToOprd(p.To), {Type: obj.TYPE_CONST}})
		case C_TLS_IE:
...
func asmMOVW(c *ctxt7, p *obj.Prog) {
	...
	case obj.TYPE_CONST:
		...
		if movcon(int64(v)) >= 0 {
			...
			a := []arg{addrToOprd(p.To), {Type: obj.TYPE_CONST, Offset: int64(^v)}}
...
func asmCondCMP(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{{Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(*from3), addrToOprd(p.To), addrToOprd(p.From)})
}
...
func asmCSELX(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{addrToOprd(p.To), {Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(*from3), addrToOprd(p.From)})
}
...
func asmFloatingCondCMP(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{addrToOprd(*from3), {Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(p.To), addrToOprd(p.From)})
}
func asmCLREX(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, CLREXi, []arg{addrToOprd(p.To)})
}
...
func asmDMBX(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{addrToOprd(p.From)})
}
...
func asmEXTRX(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{addrToOprd(p.To), addrToOprd(*from3), {Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(p.From)})
}
...
func asmREMX(c *ctxt7, p *obj.Prog) {
	...
	p1 := newInst(p1As, p1Idx, p.Pos, []arg{{Reg: REGTMP, Type: obj.TYPE_REG}, {Reg: r, Type: obj.TYPE_REG}, addrToOprd(p.From)})
...
func asmUnsignedExtend(c *ctxt7, p *obj.Prog) {
	...
	args := []arg{addrToOprd(p.To), {Reg: p.From.Reg, Type: obj.TYPE_REG},
...
func asmTBZX(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{{Reg: p.Reg, Type: obj.TYPE_REG}, addrToOprd(p.From), addrToOprd(p.To)})
}
...
func asmAtomicLoadOpStore(c *ctxt7, p *obj.Prog) {
	...
	args := []arg{addrToOprd(p.From), {Reg: p.RegTo2, Type: obj.TYPE_REG}, addrToOprd(p.To)}
...
func asmCASPX(c *ctxt7, p *obj.Prog) {
	...
	args := []arg{addrToOprd(p.From), addrToOprd(*to2), addrToOprd(p.To)}
	c.ctab[p] = progToInst(p, atomicIndex(p.As), args)
}
...
func asmBCOND(c *ctxt7, p *obj.Prog) {
	...
	p.Mark |= BRANCH19BITS
	args := []arg{{Offset: c.bCond(p.As)}, addrToOprd(p.To)}
...
func asmVMOV(c *ctxt7, p *obj.Prog) {
	...
	case C_ARNG:
		...
		c.ctab[p] = []inst{newInst(AVORR, ORRvvv_t, p.Pos, []arg{addrToOprd(p.To), {Reg: p.From.Reg, Type: obj.TYPE_REG}, addrToOprd(p.From)})}
	case C_ELEM:
		switch ttyp {
		case C_REG:
...
func asmVMOVQ(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, LDRql, []arg{{Reg: p.To.Reg - REG_V0 + REG_F0, Type: obj.TYPE_REG}, addrToOprd(p.From)})
}
...
func asmVMOVD(c *ctxt7, p *obj.Prog) {
	...
	// Adjust the registers used in Go assembly syntax to be consistent with the registers
	// used in Arm assembly syntax.
	c.ctab[p] = progToInst(p, LDRdl, []arg{{Reg: p.To.Reg - REG_V0 + REG_F0, Type: obj.TYPE_REG}, addrToOprd(p.From)})
}
...
func asmVMOVS(c *ctxt7, p *obj.Prog) {
	...
	// Adjust the registers used in Go assembly syntax to be consistent with the registers
	// used in Arm assembly syntax.
	c.ctab[p] = progToInst(p, LDRsl, []arg{{Reg: p.To.Reg - REG_V0 + REG_F0, Type: obj.TYPE_REG}, addrToOprd(p.From)})
}
...
func asmVTBL(c *ctxt7, p *obj.Prog) {
	...
	c.ctab[p] = progToInst(p, idx, []arg{addrToOprd(p.To), addrToOprd(*from3), addrToOprd(p.From)})
}
...
func asmVMOVI(c *ctxt7, p *obj.Prog) {
	...
	case C_ARNG:
		...
		if from3 != nil {
			...
			c.ctab[p] = progToInst(p, MOVIvii_ts, []arg{addrToOprd(p.To), addrToOprd(p.From), addrToOprd(*from3)})
			break
		}
		at := (p.To.Reg >> 5) & 15
		switch at {
