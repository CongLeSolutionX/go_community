...
func newRelocation(f *obj.LSym, size uint8, sym *obj.LSym, add int64, typ objabi.RelocType) *obj.Reloc {
	rel := obj.Addrel(f)
	rel.Siz = size
	rel.Sym = sym
	rel.Add = add
...
func generalStore(c *ctxt7, p *obj.Prog, toTyp argtype, eidx, uidx, pidx, widx, idx256 uint16, movToReg func(int64, int16, src.XPos) (*obj.Inst, *obj.Inst, int), checkUnpredicate bool) *obj.Inst {
	...
	case C_ADDR:
		...
		p.Reloc = newRelocation(c.cursym, 8, p.To.Sym, p.To.Offset, objabi.R_ADDRARM64)
...
func generalLoad(c *ctxt7, p *obj.Prog, fromTyp argtype, eidx, uidx, pidx, widx, idx256 uint16, movToReg func(int64, int16, src.XPos) (*obj.Inst, *obj.Inst, int), checkUnpredicate bool) *obj.Inst {
	...
	case C_ADDR:
		...
		p.Reloc = newRelocation(c.cursym, 8, p.From.Sym, p.From.Offset, objabi.R_ADDRARM64)
		p1 := adrp(REGTMP, p.Pos)
		p2 := instIRR(AADD, 0, REGTMP, REGTMP, ADDxxis, p.Pos)
...
func asmCall(c *ctxt7, p *obj.Prog) {
	...
	case C_SBRA:
		...
		p.Mark |= BRANCH26BITS
		if p.To.Sym != nil {
			p.Reloc = newRelocation(c.cursym, 4, p.To.Sym, p.To.Offset, objabi.R_CALLARM64)
		}
	case C_REG, C_ZOREG:
		// BL Rn -> BLR <Xn>
		// BL (Rn) or BL 0(Rn) -> BLR <Xn>
		p.Insts = convertToInst(p, BLRx, []obj.Addr{p.To})
		p.Reloc = newRelocation(c.cursym, 0, nil, 0, objabi.R_CALLIND)
	}
}
...
func asmMOVD(c *ctxt7, p *obj.Prog) {
	...
	case obj.TYPE_ADDR:
		...
		if ftyp == C_VCONADDR {
			// -> ADRP + ADD + reloc
			p.Reloc = newRelocation(c.cursym, 8, p.From.Sym, v, objabi.R_ADDRARM64)
	...
	case obj.TYPE_MEM:
		...
		case C_GOTADDR:
			...
			p.Reloc = newRelocation(c.cursym, 8, p.From.Sym, 0, objabi.R_ARM64_GOTPCREL)
			p1 := adrp(REGTMP, p.Pos)
		...
		case C_TLS_LE:
			...
			p.Reloc = newRelocation(c.cursym, 4, p.From.Sym, 0, objabi.R_ARM64_TLS_LE)
		...
		case C_TLS_IE:
			...
			p.Reloc = newRelocation(c.cursym, 8, p.From.Sym, 0, objabi.R_ARM64_TLS_IE)
			p1 := adrp(REGTMP, p.Pos)
			p2 := instLoad(p.As, REGTMP, p.To.Reg, 0, 0, LDRxx, p.Pos)
			p1.Link = p2
			p.Insts = p1
