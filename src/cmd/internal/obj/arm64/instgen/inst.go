...
func generateElements(outPutDir string) {
	path := path.Join(outPutDir, "elem_gen.go")
...
func generateInstructions(outPutDir string) {
	path := path.Join(outPutDir, "inst_gen.go")
	if _, err := os.Stat(path); err == nil {
...
package arm64`
	...
	for _, inst := range insts {
		for i := range inst.Classes.Iclass {
			...
			if feature != "" {
				feature = strings.ReplaceAll(feature, " && FEAT", "_") // Special case "FEAT_D128 && FEAT_THE"
...
func generateArm64Ops(dir string, arm64Ops map[string]struct{}) {
	path := path.Join(dir, "arm64ops_gen.go")
	if _, err := os.Stat(path); err == nil {
...
//go:generate stringer -type a64Type -trimprefix a`
	...
	fmt.Fprintf(w, "%saBEGIN a64Type = iota%s", tab1, next1)
...
func generateOps(dir string, goOps map[string]struct{}) {
	path := path.Join(dir, "goops_gen.go")
	if _, err := os.Stat(path); err == nil {
