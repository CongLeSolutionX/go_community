		...
		for i := range inst.Classes.Iclass {
			...
			for j := range iclass.Encodings {
				...
				for jj := 1; jj < len(enc.operands); jj++ {
					...
					if enc.operands[jj].typ == "" {
						log.Fatalf("empty operand type: %s\n", enc.asm)
					}
					// if _, ok := types[enc.operands[jj].typ]; !ok {
					// 	types[enc.operands[jj].typ] = make(map[string]int)
					// 	types[enc.operands[jj].typ][enc.operands[jj].name] = 1
					// } else {
					// 	if _, ok2 := types[enc.operands[jj].typ][enc.operands[jj].name]; !ok2 {
					// 		types[enc.operands[jj].typ][enc.operands[jj].name] = 1
					// 	} else {
					// 		types[enc.operands[jj].typ][enc.operands[jj].name]++
					// 	}
					// }
				}
