...
// instgen also accepts a download link of the manual by the -url flag,
// it will download and unpack the file. The link address is not fixed,
// so it's not hard coded in the program.
//
...
func main() {
	...
	log.Printf("len(rules) = %v\n", len(rules))

	// Step4: generate inst_gen.go, elem_gen.go, goops_gen.go and arm64ops_gen.go
	//
...
func download(dir string) string {
	log.Printf("download the xml files to %s\n", dir)
