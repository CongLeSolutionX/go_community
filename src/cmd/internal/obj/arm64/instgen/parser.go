	...
	for i := range inst.Classes.Iclass {
		...
		for j := 0; j < len(iclass.Encodings); j++ {
			...
			if !enc.parsed {
				...
				// There is a rule for this type of instruction: contain <V> but not <T[a-z]*>.
				...
				if wV.MatchString(enc.asm) && !woT.MatchString(enc.asm) {
					...
					idx := strings.LastIndex(ve.Name, "__")
					vals := strings.Split(ve.Name[idx+2:], "_")
				...
				var cloneWithDiffGoOp = func(op, suffix string) {
					clone := enc.clone()
					clone.suffix = suffix
...
func (enc *encoding) operandType(opr operand) string {
	...
	case "<prfop>", "<rprfop>", "<at_op>", "<brb_op>",
		"<dc_op>", "<tlbi_op>", "<tlbip_op>",
		"<option>nXS", "<option>", "{<option>}", "<ic_op>",
	...
	if fReg := regexp.MustCompile(`<V[a-z]?><[a-z]+>`); fReg.MatchString(name) {
