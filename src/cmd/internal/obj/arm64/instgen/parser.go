...
func (enc *encoding) ruleConsts() {
	...
	for i := 0; i < len(enc.operands); i++ {
		...
		case "AC_REGLIST2C", "AC_REGLIST4C":
			// { <Zd1>.B-<Zd2>.B }
			dotIdx := strings.Index(operand.name, ".")
			hyphenIdx := strings.Index(operand.name, "-")
			symbolName = operand.name[dotIdx+1 : hyphenIdx]
			if !strings.HasPrefix(symbolName, "<") {
				addRuleToTail(operand, symbolName)
			}
		case "AC_MEMIMM":
...
// Such operand names have the following structure: <reg>.<arng>[<index>]
