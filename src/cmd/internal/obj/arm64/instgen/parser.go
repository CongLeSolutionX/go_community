	...
	for i := range inst.Classes.Iclass {
		...
		for j := 0; j < len(iclass.Encodings); j++ {
			...
			if !enc.parsed {
				...
				if !enc.parseOperands(inst.Classes.Iclass[i]) {
					continue
				}
				enc.parsed = true
				...
				enc = splitNeonInstByWidth(iclass, j)

				// Some instructions contain an optional operand, splits them into two instruction formats.
				enc = splitOptionalOperand(iclass, j)

				// Combines several adjacent operands into a single operand, such as register list type.
				enc.combineOperands()

				// Go defines some alias instructions for the arm64 instruction.
				enc = cloneByAlias(iclass, j)
				...
				case "WHILEGE (predicate)", "WHILEGT (predicate)", "WHILEHI (predicate)",
					...
					clone.size = 32
					rEncodedLoc := 12
					if title == "CTERMEQ, CTERMNE" {
						rEncodedLoc = 22
					}
					clone.mask |= 1 << rEncodedLoc
					iclass.Encodings = append(iclass.Encodings, clone)
					enc = &iclass.Encodings[j]
					enc.binary |= 1 << rEncodedLoc
					enc.mask |= 1 << rEncodedLoc
				}

				// Some instructions contain an optional shift or extended operation,
				// splits them into two instruction formats. For example:
				// SUB <Wd>, <Wn>, <Wm>{, <shift> #<amount>} =>
				// SUB <Wd>, <Wn>, <Wm> and SUB <Wd>, <Wn>, <Wm>{, <shift> #<amount>}
				enc = splitRegShift(iclass, j)
				...
				enc = splitOperandWithPattern(iclass, j)
			...
			// If an opcode contains rules, then the opcode has different forms. For example:
			...
			enc = decodeOpcodeRules(iclass, j)
...
// splitNeonInstByWidth splits a NEON instruction into multiple instructions according to
// the operand width.
func splitNeonInstByWidth(iclass *iclass, j int) (enc *encoding) {
	enc = &iclass.Encodings[j]
	...
	// There is a rule found by naked eye observation for this type of instruction:
	// contain <V[a-z]*> but not .<T[a-z]*>.
	// But the portability is not good, a better method is to determine the encoding
	// of <V[a-z]*> through regdiagram. But it is a bit complicated.
	...
	// the element for <V> has the form like sa_v__sz__D_S, D and S are the possible values.
...
// splitOptionalOperand splits a few instructions with an optional operand into two instructions for each.
func splitOptionalOperand(iclass *iclass, j int) (enc *encoding) {
	enc = &iclass.Encodings[j]
	var clone encoding
	switch enc.operands[0].name {
	case "IRG":
...
func cloneByAlias(iclass *iclass, j int) (enc *encoding) {
	enc = &iclass.Encodings[j]
...
func splitRegShift(iclass *iclass, j int) (enc *encoding) {
	enc = &iclass.Encodings[j]
	isShift := strings.Contains(enc.asm, "{, <shift> #<amount>}")
	isExtended := strings.Contains(enc.asm, "{, <extend> {#<amount>}}")
	if !isShift && !isExtended {
		return
	}
	clone := enc.clone()
	if isShift {
...
func splitOperandWithPattern(iclass *iclass, j int) (enc *encoding) {
	enc = &iclass.Encodings[j]
...
func decodeOpcodeRules(iclass *iclass, j int) (enc *encoding) {
	enc = &iclass.Encodings[j]
...
//
// Decoding depends on the order of rules, so the order is important when adding
// these rules.
func (enc *encoding) ruleConsts() {
	prefix := "sa_const"
	symbolName := ""
	addRuleToIndex := func(operand *operand, symbolName string, idx int) {
		ruleName := prefix + "_" + strings.TrimPrefix(operand.typ, "AC_") + "_" + symbolName
		rule := element{Name: ruleName, Symbol: operand.name}
		if l := len(operand.rules); l >= idx {
			operand.rules = append(operand.rules, element{})
			copy(operand.rules[idx+1:l+1], operand.rules[idx:l])
		} else {
			for i := len(operand.rules); i <= idx; i++ {
				operand.rules = append(operand.rules, element{})
			}
		}
		operand.rules[idx] = rule
		addRule(rule)
	}
	addRuleToTail := func(operand *operand, symbolName string) {
		addRuleToIndex(operand, symbolName, len(operand.rules))
	}
	for i := 0; i < len(enc.operands); i++ {
		...
		case "AC_REGIDX":
			if strings.HasPrefix(operand.name, "ZT0[<") {
				symbolName = "ZT0"
				addRuleToIndex(operand, symbolName, 0)
			}
		case "AC_ARNGIDX": // <Vm>.H[<index>] or { <Vt>.B }[<index>]
			if !strings.Contains(operand.name, "{") {
				...
				if !strings.HasPrefix(arng, "<") {
					symbolName = arng
					addRuleToIndex(operand, symbolName, 1)
				}
				if !strings.HasPrefix(idx, "<") {
					symbolName = idx
					addRuleToIndex(operand, symbolName, 2)
				}
				continue
			}
			fallthrough
		case "AC_LISTIDX":
			...
			// The arrangement of AC_LISTIDX always seems to be a constant, so just place this
			// rule in the penultimate position of the rule slice.
			addRuleToIndex(operand, symbolName, len(operand.rules)-1)
		case "AC_REGLIST1":
			// { <Vn>.16B }
			dotIdx := strings.Index(operand.name, ".")
		...
		case "AC_REGLIST2", "AC_REGLIST3", "AC_REGLIST4":
			...
			// Only put one rule at the end of the rule slice.
			if !strings.HasPrefix(symbolName, "<") {
				addRuleToTail(operand, symbolName)
			}
		case "AC_MEMIMM":
		...
		case "AC_MEMEXT":
			...
			arng2Idx := 3
			if strings.Contains(operand.name, "(<Wm>|<Xm>)") {
				arng2Idx++
			}
			if !strings.HasPrefix(arng2, "<") {
				...
				addRuleToIndex(operand, symbolName, arng2Idx)
			}
			if !strings.HasPrefix(ext, "<") {
				...
				addRuleToIndex(operand, symbolName, arng2Idx+1)
			}
			if !strings.HasPrefix(amount, "<") {
				...
				addRuleToIndex(operand, symbolName, arng2Idx+2)
			}
		case "AC_MEMPREIMM":
		...
		case "AC_ZAVECTORIDXVG2", "AC_ZAVECTORIDXVG4":
			...
			symbolName = vg
			addRuleToIndex(operand, symbolName, 3)
		case "AC_ZAVECTORSELVG2", "AC_ZAVECTORSELVG4":
			arng, vg := parseZaVectorVg(operand.name)
			if !strings.HasPrefix(arng, "<") {
				symbolName = arng
				addRuleToIndex(operand, symbolName, 0)
			...
			symbolName = vg
			addRuleToIndex(operand, symbolName, 4)
		case "AC_ZAVECTORSEL":
...
// constructRuleName constructs a rule name in the format link__encodedIn__candidates.
// Some rules are post-processed as link__encodedIn_hbit_width__candidates because their
// meanings are not unique, so bits information is added. And a few rules will be specially
// processed, such as renaming, trucating.
func (enc *encoding) constructRuleName(link, encodedIn, candidates string, iclass iclass) string {
	linkEncodedIn := link
	if encodedIn != "" {
		linkEncodedIn += "__" + encodedIn
	}
	ruleName := linkEncodedIn
	...
	switch ruleName {
	// Refine some rule names to link__encodedIn_hbit_width__candidates, because they have diversed meanings and is
	// used in a lot of instructions.
	case "sa_zm__Zm", "sa_pg__Pg", "sa_zn1__Zn", "sa_zn2__Zn", "sa_t__tszh_tszl__B_D_H_S",
		"sa_zt2__Zt", "sa_zt1__Zt", "sa_pm__Pm", "sa_mod__xs__SXTW_UXTW", "sa_zt4__Zt",
		"sa_t__size__B_H_S", "sa_const__tszh_tszl_imm3", "sa_imm_1__i3h_i3l", "sa_imm__imm6",
		"sa_vm__Rm", "sa_offs__off3", "sa_zn4__Zn", "sa_zd1__Zd", "sa_zm1_1__Zm", "sa_zm1__Zm",
		"sa_zm2__Zm", "sa_zm4__Zm", "sa_imm__i3h_i3l", "sa_pn__Pn", "sa_index_1__i4h_i4l",
		"sa_index__i4h_i4l", "sa_index__i3h_i3l":
		fields := strings.Split(encodedIn, "_")
		ruleName = link + "_"
		for _, field := range fields {
			...
			for _, box := range iclass.Regdiagram.Boxes {
				if box.Name == field {
					...
					if box.HiBit == "" {
						log.Fatalf("box.HiBit is empty: %s\n", enc.Asmtemplate.TextA[0].Value)
					}
					ruleName += "_" + field + "_" + box.HiBit + "_" + width
					changed = true
				}
			}
			if !changed {
...
func (enc *encoding) parseOperands(iclass iclass) bool {
	...
	for m, ta := range enc.Asmtemplate.TextA {
		...
		if link := ta.Link; link != "" { // An <a> element
			...
			ruleName := enc.constructRuleName(link, encodedIn, candidates, iclass)
...
func (enc *encoding) arm64Opcode() {
	...
	// Add a prefix "a".
	enc.arm64Op = "a" + enc.operands[0].name
}
...
func separateByDash(str string) string {
	f := func(c rune) bool {
