...
to a specified boundary by padding with NOOP instruction. The alignment value supported
on arm64 must be a power of 2 and in the range [8, 2048].

  Examples:
    PCALIGN $16
    MOVD $2, R0          // This instruction will be aligned with a 16 bytes boundary.
    PCALIGN $1024
    MOVD $3, R1          // This instruction will be aligned with a 1024 bytes boundary.

The alignment of an instruction's address is relative to the start of the function where this
instruction is located, so the function's address must be aligned to the same or coarser boundary.
Therefore, PCALGIN will also change the function alignment. If a function has multiple PCALIGN
directives, its address will be aligned to the maximun of all the alignment values.

In the following example, Add() is aligned to a 128 byte boundary.
  Examples:
    TEXT ·Add(SB),$40-16
    MOVD $2, R0
    PCALIGN $32
...
In the following example, Add() is aligned to a 2048 byte boundary, and the assembler does not pad
NOOP instructions to align the first instruction "MOVD $1, R0".

  Examples:
    TEXT ·Add(SB),NOSPLIT|NOFRAME,$0-0
      PCALIGN $2048
      MOVD $1, R0
      MOVD $1, R1
      RET
