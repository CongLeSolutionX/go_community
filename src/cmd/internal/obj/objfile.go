...
func (w *writer) Aux(s *LSym) {
	...
	if typ := s.TypeInfo(); typ != nil {
		if typ.TypeDieSym != nil && typ.TypeDieSym.Size != 0 {
			w.aux1(goobj.AuxDwarfInfo, typ.TypeDieSym)
		}
		if typ.TypeDefDieSym != nil && typ.TypeDefDieSym.Size != 0 {
			w.aux1(goobj.AuxDwarfTypeDef, typ.TypeDefDieSym)
		}
	}
}
...
func genTypeInfoSyms(ctxt *Link) {
	...
	for _, s := range ctxt.Data {
		...
		dwsyms := []*LSym{typ.TypeDefDieSym, typ.TypeDieSym}
