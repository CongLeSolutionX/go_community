...
func (w *writer) Aux(s *LSym) {
	...
	if fn := s.Func(); fn != nil {
		...
		if fn.sehUnwindCodesSym != nil && fn.sehUnwindCodesSym.Size != 0 {
			w.aux1(goobj.AuxSehUnwindCodes, fn.sehUnwindCodesSym)
		}
...
func genFuncInfoSyms(ctxt *Link) {
	...
	for _, s := range ctxt.Text {
		...
		syms := []*LSym{fn.dwarfRangesSym, fn.dwarfLocSym, fn.dwarfDebugLinesSym, fn.dwarfInfoSym, fn.sehUnwindCodesSym}
		for _, s := range syms {
			if s == nil || s.Size == 0 {
				continue
			}
