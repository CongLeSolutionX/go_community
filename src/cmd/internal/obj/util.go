...
func AlignmentPaddingLength(pc int32, p *Prog, ctxt *Link) int {
	...
	// Use s for an alignment trigger offset.  If s is
	// non-zero and the bytes required to align pc are
	// less than or equal to s, then do the alignment,
	// otherwise not.
	// test, e.g., a = 16, s = 4, align for 12-15, not for 11.
	s := p.To.Offset
	if s == 0 || (pc64+s)&-a > pc64&-a {
		if pc64&(a-1) != 0 {
			return int(a - (pc64 & (a - 1)))
		}
	}
	return 0
}
