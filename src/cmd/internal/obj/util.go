...
func writeDconv(w io.Writer, p *Prog, a *Addr, abiDetail bool) {
	...
	case TYPE_REG:
		if conv.AconvReg != nil {
			fmt.Fprintf(w, "%s", conv.AconvReg(a))
			return
		}
	...
	case TYPE_MEM:
		if conv.AconvMem != nil {
			fmt.Fprintf(w, "%s", conv.AconvMem(a))
			return
		}
	...
	case TYPE_SHIFT:
		...
		fmt.Fprintf(w, "%s", conv.AconvShift(a))
...
var aconvMap map[string]AconvFuncList

func RegisterAconvFunc(arch string, aconv AconvFuncList) {
	if aconvMap == nil {
		aconvMap = make(map[string]AconvFuncList)
	}
	aconvMap[arch] = aconv
}
