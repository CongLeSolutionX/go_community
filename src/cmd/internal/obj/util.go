...
func Dconv(p *Prog, Rconv func(int) string, a *Addr) string {
	...
	case TYPE_ADDR:
		a.Type = TYPE_MEM
		str = fmt.Sprintf("$%v", Dconv(p, Rconv, a))
		a.Type = TYPE_ADDR
...
func Mconv(a *Addr) string {
	...
	case NAME_EXTERN:
		str = fmt.Sprintf("%s%s(SB)", s.Name, offConv(a.Offset))

	case NAME_STATIC:
		str = fmt.Sprintf("%s<>%s(SB)", s.Name, offConv(a.Offset))

	case NAME_AUTO:
		str = fmt.Sprintf("%s%s(SP)", s.Name, offConv(a.Offset))

	case NAME_PARAM:
		str = fmt.Sprintf("%s%s(FP)", s.Name, offConv(a.Offset))
	}
