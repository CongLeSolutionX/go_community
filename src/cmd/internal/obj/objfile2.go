...
func (a *OFuncInfo) Write(w *bytes.Buffer) {
	...
	writeUint32(uint32(len(a.Funcdata)))
...
func (a *OFuncInfo) Read(b []byte) {
	...
	funcdatalen := readUint32()
	a.Funcdata = make([]OSymRef, funcdatalen)
	for i := range a.Funcdata {
		a.Funcdata[i] = OSymRef{readUint32(), readUint32()}
	}
...
func genFuncInfoSyms(ctxt *Link) {
	...
	for _, s := range ctxt.Text {
		...
		o.Funcdata = make([]OSymRef, len(pc.Funcdata))
		for i, x := range pc.Funcdata {
			o.Funcdata[i] = makeSymRef(x)
		}
