...
func (a *OSymInfo) Size() int {
	...
	return a.GoType.Size() + 1 + 1 + 4 + 4 + 4 + 4 + 4 + 4 + (4 + 4*len(a.Pcdata)) + 4 + (4 + (&OSymRef{}).Size()*len(a.Funcdata)) + (4 + 4*len(a.Funcdataoff)) + (4 + 4*len(a.File))
...
func OSymInfoSize(s *LSym) int {
	...
	pc := &s.Func.Pcln
	return symrefsiz + 1 + 1 + 4 + 4 + 4 + 4 + 4 + 4 + (4 + 4*len(pc.Pcdata)) + 4 + (4 + symrefsiz*len(pc.Funcdata)) + (4 + 4*len(pc.Funcdataoff)) + (4 + 4*len(pc.File))
...
func WriteObjFile2(ctxt *Link, b *bio.Writer, pkgpath string) {
	...
	h.Offsets[SectNonpkgdef] = w.off
	for _, s := range ctxt.nonpkgdefs[1:] {
		w.Sym(s)
	}

	// Non-pkg symbol references
	h.Offsets[SectNonpkgref] = w.off
...
type Reader struct {
	rd    io.ReaderAt
	start uint32
	h     OHeader // keep section offsets
}
...
func (r *Reader) Autolib() []string {
	n := (r.h.Offsets[SectAutolib+1] - r.h.Offsets[SectAutolib]) / 4
	s := make([]string, n)
	for i := range s {
		off := r.h.Offsets[SectAutolib] + uint32(i)*4
		s[i] = r.StringRef(off)
	}
	return s
}
...
func (r *Reader) Pkglist() []string {
	n := (r.h.Offsets[SectPkgIdx+1] - r.h.Offsets[SectPkgIdx]) / 4
	s := make([]string, n)
	for i := range s {
		off := r.h.Offsets[SectPkgIdx] + uint32(i)*4
		s[i] = r.StringRef(off)
	}
	return s
}
...
func (r *Reader) NSym() int {
	symsiz := (&OSym{}).Size()
	return int(r.h.Offsets[SectSymdef+1]-r.h.Offsets[SectSymdef]) / symsiz
}

func (r *Reader) NNonpkgdef() int {
	symsiz := (&OSym{}).Size()
	return int(r.h.Offsets[SectNonpkgdef+1]-r.h.Offsets[SectNonpkgdef]) / symsiz
}

func (r *Reader) NNonpkgref() int {
	symsiz := (&OSym{}).Size()
	return int(r.h.Offsets[SectNonpkgref+1]-r.h.Offsets[SectNonpkgref]) / symsiz
}

// SymOff returns the offset of the i-th symbol.
func (r *Reader) SymOff(i int) uint32 {
	symsiz := (&OSym{}).Size()
	return r.h.Offsets[SectSymdef] + uint32(i*symsiz)
}

// NReloc returns the number of relocations of the i-th symbol.
func (r *Reader) NReloc(i int) int {
	relocIdxOff := r.h.Offsets[SectRelocIdx] + uint32(i*4)
...
func (r *Reader) RelocOff(i int, j int) uint32 {
	...
	return r.h.Offsets[SectReloc] + (relocIdx+uint32(j))*uint32(relocsiz)
}

// InfoOff returns the offset of the sym info of the i-th symbol.
func (r *Reader) InfoOff(i int) uint32 {
	infoIdxOff := r.h.Offsets[SectInfoIdx] + uint32(i*4)
	return r.h.Offsets[SectInfo] + r.Uint32At(infoIdxOff)
}

// DataOff returns the offset of the i-th symbol's data.
func (r *Reader) DataOff(i int) uint32 {
	dataIdxOff := r.h.Offsets[SectDataIdx] + uint32(i*4)
	return r.h.Offsets[SectData] + r.Uint32At(dataIdxOff)
}

// DataSize returns the size of the i-th symbol's data.
func (r *Reader) DataSize(i int) uint32 {
	return r.DataOff(i+1) - r.DataOff(i)
}

// AuxDataBase returns the base offset of the aux data section.
func (r *Reader) AuxDataBase() uint32 {
	return r.h.Offsets[SectAuxData]
}
