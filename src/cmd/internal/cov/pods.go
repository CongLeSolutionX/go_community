...
func CollectPodsFromFiles(files []string, warn bool) []Pod {
	metaRE := regexp.MustCompile(fmt.Sprintf(`^%s\.(\S+\.\S+)$`, coverage.MetaFilePref))
	...
	fmt.Fprintf(os.Stderr, "=-= %d metafiles\n", len(mm))
	counterRE := regexp.MustCompile(fmt.Sprintf(`^%s\.(\S+\.\S+)\.\d+$`, coverage.CounterFilePref))
	for _, f := range files {
		base := filepath.Base(f)
