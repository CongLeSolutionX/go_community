...
type CovDataReader struct {
	...
	indirs    []string
	pkgRe     *regexp.Regexp
	pkgClass  coverage.PkgClassification
	flags     CovDataReaderFlags
	err       error
	verblevel int
}

// Create a CovDataReader object to process the given set of input
// directories.
func MakeCovDataReader(vis CovDataVisitor, indirs []string, verbosityLevel int, flags CovDataReaderFlags, pkgRe *regexp.Regexp, pkgClass coverage.PkgClassification) *CovDataReader {
...
//			     VisitPackage(PK)
//		         if <PK matched according to regexp and/or class> {
...
type CovDataVisitor interface {
	...
	// Invoked when the reader is about the process a given counter
	// data file.
	...
	// Invoked when we've finished processing the counter files
	// in a POD (e.g. no more calls to VisitFuncCounterData).
	EndCounters()
	...
	// Invoked for each function in the package bring visited
...
func (r *CovDataReader) Visit() error {
	pods, err := CollectPods(r.indirs, false)
	if err != nil {
...
func (r *CovDataReader) visitPod(p Pod) error {
	...
	for k, cdf := range p.CounterDataFiles {
		...
		r.vis.VisitCounterDataFile(cdf, cdr, p.Origins[k])
...
func (r *CovDataReader) processPackage(mfname string, pd *decodemeta.CoverageMetaDataDecoder, pkgIdx uint32) error {
	if r.pkgRe != nil {
		if !r.pkgRe.Match([]byte(pd.PackagePath())) {
			return nil
		}
	}
	if r.pkgClass != coverage.PkgAll {
		if pd.PackageClassification() != r.pkgClass {
			return nil
		}
	}
	r.vis.VisitPackage(pd, pkgIdx)
	nf := pd.NumFuncs()
