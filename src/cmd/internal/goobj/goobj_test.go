...
func matchSymName(s *Sym, name string) bool {
	return s.Name == name || strings.HasPrefix(s.Name, name+" [#") // new style, with index
}
...
func TestParseCGOArchive(t *testing.T) {
	...
	var found2 bool
	for _, s := range p.Syms {
		if matchSymName(s, "mycgo.go1") {
			found1 = true
		}
		if matchSymName(s, "mycgo.go2") {
			found2 = true
		}
	}
	if !found1 {
