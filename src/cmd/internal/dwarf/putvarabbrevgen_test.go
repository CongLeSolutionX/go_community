...
// putAbstractVar:
//
//   - it shouldn't appear inside a for or switch statements
//   - it can appear within any number of nested if/else statements but the
//     conditionals must not change after putvarAbbrev/putAbstractVarAbbrev
//     are called
//   - the form argument of putattr must be a compile time constant
//   - each putattr call must be followed by a comment containing the name of
//     the attribute it is setting
func TestPutVarAbbrevGenerator(t *testing.T) {
	sdwarfgo, spvagenfile := pvagenerate(t)

	if pvaDoGenerate {
		write := func(name string, contents string) {
			err := os.WriteFile(name, []byte(contents), 0660)
			if err != nil {
				t.Fatal(err)
			}
		}
		write(pvagenfile, spvagenfile)
		write("./dwarf.go", sdwarfgo)
		return
	}
	...
	if sdwarfgo != slurp("./dwarf.go") {
		t.Error("./dwarf.go is out of date")
	}
}

func pvagenerate(t *testing.T) (string, string) {
	dwarfgobuf, err := os.ReadFile("./dwarf.go")
	if err != nil {
		t.Fatal(err)
	}

	var fset token.FileSet
	f, err := parser.ParseFile(&fset, "./dwarf.go", dwarfgobuf, parser.ParseComments)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, decl := range f.Decls {
		...
		if decl.Name.Name == "putvar" || decl.Name.Name == "putAbstractVar" {
			...
			for i := range decl.Body.List {
				if block, isblock := decl.Body.List[i].(*ast.BlockStmt); isblock {
					// We could replace the BlockStmt directly with a new one but then
					// keeping the comments aligned would be very hard, see
					// https://go.dev/issue/20744
					buf := new(bytes.Buffer)
					format.Node(buf, &token.FileSet{}, pvacfgvisit(pvagraph, abbrevs))
					newblock := buf.Bytes()
					start := delta + fset.Position(block.Lbrace).Offset + 1
					end := delta + fset.Position(block.Rbrace).Offset
					nbuf := []byte{}
					nbuf = append(nbuf, dwarfgobuf[:start]...)
					nbuf = append(nbuf, newblock...)
					nbuf = append(nbuf, dwarfgobuf[end:]...)
					dwarfgobuf = nbuf
					delta += start - end + len(newblock)
					found = true
					break
				}
			}
			if !found {
				t.Fatalf("could not find block to substitute in %s", decl.Name.Name)
			}
		}
	}
...
`)
	...
	return string(dofmt(dwarfgobuf)), string(dofmt(buf.Bytes()))
}
...
func pvacfgvisitnode(pvacfg *pvacfgnode, tag string, path []*pvacfgnode, abbrevs map[string]int) ast.Stmt {
	if pvacfg == nil {
		...
		return &ast.AssignStmt{
			Tok: token.ASSIGN,
			Lhs: []ast.Expr{&ast.Ident{Name: "abbrev"}},
