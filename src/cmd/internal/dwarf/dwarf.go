...
// Type represents a type info to be used in generating DWARF type DIE.
type Type interface {
...
type TypeContext interface {
	...
	LookupDwarfSym(name string, refOnly bool) (s Sym, exist bool)
...
func NewTypeDie(parent *DWDie, abbrev int, symName string, dwarfName string, ctx TypeContext) *DWDie {
	...
	default:
		...
		ds, _ := ctx.LookupDwarfSym(symName, false)
		die.Sym = ds
		return die
	}
...
func NewType(gotype Type, tc TypeContext, parent *DWDie) (*DWDie, *DWDie, error) {
	...
	case objabi.KindArray:
		...
		u, _ := tc.LookupDwarfSym("uintptr", true)
	...
	case objabi.KindInterface:
		...
		typedefdie = doTypeDef(parent, name, dwarfName, die, tc)
		if gotype.IsEface() {
			s, _ := tc.LookupDwarfSym("runtime.eface", true)
			NewRefAttr(die, DW_AT_type, s)
		} else {
			s, _ := tc.LookupDwarfSym("runtime.iface", true)
			NewRefAttr(die, DW_AT_type, s)
		}
...
func MkInternalType(parent *DWDie, ctx TypeContext, abbrev int, typename, keyname, valname string, f func(*DWDie)) Sym {
	...
	s, exist := ctx.LookupDwarfSym(name, true)
	if exist {
		return s
	}
