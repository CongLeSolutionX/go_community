...
type Var struct {
	...
	DictIndex       uint32 // index of the dictionary entry describing the type of this variable
}
...
func putparamtypes(ctxt Context, s *FnState, scopes []Scope, fnabbrev int) []int64 {
	...
	for i := range scopes {
		for _, v := range scopes[i].Vars {
			...
			Uleb128put(ctxt, s.Info, int64(DW_ABRV_PARAM_TYPE))
			n := fmt.Sprintf(".param%d", v.DictIndex-1)
			putattr(ctxt, s.Info, DW_ABRV_PARAM_TYPE, DW_FORM_string, DW_CLS_STRING, int64(len(n)), n)
			putattr(ctxt, s.Info, DW_ABRV_PARAM_TYPE, DW_FORM_ref_addr, DW_CLS_REFERENCE, 0, v.Type)
			putattr(ctxt, s.Info, DW_ABRV_PARAM_TYPE, DW_FORM_udata, DW_CLS_CONSTANT, int64(v.DictIndex-1), nil)
		}
	}
