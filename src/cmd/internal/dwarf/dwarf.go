...
func putAbstractVar(ctxt Context, info Sym, v *Var) {
	var abbrev int
	// The following block is autogenerated by TestPutVarAbbrevGenerator
	{
		if v.Tag == DW_TAG_variable {
			abbrev = DW_ABRV_PUTVAR_START + 0
		} else {
			abbrev = DW_ABRV_PUTVAR_START + 1
		}
	}
...
func putvar(ctxt Context, s *FnState, v *Var, absfn Sym, fnabbrev, inlIndex int, encbuf []byte) {
	...
	// The following block is autogenerated by TestPutVarAbbrevGenerator
	{
		if v.Tag == DW_TAG_variable {
			if concrete {
				if withLoclist {
					abbrev = DW_ABRV_PUTVAR_START + 2
				} else {
					abbrev = DW_ABRV_PUTVAR_START + 3
				}
			} else {
				if v.ClosureOffset > 0 {
					if withLoclist {
						abbrev = DW_ABRV_PUTVAR_START + 4
					} else {
						abbrev = DW_ABRV_PUTVAR_START + 5
					}
				} else {
					if withLoclist {
						abbrev = DW_ABRV_PUTVAR_START + 6
					} else {
						abbrev = DW_ABRV_PUTVAR_START + 7
					}
				}
			}
		} else {
			if concrete {
				if withLoclist {
					abbrev = DW_ABRV_PUTVAR_START + 8
				} else {
					abbrev = DW_ABRV_PUTVAR_START + 9
				}
			} else {
				if v.ClosureOffset > 0 {
					if withLoclist {
						abbrev = DW_ABRV_PUTVAR_START + 10
					} else {
						abbrev = DW_ABRV_PUTVAR_START + 11
					}
				} else {
					if withLoclist {
						abbrev = DW_ABRV_PUTVAR_START + 12
					} else {
						abbrev = DW_ABRV_PUTVAR_START + 13
					}
				}
			}
		}
