...
func BuildAndFindHash(id, file, separator, errorPrefix string, rewrite bool) (newID string, err error) {
	f, err := os.Open(file)
	if err != nil {
		return "", err
	}
	...
	if len(newID) != len(id) {
		return "", fmt.Errorf("%s: build ID length mismatch %q (%d) vs %q (%d)", errorPrefix, id, len(id), newID, len(newID))
	}
