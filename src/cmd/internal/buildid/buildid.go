...
var (
	...
	ErrEmptyMatchLen = fmt.Errorf("array match is empty")
)
...
func BuildAndFindHash(id, file, separator, errorPrefix string, rewrite bool) (err error, newID string) {
	f, err := os.Open(file)
	if err != nil {
		return err, ""
	}
	...
	if len(newID) != len(id) {
		return fmt.Errorf("%s: build ID length mismatch %q vs %q", errorPrefix, id, newID), ""
	}

	if len(matches) == 0 {
		// Assume the user specified -buildid= to override what we were going to choose.
		return ErrEmptyMatchLen, newID // look at usages
	}
