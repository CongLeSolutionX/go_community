...
// If withsrc is true, source code is intermixed with the assembly instructions.
func (d *Disasm) Print(w io.Writer, filter *regexp.Regexp, start, end uint64, withsrc bool) {
	...
	for _, sym := range d.syms {
		...
		f := func(pc, size uint64, file string, line int, text string, srcline string) {
			i := pc - d.textStart
		...
		if withsrc {
			// We need to intermix the source into the output, so wrap
			// the default DecodeFunc with another function that
			// writes the appropriate source line to the output before
			// delegating the rest of the job to the default
			// DecodeFunc itself.
			oldF := f

			lastPrinted := ""
			f = func(pc, size uint64, file string, line int, text string, srcline string) {
				if srcline != "" && lastPrinted != srcline {
					// TODO: list of things we don't care about
					// e.g. '}' or lines that starts with '//'
					fmt.Fprintf(tw, "%s\n", srcline)
					lastPrinted = srcline
				}
				oldF(pc, size, file, line, text, srcline)
			}
		}

		d.Decode(symStart, symEnd, relocs, withsrc, f)
		tw.Flush()
	}
	bw.Flush()
}

type DecodeFunc func(pc, size uint64, file string, line int, text string, srcline string)

// Decode disassembles the text segment range [start, end), calling f for each instruction.
func (d *Disasm) Decode(start, end uint64, relocs []Reloc, source bool, f DecodeFunc) {
	if start < d.textStart {
		start = d.textStart
	}
	if end > d.textEnd {
	...
	// A filename -> [slice of lines] map of all known source files.
	...
	for pc := start; pc < end; {
		...
		var srcline string
		if source && file != "" && file != "<autogenerated>" {
			if _, ok := srcFiles[file]; !ok {
				// Never-seen-before file.  Read it, split it, and
				// save its lines in the srcFiles map.
				dat, err := ioutil.ReadFile(file)
				if err == nil {
					src := strings.Split(string(dat), "\n")
					for i := range src {
						// remove indentation from source lines
		...
		f(pc, uint64(size), file, line, text, srcline)
		pc += uint64(size)
	}
}
