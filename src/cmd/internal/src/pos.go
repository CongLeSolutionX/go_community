...
const (
	// It is expected that the front end will usually generate positions tagged with LicoDefaultStmt,
	// but note statement boundaries with LicoIsStmt.  Simple statements will have a single boundary;
	// for loops with initialization may have one for their entry and one for their back edge (this
	// depends on exactly how the loop is compiled; the intent is to provide a good experience to a
	...
	// positions for profiling purposes. To reflect this these positions will be changed to LicoNotStmt.
	//
	// When the optimizer removes an instruction marked LicoIsStmt; it should attempt to find a nearby
	// instruction marked LicoDefaultStmt to be the new statement boundary.  I.e., the optimizer should
	// make a best-effort to conserve statement boundary positions, and might be enhanced to note when
	// a statement boundary is not conserved.
	//
	...
	//
	// In non-optimizing compilation there is still a role for LicoNotStmt because of code generation
	// into the entry block.  LicoIsStmt is statement positions should be conserved.
	//
	// When code generation occurs any remaining default-marked positions are replaced with not-statement
	// positions.
	//
	LicoDefaultStmt uint = iota // Default; position is not a statement boundary, but might be if optimization removes the designated statement boundary
	LicoIsStmt                  // Position is a statement bounday; if optimization removes the corresponding instruction, it should attempt to find a new instruction to bne the boundary.
	LicoNotStmt                 // Position should not be a statement boundary, but line should be preserved for profiling and low-level debugging purposes.
)
...
func (x lico) lineNumberHTML() string {
	if x.IsStmt() == LicoDefaultStmt {
		return fmt.Sprintf("%d", x.Line())
	}
	style := "b"
	if x.IsStmt() == LicoNotStmt {
