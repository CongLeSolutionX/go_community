...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "\tgo doc -help\n\n")
...
func main() {
	...
	// fmt.Println(buildPackage.ImportPath, ":::", symbol, "---", method)
...
func parseArg() (*build.Package, string) {
	...
	arg := flag.Arg(0)
	// If it contains slashes, it begins with a package path. The last slash
	// is the one we care about.
	slash := strings.LastIndex(arg, "/")
	// There may be periods in the package path before the slash,
	// as it may be a domain name, but after the (last) slash any period
	// will be the boundary between path and symbol.
	start := 0
	if slash > 0 {
		start = slash
	}
...
// splitGopath spolits $GOPATH into a list of strings.
...
func pathFor(root, pkg string) (result string) {
	...
	// We put a path on the pkg so can use simple string comparison below
	// yet avoid inadvertent matches, like /foobar matching bar.
	pkgString := filepath.Clean(string(os.PathSeparator) + pkg)
