...
func parsePackage(pkg *build.Package) *Package {
	...
	for name := range pkgs {
		...
		if name != pkgName {
			log.Fatalf("multiple packages %s and %s in directory %s", name, pkgName, pkg.Dir)
		}
	}
	if pkgName == "" {
		log.Fatalf("no Go package found in directory %s", pkg.Dir)
	}
	if len(pkgs) != 1 {
	...
	return &Package{
		name: pkgName,
		pkg:  pkgs[pkgName],
		file: ast.MergePackageFiles(pkgs[pkgName], 0),
		fs:   fs,
	}
}
...
func (pkg *Package) findDecl(symbol string) (*ast.FuncDecl, *ast.GenDecl, ast.Node) {
	for _, d := range pkg.file.Decls {
		switch decl := d.(type) {
		case *ast.GenDecl:
			for _, spec := range decl.Specs {
		...
		case *ast.FuncDecl:
			if match(symbol, decl.Name.Name) {
				return decl, nil, nil
			}
		default:
			log.Fatalf("can't handle node of type %T", decl)
...
// symbolDoc prints the doc for symbol. If it is a type, this includes is methods,
...
func match(user, program string) bool {
	...
	for _, u := range user {
		...
		if unicode.IsLower(u) && unicode.ToUpper(u) == p {
			continue
		}
		return false
	}
