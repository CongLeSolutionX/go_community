...
func (pkg *Package) packageDoc() {
	...
	if importPath != pkg.build.ImportPath {
		fmt.Printf("WARNING: package is being imported from %q\n", pkg.build.ImportPath)
	}
...
func (pkg *Package) symbolDoc(symbol string) {
	...
	if typ == nil {
		log.Fatalf("symbol %s not present in package %s imported from %q", symbol, pkg.name, pkg.build.ImportPath)
	}
	decl := typ.Decl
	...
	for _, constant := range typ.Consts {
		pkg.oneLineValueGenDecl(constant.Decl)
	}
}
...
func (pkg *Package) methodDoc(symbol, method string) {
	typ := pkg.findType(symbol)
	if typ == nil {
		log.Fatalf("symbol %s is not a type in package %s imported from %q", symbol, pkg.name, pkg.build.ImportPath)
	}
