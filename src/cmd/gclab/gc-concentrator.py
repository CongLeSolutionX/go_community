...
def svgNetwork(network : Network):
    margin = 5
    ...
    step = 100
    stepHeap, stepScan, stepDartboard = range(3)
    ...
    # Divide into regions
    p = g.path('fill="none" stroke="#c17d11"')
    for hPos, _ in hSpans[-1][1:]:
        p1 = pt(lastLayer, 0, hPos) + heapSep
        p.M(p1 + heapH).L(p1).L(p1 + heapD)
    ...
    for hi, hSpan in enumerate(hSpans[-1]):
        ...
        highlightFlush = (hi == highlight[-1])
        if step >= stepScan:
            highlightFlush = highlightFlush or hi in [2, 8, 30]
        if step >= stepDartboard:
        ...
        if step >= stepDartboard:
            ...
            if hi == scanSrc:
                drawHighlight(d1 + (heapSep - dSep), d2 + (heapSep - dSep + heapH), 'fill="#fcaf3e" stroke="#f57900"')
...
g = svgNetwork(densityNetwork(Config(1 * GiB, 8), markRegionBytes=32 * MiB, fanOut=4, fanIn=2))
#g = svgNetwork(densityNetwork(Config(16 * GiB, 16), fanOut=16))
g.write(sys.stdout)