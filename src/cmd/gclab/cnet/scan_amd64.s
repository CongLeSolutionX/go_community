...
TEXT ·condenseAsm(SB), NOSPLIT, $0-24
    MOVQ sizeClass+0(FP), CX
    MOVQ unpacked+8(FP), AX

    // Call the condenser for this size class
    LEAQ ·gcCondensers(SB), BX
    CALL (BX)(CX*8)

    MOVQ packed+16(FP), DI // Condensed output bitmap pointer
    VMOVDQU64 Z1, 0(DI)
    VZEROUPPER
    RET
