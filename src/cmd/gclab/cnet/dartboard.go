...
const (
	// dartboardRegion is the size of an atomic unit of the dartboard, in either
	// bits of dartboard bitmap, or words of heap. Dartboard regions are the
	// smallest unit of scanning.
	dartboardRegion heap.Words = 32 << 10

	regionHeapBytes = heap.WordBytes * heap.Bytes(dartboardRegion)

	// regionsPerArena is the number of dartboard regions in an arena.
	regionsPerArena = arenaRegionID(heap.ArenaWords / dartboardRegion)
)
...
func (b *arenaRegionBitmap) set(arenaWord heap.Words) {
	region := arenaWord / dartboardRegion
	...
	for _, addr := range addrs {
		...
		if traceDartboard {
			if arenaBitmap.Has(word) {
				log.Printf("  dartboard bit %#08x already set", word)
			} else {
				log.Printf("  dartboard bit %#08x set", word)
			}
		}
		if dartboardStats {
			if arenaBitmap.Has(word) {
...
func (id regionID) toVAddr(h *heap.Heap) heap.VAddr {
	...
	arenaOff := regionHeapBytes.Mul(int(id.arenaRegion()))
	return arenaStart.Plus(arenaOff)
}

func (id regionID) Range(h *heap.Heap) heap.Range {
	rStart := h.Arenas[id.arenaID()].Start.Plus(regionHeapBytes.Mul(int(id.arenaRegion())))
	return heap.Range{Start: rStart, Len: regionHeapBytes}
}
