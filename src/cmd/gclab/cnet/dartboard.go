...
func copyToDartboard[T anyLAddr](addrs []T, arenaBitmap bitmap.Set[heap.Words], regionBitmap *arenaRegionBitmap, heapSpan heap.Bytes) {
	...
	for _, addr := range addrs {
		word := addr.ArenaWord()
		if traceEnqueue {
			log.Printf("  enqueue bit %#08x", word)
		}
		if dartboardStats {
			if arenaBitmap.Has(word) {
				dupBits++
				if traceEnqueue {
					log.Printf("    already enqueued")
				}
			} else {
				newBits++
			}
...
func (c *CNet) regionRange(id regionID) heap.Range {
	rStart := c.heap.Arenas[id.arenaID()].Start.Plus(regionHeapBytes.Mul(int(id.arenaRegion())))
	return heap.Range{rStart, regionHeapBytes}
}
