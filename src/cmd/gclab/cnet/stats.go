...
var metricMBPerSec = invivo.NewMetric("MB/sec")

var benchmarkDrain = invivo.NewBenchmark("Drain")
var metricMarkedMBPerSec = invivo.NewMetric("marked-MB/sec")
var metricSpanMBPerSec = invivo.NewMetric("span-MB/sec")
...
func statsScanRegion() func() {
	...
	return func() {
		...
		bytes := regionHeapBytes
		mbPerSec := (float64(bytes) / 1e6) / bench.Elapsed().Seconds()
		scanRegionStatsOne.Ns = bench.Elapsed()
		scanCount++
		bench.SetMetric(mbPerSec, metricMBPerSec)
		bench.Done()
