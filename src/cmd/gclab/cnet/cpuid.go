...
func getDataCacheSizes() []heap.Bytes {
...
func getDataCacheSizesIntel(maxID uint32) []heap.Bytes {
	...
	for level := uint32(0); level < 0xFFFF; level++ {
		eax, ebx, ecx, _ := cpuid(4, level)
		...
		caches = append(caches, heap.Bytes(size))
	}

	return caches
}

func getDataCacheSizesAMD() []heap.Bytes {
	...
	l1dSize := heap.Bytes(extractBits(ecx5, 24, 31) << 10)
	...
	l2Size := heap.Bytes(extractBits(ecx6, 16, 31) << 10)
	...
	l3Size := heap.Bytes(extractBits(edx6, 18, 31) * (512 << 10))
