...
func (gc *scanner) scanRegionOld(p int, rid regionID) {
	...
	for arenaWord := range a.dartboard.Range(bitStart, bitStart+dartboardBitsPerRegion) {
		nBits++
	...
	if scanStats {
		...
		totalObjects := 0
		regionRange := heap.Range{Start: a.Start.Plus(bitStart.Bytes()), Len: dartboardBitsPerRegion.Bytes()}
...
func (gc *scanner) scanRegion(p int, rid regionID) {
	...
	pp := &gc.perP[p]
	{
		...
		copy(pp.dartboardCopy[:], srcDartboardInRegion)
		clear(srcDartboardInRegion)
	}
	dartboardInRegion := bitmap.FromWords[heap.Words](pp.dartboardCopy[:])
...
func (gc *scanner) scanObject(p int, buf []heap.VAddr, bufPos *int32, span *heap.Span, objIndex int, base heap.VAddr, length heap.Bytes) {
