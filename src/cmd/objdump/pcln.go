...
func printPclnStats(out io.Writer, encoding binary.ByteOrder, ptrSize int, data []byte) {
	d := decoder{encoding, ptrSize, data, 0}
	d.pos += 8 // Skip header

	// Read the size of the func table
	nfunc := int(d.Uint64())
	size := (2*nfunc + 1) * ptrSize // nfunc entries + end pc
	...
	if d.pos != begin+uint64(size) {
		panic(fmt.Sprintf("%d %d %d", d.pos, begin, (nfunc+1)*ptrSize))
	}
	ftabOffset := d.Uint32()
	...
	for i := 0; i < nfunc; i++ {
		...
		lastPos = d.pos
		lastPos += -lastPos & uint64(ptrSize-1)
	...
	io.WriteString(out, fmt.Sprintf("\t  _func structs:\t% 10d\n", funcTot))
