...
func TestIssue51939(t *testing.T) {
	...
	goFile := filepath.Join(t.TempDir(), "issue51939.go")
	if err := ioutil.WriteFile(goFile, []byte(goSource), 0444); err != nil {
		t.Fatal(err)
	}
	outFile := filepath.Join(t.TempDir(), "issue51939.exe")
	goTool := testenv.GoToolPath(t)
	...
	for _, s := range ef.Sections {
		if s.Flags&elf.SHF_ALLOC == 0 && s.Addr != 0 {
			t.Errorf("section:%s should not allocated with addr:%d", s.Name, s.Addr)
		}
	}
}
