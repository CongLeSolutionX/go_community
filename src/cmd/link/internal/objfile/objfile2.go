...
// Add a non-package symbol, return if it is added.
...
func LoadNew(l *Loader, arch *sys.Arch, syms *sym.Symbols, f *bio.Reader, lib *sym.Library, length int64, pn string, flags int) {
	start := f.Offset()
...
func LoadRefs(l *Loader, r *obj.Reader, lib *sym.Library, arch *sys.Arch, syms *sym.Symbols, localSymVersion int) {
	pkgprefix := objabi.PathToPrefix(lib.Pkg) + "."
...
func LoadFull(l *Loader, r *obj.Reader, lib *sym.Library, syms *sym.Symbols, localSymVersion int, libByPkg map[string]*sym.Library) {
	// PkgIdx
	pkglist := r.Pkglist()
