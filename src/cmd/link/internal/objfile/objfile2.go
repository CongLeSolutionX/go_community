...
type Loader struct {
	...
	overwrite  map[int]int     // overwrite[i]=j if symbol j overwrites symbol i
...
func (l *Loader) AddSym(name string, ver int, i int, r *oReader, dupok bool) bool {
	if l.extStart != 0 {
	...
	if oldi, ok := l.symsByName[nv]; ok {
		...
		overwrite := r.DataSize(i-l.StartIndex(r)) != 0
		if overwrite {
			oldr, li := l.ToLocal(oldi)
			if oldr.DataSize(li) != 0 {
...
func (l *Loader) ToLocal(i int) (*oReader, int) {
	...
	return l.objs[k-1].r, i - l.objs[k-1].i
}
...
// Returns the raw (unpatched) name of the i-th symbol. i is global index.
func (l *Loader) RawSymName(i int) string {
	r, li := l.ToLocal(i)
	if r == nil {
		return ""
	}
...
// Returns the (patched) name of the i-th symbol. i is global index.
func (l *Loader) SymName(i int) string {
	r, li := l.ToLocal(i)
	if r == nil {
		return ""
	}
...
// Returns the type of the i-th symbol. i is global index.
func (l *Loader) SymType(i int) sym.SymKind {
	r, li := l.ToLocal(i)
	if r == nil {
		return 0
	}
...
func (l *Loader) NReloc(i int) int {
	r, li := l.ToLocal(i)
	if r == nil {
		return 0
	}
...
func (l *Loader) RelocSym(i int, j int) int {
	r, li := l.ToLocal(i)
	rel := goobj2.Reloc{}
...
// symbol. i is global index.
func (l *Loader) RelocType(i int, j int) objabi.RelocType {
	r, li := l.ToLocal(i)
	rel := goobj2.Reloc{}
	rel.Read(r.Reader, r.RelocOff(li, j))
...
// symbol. i is global index.
func (l *Loader) AuxSym(i int, j int) int {
	r, li := l.ToLocal(i)
	a := goobj2.Aux{}
...
func LoadNew(l *Loader, arch *sys.Arch, syms *sym.Symbols, f *bio.Reader, lib *sym.Library, unit *sym.CompilationUnit, length int64, pn string, flags int) {
	...
	for i, n := 0, ndef+nnonpkgdef; i < n; i++ {
		...
		l.AddSym(name, v, istart+i, or, dupok)
	}
...
func loadObjSyms(l *Loader, syms *sym.Symbols, r *oReader) {
	...
	for i, n := 0, r.NSym()+r.NNonpkgdef(); i < n; i++ {
		...
		if !l.Reachable.Has(istart+i) && !(t == sym.SRODATA && strings.HasPrefix(name, "type.")) {
...
func loadObjFull(l *Loader, r *oReader) {
	...
	for i, n := 0, r.NSym()+r.NNonpkgdef(); i < n; i++ {
		...
		for j := 0; j < naux; j++ {
			...
			case goobj2.AuxDwarfInfo, goobj2.AuxDwarfLoc, goobj2.AuxDwarfRanges, goobj2.AuxDwarfAbsFn, goobj2.AuxDwarfLines:
				// ignored for now
			default:
