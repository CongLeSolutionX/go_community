...
func (l *Loader) AddSym(name string, ver int, i Sym, r *oReader, dupok bool) bool {
	if l.extStart != 0 {
...
func (l *Loader) RelocOff(i Sym, j int) int32 {
	r, li := l.ToLocal(i)
	rel := goobj2.Reloc{}
	rel.Read(r.Reader, r.RelocOff(li, j))
	return rel.Off
...
// symbol. i is global index.
func (l *Loader) RelocAdd(i Sym, j int) int64 {
	r, li := l.ToLocal(i)
	rel := goobj2.Reloc{}
	rel.Read(r.Reader, r.RelocOff(li, j))
	return rel.Add
...
func LoadNew(l *Loader, arch *sys.Arch, syms *sym.Symbols, f *bio.Reader, lib *sym.Library, unit *sym.CompilationUnit, length int64, pn string, flags int) {
	...
	for i, n := 0, ndef+nnonpkgdef; i < n; i++ {
		...
		l.AddSym(name, v, istart+Sym(i), or, dupok)
	}
...
func loadObjSyms(l *Loader, syms *sym.Symbols, r *oReader) {
	...
	for i, n := 0, r.NSym()+r.NNonpkgdef(); i < n; i++ {
		...
		if !l.Reachable.Has(istart+Sym(i)) && !(t == sym.SRODATA && strings.HasPrefix(name, "type.")) && name != "runtime.addmoduledata" {
