...
// LoadObjFile loads an object file f into library lib.
// The symbols loaded are added to syms.
func LoadObjFile(arch *sys.Arch, syms *ld.Symbols, f *bio.Reader, lib *ld.Library, length int64, pn string) {
	start := f.Offset()
	r := &objReader{
		rd:              f.Reader,
		lib:             lib,
...
func (r *objReader) loadObjFile() {
	...
	// Symbol references
	r.refs = []*ld.Symbol{nil} // zeroth ref is nil
	for {
		c, err := r.rd.Peek(1)
		if err != nil {
...
func (r *objReader) readSlices() {
	...
	r.data = make([]byte, n)
	n = r.readInt()
	r.reloc = make([]ld.Reloc, n)
	n = r.readInt()
	r.pcdata = make([]ld.Pcdata, n)
	n = r.readInt()
	r.autom = make([]ld.Auto, n)
	n = r.readInt()
	r.funcdata = make([]*ld.Symbol, n)
	r.funcdataoff = make([]int64, n)
	n = r.readInt()
	r.file = make([]*ld.Symbol, n)
}
...
func (r *objReader) readSym() {
	...
	var dup *ld.Symbol
	if s.Type != 0 && s.Type != ld.SXREF {
		if (t == ld.SDATA || t == ld.SBSS || t == ld.SNOPTRBSS) && len(data) == 0 && nreloc == 0 {
		...
		if (s.Type == ld.SDATA || s.Type == ld.SBSS || s.Type == ld.SNOPTRBSS) && len(s.P) == 0 && len(s.R) == 0 {
			goto overwrite
		}
		if s.Type != ld.SBSS && s.Type != ld.SNOPTRBSS && !dupok && !s.Attr.DuplicateOK() {
...
overwrite:
	...
	if t == ld.SBSS && (s.Type == ld.SRODATA || s.Type == ld.SNOPTRBSS) {
		t = s.Type
	}
	s.Type = t
	if s.Size < int64(size) {
	...
	s.Attr.Set(ld.AttrLocal, local)
	s.Attr.Set(ld.AttrMakeTypelink, makeTypelink)
	if typ != nil {
		s.Gotype = typ
	}
	...
	if s.Type == ld.STEXT {
		s.FuncInfo = new(ld.FuncInfo)
		pc := s.FuncInfo

		pc.Args = r.readInt32()
		pc.Locals = r.readInt32()
		if r.readUint8() != 0 {
			s.Attr |= ld.AttrNoSplit
		}
		flags := r.readInt()
		if flags&(1<<2) != 0 {
			s.Attr |= ld.AttrReflectMethod
		}
		if flags&(1<<3) != 0 {
			s.Attr |= ld.AttrShared
		}
		n := r.readInt()
		...
		n = r.readInt()
		pc.InlTree = make([]ld.InlinedCall, n)
		for i := 0; i < n; i++ {
	...
	if s.Type == ld.SDWARFINFO {
		r.patchDWARFName(s)
	}
}

func (r *objReader) patchDWARFName(s *ld.Symbol) {
...
func (r *objReader) readRef() {
	...
	if s.Name[0] == '$' && len(s.Name) > 5 && s.Type == 0 && len(s.P) == 0 {
		...
		s.Attr.Set(ld.AttrReachable, false)
	}
...
func (r *objReader) readSymName() string {
	...
	for {
		...
		if i == -1 {
			...
			if ld.Buildmode == ld.BuildmodeShared || *ld.FlagLinkshared {
...
func (r *objReader) readSymIndex() *ld.Symbol {
	i := r.readInt()
	return r.refs[i]
}
