...
func assignAddress(ctxt *ld.Link, sect *sym.Section, n int, s *sym.Symbol, va uint64, isTramp bool) (*sym.Section, int, uint64) {
	...
	s.Sect = sect
	s.Value = int64(len(hostImports)+n-1) << 16 // n starts at 1
	n++
...
func asmb(ctxt *ld.Link) {
	...
	for i, fn := range ctxt.Textp {
		...
		if fn.Name == "go.buildid" {
			...
			for _, r := range fn.R {
				...
				switch r.Type {
				case objabi.R_ADDR:
					writeSleb128(wfn, int32(r.Sym.Value))
				case objabi.R_CALL:
					writeSleb128(wfn, int32(r.Sym.Value>>16))
				case objabi.R_WASMIMPORT:
					writeSleb128(wfn, hostImportMap[r.Sym.Name])
				default:
	...
	writeSec(ctxt, sectionElement, func() { writeElementSec(ctxt, fns) })
	writeSec(ctxt, sectionCode, func() { writeCodeSec(ctxt, fns) })
	writeSec(ctxt, sectionData, func() { writeDataSec(ctxt) })
	writeSec(ctxt, sectionCustom, func() { writeNameSec(ctxt, append(importedFns, fns...)) })
...
func writeTableSec(ctxt *ld.Link, fns []*wasmFunc) {
	numElements := uint32(len(hostImports) + len(fns))
...
func writeElementSec(ctxt *ld.Link, fns []*wasmFunc) {
	...
	ctxt.Out.WriteByte(0x41)                        // i32.const
	writeSleb128(ctxt.Out, int32(len(hostImports))) // offset
	ctxt.Out.WriteByte(0x0b)                        // end

	writeUleb128(ctxt.Out, uint32(len(fns))) // number of entries
	for i := range fns {
		writeUleb128(ctxt.Out, uint32(len(hostImports)+i))
	}
}
