...
func assignAddress(ctxt *ld.Link, sect *sym.Section, n int, s *sym.Symbol, va uint64, isTramp bool) (*sym.Section, int, uint64) {
	...
	s.Sect = sect
	s.Value = int64(funcValueOffset+n-1) << 16 // n starts at 1
	n++
...
func asmb(ctxt *ld.Link) {
	...
	writeSec(ctxt, sectionImport, func() { writeImportSec(ctxt, hostImports) })
	writeSec(ctxt, sectionFunction, func() { writeFuncSec(ctxt, fns) })
	writeSec(ctxt, sectionTable, func() { writeTableSec(ctxt, fns) })
	writeSec(ctxt, sectionMemory, func() { writeMemSec(ctxt) })
	writeSec(ctxt, sectionGlobal, func() { writeGlobalSec(ctxt) })
	writeSec(ctxt, sectionExport, func() { writeExportSec(ctxt, rt0) })
	writeSec(ctxt, sectionElement, func() { writeElementSec(ctxt, uint64(len(hostImports)), uint64(len(fns))) })
	writeSec(ctxt, sectionCode, func() { writeCodeSec(ctxt, fns) })
	writeSec(ctxt, sectionData, func() { writeDataSec(ctxt) })
	if !*ld.FlagS {
		writeSec(ctxt, sectionCustom, func() { writeNameSec(ctxt, append(hostImports, fns...)) })
	}
...
func writeSec(ctxt *ld.Link, id uint8, writeSecFn func()) {
	...
	writeUleb128FixedLength(ctxt.Out, uint64(offsetEnd-offsetLength-5), 5)
...
func writeElementSec(ctxt *ld.Link, numImports, numFns uint64) {
	...
	ctxt.Out.WriteByte(0x41)                // i32.const
	writeSleb128(ctxt.Out, funcValueOffset) // offset
	ctxt.Out.WriteByte(0x0b)                // end
...
func writeDataSec(ctxt *ld.Link) {
	...
	for _, sec := range sections {
		writeUleb128(ctxt.Out, 0) // memidx
		ctxt.Out.WriteByte(0x41)  // i32.const
		writeSleb128(ctxt.Out, int64(sec.Vaddr))
		ctxt.Out.WriteByte(0x0b) // end
...
func writeUleb128FixedLength(w *ld.OutBuf, v uint64, length int) {
	for i := 0; i < length; i++ {
		c := uint8(v & 0x7f)
		v >>= 7
		if i < length-1 {
