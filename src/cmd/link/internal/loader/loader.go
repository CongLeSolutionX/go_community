...
func (l *Loader) cloneToExternal(symIdx Sym) Sym {
	...
	// At the moment we're not expecting invocations of cloneToExternal
	// on an actual definition from a Go object (e.g. something that has
	// data and/or relocations). Panic if this case comes up. NB: if
	// this does turn out to be a viable scenario, it should not be
	// hard to add code that copies over the content/relocs.
	if li < (r.NSym() + r.NNonpkgdef()) {
		if len(r.Data(li)) != 0 {
			panic("unexpected def with non-zero data")
		}
		if l.Relocs(symIdx).Count != 0 {
			panic("unexpected def with relocations")
		}
	}
