...
func (l *Loader) LookupOrCreate(name string, version int, syms *sym.Symbols) *sym.Symbol {
	i := l.FindOrCreate(name, version)
	if i == 0 {
		panic("Create symbol failed.")
	}
	return l.Syms[i]
}

// FindOrCreate makes a data backed symbol.
func (l *Loader) FindOrCreate(name string, version int) Sym {
...
func (l *Loader) SetType(s Sym, t sym.SymKind) {
	if !l.IsExternal(s) {
		panic("need to load the symbol")
	}
	l.Syms[s].Type = t
}

func (l *Loader) SetValue(s Sym, v int64) {
	if !l.IsExternal(s) {
		panic("need to load the symbol")
	}
	l.Syms[s].Value = v
...
func (l *Loader) SetAttr(s Sym, attr sym.Attribute) {
	if !l.IsExternal(s) {
		panic("need to load the symbol")
	}
	l.Syms[s].Attr = attr
