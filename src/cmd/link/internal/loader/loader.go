...
func (l *Loader) AssignTextSymbolOrder(libs []*sym.Library, intlibs []bool) {
	...
	assignedToUnit := make(map[Sym]bool)
	...
	for _, doInternal := range [2]bool{true, false} {
		for idx, lib := range libs {
			...
			libtextp2 := lib.Textp2[:0]
			for _, s := range lib.Textp2 {
				sym := Sym(s)
				if l.attrReachable.has(sym) && !assignedToUnit[sym] {
					libtextp2 = append(libtextp2, s)
					unit := l.SymUnit(sym)
					if unit != nil {
						unit.Textp2 = append(unit.Textp2, s)
						assignedToUnit[sym] = true
					}
				}
			}
			for _, s := range lib.DupTextSyms2 {
				sym := Sym(s)
				if l.AttrReachable(sym) && !assignedToUnit[sym] {
					libtextp2 = append(libtextp2, s)
					unit := l.SymUnit(sym)
					if unit != nil {
						unit.Textp2 = append(unit.Textp2, s)
