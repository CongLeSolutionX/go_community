...
// makes sense for external symbols.
...
func (l *Loader) growReachable(reqCap int) {
	rc := len(l.Reachable) * 32
	if reqCap > rc {
		l.Reachable = growBitmap(reqCap, rc, l.Reachable)
	}
}
...
func (l *Loader) growHasPayload(reqCap int) {
	rc := len(l.hasPayload) * 32
	if reqCap > rc {
		l.hasPayload = growBitmap(reqCap, rc, l.hasPayload)
	}
}
...
func (l *Loader) CreateExtSym(name string) Sym {
	...
	l.extSyms = append(l.extSyms, nameVer{name, sym.SymVerABI0})
	l.growSyms(int(i))
	l.hasPayload.Set(i)
	pi := i - l.extStart
	l.payloads[pi].name = name
	l.payloads[pi].ver = sym.SymVerABI0
	return i
}
