...
func growBitmap(reqLen int, b bitmap) bitmap {
	...
	if reqLen > curLen {
		...
		b = append(b, makeBitmap(newLen-curLen)...)
	}
	return b
}
...
// ssVal returns the value of a specified sub-symbol.
// FIXME: will need to be generalized.
func (l *Loader) ssVal(i Sym) int64 {
	if i < l.extStart {
		panic("not expecting object symbol")
	}
	if l.Syms[i] != nil {
		return l.Syms[i].Value
	}
	return l.payloads[i-l.extStart].value
}
...
func (l *Loader) SortSub(s Sym) Sym {
	...
	for ss := s; ss != 0; ss = l.sub[ss] {
		sl = append(sl, symWithVal{s: ss, v: l.ssVal(ss)})
	}
