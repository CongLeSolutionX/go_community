...
func (l *Loader) PropagateLoaderChangesToSymbols(toconvert []Sym, syms *sym.Symbols) []*sym.Symbol {
	...
	for _, cand := range toconvert {
		...
		// Also: if this is an info symbol and it refers to a
		// a previously unseen range/loc symbol, we'll need to fix up
		// relocations for it as well.
		...
		for ri := range rslice {
			...
			if st == sym.SDWARFINFO &&
				(rst == sym.SDWARFRANGE || rst == sym.SDWARFLOC) {
				relfix = true
				break
			}
		}
