...
func (l *Loader) addSym(name string, ver int, r *oReader, li uint32, kind int, osym *goobj2.Sym) (Sym, bool) {
	...
	case hashed64Def, hashedDef:
		...
		var checkHash func() (symSizeAlign, bool, func(symSizeAlign))
		if kind == hashed64Def {
			checkHash = func() (symSizeAlign, bool, func(symSizeAlign)) {
				hash := r.Hash64(li - uint32(r.ndef))
				s, existed := l.hashed64Syms[hash]
				return s, existed, func(ss symSizeAlign) { l.hashed64Syms[hash] = ss }
			}
		} else {
			checkHash = func() (symSizeAlign, bool, func(symSizeAlign)) {
				hash := r.Hash(li - uint32(r.ndef+r.nhashed64def))
				s, existed := l.hashedSyms[*hash]
				return s, existed, func(ss symSizeAlign) { l.hashedSyms[*hash] = ss }
			}
		}
		siz := osym.Siz()
		align := osym.Align()
		s, existed, addToHashMap := checkHash()
		if existed {
