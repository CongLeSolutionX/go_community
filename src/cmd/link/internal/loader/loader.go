...
func NewLoader(flags uint32, stats *objstats.SymStats) *Loader {
...
func loadObjRefs(l *Loader, r *oReader, arch *sys.Arch, syms *sym.Symbols) {
	...
	for i, n := 0, r.NNonpkgref(); i < n; i++ {
		...
		name := strings.Replace(osym.Name, "\"\".", r.pkgprefix, -1)
		v := abiToVer(osym.ABI, r.version)
		gsym := l.AddExtSym(name, v)
		if l.stats != nil && gsym != 0 {
...
func (l *Loader) mkAuxMap(r *oReader, istart Sym) gauxmap {
	...
	for li := 0; li < r.NSym()+r.NNonpkgdef(); li++ {
		...
		for ai := 0; ai < naux; ai++ {
			a := goobj2.Aux{}
			a.Read(r.Reader, r.AuxOff(li, ai))
			if a.Sym.PkgIdx != goobj2.PkgIdxSelf {
				continue
			}
...
func (l *Loader) emitBasicDeadStats(f *os.File, reflectSeen bool) {
	...
	fmt.Fprintf(f, "loader: objs=%d extStart=%d max=%d\n\n", len(l.objs),
		l.extStart, l.max)
...
func (l *Loader) emitDeadDetails(f *os.File, rpkg string) {
	...
	for _, o := range l.objs {
		...
		fmt.Fprintf(f, "\nlib: '%s' start:%d end:%d pdef:%d ndef:%d nref:%d\n",
			r.unit.Lib, o.i, o.e, r.NSym(), r.NNonpkgdef(), r.NNonpkgref())
		...
		for li := 0; li < r.NSym()+r.NNonpkgdef(); li++ {
			...
			gsym := l.Lookup(name, v)
			ovr := ""
			if _, ok := l.overwrite[o.i+Sym(li)]; ok {
				dt = "^"
				ovr = fmt.Sprintf(" [overridden to %d]", gsym)
				gsym = o.i + Sym(li)
			} else if gsym == 0 {
				dt = "@"
			} else if gsym != o.i+Sym(li) {
				dt = "*"
			}
			...
			if rpkg != "" && (r.unit.Lib.String() == rpkg || rpkg == "*") {
				...
				for ri := 0; ri < nr; ri++ {
					rel := goobj2.Reloc{}
					rel.Read(r.Reader, r.RelocOff(li, ri))
					target := l.resolve(r, rel.Sym)
					sn := "?"
					sv := 0
					if target != 0 {
						sn = l.SymName(target)
					...
					rsrs := relocSymRefString(rel.Sym, r.NSym())
					ovr := ""
					if oto, ok := l.overwrite[target]; ok {
						ovr = fmt.Sprintf(" [ovr to %d]", oto)
					}
					fmt.Fprintf(f, "    R%d: %-9s o=%d sr=%s si=%d%s s=%s<%d>\n",
						ri, rt.String(), rel.Off, rsrs, target, ovr, sn, sv)
				}
			}
		}
		...
		for li, n := 0, r.NNonpkgref(); li < n; li++ {
			osym := goobj2.Sym{}
			osym.Read(r.Reader, r.SymOff(ndef+li))
			name := strings.Replace(osym.Name, "\"\".", r.pkgprefix, -1)
			v := abiToVer(osym.ABI, r.version)
			gsym := l.Lookup(name, v)
			rch := " "
			if l.Reachable.Has(gsym) {
				rch = "%"
			}
	...
	for i := l.extStart; i < l.max; i++ {
		...
		s := l.extSyms[i-l.extStart]
		fmt.Fprintf(f, "  %s %s G%-7d: %s<%d>\n",
			"exts", rch, i, s.name, s.v)
	}
}
