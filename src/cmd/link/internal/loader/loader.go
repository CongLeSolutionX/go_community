...
type Loader struct {
	...
	align []uint16 // symbol alignment, indexed by global index
...
func (l *Loader) SetSymAlign(i Sym, align int32) {
	...
	// Make sure it fits into uint16
	if int32(uint16(align)) != align {
		panic("alignment value overflows 16 bits")
	}
	if int(i) >= len(l.align) {
		l.align = append(l.align, make([]uint16, l.NSym()-len(l.align))...)
	}
	l.align[i] = uint16(align)
}
