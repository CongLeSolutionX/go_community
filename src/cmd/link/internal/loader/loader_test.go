...
func TestMaterializeSymbol(t *testing.T) {
	...
	if ldr.Reachable.Has(ms1) || ldr.Reachable.Has(ms2) {
		t.Fatalf("newly materialized symbols should not be reachable")
	}
	...
	if !sameRelocSlice(rsl, expRel) {
		t.Fatalf("expected relocs %v, got %v", expRel, rsl)
	}
	for _, idx := range []Sym{ts2, ms2} {
		...
		r0 := relocs.At(0)
		if r0 != expRel[0] {
			t.Fatalf("expected reloc %v, got %v", expRel[0], r0)
		}
	}
	...
	if bytes.Compare(dat, d1) != 0 {
		t.Fatalf("expected ms1 data %v, got %v", d1, dat)
	}
	for _, idx := range []Sym{ts2, ms2} {
		dat := ldr.Data(idx)
		if bytes.Compare(dat, d2) != 0 {
			t.Fatalf("expected ms1 data %v, got %v", d2, dat)
		}
	}
