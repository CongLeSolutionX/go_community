...
//
// func main() {
//   bench := benchmark.New(true, benchmark.GC)
...
//   bar()
// }
func New(enabled bool, gc Flags) *Metrics {
	if !enabled {
		return nil
	}
	if gc == GC {
...
func (m *Metrics) Report(w io.Writer) {
	...
	for _, curMark := range m.marks {
		...
		totTime += dur
		fmt.Fprintf(w, "%s 1 %d ms/op", makeBenchString(curMark.name+gcString), dur.Milliseconds())
