...
func elfreloc1(ctxt *ld.Link, out *ld.OutBuf, ldr *loader.Loader, s loader.Sym, r loader.ExtReloc, ri int, sectoff int64) bool {
	...
	case objabi.R_LOONG64_TLS_LE_ADDR_LO:
		out.Write64(uint64(sectoff))
		out.Write64(uint64(elf.R_LARCH_TLS_LE_LO12) | uint64(elfsym)<<32)
		out.Write64(uint64(r.Xadd))

	case objabi.R_LOONG64_TLS_LE_ADDR_HI:
		out.Write64(uint64(sectoff))
...
func archreloc(target *ld.Target, ldr *loader.Loader, syms *ld.ArchSyms, r loader.Reloc, s loader.Sym, val int64) (o int64, nExtReloc int, ok bool) {
	...
	if target.IsExternal() {
		...
		case objabi.R_LOONG64_ADDR_HI,
			...
			return val, 1, true
		case objabi.R_LOONG64_TLS_LE_ADDR_HI,
			objabi.R_LOONG64_TLS_LE_ADDR_LO,
			objabi.R_CALLLOONG64,
			objabi.R_JMPLOONG64,
			objabi.R_LOONG64_TLS_IE_ADDR_HI,
			objabi.R_LOONG64_TLS_IE_ADDR_LO:
			return val, 1, true
		}
	}
	...
	case objabi.R_LOONG64_TLS_LE_ADDR_HI,
		...
		if r.Type() == objabi.R_LOONG64_TLS_LE_ADDR_LO {
	...
	case objabi.R_LOONG64_TLS_IE_ADDR_HI,
		...
		if target.IsPIE() && target.IsElf() {
			...
			t := ldr.SymAddr(rs) + r.Add()
			if r.Type() == objabi.R_LOONG64_TLS_IE_ADDR_HI {
				// pcalau12i -> lu12i.w
...
func isRequestingLowPageBits(t objabi.RelocType) bool {
	switch t {
	case objabi.R_LOONG64_ADDR_LO, objabi.R_LOONG64_GOT_LO:
		return true
	}
	return false
}
