...
func adddynrel(target *ld.Target, ldr *loader.Loader, syms *ld.ArchSyms, s loader.Sym, r loader.Reloc, rIdx int) bool {
	...
	case objabi.ElfRelocOffset + objabi.RelocType(elf.R_LARCH_GOT_PC_HI20),
		...
		if targType != sym.SDYNIMPORT {
			// TODO: trun LDR of GOT entry into ADR of symbol itself
		}
		...
		if r.Type() == objabi.ElfRelocOffset+objabi.RelocType(elf.R_LARCH_GOT_PC_HI20) {
			su.SetRelocType(rIdx, objabi.R_LOONG64_GOT_HI)
		} else {
			su.SetRelocType(rIdx, objabi.R_LOONG64_GOT_LO)
		}
	...
	case objabi.ElfRelocOffset + objabi.RelocType(elf.R_LARCH_PCALA_HI20),
		...
		if r.Type() == objabi.ElfRelocOffset+objabi.RelocType(elf.R_LARCH_PCALA_HI20) {
			su.SetRelocType(rIdx, objabi.R_LOONG64_PCREL_HI)
		} else {
			su.SetRelocType(rIdx, objabi.R_LOONG64_PCREL_LO)
		}
		return true
	}
...
func elfsetupplt(ctxt *ld.Link, ldr *loader.Loader, plt, gotplt *loader.SymbolBuilder, dynamic loader.Sym) {
	if plt.Size() == 0 {
		// pcalau12i $r14, imm
		plt.AddSymRef(ctxt.Arch, gotplt.Sym(), 0, objabi.R_LOONG64_GOT_HI, 4)
		...
		// ld.d $r15, $r14, imm
		plt.AddSymRef(ctxt.Arch, gotplt.Sym(), 0, objabi.R_LOONG64_GOT_LO, 4)
...
func addpltsym(target *ld.Target, ldr *loader.Loader, syms *ld.ArchSyms, s loader.Sym) {
	...
	if target.IsElf() {
		...
		relocs := plt.Relocs()
		plt.SetRelocType(relocs.Count()-1, objabi.R_LOONG64_GOT_HI)

		// ld.d $r15, $r15, imm
		plt.AddAddrPlus4(target.Arch, gotplt.Sym(), gotplt.Size())
		plt.SetUint32(target.Arch, plt.Size()-4, 0x28c001ef)
		relocs = plt.Relocs()
		plt.SetRelocType(relocs.Count()-1, objabi.R_LOONG64_GOT_LO)
...
func archreloc(target *ld.Target, ldr *loader.Loader, syms *ld.ArchSyms, r loader.Reloc, s loader.Sym, val int64) (o int64, nExtReloc int, ok bool) {
	...
	case objabi.R_LOONG64_PCREL_HI,
		objabi.R_LOONG64_PCREL_LO:
		pc := ldr.SymValue(s) + int64(r.Off())
		t := calculatePCAlignedReloc(r.Type(), ldr.SymAddr(rs)+r.Add(), pc)
		if r.Type() == objabi.R_LOONG64_PCREL_HI {
			return int64(val&0xfe00001f | (t << 5)), noExtReloc, isOk
		}
		return int64(val&0xfffc003ff | (t << 10)), noExtReloc, isOk
	...
	case objabi.R_LOONG64_TLS_IE_PCREL_HI,
		...
		if target.IsPIE() && target.IsElf() {
			...
			t := ldr.SymAddr(rs) + r.Add()
			if r.Type() == objabi.R_LOONG64_TLS_IE_PCREL_HI {
				// pcalau12i -> lu12i.w
...
func isRequestingLowPageBits(t objabi.RelocType) bool {
	switch t {
	case objabi.R_ADDRLOONG64, objabi.R_LOONG64_GOT_LO, objabi.R_LOONG64_PCREL_LO:
		return true
	}
	return false
}
