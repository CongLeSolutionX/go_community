...
func Load(l *loader.Loader, arch *sys.Arch, localSymVersion int, input *bio.Reader, pkg string, length int64, pn string) (textp []loader.Sym, err error) {
	...
	roLookup := func(name string, version int) (*loader.SymbolBuilder, loader.Sym) {
		return nil, l.LookupOrCreateSym(name, version)
	}
	...
	for _, sect := range f.Sections {
		...
		if sect.Name == ".rsrc" && l.Lookup(".rsrc", localSymVersion) != 0 {
			panic("too many .rsrc sections")
		}
	...
	for _, rsect := range f.Sections {
		...
		for j, r := range rsect.Relocs {
			...
			_, gosym, err := readpesym(l, arch, roLookup, f, pesym, sectsyms, localSymVersion)
			if err != nil {
				return nil, err
			}
			if gosym == 0 {
			...
			case sys.I386, sys.AMD64:
				switch r.Type {
				default:
					return nil, fmt.Errorf("%s: %v: unknown relocation type %v", pn, sectsyms[rsect], r.Type)
			...
			case sys.ARM:
				switch r.Type {
				default:
					return nil, fmt.Errorf("%s: %v: unknown ARM relocation type %v", pn, sectsyms[rsect], r.Type)
	...
	for i, numaux := 0, 0; i < len(f.COFFSymbols); i += numaux + 1 {
		...
		bld, s, err := readpesym(l, arch, roLookup, f, pesym, sectsyms, localSymVersion)
		if err != nil {
			return nil, err
		}
...
func readpesym(l *loader.Loader, arch *sys.Arch, lookup func(string, int) (*loader.SymbolBuilder, loader.Sym), f *pe.File, pesym *pe.COFFSymbol, sectsyms map[*pe.Section]loader.Sym, localSymVersion int) (*loader.SymbolBuilder, loader.Sym, error) {
