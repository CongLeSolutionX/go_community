...
func Load(l *loader.Loader, arch *sys.Arch, localSymVersion int, input *bio.Reader, pkg string, length int64, pn string) (textp []loader.Sym, rsrc []loader.Sym, err error) {
	...
	for _, sect := range f.Sections {
		...
		s := state.lookup(name, localSymVersion)
		bld := l.MakeSymbolUpdater(s)
...
func (state *peLoaderState) readpesym(pesym *pe.COFFSymbol) (*loader.SymbolBuilder, loader.Sym, error) {
	...
	case IMAGE_SYM_DTYPE_FUNCTION, IMAGE_SYM_DTYPE_NULL:
		...
		case IMAGE_SYM_CLASS_EXTERNAL: //global
			s = state.lookup(name, 0)

		case IMAGE_SYM_CLASS_NULL, IMAGE_SYM_CLASS_STATIC, IMAGE_SYM_CLASS_LABEL:
			s = state.lookup(name, state.localSymVersion)
...
func (state *peLoaderState) preprocessSymbols() error {
	...
	for i, numaux := 0, 0; i < len(state.f.COFFSymbols); i += numaux + 1 {
		...
		if aux.Selection != IMAGE_COMDAT_SELECT_ANY {
			...
			return fmt.Errorf("internal error: unsupported COMDAT selection strategy found in path=%d sec=%s idx=%d, please file a bug", state.pn, aux.SecNum, i)
		}
	}
