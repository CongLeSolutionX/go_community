...
// Locking/unlocking used for unit testing only.
...
func (snt *SymNameTable) addString(s string) uintptr {
	idx := uintptr(len(snt.allnames))
...
func (snt *SymNameTable) install(s string) uintptr {
	hc := snt.hasher.Sum64([]byte(s))
	if val, ok := snt.hash[hc]; ok {
		if isCollision(val) || s != snt.allnames[val] {
			// handle collisions
			return snt.handleCollision(s, hc, uint64(val))
		} else {
			return uintptr(val)
		}
	}
...
func (snt *SymNameTable) handleCollision(s string, hc uint64, val uint64) uintptr {
	var colslot uint64
...
// for unit testing.
func (snt *SymNameTable) fragmentString(fragment uintptr) string {
