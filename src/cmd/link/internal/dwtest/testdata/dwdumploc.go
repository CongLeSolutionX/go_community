...
// 1: in-param "loadmodule" loc="{ [0: S=8 RAX] [1: S=8 RBX] }"
...
// Return values "d" is the debug/dwarf.Data for the binary if we
// can find it, otherwise an error describing what went wrong (first
// error for binary, second error for DWARF).
type opener interface {
	Open(path string) (*dwarf.Data, error, error)
	Which() string
}

// Remark: the boilerplate for three scenarios below (Elf, Macho, and PE)
// seems like it should be an ideal scenario where you could use
// generics, but my attempts at this were not especially successful.
// The sticking point is that each of elf.Open/macho.Open etc return
// a different thing, and I couldn't quite figure out how to get this to
// work with generics.
...
// returns a dwarf.Data for the binary; if they all file, it returns
// an error.
...
// locateFuncDetails walks the DWARF for load module 'loadmodule'
...
func locateFuncDetails(loadmodule string, fcn string) (finfo, error) {
	rrv := finfo{}

	verb(1, "loading DWARF for %s", loadmodule)
	d, err := openDwarf(loadmodule)
	if err != nil {
		return finfo{}, err
	}
...
// in question and then walks the walks the formal parameters of the
// selected function, invoking the Location method on each param to
// read its location expression. Results are dumped to stdout, and an
// error is returned if something goes wrong.
func processParams(loadmodule string, fi *finfo) error {
	const _cfa = 0x1000
...
// Go binary 'loadmodule', printing results to stdout if possible.
func examineFile(loadmodule string, fcn string) {
	fi, err := locateFuncDetails(loadmodule, fcn)
	if err != nil {
		log.Fatalf("error: %v\n", err)
	}
	if !fi.valid {
		log.Fatalf("could not locate target function %s in loadmodule %s", fcn, loadmodule)

	}
	if err := processParams(loadmodule, &fi); err != nil {
		log.Fatalf("error: %v\n", err)
	}
}
