...
func (d *deadcodepass) mark(s, parent *sym.Symbol) {
	...
	if *flagDumpDep {
		p := "_"
		if parent != nil {
			p = d.ctxt.Syms.SymName(parent)
		}
		fmt.Printf("%s -> %s\n", p, d.ctxt.Syms.SymName(s))
	}
	...
	for len(d.markQueue) > 0 {
		...
		if len(methods) > 0 {
			...
			if len(methods) != len(methodsigs) {
				panic(fmt.Sprintf("%q has %d method relocations for %d methods", d.ctxt.Syms.SymName(s), len(methods), len(methodsigs)))
			}
			for i, m := range methodsigs {
				name := string(m)
				name = name[:strings.Index(name, "(")]
				iname := d.ctxt.Syms.SymName(methods[i].ifn())
				if !strings.HasSuffix(iname, name) {
					panic(fmt.Sprintf("%q relocation for %q does not match method %q", d.ctxt.Syms.SymName(s), iname, name))
				}
				methods[i].m = m
			}
