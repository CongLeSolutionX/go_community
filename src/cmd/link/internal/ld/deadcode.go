...
//
// The third case is handled by looking for methods that compiler flagged
// as REFLECTMETHOD. REFLECTMETHOD on a method F means that the compiler
// was not able to statically determine what methods F looks up via reflection.
//
// All methods that call reflect.Value.Method() or reflect.Type.Method()
// are REFLECTMETHODs. Methods that call reflect.Value.MethodByName() or
// reflect.Type.MethodByName() with a non-constant argument are REFLECTMETHODs,
// too. If we find a REFLECTMETHOD, we give up on static analysis, and mark all
// exported methods of all reachable types as reachable.
//
