...
func (q *workQueue) push(i int)  { *q = append(*q, i) }
func (q *workQueue) pop() int    { i := (*q)[len(*q)-1]; *q = (*q)[:len(*q)-1]; return i }
...
func (d *deadcodePass2) flood() {
	for !d.wq.empty() {
		...
		d.reflectSeen = d.reflectSeen || (d.loader.SymAttr(symIdx)&goobj2.SymFlagReflectMethod != 0) // TODO: maybe this file shouldn't import goobj2 package?
...
func (d *deadcodePass2) mark(symIdx int) {
...
func (d *deadcodePass2) markMethod(m methodref2) {
	d.mark(d.loader.RelocSym(m.src, m.r))
	d.mark(d.loader.RelocSym(m.src, m.r+1))
	d.mark(d.loader.RelocSym(m.src, m.r+2))
}
...
func deadcode2(ctxt *Link) {
	...
	if ctxt.BuildMode != BuildModeShared {
		...
		for i := 1; i < n; i++ {
			if strings.HasPrefix(loader.RawSymName(i), "go.itablink.") {
...
func decodeIfaceMethods2(loader *objfile.Loader, arch *sys.Arch, symIdx int) []methodsig {
	...
	r := decodeReloc2(loader, symIdx, int32(commonsize(arch)+arch.PtrSize))
	if r == -1 {
		return nil
	}
	if loader.RelocSym(symIdx, r) != symIdx {
		panic(fmt.Sprintf("imethod slice pointer in %q leads to a different symbol", loader.SymName(symIdx)))
	}
	off := int(loader.RelocAdd(symIdx, r)) // array of reflect.imethod values
...
func decodeRelocSym2(loader *objfile.Loader, symIdx int, off int32) int {
	...
	return loader.RelocSym(symIdx, r)
}
