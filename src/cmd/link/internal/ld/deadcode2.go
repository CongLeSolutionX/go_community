	...
	if d.ctxt.linkShared && (d.ctxt.BuildMode == BuildModeExe || d.ctxt.BuildMode == BuildModePIE) {
		...
		if d.ctxt.BuildMode == BuildModePlugin {
			...
			if exportsIdx != 0 {
				...
				for i := 0; i < len(d.rwork); i++ {
					d.mark(d.rwork[i].Sym)
				}
			}
		}
	}
...
func (d *deadcodePass2) markMethod(m methodref2) {
	d.ReadRelocs(m.src)
	d.mark(d.rwork[m.r].Sym)
	d.mark(d.rwork[m.r+1].Sym)
	d.mark(d.rwork[m.r+2].Sym)
}
...
func (d *deadcodePass2) decodeMethodSig2(ldr *loader.Loader, arch *sys.Arch, symIdx loader.Sym, symRelocs []loader.Reloc, off, size, count int) []methodsig {
	...
	for i := 0; i < count; i++ {
		...
		for i := 0; i < inCount; i++ {
			...
			a := d.decodetypeFuncInType2(ldr, arch, mtypSym, i)
		...
		for i := 0; i < outCount; i++ {
			...
			a := d.decodetypeFuncOutType2(ldr, arch, mtypSym, i)
			buf.WriteString(ldr.SymName(a))
		}
		buf.WriteRune(')')
...
func (d *deadcodePass2) decodetypeMethods2(ldr *loader.Loader, arch *sys.Arch, symIdx loader.Sym, rs []loader.Reloc) []methodsig {
	p := ldr.Data(symIdx)
	...
	return d.decodeMethodSig2(ldr, arch, symIdx, rs, off, sizeofMethod, mcount)
}
...
func (d *deadcodePass2) decodetypeFuncInType2(ldr *loader.Loader, arch *sys.Arch, symIdx loader.Sym, i int) loader.Sym {
	...
	// FIXME: add some sort of caching here, since we see may of the
	// same symbols over time.
	d.ReadRelocs(symIdx)
	return decodeRelocSym2(ldr, symIdx, d.rwork, int32(uadd+i*arch.PtrSize))
}

func (d *deadcodePass2) decodetypeFuncOutType2(ldr *loader.Loader, arch *sys.Arch, symIdx loader.Sym, i int) loader.Sym {
	return d.decodetypeFuncInType2(ldr, arch, symIdx, i+decodetypeFuncInCount(arch, ldr.Data(symIdx)))
}
...
func (d *deadcodePass2) ReadRelocs(symIdx loader.Sym) {
	relocs := d.ldr.Relocs(symIdx)
	d.rwork = relocs.ReadAll(d.rwork)
}
