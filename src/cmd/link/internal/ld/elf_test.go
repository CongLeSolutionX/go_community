	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, wsroname := range test.wantSecsRO {
				...
				if !foundRO {
					// Things went south. Write out some useful information
					// to go on for anyone looking at the test failure.
					t.Logf("segments: \n")
					for k, p := range elfFile.Progs {
						t.Logf("%d t=%s fl=%s st=0x%x en=0x%x\n",
							k, p.Type, p.Flags, p.Vaddr, p.Vaddr+p.Filesz)
					}
					t.Logf("%s: st=0x%x en=0x%x\n",
						wsro.Name, wsro.Addr, wsro.Addr+wsro.FileSize)
					t.Fatalf("test %s: %q section not in readonly segment",
						wsro.Name, test.name)
				}
			}
		})
