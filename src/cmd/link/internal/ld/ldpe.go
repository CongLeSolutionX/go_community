...
func ldpe(input *bio.Reader, pkg string, length int64, pn string) {
	if Debug['v'] != 0 {
		fmt.Fprintf(Bso, "%5.2f ldpe %s\n", obj.Cputime(), pn)
	}

	sectsyms := make(map[*pe.Section]*LSym)
	...
	for _, rsect := range f.Sections {
		...
		for j, r := range rsect.Relocs {
			...
			gosym, err2 := readpesym(f, pesym, sectsyms)
			if err2 != nil {
				err = err2
				goto bad
			}
			...
			switch r.Type {
			default:
				Diag("%s: unknown relocation type %d;", pn, r.Type)
				fallthrough
	...
	for i, numaux := 0, 0; i < len(f.COFFSymbols); i += numaux + 1 {
		...
		s, err2 := readpesym(f, pesym, sectsyms)
		if err2 != nil {
			err = err2
			goto bad
		}
...
func readpesym(f *pe.File, sym *pe.COFFSymbol, sectsyms map[*pe.Section]*LSym) (*LSym, error) {
