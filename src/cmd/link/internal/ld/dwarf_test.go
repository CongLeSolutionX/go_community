...
const (
	...
	MinDwarfOpt = "-gcflags=all=-minimaldwarf"
	NoAsmFlags  = "-asmflags="
	MinDwarfAsm = "-asmflags=all=-minimaldwarf"
)
...
func TestMinimalDwarf(t *testing.T) {
	testenv.MustHaveGoBuild(t)
	if testing.Short() {
		t.Skip("skipping test in short mode.")
	}
	...
	dir, err := ioutil.TempDir("", "TestMinimalDwarf")
	if err != nil {
		t.Fatalf("could not create directory: %v", err)
	}
	defer os.RemoveAll(dir)
	...
	// Build a smallish testcase with -minimaldwarf enabled. Code in
	...
	// test of -minimaldwarf, but make sure we have:
	//
	...
	for i, die := range ex.dies {
		...
		case dwarf.TagVariable, dwarf.TagConstant:
			ex.dumpEntry(i, false, 0)
			t.Errorf("unexpected tag %s found with -minimaldwarf", die.Tag)
			badcount++
		}
