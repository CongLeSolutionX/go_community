...
func TestIssue42484(t *testing.T) {
	...
	f := gobuildTestdata(t, tmpdir, pdir, NoOpt, NoAsmFlags)
	defer f.Close()
...
func TestDwarfOnlyLine(t *testing.T) {
	...
	// Walk through the generated DWARF and look to make sure there
	// aren't any unexpected things. This isn't an exhaustive test of
	// -dwarfonlyline, but make sure we have:
	//
