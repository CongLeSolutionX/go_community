...
func (sect *peSection) emitRelocations(out *OutBuf, relocfn func() int) {
	...
	cpos := out.Offset()
	out.Seek(int64(sect.pointerToRelocations))
...
func (f *peFile) addInitArray(ctxt *Link) *peSection {
	...
	ctxt.Out.Seek(int64(sect.pointerToRawData))
...
func addimports(ctxt *Link, datsect *peSection) {
	...
	ctxt.Out.Seek(startoff + int64(binary.Size(&IMAGE_IMPORT_DESCRIPTOR{}))*int64(n+1))
	...
	ctxt.Out.Seek(int64(uint64(datsect.pointerToRawData) + ftbase))
	for d := dr; d != nil; d = d.next {
		for m = d.ms; m != nil; m = m.next {
			if pe64 != 0 {
				ctxt.Out.Write64(m.off)
...
func Asmbpe(ctxt *Link) {
	...
	ctxt.Out.Seek(int64(pefile.nextFileOffset))
	if Linkmode != LinkExternal {
		addimports(ctxt, d)
		addexports(ctxt)
	}
