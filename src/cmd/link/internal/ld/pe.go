	...
	relocsect := func(sect *sym.Section, syms []*sym.Symbol, base uint64) int {
		...
		for _, sym := range syms {
			...
			for ri := range sym.R {
				...
				if r.Xsym.Dynid < 0 {
					ctxt.Errorf(sym, "reloc %d to non-coff symbol %s (outer=%s) %d", r.Type, ctxt.SymName(r.Sym), ctxt.SymName(r.Xsym), r.Sym.Type)
				}
				if !thearch.PEreloc1(ctxt, ctxt.Out, sym, r, int64(uint64(sym.Value+int64(r.Off))-base)) {
					ctxt.Errorf(sym, "unsupported obj reloc %d/%d to %s", r.Type, r.Siz, ctxt.SymName(r.Sym))
				}
				relocs++
			}
		}
...
func (f *peFile) mapToPESection(ctxt *Link, s *sym.Symbol, linkmode LinkMode) (pesectidx int, offset int64, err error) {
	if s.Sect == nil {
		return 0, 0, fmt.Errorf("could not map %s symbol with no section", ctxt.SymName(s))
	}
	...
	if s.Sect.Seg != &Segdata {
		return 0, 0, fmt.Errorf("could not map %s symbol with non .text or .rdata or .data section", ctxt.SymName(s))
	}
...
func (f *peFile) writeSymbols(ctxt *Link) {
	...
	put := func(ctxt *Link, s *sym.Symbol, name string, type_ SymbolType, addr int64, gotype *sym.Symbol) {
		...
		if ctxt.Arch.Family == sys.I386 &&
			...
			ctxt.SetSymName(s, "_"+ctxt.SymName(s))
		}
