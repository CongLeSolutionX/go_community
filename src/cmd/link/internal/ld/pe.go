...
// Represents a Base Relocation Block. A block is a collection
// of relocation entries in a page, where each entry describes
// a single relocation.
// The block page RVA is the index into peBaseRelocTable.blocks
type peBaseRelocBlock struct {
	entries []peBaseRelocEntry
}

// Type used to store list of pages for which there are base
// relocation blocks. Defined as a separate type so that it can be sorted.
type pePages []uint32
...
type peBaseRelocTable struct {
	...
	// List of keys into blocks map. Stored separately for ease of
	// sorting.
	pages pePages
}
...
func (rt *peBaseRelocTable) addentry(ctxt *Link, s *sym.Symbol, r *sym.Reloc) {
	// Size in bytes of a page described by a base relocation block
