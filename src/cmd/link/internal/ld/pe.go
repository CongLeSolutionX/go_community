...
func addpersrc(ctxt *Link) {
	...
	var size int
	for i := range rsrcsyms {
		size += len(ctxt.loader.Data(rsrcsyms[i]))
	}
	...
	for i := range rsrcsyms {
		splitResources := strings.Contains(ctxt.loader.SymName(rsrcsyms[i]), ".rsrc$")
		relocs := ctxt.loader.Relocs(rsrcsyms[i])
		data := ctxt.loader.Data(rsrcsyms[i]) //XXX: THIS HAS SIZE ZERO AND CRASHES ON GO 1.16! `ctxt.Loader.Syms[rsrcsyms[i]].P` is what we want.
		for ri := 0; ri < relocs.Count(); ri++ {
			r := relocs.At(ri)
			p := data[r.Off():]
