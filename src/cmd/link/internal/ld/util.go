...
func (ds *dumpSyms) dump(syms []loader.Sym) {
	...
	for _, s := range syms {
		...
		dlen := len(l.Data(s))
		content := ""
		if ds.acontent || ds.xcontent {
			data := l.Data(s)
			if len(data) != 0 {
				// Don't try to emit content for giant symbols.
				if len(data) > 2048 {
					dl := len(data)
					if ds.xcontent {
						content = fmt.Sprintf(" xdata: %+x ... %+x", data[0:1023], data[dl-1024:dl])
					}
					if ds.acontent {
						content = fmt.Sprintf(" adata: %q ... %q\n", string(data[0:1023]), string(data[dl-1024:dl]))
					}
				} else {
					if ds.xcontent {
						content = fmt.Sprintf(" xdata: %+x", data)
					}
					if ds.acontent {
						content = fmt.Sprintf(" adata: %q\n", string(data))
					}
				}
			}
		}
		snum := fmt.Sprintf("%d", s)
		if ds.diffFriendly {
			snum = "?"
		}
		fmt.Fprintf(os.Stderr, "%sS%s: %s<%d> t=%s sz=%d dlen=%d val=%d%s%s%s%s%s\n", rch, snum, sn, sver, st.String(), sz, dlen, sval, sct, ds.auxTag(s, am), subinfo, pkinfo, content)
		if ds.dorelocs {
			relocs := l.Relocs(s)
			for ri := 0; ri < relocs.Count(); ri++ {
				...
				rauxtag := ""
				if ds.doaux {
					rauxtag = ds.auxTag(r.Sym(), am)
				}
				fmt.Fprintf(os.Stderr, "  + R%d: %s%-9s o=%d a=%d tgt=%s%s%s%s%s\n", ri, wk, r.Type().String(), r.Off(), r.Add(), rsrs, rvs, rauxtag, frel, rvr)
			}
		}
	}
}
...
where
 ...
 {aux} is a clause of the form [<letters>...] where letters are:

  A => enables dumping of aux relationships
  C => enables dumping of data content (ascii) for each sym
  D => suppress symbol indices (diff friendly)
  X => enables dumping of data content (hex) for each sym
  I => enables dumping of relocations with tgt symbol names and indices
  O => enables dumping of outer/sub relationships
  P => enables dumping of Pkg for each sym
  R => enables dumping of relocations
 ...
 Examples:
   ...
   - dump text symbols (with relocs) and NOPTRBSS symbols at dodata + final:
       ...
       [R]STEXT,SNOPTRBSS@dodata,final
   ...
   - dump data/bss/noptrdata/noptrbss in diff-friendly format

       [D]SDATA,SBSS,SNOPTRDATA,SNOPTRBSS@dodata
`
