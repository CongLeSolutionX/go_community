...
func (ctxt *Link) inittasks() {
	if ctxt.BuildMode == BuildModePlugin {
		ctxt.inittasksForPlugin = ctxt.inittaskSym(fmt.Sprintf("%s..inittask", *flagPluginPath), "go:plugin.inittasks")
		return
	}
	ctxt.inittask("runtime..inittask", "runtime.runtime_inittasks")
	ctxt.inittask("main..inittask", "runtime.main_inittasks")
}

func (ctxt *Link) inittask(rootName, schedName string) {
	t := ctxt.inittaskSym(rootName, schedName+"_backingstore")

	// This slice header is already defined in runtime/proc.go, so we update it here with new contents.
	ldr := ctxt.loader
	sh := ldr.Lookup(schedName, 0)
...
func (ctxt *Link) inittaskSym(rootName, symName string) loader.Sym {
	...
	if root == 0 {
		Exitf("can't find inittask for %s", rootName)
	}
