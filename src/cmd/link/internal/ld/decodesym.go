...
func decodetypeGcprog(ctxt *Link, s *sym.Symbol) []byte {
	if s.Type == sym.SDYNIMPORT {
		...
		Exitf("cannot find gcprog for %s", ctxt.SymName(s))
		return nil
	}
...
func decodetypeGcmask(ctxt *Link, s *sym.Symbol) []byte {
	if s.Type == sym.SDYNIMPORT {
		...
		Exitf("cannot find gcmask for %s", ctxt.SymName(s))
		return nil
	}
...
func decodeMethodSig(ctxt *Link, s *sym.Symbol, off, size, count int) []methodsig {
	...
	for i := 0; i < count; i++ {
		...
		for i := 0; i < inCount; i++ {
			...
			buf.WriteString(ctxt.SymName(fits))
		}
		buf.WriteString(") (")
		...
		for i := 0; i < outCount; i++ {
			...
			buf.WriteString(ctxt.SymName(fots))
		}
		buf.WriteRune(')')
...
func decodeIfaceMethods(ctxt *Link, s *sym.Symbol) []methodsig {
	...
	if decodetypeKind(arch, s)&kindMask != kindInterface {
		panic(fmt.Sprintf("symbol %q is not an interface", ctxt.SymName(s)))
	}
	...
	if r.Sym != s {
		panic(fmt.Sprintf("imethod slice pointer in %q leads to a different symbol", ctxt.SymName(s)))
	}
...
func decodetypeMethods(ctxt *Link, s *sym.Symbol) []methodsig {
	arch := ctxt.Arch
	if !decodetypeHasUncommon(arch, s) {
		panic(fmt.Sprintf("no methods on %q", ctxt.SymName(s)))
	}
