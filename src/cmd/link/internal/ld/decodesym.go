...
func decodetypeStructFieldArrayOff(ldr *loader.Loader, arch *sys.Arch, symIdx loader.Sym, i int) int {
	...
	if !wideSliceAlign() && off != uint64(commonsize(arch)+4*arch.PtrSize) {
		panic("darn it!")
	}
	if decodetypeHasUncommon(arch, data) {
		off += uint64(uncommonSize(arch))
	}
	off += uint64(i * structfieldSize(arch))
	return int(off)
}
