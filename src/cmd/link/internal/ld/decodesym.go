...
func decodeTargetSym(ctxt *Link, arch *sys.Arch, s loader.Sym, off int64) loader.Sym {
	...
	if ldr.SymType(s) == sym.SDYNIMPORT {
		...
		for _, sh := range ctxt.Shlibs {
			...
			addr += uint64(off)
			relocs := sh.relocs
			i, ok := sort.Find(len(relocs),
				func(i int) int {
					if addr < relocs[i].addr {
						return -1
					}
					if addr > relocs[i].addr {
						return 1
			...
			t := ldr.Lookup(relocs[i].target, 0)
			if t == 0 {
				Exitf("can't find target of relocation in %s at offset %d: %s", name, off, relocs[i].target)
			}
			return t
		}
	}
