...
func machoCombineDwarf(ctxt *Link, exef *os.File, exem *macho.File, dsym, outexe string) error {
	...
	reader := imacho.NewLoadCmdReader(outf, exem.ByteOrder, cmdOffset)
...
func machoUpdateSegment(r imacho.LoadCmdReader, linkseg *macho.Segment, linkoffset uint64) error {
...
func machoUpdateSections(r imacho.LoadCmdReader, seg *macho.Segment64, deltaOffset uint64, compressedSects []*macho.Section) error {
	nsect := seg.Nsect
	if nsect == 0 {
		return nil
	}
...
func machoUpdateDwarfHeader(r *imacho.LoadCmdReader, compressedSects []*macho.Section, dwarfsize uint64, dwarfstart int64, realdwarf *macho.Segment) error {
	cmd, err := r.Next()
	if err != nil {
		return err
	}
...
func machoUpdateLoadCommand(r imacho.LoadCmdReader, linkseg *macho.Segment, linkoffset uint64, cmd interface{}, fields ...string) error {
