...
func (d *dwctxt2) importInfoSymbol(ctxt *Link, dsym loader.Sym) {
	...
	dsu := ctxt.loader.MakeSymbolUpdater(dsym)
	...
	for i := 0; i < len(rslice); i++ {
		...
		tsu := ctxt.loader.MakeSymbolUpdater(r.Sym)
		if tsu.Size() == 0 {
...
func (d *dwctxt2) newdie(ctxt *Link, parent *dwarf.DWDie, abbrev int, name string, version int) *dwarf.DWDie {
	...
	dsu := ctxt.loader.MakeSymbolUpdater(ds)
	dsu.SetType(sym.SDWARFINFO)
...
func (d *dwctxt2) dotypedef(ctxt *Link, parent *dwarf.DWDie, name string, def *dwarf.DWDie) *dwarf.DWDie {
	...
	tdsu := ctxt.loader.MakeSymbolUpdater(tds)
