...
func (d *dwctxt2) importInfoSymbol(ctxt *Link, dsym loader.Sym) {
	ctxt.loader.Reachable.Set(dsym)
	// dsym.Attr |= sym.AttrNotInSymbolTable
	ctxt.loader.SetType(dsym, sym.SDWARFINFO)
	relocs := ctxt.loader.Relocs(dsym)
	...
	for i := 0; i < len(rslice); i++ {
		...
		if ctxt.loader.SymSize(r.Sym) == 0 {
			n := nameFromDIESym2(ctxt, r.Sym)
...
func (d *dwctxt2) newdie(ctxt *Link, parent *dwarf.DWDie, abbrev int, name string, version int) *dwarf.DWDie {
	...
	ds := ctxt.loader.GetExtSym(dwarf.InfoPrefix+name, version)
