...
func xcoffAlign(ldr *loader.Loader, x sym.ID, t SymbolType) uint8 {
...
func (f *xcoffFile) writeSymbolFunc(ctxt *Link, x sym.ID) []xcoffSym {
...
func putaixsym(ctxt *Link, x sym.ID, t SymbolType) {
...
func (f *xcoffFile) asmaixsym(ctxt *Link) {
	...
	shouldBeInSymbolTable := func(s sym.ID, name string) bool {
	...
	for s, nsym := sym.ID(1), sym.ID(ldr.NSym()); s < nsym; s++ {
...
func (f *xcoffFile) adddynimpsym(ctxt *Link, s sym.ID) {
...
func Xcoffadddynrel(target *Target, ldr *loader.Loader, syms *ArchSyms, s sym.ID, r loader.Reloc, rIdx int) bool {
...
func (f *xcoffFile) emitRelocations(ctxt *Link, fileoff int64) {
	...
	relocsect := func(sect *sym.Section, syms []sym.ID, base uint64) uint32 {
