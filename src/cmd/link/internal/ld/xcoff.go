...
// getXCOFFalign returns the log base 2 of the symbol's alignment.
func getXCOFFalign(x *sym.Symbol, t SymbolType) uint8 {
	align := x.Align
	if align == 0 {
		if t == TextSym {
	...
	return getLogBase2(int(align))
}

// getLogBase2 returns the log in base 2 of a.
func getLogBase2(a int) uint8 {
	n := uint8(0)
	for a != 1 {
		a >>= 1
		n++
...
func (f *xcoffFile) writeSymbolNewFile(ctxt *Link, name string, firstEntry uint64, extnum int16) {
	...
	aux := &XcoffAuxCSect64{
		Xsmclas:  XMC_PR,
		Xsmtyp:   XTY_SD | getLogBase2(Funcalign)<<3,
		Xauxtype: _AUX_CSECT,
	}
	f.addSymbol(aux)
...
func (f *xcoffFile) writeSymbolFunc(ctxt *Link, x *sym.Symbol) []xcoffSym {
	...
	a4.Xsmtyp |= uint8(getXCOFFalign(x, TextSym) << 3)
...
func putaixsym(ctxt *Link, x *sym.Symbol, str string, t SymbolType, addr int64, go_ *sym.Symbol) {
	...
	case DataSym, BSSSym:
		...
		a4.Xsmtyp |= uint8(getXCOFFalign(x, t) << 3)
...
// at the very ends, especially after relocation sections which needs symbols' index.
...
func (f *xcoffFile) writeFileHeader(ctxt *Link) {
	...
	if ctxt.BuildMode == BuildModeExe && ctxt.LinkMode == LinkInternal {
		...
		f.xahdr.Oalgntext = int16(getLogBase2(int(Funcalign)))
