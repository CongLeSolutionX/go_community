...
func (f *xcoffFile) writeSymbolNewFile(ctxt *Link, name string, firstEntry uint64, extnum int16) {
	...
	for _, sect := range Segdwarf.Sections {
		...
		// This size is set during DWARF generation (cf dwarf.go).
	...
	case TextSym:
		if x.FuncInfo != nil {
			...
			if x.Name != "runtime.text" && x.Name != "runtime.etext" && x.Name != "go.buildid" {
				Exitf("putaixsym: unknown text symbol %s\n", x.Name)
			}
			s := &XcoffSymEnt64{
				Nsclass: C_HIDEXT,
 ...
 * according to information retrieve in xfile object.
 */
