...
func (ctxt *Link) pclntab() {
	...
	for _, s := range ctxt.Textp {
		...
		for _, r := range s.R {
			...
			if r.Sym != nil && ctxt.SymName(r.Sym) == "runtime.deferreturn" && r.Add == 0 {
		...
		if s.FuncInfo != nil && len(s.FuncInfo.File) > 0 {
			file = ctxt.SymName(s.FuncInfo.File[0])
		}
		...
		if off != end {
			ctxt.Errorf(s, "bad math in functab: funcstart=%d off=%d but end=%d (npcdata=%d nfuncdata=%d ptrsize=%d)", funcstart, off, end, len(pcln.Pcdata), len(pcln.Funcdata), ctxt.Arch.PtrSize)
			errorexit()
		}
	...
	for i := len(ctxt.Filesyms) - 1; i >= 0; i-- {
		s := ctxt.Filesyms[i]
		sn := ctxt.SymName(s)
		ftab.SetUint32(ctxt.Arch, int64(start)+s.Value*4, uint32(ftabaddstring(ctxt, ftab, sn)))
	}
