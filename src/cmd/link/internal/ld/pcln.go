...
func (state *pclnState) genInlTreeSym(fi loader.FuncInfo, arch *sys.Arch) loader.Sym {
	...
	for i := 0; i < int(ninl); i++ {
		...
		nameoff := state.funcNameOffset[call.Func]
...
func (ctxt *Link) pclntab() {
	...
	ctxt.pclnState.filetabOffset = start
	...
	ctxt.pclnState.numberedFiles = nil
	ctxt.pclnState.filepaths = nil
}
...
func (ctxt *Link) findfunctab() {
	ldr := ctxt.loader
	container := ctxt.pclnState.container
