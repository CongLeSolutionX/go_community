...
func walkFilenames(ctxt *Link, container loader.Bitmap, f func(*sym.CompilationUnit, goobj2.CUFileIndex)) {
	ldr := ctxt.loader
...
func (state *pclntab) generateFilenameTabs(ctxt *Link, compUnits []*sym.CompilationUnit, container loader.Bitmap) []uint32 {
	...
	cuEntries := make([]goobj2.CUFileIndex, len(compUnits))
	...
	var fileSize int64
	walkFilenames(ctxt, container, func(cu *sym.CompilationUnit, i goobj2.CUFileIndex) {
	...
	writeCutab := func(ctxt *Link, s loader.Sym) {
		...
		for i, max := range cuEntries {
			...
			cu := compUnits[i]
			for j := goobj2.CUFileIndex(0); j < max; j++ {
...
func (ctxt *Link) pclntab(container loader.Bitmap) *pclntab {
	...
	//      runtime.cutab
	//        for i=0..#CUs
	//          for j=0..#reachableFilenames in CU[i]
