...
func (state *pclntab) generatePctab(ctxt *Link, container loader.Bitmap) {
	...
	// Pclntab offsets of 0 are considered invalid in the runtime. We respect
	...
	// could be moved to a carrier symbol. However, carrier symbols don't work
	// yet with how (as of Aug 2020) pclntab changes its types from SPCLNTAB to
	// SRODATA. Once that is cleaned up in symtab.go, this can be eliminated, and
	// we can just move runtime.pctab to a carrier. (NB, there is one other
	// subtlety as well,
