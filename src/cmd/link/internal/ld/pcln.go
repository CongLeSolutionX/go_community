...
func (state *pclntab) generateFunctab(ctxt *Link, funcs, inlSyms []loader.Sym, cuOffsets []uint32, nameOffsets map[loader.Sym]uint32) {
	...
	writePcln := func(ctxt *Link, s loader.Sym) {
		...
		// Write the data.
		walkPcToFunc(ctxt, sb, funcs, startLocations, setAddr, (*loader.SymbolBuilder).SetUint)
		writeFuncs(ctxt, sb, funcs, inlSyms, startLocations, cuOffsets, nameOffsets)
		state.walkFuncData(ctxt, sb, funcs, inlSyms, startLocations, setAddr, (*loader.SymbolBuilder).SetUint)
	}
...
func funcData(fi loader.FuncInfo, inlSym loader.Sym, fdSyms []loader.Sym, fdOffs []int64) ([]loader.Sym, []int64) {
	...
	if fi.Valid() {
		...
		size := numOffsets
		if fi.NumInlTree() > 0 && size < objabi.FUNCDATA_InlTree+1 {
		...
		if fi.NumInlTree() > 0 {
			if len(fdSyms) < objabi.FUNCDATA_InlTree+1 {
				...
				newFdoffs := make([]int64, objabi.FUNCDATA_InlTree+1)
				copy(newFdsyms, fdSyms)
				copy(newFdoffs, fdOffs)
				fdSyms = newFdsyms
				fdOffs = newFdoffs
			}
