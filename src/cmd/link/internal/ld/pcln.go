	...
	for _, s := range ctxt.Textp {
		...
		if len(pcln.InlTree) > 0 {
			...
			for _, call := range pcln.InlTree {
				...
				treeoff = setuint32(ctxt, inlTreeSym, treeoff, uint32(call.Parent))
				treeoff = setuint32(ctxt, inlTreeSym, treeoff, uint32(call.File.Value))
				treeoff = setuint32(ctxt, inlTreeSym, treeoff, uint32(call.Line))
				treeoff = setuint32(ctxt, inlTreeSym, treeoff, uint32(nameoff))
			}
