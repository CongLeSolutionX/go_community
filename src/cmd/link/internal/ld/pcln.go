...
func (ctxt *Link) pclntab() {
	...
	pclntabNfunc = nfunc
	ftab.Grow(8 + int64(SysArch.PtrSize) + int64(nfunc)*2*int64(SysArch.PtrSize) + int64(SysArch.PtrSize) + 4)
	ftab.SetUint32(ctxt.Arch, 0, 0xfffffffb)
	ftab.SetUint8(ctxt.Arch, 6, uint8(SysArch.MinLC))
	ftab.SetUint8(ctxt.Arch, 7, uint8(SysArch.PtrSize))
	...
	for _, s := range ctxt.Textp {
		...
		ftab.SetAddr(ctxt.Arch, 8+int64(SysArch.PtrSize)+int64(nfunc)*2*int64(SysArch.PtrSize), s)
		ftab.SetUint(ctxt.Arch, 8+int64(SysArch.PtrSize)+int64(nfunc)*2*int64(SysArch.PtrSize)+int64(SysArch.PtrSize), uint64(funcstart))
		...
		if len(pcln.Funcdata) > 0 {
			...
			for i := 0; i < len(pcln.Funcdata); i++ {
				if pcln.Funcdata[i] == nil {
					ftab.SetUint(ctxt.Arch, int64(off)+int64(SysArch.PtrSize)*int64(i), uint64(pcln.Funcdataoff[i]))
				} else {
					// TODO: Dedup.
					funcdataBytes += pcln.Funcdata[i].Size

					ftab.SetAddrPlus(ctxt.Arch, int64(off)+int64(SysArch.PtrSize)*int64(i), pcln.Funcdata[i], pcln.Funcdataoff[i])
				}
			}
	...
	ftab.SetAddrPlus(ctxt.Arch, 8+int64(SysArch.PtrSize)+int64(nfunc)*2*int64(SysArch.PtrSize), last, last.Size)
	...
	ftab.SetUint32(ctxt.Arch, 8+int64(SysArch.PtrSize)+int64(nfunc)*2*int64(SysArch.PtrSize)+int64(SysArch.PtrSize), uint32(start))
