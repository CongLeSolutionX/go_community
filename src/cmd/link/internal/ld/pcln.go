...
// deduplicated pcdata. Returns the map of the offsets for each sym.
func (state *pclntab) generatePctab(ctxt *Link, container loader.Bitmap) map[loader.Sym]uint32 {
	ldr := ctxt.loader

	// Offsets of 0 are considered invalid in the runtime. We respect that by
	// just padding a single byte at the beginning of runtime.pctab, that way
	// no real offsets can be zero.
	size := int64(1)

	// Walk the functions, finding offset to store each pcdata.
	offsets := make(map[loader.Sym]uint32)
	saveOffset := func(pcSym loader.Sym) {
		if _, ok := offsets[pcSym]; !ok {
			datSize := ldr.SymSize(pcSym)
			if datSize != 0 {
				offsets[pcSym] = uint32(size)
			} else {
				// Invalid PC data, record as zero.
				offsets[pcSym] = uint32(0)
			}
			size += datSize
		}
	}
	...
	writePctab := func(ctxt *Link, s loader.Sym) {
		...
		for sym, offset := range offsets {
			sb.CopyData(int64(offset), ldr.Data(sym))
		}
	}
...
func (ctxt *Link) pclntab(container loader.Bitmap) *pclntab {
	...
	pcOffsets := state.generatePctab(ctxt, container)
	...
	for _, s := range ctxt.Textp {
		...
		// pcdata
		if fi.Valid() {
			off = int32(ftab.SetUint32(ctxt.Arch, int64(off), pcOffsets[fi.Pcsp()]))
			off = int32(ftab.SetUint32(ctxt.Arch, int64(off), pcOffsets[fi.Pcfile()]))
			off = int32(ftab.SetUint32(ctxt.Arch, int64(off), pcOffsets[fi.Pcline()]))
		} else {
			off += 12
		}
		...
		if fi.Valid() {
			for i, pcSym := range fi.Pcdata() {
				ftab.SetUint32(ctxt.Arch, int64(off+int32(i*4)), pcOffsets[pcSym])
			}
			if fi.NumInlTree() > 0 {
				ftab.SetUint32(ctxt.Arch, int64(off+8), pcOffsets[fi.Pcinline()])
			}
		}
		off += numPCData * 4
