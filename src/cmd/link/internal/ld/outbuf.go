...
// using it in the thread/goroutine, and closing it in the goroutine.
//
...
//    for i := 0; i < 10; i++ {
//      wg.Add(1)
//      copy, err := out.Copy(start[i], lengthi])
//      if err != nil { /* etc */ }
...
func (out *OutBuf) Copy(start, length uint64) (*OutBuf, error) {
	if out.buf == nil {
		return nil, errors.New("can't parallelize output")
	}
	return &OutBuf{
		arch:   out.arch,
		name:   out.name,
