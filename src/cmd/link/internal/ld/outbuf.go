...
func (out *OutBuf) Close() error {
	...
	if len(out.heap) != 0 {
		if err := out.f.Write(out.heap); err != nil {
			return err
		}
	}
...
// CollapseBuffer should be called before Munmap. It combines the area
// originally mmapped, with any additional heap memory allocated, and puts it
// all into a mmapped area.
func (out *OutBuf) CollapseBuffer() error {
	bufLen := len(out.buf)
