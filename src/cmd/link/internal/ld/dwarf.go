...
func (d *dwctxt) writelines(unit *sym.CompilationUnit, ls loader.Sym, syms []loader.Sym) []loader.Sym {
	...
	for _, s := range unit.Textp {
		...
		if !*FlagNewLT {
			...
			// Output the line table. TODO: Now that we have all the
			// debug information in separate symbols, it would make
			// sense to use a rope, and concatenate them all together
			// rather then the append() below. This would allow us to
			// have the compiler emit the DW_LNE_set_address and a
			// rope data structure to concat them all together in the
			// output.
			if lines != 0 {
				lsu.AddBytes(d.ldr.Data(lines))
			}
		} else {
			// Chain the line symbol untouched onto the list.
			// Note: this does introduce a discrepancy between
			// old and new scheme in that with the old scheme if there
			// was no lines sym, we would still emit the LNE_set_address
			// fragment.
			if lines != 0 {
...
func (d *dwctxt) dwarfGenerateDebugSyms() {
	...
	// Determine number of .debug_line syms needed.
	if *FlagNewLT {
		lineSymbolsNeeded := 1
		for _, u := range d.linkctxt.compUnits {
			lineSymbolsNeeded += 2 + len(u.Textp)
		}
		linesec.syms = make([]loader.Sym, 0, lineSymbolsNeeded)
		linesec.syms = append(linesec.syms, debugLine)
	}
	...
	for _, u := range d.linkctxt.compUnits {
		...
		linesec.syms = d.writelines(u, debugLine, linesec.syms)
