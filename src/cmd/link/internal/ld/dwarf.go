...
func dwarfDefineGlobal(ctxt *Link, s *sym.Symbol, str string, v int64, gotype *sym.Symbol) {
	dv := newdie(ctxt, ctxt.compUnitByPackage[s.Lib].dwinfo, dwarf.DW_ABRV_VARIABLE, str, int(s.Version))
...
func movetomodule(ctxt *Link, parent *dwarf.DWDie) {
	die := ctxt.compUnitByPackage[nil].dwinfo.Child
	if die == nil {
		ctxt.compUnitByPackage[nil].dwinfo.Child = parent.Child
		return
	}
	for die.Link != nil {
		die = die.Link
...
func dwarfGenerateDebugInfo(ctxt *Link) {
	...
	for _, lib := range ctxt.Library {
		...
		if lib.Pkg == "runtime" {
			// add the runtime package as the compilation unit for 'nil' so that
			// global variables that don't belong to any package have a compilation
			// unit to go to.
			ctxt.compUnitByPackage[nil] = unit
		}
