...
type dwctxt struct {
	...
	keepsyms map[loader.Sym]struct{}
...
var typesyms []sym.LoaderSym

func (d *dwctxt) lookupTypeDie(name string, version int) loader.Sym {

	ds := d.ldr.Lookup(dwarf.InfoPrefix+name, version)
	if ds == 0 {
		//fmt.Println("d.ldr.Lookup ", dwarf.InfoPrefix+name, " got 0")
		return 0
	}
	d.ldr.SetAttrNotInSymbolTable(ds, true)
	d.ldr.SetAttrReachable(ds, true)

	if _, ok := d.tmap[name]; !ok {
		d.tmap[name] = ds
	}
	return ds
}
...
func (d *dwctxt) keep(dwarfsym loader.Sym) {
	d.ldr.SetAttrNotInSymbolTable(dwarfsym, true)
	d.ldr.SetAttrReachable(dwarfsym, true)
	if _, ok := d.keepsyms[dwarfsym]; !ok {
		d.keepsyms[dwarfsym] = struct{}{}
		typesyms = append(typesyms, sym.LoaderSym(dwarfsym))
	} else {
		return
	}
	relocs := d.ldr.Relocs(dwarfsym)
	for i := 0; i < relocs.Count(); i++ {
		r := relocs.At(i)
		if r.Type() == objabi.R_DWARFSECREF && r.Sym() != dwarfsym {
			d.keep(r.Sym())
		}
	}
...
func (d *dwctxt) newtype(gotype loader.Sym) loader.Sym {
	sn := d.ldr.SymName(gotype)
	name := sn[5:] // could also decode from Type.string
	var die, typedefdie loader.Sym
	die = d.lookupTypeDie(name, 0)
	typedefdie = d.lookupTypeDie("typedef."+name, 0)
	// Sanity check.
...
func dwarfGenerateDebugInfo(ctxt *Link) {
	...
	d := &dwctxt{
		...
		keepsyms: make(map[loader.Sym]struct{}),
	}
...
func (d *dwctxt) dwarfGenerateDebugSyms() {
	...
	d.linkctxt.runtimeCU.TypeDIEs = typesyms
