...
func dwarfDefineGlobal(ctxt *Link, s *sym.Symbol, str string, v int64, gotype *sym.Symbol) {
	// Find a suitable DIE to include the global.
	// One would think it would be as simple as just looking at the
	// CompilationUnit of the symbol; but, it's not that simple. It's possible
	// to be a symbol without a Textp, and therefore, not actually be in the
	// set of active CUs. As such, we hang the global off a CU we know will be
	// in the output set.
	var lib *sym.Library
	if s.Unit != nil {
		lib = s.Unit.Lib
	}
	if lib == nil || len(ctxt.compUnitsByPackage[lib]) == 0 { // If we're not used, we just stuff this in a used runtime unit.
		lib = ctxt.LibraryByPkg["runtime"]
	}
	dv := newdie(ctxt, ctxt.compUnitsByPackage[lib][0].DWInfo, dwarf.DW_ABRV_VARIABLE, str, int(s.Version))
...
func movetomodule(ctxt *Link, parent *dwarf.DWDie) {
	...
	unit := ctxt.compUnitsByPackage[runtimelib][0]
...
func writelines(ctxt *Link, unit *sym.CompilationUnit, ls *sym.Symbol) {
	...
	for i, name := range unit.DWARFFileTable {
		if len(name) != 0 {
			...
			name = fmt.Sprintf("DEAD_%d", i) // Can't have empty filenames
		}
	...
	for _, s := range unit.Textp { // textp has been dead-code-eliminated already.
		// While we're looping over the text symbols here, find the path for the gdb scripts.
...
func dwarfGenerateDebugInfo(ctxt *Link) {
	...
	ctxt.compUnitsByPackage = make(map[*sym.Library][]*sym.CompilationUnit)
	...
	for _, lib := range ctxt.Library {
		includedConsts := false
		for i, unit := range lib.Units {
			if len(unit.Textp) == 0 {
				continue
			}

			if s := ctxt.Syms.ROLookup(dwarf.ConstInfoPrefix+lib.Pkg, 0); !includedConsts && s != nil {
				includedConsts = true
			...
			unit.DWInfo = newdie(ctxt, &dwroot, dwarf.DW_ABRV_COMPUNIT, fmt.Sprintf("%v.compunit.%d", unit.Lib.Pkg, i), 0)
