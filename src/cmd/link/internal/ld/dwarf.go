		...
		for ri := int32(0); ri < int32(len(f.R)); ri++ {
			...
			if ok {
				if int(int32(idx)) != idx {
					Errorf(s, "bad R_DWARFFILEREF relocation: file index overflow")
				}
				if r.Siz != 4 {
					Errorf(s, "bad R_DWARFFILEREF relocation: has size %d, expected 4", r.Siz)
				}
				if r.Off < 0 || r.Off+4 > int32(len(f.P)) {
					Errorf(s, "bad R_DWARFFILEREF relocation offset %d + 4 would write past length %d", s, r.Off, len(s.P))
					continue
				}
