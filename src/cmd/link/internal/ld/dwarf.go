...
type dwctxt struct {
	...
	// Do not add a type die to workqueue more than once.
	// So use a map to record them.
	inqueue   map[loader.Sym]struct{}
	workqueue heap
}
...
func (d *dwctxt) newdie(parent *dwarf.DWDie, abbrev int, name string) *dwarf.DWDie {
	...
	dwarf.NewAttr(die, dwarf.DW_AT_name, dwarf.DW_CLS_STRING, int64(len(name)), name)
...
func (d *dwctxt) importInfoSymbol(dsym loader.Sym) {
	...
	for i := 0; i < relocs.Count(); i++ {
		...
		rsym := r.Sym()
		// record the relocation symbol directly, do not look up the runtime type,
		// it may not exist.
		d.addQueue(rsym)
	}
}
...
// construct the name of type die by the name of gotype. Then add the type die sym
// into the workqueue.
func (d *dwctxt) markTypeDieFromGoType(gotype loader.Sym) loader.Sym {
	sn := d.ldr.SymName(gotype)
	name := dwarf.InfoPrefix + sn[5:] // could also decode from Type.string
	dwinfo := d.ldr.Lookup(name, 0)
	if dwinfo == 0 {
		d.ldr.Errorf(gotype, "can't find dwarf type info: %s", name)
	}
	d.addQueue(dwinfo)
	return dwinfo
}

// check if a type die is already in queue, otherwise, add it to the queue.
func (d *dwctxt) addQueue(typedie loader.Sym) {
	if _, ok := d.inqueue[typedie]; !ok {
		d.inqueue[typedie] = struct{}{}
		d.workqueue.push(typedie)
	}
}
...
func dwarfGenerateDebugInfo(ctxt *Link) {
	...
	d := &dwctxt{
		...
		ldr:      ctxt.loader,
		arch:     ctxt.Arch,
		inqueue:  make(map[loader.Sym]struct{}),
	}
				...
				if d.ldr.IsDict(idx) {
					...
					for i := 0; i < relocs.Count(); i++ {
						...
						if reloc.Type() == objabi.R_USEIFACE {
							d.markTypeDieFromGoType(reloc.Sym())
						}
					}
				}
			}
