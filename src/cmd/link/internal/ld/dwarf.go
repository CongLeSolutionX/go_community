		...
		for _, fn := range u.funcDIEs {
			for _, reloc := range fn.R {
				if reloc.Type == objabi.R_DWARFSECREF && strings.HasPrefix(reloc.Sym.Name, dwarf.LocPrefix) {
					...
					empty = false
					// OSX, especially dsymutil, doesn't support base address entries. Strip them out.
...
func removeLocationListBaseAddress(ctxt *Link, info, list *sym.Symbol) {
	listOffsets := make(map[int]int, len(list.R)/2)
	...
	// Fix up references from the DIE to the location lists.
	for i := range info.R {
		r := &fn.R[i]
		if r.Sym != list {
			continue
		}
		r.Add = int64(listOffsets[int(r.Add)])
	}
}
