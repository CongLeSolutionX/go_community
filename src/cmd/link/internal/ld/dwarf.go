...
func (d *dwctxt) dwarfGenerateDebugSyms() {
	...
	// A note on -mindwarf:  at the moment .debug_frame generation
	// is alway enabled, even when -mindwarf is in effect. In theory
	// we could change things so that .debug_frame entries are only
	// emitted for functions compiled with full DWARF, but this is not
