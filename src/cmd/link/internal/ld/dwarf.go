...
func (d *dwctxt2) dwarfFuncSym(ctxt *Link, fs loader.Sym, meta string, create bool) loader.Sym {
	panic("not yet implemented")
	return 0
}
...
func (d *dwctxt2) importInfoSymbol(ctxt *Link, dsym loader.Sym) {
	...
	// dsym.Attr |= sym.AttrNotInSymbolTable
	dsu, _ := ctxt.loader.MakeSymbolUpdater(dsym)
	dsu.SetType(sym.SDWARFINFO)
	rslice := dsu.Relocs()
	for i := 0; i < len(rslice); i++ {
		...
		// FIXME: do we need the zero size check?
		tsu, _ := ctxt.loader.MakeSymbolUpdater(r.Sym)
		if tsu.Size() == 0 {
			n := d.nameFromDIESym(ctxt, r.Sym)
			ts := ctxt.loader.Lookup("type."+n, 0)
			d.defgotype(ctxt, ts)
		}
	}
}
