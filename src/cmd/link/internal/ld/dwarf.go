...
func writeframes(ctxt *Link, syms []*Symbol) []*Symbol {
	...
	for _, s := range ctxt.Textp {
		...
		fs.AddUint32(ctxt.Arch, uint32(4+2*SysArch.PtrSize+len(deltaBuf))) // length (excludes itself)
		...
		fs.AddUintXX(ctxt.Arch, uint64(s.Size), SysArch.PtrSize) // address range
...
func writeinfo(ctxt *Link, syms []*Symbol, funcs, consts []*Symbol, abbrevsym *Symbol) []*Symbol {
	...
	for compunit := dwroot.Child; compunit != nil; compunit = compunit.Link {
		...
		s.AddUint8(uint8(SysArch.PtrSize)) // address_size
...
func writearanges(ctxt *Link, syms []*Symbol) []*Symbol {
	...
	for compunit := dwroot.Child; compunit != nil; compunit = compunit.Link {
		...
		s.AddUintXX(ctxt.Arch, uint64(e.Value-b.Value), SysArch.PtrSize)
		s.AddUintXX(ctxt.Arch, 0, SysArch.PtrSize)
		s.AddUintXX(ctxt.Arch, 0, SysArch.PtrSize)
	}
	if s.Size > 0 {
		syms = append(syms, s)
