...
func (d *dwctxt) writepcranges(unit *sym.CompilationUnit, base sym.ID, pcs []dwarf.Range, rangeProlog sym.ID) []sym.ID {
	...
	dwarf.PutBasedRanges(d, rangeProlog, pcs)
...
func (d *dwctxt) writeframes(fs sym.ID) dwarfSecInfo {
	...
	dwarf.Uleb128put(d, fs, 1)                         // code_alignment_factor
	dwarf.Sleb128put(d, fs, dataAlignmentFactor)       // all CFI offset calculations include multiplication with this factor
	dwarf.Uleb128put(d, fs, int64(thearch.Dwarfreglr)) // return_address_register

	fsu.AddUint8(dwarf.DW_CFA_def_cfa)                 // Set the current frame address..
	dwarf.Uleb128put(d, fs, int64(thearch.Dwarfregsp)) // ...to use the value in the platform's SP register (defined in l.go)...
	if haslr {
		dwarf.Uleb128put(d, fs, int64(0)) // ...plus a 0 offset.

		fsu.AddUint8(dwarf.DW_CFA_same_value) // The platform's link register is unchanged during the prologue.
		dwarf.Uleb128put(d, fs, int64(thearch.Dwarfreglr))

		fsu.AddUint8(dwarf.DW_CFA_val_offset)              // The previous value...
		dwarf.Uleb128put(d, fs, int64(thearch.Dwarfregsp)) // ...of the platform's SP register...
		dwarf.Uleb128put(d, fs, int64(0))                  // ...is CFA+0.
	} else {
		dwarf.Uleb128put(d, fs, int64(d.arch.PtrSize)) // ...plus the word size (because the call instruction implicitly adds one word to the frame).

		fsu.AddUint8(dwarf.DW_CFA_offset_extended)                          // The previous value...
		dwarf.Uleb128put(d, fs, int64(thearch.Dwarfreglr))                  // ...of the return address...
		dwarf.Uleb128put(d, fs, int64(-d.arch.PtrSize)/dataAlignmentFactor) // ...is saved at [CFA - (PtrSize/4)].
	}
...
func (d *dwctxt) writeUnitInfo(u *sym.CompilationUnit, abbrevsym sym.ID, infoEpilog sym.ID) []sym.ID {
	...
	dwarf.Uleb128put(d, s, int64(compunit.Abbrev))
	dwarf.PutAttrs(d, s, compunit.Abbrev, compunit.Attr)
