...
func (d *dwctxt2) newdie(parent *dwarf.DWDie, abbrev int, name string, version int) *dwarf.DWDie {
	...
	if name != "" && (abbrev <= dwarf.DW_ABRV_VARIABLE || abbrev >= dwarf.DW_ABRV_NULLTYPE) {
		...
		if abbrev != dwarf.DW_ABRV_VARIABLE || version == 0 {
			...
			dsu, ds = d.ldr.MakeSymbolUpdater(ds)
...
func (d *dwctxt2) walksymtypedef(symIdx loader.Sym) loader.Sym {
	...
	sn := d.ldr.SymName(symIdx)
	if !strings.HasPrefix(sn, "go.info.") {
		d.linkctxt.Errorf(symIdx, "internal error: walksymtypedef passed incorrectly flavored sym\n")
		return 0
	}
...
func (d *dwctxt2) adddwarfref(s loader.Sym, t loader.Sym, size int) int64 {
	var result int64
	su, _ := d.ldr.MakeSymbolUpdater(s)
	switch size {
	default:
		d.linkctxt.Errorf(s, "invalid size %d in adddwarfref\n", size)
		fallthrough
	case d.arch.PtrSize:
		result = su.AddAddrPlus(d.arch, t, 0)
	case 4:
		result = su.AddAddrPlus4(d.arch, t, 0)
	}
	rsl := su.Relocs()
	r := &rsl[len(rsl)-1]
	r.Type = objabi.R_DWARFSECREF
...
func (d *dwctxt2) dotypedef(parent *dwarf.DWDie, gotype loader.Sym, name string, def *dwarf.DWDie) *dwarf.DWDie {
	...
	tdsu, _ := d.ldr.MakeSymbolUpdater(tds)
