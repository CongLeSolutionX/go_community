...
func dwarfGenerateDebugInfo2(ctxt *Link) {
	...
	if 1 == 1 {
		// At this stage we have all the DIE objects constructed, but
		// they have loader.Sym attributes and not sym.Symbol attributes.
		// At the point when loadlibfull runs we would need to visit
		// every DIE constructed and convert the symbols.
	}
}

// dwarfConvertSymbols is invoked around the time that loader.LoadFull
// runs (converting all loader.Sym's into sym.Symbols); it walks through
// dwarf DIE objects and rewrites loader to sym as well. This is
// obviously a temporary function.
func dwarfConvertSymbols(ctxt *Link) {
	...
	convertSymbolsInDIE(ctxt, &dwtypes)
	conv
}
