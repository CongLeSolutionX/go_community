...
// DIEs that use sym.ID as opposed to *sym.Symbol. It is also
...
// ctx returns c as a dwarf.Context[sym.ID].
//
...
func (c *dwctxt) ctx() dwarf.Context[sym.ID] { return c }
...
func (c *dwctxt) AddAddress(s, data sym.ID, value int64) {
	su := c.ldr.MakeSymbolUpdater(s)
	if value != 0 {
		value -= su.Value()
	}
...
func (c *dwctxt) AddCURelativeAddress(s, data sym.ID, value int64) {
	su := c.ldr.MakeSymbolUpdater(s)
	if value != 0 {
		value -= su.Value()
	}
...
func (c *dwctxt) AddSectionOffset(s sym.ID, size int, t sym.ID, ofs int64) {
	su := c.ldr.MakeSymbolUpdater(s)
	switch size {
	default:
...
func (c *dwctxt) AddDWARFAddrSectionOffset(s, t sym.ID, ofs int64) {
	size := 4
...
func (c *dwctxt) RecordDclReference(s, t sym.ID, dclIdx int, inlIndex int) {
	panic("should be used only in the compiler")
}
...
func (dsi *dwarfSecInfo) secSym() sym.ID {
	if len(dsi.syms) == 0 {
		return 0
	}
	return dsi.syms[0]
...
func (dsi *dwarfSecInfo) subSyms() []sym.ID {
	if len(dsi.syms) == 0 {
		return []sym.ID{}
	}
	return dsi.syms[1:]
...
func (d *dwctxt) writeabbrev() dwarfSecInfo {
	...
	return dwarfSecInfo{syms: []sym.ID{abrvs.Sym()}}
}

var dwtypes dwarf.DWDie[sym.ID]
...
func newattr(die *dwarf.DWDie[sym.ID], attr uint16, cls int, value int64, data interface{}) {
	a := new(dwarf.DWAttr)
	a.Link = die.Attr
	die.Attr = a
	a.Atr = attr
...
func getattr(die *dwarf.DWDie[sym.ID], attr uint16) *dwarf.DWAttr {
...
func (d *dwctxt) newdie(parent *dwarf.DWDie[sym.ID], abbrev int, name string) *dwarf.DWDie[sym.ID] {
	die := new(dwarf.DWDie[sym.ID])
	die.Abbrev = abbrev
...
func walktypedef(die *dwarf.DWDie[sym.ID]) *dwarf.DWDie[sym.ID] {
	if die == nil {
		return nil
	}
	...
	if die.Abbrev == dwarf.DW_ABRV_TYPEDECL {
		for attr := die.Attr; attr != nil; attr = attr.Link {
			if attr.Atr == dwarf.DW_AT_type && attr.Cls == dwarf.DW_CLS_REFERENCE && attr.Data != nil {
				return attr.Data.(*dwarf.DWDie[sym.ID])
			}
		}
	}
...
func (d *dwctxt) walksymtypedef(symIdx sym.ID) sym.ID {
...
// if not.
func findchild(die *dwarf.DWDie[sym.ID], name string) *dwarf.DWDie[sym.ID] {
	var prev *dwarf.DWDie[sym.ID]
...
func (d *dwctxt) find(name string) sym.ID {
	return d.tmap[name]
}

func (d *dwctxt) mustFind(name string) sym.ID {
	r := d.find(name)
	if r == 0 {
...
func (d *dwctxt) adddwarfref(sb *loader.SymbolBuilder, t sym.ID, size int) {
	switch size {
	default:
...
func (d *dwctxt) newrefattr(die *dwarf.DWDie[sym.ID], attr uint16, ref sym.ID) {
	if ref == 0 {
		return
	}
...
func (d *dwctxt) putdie(syms []sym.ID, die *dwarf.DWDie[sym.ID]) []sym.ID {
	s := die.Sym
	if s == 0 {
		s = syms[len(syms)-1]
	} else {
	...
	sDwsym := s
	dwarf.Uleb128put(d.ctx(), sDwsym, int64(die.Abbrev))
	dwarf.PutAttrs(d.ctx(), sDwsym, die.Abbrev, die.Attr)
...
func reverselist(list **dwarf.DWDie[sym.ID]) {
	curr := *list
	var prev *dwarf.DWDie[sym.ID]
	for curr != nil {
		next := curr.Link
...
func reversetree(list **dwarf.DWDie[sym.ID]) {
	reverselist(list)
...
func newmemberoffsetattr(die *dwarf.DWDie[sym.ID], offs int32) {
	newattr(die, dwarf.DW_AT_data_member_location, dwarf.DW_CLS_CONSTANT, int64(offs), nil)
}

func (d *dwctxt) lookupOrDiag(n string) sym.ID {
...
func (d *dwctxt) dotypedef(parent *dwarf.DWDie[sym.ID], name string, def *dwarf.DWDie[sym.ID]) *dwarf.DWDie[sym.ID] {
...
func (d *dwctxt) defgotype(gotype sym.ID) sym.ID {
	if gotype == 0 {
	...
	d.tdmap[gotype] = gtdwSym.Sym
	return gtdwSym.Sym
}

func (d *dwctxt) newtype(gotype sym.ID) *dwarf.DWDie[sym.ID] {
	sn := d.ldr.SymName(gotype)
	name := sn[5:] // could also decode from Type.string
	...
	var die, typedefdie *dwarf.DWDie[sym.ID]
	switch kind {
	case abi.Bool:
...
func (d *dwctxt) nameFromDIESym(dwtypeDIESym sym.ID) string {
	sn := d.ldr.SymName(dwtypeDIESym)
	return sn[len(dwarf.InfoPrefix):]
}

func (d *dwctxt) defptrto(dwtype sym.ID) sym.ID {
...
func (d *dwctxt) copychildrenexcept(ctxt *Link, dst *dwarf.DWDie[sym.ID], src *dwarf.DWDie[sym.ID], except *dwarf.DWDie[sym.ID]) {
...
func (d *dwctxt) copychildren(ctxt *Link, dst *dwarf.DWDie[sym.ID], src *dwarf.DWDie[sym.ID]) {
	d.copychildrenexcept(ctxt, dst, src, nil)
}

// Search children (assumed to have TAG_member) for the one named
// field and set its AT_type to dwtype
func (d *dwctxt) substitutetype(structdie *dwarf.DWDie[sym.ID], field string, dwtype sym.ID) {
...
func (d *dwctxt) findprotodie(ctxt *Link, name string) *dwarf.DWDie[sym.ID] {
...
func (d *dwctxt) synthesizestringtypes(ctxt *Link, die *dwarf.DWDie[sym.ID]) {
...
func (d *dwctxt) synthesizeslicetypes(ctxt *Link, die *dwarf.DWDie[sym.ID]) {
	...
	for ; die != nil; die = die.Link {
		...
		d.copychildren(ctxt, die, prototype)
		elem := getattr(die, dwarf.DW_AT_go_elem).Data.(sym.ID)
...
func (d *dwctxt) mkinternaltype(ctxt *Link, abbrev int, typename, keyname, valname string, f func(*dwarf.DWDie[sym.ID])) sym.ID {
...
func (d *dwctxt) synthesizemaptypes(ctxt *Link, die *dwarf.DWDie[sym.ID]) {
	...
	for ; die != nil; die = die.Link {
		...
		gotype := getattr(die, dwarf.DW_AT_type).Data.(sym.ID)
		...
		dwhks := d.mkinternaltype(ctxt, dwarf.DW_ABRV_ARRAYTYPE, "[]key", keyname, "", func(dwhk *dwarf.DWDie[sym.ID]) {
		...
		dwhvs := d.mkinternaltype(ctxt, dwarf.DW_ABRV_ARRAYTYPE, "[]val", valname, "", func(dwhv *dwarf.DWDie[sym.ID]) {
		...
		dwhbs := d.mkinternaltype(ctxt, dwarf.DW_ABRV_STRUCTTYPE, "bucket", keyname, valname, func(dwhb *dwarf.DWDie[sym.ID]) {
			...
			d.newrefattr(fld, dwarf.DW_AT_type, d.defptrto(dwhb.Sym))
		...
		// Construct hash<K,V>
		dwhs := d.mkinternaltype(ctxt, dwarf.DW_ABRV_STRUCTTYPE, "hash", keyname, valname, func(dwh *dwarf.DWDie[sym.ID]) {
...
func (d *dwctxt) synthesizechantypes(ctxt *Link, die *dwarf.DWDie[sym.ID]) {
	...
	for ; die != nil; die = die.Link {
		...
		elemgotype := getattr(die, dwarf.DW_AT_type).Data.(sym.ID)
		tname := d.ldr.SymName(elemgotype)
		elemname := tname[5:]
		elemtype := d.walksymtypedef(d.defgotype(d.lookupOrDiag(tname)))

		// sudog<T>
		dwss := d.mkinternaltype(ctxt, dwarf.DW_ABRV_STRUCTTYPE, "sudog", elemname, "", func(dws *dwarf.DWDie[sym.ID]) {
		...
		// waitq<T>
		dwws := d.mkinternaltype(ctxt, dwarf.DW_ABRV_STRUCTTYPE, "waitq", elemname, "", func(dww *dwarf.DWDie[sym.ID]) {
		...
		// hchan<T>
		dwhs := d.mkinternaltype(ctxt, dwarf.DW_ABRV_STRUCTTYPE, "hchan", elemname, "", func(dwh *dwarf.DWDie[sym.ID]) {
...
func (d *dwctxt) addDwarfAddrRef(sb *loader.SymbolBuilder, t sym.ID) {
	if isDwarf64(d.linkctxt) {
...
func (d *dwctxt) calcCompUnitRanges() {
	...
	for _, s := range d.linkctxt.Textp {
		...
		// different units.
		sval := d.ldr.SymValue(s)
		u0val := d.ldr.SymValue(unit.Textp[0])
		if prevUnit != unit {
			unit.PCs = append(unit.PCs, dwarf.Range{Start: sval - u0val})
			prevUnit = unit
		}
		unit.PCs[len(unit.PCs)-1].End = sval - u0val + int64(len(d.ldr.Data(s)))
	}
}

func movetomodule(ctxt *Link, parent *dwarf.DWDie[sym.ID]) {
...
func (d *dwctxt) importInfoSymbol(dsym sym.ID) {
...
func (d *dwctxt) writeDirFileTables(unit *sym.CompilationUnit, lsu *loader.SymbolBuilder) {
	...
	for k := 0; k < len(files); k++ {
		...
		dwarf.Uleb128put(d.ctx(), lsDwsym, int64(files[k].dir))
...
func (d *dwctxt) writelines(unit *sym.CompilationUnit, lineProlog sym.ID) []sym.ID {
	...
	syms := make([]sym.ID, 0, len(unit.Textp)+2)
	syms = append(syms, lineProlog)
	lsu := d.ldr.MakeSymbolUpdater(lineProlog)
	newattr(unit.DWInfo, dwarf.DW_AT_stmt_list, dwarf.DW_CLS_PTR, 0, lineProlog)
...
func (d *dwctxt) writepcranges(unit *sym.CompilationUnit, base sym.ID, pcs []dwarf.Range, rangeProlog sym.ID) []sym.ID {
	...
	newattr(unit.DWInfo, dwarf.DW_AT_ranges, dwarf.DW_CLS_PTR, rsu.Size(), rangeProlog)
	newattr(unit.DWInfo, dwarf.DW_AT_low_pc, dwarf.DW_CLS_ADDRESS, 0, base)
	dwarf.PutBasedRanges(d.ctx(), rangeProlog, pcs)
...
func (d *dwctxt) writeframes(fs sym.ID) dwarfSecInfo {
	...
	dwarf.Uleb128put(d.ctx(), fs, 1)                         // code_alignment_factor
	dwarf.Sleb128put(d.ctx(), fs, dataAlignmentFactor)       // all CFI offset calculations include multiplication with this factor
	dwarf.Uleb128put(d.ctx(), fs, int64(thearch.Dwarfreglr)) // return_address_register

	fsu.AddUint8(dwarf.DW_CFA_def_cfa)                       // Set the current frame address..
	dwarf.Uleb128put(d.ctx(), fs, int64(thearch.Dwarfregsp)) // ...to use the value in the platform's SP register (defined in l.go)...
	if haslr {
		dwarf.Uleb128put(d.ctx(), fs, int64(0)) // ...plus a 0 offset.

		fsu.AddUint8(dwarf.DW_CFA_same_value) // The platform's link register is unchanged during the prologue.
		dwarf.Uleb128put(d.ctx(), fs, int64(thearch.Dwarfreglr))

		fsu.AddUint8(dwarf.DW_CFA_val_offset)                    // The previous value...
		dwarf.Uleb128put(d.ctx(), fs, int64(thearch.Dwarfregsp)) // ...of the platform's SP register...
		dwarf.Uleb128put(d.ctx(), fs, int64(0))                  // ...is CFA+0.
	} else {
		dwarf.Uleb128put(d.ctx(), fs, int64(d.arch.PtrSize)) // ...plus the word size (because the call instruction implicitly adds one word to the frame).

		fsu.AddUint8(dwarf.DW_CFA_offset_extended)                                // The previous value...
		dwarf.Uleb128put(d.ctx(), fs, int64(thearch.Dwarfreglr))                  // ...of the return address...
		dwarf.Uleb128put(d.ctx(), fs, int64(-d.arch.PtrSize)/dataAlignmentFactor) // ...is saved at [CFA - (PtrSize/4)].
	}
	...
	return dwarfSecInfo{syms: []sym.ID{fs}}
}
...
func (d *dwctxt) writeUnitInfo(u *sym.CompilationUnit, abbrevsym sym.ID, infoEpilog sym.ID) []sym.ID {
	...
	dwarf.Uleb128put(d.ctx(), s, int64(compunit.Abbrev))
	dwarf.PutAttrs(d.ctx(), s, compunit.Abbrev, compunit.Attr)
...
func (d *dwctxt) mkBuiltinType(ctxt *Link, abrv int, tname string) *dwarf.DWDie[sym.ID] {
	// create type DIE
...
func (d *dwctxt) dwarfVisitFunction(fnSym sym.ID, unit *sym.CompilationUnit) {
...
func dwarfGenerateDebugInfo(ctxt *Link) {
	...
	d := &dwctxt{
		...
		ldr:      ctxt.loader,
		arch:     ctxt.Arch,
		tmap:     make(map[string]sym.ID),
		tdmap:    make(map[sym.ID]sym.ID),
		rtmap:    make(map[sym.ID]sym.ID),
	}
	...
	newattr(die, dwarf.DW_AT_go_kind, dwarf.DW_CLS_CONSTANT, int64(abi.Uintptr), 0)
	newattr(die, dwarf.DW_AT_go_runtime_type, dwarf.DW_CLS_ADDRESS, 0, d.lookupOrDiag("type:uintptr"))

	d.uintptrInfoSym = d.mustFind("uintptr")

	// Prototypes needed for type synthesis.
	prototypedies = map[string]*dwarf.DWDie[sym.ID]{
	...
	// type they reference.
	for idx := sym.ID(1); idx < sym.ID(d.ldr.NDef()); idx++ {
...
func (d *dwctxt) dwarfGenerateDebugSyms() {
	...
	mkSecSym := func(name string) sym.ID {
		s := d.ldr.CreateSymForUpdate(name, 0)
		s.SetType(sym.SDWARFSECT)
		s.SetReachable(true)
		return s.Sym()
	}
	mkAnonSym := func(kind sym.SymKind) sym.ID {
	...
	lineSec := dwarfSecInfo{syms: []sym.ID{lineSym}}
	locSec := dwarfSecInfo{syms: []sym.ID{locSym}}
	rangesSec := dwarfSecInfo{syms: []sym.ID{rangesSym}}
	frameSec := dwarfSecInfo{syms: []sym.ID{frameSym}}
	infoSec := dwarfSecInfo{syms: []sym.ID{infoSym}}
	...
	markReachable := func(syms []sym.ID) []sym.ID {
		for _, s := range syms {
...
func dwarfcompress(ctxt *Link) {
	...
	for i := range dwarfp {
		go func(resIndex int, syms []sym.ID) {
	...
	for _, z := range res {
		...
		if z.compressed == nil {
			...
			ds := dwarfSecInfo{syms: []sym.ID{newSym.Sym()}}
			newDwarfp = append(newDwarfp, ds)
...
// the package name.
func (d *dwctxt) getPkgFromCUSym(s sym.ID) string {
