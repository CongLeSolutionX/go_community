...
func defdwsymb(ctxt *Link, s *sym.Symbol, str string, t SymbolType, v int64, gotype *sym.Symbol) {
	...
	case DataSym, BSSSym:
		...
		case sym.SRODATA:
			switch {
			case strings.HasSuffix(str, ".args_stackmap"):
				// ok
			case strings.HasPrefix(str, "go.itab."):
				// ok
			default:
				if gotype != nil {
...
type compilationUnit struct {
	...
	fileTable []*sym.Symbol // list of files used to create this compilation unit, including any file containing inlined functions
		...
		for _, s := range lib.Textp { // textp has been dead-code-eliminated already.
			...
			for ri := 0; ri < len(dsym.R); ri++ {
				...
				case objabi.R_DWARFFILEREF:
					...
					if _, ok := seenFile[r.Sym]; !ok {
						seenFile[r.Sym] = struct{}{}
						unit.fileTable = append(unit.fileTable, r.Sym)
					}
				}
			}

			// Import any new symbols that have turned up.
			for _, absdsym := range unit.absFnDIEs {
...
func dwarfGenerateDebugSyms(ctxt *Link) {
	...
	// These symbols are created in ctxt.symtab and dwarfGenerateDebugInfo can
	// not generate DIEs for them, therefore we export them here.
	for _, symname := range []string{
		"go.func.*",
		"runtime.bss",
		"runtime.data",
		"runtime.ebss",
		...
		"runtime.typelink",
		"runtime.types",
		"runtime/internal/sys.DefaultGoroot.str",
		"type.*",
	} {
		s := ctxt.Syms.ROLookup(symname, 0)
		...
		if diesym := ctxt.Syms.ROLookup(dwarf.InfoPrefix+s.Name, int(s.Version)); diesym != nil {
			continue
		}
		dwarfDefineGlobal(ctxt, s, s.Name, Symaddr(s), s.Gotype)
	}
