...
func createUnitLength(arch *sys.Arch, isd64 bool, s *sym.Symbol, v uint64) {
	...
	addDwarfAddrField(arch, isd64, s, v)
}

// addDwarfAddrField adds a DWARF field in DWARF 64bits or 32bits.
func addDwarfAddrField(arch *sys.Arch, isd64 bool, s *sym.Symbol, v uint64) {
	if isd64 {
		s.AddUint(arch, v)
	} else {
...
func addDwarfAddrRef(ctxt *Link, isd64 bool, s *sym.Symbol, t *sym.Symbol) {
	if isd64 {
...
func writelines(ctxt *Link, unit *compilationUnit, ls *sym.Symbol) {
	...
	addDwarfAddrField(ctxt.Arch, dwarfctxt.IsDwarf64(), ls, 0) // header_length (*), filled in at end
	headerstart = ls.Size
	...
	if ctxt.HeadType == objabi.Haix {
		dwsectCUSize[".debug_line."+unit.lib.String()] = uint64(ls.Size - unitLengthOffset)
	}
...
func writeframes(ctxt *Link, syms []*sym.Symbol) []*sym.Symbol {
	...
	addDwarfAddrField(ctxt.Arch, dwarfctxt.IsDwarf64(), fs, 0xffffffff)        // cid.
	...
	for _, s := range ctxt.Textp {
		...
		if ctxt.LinkMode == LinkExternal {
			addDwarfAddrRef(ctxt, dwarfctxt.IsDwarf64(), fs, fs)
		} else {
			addDwarfAddrField(ctxt.Arch, dwarfctxt.IsDwarf64(), fs, 0) // CIE offset
		}
...
func writeinfo(ctxt *Link, syms []*sym.Symbol, units []*compilationUnit, abbrevsym *sym.Symbol) []*sym.Symbol {
	...
	for _, u := range units {
		...
		// debug_abbrev_offset (*)
		addDwarfAddrRef(ctxt, dwarfctxt.IsDwarf64(), s, abbrevsym)
...
func writepub(ctxt *Link, sname string, ispub func(*dwarf.DWDie) bool, syms []*sym.Symbol) []*sym.Symbol {
	...
	for _, u := range ctxt.compUnits {
		...
		addDwarfAddrRef(ctxt, dwarfctxt.IsDwarf64(), s, dtolsym(compunit.Sym))   // debug_info_offset (of the Comp unit Header)
		addDwarfAddrField(ctxt.Arch, dwarfctxt.IsDwarf64(), s, uint64(culength)) // debug_info_length
		...
		for die := compunit.Child; die != nil; die = die.Link {
			...
			addDwarfAddrRef(ctxt, dwarfctxt.IsDwarf64(), s, dtolsym(die.Sym))
			Addstring(s, name)
		}

		addDwarfAddrField(ctxt.Arch, dwarfctxt.IsDwarf64(), s, 0) // Null offset
