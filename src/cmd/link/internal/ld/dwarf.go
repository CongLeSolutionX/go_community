...
func newdwctxt2(linkctxt *Link, forTypeGen bool) dwctxt2 {
	...
	d.typeRuntimeEface = d.lookupOrDiag(linkctxt, "type.runtime.eface")
	d.typeRuntimeIface = d.lookupOrDiag(linkctxt, "type.runtime.iface")
	return d
}
...
func (d *dwctxt2) walksymtypedef(ctxt *Link, symIdx loader.Sym) loader.Sym {
	...
	// Sanity check. To be removed at some point.
	sn := ctxt.loader.SymName(symIdx)
	if !strings.HasPrefix(sn, "go.info.") {
		ctxt.Errorf(symIdx, "internal error: walksymtypedef passed incorrectly flavored sym\n")
		return 0
	}
	...
	if ts, ok := d.rtmap[symIdx]; ok {
		...
		ctxt.Errorf(ts, "internal error: no entry for sym %d in tdmap\n", ts)
		return 0
	}
	ctxt.Errorf(symIdx, "internal error: no entry for sym %d in rtmap\n", symIdx)
	return 0
}
...
func (d *dwctxt2) importInfoSymbol(ctxt *Link, dsym loader.Sym) {
	...
	for i := 0; i < len(rslice); i++ {
		...
		// In the old implementation, the game played here was to
		// examine the Size field of the sym.Symbol of the relocation
		// target and see if it was zero. The idea here was that if
		// the DWARFSECREF was to a non-type symbol (e.g. abstract
		// function or file ref) it would have a non-zero size,
		// similarly if the DWARFSECREF was to the DIE for a type
		// (e.g. "go.info.<typename>" the we'd already generated, then
		// the size would be nonzero. With the new implementation we
		// do a check of the 'rtmap' instead (which will have an entry
		// for the 'go.info.<typename>' sym if we've processed it
		// already.
...
func (d *dwctxt2) mkBuiltinType(ctxt *Link, abrv int, tname string) *dwarf.DWDie {
	// create type DIE
	die := d.newdie(ctxt, &dwtypes, abrv, tname, 0)

	// Look up type symbol.
	gotype := d.lookupOrDiag(ctxt, "type."+tname)
...
func dwarfGenerateDebugInfo2(ctxt *Link) {
	...
	dwarfctxt.mkBuiltinType(ctxt, dwarf.DW_ABRV_BARE_PTRTYPE, "unsafe.Pointer")
	die := dwarfctxt.mkBuiltinType(ctxt, dwarf.DW_ABRV_BASETYPE, "uintptr")
	newattr(die, dwarf.DW_AT_encoding, dwarf.DW_CLS_CONSTANT, dwarf.DW_ATE_unsigned, 0)
	...
	for _, lib := range ctxt.Library {
		...
		for _, unit := range lib.Units {
			...
			if consts != 0 {
				...
				dwarfctxt.importInfoSymbol(ctxt, consts)
				consts = 0
			}
			...
			for _, s := range unit.Textp2 { // textp2 has been dead-code-eliminated already.
				...
				for ri := 0; ri < drelocs.Count; ri++ {
					...
					if r.Type == objabi.R_DWARFSECREF {
						...
						if _, ok := dwarfctxt.rtmap[rsym]; ok {
							// type already generated
							continue
						}
	...
	// Reclaim map storage, no more lookups needed.
	dwarfctxt.tdmap = nil
	dwarfctxt.tmap = nil
	dwarfctxt.rtmap = nil
