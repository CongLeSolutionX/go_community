...
func ldobjfile(ctxt *Link, f *objReader, pkg string, pn string) {
	ctxt.Version++
...
func readsym(ctxt *Link, f *objReader, pkg string, pn string) {
	if f.readByte() != 0xfe {
...
func readref(ctxt *Link, f *objReader, pkg string, pn string) {
	if f.readByte() != 0xfe {
...
func rdsym(ctxt *Link, f *objReader, pkg string) *LSym {
	i := f.readInt()
	if i == 0 {
		return nil
	}
