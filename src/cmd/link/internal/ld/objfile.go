...
func (s *GoSection) rdint64() int64 {
	uv := uint64(0)
	for shift := 0; ; shift += 7 {
		if shift >= 64 {
...
func (s *GoSection) rdint32() int32 {
	n := s.rdint64()
	if int64(int32(n)) != n {
		log.Panicf("%v out of range for int", n)
	}
...
func (s *GoSection) rdint16() int16 {
	n := s.rdint64()
	if int64(int16(n)) != n {
		log.Panicf("%v out of range for int", n)
	}
...
func (s *GoSection) rduint8() uint8 {
	n := s.rdint64()
	if int64(uint8(n)) != n {
		log.Panicf("%v out of range for int", n)
	}
...
overwrite:
	...
	if nreloc > 0 {
		...
		for i := 0; i < nreloc; i++ {
			r = &s.R[i]
			r.Off = int32(d.rdint32())
			r.Siz = uint8(d.rduint8())
			r.Type = int32(d.rdint32())
			r.Add = d.rdint64()
			r.Sym = d.rdsym()
		}
	}
	...
	if s.Type == obj.STEXT {
		...
		for i := 0; i < n; i++ {
			a = new(Auto)
			a.Asym = d.rdsym()
			a.Aoffset = int32(d.rdint32())
			a.Name = int16(d.rdint16())
			a.Gotype = d.rdsym()
			a.Link = s.Autom
			s.Autom = a
		}
