...
func (ls *LoaderStats) Dump(wf io.Writer) {
	...
	if ls.TotalSyms != 0 {
		...
		fmt.Fprintf(wf, "    Overwrites: %d\n", ls.Overwrites)
		fmt.Fprintf(wf, "    ExtObjRefs: %d\n", ls.ExtObjRefs)
		fmt.Fprintf(wf, "    ToLocalCalls: %d\n", ls.ToLocalCalls)
		fmt.Fprintf(wf, "    ToGlobalCalls: %d\n", ls.ToGlobalCalls)
	}
	if ls.OcacheHits+ls.OcacheMisses != 0 {
		fmt.Fprintf(wf, "    Ocache: Hit %d Miss %d hitrate %2.1f%%\n", ls.OcacheHits, ls.OcacheMisses, float64(ls.OcacheHits)/float64(ls.OcacheHits+ls.OcacheMisses)*100.0)
	}
	if ls.RcacheHits+ls.RcacheMisses != 0 {
		fmt.Fprintf(wf, "    Rcache: Hit %d Miss %d hitrate %2.1f%%\n", ls.RcacheHits, ls.RcacheMisses, float64(ls.RcacheHits)/float64(ls.RcacheHits+ls.RcacheMisses)*100.0)
	}
}
