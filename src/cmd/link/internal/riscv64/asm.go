...
func gentextLate(ctxt *ld.Link, ldr *loader.Loader) {
	...
	if ctxt.Textp == nil {
		log.Fatal("gentextLate called before Textp has been assigned")
	}
...
func findHI20Symbol(ctxt *ld.Link, ldr *loader.Loader, val int64) loader.Sym {
	...
	if s := ctxt.Textp[idx]; ldr.SymValue(s) == val && ldr.SymType(s) == sym.STEXT && ldr.SymName(s) == fakeLabelName {
		return s
	}
	return 0
}
