...
func gentextAfterAddressing(ctxt *ld.Link, ldr *loader.Loader) {
	...
	if ctxt.Textp == nil {
		log.Fatal("gentextAfterAddressing called before Textp has been assigned")
	}
	if ctxt.AdditionalELFSymbols != nil {
		log.Fatal("gentextAfterAddressing called with non-nil AdditionalELFSymbols")
	}
	for _, s := range ctxt.Textp {
		...
		for ri := 0; ri < relocs.Count(); ri++ {
			...
			ctxt.AdditionalELFSymbols = append(ctxt.AdditionalELFSymbols, sb.Sym())
		}
	}
	ldr.SortSyms(ctxt.AdditionalELFSymbols)
}

func findHI20Symbol(ctxt *ld.Link, ldr *loader.Loader, val int64) loader.Sym {
	idx := sort.Search(len(ctxt.AdditionalELFSymbols), func(i int) bool { return ldr.SymValue(ctxt.AdditionalELFSymbols[i]) >= val })
	if idx >= len(ctxt.AdditionalELFSymbols) {
		return 0
	}
	if s := ctxt.AdditionalELFSymbols[idx]; ldr.SymValue(s) == val && ldr.SymType(s) == sym.STEXT && ldr.SymName(s) == fakeLabelName {
		return s
	}
	return 0
}
