...
func findHI20Symbol(ctxt *ld.Link, ldr *loader.Loader, val int64) loader.Sym {
	for idx := sort.Search(len(ctxt.Textp), func(i int) bool { return ldr.SymValue(ctxt.Textp[i]) >= val }); idx < len(ctxt.Textp); idx++ {
		s := ctxt.Textp[idx]
		if ldr.SymValue(s) != val {
			return 0
		}
		if ldr.SymType(s) == sym.STEXT && ldr.SymName(s) == fakeLabelName {
			return s
		}
	}
	return 0
