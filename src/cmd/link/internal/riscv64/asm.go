...
func gentext2(ctxt *ld.Link, ldr *loader.Loader) {
	...
	if ctxt.Textp == nil {
		log.Fatal("gentext2 called before Textp has been assigned")
	}
	if ctxt.AddTextp != nil {
		log.Fatal("gentext2 called with non-nil AddTextp")
	}
	for _, s := range ctxt.Textp {
		...
		for ri := 0; ri < relocs.Count(); ri++ {
			...
			ctxt.AddTextp = append(ctxt.AddTextp, sb.Sym())
		}
	}
	ldr.SortSyms(ctxt.AddTextp)
}

func findHI20Symbol(ctxt *ld.Link, ldr *loader.Loader, val int64) loader.Sym {
	idx := sort.Search(len(ctxt.AddTextp), func(i int) bool { return ldr.SymValue(ctxt.AddTextp[i]) >= val })
	if idx >= len(ctxt.AddTextp) {
		return 0
	}
	if s := ctxt.AddTextp[idx]; ldr.SymValue(s) == val && ldr.SymType(s) == sym.STEXT && ldr.SymName(s) == fakeLabelName {
		return s
	}
	return 0
}
...
func elfreloc1(ctxt *ld.Link, out *ld.OutBuf, ldr *loader.Loader, s loader.Sym, r loader.ExtRelocView, sectoff int64) bool {
	...
	case objabi.R_RISCV_PCREL_ITYPE, objabi.R_RISCV_PCREL_STYPE:
		...
		// by this relocation.
		hi20Sym := findHI20Symbol(ctxt, ldr, ldr.SymValue(s)+int64(r.Off()))
		if hi20Sym == 0 {
