...
func testDump(t *testing.T, s state) {
	...
	dargs := []string{"-pkgre=^main$", "-live", "-i=" + s.outdirs[0] + "," + s.outdirs[1]}
...
func testPerc(t *testing.T, s state) {
	// Run the dumper on the two dirs we generated.
	dargs := []string{"-pkgre=^main$", "-i=" + s.outdirs[0] + "," + s.outdirs[1]}
...
func testTextfmt(t *testing.T, s state) {
	...
	want1 := "prog1/prog1.go:10.14,12.2 1 1"
	if lines[1] != want1 {
		dumplines(lines[0:10])
...
func testMergeSimple(t *testing.T, s state) {
	...
	dargs := []string{"-pkgre=^main$", "-live", "-i=" + outdir}
	lines = runToolOp(t, s, "debugdump", dargs)
	if len(lines) == 0 {
...
func testMergeCombinePrograms(t *testing.T, s state) {
	...
	// Dump the files in the merged output dir and examine the result.
	dargs := []string{"-pkgre=^main$", "-live", "-i=" + moutdir}
	lines = runToolOp(t, s, "debugdump", dargs)
	if len(lines) == 0 {
		t.Fatalf("dump run produced no output")
	}
...
func testSubtract(t *testing.T, s state) {
	...
	dargs := []string{"-pkgre=^main$", "-live", "-i=" + soutdir}
	lines = runToolOp(t, s, "debugdump", dargs)
	if len(lines) == 0 {
