...
func TestCovTool(t *testing.T) {
	...
	s.exepath1 = buildProg(t, prog1, "prog1", dir)
...
func testDump(t *testing.T, s state) {
	...
	dargs := []string{"-args", "-pkg=^main$", "-live",
...
func testMergeSimple(t *testing.T, s state) {
	...
	dargs := []string{"-args", "-pkg=^main$", "-live", "-i=" + outdir}
	lines = runToolOp(t, s, "dump", dargs)
	if len(lines) == 0 {
	...
	testpoints := []struct {
		...
		},
		{
			...
			regexp.MustCompile(`^0: L18:C3 -- L18:C11 NS=1 = 1$`),
		},
		{
			"third function unit 1",
			regexp.MustCompile(`^1: L19:C5 -- L19:C12 NS=1 = 1$`),
		},
		{
			"third function unit 2",
			regexp.MustCompile(`^2: L21:C10 -- L22:C10 NS=2 = 1$`),
		},
	}
...
func testMergeCombinePrograms(t *testing.T, s state) {
	...
	exepath2 := buildProg(t, prog2, "prog2", s.dir)
	...
	for k := 0; k < 2; k++ {
		...
		cmd := exec.Command(exepath2, args...)
	...
	// Dump the files in the merged output dir and examine the result.
	dargs := []string{"-args", "-pkg=^main$", "-live", "-i=" + moutdir}
	lines = runToolOp(t, s, "dump", dargs)
	if len(lines) == 0 {
		t.Fatalf("dump run produced no output")
	}
...
func testSubtract(t *testing.T, s state) {
	...
	dargs := []string{"-args", "-pkg=^main$", "-live", "-i=" + soutdir}
	lines = runToolOp(t, s, "dump", dargs)
	if len(lines) == 0 {
