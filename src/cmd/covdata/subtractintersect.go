...
func (s *sstate) Setup() {
	// Live flag is implied for both subtract and intersect
	*liveflag = true
	if *indirsflag == "" {
...
func (s *sstate) VisitFuncCounterData(data decodecounter.FuncPayload) {
	...
	if s.inidx == 0 {
		...
		if *verbflag >= 5 {
			fmt.Printf("visit fid=%d pk=%d found=%v len(val.ctrs)=%d len(data.ctrs)=%d", data.FuncIdx, data.PkgIdx, found, len(val.Counters), len(data.Counters))
		}
		if !found {
...
func (s *sstate) VisitFunc(pkgIdx uint32, fnIdx uint32, fd *coverage.FuncDesc) {
	...
	if *verbflag >= 5 {
		fmt.Printf("meta visit fid=%d pk=%d fname=%s file=%s found=%v len(val.ctrs)=%d", fnIdx, pkgIdx, fd.Funcname, fd.Srcfile, haveCounters, len(v.Counters))
	}
...
func (s *sstate) counterVisitor(f encodecounter.CounterVisitorFcn) bool {
