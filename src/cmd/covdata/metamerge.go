...
func (mm *metaMerge) endPod(pcombine bool) {
	...
	if pkgre != nil || pkgclass != coverage.PkgAll {
...
func (mm *metaMerge) visitPackage(pd *decodemeta.CoverageMetaDataDecoder, pkgIdx uint32, pcombine bool) {
	...
	if !ok {
		...
		if pcombine {
			...
			cmdb, err := encodemeta.NewCoverageMetaDataBuilder(pd.PackagePath(), pd.PackageName(), pd.ModulePath(), pd.PackageClassification())
			if err != nil {
