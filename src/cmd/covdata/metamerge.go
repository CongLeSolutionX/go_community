...
// packages using "-pkgre", and the "-pcombine" option is not in use),
// we can simply copy over the meta-data file from input to output.
//
// Case 2: if this is a select merge (-pkgre is in effect), then at
...
func (mm *metaMerge) emitMeta(outdir string, pcombine bool) [16]byte {
	...
	err = mfw.Write(finalHash, mm.modname, blobs, mm.Mode(), mm.Granularity())
	if err != nil {
...
func (mm *metaMerge) visitPackage(pd *decodemeta.CoverageMetaDataDecoder, pkgIdx uint32, pcombine bool) {
	...
	if !ok {
		...
		if pcombine {
			...
			cmdb, err := encodemeta.NewCoverageMetaDataBuilder(pd.PackagePath(), pd.ModulePath(), pd.PackageClassification())
			if err != nil {
