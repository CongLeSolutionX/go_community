...
func (m *mstate) VisitFuncCounterData(data decodecounter.FuncPayload) {
	...
	if *verbflag > 4 {
		fmt.Printf("visit pk=%d fid=%d len(counters)=%d", data.PkgIdx, data.FuncIdx, len(data.Counters))
	}
...
func (m *mstate) VisitFunc(pkgIdx uint32, fnIdx uint32, fd *coverage.FuncDesc) {
	...
	if *verbflag >= 3 {
		fmt.Printf("visit pk=%d fid=%d func %s", pkgIdx, fnIdx, fd.Funcname)
	}
	...
	if *pcombineflag {
		...
		if !ok {
			...
			if *verbflag >= 3 {
				fmt.Printf("new meta entry for fn %s fid=%d", fd.Funcname, gfidx)
			}
		}
		fnIdx = gfidx
	}
	...
	if ok {
		if *verbflag >= 3 {
			fmt.Printf("counter merge for %s fidx=%d", fd.Funcname, fnIdx)
		}
		// Merge.
		...
		if *verbflag >= 3 {
			fmt.Printf("null merge for %s fidx %d", fd.Funcname, fnIdx)
		}
		gfp := v
...
func (m *mstate) counterVisitor(f encodecounter.CounterVisitorFcn) bool {
	...
	for pidx, p := range m.pkgs {
		...
		sort.Ints(fids)
		if *verbflag >= 4 {
			fmt.Printf("fids for pk=%d: %+v", pidx, fids)
		}
		for _, fid := range fids {
			fp := p.ctab[uint32(fid)]
			if *verbflag >= 4 {
				fmt.Printf("counter write for pk=%d fid=%d len(ctrs)=%d", pidx, fid, len(fp.Counters))
			}
...
func (m *mstate) emitCounters(outdir string, metaHash [16]byte) {
	...
	args := []string{}
	cfw := encodecounter.NewCoverageDataFileWriter(fpath, cf, args, coverage.CtrULeb128)
