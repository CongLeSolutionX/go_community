...
func sortPorts() {
	...
	for _, result := range results {
		...
		if result.FirstClass {
			// windows amd64 is a first class port but the cgo version is not.
...
// isFirstClassPort reports whether the feature removal may be ignored.
// A removed feature can be ignored when its port string is not a first class port.
// The feature is expected to have a port string.
func isFirstClassPort(feature string) bool {
	for _, s := range firstClassStrings {
		if strings.Contains(feature, s) {
			return true
		}
	}
	return false
	...
	for len(required) > 0 || len(features) > 0 {
		...
		case len(features) == 0 || (len(required) > 0 && required[0] < features[0]):
			...
			if exceptionSet[feature] {
				...
				// okay.
			} else if *forceCtx == "" && featurePort.Match([]byte(feature)) && !isFirstClassPort(feature) {
				// okay. Default of all first class ports is selected. Non first class ports
