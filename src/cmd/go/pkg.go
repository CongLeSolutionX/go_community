...
func (p *Package) load(stk *importStack, bp *build.Package, err error) *Package {
	...
	if p.Name == "main" {
		...
		if buildLinkshared {
			...
			if _, err = os.Stat(shlibnamefile); err == nil {
				shlib, err := ioutil.ReadFile(shlibnamefile)
				if err != nil {
					// TODO(mwhudson): do something
				}
...
func isStale(p *Package, topRoot map[string]bool) bool {
	...
	// Package is stale if it is built into a shared library that
	// is older than the target
	lstat, err := os.Stat(filepath.Join(p.build.PkgTargetRoot, p.Shlib))
	if err == nil && built.After(lstat.ModTime()) {
		return true
	}
