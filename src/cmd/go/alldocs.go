...
Note: Build establishes some conventions, such as the directory
hierarchy for installed packages and the mapping of packages to
directories and code repositories, to make working with the other
components of the go command run smoothly. Not all projects can
follow these conventions, however. Installations that have their
own conventions or that use a separate software build system may
choose to use lower-level invocations such as "go tool compile" and
"go tool link" to avoid some of the overheads and design decisions
of the build tool.
