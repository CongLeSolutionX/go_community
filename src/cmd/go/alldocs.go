...
// module's vendor directory instead of downloading modules and loading packages
...
// module versions from go.mod files. However, if the "go" version in go.mod is
// 1.14 or higher, vendor/modules.txt (generated by 'go mod vendor') must
// contain metadata consistent with go.mod.
//
...
//
// Vendoring is enabled automatically when a vendor directory is present and the
// "go" version in the main module's go.mod file is 1.14 or higher. When vendoring
// is enabled, build commands like 'go build' and 'go test' will load packages
// from the vendor directory instead of accessing the network or the local
// module cache. To enable vendoring explicitly, invoke the go command with the
// flag -mod=vendor. To disable vendoring, use the flag -mod=mod.
//
