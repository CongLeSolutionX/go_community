	...
	if cleanTelemetry {
		...
		if err != nil {
			if !os.IsNotExist(err) {
				base.Fatalf("go: failed to read telemetry dir: %v", err)
			}
		} else {
			for _, entry := range entries {
				...
				entryPath := filepath.Join(telemetryDir, entry.Name())
				if err := sh.RemoveAll(entryPath); err != nil {
					base.Errorf("go: failed to remove %s: %v", entryPath, err)
				}
			}
		}
	}
