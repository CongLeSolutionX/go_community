...
func (b *Builder) reportCmd(a *Action, p *load.Package, desc, dir string, cmdOut []byte, cmdErr error) error {
	...
	out = replaceWord(out, b.WorkDir, "$WORK")
...
// replaceWord is like strings.ReplaceAll, but only replaces instances of old
// that are preceeded by ' ', '\t', or appear at the beginning of a line.
...
func (e *cmdError) Error() string {
	msg := "# " + e.desc + "\n" + e.text
	if e.importPath != "" && e.desc != e.importPath {
