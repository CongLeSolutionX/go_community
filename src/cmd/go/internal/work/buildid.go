...
//	actionID/contentID
//
// where the two halves are prepared by hashToString below. In general the
// prefix up to the first slash is the action ID and the suffix after the final
// slash is the content ID. (Usually there's only one slash, but see below for
// an exception.)
//
...
func (b *Builder) useCache(a *Action, p *load.Package, actionHash cache.ActionID, target string) bool {
	...
	// that is extraordinarily unlikely to appear in the output by chance.
