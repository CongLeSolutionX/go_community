	...
	for i := 0; i < len(list); i++ {
		...
		for _, x := range validNext {
			if arg == x {
				...
				if i+1 < len(list) {
					return fmt.Errorf("invalid flag in %s: %s %s (see https://golang.org/wiki/InvalidFlag)", source, arg, list[i+1])
				}
				return fmt.Errorf("invalid flag in %s: %s without argument (see https://golang.org/wiki/InvalidFlag)", source, arg)
			}
		}
	Bad:
