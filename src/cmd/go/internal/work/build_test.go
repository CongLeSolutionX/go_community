...
func TestRespectSetgid(t *testing.T) {
	...
	setGIDFile := filepath.Join(setgiddir, "setgid")
	if err := b.moveOrCopyFile(nil, setGIDFile, pkgfile.Name(), 0666, true); err != nil {
		t.Fatalf("moveOrCopyFile: %v", err)
	}

	got := strings.TrimSpace(cmdBuf.String())
	want := b.fmtcmd("", "cp %s %s", pkgfile.Name(), setGIDFile)
	if got != want {
		t.Fatalf("moveOrCopyFile(%q, %q): want %q, got %q", setGIDFile, pkgfile.Name(), want, got)
	}
}
