...
func InstallPackages(ctx context.Context, patterns []string, pkgs []*load.Package) {
	...
	for _, p := range pkgs {
		if p.Target == "" {
			...
			case p.Name != "main" && p.Standard && (p.ImportPath == "unsafe" || !buildinternal.NeedsInstalledDotA(p.ImportPath)):
				// unsafe is a built-in package, has no target, or
				// Stdlib package does not need an installed .a; instead
