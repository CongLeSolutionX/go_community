...
and test commands:
	...
	-overlay file
		...
		file path is empty. Support for the -overlay flag has some limitations:
		importantly, cgo includes outside the include path must be in the same
		directoryas the Go package they are included from, and overlays will
		not appear when binaries and tests are run through go run and go test
		respectively.
	-pkgdir dir
