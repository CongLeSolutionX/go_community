...
type Builder struct {
	...
	id            sync.Mutex
	toolIDCache   map[string]string // cache of tool IDs
	buildIDCache  map[string]string // cache of build IDs
	fileHashCache map[string]string // cache of file hashes
}
...
func (b *Builder) installAction(a1 *Action) *Action {
	...
	return b.cacheAction(a1.Mode+"-install", p, func() *Action {
		...
		if a1.Mode == "link" {
			buildID := b.buildID(p.Target)
			id := strings.Split(buildID, ".")
			if len(id) == 4 {
				println("PRELOAD", p.Target, buildID)
			}
		}
