...
type shellShared struct {
	...
	mkdirCache sync.Map // a cache of created directories; string -> *sync.Once
}
...
func (sh *Shell) Mkdir(dir string) error {
	...
	// Use a Once to make sure we don't return while
	// there's a concurrent mkdir for dir in flight.
	onceVal, _ := sh.mkdirCache.LoadOrStore(dir, new(sync.Once))
	...
	if cfg.BuildN || cfg.BuildX {
		...
		show := func() bool {
			for _, path := range paths {
				if _, ok := sh.mkdirCache.Load(path); ok {
					return true
				}
