...
func runTool(ctx context.Context, cmd *base.Command, args []string) {
	...
	if err != nil {
		...
		tool, mod := loadModTool(ctx, toolName)
		if tool != "" {
			buildAndRunModtool(ctx, tool, mod, args[1:])
			return
		}
...
func loadModTool(ctx context.Context, name string) (string, module.Version) {
	modload.InitWorkfile()
	modload.LoadModFile(ctx)

	matches := []string{}
	for tool, _ := range modload.MainModules.Tools() {
	...
	if len(matches) == 1 {
		return matches[0], modload.MainModules.Tools()[matches[0]]
	}
...
func buildAndRunModtool(ctx context.Context, tool string, mod module.Version, args []string) {
	work.BuildInit()
	b := work.NewBuilder("")
	defer func() {
	...
	cacheDir := filepath.Join(cache.Default().ToolDir(), mod.Path)
	p.Target = filepath.Join(cacheDir, tool)
