...
func runGitAuth(dir string, host string) {
	if host == "" {
		// No explicit host was passed, but 'git credential'
		// provides no way to enumerate existing credentials.
		// Wait for a request for a specific host.
		return
	}
	if dir == "" {
		...
		panic("GOAUTH: gitauth invoked in an arbitrary directory") // this should be caught earlier so this should never happen.
	}
	u, err := url.ParseRequestURI("https://" + host)
	if err != nil {
		if cfg.BuildX {
			log.Printf("invalid request URI (%v): %q\n", err, os.Args[1])
		}
		return
	}
	...
	for lastStatus == http.StatusUnauthorized {
		...
		if err != nil {
			if cfg.BuildX {
				log.Printf("'git credential fill' failed (url=%s): %v\n", u, err)
			}
			return
		}
		...
		if !strings.HasPrefix(u.String(), prefix.String()) {
			if cfg.BuildX {
				log.Printf("requested a credential for %q, but 'git credential fill' provided one for %q\n", u, prefix)
			}
			return
		}
		...
		if err != nil {
			if cfg.BuildX {
				log.Printf("internal error constructing HTTP HEAD request: %v\n", err)
			}
			return
		}
	...
	storeCredential([]string{host}, lastHeader)
}

func parseGitAuth(data []byte) (prefix *url.URL, username string, password string) {
	prefix = new(url.URL)
