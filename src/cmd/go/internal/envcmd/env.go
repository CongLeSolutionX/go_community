...
func MkEnv() []cfg.EnvVar {
	...
	env := []cfg.EnvVar{
		...
		{Name: "GORACE", Value: cfg.Getenv("GORACE")},
...
func checkEnvWrite(key, val string, env []cfg.EnvVar) error {
	...
	if envRE == nil {
		// These variables apply in various configurations
		// but are not reported by 'go env' by default.
		// Note that GODEBUG, GOMAXPROCS, GOTRACEBACK and the like
		// are for configuring the _runtime_ not the go command
		// and so they must not be accepted here.
		envRE = regexp.MustCompile(`^(GO_EXTLINK_ENABLED|GOARM|GO386|GOMIPS|GOMIPS64|GOWASM|CGO_(C|CPP|CXX|F|LD)FLAGS_(DIS)?ALLOW)$`)
	}
	if envRE.MatchString(key) {
		goto Known
	}
