...
func runTest(ctx context.Context, cmd *base.Command, args []string) {
	...
	if cfg.BuildCoverPkg != nil {
		...
		testCoverPkgs = load.SelectCoverPackages(plist, match, false)
	}
...
func builderTest(b *work.Builder, ctx context.Context, pkgOpts load.PackageOpts, p *load.Package, imported bool) (buildAction, runAction, printAction *work.Action, err error) {
	...
	// Setting DisableCovHooks here tells the compiler to avoid
	// emitting any additional hooks to the package "main" init
	// function. These hooks are needed for "go build -cover",
	// but for "go test -cover" the code in _testmain.go is going
