...
// incrementConfig increments counters (if Telemetry is enabled) for the
// basic config the configuration the command is running in.
func incrementConfig() {
	...
	telemetry.Inc("go/platform/host/goos:" + runtime.GOOS)
	telemetry.Inc("go/platform/host/goarch:" + runtime.GOARCH)
	switch cfg.Goarch {
	case "386":
