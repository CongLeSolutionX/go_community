...
provided with a set of required field names to be output. If so, those
required fields will always appear in JSON output, but others may be
omitted to save work in computing the JSON struct.
...
func needJSONField(s string) bool {
	if len(listJsonFields) == 0 {
		return true
	}
	return listJsonFields[s]
}
...
func runList(ctx context.Context, cmd *base.Command, args []string) {
		...
		do = func(x any) {
			if len(listJsonFields) > 0 {
				...
				for i := 0; i < v.NumField(); i++ {
					if !listJsonFields[v.Type().Field(i).Name] {
	...
	needStale := listJson || strings.Contains(*listFmt, ".Stale") || needJSONField("Stale")
