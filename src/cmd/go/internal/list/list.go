...
func (v *jsonFlag) need(s string) bool {
	if len(*v) == 0 {
		return true
	}
	return (*v)[s]
...
func runList(ctx context.Context, cmd *base.Command, args []string) {
		...
		do = func(x any) {
			if !listJsonFields.needAll() {
				...
				for i := 0; i < v.NumField(); i++ {
					if !listJsonFields.need(v.Type().Field(i).Name) {
	...
	needStale := (listJson && listJsonFields.need("Stale")) || strings.Contains(*listFmt, ".Stale")
