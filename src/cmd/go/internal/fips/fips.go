...
//
// The first call to Version (or [Enabled] or [Snapshot]) interprets the
// GOFIPS140 setting. If it is invalid, they will call [base.Fatalf].
func Version() string {
	checkInit()
	return version
}
...
//
// The first call to Enabled (or [Snapshot] or [Version]) interprets the
// GOFIPS140 setting. If it is invalid, they will call [base.Fatalf].
func Enabled() bool {
	checkInit()
	return version != "off"
...
//
// The first call to Snapshot (or [Enabled] or [Version]) interprets the
// GOFIPS140 setting. If it is invalid, they will call [base.Fatalf].
func Snapshot() bool {
	checkInit()
...
func initDir() {
	...
	mod := module.Version{"golang.org/fips140", v}
