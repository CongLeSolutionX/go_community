...
`,
			".",
			[]file{
				{".", "root", 0, os.ModeDir | 0755, true},
				{"file.txt", "file.txt", 0, 0644, false},
			},
		},
...
`,
			...
			[]file{
				...
				{"file.txt", "file.txt", 23, 0644, false},
				{"other.txt", "other.txt", 23, 0644, false},
			},
		},
...
`,
			...
			[]file{
				...
				{"dir/file.txt", "file.txt", 23, 0644, false},
				{"other.txt", "other.txt", 23, 0644, false},
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < len(got) && i < len(tc.wantFiles); i++ {
				...
				if got[i].mode != tc.wantFiles[i].mode {
					t.Errorf("mode of file #%v in walk, got %v, want %v", i, got[i].mode, tc.wantFiles[i].mode)
				}
...
`)
	...
	Walk(".", func(path string, info os.FileInfo, err error) error {
		...
		if path == "skipthisdir" || path == "dontskip/skip" {
			return filepath.SkipDir
		}
		return nil
	})

	wantSeen := []string{".", "dontskip", "dontskip/file.go", "dontskip/skip", "dummy.txt", "skipthisdir"}
...
`,
			"dir",
			file{"dir", 0, 0755 | os.ModeDir, true},
			false,
		},
		{
			"dir_in_overlay",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got.Mode() != tc.want.mode {
				t.Errorf("lstat(%q).Mode(): got %v, want %v", tc.path, got.Mode(), tc.want.mode)
			}
