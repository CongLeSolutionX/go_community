...
//
// The upgrades map indicates which steps in the path correspond to upgrades.
func NewBuildListError(err error, path []module.Version, upgrade func(module.Version) module.Version) *BuildListError {
	stack := make([]buildListErrorElem, 0, len(path))
	for len(path) > 1 {
		reason := "requires"
		if upgrade != nil && upgrade(path[0]) == path[1] {
			reason = "updating to"
		}
