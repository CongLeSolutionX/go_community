...
// The reqs slice must not be modified after this point.
//
...
// GraphReqs implements Reqs for a Graph.
type GraphReqs struct {
	g *Graph
}

// AsReqs returns a representation of g that implements the Reqs interface,
// with the root requirements attributed to the given target module (which
// must not otherwise have requirements in the graph).
func (g *Graph) AsReqs() *GraphReqs {
	gr := &GraphReqs{g}
	var _ Reqs = gr
	return gr
}

// Required returns m.g.RequiredBy(m) with a nil error.
func (gr *GraphReqs) Required(m module.Version) ([]module.Version, error) {
	reqs, _ := gr.g.RequiredBy(m)
	return reqs, nil
}

// Max returns the result of the max function originally passed to NewGraph.
func (gr *GraphReqs) Max(v1, v2 string) string {
	if gr.g.cmp(v1, v2) < 0 {
		return v2
	}
	return v1
