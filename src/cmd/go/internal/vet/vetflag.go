...
func vetFlags(args []string) (passToVet, packageNames []string) {
	...
	if err := json.Unmarshal(out.Bytes(), &analysisFlags); err != nil {
		fmt.Fprintf(os.Stderr, "can't unmarshal JSON from %s -flags: %v", tool, err)
		os.Exit(2)
	}

	// Add vet's flags to cmdflag.Defns.
	//
	// Some flags, in particular -tags and -v, are known to vet but
	// also defined as build flags. This works fine, so we don't
	// define them here but use AddBuildFlags to init them. However
	// some, like -x, are known to the build but not to vet. We
	// handle them in vetFlags.
