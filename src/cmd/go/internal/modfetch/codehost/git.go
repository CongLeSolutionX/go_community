...
func (r *gitRepo) RecentTag(rev, prefix string) (tag string, err error) {
	...
	// Some git servers are willing to serve hashes that aren't reachable from any
	// branch or tag. See if we can fetch a history for rev that way.
	//
	// Ignore errors: even if this fetch fails, some other goroutine may have
	// fetched rev while we were waiting on r.mu, and Git 2.7.4 doesn't support
	// fetch --unshallow with a commit hash, and some servers don't either.
	_ = r.fetchUnshallow(rev)
	describe()
	return tag, err
}
