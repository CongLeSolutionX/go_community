	...
	if !didStatLocal {
		if info, err := r.statLocal(rev, hash); err == nil {
			if strings.HasPrefix(ref, "refs/tags/") {
				...
				Run(r.dir, "git", "tag", strings.TrimPrefix(ref, "refs/tags/"))
			}
			return info, nil
		}
	}
...
func (r *gitRepo) ReadFileRevs(revs []string, file string, maxSize int64) ([]*FileRev, error) {
	// Create space to hold results.
	var files []*FileRev
	fileMap := make(map[string]*FileRev)
	...
	need, err := r.readFileRevs(revs, file, fileMap)
	if err != nil {
		return nil, err
	}
	if len(need) == 0 {
	...
	if _, err := r.readFileRevs(redo, file, fileMap); err != nil {
		return nil, err
	}
...
func (r *gitRepo) readFileRevs(tags []string, file string, fileMap map[string]*FileRev) (missing []string, err error) {
	...
	for _, tag := range tags {
		...
		switch commitType {
		default:
			f.Err = fmt.Errorf("unexpected commit type %q for rev %s", commitType, tag)
		...
		case "tag", "commit":
			switch fileType {
			default:
				f.Err = &os.PathError{Path: tag + ":" + file, Op: "read", Err: fmt.Errorf("unexpected blob type %q", fileType)}
			case "missing":
