...
type Repo interface {
	...
	// It should refuse to read more than maxSize bytes.
	// The result is a list of FileRevs giving the result of
	// reading each rev's file.
	// The number of FileRevs returned should match the
	// number of revs requested, with files[i] describing the result of reading from revs[i].
	// A file simply being missing or even corrupted in revs[i]
	// should be reported only in files[i].Err, not in the error result
	// from ReadFileRevs.
	// The overall call should return an error (and no FileRevs) only
	...
	ReadFileRevs(revs []string, file string, maxSize int64) (files []*FileRev, err error)
