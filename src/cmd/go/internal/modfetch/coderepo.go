...
func (r *codeRepo) convert(info *codehost.RevInfo, statVers string) (*RevInfo, error) {
	...
	checkGoMod := func() (*RevInfo, error) {
		if statVers != "" && statVers == module.CanonicalVersion(statVers) && semver.Compare(statVers, info2.Version) != 0 {
			resolved := info2.Version
			info2.Version = statVers
			return nil, invalidf("resolves to version %v (is it a branch instead of a tag?)", resolved)
		}
	...
	tagToVersion := func(tag string) (v string, tagIsCanonical bool) {
		...
		// Only consider a retracted tag if it was explicitly requested by the caller.
		if isRetracted(v) && (statVers == "" || semver.Compare(v, statVers) != 0) {
			return "", false
		}
		if v == trimmed {
			tagIsCanonical = true
