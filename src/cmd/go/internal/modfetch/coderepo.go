...
func (r *codeRepo) convert(info *codehost.RevInfo, statVers string) (*RevInfo, error) {
	...
	checkGoMod := func(v string) (*RevInfo, error) {
		...
		if !module.MatchPathMajor(base, r.pathMajor) {
			if canUseIncompatible() {
				...
				if v == strings.TrimSuffix(statVers, "+incompatible") {
					v = statVers // Include "+incompatible" in the error version if it was requested explicitly.
				}
				if r.pathMajor != "" {
		...
		if statVers != "" && statVers == module.CanonicalVersion(statVers) {
			...
			// If that occurs, error out explicitly.
	...
	for _, pathTag := range info.Tags {
		...
		if statVers != "" && semver.Compare(v, statVers) == 0 {
			...
			if tagIsCanonical {
				...
				// (If it isn't actually allowed, we'll error out in checkGoMod.)
				return checkGoMod(v)
			} else {
	...
	if highestCanonical != "" {
		return checkGoMod(highestCanonical)
	}
	...
	return checkGoMod(module.PseudoVersion(r.pseudoMajor, pseudoBase, info.Time, info.Short))
}
