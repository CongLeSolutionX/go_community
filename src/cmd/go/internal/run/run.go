...
func runRun(ctx context.Context, cmd *base.Command, args []string) {
	...
	if i > 0 {
		...
		for _, file := range files {
			if strings.HasSuffix(file, "_test.go") {
				...
				base.CmdFatalf("cannot run *_test.go files (%s)", file)
			}
		}
		...
		if len(pkgs) == 0 {
			base.CmdFatalf("no packages loaded from %s", arg)
		}
		if len(pkgs) > 1 {
			var names []string
			for _, p := range pkgs {
			...
			base.CmdFatalf("pattern %s matches multiple packages:\n\t%s", arg, strings.Join(names, "\n\t"))
		}
		p = pkgs[0]
		i++
	} else {
		base.CmdFatalf("no go files listed")
	}
	...
	if p.Internal.CmdlineFiles {
		...
		if len(p.GoFiles) > 0 {
			...
			base.CmdFatalf("no suitable source files%s", hint)
		}
