...
func CheckPath(path string) error {
	if err := checkPath(path, pathOK, false); err != nil {
...
func checkPath(path string, charOK func(rune) bool, fileName bool) error {
	...
	for i, r := range path {
		if r == '/' {
			if err := checkElem(path[elemStart:i], charOK, fileName); err != nil {
				return err
			}
			elemStart = i + 1
		}
	}
	if err := checkElem(path[elemStart:], charOK, fileName); err != nil {
		return err
	}
	return nil
}
...
func checkElem(elem string, charOK func(rune) bool, fileName bool) error {
	if elem == "" {
...
func CheckFilePath(path string) error {
	if err := checkPath(path, fileNameOK, true); err != nil {
