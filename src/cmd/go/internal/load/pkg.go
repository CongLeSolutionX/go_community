...
func PrepareForCoverageBuild(pkgs []*Package) {
	...
	matchMainModAndCommandLine := func(p *Package) bool {
		return p.Internal.CmdlineFiles || p.Internal.CmdlinePkg || (!p.Standard && p.Module != nil && p.Module.Main)
	}
	...
	if len(cfg.BuildCoverPkg) != 0 {
		...
		// Without -coverpkg, use different defaults depending on
		// whether module mode is enabled.
		if modload.Enabled() {
			// Module mode is on: default to instrumenting only
			// packages in the main module, as well as packages/files
			// specifically named on the command line.
			match = []func(*Package) bool{matchMainModAndCommandLine}
		} else {
			// Module mode is off, so instrument just those packages
			// or Go files named on the command line.
			matchCmdLine := func(p *Package) bool {
				return p.Internal.CmdlineFiles || p.Internal.CmdlinePkg
			}
			match = []func(*Package) bool{matchCmdLine}
		}
	}
