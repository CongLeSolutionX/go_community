...
func GoFilesPackage(gofiles []string) (*Package, error) {
	...
	for _, file := range gofiles {
		...
		if fi.IsDir() {
			return nil, errors.New(fmt.Sprintf("%s is a directory, should be a Go file", file))
		}
		...
		if dir == "" {
			...
			return nil, errors.New(fmt.Sprintf("named files must all be in one directory; have %s and %s", dir, dir1))
		}
