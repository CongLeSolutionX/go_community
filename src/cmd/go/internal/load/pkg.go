...
func (p *Package) setBuildInfo(ctx context.Context, autoVCS bool) {
	...
	if repoDir != "" && vcsCmd.Status != nil {
		...
		var vers string
		// Determine the correct version of this module at the current revision and update its build metadata accordingly.
		err = modfetch.TryProxies(func(proxy string) error {
			repo := modfetch.Lookup(ctx, proxy, p.Module.Path, true)
