...
func (p *PackageError) Pos() string {
	...
	if !frame.HasReturnPos() {
		return ""
	}
	return frame.ShortReturnPos()
}
...
func (p *PackageError) Error() string {
	...
	if len(stack) <= 2 && stack.LastFrame().HasReturnPos() {
		return stack.LastFrame().ShortReturnPos() + ": " + p.Err.Error()
	}
	...
	for i, frame := range stack {
		...
		if isImportPathError && i == len(stack)-2 && stack.LastFrame().HasReturnPos() {
			trace = append(trace, stack.LastFrame().ShortReturnPos())
			break
		}
...
func NewImportStackFrame(path string, importPos []token.Position) ImportStackFrame {
	result := ImportStackFrame{Path: path}
	if len(importPos) > 0 {
		result.ReturnPos = importPos[0]
	}
	return result
}

func (f ImportStackFrame) HasReturnPos() bool {
	return f.ReturnPos.IsValid()
}

func (f ImportStackFrame) ShortReturnPos() string {
	if !f.ReturnPos.IsValid() {
		return f.Path
	}
	pos := f.ReturnPos
...
func (s *ImportStack) Push(path string) {
	*s = append(*s, ImportStackFrame{Path: path})
}

// Pushes a path onto the stack, and marks where it was imported.
func (s *ImportStack) PushImport(path string, importPos []token.Position) {
	var pos token.Position
	if len(importPos) > 0 {
		pos = importPos[0]
	}
	*s = append(*s, ImportStackFrame{Path: path, ReturnPos: pos})
}
