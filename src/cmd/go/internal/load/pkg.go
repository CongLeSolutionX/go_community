...
func (p *Package) load(ctx context.Context, opts PackageOpts, path string, stk *ImportStack, importPos []token.Position, bp *build.Package, err error) {
	...
	if useBindir {
		...
		binGOOS, binGOARCH := runtime.GOOS, runtime.GOARCH
		if p.Goroot {
			binGOOS, binGOARCH = cfg.InstalledGOOS, cfg.InstalledGOARCH
		}
		if cfg.BuildContext.GOOS != binGOOS || cfg.BuildContext.GOARCH != binGOARCH {
