...
func TestPackagesAndErrors(ctx context.Context, opts PackageOpts, p *Package, cover *TestCover) (pmain, ptest, pxtest *Package) {
	...
	if goexperiment.CoverageRedesign && cover != nil {
		TestMainDeps = append(TestMainDeps, "os/exec")
	}
	...
	if cover != nil && cover.Pkgs != nil && !goexperiment.CoverageRedesign {
	...
	if goexperiment.CoverageRedesign {
		t.CovDataPath = "`" + base.Tool("covdata") + "`"
	}
	...
	if !goexperiment.CoverageRedesign {
		...
		ptest.Internal.CoverMode = cover.Mode
		pmain.Internal.CoverMode = "regonly"
	}
...
//go:linkname runtime_coverage_emitCounterData runtime/coverage.emitCounterData
func runtime_coverage_emitCounterData()

//go:linkname runtime_coverage_emitMetaData runtime/coverage.emitMetaData
func runtime_coverage_emitMetaData()

func coverTearDown(coverprofile string, gocoverdir string) int {
	var err error
	if gocoverdir == "" {
		gocoverdir, err = os.MkdirTemp("", "gocoverdir")
		if err != nil {
			println("error: setting GOCOVERDIR: os.MkdirTemp returns ", err)
			return 1
		}
		defer os.RemoveAll(gocoverdir)
	}
	err = os.Setenv("GOCOVERDIR", gocoverdir)
	if err != nil {
		println("setting GOCOVERDIR: os.Setenv returns ", err)
		return 1
	}
	runtime_coverage_emitMetaData()
	runtime_coverage_emitCounterData()
	args := []string{"dump", "-emitperc", "-i", gocoverdir}
	if coverprofile != "" {
		args = append(args, "-emitlegacy", coverprofile)
	}
	cmd := exec.Command({{.CovDataPath}}, args...)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err = cmd.Run()
	if err != nil {
		println("dumping coverage data: ", err)
		return 1
	}
    return 0
}
...
func main() {
{{if .Cover}}
	coverprofilep, gocoverdirp := testing.RegisterCover2(testing.Cover{
		Mode: {{printf "%q" .Cover.Mode}},
		CoveredPackages: {{printf "%q" .Covered}},
	})
{{end}}
...
{{with .TestMain}}
	{{.Package}}.{{.Name}}(m)
	rc := int(reflect.ValueOf(m).Elem().FieldByName("exitCode").Int())
{{else}}
	rc := m.Run()
{{end}}
{{if .Cover}}
	rc |= coverTearDown(**coverprofilep, **gocoverdirp)
{{end}}
	os.Exit(rc)
}
