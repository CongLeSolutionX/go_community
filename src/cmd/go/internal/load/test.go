...
func TestPackagesFor(p *Package, cover *TestCover) (pmain, ptest, pxtest *Package, err error) {
	...
	for _, p1 := range []*Package{ptest, pxtest, pmain} {
		...
		if len(p1.DepsErrors) > 0 {
			...
			// Check the second highest index to see if this is a direct error.
			if len(perr.ImportStack) >= 2 {
				importer := strings.TrimSuffix(perr.ImportStack[len(perr.ImportStack)-2], " (test)")
