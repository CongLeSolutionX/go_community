...
func coverTearDown(coverprofile string, gocoverdir string) (string, error) {
	...
	if gocoverdir == "" {
		...
		if err != nil {
			return "error: setting GOCOVERDIR: os.MkdirTemp error ", err
		}
		defer os.RemoveAll(gocoverdir)
	}
	err = os.Setenv("GOCOVERDIR", gocoverdir)
	if err != nil {
		return "setting GOCOVERDIR: os.Setenv error ", err
	}
	...
	err = cmd.Run()
	if err != nil {
		return "dumping coverage data: ", err
	}
    return "", nil
}
{{end}}
...
func main() {
{{if .Cover}}
	testing.RegisterCover2(testing.Cover2{
		Mode: {{printf "%q" .Cover.Mode}},
		TearDown: coverTearDown,
	})
{{end}}
	m := testing.MainStart(testdeps.TestDeps{}, tests, benchmarks, fuzzTargets, examples)
{{with .TestMain}}
