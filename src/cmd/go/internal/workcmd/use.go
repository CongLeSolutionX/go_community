...
func workUse(ctx context.Context, gowork string, wf *modfile.WorkFile, args []string) {
	...
	for _, useDir := range args {
		...
		info, err := fsys.Stat(base.ShortPathConservative(absArg))
		if err != nil {
			// Errors raised from os.Stat are formatted to be more user-friendly.
			if os.IsNotExist(err) {
				err = fmt.Errorf("directory %v does not exist", base.ShortPathConservative(absArg))
			}
			sw.Error(err)
			continue
		} else if !info.IsDir() {
			sw.Error(fmt.Errorf("%s is not a directory", base.ShortPathConservative(absArg)))
			continue
		}
