...
var useR = cmdUse.Flag.Bool("r", false, "recursively look into argument directories")
...
func runUse(ctx context.Context, cmd *base.Command, args []string) {
	...
	for _, dir := range workFile.Directory {
		haveDirs[filepath.Join(filepath.Dir(gowork), dir.Path)] = true
	}
	...
	lookDir := func(dir string) {
		...
		if err != nil {
			base.Errorf("go: determining relative path: %v", err)
		}
	...
	for dir := range removeDirs {
		workFile.DropDirectory(dir)
	}
	for dir := range addDirs {
		workFile.AddDirectory(dir, "")
	}
	workFile.SortBlocks()
	...
	err = lockedfile.Transform(gowork, func(lockedData []byte) ([]byte, error) {
		if !bytes.Equal(lockedData, data) {
			return nil, errors.New("go.work changed during command; not overwriting")
		}
		return out, nil
	})
	if err != nil {
