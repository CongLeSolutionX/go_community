		...
		if fb, ok := f.Value.(boolFlag); ok && fb.IsBoolFlag() {
			if hasValue {
				if err := fb.Set(value); err != nil {
					fmt.Fprintf(flags.Output(), "go: invalid boolean value %q for flag %s (from $GOFLAGS): %v\n", value, name, err)
					flags.Usage()
				}
			} else {
				if err := fb.Set("true"); err != nil {
					fmt.Fprintf(flags.Output(), "go: invalid boolean flag %s (from $GOFLAGS): %v\n", name, err)
					flags.Usage()
				}
			}
		} else {
			if !hasValue {
				fmt.Fprintf(flags.Output(), "go: flag needs an argument: %s (from $GOFLAGS)\n", name)
				flags.Usage()
			}
			if err := f.Value.Set(value); err != nil {
				fmt.Fprintf(flags.Output(), "go: invalid value %q for flag %s (from $GOFLAGS): %v\n", value, name, err)
				flags.Usage()
			}
		}
	}
