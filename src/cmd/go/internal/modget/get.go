...
func runGet(cmd *base.Command, args []string) {
	...
	for _, arg := range search.CleanPatterns(args) {
		...
		default:
			...
			// and set prevM if so. Otherwise, it won't match, and we'll upgrade
			// the module later when we load packages. If "path" refers to a
			// previously unknown module, we'll query it and add it to the
			// build list below.
	...
	// Upgrade modules specifically named on the command line. This skips
	// loading packages at an old version, only to upgrade and reload at a
	// new version.
