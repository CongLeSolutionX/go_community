...
//
// findAndUpgradeImports also returns a sorted list of module paths that provide
// packages, which may be used for reporting changes or problems.
...
func (r *resolver) checkPackagesAndRetractions(ctx context.Context, pkgPatterns []string) {
	...
	// Reload packages, returning errors for missing and ambiguous imports.
	var haveError bool
	...
	for i := 0; i < len(retractions); i++ {
		...
		r.work.Add(func() {
			...
			if retractErr := (*modload.ModuleRetractedError)(nil); errors.As(err, &retractErr) {
				retractions[i].rationale = "retracted by module author"
				if len(retractErr.Rationale) > 0 {
					retractions[i].rationale = modload.ShortRetractionRationale(retractErr.Rationale[0])
				}
			}
		})
	}
	...
	for _, r := range retractions {
		if r.rationale != "" {
			fmt.Fprintf(os.Stderr, "go: warning: %s is retracted: %s\n", r.m, r.rationale)
		}
	}
