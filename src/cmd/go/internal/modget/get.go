	...
	if !*getD && len(pkgPatterns) > 0 {
		...
		for _, pkg := range pkgs {
			if pkg.Name == "main" && pkg.Module != nil {
				if _, ok := modload.MainModules.VersionWithPath(pkg.Module.Path); !ok {
...
func (r *resolver) queryNone(ctx context.Context, q *query) {
	...
	if !q.isWildcard() {
		q.pathOnce(q.pattern, func() pathSet {
			...
			if v, ok := modload.MainModules.VersionWithPath(q.pattern); hasModRoot && ok {
	...
	for _, curM := range r.buildList {
		...
		q.pathOnce(curM.Path, func() pathSet {
			if modload.HasModRoot() && modload.MainModules.Contains(curM) {
