...
func (r *resolver) reportChanges() {
	...
	explicitReqs, err := modload.MinReqs().Required(modload.Target)
	if err != nil {
		base.Fatalf("go get: %v", err)
	}
	for _, req := range explicitReqs {
		old := r.initialVersion[req.Path]
		new := req.Version
		if old != new {
			changes[req.Path] = change{req.Path, old, new}
		}
	}
