...
func getQuery(path, vers string, prevM module.Version, forceModulePath bool) (module.Version, error) {
	shouldCheckUpgrade := (vers == "" || vers == "latest" || vers == "patch") && prevM.Version != ""
	switch vers {
	case "":
		vers = "latest"
	case "patch":
	...
	if forceModulePath || !strings.Contains(path, "...") {
		...
		var err error
		rev, err = modload.Query(path, vers, modload.Allowed)
	...
	// If we're upgrading to "latest" or "patch", make sure we don't
	// accidentally downgrade from a newer prerelease or pseudo-version.
	if shouldCheckUpgrade {
		rev.Version = checkUpgrade(rev, prevM.Version)
	}
