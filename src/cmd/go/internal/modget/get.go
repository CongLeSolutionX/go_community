...
type resolver struct {
	...
	initialReplacement map[string]module.Version // index of replacements in intial build list
...
func newResolver(ctx context.Context, queries []*query) *resolver {
	...
	initialReplacement := make(map[string]module.Version)
	for _, m := range buildList {
		initialVersion[m.Path] = m.Version
		if r := modload.Replacement(m); r.Path != "" {
			initialReplacement[m.Path] = r
		}
	}
...
func (r *resolver) checkPackageProblems(ctx context.Context, pkgPatterns []string) {
	...
	for i := range r.buildList {
		...
		oldActual := old
		if oldRepl := r.initialReplacement[old.Path]; oldRepl.Path != "" {
			oldActual = oldRepl
		}
		...
		r.work.Add(func() {
			if _, err := modfetch.DownloadZip(ctx, mActual); err != nil {
				if mActual == m {
					err = fmt.Errorf("finding sum for %s: %v", mActual, err)
				} else {
					err = fmt.Errorf("finding sum for %s (replacement of %s): %v", mActual, m, err)
				}
				sumErrs[i] = err
			}
		})
