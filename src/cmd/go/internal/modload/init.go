...
var (
	...
	// WorkspacesEnabled controls whether commands can run in workspace
	// mode
...
var (
	initialized bool
	workRoot    string
	modRoots    []string // These are used to initialize the MainModules. TODO matloob thread this through and delete it
	gopath      string
)
...
func (mms *mainModules) Versions() []module.Version {
	vs := make([]module.Version, len(mms.versions))
	for i := range mms.versions {
		vs[i] = mms.versions[i]
	}
	return vs
}
...
func (mms *mainModules) ModFilePath(m module.Version) string {
...
func (mms *mainModules) MustGetSingleMainModule() module.Version {
...
func (mms *mainModules) GetSingleIndexOrNil() *modFileIndex {
	if len(mms.versions) == 0 {
		return nil
	}
	if len(mms.versions) != 1 {
...
func (mms *mainModules) Index(m module.Version) *modFileIndex {
	mms.indexMu.Lock()
	defer mms.indexMu.Unlock()
	return mms.indices[m]
}

func (mms *mainModules) SetIndex(m module.Version, index *modFileIndex) {
	mms.indexMu.Lock()
	defer mms.indexMu.Unlock()
	mms.indices[m] = index
}

func (mms *mainModules) ModFile(m module.Version) *modfile.File {
	return mms.modFiles[m]
}

func (mms *mainModules) Size() int {
...
func Init() {
	...
	// Turn workRoot into a path or ""
	if WorkspacesEnabled {
		if cfg.WorkFile == "" || cfg.WorkFile == "auto" {
			workRoot = findWorkspaceRoot(base.Cwd())
		} else if cfg.WorkFile == "off" {
			workRoot = ""
		}
	} else {
		workRoot = ""
	...
	if workRoot != "" {
		...
		// TODO(matloob): go.work.sum, and also allow modfetch to fall back to individual go.sums
		// TODO(matloob): replaces
		var err error
		modRoots, err = readWorkFile(WorkFilePath())
		if err != nil {
			base.Fatalf("reading go.work: %v", err)
		}
		search.SetModRoots(modRoots) // todo(matloob) make sure in sync with targets
...
// ModRoot returns the roots of the main modules.
...
func WorkFilePath() string {
	...
	return filepath.Join(workRoot, "go.work")
}
...
func readWorkFile(path string) (modRoots []string, err error) {
	workDir := filepath.Dir(path)
	workFile, err := fsys.Open(path)
	if err != nil {
		return nil, err
	}
	workData, err := ioutil.ReadAll(workFile)
	if err != nil {
		return nil, err
	}
...
func CreateModFile(ctx context.Context, modPath string) {
	...
	commitRequirements(ctx, modFileGoVersion(), requirementsFromModFile(ctx, []*modfile.File{modFile}))
...
// the global modFile.
func requirementsFromModFile(ctx context.Context, modfiles []*modfile.File) *Requirements {
	rootCap := 0
...
func findWorkspaceRoot(dir string) (root string) {
	if dir == "" {
		panic("dir not set")
	}
	dir = filepath.Clean(dir)
	...
	// Look for enclosing go.mod.
	for {
		if fi, err := fsys.Stat(filepath.Join(dir, "go.work")); err == nil && !fi.IsDir() {
			return dir
		}
		d := filepath.Dir(dir)
		if d == dir {
...
func commitRequirements(ctx context.Context, goVersion string, rs *Requirements) {
	...
	if MainModules.Size() != 1 || !MainModules.HasModRoot(MainModules.Versions()[0]) {
