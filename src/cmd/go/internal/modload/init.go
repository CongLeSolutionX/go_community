...
var (
	initialized bool
	workRoot    string
	modRoots    []string // These are used to initialize the MainModules. TODO(matloob): move uses of modRoots to mainModules
	gopath      string
)
...
func commitRequirements(ctx context.Context, goVersion string, rs *Requirements) {
	...
	if MainModules.Len() != 1 || !MainModules.HasModRoot(MainModules.Versions()[0]) {
