...
func canUpdateGoMod() bool {
	return cfg.BuildMod != "mod" && !inWorkspaceMode()
}

func canUpdateGoSum() bool {
	return cfg.BuildMod != "readonly" || inWorkspaceMode()
}
...
func commitRequirements(ctx context.Context, goVersion string, rs *Requirements) {
	...
	if inWorkspaceMode() {
		...
		modfetch.WriteGoSum(keepSums(ctx, loaded, rs, addBuildListZipSums), !canUpdateGoSum())
		return
	}
...
func keepSums(ctx context.Context, ld *loader, rs *Requirements, which whichSums) map[module.Version]bool {
				...
				if v, ok := rs.rootSelected(pkg.mod.Path); ok && v == pkg.mod.Version {
					...
					for prefix := pkg.path; prefix != "."; prefix = path.Dir(prefix) {
						if v, ok := rs.rootSelected(prefix); ok && v != "none" {
							...
							keep[resolveReplacement(m).Version] = true
						}
					}
					continue
				}
	...
	if rs.graph.Load() == nil {
		...
		for _, m := range rs.rootModules {
			r := resolveReplacement(m).Version
			keep[modkey(r)] = true
		...
		mg.WalkBreadthFirst(func(m module.Version) {
			if _, ok := mg.RequiredBy(m); ok {
				...
				// actually selected.
				keep[modkey(resolveReplacement(m).Version)] = true
			}
		})
