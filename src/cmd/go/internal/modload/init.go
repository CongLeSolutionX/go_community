...
func (goModDirtyError) Error() string {
	if cfg.BuildModExplicit {
		return fmt.Sprintf("updates to go.mod needed, disabled by -mod=%v", cfg.BuildMod)
	}
...
func LoadModFile(ctx context.Context) *Requirements {
	...
	if modRoot == "" {
		...
		writeGoMod(ctx, newRequirements(nil, nil))
		return requirements
	}
...
func CreateModFile(ctx context.Context, modPath string) {
	...
	writeGoMod(ctx, requirementsFromModFile(ctx, modFile))
...
func WriteGoMod(ctx context.Context) {
	writeGoMod(ctx, LoadModFile(ctx))
}

// writeGoMod writes sets the global requirements variable to rs and writes its
// contents back to the go.mod file on disk.
func writeGoMod(ctx context.Context, rs *Requirements) {
	requirements = rs
