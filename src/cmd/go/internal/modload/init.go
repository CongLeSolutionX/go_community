...
func requirementsFromModFiles(ctx context.Context, workFile *modfile.WorkFile, modFiles []*modfile.File, opts *PackageOpts) *Requirements {
	...
	if inWorkspaceMode() {
		...
		modFile := modFiles[0]
		addToolchainRoot := true
		roots, direct = rootsFromModFile(MainModules.mustGetSingleMainModule(), modFile, addToolchainRoot)
	}
...
func rootsFromModFile(m module.Version, modFile *modfile.File, addToolchainRoot bool) (roots []module.Version, direct map[string]bool) {
	direct = make(map[string]bool)
	roots = make([]module.Version, 0, 2+len(modFile.Require)) // Add padding for the toolchain and go version, added upon return.
