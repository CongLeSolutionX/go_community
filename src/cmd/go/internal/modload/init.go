...
func commitRequirements(ctx context.Context, opts WriteOpts) (err error) {
	...
	origGoMod, formatErr := modFile.Format()
	...
	if opts.TidyDiff {
		...
		d := diff.Diff("old go.mod", origGoMod, "tidy go.mod", tidyGoMod)
		goModChanged := len(d) != 0
		if goModChanged {
			fmt.Println(string(d))
		}
		err = modfetch.WriteGoSum(ctx, keepSums(ctx, loaded, requirements, addBuildListZipSums), modfetch.WriteOpts{ReadOnly: true, TidyDiff: true})
