...
var ErrNoChange = errors.New("No go.mod file. No change.")

// UpdateGoModFromReqs will update the go.mod using the current requirements.
// It does not commit these changes to disk.
func UpdateGoModFromReqs(ctx context.Context, opts WriteOpts) (currentGoMod []byte, updatedGoMod []byte, modFile *modfile.File, err error) {
	...
	return currentGoMod, updatedGoMod, modFile, nil
}
