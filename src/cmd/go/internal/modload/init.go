...
func checkModulePathLax(p string) error {
	...
	if _, _, ok := module.SplitPathVersion(p); !ok {
		if strings.HasPrefix(p, "gopkg.in/") {
			...
			return errorf("module paths beginning with gopkg.in/ must always have a major version suffix in the form of .vN; eg. go mod init %s", correction)
		}
		correction := suggestModulePath(p)
		return errorf("major version suffixes must be in the form of /vN and are only allowed for v2 or later; eg. go mod init %s", correction)
	}
