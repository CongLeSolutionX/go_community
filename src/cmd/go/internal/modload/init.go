...
// vendorDirExists returns the conditions under which the vendor directory exists when
// in mod=vendor mode: when in workspace mode, or when there is a single module with a defined
// modroot.
func vendorDirExists() bool {
	return inWorkspaceMode() || MainModules.ModRoot(MainModules.mustGetSingleMainModule()) != ""
}
