...
func loadFromRoots(ctx context.Context, params loaderParams) *loader {
	...
	for {
		...
		if index.depth() == lazy && cfg.BuildMod == "mod" {
		...
		if changed {
			...
			// stabilized. If the roots are still changing, they may turn out to
			// specify a requirement on the missing package(s), and we would rather
			// use a version specified by a new root than add a new dependency on an
			// unrelated version.
			continue
		}
		...
		// consistent with ld.packages: we don't need to pass ld.packages to
		// updateRoots again. (That would waste time looking for changes that we
...
//
// The "changed" return value reports whether the selected version of any module
// that provided a loaded package has been changed as a result of the update.
