...
func ImportPathsQuiet(ctx context.Context, patterns []string, tags map[string]bool) []*search.Match {
	...
	loaded = loadFromRoots(tags, allLevel, func() (roots []string) {
		updateMatches(nil)
		for _, m := range matches {
...
func ImportFromFiles(ctx context.Context, gofiles []string) {
	...
	loaded = loadFromRoots(tags, noAll, func() (roots []string) {
...
func ReloadBuildList() []module.Version {
	loaded = loadFromRoots(imports.Tags(), noAll, func() (roots []string) { return nil })
	return buildList
}
...
func LoadVendor(ctx context.Context) []string {
	InitMod(ctx)
	return loadAll(ctx, importedByTarget)
}

func loadAll(ctx context.Context, level allLevel) []string {
	inTarget := TargetPackages(ctx, "...")
	loaded = loadFromRoots(imports.AnyTags(), level, func() (roots []string) { return inTarget.Pkgs })
	checkMultiplePaths()
	WriteGoMod()
...
type loader struct {
	...
	allLevel allLevel        // what level of "all" should we expand to?
...
const (
	...
	//
	// Packages marked with pkgInAll must propagate the marking to all of their
	// imports. When the pkgInAll flag and pkgImportsLoaded flags are both set,
	// the caller who set the last of those flags must propagate the pkgInAll
	// marking to all of the imports of the marked package.
	//
	// A test is marked with pkgInAll if that test promotes the packages it
	// imports to be in "all" (such as when the test is itself within the main
	// module, or when the allLevel is importedByTransitiveTestFromTarget.)
	//
	// Note that it is possible for a loaded package NOT to be in "all" even when
	// we are loading the "all" pattern: packages that are transitive dependencies
	// of other roots must be loaded, but are not in "all". The mod_notall test
	// illustrates this behavior.
	//
	// For example, 'go list all golang.org/x/net/idna' when run within
	// golang.org/x/tools would load the dependencies of all of the packages
	// in golang.org/x/tools, *as well as* the dependencies of
	// golang.org/x/net/idna (which is not in "all" within x/tools).
	//
	// Similarly, if LoadTests is set but the allLevel is less than
	// importedByTransitiveTestFromTarget, packages that are imported *only* by
	// tests outside of the main module are not in "all", but must be loaded
	// nonetheless in order to build the requested tests.
	...
	//
	// If the "all" pattern is included as a root (indicated by a nonzero
	// allLevel), then packages marked with pkgInAll are
	pkgIsRoot
...
func loadFromRoots(tags map[string]bool, allLevel allLevel, listRoots func() []string) *loader {
	ld := &loader{
		tags:     tags,
		allLevel: allLevel,
	...
	for {
		...
		// which adds tests (and test dependencies, if indicated by ld.allLevel)
		//
...
func (ld *loader) pkg(path string, flags loadPkgFlags) *loadPkg {
	...
	pkg := ld.pkgCache.Do(path, func() interface{} {
		...
		// Ensure that the initial flags are applied before imports are loaded.
		//
		// When we implement lazy loading, the "all" module pattern will include
		// modules that provide dependencies of tests of packages that are in the
		// "all" package pattern but outside of the main module. However, irrelevant
		// dependencies of those test-dependency modules will be pruned out. That's
		// normally ok because we can rely on the module containing the test itself
		// to include all of the dependencies relevant to the test. (That's the
		// “import invariant” in the lazy loading design doc.)
		//
		// However, if the other requirements of the module providing the test-only
		// import suddenly become relevant to the imports of the main module itself
		// — for example, because something in the main module has been changed to
		// import some *other* package from the previously-test-only module — then
		// we will need to load and incorporate the full dependencies of that
		// module. The pkgInAll flag is what indicates an import from the main
		// module, and if it really is a brand new package dependency then the
		// *only* importers will all be flagged pkgInAll (because we will apply that
		// flag as soon as we notice that the importer is in the main module). So as
		// long as we propagate the pkgInAll flag before loading imports, we can use
		// its presence to trigger the loading of module dependencies.
		//
		// This case is illustrated in the mod_lazy_import_allmod.txt test case.
...
func (ld *loader) applyPkgFlags(pkg *loadPkg, flags loadPkgFlags) {
	...
	if flags.has(pkgInAll) && ld.allLevel > noAll && !pkg.isTest() {
		// The nonzero allLevel indicates that the "all" pattern is an additional
		// root, so this package matches a root pattern by virtue of being in "all".
		flags |= pkgIsRoot
	}
	...
	// loadedWith reports whether we either added cond, or added pkgImportsLoaded
	// with cond already present.
	loadedWith := func(cond loadPkgFlags) bool {
		return new.has(cond) && !old.has(cond|pkgImportsLoaded)
	}
	...
	if !pkg.isTest() {
		...
		wantTest := false
		switch {
		case ld.allLevel >= importedByTarget && !old.has(pkgImportsLoaded) && pkg.mod == Target:
			// We are loading the packages in the "all" pattern, which includes
			// packages transitively imported by tests in the main module. Even if
			// LoadTests is not set, we need to load the test of this package in order
			// to identify its imports.
			//
			// (We will filter out the extra tests explicitly in computePatternAll.)
			wantTest = true
			testFlags |= pkgInAll

		case ld.allLevel >= importedByTransitiveTestFromTarget && loadedWith(pkgInAll):
			// This level of the "all" pattern includes imports of tests of every
			// package that is itself in "all", and pkg is in "all", so we need to
			// identify the imports of its tests (as above).
			wantTest = true
			testFlags |= pkgInAll

		case LoadTests && loadedWith(pkgIsRoot):
			// LoadTest explicitly requests tests of “the root packages”.
			wantTest = true
			if pkg.mod == Target {
				// If pkg is in the main module, then its test is in the "all"
				// pattern, in the sense that the transitive imports of that test are
				// themselves in the "all" pattern.
				//
				// When we implement lazy loading, we will record the modules providing
				// packages in "all" even when we are only loading individual packages,
				// so we should go ahead and mark the test as "in all" regardless of
				// ld.allLevel. (This is the “import invariant” described in
