...
//
// There are several exported entry points into package loading, but they are
// all implemented in terms of loadFromRoots, which itself manipulates an
// instance of the loader struct.
//
...
// requested packages and their transitive imports, then try to resolve any
// imported packages that are still missing.
//
...
// Instead, the root packages for the "all" pattern are those contained in
// the main module, and a separate field of the loader — the allLevel —
// indicates how to expand those roots dynamically during loading.
//
// A separate flag on each individual package (pkgInAll) tracks whether that
// package is known to match the "all" meta-pattern. A package is in "all" if
// 	- it is imported by any package or test in the main module, or
// 	- it is imported by another package in "all", or
// 	- the allLevel is importedByTransitiveTestFromTarget, and the package is
// 	  imported by a *test of* another package in "all".
//
...
//
// Lazy loading will add another way that a package can be loaded but not in
// "all": the "all" pattern will include only packages transitively imported by
// the main module, but when LoadTests is set we will also load the test
// dependencies of those packages. Those test dependencies are not in "all"
// (unless they are *also*, separately, imported by the main module).
//
...
// pkgInAll flag regardless of the allLevel. (This is the “import invariant”
// described in https://golang.org/design/36460-lazy-module-loading.)
//
...
// missing packages available. We add those to the module graph and repeat,
...
const (
	...
	// allLevel), then non-tests packages in "all" are also roots (and must be
	// marked pkgIsRoot).
	pkgIsRoot
...
func (ld *loader) applyPkgFlags(pkg *loadPkg, flags loadPkgFlags) {
	...
	if !pkg.isTest() {
		...
		case ld.allLevel >= importedByTransitiveTestFromTarget && loadedWith(pkgInAll):
			...
			// identify the imports of its test (as above).
			wantTest = true
			testFlags |= pkgInAll
		...
		case LoadTests && loadedWith(pkgIsRoot):
			...
			if pkg.mod == Target {
				...
				// they cause the packages they import to also be in "all".
				testFlags |= pkgInAll
			}
		}
