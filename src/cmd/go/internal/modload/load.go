...
const (
	...
	//
	// Note that it is also possible for a loaded package NOT to be in "all" even
	// when we ARE loading the "all" pattern: packages that are transitive
	// dependencies of other roots must be loaded, but are not in "all".
	// The mod_notall test illustrates this behavior.
	//
	...
	// the caller who set the last of those flags must add a test for the package.
	pkgIsRoot

	// pkgLoaded indicates that the imports and testImports fields of a loadPkg
	// have been populated.
	pkgImportsLoaded
)
...
// update the given flags in af (in addition to any flags already set).
//
...
//
// As loading completes for the package and its (transitive) imports, each
// completed package will be sent on the ld.pkgLoaded channel.
...
func (ld *loader) applyPkgFlags(pkg *loadPkg, flags loadPkgFlags) {
	...
	if flags.has(pkgInAll) && ld.allLevel > noAll && !pkg.isTest() {
		// A nonzero allLevel indicates that the given "all" pattern is included as
		// an additional root.
		flags |= pkgIsRoot
	}
	...
	if new == old || !new.has(pkgImportsLoaded) {
		...
		// its dependencies yet. Either way, we can't usefully loads its test or
	...
	if !pkg.isTest() {
		testFlags, wantTest := func() (loadPkgFlags, bool) {
			if ld.allLevel >= importedByTarget && !old.has(pkgImportsLoaded) && pkg.mod == Target {
				...
				// LoadTests is not set, we need to load this test in order to identify
				// its imports.
				//
				// (We will filter out the extra tests explicitly in computePatternAll.)
				return pkgInAll, true
			}

			if ld.allLevel >= importedByTransitiveTestFromTarget && loadedWith(pkgInAll) {
			...
			if LoadTests && loadedWith(pkgIsRoot) {
				...
				if pkg.mod == Target {
					...
					//
					// When we implement lazy loading, we will record the modules
					// providing packages in "all" even when we are only loading
					// individual packages. (This is the “import invariant” described in
...
func (ld *loader) loadAll(roots []string) {
	inRoots := make(map[*loadPkg]bool, len(roots))
	...
	<-ld.work.Idle()
	// The loader has gone idle, which means that it has finished loading all of
	// the transitive dependencies of its packages.
}
