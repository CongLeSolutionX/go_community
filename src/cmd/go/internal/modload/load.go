...
type loadPkg struct {
	...
	// Populated at construction time and updated by (*loader).updatePkgFlags:
...
const (
	...
	//
	// TODO(bcmills): Ensure that we have a test illustrating the above case.
	...
	// pkgHasTest indicates that the test of the package has been (or is being)
	// loaded.
	pkgHasTest
)
...
func (ld *loader) pkg(path string, flags loadPkgFlags) *loadPkg {
	...
	pkg := ld.pkgCache.Do(path, func() interface{} {
		...
		ld.work.Add(func() { ld.load(pkg, flags) })
		return pkg
	}).(*loadPkg)

	ld.updatePkgFlags(pkg, flags)
	return pkg
}

func (ld *loader) updatePkgFlags(pkg *loadPkg, flags loadPkgFlags) {
	if flags == 0 {
		return
	}
	...
	if loadedWith(pkgInAll) {
		for _, dep := range pkg.imports {
			ld.updatePkgFlags(dep, pkgInAll)
		}
	}
}

// load loads an individual package.
func (ld *loader) load(pkg *loadPkg, pkgFlags loadPkgFlags) {
	...
	ld.updatePkgFlags(pkg, pkgFlags|pkgImportsLoaded)
}
...
func (ld *loader) pkgTest(pkg *loadPkg, testFlags loadPkgFlags) *loadPkg {
	...
	if first {
		...
		importFlags := testFlags & pkgInAll
		...
		pkg.testImports = nil
		testFlags |= pkgImportsLoaded
	}

	ld.updatePkgFlags(test, testFlags)
	return test
}
