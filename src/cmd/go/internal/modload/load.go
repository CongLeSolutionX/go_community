...
func (mms *mainModules) DirImportPath(ctx context.Context, dir string) (path string, m module.Version) {
	...
	for _, v := range mms.Versions() {
		...
		if strings.HasPrefix(dir, modRoot+string(filepath.Separator)) {
			...
			if strings.HasPrefix(suffix, "/vendor/") {
				return strings.TrimPrefix(suffix, "/vendor/"), v
			}
			return mms.PathPrefix(v) + suffix, v
		}
	}
...
func (ld *loader) stdVendor(parentPath, path string) string {
	...
	if str.HasPathPrefix(parentPath, "cmd") {
		if _, ok := MainModules.VersionWithPath("cmd"); !ok || !ld.VendorModulesInGOROOTSrc {
	...
	} else if _, ok := MainModules.VersionWithPath("std"); !ok || !ld.VendorModulesInGOROOTSrc || str.HasPathPrefix(parentPath, "vendor") {
