...
var (
	...
	vendorUnused    []module.Version          // included in vendor/modules.txt for metadata only
		...
		for _, line := range strings.Split(string(data), "\n") {
			if strings.HasPrefix(line, "# ") {
				...
				if semver.IsValid(f[2]) {
					// A module in the build list.
					mod = module.Version{Path: f[1], Version: f[2]}
					if unused {
						vendorUnused = append(vendorUnused, mod)
					} else {
				...
				if len(f) >= 2 && f[0] == "=>" {
					...
					if len(f) == 2 {
						...
						// Not something we understand. Ignore it.
					}
					vendorMeta[mod] = meta
				}
				continue
