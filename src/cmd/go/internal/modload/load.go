				...
				for _, pkg := range pkgs {
					...
					if modRoot != "" && dir == modRoot {
						...
						if strings.HasPrefix(suffix, "/vendor/") {
							...
							// Don't add "std" to packages in the "std" module. It's the one
							// module path that isn't a prefix of the packages it contains.
...
func (r *mvsReqs) required(mod module.Version) ([]module.Version, error) {
	...
	case "std", "cmd":
		...
		// explicitly running a command that can change the module graph.
