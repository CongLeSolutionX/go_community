...
func resolveLocalPackage(ctx context.Context, dir string, rs *Requirements) (string, error) {
	...
	if pkg == "" {
		if inWorkspaceMode() {
			if findModuleRoot(absDir) != "" {
				return "", fmt.Errorf("directory %s is contained in a module that is not one of the workspace modules listed in go.work. You can add the module to the workspace using go work use %s", base.ShortPath(absDir), base.ShortPath(absDir))
			}
