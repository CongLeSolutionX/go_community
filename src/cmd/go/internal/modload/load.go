...
func (ld *loader) loadAll(roots []string) {
	...
	for {
		...
		case <-ld.idle:
			ld.idle <- token{}
			// The loader has gone idle, which means that it has finished loading (and
			// sending on pkgLoaded) all of the transitive dependencies of its
			// packages. No further packages will be sent on ld.pkgLoaded until the
			// next call to ld.pkg, which will remove the token from the idle channel
			// until the loader is once again idle.
			return
		case pkg = <-ld.pkgLoaded:
		}
		pkg.loaded = true

		if LoadTests && (pkg.isRoot || (pkg.inAll && ld.allLevel > noAll)) {
			// LoadTests explicitly requests tests of “the root packages”.
			// If ld.allLevel is nonzero, "all" itself functions as a root.
			ld.loadTestOf(pkg)
		}

		if pkg.mod == Target {
			// "all" is defined as the transitive dependencies of the packages in the
			// main module. The definition of “transitive dependencies” varies with
			// the allLevel, but the definition of “packages in the main module”
			// does not.
			pkg.inAll = true
			...
			if pkg.test != nil {
				// If the test was loaded — regardless of whether due to the "all"
				// pattern or the LoadTests setting — mark its dependencies as in "all".
				//
				...
				// (This is the “import invariant” described in
				// https://golang.org/design/36460-lazy-module-loading.)
				pkg.test.inAll = true
				ld.propagateInAll(pkg.test)
			}
		}

		if pkg.inAll {
			// When pkg was marked inAll, we might not have identified its
			// dependendencies yet, and we might not have loaded its test. Now that we
			// are sure that all relevant dependencies of pkg have been identified, we
			// can mark them inAll as well.
			//
			// Note that it is possible for a package not to be inAll even when
			// ld.allLevel is nonzero: packages that are transitive dependencies of
			// other roots must be loaded, but are not in "all".
			//
