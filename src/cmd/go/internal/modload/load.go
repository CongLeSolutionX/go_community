...
// resolveLocalPackage resolves a filesystem path to a package path and returns the containing module.
...
// as well as the module that contains dir.
func pathInModuleCache(ctx context.Context, dir string, rs *Requirements) (string, module.Version) {
	...
	for _, m := range mg.BuildList() {
		if importPath, found := tryMod(m); found {
			return importPath, m
		}
	}
...
// ModuleGoVersion returns the go version declared in the go.mod file
// of the module that contains the given directory, if such a module exists.
func ModuleGoVersion(ctx context.Context, dir string) (goVersion string) {
	// Load the mod file before searching the directories.
	rs := LoadModFile(ctx)
	if path, m := MainModules.DirImportPath(ctx, dir); path != "." {
		return gover.FromGoMod(MainModules.ModFile(m))
	}
	// Not in the main module, therefore we will search the dependencies.
