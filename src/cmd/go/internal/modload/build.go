...
func moduleInfo(ctx context.Context, rs *Requirements, m module.Version, mode ListMode) *modinfo.ModulePublic {
	...
	completeFromModCache := func(m *modinfo.ModulePublic, replacedFromFile string) {
		...
		if m.GoVersion == "" && checksumOk("/go.mod") {
			...
			if summary, err := rawGoModSummary(mod, replacedFromFile); err == nil && summary.goVersion != "" {
	...
	r, replacedFromFile := Replacement(m)
	if r.Path == "" {
	...
	if r.Version == "" {
		if filepath.IsAbs(r.Path) {
			...
			info.Replace.Dir = filepath.Join(filepath.Dir(replacedFromFile), r.Path)
		}
		info.Replace.GoMod = filepath.Join(info.Replace.Dir, "go.mod")
	}
	if cfg.BuildMod != "vendor" {
		completeFromModCache(info.Replace, replacedFromFile)
