...
func matchPackages(ctx context.Context, m *search.Match, tags map[string]bool, filter stdFilter, modules []module.Version) {
	...
	testAndSetHave := func(p string) bool {
		mu.Lock()
		defer mu.Unlock()
		v := have[p]
		if !v {
	...
	for _, mod := range modules {
		...
		if mi, err := modindex.Get(root); err == nil {
			walkFromIndex(mi, modPrefix, isMatch, treeCanMatch, tags, testAndSetHave, addPkg)
			continue
...
func walkFromIndex(index *modindex.ModuleIndex, importPathRoot string, isMatch, treeCanMatch func(string) bool, tags map[string]bool, testAndSetHave func(string) bool, addPkg func(string)) {
loopPackages:
