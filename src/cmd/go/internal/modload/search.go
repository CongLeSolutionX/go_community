...
loopPackages:
	for _, reldir := range index.Packages() {
		...
		// Avoid .foo, _foo, and testdata subdirectory trees.
		elems := strings.Split(reldir, string(filepath.Separator))
		...
		if importPathRoot == "" {
			name = name[1:] // cut leading slash path.Join will prepend to pkgPath if importPathRoot is empty
		}
		if !treeCanMatch(name) {
			continue
		}
		...
		if !have[name] {
			have[name] = true
			if isMatch(name) {
				if _, _, err := scanDir(modroot, filepath.Join(modroot, reldir), tags); err != imports.ErrNoGo {
					m.Pkgs = append(m.Pkgs, name)
				}
			}
		}
