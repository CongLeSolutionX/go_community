...
func queryProxy(ctx context.Context, proxy, path, query, current string, allowed AllowedFunc) (*modfetch.RevInfo, error) {
	...
	// Look up the repo only after parsing the query: we would rather report a
	// bogus query than a bogus repo.
	...
	if qm.exact {
		...
		if err := qm.allowsVersion(ctx, info.Version); errors.Is(err, ErrDisallowed) {
			return nil, err
		}
		return info, nil
	}
	...
	lookup := func(v string) (*modfetch.RevInfo, error) {
		...
		if current != "" && (query == "upgrade" || query == "patch") && !rev.Time.IsZero() {
	...
	if qm.mayUseLatest {
		...
		if err == nil {
			if err := qm.allowsVersion(ctx, latest.Version); !errors.Is(err, ErrDisallowed) {
...
type queryMatcher struct {
	...
	prefix             string
	exact              bool // if true, only one version can match
	preferLower        bool // if true, choose the
	mayUseLatest       bool
...
func newQueryMatcher(path string, query, current string, allowed AllowedFunc) (*queryMatcher, error) {
	...
	case query == "patch":
		if current == "" {
			...
			qm.filter = func(mv string) bool {
				return matchSemverPrefix(qm.prefix, mv) && semver.Compare(mv, current) >= 0
			}
		}
	...
	case semver.IsValid(query):
		if isSemverPrefix(query) {
			qm.filter = func(mv string) bool { return matchSemverPrefix(query, mv) }
			qm.prefix = query + "."
		} else {
			qm.exact = true
...
func (qm *queryMatcher) allowsVersion(ctx context.Context, v string) error {
	...
	return qm.allowed(ctx, module.Version{Path: qm.path, Version: v})
}
...
func (qm *queryMatcher) filterVersions(ctx context.Context, versions []string) (releases, prereleases []string, err error) {
	...
	for _, v := range versions {
		if err := qm.allowsVersion(ctx, v); errors.Is(err, ErrDisallowed) {
			continue
		}
...
func (er emptyRepo) Versions(prefix string) ([]string, error)   { return nil, er.err }
...
func (rr *replacementRepo) Versions(prefix string) ([]string, error) {
	...
	if index != nil && len(index.replace) > 0 {
		...
		for m, _ := range index.replace {
			if m.Path == path && !modfetch.IsPseudoVersion(m.Version) && strings.HasPrefix(m.Version, prefix) {
...
func (rr *replacementRepo) Stat(rev string) (*modfetch.RevInfo, error) {
	...
	if v != rev {
		...
		// so a non-canonical version can't possible have a replacement.
		return info, err
	}
