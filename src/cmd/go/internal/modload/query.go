...
//
// If the path is in the main module and the query is "latest", QueryPackage
// returns Target as the version, without checking for other possible modules.
...
func QueryPattern(pattern string, query string, allowed func(module.Version) bool) ([]QueryResult, error) {
	...
	for {
		go func(p string, r *result) (err error) {
			...
			if HasModRoot() && p == Target.Path {
				if query != "latest" {
					return fmt.Errorf("can't query specific version (%q) for package %s in the main module (%s)", query, pattern, Target.Path)
				}
				if !allowed(Target) {
					return fmt.Errorf("internal error: package %s is in the main module (%s), but version is not allowed", pattern, Target.Path)
				}
				r.Mod = Target
				...
				r.Mod = module.Version{Path: p, Version: r.Rev.Version}
				root, isLocal, err = fetch(r.Mod)
				if err != nil {
					return err
				}
			}
			pkgs := match(r.Mod, root, isLocal)
			if len(pkgs) == 0 {
	...
	for _, r := range results {
		...
		// Any error is more useful than no error, and an error for any other module
		// is more useful than one for the Target.
		if mostUseful.err == nil || mostUseful.Mod == Target {
			mostUseful = r
			continue
		} else if r.Mod == Target {
			continue
		}
