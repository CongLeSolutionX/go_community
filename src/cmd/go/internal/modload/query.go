...
Lookup:
	...
	if rev != nil && current != "" && (query == "latest" || query == "patch") {
		prevTime, err := modfetch.PseudoVersionTime(current)
		if semver.Compare(rev.Version, current) < 0 || (err == nil && rev.Time.Before(prevTime)) {
...
// modules.
func QueryPackage(path, query, current string, allowed func(module.Version) bool) ([]QueryResult, error) {
	if search.IsMetaPackage(path) || strings.Contains(path, "...") {
		return nil, fmt.Errorf("pattern %s is not an importable package", path)
	}
	return QueryPattern(path, query, current, allowed)
}
...
// possible modules.
func QueryPattern(pattern, query, current string, allowed func(module.Version) bool) ([]QueryResult, error) {
	base := pattern
