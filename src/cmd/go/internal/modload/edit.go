...
func editRequirements(ctx context.Context, rs *Requirements, tryUpgrade, mustSelect []module.Version) (edited *Requirements, changed bool, err error) {
	...
	relevant, changed, err := upgradeRelevantModules(ctx, limiter, rs, tryUpgrade)
	if err != nil {
		return rs, false, err
	}
	...
	if rs.depth == eager {
		// In an eager module, relevant dependencies may either be explicit roots or
		// implicit transitive dependencies. We promote the modules in mustSelect to
		// be explicit requirements.
		...
		roots, err = mvs.Req(Target, rootPaths, &mvsReqs{roots: relevant})
		if err != nil {
			return nil, false, err
		}
	} else {
		// In a lazy module, all relevant dependencies are retained as roots.
		roots = relevant
		if !changed {
...
func raiseLimitsForUpgrades(ctx context.Context, maxVersion map[string]string, depth modDepth, tryUpgrade []module.Version, mustSelect []module.Version) error {
	...
	allow := func(m module.Version) {
		v, ok := maxVersion[m.Path]
		if !ok {
			// m.Path is unrestricted and presumed irrelevant.
			return
		}
...
func upgradeRelevantModules(ctx context.Context, limiter *versionLimiter, rs *Requirements, tryUpgrade []module.Version) (mods []module.Version, changed bool, err error) {
	for _, m := range tryUpgrade {
...
type versionLimiter struct {
	// depth is the depth at which
	depth modDepth

	// max maps each module path to the maximum version that may be selected for
	// that path.
	//
	// Paths with no entry are unrestricted and assumed to be irrelevant.
	// Irrelevant dependencies of lazy modules will not be followed to check for
	// conflicts.
...
//
// max maps each relevant module path to its maximum version; paths that are not
// present in the map are unrestricted and presumed irrelevant.
//
// If depth is lazy, then if a module passed to UpgradeToward or Select is
// itself lazy, its irrelevant dependencies are skipped when scanning
// requirements.
...
//
// If depth is lazy and m itself is lazy, the the dependencies of irrelevant
// (unrestricted) dependencies of m will not be followed.
...
//
// If depth is lazy and m itself is lazy, then the dependencies of irrelevant
// (unrestricted) dependencies of m will not be followed.
