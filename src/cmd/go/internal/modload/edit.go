...
// editBuildList returns an edited version of list such that:
//
...
//
// 	3. Each module version in list is downgraded from its original version only
// 	   to the extent needed to satisfy (1), or upgraded only to the extent
...
func editBuildList(ctx context.Context, initial, tryUpgrade, mustSelect []module.Version) ([]module.Version, error) {
	...
	// upgrade any existing module unnecessarily in the process. So we record the
	// versions selected from the combination of the original build list, the
	// tentative upgrades, and mustSelect together, and those versions place an
	// upper bound on the selected versions in final build list.
