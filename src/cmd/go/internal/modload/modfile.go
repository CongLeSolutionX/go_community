		...
		if index := MainModules.Index(v); index != nil {
			if from, r, ok := replacement(mod, index.replace); ok {
				...
				if foundModRoot != "" && foundFrom != from && found != r {
					...
					return relativizeReplace(found, foundModRoot)
				}
...
func relativizeReplace(version module.Version, modRoot string) module.Version {
	workFilePath := WorkFilePath()
	if workFilePath == "" || filepath.IsAbs(version.Path) || version.Version != "" {
		return version
	}
	abs := filepath.Join(modRoot, version.Path)
	if rel, err := filepath.Rel(workFilePath, abs); err == nil {
		return module.Version{Path: rel, Version: version.Version}
	}
	// If the workFilePath isn't already absolute, make it absolute and try again.
	if !filepath.IsAbs(workFilePath) {
		workFilePathAbs := filepath.Join(base.Cwd(), workFilePath)
		if rel, err := filepath.Rel(workFilePathAbs, abs); err == nil {
			return module.Version{Path: rel, Version: version.Version}
		}
	}
	...
	return module.Version{Path: abs, Version: version.Version}
}
...
func rawGoModData(m module.Version) (name string, data []byte, err error) {
	if m.Version == "" {
		...
		if !filepath.IsAbs(dir) {
			// The directory is relative to the go.work file in
			var relativeTo string
			if workFilePath := WorkFilePath(); workFilePath != "" {
				relativeTo = filepath.Dir(workFilePath)
			} else {
				relativeTo = MainModules.ModRoot(MainModules.mustGetSingleMainModule())
			}
			dir = filepath.Join(relativeTo, dir)
		}
