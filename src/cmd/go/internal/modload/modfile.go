	...
	for _, v := range MainModules.Versions() {
		if index := MainModules.Index(v); index != nil {
			if r, ok := replacement(mod, index); ok {
				// Absolutize the path now becasue once we return the module.Version,
				// the context for which module adds the replace (and thus which
				// directory the path is relative to) is gone.
				if !filepath.IsAbs(r.Path) {
					r.Path = filepath.Join(MainModules.ModRoot(v), r.Path)
				}
	...
	c := rawGoModSummaryCache.Do(m, func() interface{} {
		...
		if m.Version == "" {
			...
			if !filepath.IsAbs(dir) {
				dir = filepath.Join(MainModules.ModRoot(m), dir)
			}
