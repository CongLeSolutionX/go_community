...
// A cachedGraph is a *ModuleGraph, together with any error discovered while
// loading that graph.
type cachedGraph struct {
	mg  *ModuleGraph
	err error // If err is non-nil, mg must also be non-nil (but may be incomplete).
}
...
func (rs *Requirements) initVendor(vendorList []module.Version) {
	rs.graphOnce.Do(func() {
		...
		if go117LazyTODO {
			...
			// module. That allows us to elide the actuall structure of the module
	...
	if !inconsistent {
		...
		for _, m := range final[1:] {
			if v, ok := rs.rootSelected(m.Path); ok && (v == m.Version || rs.direct[m.Path]) {
				// m.Path was formerly a root, so for now we'll assume that it is still
				// relevant. We can only know for sure whether it remains relevant if we
				// actually load the packages in the main module, which we are not doing
				// here.
				explicit = append(explicit, m.Path)
			}
		}
...
func updateRoots(ctx context.Context, direct map[string]bool, pkgs []*loadPkg, rs *Requirements) (*Requirements, error) {
	...
	if rs != nil {
		...
		for _, root := range rs.rootModules {
			...
			if !inExplicit[root.Path] && mg.Selected(root.Path) == root.Version {
				explicit = append(explicit, root.Path)
				inExplicit[root.Path] = true
			}
		}
	} else if pkgs != nil {
	...
	// “Every module version that provides (or may provide) a package directly
	// imported by the main module is included as a root.”
	//
	// If we didn't load every package and test in the main module, this may be
	...
	// or any other module).
	// TODO(#36460): Update the above comment to reflect lazy loading once implemented.
	for _, m := range keep {
	...
	min, err := mvs.Req(Target, explicit, &mvsReqs{buildList: keep})
	if err != nil {
		return rs, err
	}
