...
func tidyLazyRoots(ctx context.Context, direct map[string]bool, pkgs []*loadPkg) (*Requirements, error) {
	...
	//
	// Once that is done, we may still need to add more roots to cover upgraded
...
// 	4. Ever version in add is selected at its given version unless upgraded by
...
func updateLazyRoots(ctx context.Context, direct map[string]bool, rs *Requirements, pkgs []*loadPkg, add []module.Version) (*Requirements, error) {
	...
	for _, pkg := range pkgs {
		...
		case pkg.flags.has(pkgInAll):
			// pkg is transitively imported by a package or test in the main
			// module. We need to promote the module that maintains it to a root:
			// if some other module depends on the main module, and that other
			// module also uses lazy loading, it will expect to find all of our
			// transitive dependencies by reading just our go.mod file, not the
			// go.mod files of everything we depend on.
			//
			// (This is the “import invariant” that makes lazy loading possible.)

		case pkg.flags.has(pkgIsRoot):
			// pkg is a root of the package-import graph. (Generally this means
			// that it matches a command-line argument.) We want future
			// invocations of the 'go' command — such as 'go test' on the same
			// package — to continue to use the same versions of its dependencies
			// that we are using right now. So we need to bring this package's
			// dependencies inside the lazy-loading horizon.
			//
			// Making the module containing this package a root of the module
			// graph does exactly that: if the module containing the package is
			// lazy it should satisfy the import invariant itself, so all of its
			// dependencies should be in its go.mod file, and if the module
			// contain the package is eager then if we make it a root we will load
			// all of its transitive dependencies into the module graph.
			//
			// (This is the “argument invariant” of lazy loading, and is important
			// for reproducibility.)

		default:
			// pkg is a dependency of some other package outside of the main
			// module. As far as we know it's not relevant to the main module (and
			// thus not relevant to consumers of the main module either), and its
			// dependencies should already be in the module graph — included in
			// the dependencies of the package that imported it.
			...
			if go117LazyTODO {
				...
				// perhaps the author forgot to 'git add' their updated go.mod file after
				// adding a new package import. If that happens, ideally we want to detect
				// the missing requirements and fix them up here.
				//
		...
		if _, ok := rs.rootSelected(pkg.mod.Path); !ok {
			roots = append(roots, pkg.mod)
			rootsUpgraded = true
			// The roots slice was initially sorted because rs.rootModules sorted,
	...
	for _, m := range add {
		if _, ok := rs.rootSelected(m.Path); !ok {
			roots = append(roots, m)
			rootsUpgraded = true
			needSort = true
		}
	...
	for {
		...
		for _, m := range rs.rootModules {
			if inRootPaths[m.Path] {
				...
				//
				// Omitting a root that was previously present may *reduce* the selected
				// versions of non-roots, but merely removing a requirement cannot
				// *increase* the selected versions of other roots as a result — so we
				// don't need to mark this change as an upgrade. (This particular change
				// does not invalidate any other roots.)
				continue
			}
		...
		if !rootsUpgraded {
			...
			// already at its selected version.
			break
		}
	}
