	...
	if cfg.BuildMod == "vendor" {
		...
		if HasModRoot() {
			...
			if inVendorDir {
				...
				// Go 1.23 and later require packages to be present in modules.txt to be imported.
				_, ok := vendorPkgModule[path]
				if ok || (gover.Compare(MainModules.GoVersion(), gover.ExplicitImportModulesVersion) < 0) {
