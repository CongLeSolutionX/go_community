	...
	if cfg.BuildMod == "vendor" {
		...
		if HasModRoot() {
			...
			if inVendorDir {
				...
				// 1. vendor/modules.txt does not exist
				// 2. The user manually added directories to the vendor directory.
				// We chose to ignore this import post 1.23 and treat modules.txt as the source of truth.
				pre123 := false
				if gover.Compare(MainModules.GoVersion(), "1.23") < 0 {
					pre123 = true
				}
