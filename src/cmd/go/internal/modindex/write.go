...
// endoceModule may modify the packages slice.
func encodeModule(packages []*rawPackage) ([]byte, error) {
	...
	packagesOffsetPos := make([]int, len(packages))
	for i := range packages {
		packagesOffsetPos[i] = e.Pos()
		e.Int(0)
	}
	for i, p := range packages {
		e.IntAt(e.Pos(), packagesOffsetPos[i])
		encodePackage(e, p)
	}
...
func encodePackage(e *encoder, p *rawPackage) {
	...
	sourceFileOffsetPos := make([]int, len(p.sourceFiles)) // where to place the ith source file's offset
	for i := range p.sourceFiles {
		sourceFileOffsetPos[i] = e.Pos()
		e.Int(0)
	}
	for i, f := range p.sourceFiles {
		e.IntAt(e.Pos(), sourceFileOffsetPos[i])
		encodeFile(e, f)
	}
}
