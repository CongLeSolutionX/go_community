...
func moduleHash(modroot string, isReadOnly bool) (cache.ActionID, error) {
...
// modroot returns the modroot for dir, by path manipulation in the module cache,
// or climbing up parent directories otherwise.
func modroot(dir string) (modroot string, isModCache, ok bool) {
	type result struct {
		modroot        string
		isModCache, ok bool
	}
	r := modrootCache.Do(dir, func() any {
		if str.HasFilePathPrefix(dir, cfg.GOROOT) {
			// TODO(matloob): add a case for stdlib here.
			return result{"", false, false}
		}
		if !str.HasFilePathPrefix(dir, cfg.GOMODCACHE) {
			modroot = findModuleRoot(dir)
			ok = modroot != "" // findModuleRoot returns "" if it couldn't find a module root
			return result{modroot, false, ok}
		}
		dir = str.TrimFilePathPrefix(dir, cfg.GOMODCACHE)
		dir = dir[len(cfg.GOMODCACHE):]
		...
		return result{modroot, true, true}
	}).(result)
	return r.modroot, r.isModCache, r.ok
}

var ErrNotIndexed = errors.New("not in module index")

// Get returns the ModuleIndex for the module containing the directory dir.
...
// is not in a module.
func Get(dir string) (*ModuleIndex, error) {
...
func openIndex(modPath string, isReadOnly bool) (*ModuleIndex, error) {
	type result struct {
		mi  *ModuleIndex
		err error
	}
	r := fcache.Do(modPath, func() any {
		id, err := moduleHash(modPath, isReadOnly)
		if err != nil {
...
func fromBytes(moddir string, data []byte) (mi *ModuleIndex, err error) {
	...
	if len(data) < len(indexVersion) {
		return nil, fmt.Errorf("bad index version string: %q", data)
	}
