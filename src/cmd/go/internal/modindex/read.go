...
func moduleHash(modPath string, isReadOnly bool) cache.ActionID {
	...
	fsys.Walk(modPath, func(path string, info fs.FileInfo, err error) error {
		if err != nil {
			return err
		}
		if info.IsDir() && path != modPath {
...
func (mi *ModuleIndex) indexPackage(path string) *indexPackage {
	...
	if !ok {
		return &indexPackage{error: fmt.Sprintf("cannot find package in:\n\t%s", filepath.Join(mi.modroot, path))}
	}
