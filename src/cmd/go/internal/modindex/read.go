...
var enabled = func() bool {
	...
	ok, _ := strconv.ParseBool(goindex)
	return ok
}()
...
func (mi *ModuleIndex) Import(bctxt build.Context, relpath string, mode build.ImportMode) (p *build.Package, err error) {
	...
	if !inTestdata(relpath) {
		...
		// below.
		if ctxt.GOROOT != "" && mi.modroot != cfg.GOROOTsrc && str.HasFilePathPrefix(mi.modroot, cfg.GOROOTsrc) {
			p.Root = ctxt.GOROOT
			p.Goroot = true
		...
		for _, root := range ctxt.gopath() {
			...
			if rootsrc := filepath.Join(root, "src"); mi.modroot != rootsrc && str.HasFilePathPrefix(mi.modroot, rootsrc) {
				p.Root = root
			}
		}
	}
	if p.Root != "" {
		...
		case "gccgo":
			...
			dir, elem := path.Split(relpath)
			pkga = pkgtargetroot + "/" + dir + "lib" + elem + ".a"
		case "gc":
			pkgtargetroot = "pkg/" + ctxt.GOOS + "_" + ctxt.GOARCH + suffix
			pkga = pkgtargetroot + "/" + relpath + ".a"
		}
