...
func (mi *ModuleIndex) Import(bctxt build.Context, relpath string, mode build.ImportMode) (p *build.Package, err error) {
	...
	if ctxt.GOROOT != "" && str.HasFilePathPrefix(mi.modroot, cfg.GOROOTsrc) && !inTestdata(relpath) {
		...
		// In build.go, p.Root is only set in the non-local-import cale, or in
		...
		// on setting p.Root above, p.Root is only set in the GOROOT case for the
...
func IsStandardPackage(goroot_, compiler, path string) bool {
	...
	return i != len(reldir) && pkgs[i] == reldir
}
