...
func openIndex(modPath string) *ModuleIndex {
...
func Get(dir string) (*ModuleIndex, bool) {
	...
	moddir := cfg.GOMODCACHE + string(filepath.Separator) + dir[:at+1] + rest
	return openIndex(moddir), true
}
...
// fromButes returns a *ModuleIndex given the encoded representation.
func fromBytes(moddir string, data []byte) (mi *ModuleIndex, err error) {
	...
	d := decoder{b: data[len(indexVersion):]}
...
func (mi *ModuleIndex) RawPackage(path string) *RawPackage {
	...
	if !ok {
		return &RawPackage{error: "directory does not exist", path: path}
	}
	rp := new(RawPackage)
