...
//
// Cmd is a marker interface, and may be embedded in external
// implementations. A concrete Cmd implementation must also implement
// exactly one of SimpleCommand or BackgroundCommand.
type Cmd interface {
...
// (which can changed based on its registration).
