...
func startCacheProg(progAndArgs string, fuzzDirCache Cache) Cache {
	...
	// TODO(bradfitz): do better (any) shell quote parsing? For now this permits
	// basic use cases. For anything else people can write a wrapper script.
...
func (c *ProgCache) Close() error {
	...
	ctx := context.Background() // TODO(bradfitz): change Cache interface to pass one? add timeout?
	_, err := c.send(ctx, &ProgRequest{Command: cmdClose})
	return err
}
