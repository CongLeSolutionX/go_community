...
func (c *Cache) Get(id ActionID) (OutputID, int64, error) {
	...
	if entry[0] != 'v' || entry[1] != '1' || entry[2] != ' ' || entry[3+64] != ' ' || entry[3+64+1+64] != ' ' || entry[entrySize-1] != '\n' {
		return missing()
	}
	eid, eout, esize := entry[3:3+64], entry[3+64+1:3+64+1+64], entry[3+64+1+64+1:entrySize-1]
	var buf [32]byte
	if _, err := hex.Decode(buf[:], eid); err != nil || !bytes.Equal(buf[:], id[:]) {
		return missing()
	}
...
func (c *Cache) copyFile(file io.ReadSeeker, out OutputID, size int64) error {
	...
	if err == nil && info.Size() == size {
		// File exists and is correct size, assume OK.
		return nil
	}
	...
	info, err = f.Stat()
	if err == nil && info.Size() == size {
		// File now exists and is correct size, assume OK (don't rehash it).
		return nil
	}
