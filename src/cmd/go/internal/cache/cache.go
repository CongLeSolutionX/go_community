...
// Cache is the interface as used by the cmd/go. This was originally the interface
// for *DiskCache (then called just *Cache) but then it was defined as an interface
// so *ProgCache could also implement it.
//
// TODO(bradfitz): this isn't an ideal Go interface. Maybe we can simplify it.
// In particular, all the Get and Put variants can probably merge together.
type Cache interface {
	// Get, GetFile, GetBytes, and GetMmap returns the cache entry for the provided ActionID,
	// in different forms.
	//
	// On miss, the error type should be of type *entryNotFoundError.
	Get(ActionID) (Entry, error)
	GetFile(ActionID) (file string, _ Entry, _ error)
	GetBytes(ActionID) ([]byte, Entry, error)
	GetMmap(ActionID) ([]byte, Entry, error)

	Put(ActionID, io.ReadSeeker) (OutputID, int64, error)
	PutBytes(ActionID, []byte) error
	PutNoVerify(ActionID, io.ReadSeeker) (OutputID, int64, error)

	// Trim does some clean-up at the end of a build.
	// TODO(bradfitz): is that right?
	Trim() error

	// OutputFile returns the path on disk where OutputID would be stored
	// if it existed. It need not exist.
	//
	// TODO(bradfitz): this is only used in three cases where the Get already
	// succeeded. Perhaps when the four Get methods get cleaned up one can
	// return the filename and contents. Maybe mmap can be a bool.
...
func (c *DiskCache) GetFile(id ActionID) (file string, entry Entry, err error) {
	entry, err = c.Get(id)
	if err != nil {
		return "", Entry{}, err
	}
...
func (c *DiskCache) GetBytes(id ActionID) ([]byte, Entry, error) {
	entry, err := c.Get(id)
	if err != nil {
...
func (c *DiskCache) GetMmap(id ActionID) ([]byte, Entry, error) {
	entry, err := c.Get(id)
	if err != nil {
		return nil, entry, err
	}
...
func (c *DiskCache) PutNoVerify(id ActionID, file io.ReadSeeker) (OutputID, int64, error) {
...
func (c *DiskCache) PutBytes(id ActionID, data []byte) error {
