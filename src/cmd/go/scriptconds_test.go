...
func scriptConditions() map[string]script.Cond {
	...
	add("asan", sysCondition("-asan", sys.ASanSupported))
	...
	add("msan", sysCondition("-msan", sys.MSanSupported))
	add("net", lazyBool("testenv.HasExternalNetwork()", testenv.HasExternalNetwork))
	add("race", sysCondition("-race", sys.RaceDetectorSupported))
...
func hasBuildmode(s *script.State, mode string) (bool, error) {
	GOOS, _ := s.LookupEnv("GOOS")
	GOARCH, _ := s.LookupEnv("GOARCH")
	return sys.BuildModeSupported(runtime.Compiler, mode, GOOS, GOARCH), nil
}
