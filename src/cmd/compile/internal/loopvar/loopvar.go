...
// note that LoopVarHash != nil sets base.Debug.LoopVar to 0.
//
...
// base.Debug.LoopVar == -2 => do not transform, but log opportunities. // TODO is this worth keeping?
//

func ForCapture(fn *ir.Func) []*ir.Name {
	if base.Debug.LoopVar == 0 && base.LoopVarHash == nil || base.Debug.LoopVar == -1 {
		return nil
	}
	seq := 1
	...
	maybeReplaceVar := func(k ir.Node, x *ir.RangeStmt) ir.Node {
		if n, ok := k.(*ir.Name); ok && possiblyLeaked[n] {
			...
			if base.Debug.LoopVar == -2 {
				...
				base.WarnfAt(x.Pos(), "maybe %v loop variable leak %v", x.Op(), k)
			}

			if base.Debug.LoopVar > 0 ||
				base.LoopVarHash != nil &&
					base.LoopVarHash.DebugHashMatchPos(base.Ctxt, k.Pos()) {
	...
	//  of iteration variables and the transformation is more involved.
	...
	scanChildrenThenTransform = func(n ir.Node) bool {
		...
		case *ir.ForStmt:
			...
			if len(leaked) > 0 {
				...
				if base.Debug.LoopVar > 0 ||
					base.LoopVarHash != nil &&
						base.LoopVarHash.DebugHashMatchPos(base.Ctxt, x.Pos()) {
