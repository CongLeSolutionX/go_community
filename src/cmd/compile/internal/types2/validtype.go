...
func (check *Checker) validType0(typ Type, env *tparamEnv, path []Object) typeInfo {
	...
	case *Named:
		...
		case marked:
			// We have seen this type before and thus must have a cycle.
			// A cycle through imported types is only possible through
			// generic types, which must be instantiated for use. If we
			// have an instantiated type, the instance and corresponding
			// type object are created in the local package. Thus we should
			// never see type objects that are external.
			...
			for i, tn := range path {
				if tn == t.obj {
					...
					check.infoMap[t] = invalid
					t.underlying = Typ[Invalid] // types in cycles are always in the local package
					return invalid
				}
			}
