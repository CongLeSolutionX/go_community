	...
	if got < want {
		...
		if tsig != nil && sig.tparams != nil && tsig.params.Len() == sig.params.Len() && tsig.results.Len() == sig.results.Len() {
			...
			if !check.allowVersion(check.pkg, pos, 1, 21) {
				if inst != nil {
					check.versionErrorf(posn, "go1.21", "partially instantiated function in assignment")
				} else {
					check.versionErrorf(posn, "go1.21", "inplicitly instantiated function in assignment")
				}
			}
			n := tsig.params.Len()
			m := tsig.results.Len()
...
func (check *Checker) arguments(call *syntax.CallExpr, sig *Signature, targs []Type, args []*operand, xlist []syntax.Expr) (rsig *Signature) {
	...
	if len(genericArgs) > 0 && !check.allowVersion(check.pkg, call.Pos(), 1, 21) {
		...
		check.versionErrorf(args[genericArgs[0]].Pos(), "go1.21", "inplicitly instantiated function as argument")
	}
