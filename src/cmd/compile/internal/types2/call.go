...
func (check *Checker) callExpr(x *operand, call *syntax.CallExpr) exprKind {
	...
	// evaluate arguments
	args := check.exprList(call.ArgList, true)
...
func (check *Checker) exprList(elist []syntax.Expr, genericOk bool) (xlist []*operand) {
	switch len(elist) {
	case 0:
		// nothing to do
	case 1:
...
func (check *Checker) arguments(call *syntax.CallExpr, sig *Signature, targs []Type, args []*operand, xlist []syntax.Expr) (rsig *Signature) {
	...
	//check.dump("sig = %s (%d type params)", sig, sig.TypeParams().Len())
	if allowGenericArgs {
		// collect all type parameters
		tparams := sig.TypeParams().list()
		...
		for i, arg := range args {
			...
			if asig, _ := arg.typ.(*Signature); asig != nil && asig.TypeParams().Len() > 0 {
				//check.dump("%d. asig = %s (%d type params)", i, asig, asig.TypeParams().Len())
