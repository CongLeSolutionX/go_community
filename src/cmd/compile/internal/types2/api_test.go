...
func typecheck(path, source string, info *Info) (*Package, error) {
	f, err := parse(path, source)
...
func mustTypecheck(path, source string, info *Info) *Package {
	pkg, err := typecheck(path, source, info)
	if err != nil {
		panic(err) // so we don't need to pass *testing.T
	}
	return pkg
...
func TestCompositeLitTypes(t *testing.T) {
	for _, test := range []struct {
		...
		f, err := parse(test.lit, "package p; var _ = "+test.lit)
		if err != nil {
