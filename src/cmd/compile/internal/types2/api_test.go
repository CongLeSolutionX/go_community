...
func F[T *U, U any](param1, param2 int) /*param1=undef*/ (res1 /*res1=undef*/, res2 int) /*param1=var:12*/ /*res1=var:12*/ /*U=typeparam:12*/ {
...
`
	...
	syntax.CommentsDo(strings.NewReader(mainSrc), func(line, col uint, text string) {
		...
		t.Error(line, col, text, inner) // FIXME
		got := "undef"
	...
	for id, wantObj := range info.Uses {
		...
		// TODO(adonovan): while fixing #64292 I noticed that
		...
		if gotObj != wantObj {
			t.Errorf("%v: LookupParent(%s) got %v, want %v",
				id.Pos(), id.Value, gotObj, wantObj)
			continue
		}
	}
}
