...
// exported API call, i.e., when all methods have been type-checked. In
// that case only, cause may be nil, too.
func (x *operand) convertibleTo(check *Checker, T Type, cause *string) bool {
	...
	case Vp != nil && Tp != nil:
		return Vp.is(func(V *term) bool {
			return Tp.is(func(T *term) bool {
				if !x.convertibleToImpl(check, V.typ, T.typ, cause) {
	...
	case Vp != nil:
		return Vp.is(func(V *term) bool {
			if !x.convertibleToImpl(check, V.typ, T, cause) {
	...
	case Tp != nil:
		return Tp.is(func(T *term) bool {
			if !x.convertibleToImpl(check, x.typ, T.typ, cause) {
	...
	// non-generic case
	return x.convertibleToImpl(check, x.typ, T, cause)
}

// convertibleToImpl should only be called by convertibleTo
func (x *operand) convertibleToImpl(check *Checker, V, T Type, cause *string) bool {
