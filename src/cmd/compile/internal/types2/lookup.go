...
func LookupFieldOrMethod(T Type, addressable bool, pkg *Package, name string) (obj Object, index []int, indirect bool) {
	...
	if t := asNamed(T); t != nil {
		if p, _ := t.Underlying().(*Pointer); p != nil {
			obj, index, indirect = rawLookupFieldOrMethod(p, false, pkg, name)
			if _, ok := obj.(*Func); ok {
	...
	return rawLookupFieldOrMethod(T, addressable, pkg, name)
}
...
func rawLookupFieldOrMethod(T Type, addressable bool, pkg *Package, name string) (obj Object, index []int, indirect bool) {
...
func (check *Checker) missingMethod(V Type, T *Interface, static bool) (method, wrongType *Func) {
	...
	for _, m := range T.typeSet().methods {
		...
		obj, _, _ := rawLookupFieldOrMethod(V, false, m.pkg, m.name)

		// Check if *V implements this method of T.
		if obj == nil {
			ptr := NewPointer(V)
			obj, _, _ = rawLookupFieldOrMethod(ptr, false, m.pkg, m.name)
			if obj != nil {
				return m, obj.(*Func)
			}
		}
