...
// Conparable is like _IsComparable except that non-type parameter interfaces are always comparable.
func Comparable(T Type) bool {
	return isComparable(T, true, nil, nil)
}

// _IsComparable reports whether values of type T are comparable.
// Interfaces are comparable if all types in their type set are comparable.
// TODO(gri) decide if this should be exported for 1.18
func _IsComparable(T Type) bool {
	return isComparable(T, false, nil, nil)
}

// If dynamic is set, non-type parameter interfaces are always comparable.
// If reportf != nil, it may be used to report why T is not comparable.
func isComparable(T Type, dynamic bool, seen map[Type]bool, reportf func(string, ...interface{})) bool {
	if seen[T] {
		return true
	}
	if seen == nil {
	...
	case *Struct:
		for _, f := range t.fields {
			if !isComparable(f.typ, dynamic, seen, nil) {
				if reportf != nil {
		...
		return true
	case *Array:
		if !isComparable(t.elem, dynamic, seen, nil) {
			if reportf != nil {
