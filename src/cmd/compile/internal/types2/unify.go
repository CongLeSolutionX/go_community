...
type tparamsList struct {
	unifier *unifier
	tparams []*TypeParam
	// For each tparams element, there is a corresponding mask bit in maskeds.
...
func (d *tparamsList) init(tparams []*TypeParam) {
	...
	d.tparams = tparams
	d.maskeds = make([]bool, len(tparams))
	d.indices = make([]int, len(tparams))
}

// mask and unmask permit the masking/unmasking of the i'th type parameter of d.
func (d *tparamsList) mask(i int)   { d.maskeds[i] = true }
func (d *tparamsList) unmask(i int) { d.maskeds[i] = false }
...
func (d *tparamsList) index(typ Type) int {
	if tpar, ok := typ.(*TypeParam); ok {
		if i := tparamIndex(d.tparams, tpar); i >= 0 && !d.maskeds[i] {
			return i
		}
	}
	return -1
