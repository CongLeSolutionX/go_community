...
// The Qualifier controls the printing of
// package-level objects, and may be nil.
func WriteSignature(sb *strings.Builder, sig *Signature, qf Qualifier) {
	newTypeWriter(sb, qf).signature(sig)
}
...
func (w *typeWriter) typeSet(s *_TypeSet) {
	...
	default:
		...
		for _, term := range s.terms {
			...
			newTypeHasher(&sb, w.ctxt).typ(term.typ)
			termHashes = append(termHashes, sb.String())
		}
