...
const UseInterface2 = false // exported so tests can use it
...
func (t *Interface2) isSatisfiedBy(typ Type) bool {
	types := t.flat().types
	if len(types) == 0 {
		return true
	}
...
func (t *Interface2) IsComparable() bool {
	_, m := t.lookupMethod(nil, "==")
	return m != nil
}

func (t *Interface2) IsConstraint() bool {
	return t.IsComparable() || len(t.flat().types) > 0
}

// is reports whether interface t represents types that all satisfy pred.
func (t *Interface2) is(pred func(Type) bool) bool {
	types := t.flat().types
	if len(types) == 0 {
...
func (t *Interface2) lookupMethod(pkg *Package, name string) (int, *Func) {
	return lookupMethod(t.flat().methods, pkg, name)
}
