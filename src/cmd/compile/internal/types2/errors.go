...
func (err *error_) report() {
	...
	if multiError {
		for i := range err.desc {
			p := &err.desc[i]
			check.callErrorHandler(i, p.pos, err.code, p.msg, err.soft)
		}
	} else {
		check.callErrorHandler(0, err.pos(), err.code, err.msg(), err.soft)
	}
}

func (check *Checker) callErrorHandler(index int, pos syntax.Pos, code Code, msg string, soft bool) {
	assert(code != 0)

	if check.conf.Trace {
		check.trace(pos.Pos(), "ERROR: %s (code = %d)", msg, code)
	}
	...
	if index == 0 {
		...
		if index == 0 && check.conf.ErrorURL != "" {
