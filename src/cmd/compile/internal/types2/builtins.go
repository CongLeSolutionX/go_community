...
// varSize reports if the size of a type is variable because due to type parameters.
func varSize(t Type) bool {
	switch t := under(t).(type) {
	case *Array:
		return varSize(t.elem)
	case *Struct:
	...
	case *Named:
		return varSize(t.underlying)
	case *TypeParam:
		return true
	case *Union, *instance, *top:
		unreachable()
	}
	return false
}
