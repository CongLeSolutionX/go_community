...
func (check *Checker) builtin(x *operand, call *syntax.CallExpr, id builtinId) (_ bool) {
	...
	case _Add:
		...
		if !check.allowVersionf(check.pkg, call.Pos(), 1, 17, "unsafe.Add") {
			return
		}
	...
	case _Slice:
		...
		if !check.allowVersionf(check.pkg, call.Pos(), 1, 17, "unsafe.Slice") {
			return
		}
	...
	case _SliceData:
		...
		if !check.allowVersionf(check.pkg, call.Pos(), 1, 20, "unsafe.SliceData") {
			return
		}
	...
	case _String:
		...
		if !check.allowVersionf(check.pkg, call.Pos(), 1, 20, "unsafe.String") {
			return
		}
	...
	case _StringData:
		...
		if !check.allowVersionf(check.pkg, call.Pos(), 1, 20, "unsafe.StringData") {
			return
		}
