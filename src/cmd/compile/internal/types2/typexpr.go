...
func (check *Checker) typInternal(e0 syntax.Expr, def *Named) (T Type) {
	...
	case *syntax.ChanType:
		...
		case syntax.RecvOnly:
			dir = RecvOnly
		default:
			check.invalidASTf(e.Pos(), "unknown channel direction %d", e.Dir)
			// ok to continue
		}
...
func (check *Checker) collectParams(scope *Scope, list []*syntax.Field, type0 syntax.Expr, variadicOk bool) (params []*Var, variadic bool) {
	...
	for i, field := range list {
		...
		if field.Name != nil {
			...
			if name == "" {
				check.invalidASTf(field.Name.Pos(), "anonymous parameter")
				// ok to continue
			}
	...
	if named && anonymous {
		check.invalidASTf(list[0].Pos(), "list contains both named and anonymous parameters")
		// ok to continue
	}
...
func (check *Checker) interfaceType(ityp *Interface, iface *syntax.InterfaceType, def *Named) {
	...
	for _, f := range iface.MethodList {
		if f.Name != nil {
			...
			if sig == nil {
				if typ != Typ[Invalid] {
					check.invalidASTf(f.Type.Pos(), "%s is not a method signature", typ)
				}
				continue // ignore
			}
...
func (check *Checker) tag(t *syntax.BasicLit) string {
	if t != nil {
		...
		check.invalidASTf(t.Pos(), "incorrect tag syntax: %q", t.Value)
	}
	return ""
}
