...
func (x *operand) assignableTo(check *Checker, T Type, reason *string) (bool, errorCode) {
		...
		if m, wrongType := check.missingMethod(V, Ti, true); m != nil /* !Implements(V, Ti) */ {
			if reason != nil {
				if check.conf.CompilerErrorMessages {
					*reason = check.missingMethodReason(check.sprintf("%s does not implement %s", x.typ, T),
						x.typ, T, m, wrongType)
				} else {
					// Compatibilty code for types2-only messages
					if wrongType != nil {
	...
	// Provide extra info in compiler in some cases that T is not an interface.
	if check != nil && check.conf.CompilerErrorMessages {
		if isPtrToInterface(Tu) {
