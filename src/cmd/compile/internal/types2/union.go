...
// A Term's type may be an interface, in contrast to a term's type
// which must never be an interface.
type Term struct {
	typ   Type
	tilde bool
	_     bool // dummy field to prevent accidental conversion to term
}

// NewTerm returns a new union term.
func NewTerm(tilde bool, typ Type) *Term {
	assert(typ != nil)
	return &Term{typ, tilde, false}
}
