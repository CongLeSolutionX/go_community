...
func (check *Checker) typeDecl(obj *TypeName, tdecl *syntax.TypeDecl, def *Named) {
	...
	// Note that we don't need this restriction anymore if we switch to tparamIsIface:
	// in that case, if the RHS is a lone type parameter, we will use its underlying
	// type (like we do for any RHS in a type declaration), and its underlying type is
	// an interface and the type declaration is well defined.
	// This code is only here for compatibility.
	if isTypeParam(rhs) {
