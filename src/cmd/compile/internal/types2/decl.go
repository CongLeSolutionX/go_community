...
func (check *Checker) typeDecl(obj *TypeName, tdecl *syntax.TypeDecl, def *Named) {
	...
	check.later(func() {
		...
		if check.isImportedConstraint(rhs) {
			check.allowVersionf(check.pkg, tdecl.Pos(), 1, 18, "using type constraint %s", rhs)
		}
	...
	// alias declaration
	if alias {
		check.allowVersionf(check.pkg, tdecl.Pos(), 1, 9, "type aliases")
