...
func (check *Checker) typeDecl(obj *TypeName, tdecl *syntax.TypeDecl, def *Named) {
	...
	// If the RHS is a type parameter, it must from this type declaration.
	if tpar, _ := named.underlying.(*TypeParam); tpar != nil && tparamIndex(named.tparams, tpar) < 0 {
		// only report an error if we don't have another one already
		if ok {
			check.errorf(tdecl.Type, "type parameter %s must be declared by this type declaration", tpar)
		}
