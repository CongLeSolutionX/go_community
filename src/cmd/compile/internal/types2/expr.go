...
// arbitrarily large. The operator op and position opPos are used
// for an improved error message if possible.
// TODO(gri) remove dependency on opPos since we now have better
//           logic for determining the right position in the error
//           reporting code.
func (check *Checker) overflow(x *operand, op syntax.Operator, opPos syntax.Pos) {
	...
	if isTyped(x.typ) {
		// TODO(gri) Here we don't use opPos because
		//           we can't with the existing API.
		check.representable(x, x.typ.Basic())
		return
	}

	// Untyped integer values must not grow arbitrarily.
	if x.val.Kind() == constant.Int && constant.BitLen(x.val) >= 2048 {
		check.errorf(opPos, "constant %soverflow", op2str[op]) // %s argument is "" or ends with a space
...
var op2str = [...]string{
	...
	syntax.AndNot: "bitwise complement ",
	syntax.Shl:    "shift ",
}
...
func (check *Checker) shift(x, y *operand, op syntax.Operator, opPos syntax.Pos) {
...
func (check *Checker) binary(x *operand, lhs, rhs syntax.Expr, op syntax.Operator, opPos syntax.Pos) {
	var y operand
...
func (check *Checker) exprInternal(x *operand, e syntax.Expr, hint Type) exprKind {
	...
	case *syntax.Operation:
		...
		// binary expression
		check.binary(x, e.X, e.Y, e.Op, e.Y.Pos()) // TODO(gri) should have OpPos here (like in go/types)
		if x.mode == invalid {
			goto Error
		}
