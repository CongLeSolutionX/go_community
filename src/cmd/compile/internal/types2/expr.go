...
func (check *Checker) overflow(x *operand) {
	...
	if op, _ := x.expr.(*syntax.Operation); op != nil {
		...
		if int(op.Op) < len(op2str) {
			what = op2str[op.Op] // is "" or ends with a space
		}
	}
	...
	if x.val.Kind() == constant.Int && constant.BitLen(x.val) > limit {
		check.errorf(pos, "constant %soverflow", what)
...
// (when invoked for an assignment operation where we binary expression is implicit).
