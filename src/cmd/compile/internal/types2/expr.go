...
func (check *Checker) typeAssertion(e syntax.Expr, x *operand, xtyp *Interface, T Type, typeSwitch bool) {
	...
	if typeSwitch {
		...
		msg = check.missingMethodReason(check.sprintf("%s cannot have dynamic type %s", x, T),
			T, x.typ, method, wrongType)

	} else {
		err.errorf(e.Pos(), "impossible type assertion: %s", e)
		msg = check.missingMethodReason(check.sprintf("%s does not implement %s", T, x.typ),
			T, x.typ, method, wrongType)
