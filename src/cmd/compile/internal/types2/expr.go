...
func (check *Checker) exprInternal(T *target, x *operand, e syntax.Expr, hint Type) exprKind {
	...
	case *syntax.CompositeLit:
		...
		case *Interface:
			...
			if len(methods) == 0 && len(e.ElemList) > 0 {
				check.errorf(syntax.StartPos(e.ElemList[0]), InvalidLit, "non-empty interface literal for empty interface %s", utyp)
			...
			for _, elem := range e.ElemList {
				...
				var key syntax.Expr
				val := elem
				if kv, _ := elem.(*syntax.KeyValueExpr); kv != nil {
					key = kv.Key
					val = kv.Value
				}
				// check method name
				...
				} else if mname, _ := key.(*syntax.Name); mname != nil {
					var i int
					if i, meth = methodIndex(methods, check.pkg, mname.Value, false); i < 0 {
						check.errorf(mname, InvalidLit /* MissingMethod */, "method %s not found in interface %s", mname.Value, base)
						continue
					}
					// 0 <= i < len(methods)
					if visited[i] {
						check.errorf(mname, DuplicateLitKey, "duplicate method name %s in interface literal", mname.Value)
						continue
					}
					visited[i] = true
				} else {
