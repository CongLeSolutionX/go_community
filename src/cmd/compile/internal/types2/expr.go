...
func (check *Checker) shift(x, y *operand, e syntax.Expr, op syntax.Operator) {
	...
	if y.mode == constant_ {
		...
		if yval.Kind() == constant.Int && constant.Sign(yval) < 0 {
			check.errorf(x, invalidOp+"negative shift count %s", y)
			x.mode = invalid
			return
		}
		...
		case allInteger(y.typ):
			if !allUnsigned(y.typ) && !check.allowVersion(check.pkg, 1, 13) {
				check.errorf(x, invalidOp+"signed shift count %s requires go1.13 or later", y)
				x.mode = invalid
				return
			}
		...
		default:
			check.errorf(x, invalidOp+"shift count %s must be integer", y)
			x.mode = invalid
			return
		}
	}
	...
	if x.mode == constant_ {
		if y.mode == constant_ {
			...
			if !ok || s > shiftBound {
				check.errorf(x, invalidOp+"invalid shift count %s", y)
				x.mode = invalid
				return
			}
