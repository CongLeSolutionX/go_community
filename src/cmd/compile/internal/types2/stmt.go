...
func (check *Checker) stmt(ctxt stmtContext, s syntax.Stmt) {
	...
	case *syntax.SendStmt:
		...
		tch := ch.typ.Chan()
		if tch == nil {
			check.invalidOpf(s.Pos(), "cannot send to non-chan type %s", ch.typ)
			return
		}

		if tch.dir == RecvOnly {
			check.invalidOpf(s.Pos(), "cannot send to receive-only type %s", tch)
			return
		}
	...
	case *syntax.AssignStmt:
		...
		if s.Op == 0 || s.Op == syntax.Def {
			...
			if len(lhs) == 0 {
				check.invalidASTf(s.Pos(), "missing lhs in assignment")
				return
			}
			if s.Op == syntax.Def {
...
func (check *Checker) switchStmt(inner stmtContext, s *syntax.SwitchStmt) {
	...
	for i, clause := range s.Body {
		if clause == nil {
			check.invalidASTf(clause.Pos(), "incorrect expression switch case")
			continue
		}
...
func (check *Checker) typeSwitchStmt(inner stmtContext, s *syntax.SwitchStmt, guard *syntax.TypeSwitchGuard) {
	...
	for _, clause := range s.Body {
		if clause == nil {
			check.invalidASTf(s.Pos(), "incorrect type switch case")
			continue
		}
...
func (check *Checker) rangeStmt(inner stmtContext, s *syntax.ForStmt, rclause *syntax.RangeClause) {
	...
	if p, _ := sKey.(*syntax.ListExpr); p != nil {
		if len(p.ElemList) != 2 {
			check.invalidASTf(s.Pos(), "invalid lhs in range clause")
			return
		}
		sKey = p.ElemList[0]
		sValue = p.ElemList[1]
