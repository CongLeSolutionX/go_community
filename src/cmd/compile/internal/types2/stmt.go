...
func (check *Checker) rangeStmt(inner stmtContext, s *syntax.ForStmt, rclause *syntax.RangeClause) {
	...
	rclause = nil
	_ = rclause
	// Everything from here on is shared with cmd/compile/internal/types2.
	...
	if x.mode != invalid {
		...
		k, v, cause, ok := rangeKeyVal(x.typ)
		switch {
		case !ok && cause != "":
...
func rangeKeyVal(typ Type) (key, val Type, cause string, ok bool) {
	bad := func(cause string) (key, val Type, _ string, ok bool) {
		return Typ[Invalid], Typ[Invalid], cause, false
	}
	toSig := func(t Type) *Signature {
		sig, _ := under(t).(*Signature)
		return sig
	}
	...
	case *Signature:
		...
		switch {
		case typ.Recv() != nil:
			return bad("func has receiver")
		case typ.Params().Len() != 1 || toSig(typ.Params().At(0).Type()) == nil:
			return bad("func must have one func argument")
		case typ.Results().Len() != 1 || !isBoolean(typ.Results().At(0).Type()):
			return bad("func does not return bool")
		}
		cb := toSig(typ.Params().At(0).Type())
		switch {
		case cb.Recv() != nil:
			return bad("callback func has receiver")
		case cb.Params().Len() > 2:
			return bad("callback has too many arguments")
		case cb.Results().Len() != 1 || !isBoolean(cb.Results().At(0).Type()):
			return bad("callback does not return bool")
		}
		if cb.Params().Len() >= 1 {
