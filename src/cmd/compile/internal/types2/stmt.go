...
func (check *Checker) suspendedCall(keyword string, call syntax.Expr) {
	...
	var x operand
	var msg string
	switch check.rawExpr(&x, call, nil, nil, false) {
	case conversion:
...
L:
	for _, e := range types {
		...
		if check.isNil(e) {
			T = nil
			check.expr(&dummy, e, nil) // run e through expr so we get the usual Info recordings
		} else {
			T = check.varType(e)
			if T == Typ[Invalid] {
				continue L
...
func (check *Checker) stmt(ctxt stmtContext, s syntax.Stmt) {
	...
	case *syntax.ExprStmt:
		...
		var x operand
		kind := check.rawExpr(&x, s.X, nil, nil, false)
		var msg string
		var code Code
		switch x.mode {
		default:
	...
	case *syntax.SendStmt:
		var ch, val operand
		check.expr(&ch, s.Chan, nil)
		check.expr(&val, s.Value, nil)
