...
type TypeSet struct {
	...
	terms   []*term // type terms of the type set; nil means ⊤ (no type constraints)
}

// IsTop reports whether type set s is the top type set (corresponding to the empty interface).
func (s *TypeSet) IsTop() bool { return !s.comparable && len(s.methods) == 0 && len(s.terms) == 0 }

// IsMethodSet reports whether the type set s is described by a single set of methods.
func (s *TypeSet) IsMethodSet() bool { return !s.comparable && len(s.terms) == 0 }
...
func (s *TypeSet) String() string {
	...
	if s.comparable {
		...
		if len(s.methods) > 0 || len(s.terms) != 0 {
			buf.WriteByte(';')
		}
	}
	...
	if len(s.terms) != 0 {
		buf.WriteByte(' ')
		// TODO(gri) consider misusing a Union and print that one instead?
		first := true
		s.is(func(t *term) bool {
			if !first {
				buf.WriteByte('|')
			}
			first = false
			...
			writeType(&buf, t.typ, nil, nil)
			return true
		})
	}

	buf.WriteString(" }") // there was a least one method or term
	return buf.String()
}
...
func (s *TypeSet) structuralType() Type {
	if len(s.terms) == 1 {
		return s.terms[0].typ
	}
	return nil
}
...
func (s *TypeSet) is(f func(*term) bool) bool {
	if len(s.terms) == 0 {
		// TODO(gri) should not allocate a new term each time
		return f(new(term)) // top term
	}
...
func (s *TypeSet) includes(typ Type) bool {
	if len(s.terms) == 0 {
		return true
	}
	for _, t := range s.terms {
...
func (s1 *TypeSet) subsetOf(s2 *TypeSet) bool {
	switch 0 {
	case len(s1.terms):
		return len(s2.terms) == 0
	case len(s2.terms):
		return true
	}
	// each term of s1 must be a subset of s2
L:
	for _, t1 := range s1.terms {
		for _, t2 := range s2.terms {
			if t1.subsetOf(t2) {
				continue L
			}
		}
		return false // t1 is not a subset of any t2
	}
	return true
}
...
func computeInterfaceTypeSet(check *Checker, pos syntax.Pos, ityp *Interface) *TypeSet {
	...
	for i, typ := range ityp.embeddeds {
		...
		default:
			...
			terms = []*term{{false, typ}}
		}
		allTerms = intersectTerms(allTerms, terms)
	}
...
func computeUnionTypeSet(check *Checker, pos syntax.Pos, utyp *Union) *TypeSet {
	...
	allTerms := make([]*term, 0, len(utyp.terms))
	for _, t := range utyp.terms {
		...
		allTerms = unionTerms(allTerms, terms)
	}
