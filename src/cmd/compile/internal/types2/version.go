...
// A version describes a semantic version.
type version struct {
	major, minor int
}
...
func (check *Checker) allowVersion(pkg *Package, major, minor int) bool {
	...
	ma, mi := check.lang.major, check.lang.minor
	return ma == 0 && mi == 0 || ma > major || ma == major && mi >= minor
}

// parseVersion parses a Go version string (such as "go1.12")
...
func parseVersion(s string) (v version, err error) {
	...
	matches := versionRx.FindStringSubmatch(s)
	if matches == nil {
...
var versionRx = regexp.MustCompile(`^go([1-9][0-9]*)\.(0|[1-9][0-9]*)$`)
