...
// and debug/report logging for the -jsonreport debug flag to the Go compiler
// The json flag option is -json 0,<destination>,
// the jsonreport flag option is -d[=<pkg>]=jsonreport=<N>,<destination>
//
// For the json flag 0 is the version number; to avoid the need for synchronized updates,
// if new versions of the logging appear, the compiler will support both, for a while,
// and clients will specify what they need.
//
// json report numbers are bit-encoded, for combinations of reports
// that will appear in single LSP-encoded output files.
//
...
// It also happens that using the LSP format to transmit
// other compilation-related information turns out to be useful,
...
func parseLogFlag(flag, value, sep string) (version logFormat, directory string) {
	sepAt := strings.Index(value, sep)
	if sepAt <= 0 {
		log.Fatalf("-%s option should be '<version>%s<destination>' where <version> is a number", flag, sep)
	}
	beforeSep := value[:sepAt]
	v, err := strconv.ParseUint(beforeSep, 0, 64)
	if err != nil {
		log.Fatalf("-%s option should be '<version>%s<destination>' where <version> is a number: err=%v", flag, sep, err)
	}
	version = logFormat(v)
	directory = value[sepAt+1:]
	return
}
...
func Enabled() bool {
	...
	log.Fatalf("unexpected optimizer-logging level")
	return false
}
