...
const (
	None                logFormat = 0
	Json0               logFormat = 1 // version 0 for LSP 3.14, 3.15; future versions of LSP may change the format and the compiler may need to support both as clients are updated.
	LastFormat          logFormat = 1
	JsonReportNewObject logFormat = 1 // report allocation type shapes
)
...
var Format = None      // how are the messages written
...
func LogJsonOption(flagValue string) {
	...
	format, report := version%100, version/100
	if format > 0 || report > int(JsonReportNewObject) {
		log.Fatal("-json format version must be 0 (log missed optimizations) or 100 (report allocation shapes)")
	}
	dest = checkLogPath(directory)
	Report = logFormat(report)
	if format == 0 {
		if report == 0 {
			Format = logFormat(format + 1) // Json0
		}
	} else {
		Format = logFormat(format + 1)
	}
}
