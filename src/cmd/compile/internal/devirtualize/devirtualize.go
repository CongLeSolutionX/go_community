		...
		case *ir.ForStmt:
			...
			if len(leaked) > 0 {
				...
				if base.Flag.LowerM != -4 && base.DebugHashMatch(ir.PkgFuncName(fn)) {
					...
					// (1) cond' = {}
					// (2) prebody = {z := z' for z in leaked}
					// (3) postbody = {z' = z for z in leaked}
					// (4) body_continue = {body : s/continue/goto next}
					// (5) init' = (init : s/z/z' for z in leaked) + tmp_first := true
					// (6) body' = prebody +
					// (7)        if tmp_first {tmp_first = false} else {Post} +
					// (8)        if !cond {break} +
					// (9)        body_continue +
					// (10)       next: postbody
					// (11) post' = {}
					...
					// (2,3) initialize preBody and postBody
					...
					// (4) rewrite continues in body -- rewrite is inplace, so works for top level visit, too.
