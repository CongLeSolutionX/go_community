...
func (m *ScopeMarker) checkPos(pos src.XPos) ir.ScopeID {
	...
	if lasti := len(m.marks) - 1; lasti >= 0 {
		last := &m.marks[lasti]
...
func (m *ScopeMarker) Unpush() {
	last := len(m.marks) - 1
	current := m.marks[last].Scope

	if current != ir.ScopeID(len(m.parents)) {
		base.FatalfAt(m.marks[last].Pos, "current scope is not empty: %v, %+v, %+v", current, m.parents, m.marks)
	}

	m.parents = m.parents[:current-1]
	m.marks = m.marks[:last]
}
