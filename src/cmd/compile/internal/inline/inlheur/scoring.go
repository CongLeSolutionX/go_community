...
func computeCallSiteScore(callee *ir.Func, calleeProps *FuncProps, call ir.Node, csflags CSPropBits) (int, scoreAdjustTyp) {
	...
	calleeRecvrParms := paramFieldsForFunc(callee)
	ce := call.(*ir.CallExpr)
...
// given score. Parameters: 'profile' is the PGO profile in use (may be
...
// hairyness estimate to form the score. "Status" shows whether
// anything changed with the site -- did the adjustment bump it down
// just below the threshold ("PROMOTED") or instead bump it above the
// threshold ("DEMOTED") or did nothing happen as a result of the
// heuristics ("---"); "Status" also shows whether PGO was involved.
// "Callee" is the name of the function called, "CallerPos" is the
// position of the callsite, and "ScoreFlags" is a digest of the
// specific properties we used to make adjustments to callsite score
// via heuristics.
