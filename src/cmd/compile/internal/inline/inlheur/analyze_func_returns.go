...
func (ra *returnsAnalyzer) results() []funcprop.ReturnPropBits {
	return ra.values
}

func (ra *returnsAnalyzer) pessimize() {
	for i := range ra.values {
		ra.values[i] = funcprop.ReturnNoInfo
	}
}

func (ra *returnsAnalyzer) nodeVisit(n ir.Node, aux any) {
	if len(ra.values) == 0 {
		return
	}
	if n.Op() != ir.ORETURN {
...
func (ra *returnsAnalyzer) analyzeReturn(ii int, n ir.Node) {
	...
	switch curp {
	case funcprop.ReturnTop:
		// top element: this is the first return we're seen
		switch {
		case isAllocMem:
			newp = funcprop.ReturnIsAllocatedMem
		case isConcConvItf:
			newp = funcprop.ReturnIsConcreteTypeConvertedToInterface
		case isConst:
			newp = funcprop.ReturnAlwaysSameConstant
			newlit = lit
		}
	case funcprop.ReturnIsAllocatedMem:
		if ra.isAllocMem(n) {
			newp = funcprop.ReturnIsAllocatedMem
		}
	case funcprop.ReturnIsConcreteTypeConvertedToInterface:
		if ra.isConcreteConvIface(n) {
			newp = funcprop.ReturnIsConcreteTypeConvertedToInterface
		}
	case funcprop.ReturnAlwaysSameConstant:
		if isConst && ra.isSameLiteral(lit, ra.literals[ii]) {
			newp = funcprop.ReturnAlwaysSameConstant
		}
	}
	ra.values[ii] = newp
...
func (ra *returnsAnalyzer) isAllocMem(n ir.Node) bool {
	sv := ir.StaticValue(n)
	switch sv.Op() {
...
func (ra *returnsAnalyzer) isLiteral(n ir.Node) (constant.Value, bool) {
	sv := ir.StaticValue(n)
	if sv.Op() == ir.ONIL {
		return nil, true
	}
...
func (ra *returnsAnalyzer) isSameLiteral(v1, v2 constant.Value) bool {
	if v1 == nil && v2 == nil {
		return true
	}
	if v1 == nil || v2 == nil {
...
func (ra *returnsAnalyzer) isConcreteConvIface(n ir.Node) bool {
	sv := ir.StaticValue(n)
	if sv.Op() != ir.OCONVIFACE {
		return false
	}
	cie := sv.(*ir.ConvExpr)
	return ra.isAllocMem(cie.X)
}
