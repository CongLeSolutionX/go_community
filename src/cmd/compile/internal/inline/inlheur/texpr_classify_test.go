...
func TestClassifyIntegerCompare(t *testing.T) {
	...
	wantv := true
	v := ShouldFoldIfNameConstant(nandand, nn)
	if v != wantv {
...
func TestClassifyStringCompare(t *testing.T) {
	...
	wantv := true
	v := ShouldFoldIfNameConstant(top, nn)
	if v != wantv {
...
func TestClassifyAssortedShifts(t *testing.T) {
	...
	for _, bc := range badcases {
		wantv := false
		v := ShouldFoldIfNameConstant(typecheck.Expr(bc), nn)
		if v != wantv {
...
func TestClassifyFloat(t *testing.T) {
	...
	wantv := false
	v := ShouldFoldIfNameConstant(typecheck.Expr(add), nn)
	if v != wantv {
