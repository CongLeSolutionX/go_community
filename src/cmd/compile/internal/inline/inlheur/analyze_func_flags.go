...
// funcFlagsAnalyzer computes the "Flags" value for
// the funcprop.FuncProps object we're computing. The main item
// of interest here is "nstate", which stores the diposition of
// a given ir Node with respect to the flags/properties we're
// trying to compute.
...
// results returns the final results from the analysis in the
// form of an funcprop.FuncPropBits value.
func (ffa *funcFlagsAnalyzer) results() funcprop.FuncPropBits {
	var rv funcprop.FuncPropBits
	if !ffa.noInfo && ffa.dolist(ffa.fn.Body) == psCallsPanic {
		rv = funcprop.FuncPropUnconditionalPanicExit
	}
	return rv
}
...
// statements, where 'succ' is a statement that appears immediately after
// 'pred'. Examples:
//
//	case 1:             case 2:
...
func isPanicLike(n ir.Node) bool {
	...
	if name.Func.Inl != nil && name.Func.Inl.Properties != nil &&
		name.Func.Inl.Properties.Flags&funcprop.FuncPropUnconditionalPanicExit != 0 {
		return true
	}
	return false
}
