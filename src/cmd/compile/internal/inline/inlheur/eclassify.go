...
// whether it contains only combinations of simple references to the
// name 'n' and selected constants + operators. The intent is to
// identify expression that could be folded away to a constant if the
// value of 'n' were available. Return value is TRUE if 'e' does look
// foldable given the value of 'n', and given that 'e' actually makes
// reference to 'n'. Some examples where the type of "n" is int64,
// type of "s" is string, and type of "p" is *byte:
//
//	 Simple?	Expr
//	 yes		n<10
//	 yes		n*n-100
...
func ShouldFoldIfNameConstant(n ir.Node, name *ir.Name) bool {
	cl := makeExprClassifier(name)
	...
	doNode(n)
	return cl.getdisp(n) == exprSimple
}

// exprClassifier holds intermediate state about nodes within
// an expression tree being analyzed by ShouldFoldIfNameConstant.
// Here "name" is the name node passed in, and "disposition"
// stores the result of classifying a given IR node.
...
const (
	...
	// expr is legal combination of literals and specified name
	exprSimple
)
...
func makeExprClassifier(name *ir.Name) *exprClassifier {
...
// calculated classifications for n's children, as part of
// a bottom-up walk over an expression tree.
