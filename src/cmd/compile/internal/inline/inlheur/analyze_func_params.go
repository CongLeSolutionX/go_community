...
func paramFieldsForFunc(fn *ir.Func) []*types.Field {
	recvrParms := []*types.Field{}
	for _, fs := range &types.RecvsParams {
		for _, f := range fs(fn.Type()) {
			recvrParms = append(recvrParms, f)
		}
	}
	return recvrParms
}
...
func getParams(fn *ir.Func) []*ir.Name {
	...
	dclidx := 0
	recvrParms := paramFieldsForFunc(fn)
