...
func makeParamsAnalyzer(fn *ir.Func) *paramsAnalyzer {
	...
	vals := make([]funcprop.ParamPropBits, len(params))
...
func (pa *paramsAnalyzer) results() []funcprop.ParamPropBits {
...
func (pa *paramsAnalyzer) checkParams(x ir.Node, flag funcprop.ParamPropBits, testf func(x ir.Node, param *ir.Name) bool) {
	for idx, p := range pa.params {
		if pa.values[idx] == funcprop.ParamNoInfo {
			continue
		}
		result := testf(x, p)
		if debugTrace&debugTraceParams != 0 {
...
// expr) to see if the expr would fold away of a specific parameter
// had a constant value.
func (pa *paramsAnalyzer) foldCheckParams(x ir.Node) {
	pa.checkParams(x, funcprop.ParamFeedsIfOrSwitch,
...
func (pa *paramsAnalyzer) callCheckParams(ce *ir.CallExpr) {
	...
	case ir.OCALLINTER:
		...
		pa.checkParams(r, funcprop.ParamFeedsInterfaceMethodCall,
			func(x ir.Node, p *ir.Name) bool {
				name := x.(*ir.Name)
				return name == p
			})
	case ir.OCALLFUNC:
		...
		pa.checkParams(called, funcprop.ParamFeedsIndirectCall,
			func(x ir.Node, p *ir.Name) bool {
				name := x.(*ir.Name)
				return name == p
			})
...
func (pa *paramsAnalyzer) nodeVisit(n ir.Node, aux any) {
	if len(pa.values) == 0 {
		return
	}
	switch n.Op() {
