...
// AnalyzeFunc computes function properties for fn and for all of its
// contained closures, updating the global 'fpmap' table. It is
// assumed that "CanInline" has been run on fn and on those closures
// that feed directly into calls; this function will  check inlinability for
// other closures in case they are returned as a result.
func AnalyzeFunc(fn *ir.Func, canInline func(*ir.Func), budgetForFunc func(*ir.Func) int32, inlineMaxBudget int) {
	...
	funcs := []*ir.Func{fn}
	ir.Any(fn, func(n ir.Node) bool {
		if c, ok := n.(*ir.ClosureExpr); ok {
			funcs = append(funcs, c.Func)
			if !c.Func.InlinabilityChecked() {
				canInline(c.Func)
			}
		}
		return false
	...
	// been analyzer, revisit the question of whether it should be
...
// inline candidate.
func revisitInlinability(fn *ir.Func, fp *FuncProps, budgetForFunc func(*ir.Func) int32) {
	if fn.Inl == nil {
		return
	}
	mxAdjust := int32(largestScoreAdjustment(fn, fp))
...
func computeFuncProps(fn *ir.Func, inlineMaxBudget int) (*FuncProps, CallSiteTab) {
	...
	// Now build up a table of callsites for this func.
...
func captureFuncDumpEntry(fn *ir.Func) {
	...
	fih, ok := fpmap[fn]
	if !ok {
		panic(fmt.Sprintf("captureFuncDumpEntry invoked on %v: no props", fn))
	}
	if dumpBuffer == nil {
