...
type resultUseAnalyzer struct {
	rt    map[*ir.Name]resultPropAndCS
	fn    *ir.Func
	cstab CallSiteTab
	*condLevelTracker
}
...
func rescoreBasedOnCallResultUses(fn *ir.Func, rt map[*ir.Name]resultPropAndCS, cstab CallSiteTab) {
...
func examineCallResults(cs *CallSite, rt map[*ir.Name]resultPropAndCS) {
	...
	for idx, n := range names {
		...
		if autoTemps[idx] != nil {
			rt[autoTemps[idx]] = entry
		}
		if debugTrace&debugTraceScoring != 0 {
			fmt.Fprintf(os.Stderr, "=-= add rt table entry n=%v autotemp=%v props=%s\n", n, autoTemps[idx], rprop.String())
		}
	}
}
...
// foldCheckResults examines the specified if/switch conditon 'cond'
