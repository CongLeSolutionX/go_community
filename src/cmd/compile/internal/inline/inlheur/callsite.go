...
func (csa *callSiteAnalyzer) flagsForNode(call ir.Node) CSPropBits {
	...
	// walking back up the node stack to see if we can find either
	// A) an enclosing panic, or B) a statement node that we've determined
	// leads to a panic/exit.
	for ri := range csa.nstack[:len(csa.nstack)-1] {
		...
		if debugTrace&debugTraceCalls != 0 {
			ps, inps := csa.ptab[n]
			fmt.Fprintf(os.Stderr, "=-= callpar %d op=%s ps=%s inptab=%v\n",
				i, n.Op().String(), ps.String(), inps)
		}
