...
func Inline_Flood(n *ir.Name, exportsym func(*ir.Name)) {
	...
	ir.VisitList(ir.Nodes(fn.Inl.Body), func(n ir.Node) {
		...
		case ir.OCLOSURE:
			...
			// TODO(danscales) Add inlFlood call?
		}
	})
}
...
func inlcopy(n ir.Node) ir.Node {
	...
	edit = func(x ir.Node) ir.Node {
		...
		if x.Op() == ir.OCLOSURE {
			x := x.(*ir.ClosureExpr)
			// Need to save/duplicate x.Func.Nname.Ntype, x.Func.Body,
			// x.Func.Dcl, and x.Func.ClosureVars for iexport and inlining.
			oldfn := x.Func
			newfn := &ir.Func{}
			*newfn = *oldfn
...
func inlCallee(fn ir.Node) *ir.Func {
	...
	case ir.OCLOSURE:
		// XXXX Causes bugs that I haven't figured out yet.
		return nil
...
func (subst *inlsubst) node(n ir.Node) ir.Node {
	...
	case ir.ONAME:
		...
		if n.IsClosureVar() && subst.newxfunc == nil {
			o := n.Outer
	...
	if n.Op() == ir.OCLOSURE {
		...
		*newfn = *oldfn
		...
		m.(*ir.ClosureExpr).Func = newfn
		newfn.ClosureType = subst.node(oldfn.ClosureType).(ir.Ntype)
		...
		// TODO(danscales) - make sure this works and export ctxCallee.
		top := int(0)
		if oldfn.ClosureCalled() {
			top = 8 /* typecheck.ctxCallee */
		}
		typecheck.TcClosure(m.(*ir.ClosureExpr), top)
	}
