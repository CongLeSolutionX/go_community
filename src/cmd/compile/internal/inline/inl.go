...
func (v *hairyVisitor) tooHairy(fn *ir.Func) bool {
	...
	if ir.DoChildren(fn, func(x ir.Node) bool { return v.do(x) }) {
		return true
	}
...
func (v *hairyVisitor) doNode(n ir.Node) bool {
	...
	case ir.OIF:
		...
		if ir.IsConst(n.Cond, constant.Bool) {
			...
			if errList(n.Init(), v.do) {
				return true
			}
			if errList(n.Body, v.do) {
				return true
			}
			if errList(n.Else, v.do) {
				return true
			}
			return false
		}
	...
	return ir.DoChildren(n, func(x ir.Node) bool {
		return v.do(x)
	})
}
...
func errList(list []ir.Node, do func(ir.Node) bool) bool {
	for _, x := range list {
		if x != nil {
			return do(x)
		}
