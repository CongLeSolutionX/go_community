...
func (v *hairyVisitor) doNode(n ir.Node) error {
	...
	case ir.OCLOSURE:
		// TODO(danscales) - should cost of inlining a function with a
		// closure be higher
		v.budget -= v.extraCallCost
		v.hadClosure = true
...
type inlsubst struct {
	...
	fn *ir.Func // For debug, will remove.  Func that is being inlined
}
...
func (subst *inlsubst) node(n ir.Node) ir.Node {
	...
	if n.Op() == ir.OCLOSURE {
		...
		*newtfunc = *(oldt.FuncType())
		...
		// the closure is inlined in a specific function (ir.CurFunc)
		typecheck.TcClosure(m.(*ir.ClosureExpr), oldfn.ClosureCalled())
	}
