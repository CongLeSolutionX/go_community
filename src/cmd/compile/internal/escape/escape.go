...
type location struct {
	...
	assigned  bool // has this variable been assigned to?
...
func Batch(fns []*ir.Func, recursive bool) {
	...
	for _, closure := range b.closures {
		b.captureFunc(closure.k, closure.clo)
	}
	b.closures = nil
...
func (b *batch) captureFunc(k hole, clo *ir.ClosureExpr) {
	fn := clo.Func
	for _, cv := range fn.ClosureVars {
		...
		// Check that we compute byval the same as typecheck.
		byval := !loc.addrtaken && !loc.assigned && cv.Type().Size() <= 128
		if cv.Byval() != byval {
			base.FatalfAt(cv.Pos(), "byval mismatch: %v: %v != %v (addr=%v assign=%v size=%v)", cv, byval, cv.Byval(), loc.addrtaken, loc.assigned, cv.Type().Size())
		}
...
func (e *escape) markAssigned(ks []hole, where ir.Node) {
	for _, k := range ks {
		loc := k.dst
