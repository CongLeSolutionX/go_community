...
func (o *orderState) expr1(n, lhs ir.Node) ir.Node {
	...
	case ir.OMAPLIT:
		...
		// Remember that we issued this assignments so we can include that count
		// in the map alloc hint.
		// Note: we're assuming here that all the keys in the map literal are distinct.
		// If in fact they are all equal, this will be an overcount. Probably not worth
		// accounting for that, as equal keys in map literals are rare, and at worst we
