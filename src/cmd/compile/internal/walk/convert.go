...
func walkConvInterface(n *ir.ConvExpr, init *ir.Nodes) ir.Node {
	...
	if n.Op() != ir.OCONVIDATA && !fromType.IsInterface() && !ir.IsBlank(ir.CurFunc.Nname) {
		// skip CONVIDATA conversions, because the type is likely a shape type
		// that we don't want in interfaces. The appropriate call to MarkTypeUsedInInterface
		// will come when building the dictionary (from which the matching type word will come).
