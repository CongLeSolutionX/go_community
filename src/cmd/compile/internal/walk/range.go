...
func rangeConvert(nrange *ir.RangeStmt, dst *types.Type, src, typeWord, srcRType ir.Node) ir.Node {
	if types.Identical(dst, src.Type()) {
		return src
	}

	op, why := typecheck.Assignop(src.Type(), dst)
	if op == ir.OXXX {
		base.FatalfAt(nrange.Pos(), "cannot assign %L to %v%v", src, dst, why)
	}

	n := ir.NewConvExpr(nrange.Pos(), op, dst, src)
	n.TypeWord = typeWord
	n.SrcRType = srcRType
	return n
}
