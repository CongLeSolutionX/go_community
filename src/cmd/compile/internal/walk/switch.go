...
func (s *exprSwitch) flush() {
	...
	if s.exprname.Type().IsString() && len(cc) >= 2 {
		...
		fallThrough := typecheck.AutoLabel(".s")
		...
		for _, run := range runs {
			...
			s.search(run, &s.done)
			s.done.Append(ir.NewBranchStmt(pos, ir.OGOTO, fallThrough))
		...
		outer.Emit(&s.done)
		s.done.Append(ir.NewLabelStmt(s.pos, fallThrough))
		return
	}
...
func (s *exprSwitch) tryJumpTable(cc []exprClause, out *ir.Nodes) bool {
	const minCases = 8   // have at least jumpTableCases cases in the switch
	const minDensity = 4 // use at least 1 out of every jumpTableDensity entries

	if !ssagen.Arch.LinkArch.CanJumpTable {
		return false
	}
	if len(cc) < minCases {
	...
	limit := constant.BinaryOp(constant.MakeInt64(int64(len(cc))), token.MUL, constant.MakeInt64(minDensity))
