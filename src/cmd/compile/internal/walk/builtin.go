...
func walkGrowslice(slice *ir.Name, init *ir.Nodes, oldPtr, newLen, oldCap, num ir.Node) *ir.CallExpr {
	...
	case elemtype.Size() == 4 && !elemtype.HasPointers():
		fn := typecheck.LookupRuntime("growslice32")
		fn = typecheck.SubstArgTypes(fn, elemtype, elemtype)
		return mkcall1(fn, slice.Type(), init, oldPtr, newLen, oldCap, num)
	case elemtype.Size() == 8 && !elemtype.HasPointers():
		fn := typecheck.LookupRuntime("growslice64")
		fn = typecheck.SubstArgTypes(fn, elemtype, elemtype)
		return mkcall1(fn, slice.Type(), init, oldPtr, newLen, oldCap, num)
