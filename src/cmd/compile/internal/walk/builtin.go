...
func walkUnsafeSlice(n *ir.BinaryExpr, init *ir.Nodes) ir.Node {
	...
	if ir.ShouldCheckPtr(ir.CurFunc, 1) {
		...
		// Keep these codes in sync with runtime.unsafeslice{,64}
		if len.Type().IsKind(types.TIDEAL) || len.Type().Size() <= types.Types[types.TUINT].Size() {
			...
			nif.Body = ir.Nodes{mkcall("panicunsafeslicelen", nil, init)}
			appendWalkStmt(init, nif)
		}
		...
		// size := et.size
		// mem, overflow := runtime.mulUintptr(t.Size, len)
		...
		size := typecheck.Conv(ir.NewInt(t.Size()), types.Types[types.TUINTPTR])
		call := mkcall1(fn, fn.Type().Results(), init, size, typecheck.Conv(typecheck.Conv(ptr, types.Types[types.TUNSAFEPTR]), types.Types[types.TUINTPTR]))
		...
		// }
		nif = ir.NewIfStmt(base.Pos, nil, nil, nil)
		memCond := ir.NewBinaryExpr(base.Pos, ir.OGT, mem, ir.NewUnaryExpr(base.Pos, ir.ONEG, typecheck.Conv(typecheck.Conv(ptr, types.Types[types.TUNSAFEPTR]), types.Types[types.TUINTPTR])))
		nif.Cond = ir.NewLogicalExpr(base.Pos, ir.OOROR, overflow, memCond)
		nifPtr := ir.NewIfStmt(base.Pos, nil, nil, nil)
		nifPtr.Cond = ir.NewBinaryExpr(base.Pos, ir.OEQ, ptr, typecheck.NodNil())
		nifPtr.Body = ir.Nodes{mkcall("panicunsafesliceptrnil", nil, init)}
		nif.Body = ir.Nodes{
			nifPtr,
			mkcall("panicunsafeslicelen", nil, init),
		}
		appendWalkStmt(init, nif)
	}
