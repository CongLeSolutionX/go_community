...
func walkUnsafeSlice(n *ir.BinaryExpr, init *ir.Nodes) ir.Node {
	...
	if ir.ShouldCheckPtr(ir.CurFunc, 1) {
		...
		fn := typecheck.LookupRuntime(fnname)
		init.Append(mkcall1(fn, nil, init, reflectdata.TypePtr(t.Elem()), unsafePtr, typecheck.Conv(len, lenType)))
	} else {
		...
		// size := uintptr(et.size)
		// mem, overflow := runtime.mulUintptr(size, len)
		...
		size := typecheck.Conv(ir.NewLiteral(constant.MakeUint64(uint64(t.Elem().Size()))), types.Types[types.TUINTPTR])
		call := mkcall1(fn, fn.Type().Results(), init, size, typecheck.Conv(typecheck.Conv(len, lenType), types.Types[types.TUINTPTR]))
