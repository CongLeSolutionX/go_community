...
func (s *InitSchedule) staticcopy(l *ir.Name, loff int64, rn *ir.Name, typ *types.Type) bool {
	...
	case ir.OSLICELIT:
		// copy slice
		r := r.(*ir.CompLitExpr)
		staticdata.InitSlice(l, loff, s.Temps[r], ir.Int64Val(r.Ntype))
		return true
...
func isStaticCompositeLiteral(n ir.Node) bool {
	...
	case ir.OCONVIFACE:
		...
		if types.IsInterfaceWord(val.Type()) && val.Op() == ir.ONIL {
			return true
		}
...
func isSmallSliceLit(n *ir.CompLitExpr) bool {
	...
	return ir.IsSmallIntConst(r) && (n.Type().Elem().Width == 0 || ir.Int64Val(r) <= ir.MaxSmallArraySize/n.Type().Elem().Width)
}
