...
func fakePC(n ir.Node) ir.Node {
	...
	// Include the source code file index in the calculation of the fake PC
	_ = binary.Write(hash, binary.LittleEndian, n.Pos().FileIndex())
	// Include the line number in the calculation of the fake PC
	_ = binary.Write(hash, binary.LittleEndian, int64(n.Pos().Line()))
	// Include the column number in the calculation of the fake PC
...
func walkCompareString(n *ir.BinaryExpr, init *ir.Nodes) ir.Node {
	...
	if base.Debug.Libfuzzer != 0 {
		...
		init.Append(mkcall(fn, nil, init, tracecmpArg(x, paramType, init), tracecmpArg(y, paramType, init), fakePC(n)))
	}
