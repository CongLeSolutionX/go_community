...
func (g *irgen) expr(expr syntax.Expr) ir.Node {
	...
	case !tv.IsValue():
		base.FatalfAt(g.pos(expr), "TODO: %v %v", expr, tv)
	}
	...
	n := g.expr0(tv.Type, expr)

	// Might be a tuple due to multi-value function call or has-ok expression.
	// TODO(mdempsky): Decide how to handle this.
	if _, isTuple := tv.Type.(*types2.Tuple); isTuple {
		return n
	}
	...
	n = typecheck.Expr(n)
	if n.Type() == nil {
		base.FatalfAt(n.Pos(), "typechecking %v failed unexpectedly", n)
	}

	typ := g.typ(tv.Type)
	if n.Type() == types.UntypedBool && typ.IsBoolean() {
		// ok: comparisons return untyped bool
	} else if !types.Identical(typ, n.Type()) {
		base.FatalfAt(g.pos(expr), "unexpected type for %v: %v != %v", n, n.Type(), typ)
	}
	return n
}
...
func (g *irgen) expr0(typ types2.Type, expr syntax.Expr) ir.Node {
	...
	case *syntax.CallExpr:
		...
		n := ir.NewCallExpr(pos, ir.OCALL, fun, g.exprs(expr.ArgList))
...
func (g *irgen) funcLit(typ types2.Type, expr *syntax.FuncLit) ir.Node {
	...
	fn.ClosureType = g.typeExpr(expr.Type)
