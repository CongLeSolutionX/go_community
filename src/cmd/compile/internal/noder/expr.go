...
func (g *irgen) match(t1 *types.Type, t2 types2.Type, hasOK bool) bool {
	...
	if hasOK {
		// For has-ok values, go/types represents the
		// expression as having a 2-element tuple type,
		// whereas ir just uses the first/main type and infers
		// that the second type is boolean.
