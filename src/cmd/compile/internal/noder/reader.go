...
func (r *reader) syntheticArgs(pos src.XPos) (recvs, params ir.Nodes) {
	...
	endRecvs := sig.NumRecvs()
	endParams := endRecvs + sig.NumParams()

	return ir.ToNodes(r.curfn.Dcl[:endRecvs]), ir.ToNodes(r.curfn.Dcl[endRecvs:endParams])
}
...
func (r *reader) assign() (ir.Node, bool) {
	...
	case assignDef:
		name := r.curfn.NewLocal(r.pos(), r.localIdent(), r.typ())
		setBasePos(name.Pos())
		r.addLocal(name)
		return name, true
