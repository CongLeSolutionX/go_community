...
type readerDict struct {
	...
	// If the object is an instantiated type, then baseSym is the
	// mangled symbol, including any type arguments.
	baseSym *types.Sym
...
func shapify(targ *types.Type) *types.Type {
	...
	// TODO(mdempsky): Better shaping than merely converting to
	// underlying type. E.g., shape pointer types to unsafe.Pointer when
	// we know the element type doesn't matter.
...
func (r *reader) callShaped(pos src.XPos) {
	...
	// dictionary, and finally normal paramaters.
	var args ir.Nodes
	args.Append(recvs...)
...
func (r *reader) expr() (res ir.Node) {
	...
	case exprMethodVal:
		...
		return r.curry(pos, true, baseFn, recv, dictArg)
	...
	case exprMethodExpr:
		...
		return r.methodExprWrap(pos, recv, implicits, deref, addr, baseFn, dictArg)
