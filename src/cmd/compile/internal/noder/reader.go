...
// TODO(mdempsky): Cleanup these maps.
...
func readPackage(pr0 pkgDecoder, importpkg *types.Pkg) {
	pr := newPkgReader(pr0)
...
func readPkgBits(pr0 pkgDecoder, self *types.Pkg, target *ir.Package) {
	...
	r := pr.newReader(relocMeta, privateRootIdx, syncPublic)
	r.top = true
	...
	for _, path := range r.strings() {
		...
		assert(ipkg.Direct) // already loaded and added to target by importer
	}
...
type pkgReader struct {
	...
	pkgs     []*types.Pkg
	typs     []*types.Type
	objs     []*ir.Name // TODO: remove; only needed for inits anymore
...
func newPkgReader(pr pkgDecoder) *pkgReader {
	return &pkgReader{
		...
		objs:     make([]*ir.Name, pr.numElems(relocObj)),

		newindex: make([]int, len(pr.elemEnds)),
	}
}
...
func (r *reader) expandFunc(name *ir.Name) *ir.Func {
	...
	b, ok := bodyIndex[fn]
	assert(ok)
	b.pr.bodyIdx(fn, b.idx, b.implicits)
...
func flushBodies(check bool) {
	for len(todoBodies) != 0 {
		for fn := range todoBodies {
			...
			if fn.OClosure == nil && len(b.implicits) != 0 {
...
func (r *reader) sym() (pkg *types.Pkg, sym *types.Sym) {
	r.sync(syncSym)
	pkg = r.pkg()
...
func (r *reader) compLit() ir.Node {
	...
	for i := range elems {
		...
		if isStruct {
			...
			elem = ir.NewStructKeyExpr(pos, field, value)
		} else if r.bool() {
			pos := r.pos()
			key := r.expr()
			value := r.wrapname(r.expr())
...
func (r *reader) addBody(fn *ir.Func) {
	...
	bodyIndex[fn] = readerBody{r.p, idx, implicits}

	if r.curfn != nil {
		r.p.bodyIdx(fn, idx, implicits)
	} else {
		todoBodies[fn] = true
	}
}

func (pr *pkgReader) bodyIdx(fn *ir.Func, idx int, implicits []*types.Type) {
	r := pr.newReader(relocBody, idx, syncFuncBody)
	r.curfn = fn
...
func (r *reader) funcBody() {
	fn := r.curfn

	// TODO(mdempsky): Do we still need to set ir.CurFunc here? I don't
	// think we do.
...
func (r *reader) doObj(explicits []*types.Type, must bool) ir.Node {
	r.sync(syncObject)

	return r.p.globalIdx(r.reloc(relocObj), r.implicits, explicits, must)
}

func (pr *pkgReader) globalIdx(idx int, implicits, explicits []*types.Type, must bool) ir.Node {
	r := pr.newReader(relocObj, idx, syncObject1)
	r.bside = pr.newReader(relocObjExt, idx, syncObject1)
	...
	if strings.Contains(sym.Name, "·") {
		r.implicits = implicits
		r.bside.implicits = implicits
	}
	r.explicits = explicits
	r.bside.explicits = explicits
	...
	do := func(op ir.Op, hasTParams bool) *ir.Name {
		pos := r.pos()
		if !r.typeParams(origSym, hasTParams, must) {
			return nil
		}
	...
	case objStub:
		...
		return b.pr.globalIdx(b.idx, implicits, explicits, true)
...
func (r *reader) typeExt(name *ir.Name) {
	...
	// descriptor as DUPOK.
	// TODO(mdempsky): Understand the dictionary stuff, and maybe set
	// r.implicits/r.explicits on typ instead.
	typ.SetInstance(len(r.implicits)+len(r.explicits) != 0)
	...
	i, pi := r.len()-1, r.len()-1
	if i != -1 && pi != -1 {
		typecheck.TypeSymIdx[typ] = [2]int64{int64(i), int64(pi)}
	}
}
...
func (r *reader) typeParams(sym *types.Sym, hasTParams, must bool) bool {
	if !hasTParams {
		assert(len(r.explicits) == 0)
		return true
	}
	...
	if !must && nexplicits != 0 && len(r.explicits) == 0 {
		// Should only happen for top-level declarations, which don't have
		// implicit type parameters.
		assert(nimplicits == 0)
		assert(len(r.implicits) == 0)
		return false
	}
	...
	for i := 0; i < nexplicits; i++ {
		r.pos()
		r.sym()
	}
	for i := 0; i < nexplicits; i++ {
		r.typ() // TypeParam.Bound
	}
...
func (pr *pkgReader) typIdx(idx int) *types.Type {
...
func InlineCall(call *ir.CallExpr, fn *ir.Func, inlIndex int) *ir.InlinedCallExpr {
	...
	r := bi.pr.newReader(relocBody, bi.idx, syncFuncBody)
...
func ExpandInline(fn *ir.Func) {
	if QuirksMode() {
		base.FatalfAt(fn.Pos(), "i don't think this happens anymore")
		return
	}
