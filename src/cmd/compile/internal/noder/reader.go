...
func (r *reader) expr() (res ir.Node) {
	...
	case exprConvert:
		...
		// function to single method interface conversions
		if n, ok := n.(*ir.ConvExpr); ok && n.Op() == ir.OFUNCIFACE {
			base.AssertfAt(n.Type().IsInterface(), pos, "type %v is not an interface", n)
			base.AssertfAt(srcRType.Type().Kind() != types.TFUNC, pos, "type %v is not a func", srcRType)

			wrapper := ifaceFuncWrapper{
				iface:     n.Type(),
				signature: n.X.Type(),
			}
			needFunctionWrappers = append(needFunctionWrappers, wrapper)
		}
...
// needFunctionWrappers lists functions for which we need to
// generate function wrappers for.
var needFunctionWrappers []ifaceFuncWrapper
...
// ifaceFuncWrapper represents functions that need to be wrapped and
// the interfaces which will become the receivers.
type ifaceFuncWrapper struct {
	iface     *types.Type // dst iface type
	signature *types.Type // src function type
}
...
func MakeWrappers(target *ir.Package) {
	...
	for _, wrapper := range needFunctionWrappers {
		wrapFunctionIfaceFunc(wrapper.iface, wrapper.signature, target)
	}
	needFunctionWrappers = nil
}
...
func methodWrapper(derefs int, tbase *types.Type, method *types.Field, target *ir.Package) {
	...
	fn := newMethodWrapperFunc(pos, sym, wrapper, method)
...
func wrapMethodValue(recvType *types.Type, method *types.Field, target *ir.Package, needed bool) {
	...
	fn := newMethodWrapperFunc(pos, sym, nil, method)
	sym.Def = fn.Nname
...
// wrapFunctionIfaceFunc wraps sig with a function that sets the iface interface
// as the receiver. iface is the destination interface type. sig is the source function type.
func wrapFunctionIfaceFunc(iface, sig *types.Type, target *ir.Package) {
	oriSym := types.TypeSym(sig)
	sym := ir.MethodSymSuffix(iface, oriSym, ir.IfaceFuncSuffix) // "-fi" -> for function interfaces
	if sym.Uniq() {
		return
	}
	sym.SetUniq(true)
	...
	pos := base.AutogeneratedPos
	// same params as sig with an extra rcvr param of type sig
	fn := newIfaceFunc(pos, sym, sig)

	sym.Def = fn.Nname

	dot := fn.Nname.Type().Recv().Nname.(*ir.Name)
	addTailCall(pos, fn, dot)

	finishWrapperFunc(fn, target)
}

func newWrapperFunc(pos src.XPos, sym *types.Sym, sig *types.Type) *ir.Func {
...
func newMethodWrapperFunc(pos src.XPos, sym *types.Sym, wrapper *types.Type, method *types.Field) *ir.Func {
	var recv *types.Field
	if wrapper != nil {
		recvSym := method.Type.Recv().Sym
		recvPos := method.Type.Pos()
		recv = types.NewField(recvPos, recvSym, wrapper)
	}
	sig := newWrapperType(recv, method.Type)

	return newWrapperFunc(pos, sym, sig)
}

func newIfaceFunc(pos src.XPos, sym *types.Sym, sig *types.Type) *ir.Func {
	base.AssertfAt(types.IsDirectIface(sig), pos, "sig %v is not a direct interface", sig)

	recvSym := typecheck.Lookup(".this")
	recvPos := sig.Pos()
	recv := types.NewField(recvPos, recvSym, sig)
	newSig := newWrapperType(recv, sig)

	return newWrapperFunc(pos, sym, newSig)
}
...
func addTailCall(pos src.XPos, curFn *ir.Func, callee ir.Node) {
	fnSig := curFn.Nname.Type()
	args := make([]ir.Node, fnSig.NumParams())
	for i, param := range fnSig.Params() {
	...
	curFn.SetWrapper(true) // TODO(mdempsky): Leave unset for tail calls?

	calleeSig := callee.Type()
	call := typecheck.Call(pos, callee, args, calleeSig.IsVariadic()).(*ir.CallExpr)

	if calleeSig.NumResults() == 0 {
		curFn.Body.Append(call)
		return
	}

	ret := ir.NewReturnStmt(pos, nil)
	ret.Results = []ir.Node{call}
	curFn.Body.Append(ret)
}
