...
func (r *reader) dictPtrSym(baseSym *types.Sym) *ir.Name {
	sym := baseSym.Pkg.Lookup(objabi.GlobalDictPrefix + "." + baseSym.Name)
...
// rtypesOffset returns the offset of the runtime dictionary's type
// parameter method expressions section, in words.
func (dict *readerDict) typeParamMethodExprsOffset() int { return 0 }

// rtypesOffset returns the offset of the runtime dictionary's
...
// rtypesOffset returns the offset of the runtime dictionary's itabs
// section, in words.
	...
	case exprMethodVal:
		...
		if method, ok := wrapperFn.(*ir.SelectorExpr); ok && method.Op() == ir.OMETHEXPR {
			...
			if recv.Type().HasShape() {
				typ := method.Type().Params().Field(0).Type
				if !types.Identical(typ, recv.Type()) {
					base.FatalfAt(method.Pos(), "receiver %L does not match %L", recv, method)
				}
				recv.SetType(typ)
			}
	...
	case exprMethodExpr:
		...
		if wrapperFn != nil && len(implicits) == 0 && !deref && !addr {
			if !types.Identical(recv, wrapperFn.Type().Params().Field(0).Type) {
				base.FatalfAt(wrapperFn.Pos(), "want receiver type %v, but have method %L", recv, wrapperFn)
			}
			return wrapperFn
		}
...
func (r *reader) methodExpr() (wrapperFn, baseFn, dictArg ir.Node) {
	...
	if r.Bool() { // static dictionary
		...
		base.AssertfAt(!embed.HasShape(), pos, "shaped receiver %v", embed)
	...
	base.AssertfAt(!embed.HasShape() || embed.IsInterface(), pos, "shaped receiver %v", embed)
