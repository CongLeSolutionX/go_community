...
type readerDict struct {
	// obj is the package-scope declared object that this dict belongs
	// to. For functions, this is the function itself; for methods, it's
	// the receiver type.
	obj *ir.Name
...
func (pr *pkgReader) objIdx(idx int, implicits, explicits []*types.Type) ir.Node {
	...
	if !sym.IsBlank() && sym.Def != nil {
		...
		if n, ok := n.(*ir.Name); ok {
			r.dict.obj = n
			if n.Op() == ir.ONAME && n.Class == ir.PFUNC && r.needDictParam() {
				return dictWrapper(n)
			}
		}
		return n
	}
...
func (r *reader) funcExt(name *ir.Name) {
	...
	if r.needDictParam() {
		r.dictVar = ir.NewHiddenParam(fn.Pos(), fn, fn.Sym().Pkg.Lookup(".dict"), types.Types[types.TUNSAFEPTR])
	}
...
func (r *reader) needDictParam() bool {
	return base.Debug.UnifiedDicts != 0 && r.dict != nil && len(r.dict.targs)+len(r.dict.derived) != 0
}
...
func (r *reader) funcBody(fn *ir.Func) {
	...
	if r.needDictParam() {
		r.dictVar = r.closureVars[0]
		r.closureVars = r.closureVars[1:]
	}
	...
	ir.WithFunc(fn, func() {
		...
		if r.dictVar != nil {
			// For now, just check that the dict is nil, as expected.  But
			// only for normal functions; we need to fix interface wrapper
			// generation before we can check methods too.
			pos := base.AutogeneratedPos
			nif := ir.NewIfStmt(pos,
				ir.NewBinaryExpr(pos, ir.ONE, r.dictVar, types.BuiltinPkg.Lookup("nil").Def.(ir.Node)),
				[]ir.Node{
					ir.NewCallExpr(pos, ir.OCALL, types.BuiltinPkg.Lookup("panic").Def.(ir.Node), []ir.Node{ir.NewString("oh no")}),
				}, nil)
			body.Append(typecheck.Stmt(nif))
		}
		...
		if len(body) == 0 {
			...
			body.Append(typecheck.Stmt(ir.NewBlockStmt(pos, nil)))
		}
...
func (r *reader) expr() ir.Node {
	...
	case exprSelector:
		...
		if methodExpr {
			n = typecheck.Expr(n).(*ir.SelectorExpr)
			tbase := n.X.Type()
			if tbase.IsPtr() {
				tbase = tbase.Elem()
			}
			if !tbase.IsInterface() && tbase.IsFullyInstantiated() {
				return dictWrapper(n.FuncName())
			}
		}
	...
	case exprCall:
		...
		if _, ok := fun.(*ir.SelectorExpr); ok && !quirksMode() {
			n.X = typecheck.Callee(n.X)
			if *base.Flag.LowerV {
				base.WarnfAt(n.X.Pos(), "callee: %+v", n.X)
			}
			if n.X.Op() == ir.ODOTMETH {
				recv := n.X.(*ir.SelectorExpr).Selection.Type.Recv().Type
				if recv.IsFullyInstantiated() || recv.IsPtr() && recv.Elem().IsFullyInstantiated() {
					typecheck.Call(n)
					assert(n.Op() == ir.OCALLFUNC)
					assert(n.X.Op() == ir.OMETHEXPR)

					n.X = dictWrapper(n.X.(*ir.SelectorExpr).FuncName())

					if *base.Flag.LowerV {
						base.WarnfAt(pos, "rewrote call: %+v", n)
					}

					// Reset so typecheck can try again later, and get n.Use
					// correct this time.
					n.SetOp(ir.OCALL)
					n.SetTypecheck(0)
				}
...
func (r *reader) funcLit() ir.Node {
	...
	if r.needDictParam() {
		ir.NewClosureVar(pos, fn, r.dictVar)
	}
...
func (r *reader) wrapType(typ *types.Type, target *ir.Package) {
	...
	for _, meth := range typ.AllMethods().Slice() {
		...
		if typ.IsInterface() {
			r.methodWrapper(typ, meth, false, target)
		} else {
			r.methodWrapper(typ, meth, false, target)
			r.methodWrapper(typ, meth, true, target)

			ptrTyp := types.NewPtr(typ)
			r.methodWrapper(ptrTyp, meth, false, target)
			r.methodWrapper(ptrTyp, meth, true, target)
		}
	}
}

func unifiedMethodWrapper(recv *types.Type, method *types.Field, forItab bool) *obj.LSym {
	tbase := recv
	if tbase.IsPtr() {
		assert(tbase.Sym() == nil)
		tbase = tbase.Elem()
		assert(!tbase.IsPtr())
	}

	wrapper := recv
	for forItab && !types.IsDirectIface(wrapper) {
		wrapper = types.NewPtr(wrapper)
	}

	suffix := ""
	if forItab && tbase.IsFullyInstantiated() {
		suffix = "-itab"
	}

	sym := ir.MethodSymSuffix(wrapper, method.Sym, suffix)
	sym.SetFunc(true)
	assert(sym.Siggen())
	return sym.Linksym()
}

func (r *reader) methodWrapper(wrapper *types.Type, method *types.Field, forItab bool, target *ir.Package) {
	derefs := 0
	...
	if (types.Identical(wrapper, wrappee) && suffix == "") ||
...
func addTailCall(pos src.XPos, fn *ir.Func, recv ir.Node, method *types.Field) {
	...
	callee := typecheck.Callee(ir.NewSelectorExpr(pos, ir.OXDOT, recv, method.Sym))
	call := ir.NewCallExpr(pos, ir.OCALL, callee, args)
	call.IsDDD = method.Type.IsVariadic()
	typecheck.Call(call)

	if owner := method.Type.Recv().Type; !owner.IsInterface() && owner.IsFullyInstantiated() && base.Debug.UnifiedDicts != 0 {
		assert(call.Op() == ir.OCALLFUNC)
		assert(call.X.Op() == ir.OMETHEXPR)

		if strings.HasSuffix(fn.Sym().Name, "-itab") && false {
			fmt.Printf("TODO: %v\n", fn)
		}

		call.X = dictWrapper(call.X.(*ir.SelectorExpr).FuncName())
	}
...
func dictWrapper(name *ir.Name) ir.Node {
	sym := name.Sym()
	lsym := sym.Pkg.Lookup(ir.FuncSymName(sym)).Linksym()
	...
	if len(lsym.P) == 0 {
		...
		off := 0
		off = objw.SymPtr(lsym, off, target, 0)
		off = objw.Uintptr(lsym, off, 0) // TODO(mdempsky): Pointer to dictionary.
		objw.Global(lsym, int32(off), obj.DUPOK|obj.RODATA)
	}

	// TODO(mdempsky): Have caller supply this.
	pos := base.AutogeneratedPos

	n := ir.NewLinksymExpr(pos, lsym, types.Types[types.TUINT8])
	addr := typecheck.Expr(typecheck.NodAddr(n)).(*ir.AddrExpr)
	return typecheck.ConvNop(addr, name.Type())
}
