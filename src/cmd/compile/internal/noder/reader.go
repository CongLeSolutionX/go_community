...
// dictPtr returns a pointer to the runtime dictionary variable for
// r.curfn.
func (r *reader) dictPtr() ir.Node {
	...
	if r.inlCall != nil {
		...
		fnSym = r.inlFunc.Nname.Sym()
	} else {
		fnSym = r.curfn.Nname.Sym()
	}
	sym := fnSym.Pkg.Lookup(fnSym.Name + "-dict")
...
func (r *reader) rtype(pos src.XPos) ir.Node {
	...
	return ir.NewConvExpr(pos, ir.OCONVNOP, types.NewPtr(types.Types[types.TUINT8]), r.dictWord(pos, int64(info.idx)))
}
