...
func (g *irgen) walkScope(scope *types2.Scope) bool {
	// go/types doesn't provide a proper API for determining the lexical
	// element a scope represents, so we have to resort to string
	// matching. Conveniently though, this allows us to skip both
	// function types and function literals, neither of which are
