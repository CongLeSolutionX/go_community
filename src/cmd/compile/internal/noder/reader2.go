...
func (r *reader2) doTyp() (res types2.Type) {
	...
	case typeTypeParam:
		idx := r.len()
		assert(r.dict != nil)
		if idx >= len(r.dict.tparams) {
			base.Fatalf("index %v is out of bounds for %v", idx, r.dict.tparams)
		}
		assert(idx < len(r.dict.tparams))
		return r.dict.tparams[idx].Type().(*types2.TypeParam)
...
func (r *reader2) typeParamNames() []*types2.TypeName {
	...
	r.dict.tparams = make([]*types2.TypeName, len(r.dict.bounds))
	for i := range r.dict.bounds {
		pos := r.pos()
		pkg, name := r.localIdent()
	...
	for i, bound := range r.dict.bounds {
		...
		r.dict.tparams[i].Type().(*types2.TypeParam).SetBound(boundType)
	}
