		...
		if nt2, ok := t2.(*types2.Named); ok {
			...
			if numMethods > 0 {
				...
				for i := range methods {
					...
					f := types.NewField(p.makeXPos(fun.Pos()), typecheck.Lookup(fun.Name()), ft)
					methods[i] = f
				}
...
func (p *noder) stmtFall(stmt syntax.Stmt, fallOK bool) ir.Node {
	...
	case *syntax.AssignStmt:
		if stmt.Op != 0 && stmt.Op != syntax.Def {
			...
			if base.Flag.G > 0 && n.IncDec && n.X.HasType2() {
...
// field for the receiver if we are converting a Signature. This is to avoid
// circularity for methods in interfaces.
func (p *noder) typeExpr2(typ types2.Type, forceRecv *types.Field) (r *types.Type) {
	// TODO(danscales) - make sure LocalPkg is correct in cases below.
	var TODO *types.Pkg = types.LocalPkg
	...
	fieldOf := func(v *types2.Var) *types.Field {
		...
		// TODO(danscales): fix Pos stuff, p.makeXPos(v.Pos()) gives error about
		// infinite recursion.
		return types.NewField(src.NoXPos /* p.makeXPos(v.Pos()) */, s, vt)
	}
	...
	case *types2.Named:
		...
		if obj.Pkg().Name() == p.file.PkgName.Value { /* current package */
	...
	case *types2.Basic:
		if k := typ.Kind(); uint64(k) < uint64(len(etypes)) {
			if k == types2.Uint8 && typ.Name() == "byte" {
				...
				// TODO(danscales) Do the same for rune alias?
	...
	case *types2.Signature:
		...
		return types.NewSignature(TODO, recv, in, out)
	case *types2.Map:
