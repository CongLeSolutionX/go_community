...
func LoadPackage(filenames []string) {
	...
	for i, filename := range filenames {
		...
		go func() {
			...
			defer func() { <-sem }()
			defer close(p.err)
			fbase := syntax.NewFileBase(filename, false)
...
func trimFilename(b *syntax.PosBase) string {
	...
	return objabi.AbsFile(base.Ctxt.Pathname, b.Filename(), base.Flag.TrimPath)
}
...
func isCgoGeneratedFile(pos syntax.Pos) bool {
	return strings.HasPrefix(filepath.Base(filepath.Clean(trimFilename(pos.Base()))), "_cgo_")
}
