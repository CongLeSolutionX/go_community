...
func (g *irgen) signature(recv *types.Field, sig *types2.Signature) *types.Type {
	do := func(result bool, typ *types2.Tuple) []*types.Field {
		fields := make([]*types.Field, typ.Len())
		for i := range fields {
			fields[i] = g.param(result, typ.At(i))
		}
		return fields
	}

	params := do(false, sig.Params())
	results := do(true, sig.Results())
	if sig.Variadic() {
...
func (g *irgen) param(result bool, v *types2.Var) *types.Field {
	n := g.obj(v)
	if result {
		n.Class = ir.PPARAMOUT
	} else {
		n.Class = ir.PPARAM
	}

	f := types.NewField(g.pos(v), g.sym(v), g.typ(v.Type()))
	f.Nname = n
	return f
}
