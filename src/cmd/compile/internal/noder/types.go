...
func (g *irgen) typ(typ types2.Type) *types.Type {
	...
	case *types2.Interface:
		...
		for i := range methods {
			...
			mtyp := g.signature(typecheck.FakeRecvField(), m.Type().(*types2.Signature))
...
func (g *irgen) field(v *types2.Var) *types.Field {
	return types.NewField(g.pos(v), g.selector(v), g.typ(v.Type()))
}
