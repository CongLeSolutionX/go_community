...
func (g *irgen) typ0(typ types2.Type) *types.Type {
	...
	case *types2.Named:
		...
		if typ.TParams() != nil && typ.TArgs() != nil {
			...
			// instantiated type.
			rparams := make([]*types.Type, typ.TArgs().Len())
			for i := range rparams {
				rparams[i] = g.typ1(typ.TArgs().At(i))
			}
...
func (g *irgen) fillinMethods(typ *types2.Named, ntyp *types.Type) {
	if typ.NumMethods() != 0 {
		targs := make([]*types.Type, typ.TArgs().Len())
		for i := range targs {
			targs[i] = g.typ1(typ.TArgs().At(i))
		}
...
func (g *irgen) signature(recv *types.Field, sig *types2.Signature) *types.Type {
	...
	for i := range tparams {
		tp := tparams2.At(i).(*types2.TypeParam).Obj()
