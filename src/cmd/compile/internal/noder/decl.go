...
func (g *irgen) obj(obj types2.Object) *ir.Name {
	...
	var name *ir.Name
	do := func(op ir.Op, ctxt ir.Class, defined *types2.TypeName) {
	...
	switch obj := obj.(type) {
	case *types2.Const:
		do(ir.OLITERAL, ir.PEXTERN, nil)
		name.SetVal(obj.Val())
	case *types2.Func:
		if obj.Name() == "init" && obj.Type().(*types2.Signature).Recv() == nil {
			sym = renameinit()
		}
		do(ir.ONAME, ir.PFUNC, nil)
	case *types2.TypeName:
		if obj.IsAlias() {
			do(ir.OTYPE, ir.PEXTERN, nil)
		} else {
			do(ir.OTYPE, ir.PEXTERN, obj)
		}
	case *types2.Var:
