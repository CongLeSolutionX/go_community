...
func makeInstName(inst *ir.InstExpr) *types.Sym {
	...
	for i, targ := range inst.Targs {
		...
		if i < len(inst.Targs)-1 {
			s += ","
		}
	}
	s += "]"
	return typecheck.Lookup(s)
}

// Struct containing info needed for doing the substitution as we create the
// instantiation of a generic function with specified type arguments.
type gensubster struct {
...
// genericSubst returns a new function which is an instantiation of a generic
// function with type params, as specified by inst. sym specifies the name of the
// new function.
func genericSubst(sym *types.Sym, inst *ir.InstExpr) *ir.Func {
...
// nodes.
func (subst *gensubster) node(n ir.Node) ir.Node {
	var edit func(ir.Node) ir.Node

	// We need a closure here (as opposed to a separate function) so that subst can
	// be available to the callees of EditChildren.
...
func (subst *gensubster) list(l []ir.Node) []ir.Node {
...
func (subst *gensubster) typ(t *types.Type) *types.Type {
...
func (subst *gensubster) fields(cl ir.Class, oldt *types.Type, dcl []*ir.Name) []*types.Field {
