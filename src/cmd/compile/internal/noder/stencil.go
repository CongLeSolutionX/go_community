...
func (subst *subster) getDictionaryType(pos src.XPos, t *types.Type) ir.Node {
	...
	typed(types.Types[types.TUINT8].PtrTo(), r) // TODO: correct type?
	return r
}
...
func (subst *subster) node(n ir.Node) ir.Node {
	...
	edit = func(x ir.Node) ir.Node {
		...
		case ir.OCONVIFACE:
			...
			// type parameters.
			if x.X.Type().Kind() == types.TTYPEPARAM {
				if !x.Type().IsEmptyInterface() {
					// This doesn't work at tip right now.
					panic("bad")
				}
				...
				if !x.Type().IsEmptyInterface() {
					// We just built an interface{}. Type it as such, then
					// cast it to the required non-empty interface.
					typed(types.NewInterface(types.LocalPkg, nil), m)
					m = ir.NewUnaryExpr(m.Pos(), ir.OCONVIFACE, m)
				}
				typed(x.Type(), m)
				// TODO: we're throwing away the type field of m, which probably
				// took some work to generate. Can we avoid generating it at all?
