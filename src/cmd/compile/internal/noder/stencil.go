...
func (subst *subster) node(n ir.Node) ir.Node {
	...
	edit = func(x ir.Node) ir.Node {
		...
		case ir.OXDOT:
			...
			} else if src.HasShape() && !src.IsInterface() {
				if mse.X.Op() == ir.OTYPE {
					m = subst.g.buildClosure2(subst.newf, subst.info, m, x)
					skip = true
				}
...
// remaining arguments. Otherwise, the receiver is an instantiation with a shape
// param. In that case, the body of closure just does a method call with the first
// argument as receiver and supplying the remaining args.
//
// The returned closure if fully substituted and has already has any needed
...
func (g *irgen) buildClosure2(outer *ir.Func, info *instInfo, m, x ir.Node) ir.Node {
	...
	rcvr := args[0]
	args = args[1:]
	if m.(*ir.SelectorExpr).X.Type().IsShape() {
		...
		assert(m.(*ir.SelectorExpr).X.Type().HasShape())
		rcvr := typecheck.AddImplicitDots(ir.NewSelectorExpr(base.Pos, ir.OXDOT, rcvr, x.(*ir.SelectorExpr).Sel)).X
		didStar := false
		if rcvr.Type().IsPtr() && !x.(*ir.SelectorExpr).Selection.Type.Recv().Type.IsPtr() {
			// For handling the (*T).M case: if we have a pointer
			// receiver after following all the embedded fields,
			// but it's a value method, add a star operator.
			t := rcvr.Type()
			rcvr = ir.NewStarExpr(pos, rcvr)
			rcvr.SetType(t.Elem())
			didStar = true
		}
		dot := ir.NewSelectorExpr(pos, ir.ODOTMETH, rcvr, x.(*ir.SelectorExpr).Sel)

		rfield := m.Type().Params().FieldSlice()[0]
		if didStar {
			rfield = types.NewField(rfield.Pos, rfield.Sym, rfield.Type.Elem())
		}
		typed(types.NewSignature(m.Type().Pkg(), rfield, nil, m.Type().Params().FieldSlice()[1:], m.Type().Results().FieldSlice()), dot)
		innerCall = ir.NewCallExpr(pos, ir.OCALLMETH, dot, args)
		cinfo := *info
		cinfo.dictParam = dictVar
		g.instInfoMap[fn.Sym()] = &cinfo
		for i, de := range info.gfInfo.subDictCalls {
			if de == x {
				// Mark the substituted node that is now the node
				// that will need this subdictionary.
				info.dictEntryMap[innerCall] = info.startSubDict + i
				break
			}
		}
	}
