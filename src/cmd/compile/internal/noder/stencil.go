...
// subst.vars, if Nname is in subst.vars. If forceNew, tstruct forces the
// creation of a new top-level type, regardless of whether anything changed with
// the types or names of the fields.
func (subst *subster) tstruct(t *types.Type, forceNew bool) *types.Type {
	if t.NumFields() == 0 {
		if t.HasTParam() {
...
func (subst *subster) typ(t *types.Type) *types.Type {
	...
	case types.TFUNC:
		...
		if newrecvs != t.Recvs() || newparams != t.Params() || newresults != t.Results() {
			// If any types have changed, then the top-level structs
			// of params and results must be copied, because they have
			// offset fields that are dependent, and so must have an
			// independent copy for each new signature. (recv is
			// always changed.)
