...
func (g *irgen) getInstantiation(nameNode *ir.Name, shapes []*types.Type, isMeth bool) *instInfo {
	...
	if info == nil {
		...
		info = &instInfo{
			dictInfo:      &dictInfo{},
			tparamToBound: make(map[*types.Type]*types.Type),
		}
...
func (g *irgen) genericSubst(newsym *types.Sym, nameNode *ir.Name, shapes []*types.Type, isMethod bool, info *instInfo) *ir.Func {
	...
	for i, tp := range tparams {
		info.tparamToBound[shapes[i]] = subst.ts.Typ(tp.Bound())
	}
...
// dictPass does takes a function instantiation and does the transformations on the
// operations that need to make use fo the dictionary param.
	...
	edit = func(m ir.Node) ir.Node {
		...
		case ir.OXDOT:
			...
			if mse.X.Op() == ir.OTYPE {
				...
				dst := info.tparamToBound[m.(*ir.SelectorExpr).X.Type()]
				if src.IsInterface() {
...
func findDictType(info *instInfo, t *types.Type) int {
	for i, dt := range info.dictInfo.tparams {
		if dt == t {
			return i
		}
	}
	for i, dt := range info.dictInfo.derivedTypes {
		if types.IdenticalStrict(dt, t) {
			return i + len(info.dictInfo.tparams)
		}
	}
	return -1
}
			...
			if call.X.Op() == ir.OXDOT || call.X.Op() == ir.ODOTMETH {
				...
				if se.X.Type().IsShape() {
					...
					if !tparam.IsShape() {
						...
						// really on a typearam.
						break
					}
					ix := -1
					for i, shape := range info.tparams {
						if shape == tparam {
							ix = i
							break
						}
	...
	for _, d := range g.dictSymsToFinalize {
		...
		for _, n := range info.itabConvs {
			...
			case ir.OXDOT, ir.OMETHVALUE:
				...
				dsttype = subst.Typ(instInfo.tparamToBound[se.X.Type()])
				found := false
...
func (g *irgen) getInstInfo(st *ir.Func, shapes []*types.Type, instInfo *instInfo) {
	...
	info.startSubDict = len(info.tparams) + len(info.derivedTypes)
	info.startItabConv = len(info.tparams) + len(info.derivedTypes) + len(info.subDictCalls)
	info.dictLen = len(info.tparams) + len(info.derivedTypes) + len(info.subDictCalls) + len(info.itabConvs)
}
