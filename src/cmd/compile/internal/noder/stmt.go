...
func (g *irgen) funcBody(fn *ir.Func, recv *syntax.Field, sig *syntax.FuncType, block *syntax.BlockStmt) {
	...
	for i, result := range typ.Results().FieldSlice() {
		...
		result.Sym = result.Nname.Sym() // in case it changed
	}
...
func (g *irgen) switchStmt(stmt *syntax.SwitchStmt) ir.Node {
	...
	for i, clause := range stmt.Body {
		...
		//
		// Also, override its position to the start of the
		// clause, so that dwarfgen can find the right scope
		// for it later. (TODO(mdempsky): We should probably
		// just store the scope directly in the ir.Name.)
		var cv *ir.Name
		if obj, ok := g.info.Implicits[clause]; ok {
			cv = g.obj(obj)
			cv.SetPos(g.pos(clause))
		}
