...
func (p *irgen) stmt(stmt syntax.Stmt) ir.Node {
	...
	case *syntax.BranchStmt:
		return ir.NewBranchStmt(p.pos(stmt), p.tokOp(int(stmt.Tok), branchOps[:]), p.name(stmt.Label))
	case *syntax.CallStmt:
		return ir.NewGoDeferStmt(p.pos(stmt), p.tokOp(int(stmt.Tok), callOps[:]), p.expr(stmt.Call))
...
func (p *irgen) tokOp(tok int, ops []ir.Op) ir.Op {
