...
func (w *writer) methodExpr(expr *syntax.SelectorExpr, embed types2.Type, sel *types2.Selection) {
	...
	if typeParam, ok := embed.(*types2.TypeParam); w.Bool(ok) {
		// Type parameter method expressions can be indexed by their type
		// parameter + constraint method index.
		//
		// Otherwise, we should have a named type available.
	...
	if isInterface(embed) != isInterface(sig.Recv().Type()) {
		w.p.fatalf(expr, "isInterface inconsistency: %v and %v", embed, sig.Recv().Type())
	}

	if !isInterface(embed) {
		if named, ok := deref2(embed).(*types2.Named); ok {
