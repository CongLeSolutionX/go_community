...
func (w *writer) pack(noders []*noder) {
	...
	for _, p := range noders {
		...
		syntax.Walk(p.file, func(n syntax.Node) bool {
			...
			case *syntax.ImportDecl:
				...
				switch path := pkgNameOf(w.p.info, n).Imported().Path(); path {
				case "embed":
					importedEmbed = true
				case "unsafe":
					importedUnsafe = true
	...
	w.len(len(w.p.curpkg.Imports()))
	for _, imp := range w.p.curpkg.Imports() {
		w.string(imp.Path())
	}
...
func (w *writer) useObj(obj types2.Object, explicits []types2.Type) {
	w.sync(syncUseObj)
	assert(obj.Name() != "_")
	assert(!isMethod(obj))
...
func (w *writer) selector(obj types2.Object) {
	...
	// TODO(mdempsky): In theory, we can omit pkg here, because it
	// should always be known from context. In practice, this was a
	// recurring sore point with iexport, so I'm a little wary to repeat
	// that here.
	w.pkg(obj.Pkg())
	w.string(obj.Name())
}
	...
	for i, elem := range lit.ElemList {
		if isStruct {
			if kv, ok := elem.(*syntax.KeyValueExpr); ok {
				...
				w.pos(kv.Key)
				w.len(w.fieldIndex(str, w.p.info.Uses[kv.Key.(*syntax.Name)].(*types2.Var)))
				elem = kv.Value
			} else {
				w.pos(elem)
				w.len(i)
...
func (w *writer) fieldIndex(str *types2.Struct, field *types2.Var) int {
	for i := 0; i < str.NumFields(); i++ {
		if str.Field(i) == field {
			return i
		}
	}
	w.p.fatalf(field, "%v not found in %v", field, str)
	panic("unreachable")
}
...
func (w *writer) rawObj(obj types2.Object) {
	...
	w.reloc(relocObj, w.p.globalIdx(obj, implicitIdx))
}

func (pw *pkgWriter) globalIdx(obj types2.Object, implicitIdx map[*types2.TypeParam]int) int {
	if idx, ok := pw.globalsIdx[obj]; ok {
		return idx
	}

	w := pw.newWriter(relocObj, syncObject1)
	w.bside = pw.newWriter(relocObjExt, syncObject1)
	assert(w.bside.idx == w.idx)

	pw.globalsIdx[obj] = w.idx

	w.implicitIdx = implicitIdx
	w.bside.implicitIdx = implicitIdx
...
func (w *writer) typeParams(tparams []*types2.TypeName) {
	...
	if len(tparams) != 0 {
		...
		w.bside.explicitIdx = explicitIdx
	}
...
func (w *writer) method(meth *types2.Func) {
	...
	w.explicitIdx = nil
	w.bside.explicitIdx = nil

	w.sync(syncMethod)
	w.pos(meth)
	w.selector(meth)
	w.typeParams(sig.RParams())
	w.param(sig.Recv())
	w.signature(sig)
...
func (w *writer) typ(typ types2.Type) {
	...
	if w.bool(len(w.implicitIdx)+len(w.explicitIdx) != 0) {
		w.doTyp(typ)
		return
	}
