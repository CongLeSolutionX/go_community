...
func (w *writer) switchStmt(stmt *syntax.SwitchStmt) {
	...
	if guard, ok := stmt.Tag.(*syntax.TypeSwitchGuard); w.Bool(ok) {
		...
		// The backend is going to emit comparisons between the tag value
		// and each case expression, and we want these comparisons to
		...
		for _, clause := range stmt.Body {
			for _, cas := range unpackListExpr(clause.Cases) {
				if casType := w.p.typeOf(cas); !types2.AssignableTo(casType, tagType) {
					w.p.warnf(stmt, "tricky switch statement: %v (type %v) with %v (type %v)", syntax.String(tag), tagType, syntax.String(cas), casType)
