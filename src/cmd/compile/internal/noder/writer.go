...
// sym writes out the name of an object declared at package scope.
// (For now, it's also used to refer to local defined types.)
...
func (w *writer) label(label *syntax.Name) {
	...
	// TODO(mdempsky): Replace label strings with indices.
...
func isGlobal(obj types2.Object) bool {
	return obj.Parent() == pkgScope(obj.Pkg())
}

// pkgScope returns the scope for the given package, or the universe
// scope if pkg is nil.
//
// TODO(mdempsky): Remove if #46594 is accepted.
func pkgScope(pkg *types2.Package) *types2.Scope {
	if pkg == nil {
		return types2.Universe
	}
	return pkg.Scope()
}
