...
func ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpARMLoweredNilCheck:
		...
		for _, w := range v.Block.Succs[0].Block().Values {
			...
			case ssa.OpARMDUFFZERO, ssa.OpARMLoweredZero, ssa.OpARMLoweredZero1:
				// arg0 is ptr
			...
			case ssa.OpARMDUFFCOPY, ssa.OpARMLoweredMove, ssa.OpARMLoweredMove1:
	...
	case ssa.OpARMLoweredZero,
		...
		sz := 4
		mov := arm.AMOVW
		if v.Op == ssa.OpARMLoweredZero1 { // unaligned
			sz = 1
			mov = arm.AMOVB
		}
		p := gc.Prog(mov)
	...
	case ssa.OpARMLoweredMove,
		...
		sz := 4
		mov := arm.AMOVW
		if v.Op == ssa.OpARMLoweredMove1 { // unaligned
			sz = 1
			mov = arm.AMOVB
		}
		p := gc.Prog(mov)
