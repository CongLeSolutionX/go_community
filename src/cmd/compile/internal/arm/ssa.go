...
// makeshift encodes a shift into an int64 used as Prog.Offset of obj.TYPE_SHIFT operands
func makeshift(reg int16, typ int64, shift int64) int64 {
	return int64(reg&0xf) | typ | (shift&31)<<7
}

// genshift generates a Prog for r = r0 op (r1 shifted by shift)
func genshift(as obj.As, r0, r1, r int16, typ int64, shift int64) *obj.Prog {
	p := gc.Prog(as)
	p.From.Type = obj.TYPE_SHIFT
	p.From.Offset = makeshift(r1, typ, shift)
	p.Reg = r0
	if r != 0 {
...
// makeregshift encodes a register shift into an int64 used as Prog.Offset of obj.TYPE_SHIFT operands
func makeregshift(r1 int16, typ int64, r2 int16) int64 {
	return int64(r1&0xf) | typ | int64(r2&0xf)<<8 | 1<<4
}
...
func genregshift(as obj.As, r0, r1, r2, r int16, typ int64) *obj.Prog {
	p := gc.Prog(as)
	p.From.Type = obj.TYPE_SHIFT
	p.From.Offset = makeregshift(r1, typ, r2)
	p.Reg = r0
	if r != 0 {
		p.To.Type = obj.TYPE_REG
		p.To.Reg = r
...
func ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpARMMOVWstoreshiftLL:
		...
		p.To.Reg = gc.SSARegNum(v.Args[0])
		p.To.Offset = makeshift(gc.SSARegNum(v.Args[1]), arm.SHIFT_LL, v.AuxInt)
	case ssa.OpARMMOVWstoreshiftRL:
		...
		p.To.Type = obj.TYPE_SHIFT
		p.To.Reg = gc.SSARegNum(v.Args[0])
		p.To.Offset = makeshift(gc.SSARegNum(v.Args[1]), arm.SHIFT_LR, v.AuxInt)
	case ssa.OpARMMOVWstoreshiftRA:
		...
		p.To.Type = obj.TYPE_SHIFT
		p.To.Reg = gc.SSARegNum(v.Args[0])
		p.To.Offset = makeshift(gc.SSARegNum(v.Args[1]), arm.SHIFT_AR, v.AuxInt)
