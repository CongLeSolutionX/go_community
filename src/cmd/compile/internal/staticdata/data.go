...
// InitAddr writes the static name symbol lsym to n, it does not modify n.
// It's the caller responsibility to make sure lsym is from ONAME/PEXTERN node.
func InitAddr(n *ir.Name, noff int64, lsym *obj.LSym, off int64) {
	if n.Op() != ir.ONAME {
...
// InitFunc writes the static address of f to n, it does not modify n.
// It's the caller responsibility to make sure lsym is from a global function.
func InitFunc(n *ir.Name, noff int64, lsym *obj.LSym) {
	if n.Op() != ir.ONAME {
