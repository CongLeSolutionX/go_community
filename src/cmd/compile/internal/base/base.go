...
// requested size before GC occurs.  Once this size is reached (approximately)
// GOGC will be reset to 100.  If GOGC is supplied as an environment variable,
// this function effect and the explicit request takes priority.
//
// Setting the environment variable GO_gc_LOG_HEAP_TWEAKS to a non-empty string
// will log GC-provoked GC adjustments to os.Stderr.
func AdjustStartingHeap(requestedHeapGoal uint64) {
	...
	// Go's GC has to guess at its initial heap limit (the point at which its first GC should complete)
	// and currently that guess is 4MiB.  It is currently possible that it could be as low as 0.5MiB.
	// The first GC's goal is also, currently, not adjustable, and it is not entirely clear when it
	// occurs, so this code deals with the possibility that the first adjustment did not "take" and
	// retries the GOGC adjustment.
	...
	for _, s := range sample {
		if s.Value.Kind() == metrics.KindBad {
			...
			if logHeapTweaks {
				fmt.Fprintf(os.Stderr, "AdjustStartingHeap: unexpected kind-bad for metric %s\n", s.Name)
			}
			return
		}
	}
	...
	var adjustFunc func(px *uint64)
	adjustFunc = func(px *uint64) {
		metrics.Read(sample)
		...
		if candidateGogc > 150 {
			...
			if logHeapTweaks {
				fmt.Fprintf(os.Stderr, "Retry GOGC adjust, current inuse %d, current goal %d, gogc was %d, is now %d, count is %d\n",
		...
		if logHeapTweaks {
			fmt.Fprintf(os.Stderr, "Reset GC, current inuse %d, old goal %d, gogc was %d, count is %d\n",
				inUse, goal, oldGogc, count)
		}
	}

	var x uint64 // the only purpose for x is to have a finalizer attached to it.
