...
func AdjustStartingHeap(requestedHeapGoal uint64) {
	// environment variable works for building Go bootstrap in make.bash, otherwise, use the command line flag.
	logHeapTweaks := Debug.GCAdjust == 1 || os.Getenv("GOgc_GCADJUST") != ""
	...
	adjustFunc := func() bool {
		...
		if goal < requestedHeapGoal*9/8 { // Stay the course
			if logHeapTweaks {
		...
		// Believe goal has been adjusted upwards, else it would be less than requestedHeapGoal
		...
		if 2*calcLive < requestedHeapGoal { // calcLive can exceed requestedHeapGoal!
			...
			if myGogc > 150 {
				...
				// Check that the new goal is safe enough.  3% of make.bash fails this test.  Why, TBD.
				if pctOff < 5 {
					if logHeapTweaks {
				...
				// retry adjust with smaller GOGC.
				pctOff0 := pctOff
				myGogc = (myGogc+100)*requestedHeapGoal/newGoal - 100
				if myGogc > 150 {
					oldGogc = debug.SetGCPercent(int(myGogc))
					metrics.Read(sample)
					newGoal = sample[GOAL].Value.Uint64()
					pctOff = 100 * (int64(newGoal) - int64(requestedHeapGoal)) / int64(requestedHeapGoal)
					if logHeapTweaks {
						// TODO: the reported goal is the same as the earlier one.  Is goal not adjusted, or is this a metrics bug?
						fmt.Fprintf(os.Stderr, "GCAdjust: Re-retry GOGC adjust, current goal %d, count is %d, gogc was %d, is now %d, calcLive %d pctOff0 %d pctOff %d\n",
							goal, count, oldGogc, myGogc, calcLive, pctOff0, pctOff)
					}
					return true
				}
			}
