...
func AdjustStartingHeap(requestedHeapGoal uint64) {
	...
	sample := []metrics.Sample{{Name: goal}, {Name: count}}
	const (
		GOAL  = 0
		COUNT = 1
	)
	...
	adjustFunc := func() bool {
		...
		if myGogc > 150 {
			...
			if logHeapTweaks {
				// check new goal against requested, just in case:
				metrics.Read(sample)
		...
		oldGogc := debug.SetGCPercent(100)
		if logHeapTweaks {
			fmt.Fprintf(os.Stderr, "GCAdjust: Reset GOGC adjust, old goal %d, count is %d, gogc was %d, calcLive %d\n",
				goal, count, oldGogc, calcLive)
		}
		return false
	}
