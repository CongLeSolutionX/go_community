...
func ParseFlags() {
	...
	*Flag.DwarfLocationLists = true
	...
	if Debug.LoopVarHash != "" {
		// This first little bit facilitates testing, nothing more.
		basenameOnly := false
		mostInlineOnly := true
		if len(Debug.LoopVarHash) > 2 && Debug.LoopVarHash[:2] == "FS" {
			// Magic handshake for testing, use file suffix only.
			Debug.LoopVarHash = Debug.LoopVarHash[2:]
			basenameOnly = true
		}
		if len(Debug.LoopVarHash) > 2 && Debug.LoopVarHash[:2] == "IL" {
			// Magic handshake for testing, use file suffix only.
			Debug.LoopVarHash = Debug.LoopVarHash[2:]
