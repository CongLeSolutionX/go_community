...
func (r *rewriter) exitVar(p syntax.Pos) (types2.Object, *syntax.VarDecl) {
	r.exitVarCount++

	name := fmt.Sprintf("#exit%d", r.exitVarCount)
	typ := r.bool.Type()
	obj := types2.NewVar(p, r.pkg, name, typ)
	n := syntax.NewName(p, name)
...
func (r *rewriter) editBranch(x *syntax.BranchStmt) syntax.Stmt {
	...
	for i := outerExited; i < len(r.forStack); i++ {
		bl.List = append(bl.List, r.setExitedAt(i))
	}
