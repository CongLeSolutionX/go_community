			...
			g(func() { // 3,4
				...
				h(func() { // 5,6
					...
					...
					{
						// break F
						#next = 2
						#state1, #state2, #state3 = DONE, DONE, DONE
						return false
					}
					...
					{
...
assigned a unique negative #next value (below -2, since -1 is return, and
-2 was previously used for "return v"). Then the post-checks for a given loop
			...
			g(func() {
				...
				h(func() {
					...
					...
					{
						// goto Top
						#next = -3
						#state3 = DONE // could also #state1, #state2 = DONE, DONE
						return false
					}
					...
					#state2 = READY
...
func (r *rewriter) checks(loop *forLoop, pos syntax.Pos) []syntax.Stmt {
	...
	if len(r.forStack) == 1 {
		...
		if loop.checkRet {
			...
			if r.checkFuncMisuse() {
				list = append(list, r.ifNext2(syntax.Lss, 0, r.setStateAt(curLoopIndex, DONE), retStmt(r.useObj(r.false)), false))
			} else {
				list = append(list, r.ifNext(syntax.Lss, 0, retStmt(r.useObj(r.false)), false))
			}
		}
		...
		if r.checkFuncMisuse() {
			if loop.checkContinue {
				...
				list = append(list, r.ifNext2(syntax.Eql, depthStep-1, r.setStateAt(curLoopIndex, READY), retStmt(r.useObj(r.true)), true))
			}

			if loop.checkBreak {
				// fmt.Printf("ifNext2 #next == %d {#state(%d)=DONE; return false}\n", depthStep, curLoopIndex)
				list = append(list, r.ifNext2(syntax.Eql, depthStep, r.setStateAt(curLoopIndex, DONE), retStmt(r.useObj(r.false)), true))
			}

			if loop.checkContinue || loop.checkBreak {
				list = append(list, r.ifNext2(syntax.Gtr, 0, r.setStateAt(curLoopIndex, DONE), retStmt(r.useObj(r.false)), false))
			}

		} else {
			if loop.checkContinue {
				list = append(list, r.ifNext(syntax.Eql, depthStep-1, retStmt(r.useObj(r.true)), true))
			}
			if loop.checkBreak {
				list = append(list, r.ifNext(syntax.Eql, depthStep, retStmt(r.useObj(r.false)), true))
			}
			if loop.checkContinue || loop.checkBreak {
				list = append(list, r.ifNext(syntax.Gtr, 0, retStmt(r.useObj(r.false)), false))
			}
		}
	}
