...
// which is specialized for scanning Go code: Reading
// ASCII characters, maintaining current (line, col)
// position information, and recording of the most
// recently read source segment is highly optimized.
...
// The source buffer is accessed via three indices b, r, and e:
// If the begin index b >= 0, it points to the beginning of a
// segment of most recently read characters (typically a Go
// literal).
// The read index r points to the byte immediately following
// the most recently read character ch, which starts at r-chw.
// The end index e points to the byte immediately following
// the last byte that was read into the buffer.
...
redo:
	...
	// slower general case
	// TODO(gri) review this (what if s.b >= 0?)
	for s.r+utf8.UTFMax > s.e && !utf8.FullRune(s.buf[s.r:s.e]) && s.ioerr == nil && s.e-s.r < len(s.buf) {
		s.fill() // s.e-s.r < len(s.buf) => buffer is not full
	}
...
func nextSize(size int) int {
	...
	const max = 1 << 20 // 1M: maximum buffer size which is stil doubled
	if size < min {
		return min
	}
	if size <= max {
