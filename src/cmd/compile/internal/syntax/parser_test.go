...
func TestLineDirectives(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"//line /src/a/a.go:123\n   foo", "syntax error: package statement must be first", "/src/a/a.go", 123 - linebase, 3},
		...
		if filename := perr.Pos.Base().Filename(); filename != test.filename {
			t.Errorf("%s: got filename = %q; want %q", test.src, filename, test.filename)
		}
		// TODO(gri) fix this test
