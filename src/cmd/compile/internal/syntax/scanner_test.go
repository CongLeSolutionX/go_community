...
func TestSmoke(t *testing.T) {
	...
	for _, want := range []token{_If, _Lparen, _Operator, _Name, _AssignOp, _Dot, _Literal, _Literal, _Literal, _Literal, _Literal, _Semi, _EOF} {
		got.next()
		if got.tok != want {
...
func TestScanErrors(t *testing.T) {
	for _, test := range []struct {
		...
		{`'\x'`, "invalid character '\\'' in hex escape sequence", 0, 3},
		{`'\y'`, "unknown escape sequence", 0, 2},
		{`'\x0'`, "invalid character '\\'' in hex escape sequence", 0, 4},
		{`'\00'`, "invalid character '\\'' in octal escape sequence", 0, 4},
		{`'\377' /*`, "comment not terminated", 0, 7}, // valid octal escape
		{`'\378`, "invalid character '8' in octal escape sequence", 0, 4},
		{`'\400'`, "octal escape sequence value 256 > 255", 0, 5},
		...
		{`"\x`, "string not terminated", 0, 0},
		{`"\x"`, "invalid character '\"' in hex escape sequence", 0, 3},
		{`var s string = "\x"`, "invalid character '\"' in hex escape sequence", 0, 18},
