...
redo:
	...
	s.line, s.col = s.pos()
	s.start()
	if isLetter(s.ch) || s.ch >= utf8.RuneSelf && s.isIdentRune(s.ch, true) {
		s.nextch()
		s.ident()
		return
	}
...
func (s *scanner) isIdentRune(ch rune, first bool) bool {
	switch {
	case unicode.IsLetter(ch) || ch == '_':
		// ok
	case unicode.IsDigit(ch):
		if first {
			s.errorf("identifier cannot begin with digit %#U", ch)
		}
	case ch >= utf8.RuneSelf:
		s.errorf("invalid character %#U in identifier", ch)
	default:
		return false
	}
	return true
...
func (s *scanner) escape(quote rune) {
	...
	for i := n; i > 0; i-- {
		...
		if d >= base {
			...
			s.errorf("invalid character %q in %s escape sequence", s.ch, kind)
			return
		}
		// d < base
		x = x*base + d
	...
	if x > max && base == 8 {
		s.errorf("octal escape sequence value %d > 255", x)
		return
	}
