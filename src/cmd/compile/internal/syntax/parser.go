...
func (p *parser) init(base *PosBase, r io.Reader, errh ErrorHandler, pragh PragmaHandler, mode Mode) {
	p.base = base
	p.errh = errh
	p.mode = mode
	p.scanner.init(
...
func (p *parser) updateBase(line, col uint, text string) {
	...
	// TODO(gri) pass column n2 to NewLineBase
	p.base = NewLineBase(MakePos(p.base.Pos().Base(), line, col), filename, uint(n) /*uint(n2)*/)
}
