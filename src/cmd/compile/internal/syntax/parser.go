...
func (p *parser) typeDecl(group *Group) Decl {
	...
	if p.allowGenerics() && p.tok == _Lbrack {
		...
		case _Name:
			...
			case *Operation:
				// If we have an expression of the form name*T, and T is a type literal
				// or the next token is a comma, we are at the start of a type parameter
				// list.
			...
			if pname != nil {
				...
				if p.tok == _Comma {
					// We have a "," but no type parameter list.
...
func isTypeLit(x Expr) bool {
	switch x := x.(type) {
	case *ArrayType, *SliceType, *StructType, *InterfaceType, *FuncType, *MapType, *ChanType:
		return true
	case *Operation: // pointer type
