...
func TestGoAMD64v1(t *testing.T) {
	...
	var features []string
	for feature, opcodeList := range feature2opcodes {
...
func clobber(t *testing.T, src string, dst *os.File, opcodes map[string]bool) {
	...
	for scanner.Scan() {
		...
		n := len(strings.ReplaceAll(parts[2], " ", "")) / 2 // number of bytes in instruction encoding
		for i := 0; i < n; i++ {
...
var feature2opcodes = map[string][]string{
	"popcnt": []string{"popcntq", "popcntl", "popcnt" /* no [ql] because of go tool objdump bug, see CL 351889 */},
	"bmi1":   []string{"andnq", "andnl", "blsiq", "blsil", "blsmskq", "blsmskl", "blsrq", "blsrl"},
	// TODO: more?
}
