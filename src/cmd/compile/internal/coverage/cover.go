...
func fixupMetaAndCounterVariables() (*ir.Name, *ir.Name, *ir.Func, coverage.CounterMode, coverage.CounterGranularity) {
	metaVarName := base.Flag.Cfg.CoverageInfo[coverage.MetaVarTag]
	pkgIdVarName := base.Flag.Cfg.CoverageInfo[coverage.PkgIdVarTag]
	counterMode := base.Flag.Cfg.CoverageInfo[coverage.CounterModeTag]
	counterGran := base.Flag.Cfg.CoverageInfo[coverage.CounterGranularityTag]
	counterPrefix := base.Flag.Cfg.CoverageInfo[coverage.CounterPrefixTag]
	var metavar *ir.Name
	...
	for _, n := range typecheck.Target.Decls {
		...
		case metaVarName:
			metavar = nm
			ckTypSanity(nm, coverage.MetaVarTag)
			nm.MarkReadonly()
			continue
		case pkgIdVarName:
			pkgidvar = nm
			ckTypSanity(nm, coverage.PkgIdVarTag)
		...
		if strings.HasPrefix(s.Name, counterPrefix) {
			ckTypSanity(nm, coverage.CounterVarTag)
	...
	case "count":
		cm = coverage.CtrModeCount
	case "atomic":
		cm = coverage.CtrModeAtomic
	case "regonly":
		cm = coverage.CtrModeRegOnly
	default:
...
func metaHashAndLen() ([16]byte, int) {
	...
	mhash := base.Flag.Cfg.CoverageInfo[coverage.MetaHashTag]
	if len(mhash) != 32 {
	...
	mls := base.Flag.Cfg.CoverageInfo[coverage.MetaLenTag]
...
func registerMeta(mdname *ir.Name, initfn *ir.Func, hash [16]byte, mdlen int, pkgIdVar *ir.Name, cmode coverage.CounterMode, cgran coverage.CounterGranularity) {
	...
	// runtime/coverage.initHook(), which will kick off the process
	// for coverage data writing (emit meta data, and register an exit
	// hook to emit counter data).
