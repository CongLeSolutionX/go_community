...
func Fixup() {
	...
	if covermode == coverage.CtrModeTestMain {
		// When building _testmain.go (as part of a "go test -cover"
		// run) we don't want any coverage-related init calls,
		// everything will be handled by code emitted by the Go
		// command, so make an early exit here.
		return
	}
