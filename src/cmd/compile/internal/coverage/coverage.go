...
func (c *covmgr) fixup(fn *ir.Func, funcId uint32, nCtrs uint32) {
	...
	// Emit a pseudo-op that will be expanded later on into something like
...
func (c *covmgr) counterGen(pos src.XPos) []ir.Node {
	...
	// Create a pseudo-op that will be expanded later on into something along the lines of
	//
	//   counter[k] = 1
	//
	k := len(c.units)
	...
	cup := ir.NewCoverCtrUpdateExpr(pos, c.counters, uint32(k+coverage.FirstCtrOffset))
	rval = append(rval, typecheck.Expr(cup))
	return rval
}
