...
func ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpARM64MOVBloadidx,
		...
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg()
		if v.Op == ssa.OpARM64MOVDloadidx8 {
			p.From.Index = arm64.REG_LSL | 3<<5 | v.Args[1].Reg()&31
		} else if v.Op == ssa.OpARM64MOVWloadidx4 || v.Op == ssa.OpARM64MOVWUloadidx4 {
			p.From.Index = arm64.REG_LSL | 2<<5 | v.Args[1].Reg()&31
		} else if v.Op == ssa.OpARM64MOVHloadidx2 || v.Op == ssa.OpARM64MOVHUloadidx2 {
			p.From.Index = arm64.REG_LSL | 1<<5 | v.Args[1].Reg()&31
		} else { // not shifted
	...
	case ssa.OpARM64MOVBstoreidx,
		...
		p.To.Reg = v.Args[0].Reg()
		if v.Op == ssa.OpARM64MOVDstoreidx8 {
			p.To.Index = arm64.REG_LSL | 3<<5 | v.Args[1].Reg()&31
		} else if v.Op == ssa.OpARM64MOVWstoreidx4 {
			p.To.Index = arm64.REG_LSL | 2<<5 | v.Args[1].Reg()&31
		} else if v.Op == ssa.OpARM64MOVHstoreidx2 {
			p.To.Index = arm64.REG_LSL | 1<<5 | v.Args[1].Reg()&31
		} else { // not shifted
	...
	case ssa.OpARM64MOVBstorezeroidx,
		...
		p.To.Name = obj.NAME_NONE
		p.To.Reg = v.Args[0].Reg()
		if v.Op == ssa.OpARM64MOVDstorezeroidx8 {
			p.To.Index = arm64.REG_LSL | 3<<5 | v.Args[1].Reg()&31
		} else if v.Op == ssa.OpARM64MOVWstorezeroidx4 {
			p.To.Index = arm64.REG_LSL | 2<<5 | v.Args[1].Reg()&31
		} else if v.Op == ssa.OpARM64MOVHstorezeroidx2 {
			p.To.Index = arm64.REG_LSL | 1<<5 | v.Args[1].Reg()&31
		} else { // not shifted
			p.To.Index = v.Args[1].Reg()
		}
