...
func canInstrumentGlobal(g ir.Node) bool {
	...
	// Do not instrument global symbols defined in packages which are not instrumented.
	for _, pkg := range base.NoInstrumentPkgs {
		if strings.HasPrefix(n.Sym().Linkname, pkg) {
			return false
		}
	}

	// Do not instrument buildcfg.defaultGOROOT, because it will be set to a final value by linker.
	if strings.Contains(n.Linksym().Name, "defaultGOROOT") && types.LocalPkg.Name == "buildcfg" {
		return false
	}
