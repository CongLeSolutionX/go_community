...
func instrumentGlobals(fn *ir.Func) *ir.Name {
	...
	// Because range over maps is nondeterministic, make a slice with enough capacity to
	// hold all the values and iterate over the slice here.
	instrumentGlobals := make([]ir.Node, 0, len(ShouldInstrumentGlobals))
	for _, n := range ShouldInstrumentGlobals {
		instrumentGlobals = append(instrumentGlobals, n)
	}
	sort.Slice(instrumentGlobals, func(i, j int) bool { return instrumentGlobals[i].Sym().Name < instrumentGlobals[j].Sym().Name })

	var init ir.Nodes
	var c ir.Node
	// globals[i].odrIndicator = 0 is the default, no need to set it explicitly here.
	for i, n := range instrumentGlobals {
...
// ShouldInstrumentGlobals contains only package-local (and unlinknamed from somewhere else)
// globals.
