...
func instrumentGlobals(fn *ir.Func) *ir.Name {
	asanGlobalStruct, asanLocationT, defStringstruct := createtypes()
	...
	// Make a global asanLocation variable.
	// var asanL asanLocation
	symL := lname(".asanL")
	asanlocation := typecheck.NewName(symL)
	asanlocation.SetType(asanLocationT)
	...
	// var asanSym string
	nn := typecheck.NewName(lname("asanSym"))
	...
	for symName, n := range ShouldInstrumentGlobals {
		...
		init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, nn, ir.NewString(symName+"\000"))))
		c = tconv(typecheck.NodAddr(nn), types.Types[types.TUNSAFEPTR])
		...
		// asanSym = pkgName + "\000"
		r := ir.NewAssignStmt(base.Pos, nn, ir.NewString(types.LocalPkg.Name+"\000"))
		init.Append(typecheck.Stmt(r))
		c = tconv(typecheck.NodAddr(nn), types.Types[types.TUNSAFEPTR])
		c = tconv(c, types.NewPtr(defStringstruct))
		c = ir.NewSelectorExpr(base.Pos, ir.ODOT, c, lname("data"))
		setField("moduleName", c, i)
		// Assign globals[i].location = uintptr(unsafe.Pointer(&asanL))
		// TODO: How do we get the file name here?
		// We can not get the file name here, so set the name of package being compiled.
		// asanL.filename = pkgName + "\000"
		init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, ir.NewSelectorExpr(base.Pos, ir.ODOT, asanlocation, lname("filename")), c)))
		line := ir.NewInt(int64(n.Pos().Line()))
		init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, ir.NewSelectorExpr(base.Pos, ir.ODOT, asanlocation, lname("line")), line)))
		col := ir.NewInt(int64(n.Pos().Col()))
		init.Append(typecheck.Stmt(ir.NewAssignStmt(base.Pos, ir.NewSelectorExpr(base.Pos, ir.ODOT, asanlocation, lname("column")), col)))
		c = tconv(typecheck.NodAddr(asanlocation), types.Types[types.TUNSAFEPTR])
		c = tconv(c, types.Types[types.TUINTPTR])
