	...
	if base.Flag.ASan {
		...
		for _, n := range typecheck.Target.Externs {
			if canInstrumentGlobal(n) {
				// Save the symbol's linkname when it has one.
				// When a global variable vv is marked "//go:linkname vv B.vv" in package A.
				// In pakcage A, its symbol name and link name are "B.vv".
				// In package B, its symbol name is "vv" and its link name is empty.
				// Use the same name "B.VV" because they are the same symbol.
				name := n.Sym().Linkname
				if name == "" {
