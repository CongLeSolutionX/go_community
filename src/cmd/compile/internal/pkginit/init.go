	...
	if base.Flag.ASan {
		...
		for _, n := range typecheck.Target.Externs {
			if canInstrumentGlobal(n) {
				// For linkname'd global variables, we may instrument them in the package in which
				// they are declared, when they are written to the object file, their size should
				// be the new size with the trailing red area. To ensure that when writing the
				// linkname'd symbols to the object file, their size is correct, set the "pkg.name"
				// as the key.
				// Because when a global variable vv is marked "//go:linkname vv B.vv" in package A.
				// In pakcage A, its symbol name and link name are "B.vv".
				// In package B, its symbol name is "vv" and its link name is empty.
				// Use the same key value "B.VV".
				name := types.LocalPkg.Name + "." + n.Sym().Name
				ShouldInstrumentGlobals[name] = n
			}
		}
		ni := len(ShouldInstrumentGlobals)
		if ni != 0 {
