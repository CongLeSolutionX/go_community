...
type stubUintptr struct {
	dwarf.Type
}

func (stubUintptr) RuntimeType(dwctxt interface{}) dwarf.Sym {
	return types.TypeSym(types.Types[types.TUINTPTR]).Linksym()
}

type stubEface struct {
	dwarf.Type
}

func (stubEface) Name(dwctxt interface{}) string {
	if base.Ctxt.Pkgpath == "runtime" {
		return `"".eface`
	}
	return `runtime.eface`
}
...
func (stubIface) Name(dwctxt interface{}) string {
	if base.Ctxt.Pkgpath == "runtime" {
		return `"".iface`
	}
	return `runtime.iface`
}
