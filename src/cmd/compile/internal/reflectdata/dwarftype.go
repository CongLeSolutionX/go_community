...
func (d DwarfType) Kind() int {
	return kinds[d.Type.Kind()]
}
...
func (d DwarfType) NumElem() int64 {
	...
	if d.Type.Kind() == types.TFUNC {
		if d.Type.Recv() != nil {
			base.Fatalf("misuse of method type: %v", d.Type)
		}
...
func (d DwarfType) FieldName(g dwarf.FieldsGroup, i int) string {
	...
	case dwarf.GroupParams:
		// Linker use the name of the parameter types as the name of parameters.
		// Keep this behaviour currently.
