	...
	case types.TARRAY:
		...
		checkAll := func(unroll int64, last bool, eq func(pi, qi ir.Node) ir.Node) {
			...
			if iterateTo > 0 {
				...
				post := ir.NewAssignStmt(base.Pos, i, ir.NewBinaryExpr(base.Pos, ir.OADD, i, ir.NewInt(unroll)))
				loop := ir.NewForStmt(base.Pos, nil, cond, post, nil)
				loop.PtrInit().Append(init)
				...
				for j := int64(0); j < unroll; j++ {
					...
					idxj := ir.NewBinaryExpr(base.Pos, ir.OADD, i, ir.NewInt(j))
					nif := ir.NewIfStmt(base.Pos, checkIdx(idxj), nil, nil)
