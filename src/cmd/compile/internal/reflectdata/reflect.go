...
func WriteType(t *types.Type) *obj.LSym {
	...
	case types.TFUNC:
		...
		for _, t1 := range t.Recvs().Fields().Slice() {
			ot = wobj.SymPtr(lsym, ot, WriteType(t1.Type), 0)
		}
		for _, t1 := range t.Params().Fields().Slice() {
			ot = wobj.SymPtr(lsym, ot, WriteType(t1.Type), 0)
		}
	...
	case types.TINTER:
		...
		for _, a := range m {
			...
			ot = wobj.SymPtrOff(lsym, ot, nsym)
			ot = wobj.SymPtrOff(lsym, ot, WriteType(a.type_))
		}
	...
	case types.TMAP:
		...
		ot = wobj.Uint16(lsym, ot, uint16(MapBucketType(t).Width))
	...
	case types.TSTRUCT:
		...
		for _, f := range fields {
			...
			ot = wobj.SymPtr(lsym, ot, WriteType(f.Type), 0)
...
func WriteTabs() {
	...
	for _, i := range itabs {
		...
		// }
		o := wobj.SymPtr(i.lsym, 0, WriteType(i.itype), 0)
		o = wobj.SymPtr(i.lsym, o, WriteType(i.t), 0)
