...
func writeType(t *types.Type) *obj.LSym {
	...
	// These are the fake types that we constructed for calling into the
	// builtin runtime package. The compiler will emit code for the real
	// ones while compiling package anyway, so we don't need to emit
	// anything here.
	if tbase == hmapType || tbase == hiterType {
		return lsym
	}
