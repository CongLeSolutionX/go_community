...
// assertOp asserts that n is an op1 or op2.
...
func ConvIfaceDataWordRType(pos src.XPos, n *ir.ConvExpr) ir.Node {
	...
	// TODO(mdempsky): Need to handle implicit interface conversions.
	if haveRType(n, n.DataWord, "DataWord", false) {
		return n.DataWord
	}
...
// MakeSliceRType asserts that n is a "make" operation for a slice
// type, and returns an expression that yields the *runtime._type
...
// MakeMapRType asserts that n is a "range" loop over a map value, and
// returns an expression that yields the *runtime._type value
// representing that map type.
