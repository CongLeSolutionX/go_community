...
func swissTableType() *types.Type {
	...
	if size := int64(3*2 + 2*1 /* one extra for padding */ + 2*8 + 4*types.PtrSize); table.Size() != size {
...
func SwissMapType() *types.Type {
	...
	if size := int64(2*8 + 7*types.PtrSize /* two extra for globalDepth + writing + padding */); m.Size() != size {
