...
func swissTableType() *types.Type {
	...
	if size := int64(3*2 + 2*1 /* one extra for padding */ + 1*8 + 4*types.PtrSize); table.Size() != size {
...
func SwissMapIterType() *types.Type {
	...
	//    tab *table
	//
	//    group unsafe.Pointer // actually groupReference
	//
	//    entryIdx uint64
	//
