...
// function fn and identigies a list of candidate variables for
...
func (cs *cstate) collectMergeCandidates() {
	...
	// This may be empy if we don't have enough compatible candidates.
...
func (cs *cstate) setupHashTrace(cands []*ir.Name) {
	...
	// candidates are selected-- if yes then enable tracing.
	// Hack: create a new hashdebug with verbosity turned off
	// and use that to test, so as not to confuse bisect.
...
func (cs *cstate) populateIndirectUseTable(cands []*ir.Name) ([]*ir.Name, []candRegion) {
	...
	for k := 0; k < len(cs.f.Blocks); k++ {
		...
		ids = ids[:0]
		for i := range blockIndirectUE {
			ids = append(ids, i)
		}
		sort.Slice(ids, func(i, j int) bool { return ids[i] < ids[j] })
		if indirectUE == nil {
			indirectUE = MakeIndUETab(cands, len(cands))
		}
		for _, id := range ids {
			sl := blockIndirectUE[id]
			for _, v := range sl {
				if evicted[v] {
					continue
				}
				if err := indirectUE.Add(id, v); err != nil {
					base.FatalfAt(cs.fn.Pos(), "internal error on indirectUE.Add: %v", err)
				}
				if cs.trace > 2 {
					fmt.Fprintf(os.Stderr, "=-= add final indUE v%d name %s\n", id, v.Sym().Name)
				}
			}
			if err := indirectUE.Finalize(id); err != nil {
				base.FatalfAt(cs.fn.Pos(), "internal error on indirectUE.Finalize: %v", err)
			}
		}
	}
	if len(rawcands) < 2 {
	...
	for k := len(lv.f.Blocks) - 1; k >= 0; k-- {
		...
		for i := len(b.Values) - 1; i >= 0; i-- {
			...
			if cs.indirectUE != nil {
				...
				var err error
				uetmp, err = cs.indirectUE.Get(v.ID, uetmp)
				if err != nil {
					base.FatalfAt(v.Pos, "indirect UE: %v", err)
				}
				for _, n := range uetmp {
