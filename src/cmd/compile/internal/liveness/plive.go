...
type Map struct {
	Vals map[ssa.ID]wobj.LivenessIndex
	// The set of live, pointer-containing variables at the deferreturn
	// call (only set when open-coded defers are used).
	DeferReturn wobj.LivenessIndex
}

func (m *Map) reset() {
	if m.Vals == nil {
		m.Vals = make(map[ssa.ID]wobj.LivenessIndex)
	} else {
		for k := range m.Vals {
			delete(m.Vals, k)
		}
	}
	m.DeferReturn = wobj.LivenessDontCare
}

func (m *Map) set(v *ssa.Value, i wobj.LivenessIndex) {
	m.Vals[v.ID] = i
}

func (m Map) Get(v *ssa.Value) wobj.LivenessIndex {
	// If v isn't in the map, then it's a "don't care" and not an
	// unsafe-point.
	if idx, ok := m.Vals[v.ID]; ok {
		return idx
	}
	return wobj.LivenessIndex{StackMapIndex: wobj.StackMapDontCare, IsUnsafePoint: false}
}
...
func newliveness(fn *ir.Func, f *ssa.Func, vars []*ir.Name, idx map[*ir.Name]int32, stkptrsize int64) *liveness {
	...
	if lc, _ := f.Cache.Liveness.(*livenessFuncCache); lc == nil {
		...
		lv.livenessMap = Map{Vals: lc.livenessMap.Vals, DeferReturn: wobj.LivenessDontCare}
...
func (lv *liveness) markUnsafePoints() {
	...
	for _, wbBlock := range lv.f.WBLoads {
		...
		for {
			if sym, ok := v.Aux.(*obj.LSym); ok && sym == types.Syms.WriteBarrier {
				load = v
				break
			}
			switch v.Op {
...
func (lv *liveness) epilogue() {
	...
	if lv.fn.OpenCodedDeferDisallowed() {
		lv.livenessMap.DeferReturn = wobj.LivenessDontCare
	} else {
		lv.livenessMap.DeferReturn = wobj.LivenessIndex{
...
func (lv *liveness) compact(b *ssa.Block) {
	...
	for _, v := range b.Values {
		...
		idx := wobj.LivenessIndex{StackMapIndex: wobj.StackMapDontCare, IsUnsafePoint: isUnsafePoint}
		if hasStackMap {
...
func (lv *liveness) emit() (argsSym, liveSym *obj.LSym) {
	...
	aoff := wobj.Uint32(&argsSymTmp, 0, uint32(len(lv.stackMaps))) // number of bitmaps
	aoff = wobj.Uint32(&argsSymTmp, aoff, uint32(args.N))          // number of bits in each bitmap

	locals := bitvec.New(int32(maxLocals / int64(types.PtrSize)))
	loff := wobj.Uint32(&liveSymTmp, 0, uint32(len(lv.stackMaps))) // number of bitmaps
	loff = wobj.Uint32(&liveSymTmp, loff, uint32(locals.N))        // number of bits in each bitmap
	...
	for _, live := range lv.stackMaps {
		...
		aoff = wobj.BitVec(&argsSymTmp, aoff, args)
		loff = wobj.BitVec(&liveSymTmp, loff, locals)
	}
...
func Compute(curfn *ir.Func, f *ssa.Func, stkptrsize int64, pp *wobj.Progs) Map {
...
func WriteFuncMap(fn *ir.Func) {
	...
	off := wobj.Uint32(lsym, 0, uint32(nbitmap))
	off = wobj.Uint32(lsym, off, uint32(bv.N))
	...
	wobj.Global(lsym, int32(off), obj.RODATA|obj.LOCAL)
}
