...
func walkstmt(n ir.Node) ir.Node {
	...
	case ir.ODCL:
		...
		if v.Class_ == ir.PAUTOHEAP {
			...
			nn := ir.NewAssignStmt(base.Pos, v.Name().Heapaddr, prealloc[v])
			nn.Def = true
	...
	case ir.OGO:
		n := n.(*ir.GoDeferStmt)
		var init ir.Nodes
		switch left := n.Call; left.Op() {
		case ir.OPRINT, ir.OPRINTN:
			left := left.(*ir.CallExpr)
			n.Call = wrapCall(left, &init)

		case ir.ODELETE:
			left := left.(*ir.CallExpr)
			if mapfast(left.Args.First().Type()) == mapslow {
				n.Call = wrapCall(left, &init)
			} else {
				n.Call = walkexpr(left, &init)
			}

		case ir.OCOPY:
			left := left.(*ir.BinaryExpr)
			n.Call = copyany(left, &init, true)
	...
	case ir.OSELECT:
		walkselect(n.(*ir.SelectStmt))
		return n

	case ir.OSWITCH:
		walkswitch(n.(*ir.SwitchStmt))
		return n
...
func walkexpr1(n ir.Node, init *ir.Nodes) ir.Node {
	...
	case ir.ORECOVER:
		n := n.(*ir.CallExpr)
		return mkcall("gorecover", n.Type(), init, ir.NewAddrExpr(base.Pos, nodfp))
	...
	case ir.OCALLINTER, ir.OCALLFUNC, ir.OCALLMETH:
		...
		if n.Op() == ir.OCALLFUNC && n.X.Op() == ir.OCLOSURE {
			...
			n.Args.Prepend(n.X.Func().ClosureEnter.Slice()...)
			n.X.Func().ClosureEnter.Set(nil)
	...
	case ir.OAS, ir.OASOP:
		...
		if mapAppend != nil && !samesafeexpr(left, mapAppend.Args.First()) {
		...
		case ir.ORECV:
			...
			recv := n.Y.(*ir.UnaryExpr)
			recv.X = walkexpr(recv.X, init)

			n1 := ir.NewAddrExpr(base.Pos, n.X)
		...
		case ir.OAPPEND:
			...
			case isAppendOfMake(r):
				// x = append(y, make([]T, y)...)
				n.Y = extendslice(r, init)
			case r.DDD:
				n.Y = appendslice(r, init) // also works for append(slice, string).
			default:
				n.Y = walkappend(r, init, n)
			}
			if r.Op() == ir.OAPPEND {
				// Left in place for back end.
				// Do not add a new write barrier.
				// Set up address of type for back end.
				r.X = typename(r.Type().Elem())
				return n
			}
	...
	case ir.OAS2RECV:
		...
		if ir.IsBlank(n.Lhs.First()) {
			...
			n1 = ir.NewAddrExpr(base.Pos, n.Lhs.First())
		}
	...
	case ir.OAS2MAPR:
		...
		if fast != mapslow {
			...
			key = ir.NewAddrExpr(base.Pos, r.Index)
		}
	...
	case ir.ODELETE:
		...
		if fast == mapslow {
			...
			key = ir.NewAddrExpr(base.Pos, key)
		}
	...
	case ir.OCONVIFACE:
		...
		if value != nil {
			...
			l := ir.NewBinaryExpr(base.Pos, ir.OEFACE, typeword(), typecheck(ir.NewAddrExpr(base.Pos, value), ctxExpr))
			l.SetType(toType)
			l.SetTypecheck(n.Typecheck())
			return l
		}
	...
	case ir.OSLICE, ir.OSLICEARR, ir.OSLICESTR, ir.OSLICE3, ir.OSLICE3ARR:
		...
		if checkPtr(Curfn, 1) && n.Op() == ir.OSLICE3ARR && n.X.Op() == ir.OCONVNOP && n.X.(*ir.ConvExpr).X.Type().IsUnsafePtr() {
			conv := n.X.(*ir.ConvExpr)
			conv.X = walkexpr(conv.X, init)
			n.X = walkCheckPtrAlignment(conv, init, max)
		} else {
			n.X = walkexpr(n.X, init)
		}
	...
	case ir.ONEW:
		...
		if n.Esc() == EscNone {
			...
			return typecheck(ir.NewAddrExpr(base.Pos, r), ctxExpr)
		}
	...
	case ir.ORUNESTR:
		...
		if n.Esc() == EscNone {
			...
			a = ir.NewAddrExpr(base.Pos, temp(t))
		}
...
func varexpr(n ir.Node) bool {
	...
	case ir.OADD, ir.OSUB, ir.OOR, ir.OXOR, ir.OMUL, ir.ODIV, ir.OMOD, ir.OLSH, ir.ORSH, ir.OAND, ir.OANDNOT:
		n := n.(*ir.BinaryExpr)
		return varexpr(n.X) && varexpr(n.Y)
	...
	case ir.OCONV, ir.OCONVNOP, ir.OCONVIFACE:
		n := n.(*ir.ConvExpr)
		return varexpr(n.X)
...
func refersToName(name *ir.Name, r ir.Node) bool {
	return ir.Find(r, func(r ir.Node) bool {
...
func addstr(n *ir.AddStringExpr, init *ir.Nodes) ir.Node {
	...
	if n.Esc() == EscNone {
		...
		if sz < tmpstringbufsize {
			...
			t := types.NewArray(types.Types[types.TUINT8], tmpstringbufsize)
			buf = ir.NewAddrExpr(base.Pos, temp(t))
		}
	}
	...
	if c <= 5 {
		...
		slice := ir.NewCompLitExpr(base.Pos, ir.OCOMPLIT, ir.TypeNode(t).(ir.Ntype), nil)
		if prealloc[n] != nil {
...
func appendslice(n *ir.CallExpr, init *ir.Nodes) ir.Node {
	...
	if elemtype.HasPointers() {
		...
		ix.SetBounded(true)
		addr := ir.NewAddrExpr(base.Pos, ix)
...
func isAppendOfMake(n ir.Node) bool {
	...
	call := n.(*ir.CallExpr)
	if !call.DDD || call.Args.Len() != 2 || call.Args.Second().Op() != ir.OMAKESLICE {
		return false
	}
...
func extendslice(n *ir.CallExpr, init *ir.Nodes) ir.Node {
	...
	ix := ir.NewIndexExpr(base.Pos, s, ir.NewUnaryExpr(base.Pos, ir.OLEN, l1))
	ix.SetBounded(true)
	hp := convnop(ir.NewAddrExpr(base.Pos, ix), types.Types[types.TUNSAFEPTR])
...
func walkcompare(n *ir.BinaryExpr, init *ir.Nodes) ir.Node {
	...
	if !inline {
		...
		fn, needsize := eqfor(t)
		call := ir.NewCallExpr(base.Pos, ir.OCALL, fn, nil)
		call.Args.Append(ir.NewAddrExpr(base.Pos, cmpl))
		call.Args.Append(ir.NewAddrExpr(base.Pos, cmpr))
		if needsize {
