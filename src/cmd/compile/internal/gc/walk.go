...
func walkstmtlist(s []ir.INode) {
	for i := range s {
		s[i] = walkstmt(s[i])
	}
}

func paramoutheap(fn ir.INode) bool {
	for _, ln := range fn.Func().Dcl {
		switch ln.Class() {
		case ir.PPARAMOUT:
...
func walkstmt(n ir.INode) ir.INode {
	if n == nil {
		return n
	}
	...
	case ir.ODCL:
		...
		if v.Class() == ir.PAUTOHEAP {
			...
			nn := nod(ir.OAS, v.Name().Param.Heapaddr, prealloc[v])
			nn.SetColas(true)
	...
	case ir.OGO:
		...
		case ir.OPRINT, ir.OPRINTN:
			n.SetLeft(wrapCall(n.Left(), n.PtrNinit()))

		case ir.ODELETE:
			if mapfast(n.Left().List().First().Type()) == mapslow {
				n.SetLeft(wrapCall(n.Left(), n.PtrNinit()))
			} else {
				n.SetLeft(walkexpr(n.Left(), n.PtrNinit()))
			}

		case ir.OCOPY:
			n.SetLeft(copyany(n.Left(), n.PtrNinit(), true))
	...
	case ir.ORETURN:
		...
		if (Curfn.Type().FuncType().Outnamed && n.List().Len() > 1) || paramoutheap(Curfn) {
			...
			for _, ln := range Curfn.Func().Dcl {
				...
				if cl == ir.PPARAMOUT {
					if isParamStackCopy(ln) {
						ln = walkexpr(typecheck(nod(ir.ODEREF, ln.Name().Param.Heapaddr, nil), ctxExpr), nil)
					}
					rl = append(rl, ln)
				}
			}
			...
			walkexprlistsafe(n.List().Slice(), n.PtrNinit())

			ll := ascompatee(n.Op(), rl, n.List().Slice(), n.PtrNinit())
			n.PtrList().Set(reorder3(ll))
			break
		}
		walkexprlist(n.List().Slice(), n.PtrNinit())

		// For each return parameter (lhs), assign the corresponding result (rhs).
		lhs := Curfn.Type().Results()
		rhs := n.List().Slice()
		res := make([]ir.INode, lhs.NumFields())
		for i, nl := range lhs.FieldSlice() {
			nname := ir.AsNode(nl.Nname)
			if isParamHeapCopy(nname) {
				nname = nname.Name().Param.Stackcopy
			}
			a := nod(ir.OAS, nname, rhs[i])
...
func walkexprlist(s []ir.INode, init *ir.Nodes) {
	for i := range s {
		s[i] = walkexpr(s[i], init)
	}
}

func walkexprlistsafe(s []ir.INode, init *ir.Nodes) {
	for i, n := range s {
		s[i] = safeexpr(n, init)
		s[i] = walkexpr(s[i], init)
	}
...
func walkexprlistcheap(s []ir.INode, init *ir.Nodes) {
	for i, n := range s {
...
func walkexpr(n ir.INode, init *ir.Nodes) ir.INode {
	if n == nil {
		return n
	}
...
opswitch:
	...
	case ir.OAS, ir.OASOP:
		...
		if n.Op() == ir.OASOP {
			...
			n.SetRight(nod(n.SubOp(), n.Left(), n.Right()))
	...
	case ir.OAS2MAPR:
		...
		if !a.IsBlank() {
			...
			n = walkexpr(n, init)
			init.Append(n)
			n = nod(ir.OAS, a, nod(ir.ODEREF, var_, nil))
		}
	...
	case ir.OCONVIFACE:
		...
		if value != nil {
			...
			l := nod(ir.OEFACE, typeword(), typecheck(nod(ir.OADDR, value, nil), ctxExpr))
			l.SetType(toType)
			l.SetTypecheck(n.Typecheck())
			n = l
			break
		...
		if toType.IsEmptyInterface() && fromType.IsInterface() && !fromType.IsEmptyInterface() {
			...
			init.Append(nod(ir.OAS, tmp, typecheck(nod(ir.OITAB, c, nil), ctxExpr)))

			// Get the type out of the itab.
			nif := nod(ir.OIF, typecheck(nod(ir.ONE, tmp, nodnil()), ctxExpr), nil)
			nif.PtrNbody().Set1(nod(ir.OAS, tmp, itabType(tmp)))
			init.Append(nif)
	...
	case ir.OINDEX:
		...
		if ir.IsConst(n.Right(), ir.CTINT) {
			if n.Right().Val().U.(*ir.Int).CmpInt64(0) < 0 || n.Right().Val().U.(*ir.Int).Cmp(maxintval[types.TINT]) > 0 {
				base.Error("index out of bounds")
			}
		}
	...
	case ir.OMAKECHAN:
		...
		if size.Type().IsKind(types.TIDEAL) || maxintval[size.Type().Etype].Cmp(maxintval[types.TUINT]) <= 0 {
			fnname = "makechan"
	...
	case ir.OMAKEMAP:
		...
		if n.Esc() == EscNone {
			...
			// var hv hmap
			hv := temp(hmapType)
			zero := nod(ir.OAS, hv, nil)
			zero = typecheck(zero, ctxStmt)
			init.Append(zero)
			// h = &hv
			h = nod(ir.OADDR, hv, nil)
			...
			if !ir.IsConst(hint, ir.CTINT) ||
				hint.Val().U.(*ir.Int).CmpInt64(BUCKETSIZE) <= 0 {
				...
				nif := nod(ir.OIF, nod(ir.OLE, hint, nodintconst(BUCKETSIZE)), nil)
				nif.SetLikely(true)

				// var bv bmap
				bv := temp(bmap(t))
				zero = nod(ir.OAS, bv, nil)
				nif.PtrNbody().Append(zero)

				// b = &bv
				b := nod(ir.OADDR, bv, nil)

				// h.buckets = b
				bsym := hmapType.Field(5).Sym // hmap.buckets see reflect.go:hmap
				na := nod(ir.OAS, nodSym(ir.ODOT, h, bsym), b)
				nif.PtrNbody().Append(na)
		...
		if ir.IsConst(hint, ir.CTINT) && hint.Val().U.(*ir.Int).CmpInt64(BUCKETSIZE) <= 0 {
			...
			if n.Esc() == EscNone {
				...
				a := nod(ir.OAS, nodSym(ir.ODOT, h, hashsym), rand)
			...
			if hint.Type().IsKind(types.TIDEAL) || maxintval[hint.Type().Etype].Cmp(maxintval[types.TUINT]) <= 0 {
				fnname = "makemap"
				argtype = types.Types[types.TINT]
			}
	...
	case ir.OMAKESLICE:
		...
		if n.Esc() == EscNone {
			...
			//     panicmakeslicecap()
			// }
			nif := nod(ir.OIF, nod(ir.OGT, conv(l, types.Types[types.TUINT64]), nodintconst(i)), nil)
			niflen := nod(ir.OIF, nod(ir.OLT, l, nodintconst(0)), nil)
			niflen.PtrNbody().Set1(mkcall("panicmakeslicelen", nil, init))
			nif.PtrNbody().Append(niflen, mkcall("panicmakeslicecap", nil, init))
			nif = typecheck(nif, ctxStmt)
			init.Append(nif)

			t = types.NewArray(t.Elem(), i) // [r]T
			var_ := temp(t)
			a := nod(ir.OAS, var_, nil) // zero temp
			a = typecheck(a, ctxStmt)
			init.Append(a)
			r := nod(ir.OSLICE, var_, nil) // arr[:l]
			...
			if (len.Type().IsKind(types.TIDEAL) || maxintval[len.Type().Etype].Cmp(maxintval[types.TUINT]) <= 0) &&
				(cap.Type().IsKind(types.TIDEAL) || maxintval[cap.Type().Etype].Cmp(maxintval[types.TUINT]) <= 0) {
				fnname = "makeslice"
				argtype = types.Types[types.TINT]
			}

			m := nod(ir.OSLICEHEADER, nil, nil)
			m.SetType(t)
	...
	case ir.OMAKESLICECOPY:
		...
		if !t.Elem().HasPointers() && n.Bounded() {
			...
			size := nod(ir.OMUL, conv(length, types.Types[types.TUINTPTR]), conv(nodintconst(t.Elem().Width), types.Types[types.TUINTPTR]))
			...
			s := temp(t)
			r := typecheck(nod(ir.OAS, s, sh), ctxStmt)
			r = walkexpr(r, init)
			init.Append(r)
			...
			ncopy := mkcall1(fn, nil, init, nod(ir.OSPTR, s, nil), copyptr, size)
	...
	case ir.OSTR2BYTES:
		...
		if ir.IsConst(s, ir.CTSTR) {
			...
			init.Append(typecheck(nod(ir.OAS, p, a), ctxStmt))

			// Copy from the static string data to the [n]byte.
			if len(sc) > 0 {
				as := nod(ir.OAS,
					nod(ir.ODEREF, p, nil),
					nod(ir.ODEREF, convnop(nod(ir.OSPTR, s, nil), t.PtrTo()), nil))
...
func markUsedIfaceMethod(n ir.INode) {
...
func reduceSlice(n ir.INode) ir.INode {
	low, high, max := n.SliceBounds()
...
func ascompatee1(l ir.INode, r ir.INode, init *ir.Nodes) ir.INode {
...
func ascompatee(op ir.Op, nl, nr []ir.INode, init *ir.Nodes) []ir.INode {
...
func fncall(l ir.INode, rt *types.Type) bool {
...
// an expression list. called in
//	expr-list = func()
func ascompatet(nl ir.Nodes, nr *types.Type) []ir.INode {
...
func mkdotargslice(typ *types.Type, args []ir.INode) ir.INode {
	var n ir.INode
	if len(args) == 0 {
		n = nodnil()
		n.SetType(typ)
	} else {
		n = nod(ir.OCOMPLIT, nil, typenod(typ))
...
func fixVariadicCall(call ir.INode) {
...
func walkCall(n ir.INode, init *ir.Nodes) {
	if n.Rlist().Len() != 0 {
	...
	if n.Op() == ir.OCALLMETH {
		withRecv := make([]ir.INode, len(args)+1)
...
func walkprint(nn ir.INode, init *ir.Nodes) ir.INode {
	...
	s := nn.List().Slice()
	t := make([]ir.INode, 0, len(s))
	for i := 0; i < len(s); {
		var strs []string
		for i < len(s) && ir.IsConst(s[i], ir.CTSTR) {
	...
	calls := []ir.INode{mkcall("printlock", nil, init)}
	...
	r := nod(ir.OEMPTY, nil, nil)
	r = typecheck(r, ctxStmt)
	r = walkexpr(r, init)
	r.PtrNinit().Set(calls)
	return r
}

func callnew(t *types.Type) ir.INode {
	dowidth(t)
	n := nod(ir.ONEWOBJ, typename(t), nil)
...
func isReflectHeaderDataField(l ir.INode) bool {
...
func convas(n ir.INode, init *ir.Nodes) ir.INode {
	if n.Op() != ir.OAS {
...
func reorder3(all []ir.INode) []ir.INode {
...
func reorder3save(n ir.INode, all []ir.INode, i int, early *[]ir.INode) ir.INode {
	if !aliased(n, all[:i]) {
		return n
	}
...
func outervalue(n ir.INode) ir.INode {
	for {
		switch n.Op() {
		case ir.OXDOT:
...
func aliased(r ir.INode, all []ir.INode) bool {
	if r == nil {
		return false
	}
...
func vmatch2(l ir.INode, r ir.INode) bool {
	if r == nil {
		return false
	}
	switch r.Op() {
...
// called by sinit.go
func vmatch1(l ir.INode, r ir.INode) bool {
...
func paramstoheap(params *types.Type) []ir.INode {
	var nn []ir.INode
	for _, t := range params.Fields().Slice() {
		...
		if stackcopy := v.Name().Param.Stackcopy; stackcopy != nil {
			nn = append(nn, walkstmt(nod(ir.ODCL, v, nil)))
			if stackcopy.Class() == ir.PPARAM {
				nn = append(nn, walkstmt(typecheck(nod(ir.OAS, v, stackcopy), ctxStmt)))
			}
		}
	}
...
// back to the stack.
func returnsfromheap(params *types.Type) []ir.INode {
	var nn []ir.INode
	for _, t := range params.Fields().Slice() {
		v := ir.AsNode(t.Nname)
		if v == nil {
		...
		if stackcopy := v.Name().Param.Stackcopy; stackcopy != nil && stackcopy.Class() == ir.PPARAMOUT {
			nn = append(nn, walkstmt(typecheck(nod(ir.OAS, stackcopy, v), ctxStmt)))
		}
	}
...
func vmkcall(fn ir.INode, t *types.Type, init *ir.Nodes, va []ir.INode) ir.INode {
...
func mkcall(name string, t *types.Type, init *ir.Nodes, args ...ir.INode) ir.INode {
	return vmkcall(syslook(name), t, init, args)
}

func mkcall1(fn ir.INode, t *types.Type, init *ir.Nodes, args ...ir.INode) ir.INode {
	return vmkcall(fn, t, init, args)
}

func conv(n ir.INode, t *types.Type) ir.INode {
...
func convnop(n ir.INode, t *types.Type) ir.INode {
	if types.Identical(n.Type(), t) {
		return n
	}
...
func byteindex(n ir.INode) ir.INode {
...
func chanfn(name string, n int, t *types.Type) ir.INode {
	if !t.IsChan() {
...
func mapfn(name string, t *types.Type) ir.INode {
	if !t.IsMap() {
...
func mapfndel(name string, t *types.Type) ir.INode {
	if !t.IsMap() {
		base.Fatal("mapfn %v", t)
	}
	fn := syslook(name)
...
func writebarrierfn(name string, l *types.Type, r *types.Type) ir.INode {
	fn := syslook(name)
	fn = substArgTypes(fn, l, r)
	return fn
}

func addstr(n ir.INode, init *ir.Nodes) ir.INode {
	...
	if c <= 5 {
		...
		slice := nod(ir.OCOMPLIT, nil, typenod(t))
		if prealloc[n] != nil {
			prealloc[slice] = prealloc[n]
		}
		slice.PtrList().Set(args[1:]) // skip buf arg
		args = []ir.INode{buf, slice}
		slice.SetEsc(EscNone)
	}

	cat := syslook(fn)
	r := nod(ir.OCALL, cat, nil)
	r.PtrList().Set(args)
	r = typecheck(r, ctxExpr)
	r = walkexpr(r, init)
	r.SetType(n.Type())
...
func walkAppendArgs(n ir.INode, init *ir.Nodes) {
	walkexprlistsafe(n.List().Slice(), init)
...
func appendslice(n ir.INode, init *ir.Nodes) ir.INode {
	...
	// var s []T
	s := temp(l1.Type())
	nodes.Append(nod(ir.OAS, s, l1)) // s = l1

	elemtype := s.Type().Elem()

	// n := len(s) + len(l2)
	nn := temp(types.Types[types.TINT])
	nodes.Append(nod(ir.OAS, nn, nod(ir.OADD, nod(ir.OLEN, s, nil), nod(ir.OLEN, l2, nil))))
	...
	// s = growslice(T, s, n)
	nif.PtrNbody().Set1(nod(ir.OAS, s, mkcall1(fn, s.Type(), nif.PtrNinit(), typename(elemtype), s, nn)))
	nodes.Append(nif)

	// s = s[:n]
	nt := nod(ir.OSLICE, s, nil)
	nt.SetSliceBounds(nil, nn, nil)
	nt.SetBounded(true)
	nodes.Append(nod(ir.OAS, s, nt))

	var ncopy ir.INode
	if elemtype.HasPointers() {
		// copy(s[len(l1):], l2)
		nptr1 := nod(ir.OSLICE, s, nil)
		nptr1.SetType(s.Type())
		nptr1.SetSliceBounds(nod(ir.OLEN, l1, nil), nil, nil)
		...
		// memmove(&s[len(l1)], &l2[0], len(l2)*sizeof(T))
		nptr1 := nod(ir.OINDEX, s, nod(ir.OLEN, l1, nil))
		nptr1.SetBounded(true)
		nptr1 = nod(ir.OADDR, nptr1, nil)

		nptr2 := nod(ir.OSPTR, l2, nil)

		nwid := cheapexpr(conv(nod(ir.OLEN, l2, nil), types.Types[types.TUINTPTR]), &nodes)
		nwid = nod(ir.OMUL, nwid, nodintconst(elemtype.Width))
...
func isAppendOfMake(n ir.INode) bool {
...
//   }
//   s
func extendslice(n ir.INode, init *ir.Nodes) ir.INode {
	...
	nifneg := nod(ir.OIF, nod(ir.OGE, l2, nodintconst(0)), nil)
	...
	// s := l1
	s := temp(l1.Type())
	nodes = append(nodes, nod(ir.OAS, s, l1))

	elemtype := s.Type().Elem()

	// n := len(s) + l2
	nn := temp(types.Types[types.TINT])
	nodes = append(nodes, nod(ir.OAS, nn, nod(ir.OADD, nod(ir.OLEN, s, nil), l2)))

	// if uint(n) > uint(cap(s))
	nuint := conv(nn, types.Types[types.TUINT])
	capuint := conv(nod(ir.OCAP, s, nil), types.Types[types.TUINT])
	nif := nod(ir.OIF, nod(ir.OGT, nuint, capuint), nil)
	...
	nt.SetSliceBounds(nil, nn, nil)
	nt.SetBounded(true)
	nodes = append(nodes, nod(ir.OAS, s, nt))

	// lptr := &l1[0]
	l1ptr := temp(l1.Type().Elem().PtrTo())
	tmp := nod(ir.OSPTR, l1, nil)
	nodes = append(nodes, nod(ir.OAS, l1ptr, tmp))

	// sptr := &s[0]
	sptr := temp(elemtype.PtrTo())
	tmp = nod(ir.OSPTR, s, nil)
	nodes = append(nodes, nod(ir.OAS, sptr, tmp))

	// hp := &s[len(l1)]
	hp := nod(ir.OINDEX, s, nod(ir.OLEN, l1, nil))
	hp.SetBounded(true)
	hp = nod(ir.OADDR, hp, nil)
	hp = convnop(hp, types.Types[types.TUNSAFEPTR])

	// hn := l2 * sizeof(elem(s))
	hn := nod(ir.OMUL, l2, nodintconst(elemtype.Width))
...
//   }
//   s
func walkappend(n ir.INode, init *ir.Nodes, dst ir.INode) ir.INode {
	...
	ns := temp(nsrc.Type())
	l = append(l, nod(ir.OAS, ns, nsrc)) // s = src
	...
	nx.PtrNbody().Set1(nod(ir.OAS, ns,
		mkcall1(fn, ns.Type(), nx.PtrNinit(), typename(ns.Type().Elem()), ns,
			nod(ir.OADD, nod(ir.OLEN, ns, nil), na))))

	l = append(l, nx)

	nn := temp(types.Types[types.TINT])
	l = append(l, nod(ir.OAS, nn, nod(ir.OLEN, ns, nil))) // n = len(s)

	nx = nod(ir.OSLICE, ns, nil) // ...s[:n+argc]
	nx.SetSliceBounds(nil, nod(ir.OADD, nn, na), nil)
	nx.SetBounded(true)
	l = append(l, nod(ir.OAS, ns, nx)) // s = s[:n+argc]

	ls = n.List().Slice()[1:]
	for i, n := range ls {
		nx = nod(ir.OINDEX, ns, nn) // s[n] ...
		nx.SetBounded(true)
		l = append(l, nod(ir.OAS, nx, n)) // s[n] = arg
		if i+1 < len(ls) {
			l = append(l, nod(ir.OAS, nn, nod(ir.OADD, nn, nodintconst(1)))) // n = n + 1
		}
	}
...
//
func copyany(n ir.INode, init *ir.Nodes, runtimecall bool) ir.INode {
	...
	nto := nod(ir.OSPTR, nl, nil)

	nlen := temp(types.Types[types.TINT])

	// n = len(to)
	l = append(l, nod(ir.OAS, nlen, nod(ir.OLEN, nl, nil)))

	// if n > len(frm) { n = len(frm) }
	nif := nod(ir.OIF, nil, nil)

	nif.SetLeft(nod(ir.OGT, nlen, nod(ir.OLEN, nr, nil)))
	nif.PtrNbody().Append(nod(ir.OAS, nlen, nod(ir.OLEN, nr, nil)))
	l = append(l, nif)

	// if to.ptr != frm.ptr { memmove( ... ) }
	ne := nod(ir.OIF, nod(ir.ONE, nto, nfrm), nil)
	ne.SetLikely(true)
	l = append(l, ne)
	...
	setwid := nod(ir.OAS, nwid, conv(nlen, types.Types[types.TUINTPTR]))
	ne.PtrNbody().Append(setwid)
	nwid = nod(ir.OMUL, nwid, nodintconst(nl.Type().Elem().Width))
	call := mkcall1(fn, nil, init, nto, nfrm, nwid)
	ne.PtrNbody().Append(call)
...
func eqfor(t *types.Type) (n ir.INode, needsize bool) {
...
func walkcompare(n ir.INode, init *ir.Nodes) ir.INode {
	...
	if !inline {
		...
		fn, needsize := eqfor(t)
		call := nod(ir.OCALL, fn, nil)
		call.PtrList().Append(nod(ir.OADDR, cmpl, nil))
		call.PtrList().Append(nod(ir.OADDR, cmpr, nil))
		if needsize {
			call.PtrList().Append(nodintconst(t.Width))
		}
		res := call
		if n.Op() != ir.OEQ {
			res = nod(ir.ONOT, res, nil)
		}
	...
	if t.IsStruct() {
		...
		for i := int64(0); remains > 0; {
			...
			if step == 1 {
				compare(
					nod(ir.OINDEX, cmpl, nodintconst(i)),
					nod(ir.OINDEX, cmpr, nodintconst(i)),
				)
				i++
				remains -= t.Elem().Width
			} else {
				elemType := t.Elem().ToUnsigned()
				cmplw := nod(ir.OINDEX, cmpl, nodintconst(i))
				cmplw = conv(cmplw, elemType) // convert to unsigned
				cmplw = conv(cmplw, convType) // widen
				cmprw := nod(ir.OINDEX, cmpr, nodintconst(i))
				cmprw = conv(cmprw, elemType)
				cmprw = conv(cmprw, convType)
				// For code like this:  uint32(s[0]) | uint32(s[1])<<8 | uint32(s[2])<<16 ...
				// ssa will generate a single large load.
				for offset := int64(1); offset < step; offset++ {
					lb := nod(ir.OINDEX, cmpl, nodintconst(i+offset))
					lb = conv(lb, elemType)
					lb = conv(lb, convType)
					lb = nod(ir.OLSH, lb, nodintconst(8*t.Elem().Width*offset))
					cmplw = nod(ir.OOR, cmplw, lb)
					rb := nod(ir.OINDEX, cmpr, nodintconst(i+offset))
					rb = conv(rb, elemType)
					rb = conv(rb, convType)
					rb = nod(ir.OLSH, rb, nodintconst(8*t.Elem().Width*offset))
					cmprw = nod(ir.OOR, cmprw, rb)
				}
				compare(cmplw, cmprw)
				i += step
	...
	if expr == nil {
		...
		t := temp(cmpl.Type())
		a1 := nod(ir.OAS, t, cmpl)
		a1 = typecheck(a1, ctxStmt)
		a2 := nod(ir.OAS, t, cmpr)
...
func tracecmpArg(n ir.INode, t *types.Type, init *ir.Nodes) ir.INode {
...
func walkcompareInterface(n ir.INode, init *ir.Nodes) ir.INode {
	n.SetRight(cheapexpr(n.Right(), init))
	n.SetLeft(cheapexpr(n.Left(), init))
	...
	if n.Op() == ir.OEQ {
		...
		eqtab.SetOp(ir.ONE)
		cmp = nod(ir.OOROR, eqtab, nod(ir.ONOT, eqdata, nil))
	}
	return finishcompare(n, cmp, init)
}

func walkcompareString(n ir.INode, init *ir.Nodes) ir.INode {
	// Rewrite comparisons to short constant strings as length+byte-wise comparisons.
	var cs, ncs ir.INode // const string, non-const string
	switch {
	...
	if cs != nil {
		...
		if s := cs.StringVal(); len(s) <= maxRewriteLen {
			...
			r := nod(cmp, nod(ir.OLEN, ncs, nil), nodintconst(int64(len(s))))
			remains := len(s)
			for i := 0; remains > 0; {
				if remains == 1 || !canCombineLoads {
					cb := nodintconst(int64(s[i]))
					ncb := nod(ir.OINDEX, ncs, nodintconst(int64(i)))
				...
				ncsubstr := nod(ir.OINDEX, ncs, nodintconst(int64(i)))
				...
				for offset := 1; offset < step; offset++ {
					b := nod(ir.OINDEX, ncs, nodintconst(int64(i+offset)))
					b = conv(b, convType)
					b = nod(ir.OLSH, b, nodintconst(int64(8*offset)))
					ncsubstr = nod(ir.OOR, ncsubstr, b)
	...
	if n.Op() == ir.OEQ || n.Op() == ir.ONE {
		...
		if n.Op() == ir.OEQ {
			...
			r = nod(ir.OANDAND, eqlen, eqmem)
		} else {
			// len(left) != len(right) || !memequal(left, right, len)
			eqlen.SetOp(ir.ONE)
			r = nod(ir.OOROR, eqlen, nod(ir.ONOT, eqmem, nil))
		}
	} else {
...
func finishcompare(n, r ir.INode, init *ir.Nodes) ir.INode {
	r = typecheck(r, ctxExpr)
	r = conv(r, n.Type())
	r = walkexpr(r, init)
	return r
...
func bounded(n ir.INode, max int64) bool {
...
func candiscard(n ir.INode) bool {
	if n == nil {
		return true
	}
	...
	case ir.ODIV, ir.OMOD:
		if ir.IsConst(n.Right(), ir.CTINT) && n.Right().Val().U.(*ir.Int).CmpInt64(0) != 0 {
			break
		}
		if ir.IsConst(n.Right(), ir.CTFLT) && n.Right().Val().U.(*ir.Float).CmpFloat64(0) != 0 {
			break
		}
		return false
...
func wrapCall(n ir.INode, init *ir.Nodes) ir.INode {
	if n.Ninit().Len() != 0 {
		walkstmtlist(n.Ninit().Slice())
		init.AppendNodes(n.PtrNinit())
	}
	...
	origArgs := make([]ir.INode, n.List().Len())
...
func isRuneCount(n ir.INode) bool {
	return base.Flag.N == 0 && !instrumenting && n.Op() == ir.OLEN && n.Left().Op() == ir.OSTR2RUNES
}

func walkCheckPtrAlignment(n ir.INode, init *ir.Nodes, count ir.INode) ir.INode {
	if !n.Type().IsPtr() {
...
func walkCheckPtrArithmetic(n ir.INode, init *ir.Nodes) ir.INode {
	...
	var originals []ir.INode
	var walk func(n ir.INode)
	walk = func(n ir.INode) {
		switch n.Op() {
		case ir.OADD:
			walk(n.Left())
			walk(n.Right())
...
// levels.
func checkPtr(fn ir.INode, level int) bool {
