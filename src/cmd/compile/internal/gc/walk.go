...
opswitch:
	...
	case ir.OCONVIFACE:
		...
		if toType.IsEmptyInterface() && fromType.IsInterface() && !fromType.IsEmptyInterface() {
			...
			nif.PtrNbody().Set1(ir.Nod(ir.OAS, tmp, itabType(tmp)))
			init.Append(nif)
...
func appendslice(n ir.INode, init *ir.Nodes) ir.INode {
	...
	// s = growslice(T, s, n)
	nif.PtrNbody().Set1(ir.Nod(ir.OAS, s, mkcall1(fn, s.Type(), nif.PtrNinit(), typename(elemtype), s, nn)))
	nodes.Append(nif)
...
func walkappend(n ir.INode, init *ir.Nodes, dst ir.INode) ir.INode {
	...
	nx.PtrNbody().Set1(ir.Nod(ir.OAS, ns,
...
func copyany(n ir.INode, init *ir.Nodes, runtimecall bool) ir.INode {
	...
	nif.PtrNbody().Append(ir.Nod(ir.OAS, nlen, ir.Nod(ir.OLEN, nr, nil)))
	l = append(l, nif)
