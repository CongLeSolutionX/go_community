...
opswitch:
	...
	case ir.OINDEX:
		...
		if ir.IsConst(n.Right(), constant.Int) {
			if n.Right().Val().U.(*ir.Int).CmpInt64(0) < 0 || doesoverflow(n.Right().Val(), types.Types[types.TINT]) {
				base.Errorf("index out of bounds")
			}
		}
	...
	case ir.OMAKEMAP:
		...
		if ir.IsConst(hint, constant.Int) && hint.Val().U.(*ir.Int).CmpInt64(BUCKETSIZE) <= 0 {
...
func candiscard(n *ir.Node) bool {
	...
	case ir.ODIV, ir.OMOD:
		if ir.IsConst(n.Right(), constant.Int) && n.Right().Val().U.(*ir.Int).CmpInt64(0) != 0 {
			break
		}
		if ir.IsConst(n.Right(), constant.Float) && n.Right().Val().U.(*ir.Float).CmpFloat64(0) != 0 {
			break
		}
		return false
	...
	case ir.OMAKECHAN, ir.OMAKEMAP:
		if ir.IsConst(n.Left(), constant.Int) && n.Left().Val().U.(*ir.Int).CmpInt64(0) == 0 {
			break
		}
		return false
