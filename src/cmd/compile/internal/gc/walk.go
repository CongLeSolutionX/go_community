...
func walkstmt(n ir.Node) ir.Node {
	...
	case ir.OAS,
		...
		if wascopy && n.Op() == ir.ONAME {
			...
			e := ir.NodAt(n.Pos(), ir.OEMPTY, nil, nil)
			e.SetInit(n.Init())
			n = e
		}
...
func walkcompare(n ir.Node, init *ir.Nodes) ir.Node {
	...
	cmpl := n.Left()
	for cmpl != nil && (cmpl.Op() == ir.OCONVNOP || cmpl.Op() == ir.OSTMTEXPR) {
		cmpl = cmpl.Left()
	}
	cmpr := n.Right()
	for cmpr != nil && (cmpr.Op() == ir.OCONVNOP || cmpr.Op() == ir.OSTMTEXPR) {
		cmpr = cmpr.Left()
	}
