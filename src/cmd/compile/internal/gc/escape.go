...
func (e *Escape) tagHole(ks []EscHole, param *types.Field, direct bool, where *Node) EscHole {
	...
	const escPrefix = "esc:0x"
	if !strings.HasPrefix(tag, escPrefix) {
		Fatalf("weird tag: %q", tag)
	}
	esc, err := strconv.ParseUint(tag[len(escPrefix):], 16, 16)
	if err != nil {
...
func (l *EscLocation) leak(sink *EscLocation, derefs int) {
	...
	// otherwise, pessimistically record as a leak to the heap.
	ri := -1
	...
	//
	// We have a few bits per result parameter to encode the leak
	// distance. Zero indicates no leak edge, whereas 1+x
	// indicates a leak edge of length x.
