...
type EscLocation struct {
	n         ir.INode  // represented variable or expression, if any
	curfn     ir.INode  // enclosing function
...
func escFmt(n ir.INode, short bool) string {
	text := ""
	switch n.Esc() {
	case EscUnknown:
		break
...
// functions.
func escapeFuncs(fns []ir.INode, recursive bool) {
	for _, fn := range fns {
...
func (e *Escape) initFunc(fn ir.INode) {
...
func (e *Escape) walkFunc(fn ir.INode) {
...
func (e *Escape) stmt(n ir.INode) {
	...
	switch n.Op() {
	default:
		base.Fatal("unexpected stmt: %v", n)
...
// hole k.
func (e *Escape) expr(k EscHole, n ir.INode) {
	if n == nil {
		return
	}
	e.stmts(n.Ninit())
...
func (e *Escape) exprSkipInit(k EscHole, n ir.INode) {
	if n == nil {
		return
	}
	...
	case ir.OCALLPART:
		...
		paramK := e.tagHole(ks, ir.AsNode(m.Type.Nname()), m.Type.Recv())
...
func (e *Escape) unsafeValue(k EscHole, n ir.INode) {
...
// its value.
func (e *Escape) discard(n ir.INode) {
...
func (e *Escape) addr(n ir.INode) EscHole {
	if n == nil || n.IsBlank() {
	...
	switch n.Op() {
	default:
		base.Fatal("unexpected addr: %v", n)
	case ir.ONAME:
...
func (e *Escape) assign(dst, src ir.INode, why string, where ir.INode) {
...
func (e *Escape) assignHeap(src ir.INode, why string, where ir.INode) {
...
func (e *Escape) call(ks []EscHole, call, where ir.INode) {
	...
	argument := func(k EscHole, arg ir.INode) {
		if topLevelDefer {
	...
	switch call.Op() {
	default:
		base.Fatal("unexpected call op: %v", call.Op())
...
// if any.
func (e *Escape) tagHole(ks []EscHole, fn ir.INode, param *types.Field) EscHole {
...
func (e *Escape) inMutualBatch(fn ir.INode) bool {
...
func (k EscHole) note(where ir.INode, why string) EscHole {
	if where == nil || why == "" {
		base.Fatal("note: missing where/why")
	}
...
func (k EscHole) shift(delta int) EscHole {
	k.derefs += delta
	if k.derefs < -1 {
		base.Fatal("derefs underflow: %v", k.derefs)
	}
	return k
}

func (k EscHole) deref(where ir.INode, why string) EscHole { return k.shift(1).note(where, why) }
func (k EscHole) addr(where ir.INode, why string) EscHole  { return k.shift(-1).note(where, why) }

func (k EscHole) dotType(t *types.Type, where ir.INode, why string) EscHole {
...
func (e *Escape) teeHole(ks ...EscHole) EscHole {
	...
	for _, k := range ks {
		...
		// instead.
		if k.derefs < 0 {
			base.Fatal("teeHole: negative derefs")
		}
...
func (e *Escape) dcl(n ir.INode) EscHole {
	loc := e.oldLoc(n)
...
func (e *Escape) spill(k EscHole, n ir.INode) EscHole {
	loc := e.newLoc(n, true)
...
// represents.
func canonicalNode(n ir.INode) ir.INode {
...
func (e *Escape) newLoc(n ir.INode, transient bool) *EscLocation {
	if e.curfn == nil {
...
func (e *Escape) oldLoc(n ir.INode) *EscLocation {
	n = canonicalNode(n)
...
func (e *Escape) explainPath(root, src *EscLocation) []*logopt.LoggedOpt {
	...
	for {
		...
		if edge.src != src {
			base.Fatal("path inconsistency: %v != %v", edge.src, src)
		}
...
func containsClosure(f, c ir.INode) bool {
	if f.Op() != ir.ODCLFUNC || c.Op() != ir.ODCLFUNC {
		base.Fatal("bad containsClosure: %v, %v", f, c)
	}
...
func (e *Escape) finish(fns []ir.INode) {
...
func (l *EscLeaks) set(i, derefs int) {
	v := derefs + 1
	if v < 0 {
		base.Fatal("invalid derefs count: %v", derefs)
	}
	if v > math.MaxUint8 {
		v = math.MaxUint8
	}
