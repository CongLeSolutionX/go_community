...
func (e *Escape) call(ks []EscHole, call, where *Node) {
	...
	if call.Op == OCALLFUNC {
		...
		e.expr(e.callStmt(e.discardHole(), where), call.Left)
	}

	if recv != nil {
		// TODO(mdempsky): Handle go:uintptrescapes here too?
		e.expr(e.callStmt(recvK, where), recv)
	}

	// Apply callStmt before ODDDARG so that it affects the
	// implicit slice allocation for variadic calls, if any.
	for i, paramK := range paramKs {
		paramKs[i] = e.callStmt(paramK, where)
	}
	...
	for i, arg := range args {
		...
		// no callStmt here; handled in loop before ODDDARG
		e.expr(paramKs[i], arg)
	}
}

// callStmt augments parameter holes as necessary for use in go/defer
// statements.
func (e *Escape) callStmt(k EscHole, where *Node) EscHole {
	if where == nil {
		return k
	}
