...
func evconst(n *Node) {
	...
	case OPLUS, OMINUS, OCOM, ONOT:
		if nl.Op == OLITERAL {
			setconst(n, unaryOp(n.Op, nl.Val(), n.Type))
		}

	case OADD, OSUB, OMUL, ODIV, OMOD, OOR, OXOR, OAND, OANDNOT, OOROR, OANDAND:
		if nl.Op == OLITERAL && nr.Op == OLITERAL {
			setconst(n, binaryOp(nl.Val(), n.Op, nr.Val()))
		}
	...
	case OEQ, ONE, OLT, OLE, OGT, OGE:
		if nl.Op == OLITERAL && nr.Op == OLITERAL {
			if nl.Type.IsInterface() != nr.Type.IsInterface() {
				...
				setboolconst(n, compareOp(nl.Val(), n.Op, nr.Val()))
			}
		}

	case OLSH, ORSH:
		if nl.Op == OLITERAL && nr.Op == OLITERAL {
			setconst(n, shiftOp(nl.Val(), n.Op, nr.Val()))
		}
