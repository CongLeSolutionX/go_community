...
func toint(v Val) Val {
	...
	case *Mpflt:
		...
		if i.SetFloat(u) < 0 {
			...
			// evaluation of the rhs of the assignment (9.99...99e-01)
			// is too long to be fully printed in an error message,
			// and cannot be shortened without making the error
			// message misleading (rounding it would result in a "1",
			// which is an integer constant, and it makes little sense
			// in an error message about an invalid floating point
			// expression).
			//
