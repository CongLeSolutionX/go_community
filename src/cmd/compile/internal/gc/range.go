...
func walkrange(nrange *ir.RangeStmt) ir.Node {
	...
	case types.TARRAY, types.TSLICE:
		...
		tmp.SetBounded(true)
		init = append(init, ir.NewAssignStmt(base.Pos, hp, ir.NewAddrExpr(base.Pos, tmp)))
	...
	case types.TMAP:
		...
		init = append(init, mkcall1(fn, nil, nil, typename(t), ha, ir.NewAddrExpr(base.Pos, hit)))
		nfor.SetLeft(ir.NewBinaryExpr(base.Pos, ir.ONE, ir.NewSelectorExpr(base.Pos, ir.ODOT, hit, keysym), nodnil()))

		fn = syslook("mapiternext")
		fn = substArgTypes(fn, th)
		nfor.SetRight(mkcall1(fn, nil, nil, ir.NewAddrExpr(base.Pos, hit)))
...
func arrayClear(loop *ir.RangeStmt, v1, v2, a ir.Node) ir.Node {
	...
	ix.SetBounded(true)
	addr := convnop(ir.NewAddrExpr(base.Pos, ix), types.Types[types.TUNSAFEPTR])
