...
func cgen_wb(n, res *Node, wb bool) {
	...
	if Ctxt.Arch.Char == sys.CharARM { // TODO(rsc): Maybe more often?
	...
	if Ctxt.Arch.HasChar(sys.CharAMD64, sys.Char386) && n.Addable {
		Thearch.Gmove(n, res)
		return
	}

	if Ctxt.Arch.HasChar(sys.CharMIPS64, sys.CharARM64, sys.CharPPC64) {
		// if both are addressable, move
		if n.Addable {
	...
	if Ctxt.Arch.Char == sys.CharARM && !Is64(n.Type) && !Is64(res.Type) && !n.Type.IsComplex() && !res.Type.IsComplex() {
	...
	case OMINUS:
		...
		Cgen(nl, &n1)
		if Ctxt.Arch.Char == sys.CharARM {
			var n2 Node
			Nodconst(&n2, nl.Type, 0)
			Thearch.Gins(a, &n2, &n1)
		} else if Ctxt.Arch.Char == sys.CharARM64 {
...
abop: // asymmetric binary
	...
	if nl.Ullman >= nr.Ullman {
		...
		if Smallintconst(nr) && Ctxt.Arch.Char != sys.CharMIPS64 && Ctxt.Arch.Char != sys.CharARM && Ctxt.Arch.Char != sys.CharARM64 && Ctxt.Arch.Char != sys.CharPPC64 { // TODO(rsc): Check opcode for arm
			n2 = *nr
		} else {
...
func addOffset(res *Node, offset int64) {
	if Ctxt.Arch.HasChar(sys.CharAMD64, sys.Char386) {
...
func bgenx(n, res *Node, wantTrue bool, likely int, to *obj.Prog) {
	...
	case ONAME:
		...
		mayNeedTemp := Ctxt.Arch.HasChar(sys.CharARM, sys.CharARM64, sys.CharMIPS64, sys.CharPPC64)
	...
	if nr.Ullman >= UINF {
		...
		var n1 Node
		if !nl.Addable && Ctxt.Arch.Char == sys.Char386 {
			Tempname(&n1, nl.Type)
		} else {
			Regalloc(&n1, nl.Type, nil)
			defer Regfree(&n1)
		...
		if Smallintconst(nr) && Ctxt.Arch.Char != sys.CharMIPS64 && Ctxt.Arch.Char != sys.CharPPC64 {
			Thearch.Gins(Thearch.Optoas(OCMP, nr.Type), nl, nr)
			bins(nr.Type, res, op, likely, to)
			return
		}

		if !nr.Addable && Ctxt.Arch.Char == sys.Char386 {
			nr = CgenTemp(nr)
		}
	...
	if Ctxt.Arch.Char == sys.CharAMD64 && nl.Type.IsFloat() && (op == OGT || op == OGE) {
		l, r = r, l
		op = Brrev(op)
	}

	// MIPS does not have CMP instruction
	if Ctxt.Arch.Char == sys.CharMIPS64 {
	...
	if nl.Type.IsFloat() {
		...
		case sys.CharARM64, sys.CharPPC64:
			if genval {
...
func bgenNonZero(n, res *Node, wantTrue bool, likely int, to *obj.Prog) {
	...
	// MIPS does not have CMP instruction
	if Thearch.LinkArch.Char == sys.CharMIPS64 {
	...
	case 0, // normal call
		...
		if f.Op == ONAME && f.Class == PFUNC {
			if f == Deferreturn {
				...
				if Thearch.LinkArch.Char == sys.CharPPC64 {
...
func cgen_div(op Op, nl *Node, nr *Node, res *Node) {
	...
	// in peep and optoas in order to enable this.
	if nr.Op != OLITERAL || Ctxt.Arch.Char == sys.CharMIPS64 || Ctxt.Arch.Char == sys.CharARM64 || Ctxt.Arch.Char == sys.CharPPC64 {
		goto longdiv
	}
...
func cgen_slice(n, res *Node, wb bool) {
	...
	gins := Thearch.Gins
	if Thearch.LinkArch.Char == sys.Char386 {
