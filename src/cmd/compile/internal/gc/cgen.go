...
func Igen(n *Node, a *Node, res *Node) {
	...
	case OCALLFUNC, OCALLMETH, OCALLINTER:
		...
		*a = Node{}
		...
		a.Reg = int16(Thearch.REGSP)
		a.Addable = true
		a.Xoffset = fp.Width + Ctxt.FixedStackSize()
		a.Type = n.Type
		return
...
func stkof(n *Node) int64 {
	...
	case OCALLMETH, OCALLINTER, OCALLFUNC:
		...
		if t != nil {
			return t.Width + Ctxt.FixedStackSize()
		}
	}
...
func Ginscall(f *Node, proc int) {
	...
	case 1, // call in new proc (go)
		...
		stk.Xoffset = Ctxt.FixedStackSize()
		Thearch.Ginscon(Thearch.Optoas(OAS, Types[TINT32]), int64(Argsize(f.Type)), &stk)

		// FuncVal* at 8(SP)
		stk.Xoffset = int64(Widthptr) + Ctxt.FixedStackSize()
...
func cgen_callinter(n *Node, res *Node, proc int) {
	...
	nodsp.Xoffset = Ctxt.FixedStackSize()
	if proc != 0 {
...
func cgen_callret(n *Node, res *Node) {
	...
	nod.Xoffset = fp.Width + Ctxt.FixedStackSize()
	nod.Type = fp.Type
	Cgen_as(res, &nod)
}
...
func cgen_aret(n *Node, res *Node) {
	...
	nod1.Addable = true
	nod1.Xoffset = fp.Width + Ctxt.FixedStackSize()
	nod1.Type = fp.Type
...
func cgen_append(n, res *Node) {
	...
	arg.Addable = true
	arg.Xoffset = Ctxt.FixedStackSize()
