...
func cgen_wb(n, res *Node, wb bool) {
	...
	if Ctxt.Arch.HasFamily(sys.AMD64, sys.X86) && n.Addable {
		Thearch.Gmove(n, res)
		return
	}

	if Ctxt.Arch.HasFamily(sys.MIPS64, sys.ARM64, sys.PPC64) {
		// if both are addressable, move
		if n.Addable {
...
func addOffset(res *Node, offset int64) {
	if Ctxt.Arch.HasFamily(sys.AMD64, sys.X86) {
...
func bgenx(n, res *Node, wantTrue bool, likely int, to *obj.Prog) {
	...
	case ONAME:
		...
		mayNeedTemp := Ctxt.Arch.HasFamily(sys.ARM, sys.ARM64, sys.MIPS64, sys.PPC64)
	...
	if nr.Ullman >= UINF {
		...
		var n1 Node
		if !nl.Addable && Ctxt.Arch.Family == sys.X86 {
			Tempname(&n1, nl.Type)
		} else {
			Regalloc(&n1, nl.Type, nil)
			defer Regfree(&n1)
		...
		if !nr.Addable && Ctxt.Arch.Family == sys.X86 {
			nr = CgenTemp(nr)
		}
...
func cgen_slice(n, res *Node, wb bool) {
	...
	gins := Thearch.Gins
	if Thearch.LinkArch.Family == sys.X86 {
