...
func ggloblnod(nam *ir.Name) {
	...
	size := nam.Type().Size()
	// For linkname'd global variables, if we instrument them in the package in which
	// they are declared, when they are written to the object file, their size should
	// be the new size with the trailing red area, so use their link name to check if
	// they are instrumented in their home package.
	name := nam.Sym().Linkname
	if name == "" {
		name = nam.Sym().Pkg.Name + "." + nam.Sym().Name
	}
