...
func dumpdata() {
	...
	if exportlistLen != len(exportlist) {
		base.Fatal("exportlist changed after compile functions loop")
	}
	if ptabsLen != len(ptabs) {
		base.Fatal("ptabs changed after compile functions loop")
	}
	if itabsLen != len(itabs) {
		base.Fatal("itabs changed after compile functions loop")
	}
}
...
func dumpLinkerObj(bout *bio.Writer) {
	...
	if len(pragcgobuf) != 0 {
		...
		if err := json.NewEncoder(bout).Encode(pragcgobuf); err != nil {
			base.Fatal("serializing pragcgobuf: %v", err)
		}
		fmt.Fprintf(bout, "\n$$\n\n")
	}
...
func dumpGlobalConst(n *ir.Node) {
	...
	case types.TUINTPTR:
		// ok
	case types.TIDEAL:
		if !ir.IsConst(n, ir.CTINT) {
			return
		}
		x := n.Val().U.(*ir.Int)
		if x.Cmp(minintval[types.TINT]) < 0 || x.Cmp(maxintval[types.TINT]) > 0 {
			return
		}
...
func duintxx(s *obj.LSym, off int, v uint64, wid int) int {
	if off&(wid-1) != 0 {
		base.Fatal("duintxxLSym: misaligned: v=%d wid=%d off=%d", v, wid, off)
	}
...
func dstringdata(s *obj.LSym, off int, t string, pos src.XPos, what string) int {
	...
	if int64(len(t)) > 2e9 {
		base.ErrorAt(pos, "%v with length %v is too big", what, len(t))
		return 0
	}
