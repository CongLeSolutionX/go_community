...
func dumpdata() {
	...
	if zerosize > 0 {
		...
		wobj.Global(zero.Linksym(), int32(zerosize), obj.DUPOK|obj.RODATA)
	}
...
func dumpfuncsyms() {
	...
	for _, s := range funcsyms {
		...
		wobj.SymPtr(sf, 0, s.Linksym(), 0)
		wobj.Global(sf, int32(types.PtrSize), obj.DUPOK|obj.RODATA)
	}
}
	...
	for _, s := range base.Ctxt.Text {
		...
		for _, gcsym := range []*obj.LSym{fn.GCArgs, fn.GCLocals} {
			if gcsym != nil && !gcsym.OnList() {
				wobj.Global(gcsym, int32(len(gcsym.P)), obj.RODATA|obj.DUPOK)
			}
		}
		if x := fn.StackObjects; x != nil {
			attr := int16(obj.RODATA)
			wobj.Global(x, int32(len(x.P)), attr)
			x.Set(obj.AttrStatic, true)
		}
		if x := fn.OpenCodedDeferInfo; x != nil {
			wobj.Global(x, int32(len(x.P)), obj.RODATA|obj.DUPOK)
		}
	}
}
...
func stringsym(pos src.XPos, s string) (data *obj.LSym) {
	...
	if !symdata.OnList() {
		...
		wobj.Global(symdata, int32(off), obj.DUPOK|obj.RODATA|obj.LOCAL)
...
func fileStringSym(pos src.XPos, file string, readonly bool, hash []byte) (*obj.LSym, int64, error) {
	...
	if readonly {
		...
		if !symdata.OnList() {
			...
			info.Name = file
			info.Size = size
			wobj.Global(symdata, int32(size), obj.DUPOK|obj.RODATA|obj.LOCAL)
...
func slicedata(pos src.XPos, s string) *ir.Name {
	...
	wobj.Global(lsym, int32(off), obj.NOPTR|obj.LOCAL)
