...
func Main(archInit func(*Arch)) {
	...
	if base.Flag.LowerL != 0 {
		...
		visitBottomUp(xtop, func(list []ir.INode, recursive bool) {
...
func numNonClosures(list []ir.INode) int {
	count := 0
	for _, n := range list {
...
func importfile(f *ir.Val) *types.Pkg {
	path_, ok := f.U.(string)
	if !ok {
...
func mkpackage(pkgname string) {
	if ir.LocalPkg.Name == "" {
		...
		if pkgname != ir.LocalPkg.Name {
			base.Error("package %s; expected %s", pkgname, ir.LocalPkg.Name)
		}
	}
}
