...
func Main() {
	...
	if !strings.HasPrefix(p, Thearch.LinkArch.Name) {
		log.Fatalf("cannot use %cg with GOARCH=%s", Thearch.LinkArch.Char, p)
	}
	goarch = p
	...
	if Thearch.LinkArch.HasChar(sys.CharARM, sys.CharAMD64, sys.CharARM64, sys.Char386, sys.CharPPC64) {
		obj.Flagcount("shared", "generate code that can be linked into a shared library", &flag_shared)
	}
	if Thearch.LinkArch.Char == sys.CharAMD64 {
