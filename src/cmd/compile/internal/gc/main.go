...
func Main(archInit func(*Arch)) {
	...
	types.Pkgs.Unsafe = types.NewPkg("unsafe", "unsafe")
	...
	// insignificantly.
	types.Pkgs.Runtime = types.NewPkg("go.runtime", "runtime")
	types.Pkgs.Runtime.Prefix = "runtime"

	// pseudo-packages used in symbol tables
	types.Pkgs.Itab = types.NewPkg("go.itab", "go.itab")
	types.Pkgs.Itab.Prefix = "go.itab" // not go%2eitab

	types.Pkgs.Itablink = types.NewPkg("go.itablink", "go.itablink")
	types.Pkgs.Itablink.Prefix = "go.itablink" // not go%2eitablink

	types.Pkgs.Track = types.NewPkg("go.track", "go.track")
	types.Pkgs.Track.Prefix = "go.track" // not go%2etrack

	// pseudo-package used for map zero values
	types.Pkgs.Map = types.NewPkg("go.map", "go.map")
	types.Pkgs.Map.Prefix = "go.map"

	// pseudo-package used for methods with anonymous receivers
	types.Pkgs.Go = types.NewPkg("go", "")
	...
	startProfile()
	if base.Flag.Race {
		types.Pkgs.Race = types.NewPkg("runtime/race", "")
	}
	if base.Flag.MSan {
		types.Pkgs.Msan = types.NewPkg("runtime/msan", "")
	}
	...
	typecheck.AddFunc = func(fn *ir.Func) { xtop = append(xtop, fn) }
	typecheck.AddGlobal = func(n *ir.Name) { externdcl = append(externdcl, n) }
	typecheck.AutoExport = autoexport
	typecheck.MarkNoEscape = func(n ir.Node) { n.SetEsc(EscNever) }
	typecheck.NeedFuncSym = makefuncsym
	typecheck.NeedItab = func(t, itype *types.Type) { itabname(t, itype) }
	typecheck.NeedRuntimeType = addsignat
	...
	typecheck.FinishUniverse()

	nodfp = typecheck.NewName(typecheck.Lookup(".fp"))
	...
	for i, n := range externdcl {
		if n.Op() == ir.ONAME {
			externdcl[i] = typecheck.Expr(externdcl[i])
		}
	}
	...
	for _, n := range xtop {
		if n.Op() == ir.ODCLFUNC && n.Func().OClosure != nil {
			ir.CurFunc = n.(*ir.Func)
			capturevars(ir.CurFunc)
		}
	}
	...
	if len(typecheck.TODO_funcStack) != 0 {
		base.Fatalf("funcStack is non-empty: %v", len(typecheck.TODO_funcStack))
	}
...
func initializeTypesPackage() {
	types.RecordFrameOffset = typecheck.RecordFrameOffset
