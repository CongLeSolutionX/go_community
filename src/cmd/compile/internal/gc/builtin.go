...
func runtimeTypes() []*types.Type {
	...
	typs[80] = tostruct([]*Node{namedfield("enabled", typs[11]), namedfield("pad", typs[79]), namedfield("needed", typs[11]), namedfield("cgo", typs[11]), namedfield("gen", typs[11]), namedfield("alignme", typs[17])})
	typs[81] = tostruct([]*Node{namedfield("arenaStart", typs[48]), namedfield("cardMarks", typs[1]), namedfield("cardMarksMapped", typs[48])})
	...
	typs[85] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[2]), anonfield(typs[2])}, []*Node{anonfield(typs[32])})
	typs[86] = functype(nil, []*Node{anonfield(typs[77]), anonfield(typs[3])}, []*Node{anonfield(typs[11])})
	typs[87] = functype(nil, []*Node{anonfield(typs[3]), anonfield(typs[74])}, []*Node{anonfield(typs[11])})
	typs[88] = types.NewPtr(typs[11])
	typs[89] = functype(nil, []*Node{anonfield(typs[3]), anonfield(typs[88]), anonfield(typs[74])}, []*Node{anonfield(typs[11])})
	typs[90] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[15]), anonfield(typs[8])}, nil)
	typs[91] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[77]), anonfield(typs[3])}, nil)
	typs[92] = functype(nil, []*Node{anonfield(typs[1]), anonfield(typs[74]), anonfield(typs[3]), anonfield(typs[88])}, nil)
	typs[93] = functype(nil, []*Node{anonfield(typs[1])}, []*Node{anonfield(typs[32])})
