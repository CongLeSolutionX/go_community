...
// data.
func ImportData(data []byte) (*Pkg, error) {
	datalen := len(data)
	...
	if len(p.data) > 0 {
		return nil, fmt.Errorf("not all export data consumed")
	}
...
func (p *importer) marker(want byte) {
	if debug {
		if got := p.data[0]; got != want {
			Fatal("incorrect marker: got %c; want %c (pos = %d)", got, want, p.consumed())
		}
		p.data = p.data[1:]
