...
func (p *importer) obj(tag int) {
	...
	case constTag:
		...
		typ := p.typ()
		val := p.value(typ)
		importconst(pos, sym, idealType(typ), val, p.imp)

	case aliasTag:
		pos := p.pos()
		sym := p.qualifiedName()
		typ := p.typ()
		importalias(pos, sym, typ, p.imp)
