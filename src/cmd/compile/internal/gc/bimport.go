...
func Import(data []byte) {
	...
	// sanity checks
	if p.pkgList[0] != pkg {
		Fatal("imported package not found in pkgList[0]")
	}
	if pkg != importpkg {
		Fatal("incorrect package setup")
	}
...
func (p *importer) pkg() *Pkg {
	...
	if name == "" {
		Fatal("empty package name in import")
	}

	// we should never see a bad import path
	if isbadimport(path) {
		Fatal("bad path in import: %q", path)
	}
	...
	if pkg.Name == "" {
		...
		Fatal("inconsistent package names: got %s; want %s (path = %s)", pkg.Name, name, path)
	}
