...
func newpkg(path, name string) *Pkg {
	if p := mypkgMap[path]; p != nil {
		if p.Name != name {
			Fatal("package name inconsistency")
		}
		return p
	}
...
// Import imports a package from the serialized package data
// and returns a reference to the package.
func Import(data []byte) *Pkg {
	...
	if format != 'c' && format != 'd' {
		Fatal("invalid encoding format in export data: got %q; want 'c' or 'd'", format)
	}
	...
	pkg := p.pkg()
	if p.pkgList[0] != pkg {
		Fatal("imported packaged not found in pkgList[0]")
	}

	// TODO(gri) where is the right place to do this?
	// (inside this file, we pass the importer around instead of relying on global variables...)
	importpkg = pkg
...
func (p *importer) pkg() *Pkg {
	...
	if i != packageTag {
		Fatal("unexpected package tag %d", i)
	}
	...
	// we should never see a bad import path in import data
	if isbadimport(path) {
		errorexit()
	}

	// if the package was imported before, use that one; otherwise create a new one
	pkg := newpkg(path, name)
...
func (p *importer) obj() bool {
	...
	case funcTag:
		...
		importsym(sym, ONAME)
		if sym.Def != nil {
			Fatal("function imported before?")
		}
		n := newfuncname(sym)
		n.Type = typ
		declare(n, PFUNC)
	...
	default:
		Fatal("unexpected object tag %d", tag)
	}
	return true
}
...
func (p *importer) typ() *Type {
	...
	if i >= 0 {
		...
		Fatal("typ index %d out of range (len = %d)", i, len(p.typList))
	}
	...
	case interfaceTag:
		t = p.newtyp(TINTER)
		if p.int() != 0 {
			Fatal("unexpected embedded interface")
		}
	...
	case chanTag:
		...
		case 's':
			dir = Csend
		default:
			Fatal("unexpected channel direction %d", dir)
		}
		t.Chan = uint8(dir)
		t.Type = p.typ()

	default:
		Fatal("unexpected type (tag = %d)", i)
	}

	if t == nil {
		Fatal("nil type (type tag = %d)", i)
	}
...
func (p *importer) value() (x Val) {
	...
	case stringTag:
		x.U = p.string()
	default:
		Fatal("unexpected value tag %d", tag)
	}
	return
}
...
func (p *importer) marker(want byte) {
	if got := p.data[0]; got != want {
		Fatal("incorrect marker: got %c; want %c (pos = %d)", got, want, p.read)
	}
	p.data = p.data[1:]
	p.read++
	...
	if n := int(p.rawInt64()); n != pos {
		Fatal("incorrect position: got %d; want %d", n, pos)
	}
}
