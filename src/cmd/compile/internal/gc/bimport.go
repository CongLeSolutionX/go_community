...
func Import(in *bufio.Reader) {
	...
	default:
		formatError("unknown export format version %d (%q)", p.version, versionstr)
	}
	...
	if count := p.int(); count != objcount {
		formatError("got %d objects; want %d", objcount, count)
	}
	...
	if dclcontext != PEXTERN {
		formatError("unexpected context %d", dclcontext)
	}
	...
	for i0 := -1; ; {
		...
		if i <= i0 {
			formatError("index not increasing: %d <= %d", i, i0)
		}
		i0 = i

		if Funcdepth != 0 {
			formatError("unexpected Funcdepth %d", Funcdepth)
		}
	...
	// self-verification
	if count := p.int(); count != objcount {
		formatError("got %d functions; want %d", objcount, count)
	}
...
func formatError(format string, args ...interface{}) {
	if debugFormat {
		Fatalf(format, args...)
	}
...
func (p *importer) pkg() *Pkg {
	...
	if i != packageTag {
		formatError("expected package tag, found tag = %d", i)
	}
	...
	if name == "" {
		formatError("empty package name for path %q", path)
	}

	// we should never see a bad import path
	if isbadimport(path) {
		formatError("bad package path %q for package %s", path, name)
	}

	// an empty path denotes the package we are currently importing;
	// it must be the first package we see
	if (path == "") != (len(p.pkgList) == 0) {
		formatError("package path %q for pkg index %d", path, len(p.pkgList))
	}
...
func (p *importer) obj(tag int) {
	...
	case funcTag:
		...
		if sym.Def != nil && sym.Def.Op == ONAME {
			...
			if !Eqtype(sig, sym.Def.Type) {
				formatError("inconsistent definition for func %v during import\n\t%v\n\t%v", sym, sym.Def.Type, sig)
			}
	...
	default:
		formatError("unexpected object (tag = %d)", tag)
	}
}
...
func (p *importer) typ() *Type {
	...
	case interfaceTag:
		t = p.newtyp(TINTER)
		if p.int() != 0 {
			formatError("unexpected embedded interface")
		}
	...
	default:
		formatError("unexpected type (tag = %d)", i)
	}

	if t == nil {
		formatError("nil type (type tag = %d)", i)
	}
...
func (p *importer) param(named bool) *Node {
	...
	if named {
		name := p.string()
		if name == "" {
			formatError("expected named parameter")
		}
...
func (p *importer) value(typ *Type) (x Val) {
	...
	case unknownTag:
		formatError("unknown constant (importing package with errors)")

	case nilTag:
		x.U = new(NilVal)

	default:
		formatError("unexpected value tag %d", tag)
	}

	// verify ideal type
	if typ.IsUntyped() && untype(x.Ctype()) != typ {
		formatError("value %v and type %v don't match", x, typ)
	}
...
func (p *importer) int() int {
	x := p.int64()
	if int64(int(x)) != x {
		formatError("exported integer too large")
	}
	return int(x)
}
...
func (p *importer) marker(want byte) {
	if got := p.rawByte(); got != want {
		formatError("incorrect marker: got %c; want %c (pos = %d)", got, want, p.read)
	}

	pos := p.read
	if n := int(p.rawInt64()); n != pos {
		formatError("incorrect position: got %d; want %d", n, pos)
	}
}
...
func (p *importer) rawByte() byte {
	...
	if c == '|' {
		...
		case '|':
			// nothing to do
		default:
			formatError("unexpected escape sequence in export data")
		}
	}
	return c
}
