...
// visitIntegral examines a param/reuslt of integral type 't' to
...
// visitArray processes an array type (or array component within some other
...
func setup() {
	pu := types.NewPtr(types.Types[types.TUINTPTR])
	synthOnce.Do(func() {
		slf := []*types.Type{
			pu,
		...
		synthSlice = mkstruct(slf)
		stf := []*types.Type{
			pu,
			types.Types[types.TUINTPTR],
		}
		synthString = mkstruct(stf)
		iff := []*types.Type{
			pu,
			pu,
		}
		synthIface = mkstruct(iff)
	})
}
...
func (state *assignState) visit(pt *types.Type) bool {
	typ := pt.Kind()
	if typ <= types.TFUNC || pt.IsPtrShaped() {
