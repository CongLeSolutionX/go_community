...
func main() {
	...
	for _, typ := range types {
		...
		fmt.Fprintf(w, "	msg string // test description for error message\n")
		fmt.Fprintf(w, "}{\n")
		for i, r := range values {
			// TODO: could also test constant on lhs.
			for _, op := range operators {
				fmt.Fprintf(w, "{idx: %v,", i)
				fmt.Fprintf(w, "exp: %v,", "_"+op.name)
				fmt.Fprintf(w, "fn:  func(x %v) bool { return x %v %v },", typ, op.op, r.String())
				fmt.Fprintf(w, "msg: \"x %v %v\"},\n", op.op, r.String())
			}
		}
		fmt.Fprintf(w, "}\n")
	}
	...
	for _, typ := range types {
		fmt.Fprintf(w, "for _, test := range %v_tests {\n", typ)
		fmt.Fprintf(w, "	for i := range %v_vals {\n", typ)
		fmt.Fprintf(w, "		want := test.exp.l\n")
		fmt.Fprintf(w, "		if i == test.idx {\nwant = test.exp.e\n}")
		fmt.Fprintf(w, "		else if i > test.idx {\nwant = test.exp.r\n}\n")
		fmt.Fprintf(w, "		if test.fn(%v_vals[i]) != want {\n", typ)
		fmt.Fprintf(w, "			msg := fmt.Sprintf(\"test failed: %%v != %%v (x = %v(%%v))\", test.msg, want, %v_vals[i])\n", typ, typ)
