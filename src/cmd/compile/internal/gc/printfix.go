...
func main() {
	...
	for index, file := range files {
		ast.Inspect(file, func(n ast.Node) bool {
			if call, ok := n.(*ast.CallExpr); ok {
				...
				if functionBlacklist[nodeString(call.Fun)] {
					return true
				}
				...
				for i, arg := range call.Args {
					if s, ok := stringLit(arg); ok && isFormat(s) {
						...
						if i+1+n > len(call.Args) {
							errorf("%s: not enough format args?", posString(call))
						}
	...
	for _, p := range callSites {
		out := formatReplace(p.str, func(index int, in string) string {
			...
			if !found && *test {
				errorf("%s: unknown format %q for %s argument\n", posString(p.lit), in, typ)
			}
			if out == "" {
				out = in
			}
	...
	if dirty && *update {
		for i, d := range dirtyFiles {
			...
			if err := format.Node(&buf, fset, files[i]); err != nil {
				errorf("WARNING: formatting %s failed: %v\n", filename, err)
				continue
			}
			if err := ioutil.WriteFile(filename, buf.Bytes(), 0x666); err != nil {
				errorf("WARNING: writing %s failed: %v\n", filename, err)
				continue
			}
...
// functionBlacklist is the set of functions which are known to
