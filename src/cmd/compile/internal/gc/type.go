...
type Type struct {
	...
	// TSTRUCT: *StructType
	// TINTER: *InterType
	// TDDDFIELD, TFUNCARGS, TCHANARGS: WrapperType
	// TCHAN: *ChanType
...
// WrapperType holds a simple link to another type.
// It is used with psuedo-types.
// For TDDDFIELD types, it holds a reference to a slice type for ... args.
// For TCHANARGS types, it holds a reference to a chan type whose elements need a width check.
// For TFUNCARGS types, it holds a reference to a func type whose elements need a width check.
...
func (t *Type) SetNumElem(n int64) {
	...
	case n == sliceBound:
		if t.IsSlice() {
			// nothing to do, already a slice
			return
		}
		t.Extra = SliceType{Elem: t.Elem()}
	case n == dddBound:
		if t.isDDDArray() {
			// nothing to do, already a ddd array
			return
		}
		t.Extra = DDDArrayType{Elem: t.Elem()}
	default:
		Fatalf("SetNumElem bad extra type %T %v", t.Extra, t)
	}
}
