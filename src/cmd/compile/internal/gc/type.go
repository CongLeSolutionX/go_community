...
// WalkFields initializes Iter to iterate across a struct type's fields
// or an interface type's methods.
func (s *Iter) WalkFields(t *Type) *Type {
	if t.Etype != TSTRUCT && t.Etype != TINTER {
		Fatalf("Iter.WalkFields: type %v does not have fields", t)
	}
	s.a = t.Type
	return s.Next()
}

// WalkParams initializes Iter to iterate across a function type's receiver
// and input parameters.
func (s *Iter) WalkParams(t *Type) *Type {
	if t.Etype != TFUNC {
		Fatalf("Iter.WalkParams: type %v does not have params", t)
	}
