...
func (o *Order) stmt(n ir.Node) {
	...
	case ir.OSELECT:
		...
		for _, ncas := range n.List().Slice() {
			if ncas.Op() != ir.OCASE {
				base.Fatalf("order select case %v", ncas.Op())
			}
			r := ncas.Left()
			setlineno(ncas)
			...
			case ir.OAS2RECV:
				...
				colas := r.Colas()
				do := func(i int, t *types.Type, clear bool) {
					n := r.List().Index(i)
					...
					if colas {
						if len(init) > 0 && init[0].Op() == ir.ODCL && init[0].Left() == n {
							init = init[1:]
						}
					}
					if !ir.IsBlank(n) {
				...
				do(0, recv.Left().Type().Elem(), recv.Left().Type().Elem().HasPointers())
				do(1, types.Types[types.TBOOL], false)
				if len(init) != 0 {
