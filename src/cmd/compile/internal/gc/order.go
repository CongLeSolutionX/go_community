...
func (o *Order) edge() {
	...
	// counter += 1
	incr := ir.NewAssignOpStmt(base.Pos, ir.OASOP, counter, nodintconst(1))
	incr.AsOp = ir.OADD
	o.append(incr)
}
		...
		for _, cas := range n.Cases.Slice() {
			...
			case ir.OSELRECV, ir.OSELRECV2:
				...
				if r.Op() == ir.OSELRECV {
					...
					r := r.(*ir.AssignStmt)
					def, dst, ok, recv = r.Def, r.X, ir.BlankNode, r.Y.(*ir.UnaryExpr)
				} else {
					r := r.(*ir.AssignListStmt)
					// case x, ok = <-c
					def, dst, ok, recv = r.Def, r.Lhs.First(), r.Lhs.Second(), r.Rhs.First().(*ir.UnaryExpr)
				}
				...
				if def {
					init := r.Init().Slice()
					if len(init) > 0 && init[0].Op() == ir.ODCL && init[0].(*ir.Decl).X == dst {
						init = init[1:]
					}
					if len(init) > 0 && init[0].Op() == ir.ODCL && init[0].(*ir.Decl).X == ok {
						init = init[1:]
					}
					r.PtrInit().Set(init)
				}
				...
				if !ir.IsBlank(dst) {
					...
					if def {
						dcl := ir.NewDecl(base.Pos, ir.ODCL, dst)
						cas.PtrInit().Append(typecheck(dcl, ctxStmt))
					}

					tmp := o.newTemp(recv.X.Type().Elem(), recv.X.Type().Elem().HasPointers())
					as := ir.NewAssignStmt(base.Pos, dst, tmp)
					cas.PtrInit().Append(typecheck(as, ctxStmt))
					dst = tmp
				}
				if !ir.IsBlank(ok) {
					if def {
						dcl := ir.NewDecl(base.Pos, ir.ODCL, ok)
						cas.PtrInit().Append(typecheck(dcl, ctxStmt))
					}

					tmp := o.newTemp(types.Types[types.TBOOL], false)
					as := ir.NewAssignStmt(base.Pos, ok, conv(tmp, ok.Type()))
					cas.PtrInit().Append(typecheck(as, ctxStmt))
					ok = tmp
				}
...
func (o *Order) expr1(n, lhs ir.Node) ir.Node {
	...
	case ir.OCLOSURE:
		n := n.(*ir.ClosureExpr)
		if n.Transient() && len(n.Func_.ClosureVars) > 0 {
