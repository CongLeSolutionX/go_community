...
type Order struct {
	out  []ir.INode            // list of generated statements
	temp []ir.INode            // stack of temporary variables
	free map[string][]ir.INode // free list of unused temporaries, by type.LongString().
}
...
func order(fn ir.INode) {
	...
	orderBlock(fn.PtrNbody(), map[string][]ir.INode{})
}

// newTemp allocates a new temporary with the given type,
// pushes it onto the temp stack, and returns it.
// If clear is true, newTemp emits code to zero the temporary.
func (o *Order) newTemp(t *types.Type, clear bool) ir.INode {
	var v ir.INode
...
func (o *Order) cheapExpr(n ir.INode) ir.INode {
	if n == nil {
		return nil
	}
...
func (o *Order) safeExpr(n ir.INode) ir.INode {
	switch n.Op() {
	case ir.ONAME, ir.OLITERAL:
		return n
	...
	default:
		base.Fatal("order.safeExpr %v", n.Op())
...
func isaddrokay(n ir.INode) bool {
...
func (o *Order) addrTemp(n ir.INode) ir.INode {
	if ir.ConstType(n) != ir.CTxxx {
		...
		if s.out != nil {
			base.Fatal("staticassign of const generated code: %+v", n)
		}
...
func (o *Order) mapKeyTemp(t *types.Type, n ir.INode) ir.INode {
...
func mapKeyReplaceStrConv(n ir.INode) bool {
	var replaced bool
	switch n.Op() {
	case ir.OBYTES2STR:
...
func (o *Order) cleanTempNoPop(mark ordermarker) []ir.INode {
	var out []ir.INode
...
func orderMakeSliceCopy(s []ir.INode) {
...
func orderBlock(n *ir.Nodes, free map[string][]ir.INode) {
	var order Order
	order.free = free
	mark := order.markTemp()
	order.edge()
...
func (o *Order) exprInPlace(n ir.INode) ir.INode {
	var order Order
	order.free = o.free
	n = order.expr(n, nil)
...
// free is a map that can be used to obtain temporary variables by type.
func orderStmtInPlace(n ir.INode, free map[string][]ir.INode) ir.INode {
	var order Order
	order.free = free
	mark := order.markTemp()
	order.stmt(n)
...
func (o *Order) init(n ir.INode) {
	if n.MayBeShared() {
		// For concurrency safety, don't mutate potentially shared nodes.
		// First, ensure that no work is required here.
		if n.Ninit().Len() > 0 {
			base.Fatal("order.init shared node with ninit")
		}
		return
	}
	o.stmtList(n.Ninit())
...
func (o *Order) call(n ir.INode) {
	if n.Ninit().Len() > 0 {
		// Caller should have already called o.init(n).
		base.Fatal("%v with unexpected ninit", n.Op())
	}
...
func (o *Order) mapAssign(n ir.INode) {
	switch n.Op() {
	default:
		base.Fatal("order.mapAssign %v", n.Op())
...
func (o *Order) stmt(n ir.INode) {
	...
	case ir.OFOR:
		...
		n.PtrNbody().Prepend(o.cleanTempNoPop(t)...)
		orderBlock(n.PtrNbody(), o.free)
	...
	case ir.ORANGE:
		...
		switch n.Type().Etype {
		default:
			base.Fatal("order.stmt range %v", n.Type())
	...
	case ir.OSELECT:
		...
		for _, n2 := range n.List().Slice() {
			if n2.Op() != ir.OCASE {
				base.Fatal("order select case %v", n2.Op())
			}
			r := n2.Left()
			setlineno(n2)
			...
			default:
				...
				base.Fatal("unknown op in select %v", r.Op())
			...
			case ir.OSELRECV, ir.OSELRECV2:
				...
				if r.Ninit().Len() != 0 {
					...
					base.Fatal("ninit on select recv")
				}
				...
				orderBlock(n2.PtrNinit(), o.free)

			case ir.OSEND:
				if r.Ninit().Len() != 0 {
					ir.DumpList("ninit", r.Ninit())
					base.Fatal("ninit on select send")
				}
		...
		for _, n3 := range n.List().Slice() {
			orderBlock(n3.PtrNbody(), o.free)
			n3.PtrNbody().Prepend(o.cleanTempNoPop(t)...)

			// TODO(mdempsky): Is this actually necessary?
			// walkselect appears to walk Ninit.
			n3.PtrNbody().Prepend(n3.Ninit().Slice()...)
			n3.PtrNinit().Set(nil)
		}
	...
	case ir.OSWITCH:
		...
		for _, ncas := range n.List().Slice() {
			if ncas.Op() != ir.OCASE {
				base.Fatal("order switch case %v", ncas.Op())
			}
			o.exprListInPlace(ncas.List())
			orderBlock(ncas.PtrNbody(), o.free)
		}
...
func hasDefaultCase(n ir.INode) bool {
	for _, ncas := range n.List().Slice() {
		if ncas.Op() != ir.OCASE {
			base.Fatal("expected case, found %v", ncas.Op())
		}
		if ncas.List().Len() == 0 {
			return true
		}
...
var prealloc = map[ir.INode]ir.INode{}
...
func (o *Order) expr(n, lhs ir.INode) ir.INode {
	if n == nil {
		return n
	}
	...
	case ir.OMAPLIT:
		...
		statics := entries[:0]
		var dynamics []ir.INode
		for _, r := range entries {
			if r.Op() != ir.OKEY {
				base.Fatal("OMAPLIT entry not OKEY: %v\n", r)
			}
...
func okas(ok, val ir.INode) ir.INode {
	if !ok.IsBlank() {
...
func (o *Order) okAs2(n ir.INode) {
	var tmp1, tmp2 ir.INode
