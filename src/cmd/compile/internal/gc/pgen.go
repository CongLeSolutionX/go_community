...
func createSimpleVars(automDecls []*Node) ([]*Node, []*dwarf.Var, map[*Node]bool) {
	...
	return addSimpleVars(automDecls, decls, vars, selected, nil)
}
...
// if fnsym is not nil, then NAME_DELETED_AUTO entries are also emitted
// to ensure that types are present at link time.
func addSimpleVars(inDecls, decls []*Node, vars []*dwarf.Var, selected map[*Node]bool, fnsym *obj.LSym) ([]*Node, []*dwarf.Var, map[*Node]bool) {
...
func createDwarfVars(fnsym *obj.LSym, fn *Func, automDecls, nonSsaNodes []*Node) ([]*Node, []*dwarf.Var) {
	...
	if Ctxt.Flag_locationlists && Ctxt.Flag_optimize && fn.DebugInfo != nil {
		...
		decls, vars, selected = addSimpleVars(nonSsaNodes, decls, vars, selected, fnsym)
	} else {
