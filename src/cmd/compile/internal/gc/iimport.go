...
func importReaderFor(n ir.INode, importers map[*types.Sym]iimporterAndOffset) *importReader {
...
func iimport(pkg *types.Pkg, in *bio.Reader) (fingerprint goobj.FingerprintType) {
	...
	for nPkgs := ird.uint64(); nPkgs > 0; nPkgs-- {
		...
		for nSyms := ird.uint64(); nSyms > 0; nSyms-- {
			...
			if s.Def != nil {
				base.Fatal("unexpected definition for %v: %v", s, ir.AsNode(s.Def))
			}
...
func (r *importReader) doDecl(n ir.INode) {
	if n.Op() != ir.ONONAME {
	...
	case 'T':
		...
		for i := range ms {
			...
			mtyp.SetNname(ir.AsTypesNode(m))
		}
		t.Methods().Set(ms)
...
func (p *importReader) value() (typ *types.Type, v ir.Val) {
	typ = p.typ()
...
func (p *importReader) float(x *ir.Float, typ *types.Type) {
	var mant big.Int
	p.mpint(&mant, typ)
	m := x.Val.SetInt(&mant)
	if m.Sign() == 0 {
...
func (r *importReader) varExt(n ir.INode) {
	r.linkname(n.Sym())
	r.symIdx(n.Sym())
}

func (r *importReader) funcExt(n ir.INode) {
	r.linkname(n.Sym())
	r.symIdx(n.Sym())
...
func (r *importReader) methExt(m *types.Field) {
	...
	r.funcExt(ir.AsNode(m.Type.Nname()))
}
...
func (r *importReader) doInline(n ir.INode) {
...
func (r *importReader) stmtList() []ir.INode {
	var list []ir.INode
	for {
		n := r.node()
		if n == nil {
...
func (r *importReader) exprList() []ir.INode {
	var list []ir.INode
	for {
		n := r.expr()
		if n == nil {
...
func (r *importReader) expr() ir.INode {
	n := r.node()
...
func (r *importReader) node() ir.INode {
	switch op := r.op(); op {
	// expressions
	// case OPAREN:
	...
	case ir.OKEY:
		...
		return nodl(pos, ir.OKEY, left, right)
	...
	case ir.ODCL:
		...
		typ := typenod(r.typ())
		return npos(pos, liststmt(variter([]ir.INode{lhs}, typ, nil))) // TODO(gri) avoid list creation
	...
	case ir.OAS:
		return nodl(r.pos(), ir.OAS, r.expr(), r.expr())
...
func (r *importReader) elemList() []ir.INode {
	c := r.uint64()
	list := make([]ir.INode, c)
	for i := range list {
		s := r.ident()
...
func (r *importReader) exprsOrNil() (a, b ir.INode) {
	ab := r.uint64()
	if ab&1 != 0 {
		a = r.expr()
	}
