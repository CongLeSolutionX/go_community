...
func typecheckTypeSwitch(n *ir.SwitchStmt) {
	tag := n.Left().(*ir.TypeSwitchGuard)
	tag.SetRight(typecheck(tag.Right(), ctxExpr))
	t := tag.Right().Type()
	if t != nil && !t.IsInterface() {
		base.ErrorfAt(n.Pos(), "cannot type switch on non-interface value %L", tag.Right())
		t = nil
	}
	...
	if v := tag.Left(); v != nil && !ir.IsBlank(v) && n.List().Len() == 0 {
			...
			if !n1.Type().IsInterface() && !implements(n1.Type(), t, &missing, &have, &ptr) && !missing.Broke() {
				if have != nil && !have.Broke() {
					base.ErrorfAt(ncase.Pos(), "impossible type switch case: %L cannot have dynamic type %v"+
						" (wrong type for %v method)\n\thave %v%S\n\twant %v%S", tag.Right(), n1.Type(), missing.Sym, have.Sym, have.Type, missing.Sym, missing.Type)
				} else if ptr != 0 {
					base.ErrorfAt(ncase.Pos(), "impossible type switch case: %L cannot have dynamic type %v"+
						" (%v method has pointer receiver)", tag.Right(), n1.Type(), missing.Sym)
				} else {
					base.ErrorfAt(ncase.Pos(), "impossible type switch case: %L cannot have dynamic type %v"+
						" (missing %v method)", tag.Right(), n1.Type(), missing.Sym)
				}
				continue
			}
