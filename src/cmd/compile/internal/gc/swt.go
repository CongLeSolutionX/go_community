...
func (s *typeSet) add(pos src.XPos, typ *types.Type) {
	...
	for _, prev := range prevs {
		if types.Identical(typ, prev.typ) {
			base.ErrorAt(pos, "duplicate case %v in type switch\n\tprevious case at %s", typ, base.FmtPos(prev.pos))
			return
		}
	}
...
func walkExprSwitch(sw *ir.Node) {
	...
	cond = walkexpr(cond, sw.PtrNinit())
	if cond.Op() != ir.OLITERAL {
		cond = copyexpr(cond, cond.Type(), sw.PtrNbody())
	}
	...
	for _, ncase := range sw.List().Slice() {
		...
		if ncase.List().Len() == 0 {
			if defaultGoto != nil {
				base.Fatal("duplicate default case not detected during typechecking")
			}
			defaultGoto = jmp
		}
	...
	s.Emit(sw.PtrNbody())
	sw.PtrNbody().Append(defaultGoto)
	sw.PtrNbody().AppendNodes(&body)
...
func (s *exprSwitch) flush() {
	...
	sort.Slice(cc, func(i, j int) bool {
		return compareOp(cc[i].lo.Val(), ir.OLT, cc[j].lo.Val())
	})
...
func (c *exprClause) test(exprname *ir.Node) *ir.Node {
	...
	if ir.IsConst(exprname, ir.CTBOOL) && !c.lo.Type().IsInterface() {
		if exprname.BoolVal() {
			return c.lo
		} else {
...
func walkTypeSwitch(sw *ir.Node) {
	...
	s.facename = walkexpr(s.facename, sw.PtrNinit())
	s.facename = copyexpr(s.facename, s.facename.Type(), sw.PtrNbody())
	...
	if s.facename.Type().IsEmptyInterface() {
		dotHash.SetXoffset(int64(2 * Widthptr)) // offset of hash in runtime._type
	} else {
		dotHash.SetXoffset(int64(2 * Widthptr)) // offset of hash in runtime.itab
	}
	...
	for _, ncase := range sw.List().Slice() {
		...
		for _, n1 := range ncase.List().Slice() {
			if n1.IsNil() { // case nil:
				if nilGoto != nil {
					base.Fatal("duplicate nil case not detected during typechecking")
				}
				nilGoto = jmp
				continue
			}
