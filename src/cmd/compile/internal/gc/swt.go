...
func typecheckTypeSwitch(n *ir.SwitchStmt) {
	tag := n.Tag.(*ir.TypeSwitchGuard)
	tag.X = typecheck(tag.X, ctxExpr)
	t := tag.X.Type()
	if t != nil && !t.IsInterface() {
		base.ErrorfAt(n.Pos(), "cannot type switch on non-interface value %L", tag.X)
		t = nil
	}
	...
	if v := tag.Tag; v != nil && !ir.IsBlank(v) && len(n.Cases) == 0 {
			...
			if !n1.Type().IsInterface() && !implements(n1.Type(), t, &missing, &have, &ptr) && !missing.Broke() {
				if have != nil && !have.Broke() {
					base.ErrorfAt(ncase.Pos(), "impossible type switch case: %L cannot have dynamic type %v"+
						" (wrong type for %v method)\n\thave %v%S\n\twant %v%S", tag.X, n1.Type(), missing.Sym, have.Sym, have.Type, missing.Sym, missing.Type)
				} else if ptr != 0 {
					base.ErrorfAt(ncase.Pos(), "impossible type switch case: %L cannot have dynamic type %v"+
						" (%v method has pointer receiver)", tag.X, n1.Type(), missing.Sym)
				} else {
					base.ErrorfAt(ncase.Pos(), "impossible type switch case: %L cannot have dynamic type %v"+
						" (missing %v method)", tag.X, n1.Type(), missing.Sym)
				}
				continue
			}
...
func walkExprSwitch(sw *ir.SwitchStmt) {
	...
	for _, ncase := range sw.Cases {
		...
		if fall, pos := hasFall(ncase.Body); !fall {
