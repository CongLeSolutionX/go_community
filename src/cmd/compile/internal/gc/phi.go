...
type phiState struct {
	...
	defvars []map[ir.INode]*ssa.Value // defined variables at end of each block

	varnum map[ir.INode]int32 // variable numbering
...
func (s *phiState) insertVarPhis(n int, var_ ir.INode, defs []*ssa.Block, typ *types.Type) {
	priq := &s.priq
	q := s.q
	queued := s.queued
	queued.clear()
...
func (s *simplePhiState) lookupVarOutgoing(b *ssa.Block, t *types.Type, var_ ir.INode, line src.XPos) *ssa.Value {
	for {
