...
func ParseFlags() {
	...
	objabi.Flagcount("%", "debug non-static initializers", &Flag.P)
	...
	flag.StringVar(&Flag.Specte, "spectre", Flag.Specte, "enable spectre mitigations in `list` (all, index, ret)")
	if enableTrace {
		flag.BoolVar(&Flag.LowerT, "t", false, "trace type-checking")
	}
	flag.StringVar(&Flag.TrimPrefix, "trimpath", "", "remove `prefix` from recorded source file paths")
	...
	Ctxt.Pkgpath = Ctxt.Pkgpath

	for _, f := range strings.Split(Flag.Specte, ",") {
