...
func bvbulkalloc(nbit int32, count int32) bulkBvec {
	...
	if int64(int32(size*4)) != size*4 {
		base.Fatal("bvbulkalloc too big: nbit=%d count=%d nword=%d size=%d", nbit, count, nword, size)
	}
	return bulkBvec{
...
func (bv1 bvec) Eq(bv2 bvec) bool {
	if bv1.n != bv2.n {
		base.Fatal("bvequal: lengths %d and %d are not equal", bv1.n, bv2.n)
	}
	for i, x := range bv1.b {
		if x != bv2.b[i] {
			return false
...
func (bv bvec) Get(i int32) bool {
	if i < 0 || i >= bv.n {
		base.Fatal("bvget: index %d is out of bounds with length %d\n", i, bv.n)
	}
...
func (bv bvec) Set(i int32) {
	if i < 0 || i >= bv.n {
		base.Fatal("bvset: index %d is out of bounds with length %d\n", i, bv.n)
	}
	mask := uint32(1 << uint(i%wordBits))
...
func (bv bvec) Unset(i int32) {
	if i < 0 || i >= bv.n {
		base.Fatal("bvunset: index %d is out of bounds with length %d\n", i, bv.n)
	}
	mask := uint32(1 << uint(i%wordBits))
	bv.b[i/wordBits] &^= mask
}
