...
// foundinitloop print an init loop error and exits.
func foundinitloop(n, nv *Node) {
	...
	// Find n and nv.
	var ni, nvi int
	for ; initlist[ni] != n; ni++ {
	}
	for ; initlist[nvi] != nv; nvi++ {
	}
	...
	// Print nv -> ... -> n1 -> n.
	for _, n := range initlist[nvi:] {
		fmt.Printf("\t%v %v refers to\n", n.Line(), n.Sym)
	}

	// Print n -> ... -> nv.
	for _, n := range initlist[ni:nvi] {
		fmt.Printf("\t%v %v refers to\n", n.Line(), n.Sym)
	}
