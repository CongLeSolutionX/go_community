...
func walkselectcases(cases *Nodes) []*Node {
	...
	if n == 2 && (cases.First().Left == nil || cases.Second().Left == nil) {
		...
		case OSELRECV2:
			...
			r.Left = mkcall1(chanfn("selectnbrecv2", 2, ch.Type), types.Types[TBOOL], &r.Ninit, n.Left, receivedp, ch)
		}
	...
	// generate sel-struct
	lineno = sellineno
	selv := temp(typArray(scasetype(), int64(n)))
	r := nod(OAS, selv, nil)
	r = typecheck(r, Etop)
	init = append(init, r)

	order := temp(typArray(Types[TUINT16], 2*int64(n)))
	r = nod(OAS, order, nil)
	r = typecheck(r, Etop)
	init = append(init, r)
