...
func walkselectcases(cases ir.Nodes) []ir.Node {
	...
	for _, cas := range cases.Slice() {
		...
		if sel := n; sel.Op() == ir.OSELRECV2 {
			sel := sel.(*ir.AssignListStmt)
			if ir.IsBlank(sel.Lhs.Second()) {
				as := ir.NewAssignStmt(sel.Pos(), sel.Lhs.First(), sel.Rhs.First())
				as.SetOp(ir.OSELRECV)
				as.SetTypecheck(1)
				n = as
				cas.Comm = n
		...
		case ir.OSEND:
			n := n.(*ir.SendStmt)
			n.Value = ir.NewAddrExpr(base.Pos, n.Value)
			n.Value = typecheck(n.Value, ctxExpr)

		case ir.OSELRECV:
			n := n.(*ir.AssignStmt)
			if !ir.IsBlank(n.X) {
				n.X = ir.NewAddrExpr(base.Pos, n.X)
		...
		case ir.OSELRECV2:
			...
			if !ir.IsBlank(n.Lhs.First()) {
				n.Lhs.SetIndex(0, ir.NewAddrExpr(base.Pos, n.Lhs.First()))
	...
	if ncas == 2 && dflt != nil {
		...
		case ir.OSELRECV2:
			...
			receivedp := typecheck(ir.NewAddrExpr(base.Pos, n.Lhs.Second()), ctxExpr)
