...
func walkselectcases(cases ir.Nodes) []ir.Node {
	...
	for _, cas := range cases {
		...
		if sel := n; sel.Op() == ir.OSELRECV2 {
			...
			if ir.IsBlank(sel.Lhs[1]) {
				as := ir.NewAssignStmt(sel.Pos(), sel.Lhs[0], sel.Rhs[0])
				as.SetOp(ir.OSELRECV)
				as.SetTypecheck(1)
				n = as
				cas.Comm = n
		...
		case ir.OSELRECV2:
			...
			if !ir.IsBlank(n.Lhs[0]) {
				n.Lhs[0] = ir.NewAddrExpr(base.Pos, n.Lhs[0])
	...
	if ncas == 2 && dflt != nil {
		...
		case ir.OSELRECV2:
			...
			receivedp := typecheck(ir.NewAddrExpr(base.Pos, n.Lhs[1]), ctxExpr)
