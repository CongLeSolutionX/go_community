...
func Export(out io.Writer) int {
	...
	myexports := append([]*Node(nil), exportlist...)
	exportlist = nil
	...
	// determine which types still left to write
	i := 0
	for _, t := range types {
...
func (p *exporter) collectInlined(n *Node) {
	if n != nil && n.Func != nil && n.Func.Inl != nil {
		...
		// TODO(gri) record inlined function for export in compiler-specific export section
	}
}
...
// for anonymous fields (sym == nil), or blank or exported names.
func (p *exporter) fieldName(sym *Sym) {
	var name string
	if sym != nil {
		name = sym.Name
	}
	p.string(name)
	if name != "" && name != "_" && !exportname(name) {
		p.pkg(sym.Pkg)
	}
}
