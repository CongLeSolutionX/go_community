...
func export(out *bufio.Writer, trace bool) int {
	...
	for _, n := range exportlist {
		...
		if sym.Exported() {
			...
			// already set. The export loops below over them, so
...
func (p *exporter) markType(t *types.Type) {
	...
	// If this is a named type, mark all of its methods. Skip
	// interface types because t.Methods is reused
	...
	// value of type t: dereferencing an pointer; indexing an
...
func isInlineable(n *Node) bool {
	if exportInlined && n != nil && n.Func != nil {
		...
		//
		// TODO(mdempsky): Having the Inl.Len and ExporInline
		// checks here instead of the outer if statement means
		// we end up exporting parameter names even for
		// functions that aren't inlineable or whose inline
		// body won't be exported. This is currently necessary
		// because we might first import a function/method
		// from a package where it doesn't need to be
		// re-exported, and then from a package where it
		// does. If this happens, we'll need the parameter
		// names.
		//
		// We could initially do without the parameter names,
		// and then replace them when importing the inline
		// body. But parameter names are attached to the
		// function type, and replacing types after the fact
		// is a little sketchy.
		if Debug_typecheckinl == 0 && n.Func.Inl.Len() != 0 && n.Func.ExportInline() {
			typecheckinl(n)
		}
		return true
	}
