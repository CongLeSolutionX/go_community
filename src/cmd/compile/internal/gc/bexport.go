...
// debugging support
const (
	debugFormat = true // use debugging format for export data (emits a lot of additional data)
	traceExport = true // enable printing of export data in human-readable form
)

const exportVersion = "v0"

// Export writes the export data for pkg to out.
func Export(out io.Writer, pkg *Pkg) {
	...
	if traceExport {
		p.tracef("\n--- generic export data ---\n")
		if p.indent != 0 {
			Fatal("incorrect indentation %d", p.indent)
		}
	}
	...
	// TODO(gri) support "safe" mode in export data
...
func (p *exporter) flush() {
	...
	if err != nil || w != len(p.buf) {
		Fatal("error while writing export data")
	}
	p.buf = p.buf[:0]
}
