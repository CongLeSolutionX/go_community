...
// 	inlineAllocatorCallSize    = 60 // The number of nodes (not "budget") for which an allocator call penalty applies
// 	inlineAllocatorCallPenalty = 20 // A function that allocates and could plausibly be inlined applies this penalty to any calls it makes.
// 	inlineCalleeNoleakPenalty  = 20 // Be less aggressive about inlining functions that don't leak their parameters.
...
var inlineExtraThrowCost = getEnvInt("GO_INLTHROWEXTRA", 80)
var inlineExtraPanicCost = getEnvInt("GO_INLPANICEXTRA", 1)

var inlineAllocatorCallSize = int(getEnvInt("GO_INLALLOCATORCALLSIZE", 0))  // The number of nodes (not "budget") for which an allocator call penalty applies
var inlineAllocatorCallPenalty = getEnvInt("GO_INLALLOCATORCALLPENALTY", 0) // A function that allocates and could plausibly be inlined applies this penalty to any calls it makes.
var inlineCalleeNoleakPenalty = getEnvInt("GO_INLCALLEENOLEAKPENALTY", 0)   // Be less aggressive about inlining functions that don't leak their parameters.  Cumulative w/ allocator call penalty.
...
func debugThisFn(fn *Node) bool {
	return false && fn.funcname() == "NewInt"
}

// Caninl determines whether fn is inlineable.
// If so, caninl saves fn->nbody in fn->inl and substitutes it with a copy.
// fn and ->nbody will already have been typechecked.
func caninl(fn *Node, maxCost, allocatorCallPenalty int32) {
	if fn.Op != ODCLFUNC {
		Fatalf("caninl %v", fn)
	}
	...
	visitor := hairyVisitor{
		...
		allocatorCallPenalty: allocatorCallPenalty,
	...
	case OCALLMETH:
		...
		if inlfn := asNode(t.FuncType().Nname).Func; inlfn.Inl != nil && inlfn.Inl.Cost < v.maxInlineCost-v.allocatorCallPenalty-escapeTweakedInlinePenalty(asNode(t.FuncType().Nname), v.debug) {
			if v.debug {
				...
				nNodes, nAllocs := countNodes(asnode, 0, 0)
				Warnl(n.Pos, "modeling inlined call of %s, defined %v, escaped %v, nNodes %d, nAllocs %d, cost %d, limit %d", n.Left.Sym.Name, nonNilDef, escAvail, nNodes, nAllocs, inlfn.Inl.Cost, v.maxInlineCost-v.allocatorCallPenalty-escapeTweakedInlinePenalty(asnode, true))
			}
...
func countNodes(n *Node, nN, nA int) (int, int) {
	if n == nil {
		return nN, nA
	}
	nN++
	switch n.Op {
	case ONEW, ONEWOBJ: // Others?
		nA++
	}
	nN, nA = countNodes(n.Left, nN, nA)
	nN, nA = countNodes(n.Right, nN, nA)
	for _, n1 := range n.Ninit.Slice() {
		nN, nA = countNodes(n1, nN, nA)
	}
...
func maxInlineCost(fn *Node) (int32, int32) {
	...
	nNodes, nAllocs := countNodes(fn, 0, 0)
	if nNodes >= inlineBigFunctionNodes {
		return inlineBigFunctionMaxCost, 0
	}
	if nNodes < inlineAllocatorCallSize && nAllocs > 0 {
...
func inlcalls(fn *Node, maxCost, allocatorCallPenalty int32) {
	savefn := Curfn
	Curfn = fn
	...
	if debugThisFn(fn) {
		Warnl(fn.Pos, "inlnode(%s.%s,%d,%d)", myimportpath, fn.funcname(), maxCost, allocatorCallPenalty)
	}
	fn = inlnode(fn, maxCost, allocatorCallPenalty, inlMap)
	if fn != Curfn {
...
func inlnode(n *Node, maxCost, allocatorCallPenalty int32, inlMap map[*Node]bool) *Node {
	if n == nil {
		return n
	}
	...
	inlnodelist(n.Ninit, maxCost, allocatorCallPenalty, inlMap)
	for _, n1 := range n.Ninit.Slice() {
		if n1.Op == OINLCALL {
			inlconv2stmt(n1)
		}
	...
	n.Left = inlnode(n.Left, maxCost, allocatorCallPenalty, inlMap)
	if n.Left != nil && n.Left.Op == OINLCALL {
		n.Left = inlconv2expr(n.Left)
	}

	n.Right = inlnode(n.Right, maxCost, allocatorCallPenalty, inlMap)
	...
	inlnodelist(n.List, maxCost, allocatorCallPenalty, inlMap)
	if n.Op == OBLOCK {
	...
	inlnodelist(n.Rlist, maxCost, allocatorCallPenalty, inlMap)
	s := n.Rlist.Slice()
	for i1, n1 := range s {
		if n1.Op == OINLCALL {
			if n.Op == OIF {
	...
	inlnodelist(n.Nbody, maxCost, allocatorCallPenalty, inlMap)
	...
	case OCALLFUNC:
		...
		if fn := inlCallee(n.Left, maxCost, allocatorCallPenalty); fn != nil && fn.Func.Inl != nil {
			n = mkinlcall(n, fn, maxCost, allocatorCallPenalty, inlMap)
		}
	...
	case OCALLMETH:
		...
		n = mkinlcall(n, asNode(n.Left.Type.FuncType().Nname), maxCost, allocatorCallPenalty, inlMap)
	}
...
func inlCallee(fn *Node, maxCost, allocatorCallPenalty int32) *Node {
	fn = staticValue(fn)
	switch {
	...
	case fn.Op == OCLOSURE:
		c := fn.Func.Closure
		caninl(c, maxCost, allocatorCallPenalty)
		return c.Func.Nname
	}
	return nil
}
...
func mkinlcall(n, fn *Node, maxCost, allocatorCallPenalty int32, inlMap map[*Node]bool) *Node {
	if fn.Func.Inl == nil {
		if logopt.Enabled() {
	...
	inlnodelist(call.Nbody, maxCost, allocatorCallPenalty, inlMap)
