...
const (
	...
	inlineAllocatorCallPenalty = 8  // A function that allocates and could plausibly be inlined applies this penalty to any calls it makes.
...
// countNodes returns numnber of nodes, allocations, and calls
func countNodes(n *Node, nN, nA, nC int) (int, int, int) {
	if n == nil {
		return nN, nA, nC
	}
	nN++
	...
	nN, nA, nC = countNodes(n.Left, nN, nA, nC)
	nN, nA, nC = countNodes(n.Right, nN, nA, nC)
	for _, n1 := range n.Ninit.Slice() {
		nN, nA, nC = countNodes(n1, nN, nA, nC)
	}
...
func maxInlineCost(fn *Node) (int32, int32) {
	...
	if nNodes < inlineAllocatorCallSize && nCalls == 1 && nAllocs > 0 {
		...
		// Zero calls means no point settings this, two or more calls means this is not getting inlined anyway, modulo OIF-max-not-sum weighting.
