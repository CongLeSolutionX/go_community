...
func inlconv2stmt(n *Node, maxCost int32) {
	n.Op = OBLOCK
...
func inlconv2expr(n *Node, maxCost int32) *Node {
	r := n.Rlist.First()
...
// statements.
func inlconv2list(n *Node, maxCost int32) []*Node {
...
func inlnode(n *Node, maxCost int32) *Node {
	...
	for _, n1 := range n.Ninit.Slice() {
		if n1.Op == OINLCALL {
			inlconv2stmt(n1, maxCost)
		}
	}

	n.Left = inlnode(n.Left, maxCost)
	if n.Left != nil && n.Left.Op == OINLCALL {
		n.Left = inlconv2expr(n.Left, maxCost)
	}

	n.Right = inlnode(n.Right, maxCost)
	if n.Right != nil && n.Right.Op == OINLCALL {
		if n.Op == OFOR || n.Op == OFORUNTIL {
			inlconv2stmt(n.Right, maxCost)
		} else {
			n.Right = inlconv2expr(n.Right, maxCost)
		}
	}
	...
	case ORETURN, OCALLFUNC, OCALLMETH, OCALLINTER, OAPPEND, OCOMPLEX:
		...
		if n.List.Len() == 1 && n.List.First().Op == OINLCALL && n.List.First().Rlist.Len() > 1 {
			n.List.Set(inlconv2list(n.List.First(), maxCost))
			break
		}
		fallthrough
	...
	default:
		...
		for i1, n1 := range s {
			if n1 != nil && n1.Op == OINLCALL {
				s[i1] = inlconv2expr(s[i1], maxCost)
			}
		}
	}
	...
	if n.Op == OAS2FUNC && n.Rlist.First().Op == OINLCALL {
		n.Rlist.Set(inlconv2list(n.Rlist.First(), maxCost))
		n.Op = OAS2
		n.SetTypecheck(0)
		n = typecheck(n, Etop)
	} else {
