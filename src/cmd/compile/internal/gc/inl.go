...
func tinlvar(t *Field, inlvars map[*Name]*Node) *Node {
	if t.Nname != nil && !isblank(t.Nname) {
		inlvar := inlvars[t.Nname.Name]
		if inlvar == nil {
...
func mkinlcall1(n *Node, fn *Node, isddd bool) *Node {
	...
	for _, ln := range dcl {
		...
		if ln.Op == ONAME {
			inlvars[ln.Name] = typecheck(inlvar(ln), Erv)
			if ln.Class == PPARAM || ln.Name.Param.Stackcopy != nil && ln.Name.Param.Stackcopy.Class == PPARAM {
				ninit.Append(nod(ODCL, inlvars[ln.Name], nil))
			}
		}
	}
	...
	if fn.IsMethod() && n.Left.Op == ODOTMETH {
		...
		if t != nil && t.Nname != nil && !isblank(t.Nname) && inlvars[t.Nname.Name] == nil {
			Fatalf("missing inlvar for %v\n", t.Nname)
		}
		if n.Left.Left == nil {
...
func (subst *inlsubst) node(n *Node) *Node {
	...
	switch n.Op {
	case ONAME:
		if inlvar := subst.inlvars[n.Name]; inlvar != nil { // These will be set during inlnode
			if Debug['m'] > 2 {
