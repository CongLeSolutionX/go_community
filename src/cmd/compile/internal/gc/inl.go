	...
	case OCALLMETH:
		...
		if inlfn := asNode(t.FuncType().Nname).Func; inlfn.Inl != nil && inlfn.Inl.Cost < v.maxInlineCost-v.allocatorCallPenalty {
			if v.debug {
				...
				Warnl(n.Pos, "modeling inlined call of %s, defined %v, escaped %v, nNodes %d, nAllocs %d, cost %d, limit %d", n.Left.Sym.Name, nonNilDef, escAvail, nNodes, nAllocs, inlfn.Inl.Cost, v.maxInlineCost-v.allocatorCallPenalty)
			}
