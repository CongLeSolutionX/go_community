...
func mkinlcall1(n *Node, fn *Node, isddd bool) *Node {
	...
	for _, ln := range dcl {
		...
		if ln.Op == ONAME {
			...
			if ln.Class == PPARAM || ln.Name.Param.Stackcopy != nil && ln.Name.Param.Stackcopy.Class == PPARAM {
				ninit.AppendNode(nod(ODCL, inlvars[ln], nil))
			}
		}
	}
	...
	for _, t := range fn.Type.Results().Fields().Slice() {
		...
		ninit.AppendNode(nod(ODCL, m, nil))
	...
	if fn.IsMethod() && n.Left.Op != ODOTMETH {
		...
		as.List.AppendNode(tinlvar(t, inlvars))
		li++
	}
	...
	case ORETURN:
		...
		if len(subst.retvars) != 0 && n.List.Len() != 0 {
			...
			for _, n := range subst.retvars {
				as.List.AppendNode(n)
			}
			as.Rlist.Set(subst.list(n.List))
			as = typecheck(as, Etop)
			m.Ninit.AppendNode(as)
		}
