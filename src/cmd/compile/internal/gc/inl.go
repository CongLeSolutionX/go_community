...
func caninl(fn *ir.Func) {
	...
	n := fn.Nname
	if n.Func_.InlinabilityChecked() {
		return
	}
	defer n.Func_.SetInlinabilityChecked(true)
	...
	if base.Flag.LowerM > 1 {
		fmt.Printf("%v: can inline %v with cost %d as: %v { %v }\n", ir.Line(fn), n, inlineMaxBudget-visitor.budget, fn.Type(), ir.AsNodes(n.Func_.Inl.Body))
...
func (v *hairyVisitor) doNode(n ir.Node) error {
	...
	case ir.OCALLMETH:
		...
		if inlfn := methodExprName(n.X).Func_; inlfn.Inl != nil {
...
func inlnode(n ir.Node, maxCost int32, inlMap map[*ir.Func]bool, edit func(ir.Node) ir.Node) ir.Node {
	...
	case ir.OCALLMETH:
		...
		n = mkinlcall(call, methodExprName(call.X).Func_, maxCost, inlMap, edit)
	}
	...
	if c := fn.OClosure; c != nil {
		for _, v := range fn.ClosureVars {
			...
			if v.Byval() {
				...
				ninit.Append(typecheck(ir.NewAssignStmt(base.Pos, ia, ir.NewAddrExpr(base.Pos, o)), ctxStmt))
				inlvars[addr] = ia
