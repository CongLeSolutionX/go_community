...
func deferstruct(stksize int64) *types.Type {
	...
	s.SetNoalg(true)
	old := types.CalcSizeDisabled
	types.CalcSizeDisabled = false
	types.CalcSize(s)
	types.CalcSizeDisabled = old
	return s
}
...
func dimportpath(p *types.Pkg) {
	...
	if base.Ctxt.Pkgpath == "runtime" && p == types.Pkgs.Runtime {
		return
	}
	...
	wobj.Global(s, int32(ot), obj.DUPOK|obj.RODATA)
...
func dgopkgpath(s *obj.LSym, ot int, pkg *types.Pkg) int {
	...
	if pkg == types.LocalPkg && base.Ctxt.Pkgpath == "" {
		...
		return wobj.SymPtr(s, ot, ns, 0)
	}

	dimportpath(pkg)
	return wobj.SymPtr(s, ot, pkg.Pathsym, 0)
}
...
func dgopkgpathOff(s *obj.LSym, ot int, pkg *types.Pkg) int {
	...
	if pkg == types.LocalPkg && base.Ctxt.Pkgpath == "" {
		...
		// See also https://groups.google.com/forum/#!topic/golang-dev/myb9s53HxGQ.
		ns := base.Ctxt.Lookup(`type..importpath."".`)
		return wobj.SymPtrOff(s, ot, ns)
	}

	dimportpath(pkg)
	return wobj.SymPtrOff(s, ot, pkg.Pathsym)
}
...
func dnameField(lsym *obj.LSym, ot int, spkg *types.Pkg, ft *types.Field) int {
	...
	return wobj.SymPtr(lsym, ot, nsym, 0)
}
...
func dextratype(lsym *obj.LSym, ot int, t *types.Type, dataAdd int) int {
	...
	ot = wobj.Uint16(lsym, ot, uint16(mcount))
	ot = wobj.Uint16(lsym, ot, uint16(xcount))
	ot = wobj.Uint32(lsym, ot, uint32(dataAdd))
	ot = wobj.Uint32(lsym, ot, 0)
	return ot
}
...
func dextratypeData(lsym *obj.LSym, ot int, t *types.Type) int {
	for _, a := range methods(t) {
		...
		ot = wobj.SymPtrOff(lsym, ot, nsym)
...
func dcommontype(lsym *obj.LSym, t *types.Type) int {
	...
	//	}
	ot := 0
	ot = wobj.Uintptr(lsym, ot, uint64(t.Width))
	ot = wobj.Uintptr(lsym, ot, uint64(ptrdata))
	ot = wobj.Uint32(lsym, ot, types.TypeHash(t))
	...
	ot = wobj.Uint8(lsym, ot, t.Align) // align
	ot = wobj.Uint8(lsym, ot, t.Align) // fieldAlign
	...
	ot = wobj.Uint8(lsym, ot, uint8(i)) // kind
	if eqfunc != nil {
		ot = wobj.SymPtr(lsym, ot, eqfunc, 0) // equality function
	} else {
		ot = wobj.Uintptr(lsym, ot, 0) // type we can't do == with
	}
	ot = wobj.SymPtr(lsym, ot, gcsym, 0) // gcdata

	nsym := dname(p, "", nil, exported)
	ot = wobj.SymPtrOff(lsym, ot, nsym) // str
	// ptrToThis
	if sptr == nil {
		ot = wobj.Uint32(lsym, ot, 0)
	} else if sptrWeak {
		ot = wobj.SymPtrWeakOff(lsym, ot, sptr)
	} else {
		ot = wobj.SymPtrOff(lsym, ot, sptr)
	}
...
func tracksym(t *types.Type, f *types.Field) *types.Sym {
	return types.Pkgs.Track.Lookup(t.ShortString() + "." + f.Sym.Name)
}
...
func dtypesym(t *types.Type) *obj.LSym {
	...
	case types.TARRAY:
		...
		s2 := dtypesym(t2)
		ot = dcommontype(lsym, t)
		ot = wobj.SymPtr(lsym, ot, s1, 0)
		ot = wobj.SymPtr(lsym, ot, s2, 0)
		ot = wobj.Uintptr(lsym, ot, uint64(t.NumElem()))
		ot = dextratype(lsym, ot, t, 0)
	...
	case types.TCHAN:
		...
		ot = dcommontype(lsym, t)
		ot = wobj.SymPtr(lsym, ot, s1, 0)
		ot = wobj.Uintptr(lsym, ot, uint64(t.ChanDir()))
		ot = dextratype(lsym, ot, t, 0)
	...
	case types.TFUNC:
		...
		ot = wobj.Uint16(lsym, ot, uint16(inCount))
		ot = wobj.Uint16(lsym, ot, uint16(outCount))
		if types.PtrSize == 8 {
		...
		for _, t1 := range t.Recvs().Fields().Slice() {
			ot = wobj.SymPtr(lsym, ot, dtypesym(t1.Type), 0)
		}
		for _, t1 := range t.Params().Fields().Slice() {
			ot = wobj.SymPtr(lsym, ot, dtypesym(t1.Type), 0)
		}
	...
	case types.TINTER:
		...
		ot = wobj.SymPtr(lsym, ot, lsym, ot+3*types.PtrSize+uncommonSize(t))
		ot = wobj.Uintptr(lsym, ot, uint64(n))
		ot = wobj.Uintptr(lsym, ot, uint64(n))
		...
		for _, a := range m {
			...
			ot = wobj.SymPtrOff(lsym, ot, nsym)
			ot = wobj.SymPtrOff(lsym, ot, dtypesym(a.type_))
		}
	...
	case types.TMAP:
		...
		ot = wobj.SymPtr(lsym, ot, s1, 0)
		ot = wobj.SymPtr(lsym, ot, s2, 0)
		ot = wobj.SymPtr(lsym, ot, s3, 0)
		ot = wobj.SymPtr(lsym, ot, hasher, 0)
		var flags uint32
		// Note: flags must match maptype accessors in ../../../../runtime/type.go
		// and maptype builder in ../../../../reflect/type.go:MapOf.
		if t.Key().Width > MAXKEYSIZE {
			ot = wobj.Uint8(lsym, ot, uint8(types.PtrSize))
			flags |= 1 // indirect key
		} else {
			ot = wobj.Uint8(lsym, ot, uint8(t.Key().Width))
		}
		...
		if t.Elem().Width > MAXELEMSIZE {
			...
			ot = wobj.Uint8(lsym, ot, uint8(t.Elem().Width))
		}
		ot = wobj.Uint16(lsym, ot, uint16(bmap(t).Width))
	...
	case types.TSTRUCT:
		...
		ot = wobj.SymPtr(lsym, ot, lsym, ot+3*types.PtrSize+uncommonSize(t))
		ot = wobj.Uintptr(lsym, ot, uint64(len(fields)))
		ot = wobj.Uintptr(lsym, ot, uint64(len(fields)))
		...
		for _, f := range fields {
			...
			ot = wobj.SymPtr(lsym, ot, dtypesym(f.Type), 0)
			...
			ot = wobj.Uintptr(lsym, ot, offsetAnon)
		}
	}

	ot = dextratypeData(lsym, ot, t)
	wobj.Global(lsym, int32(ot), int16(dupok|obj.RODATA))
...
func dumptabs() {
	...
	for _, i := range itabs {
		...
		// }
		o := wobj.SymPtr(i.lsym, 0, dtypesym(i.itype), 0)
		o = wobj.SymPtr(i.lsym, o, dtypesym(i.t), 0)
		o = wobj.Uint32(i.lsym, o, types.TypeHash(i.t)) // copy of type hash
		o += 4                                          // skip unused field
		for _, fn := range genfun(i.t, i.itype) {
			o = wobj.SymPtr(i.lsym, o, fn, 0) // method pointer for each method
		}
		// Nothing writes static itabs, so they are read only.
		wobj.Global(i.lsym, int32(o), int16(obj.DUPOK|obj.RODATA))
	...
	if types.LocalPkg.Name == "main" && len(ptabs) > 0 {
		...
		for _, p := range ptabs {
			...
			tsym := dtypesym(p.t)
			ot = wobj.SymPtrOff(s, ot, nsym)
			ot = wobj.SymPtrOff(s, ot, tsym)
			// Plugin exports symbols as interfaces. Mark their types
			// as UsedInIface.
			tsym.Set(obj.AttrUsedInIface, true)
		}
		wobj.Global(s, int32(ot), int16(obj.RODATA))

		ot = 0
		s = base.Ctxt.Lookup("go.plugin.exports")
		for _, p := range ptabs {
			ot = wobj.SymPtr(s, ot, p.s.Linksym(), 0)
		}
		wobj.Global(s, int32(ot), int16(obj.RODATA))
	}
}
...
func dumpbasictypes() {
	...
	if base.Ctxt.Pkgpath == "runtime" {
		...
		if base.Flag.Race {
			dimportpath(types.Pkgs.Race)
		}
		if base.Flag.MSan {
			dimportpath(types.Pkgs.Msan)
		}
...
func dgcptrmask(t *types.Type) *obj.LSym {
	...
	sym := types.Pkgs.Runtime.Lookup(p)
	lsym := sym.Linksym()
	if !sym.Uniq() {
		sym.SetUniq(true)
		...
		wobj.Global(lsym, int32(len(ptrmask)), obj.DUPOK|obj.RODATA|obj.LOCAL)
...
func (p *GCProg) writeByte(x byte) {
	p.symoff = wobj.Uint8(p.lsym, p.symoff, x)
}

func (p *GCProg) end() {
	p.w.End()
	wobj.Uint32(p.lsym, 0, uint32(p.symoff-4))
	wobj.Global(p.lsym, int32(p.symoff), obj.DUPOK|obj.RODATA|obj.LOCAL)
	if base.Debug.GCProg > 0 {
