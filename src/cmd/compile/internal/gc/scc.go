...
type bottomUpVisitor struct {
	analyze  func([]ir.INode, bool)
	visitgen uint32
	nodeID   map[ir.INode]uint32
	stack    []ir.INode
}
...
func visitBottomUp(list []ir.INode, analyze func(list []ir.INode, recursive bool)) {
	var v bottomUpVisitor
	v.analyze = analyze
	v.nodeID = make(map[ir.INode]uint32)
	for _, n := range list {
...
func (v *bottomUpVisitor) visit(n ir.INode) uint32 {
