...
func assignconvfn(n *Node, t *types.Type, context func() string) *Node {
	...
	if op == 0 {
		if !old.Diag() {
			if t.Sym == nil || n.Type == nil || n.Type.Sym == nil || t.Sym.Name != n.Type.Sym.Name {
			...
			nName, tName := pkgNameInErr(n.Type.Sym.Pkg), pkgNameInErr(t.Sym.Pkg)
			if nName == tName || tName == "" || nName == "" {
				yyerror("cannot use %L as type %v in %s%s", n, t, context(), why)
			} else {
				yyerror("cannot use\n\t%L (package %s)\nas\n\ttype %v (package %s)\nin %s%s",
					n, nName, t, tName, context(), why)
			}
		}
		op = OCONV
	}
