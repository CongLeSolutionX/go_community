...
// with Type t. If one exists, it will be returned in save (if not nil).
...
// If one exists, it will be returned in save (if not nil), and dotlist will
// contain the path of embedded fields traversed to find it (in reverse order).
// If none exist, more will reports whether t contains any embedded fields
// at depth d, so callers can decide whether to retry at a greater depth.
