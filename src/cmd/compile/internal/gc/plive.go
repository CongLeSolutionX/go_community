...
func (lv *Liveness) initcache() {
	...
	lv.cache.textavarinit = bvalloc(int32(len(lv.vars)))
	lv.cache.retuevar = bvalloc(int32(len(lv.vars)))
	lv.cache.tailuevar = bvalloc(int32(len(lv.vars)))
	...
	for i, node := range lv.vars {
		...
		case PPARAM:
			...
			// function runs.
			lv.cache.tailuevar.Set(int32(i))

			if node.Addrtaken() {
				lv.cache.textavarinit.Set(int32(i))
			}
...
func newliveness(f *ssa.Func, vars []*Node) *Liveness {
	lv := &Liveness{
		f:    f,
		vars: vars,
	...
	bebulk := make([]BlockEffects, len(f.Blocks))
...
func livenesssolve(lv *Liveness) {
	...
	for change := true; change; {
		...
		for _, b := range po {
			...
			switch b.Kind {
			case ssa.BlockRet:
				newliveout.Copy(lv.cache.retuevar)
			case ssa.BlockRetJmp:
				newliveout.Copy(lv.cache.tailuevar)
			case ssa.BlockExit:
				newliveout.Clear()
			default:
...
func livenessepilogue(lv *Liveness) {
	...
	{
		live := bvalloc(nvars)
		live.Copy(lv.cache.textavarinit)
	...
	for _, b := range lv.f.Blocks {
		...
		for i := len(b.Values) - 1; i >= 0; i-- {
			...
			live.Or(live, livedefer) // only for calls
...
func (lv *Liveness) avarinitanyall(b *ssa.Block, any, all bvec) {
	if len(b.Preds) == 0 {
		any.Copy(lv.cache.textavarinit)
		all.Copy(lv.cache.textavarinit)
		return
	}
...
func liveness(fn *Node, f *ssa.Func, argssym, livesym *Sym) map[*ssa.Value]int {
	...
	vars := getvariables(Curfn)
	lv := newliveness(f, vars)
