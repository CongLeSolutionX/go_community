...
func livenessepilogue(lv *Liveness) {
	...
	if obj.Clobberdead_enabled != 0 {
		...
		if clobberDead && lv.f.Name == "forkAndExecInChild" {
		...
		if clobberDead {
			...
			for _, b := range lv.f.Blocks {
				// Find index of first safepoint.
				index := int32(lv.blockEffects(b).lastbitmapindex) + 1
				for _, v := range b.Values {
					if issafepoint(v) {
						index--
					}
				...
				if b == lv.f.Entry {
					if index != 1 {
						Fatalf("index of entry block wrong")
					}
				...
				for _, v := range oldSched {
					...
					if before {
						clobber(lv, b, lv.livevars[index])
					}
					b.Values = append(b.Values, v)
					clobber(lv, b, lv.livevars[index])
					index++
				}
				if index != int32(lv.blockEffects(b).lastbitmapindex+1) {
					Fatalf("indexes don't match")
				}
			}
		}
	}
