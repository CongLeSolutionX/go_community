...
func (lv *Liveness) initcache() {
	...
	for i, node := range lv.vars {
		...
		case PPARAMOUT:
			// TODO: how does PPARAMHEAP interact with this?
			// TODO: also check addrtaken / stackobject interaction
...
func (lv *Liveness) epilogue() {
	...
	for _, b := range lv.f.Blocks {
		...
		if b == lv.f.Entry {
			...
			// Set Needzero for live PPARAMOUT variables.
			// Then remove them from the live variable set.
			// (The zeroing done by Needzero happens after the
			// the morestack check but before the entry block starts.)
			// No other variables should be live here.
			for i, n := range lv.vars {
