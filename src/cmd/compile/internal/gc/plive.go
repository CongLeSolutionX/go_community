...
func clobber(bb *BasicBlock, p *obj.Prog, v *Node, args bvec, locals bvec) {
	switch v.Class {
	case PAUTO:
		clobberWalk(bb, p, v, v.Xoffset+stkptrsize, v.Type, locals)

	case PPARAM, PPARAMOUT:
		clobberWalk(bb, p, v, v.Xoffset, v.Type, args)
	}
}
...
func clobberWalk(bb *BasicBlock, p *obj.Prog, v *Node, offset int64, t *Type, bv bvec) {
	switch t.Etype {
	case TINT8,
		TUINT8,
		TINT16,
	...
	case TARRAY:
		for i := int64(0); i < t.NumElem(); i++ {
			clobberWalk(bb, p, v, offset+i*t.Elem().Size(), t.Elem(), bv)
		}

	case TSTRUCT:
		for _, t1 := range t.Fields().Slice() {
			clobberWalk(bb, p, v, offset+t1.Offset, t1.Type, bv)
		}
			...
			if issafepoint(p) {
				...
				if use_deadbeef && nvars < 10000 && len(lv.livepointers) < 10000 {
					...
					for _, v := range lv.vars {
						...
						clobber(bb, p, v, args, locals)

						// Also add clobbering before p, if we can.
						if before {
							clobber(bb, c, v, args, locals)
						}
					}
				}
