...
func getvariables(fn ir.INode) ([]ir.INode, map[ir.INode]int32) {
	...
	idx := make(map[ir.INode]int32, len(vars))
	for i, n := range vars {
		idx[n] = int32(i)
	}
	return vars, idx
...
func (lv *Liveness) initcache() {
	if lv.cache.initialized {
		base.Fatal("liveness cache initialized twice")
		return
	}
...
func affectedNode(v *ssa.Value) (ir.INode, ssa.SymEffect) {
	// Special cases.
	switch v.Op {
	case ssa.OpLoadReg:
	...
	case ssa.OpVarLive:
		return v.Aux.(ir.INode), ssa.SymRead
	case ssa.OpVarDef, ssa.OpVarKill:
		return v.Aux.(ir.INode), ssa.SymWrite
	...
	case ir.INode:
		return a, e
	default:
		base.Fatal("weird aux: %s", v.LongString())
		return nil, e
	}
}
...
func newliveness(fn ir.INode, f *ssa.Func, vars []ir.INode, idx map[ir.INode]int32, stkptrsize int64) *Liveness {
	lv := &Liveness{
		fn:         fn,
		f:          f,
		vars:       vars,
...
func onebitwalktype1(t *types.Type, off int64, bv bvec) {
	if t.Align > 0 && off&int64(t.Align-1) != 0 {
		base.Fatal("onebitwalktype1: invalid initial alignment: type %v has alignment %d, but offset is %v", t, t.Align, off)
	}
	if !t.HasPointers() {
	...
	case types.TPTR, types.TUNSAFEPTR, types.TFUNC, types.TCHAN, types.TMAP:
		if off&int64(Widthptr-1) != 0 {
			base.Fatal("onebitwalktype1: invalid alignment, %v", t)
		}
	...
	case types.TSLICE:
		...
		if off&int64(Widthptr-1) != 0 {
			base.Fatal("onebitwalktype1: invalid TARRAY alignment, %v", t)
		}
	...
	default:
		base.Fatal("onebitwalktype1: unexpected type, %v", t)
	}
}
...
func (lv *Liveness) pointerMap(liveout bvec, vars []ir.INode, args, locals bvec) {
	for i := int32(0); ; i++ {
		i = liveout.Next(i)
		if i < 0 {
			break
...
func (lv *Liveness) epilogue() {
	...
	for _, b := range lv.f.Blocks {
		...
		if b == lv.f.Entry {
			if index != 0 {
				base.Fatal("bad index for entry point: %v", index)
			}
