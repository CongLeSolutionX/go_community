				...
				if !liveout.IsEmpty() {
					for pos := int32(0); pos < liveout.n; pos++ {
						...
						if !n.Name.Needzero {
							n.Name.Needzero = true
							if debuglive >= 1 {
								Warnl(src.Lineno(p.Lineno), "%v: %L is ambiguously live", Curfn.Func.Nname, n)
							}
						}
					}
				}
			...
			if issafepoint(p) {
				...
				if p.As == obj.ATEXT {
					for j := int32(0); j < liveout.n; j++ {
						...
						n := lv.vars[j]
						if n.Class != PPARAM {
							yyerrorl(src.Lineno(p.Lineno), "internal error: %v %L recorded as live on entry, p.Pc=%v", Curfn.Func.Nname, n, p.Pc)
						}
					}
				}
