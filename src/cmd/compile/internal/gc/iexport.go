...
func iexport(out *bufio.Writer) {
	...
	if len(p.typIndex) > predeclReserved {
		base.Fatal("too many predeclared types: %d > %d", len(p.typIndex), predeclReserved)
	}
...
func (p *iexporter) doDecl(n *ir.Node) {
	...
	case ir.ONAME:
		...
		case ir.PFUNC:
			if n.IsMethod() {
				base.Fatal("unexpected method: %v", n)
			}
		...
		default:
			base.Fatal("unexpected class: %v, %v", n, n.Class())
		}
	...
	default:
		base.Fatal("unexpected node: %v", n)
	}
...
func (w *exportWriter) selector(s *types.Sym) {
	...
	if i := strings.LastIndex(name, "."); i >= 0 {
		...
		if s.Pkg != pkg {
			base.Fatal("package mismatch in selector: %v in package %q, but want %q", s, s.Pkg.Path, pkg.Path)
		}
	}
...
func (w *exportWriter) doTyp(t *types.Type) {
	if t.Sym != nil {
		if t.Sym.Pkg == ir.BuiltinPkg || t.Sym.Pkg == unsafepkg {
			base.Fatal("builtin type missing from typIndex: %v", t)
		}
...
func constTypeOf(typ *types.Type) ir.Ctype {
	switch typ {
	...
	switch typ.Etype {
	case types.TCHAN, types.TFUNC, types.TMAP, types.TNIL, types.TINTER, types.TPTR, types.TSLICE, types.TUNSAFEPTR:
		return ir.CTNIL
	case types.TBOOL:
		return ir.CTBOOL
	case types.TSTRING:
	...
	base.Fatal("unexpected constant type: %v", typ)
	return 0
}

func (w *exportWriter) value(typ *types.Type, v ir.Val) {
	if vt := idealType(v.Ctype()); typ.IsUntyped() && typ != vt {
		base.Fatal("exporter: untyped type mismatch, have: %v, want: %v", typ, vt)
	}
	w.typ(typ)
	...
	case ir.CTSTR:
		w.string(v.U.(string))
	case ir.CTINT:
		w.mpint(&v.U.(*ir.Int).Val, typ)
	case ir.CTFLT:
		w.mpfloat(&v.U.(*ir.Float).Val, typ)
	case ir.CTCPLX:
		x := v.U.(*ir.Complex)
...
func (w *exportWriter) mpint(x *big.Int, typ *types.Type) {
	...
	negative := x.Sign() < 0
	if !signed && negative {
		base.Fatal("negative unsigned integer; type %v, value %v", typ, x)
	}

	b := x.Bytes()
	if len(b) > 0 && b[0] == 0 {
		base.Fatal("leading zeros")
	}
	if uint(len(b)) > maxBytes {
		base.Fatal("bad mpint length: %d > %d (type %v, value %v)", len(b), maxBytes, typ, x)
	}
	...
	if n < maxSmall || n >= 256 {
		base.Fatal("encoding mistake: %d, %v, %v => %d", len(b), signed, negative, n)
	}
...
func (w *exportWriter) mpfloat(f *big.Float, typ *types.Type) {
	...
	if acc != big.Exact {
		base.Fatal("mantissa scaling failed for %f (%s)", f, acc)
	}
	w.mpint(manti, typ)
	if manti.Sign() != 0 {
		w.int64(exp)
...
func (w *exportWriter) methExt(m *types.Field) {
	...
	w.funcExt(ir.AsNode(m.Type.Nname()))
}
...
func (w *exportWriter) expr(n *ir.Node) {
	...
	case ir.OREAL, ir.OIMAG, ir.OAPPEND, ir.OCAP, ir.OCLOSE, ir.ODELETE, ir.OLEN, ir.OMAKE, ir.ONEW, ir.OPANIC, ir.ORECOVER, ir.OPRINT, ir.OPRINTN:
		...
		if op == ir.OAPPEND {
			...
			base.Fatal("exporter: unexpected '...' with %v call", op)
		}
...
func (w *exportWriter) localIdent(s *types.Sym, v int32) {
	...
	if i := strings.LastIndex(name, "."); i >= 0 && !strings.HasPrefix(name, ".autotmp_") {
		base.Fatal("unexpected dot in identifier: %v", name)
	}

	if v > 0 {
		if strings.Contains(name, "·") {
			base.Fatal("exporter: unexpected · in symbol name")
		}
		name = fmt.Sprintf("%s·%d", name, v)
	}

	if !types.IsExported(name) && s.Pkg != w.currPkg {
		base.Fatal("weird package in name: %v => %v, not %q", s, name, w.currPkg.Path)
	}
