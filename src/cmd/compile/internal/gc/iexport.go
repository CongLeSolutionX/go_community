...
func (p *iexporter) pushDecl(n *ir.Name) {
	...
	if n.Sym().Pkg == types.BuiltinPkg || n.Sym().Pkg == types.Pkgs.Unsafe {
		return
	}
...
func (p *iexporter) doInline(f *ir.Name) {
	...
	w.stmtList(ir.Nodes(f.Func_.Inl.Body))
...
func (w *exportWriter) doTyp(t *types.Type) {
	if t.Sym() != nil {
		if t.Sym().Pkg == types.BuiltinPkg || t.Sym().Pkg == types.Pkgs.Unsafe {
...
func (w *exportWriter) funcExt(n *ir.Name) {
	...
	// Inline body.
	if n.Func_.Inl != nil {
		w.uint64(1 + uint64(n.Func_.Inl.Cost))
		if n.Func_.ExportInline() {
			w.p.doInline(n)
		}

		// Endlineno for inlined function.
		w.pos(n.Func_.Endlineno)
	} else {
		w.uint64(0)
	}
}
