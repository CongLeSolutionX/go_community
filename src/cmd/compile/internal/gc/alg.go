...
func genhash(t *types.Type) *obj.LSym {
	...
	case types.TARRAY:
		...
		nx.SetBounded(true)
		na := ir.NewAddrExpr(base.Pos, nx)
		call.PtrList().Append(na)
		call.PtrList().Append(nh)
		n.PtrBody().Append(ir.NewAssignStmt(base.Pos, nh, call))
...
func eqmem(p ir.Node, q ir.Node, field *types.Sym, size int64) ir.Node {
	nx := typecheck(ir.NewAddrExpr(base.Pos, ir.NewSelectorExpr(base.Pos, ir.OXDOT, p, field)), ctxExpr)
	ny := typecheck(ir.NewAddrExpr(base.Pos, ir.NewSelectorExpr(base.Pos, ir.OXDOT, q, field)), ctxExpr)
