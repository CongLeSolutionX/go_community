...
func algtype1(t *types.Type) (AlgKind, *types.Type) {
	...
	base.Fatal("algtype1: unexpected type %v", t)
	return 0, nil
}
...
func hashfor(t *types.Type) ir.INode {
	var sym *types.Sym

	switch a, _ := algtype1(t); a {
	case AMEM:
		base.Fatal("hashfor with AMEM type")
	case AINTER:
...
func geneq(t *types.Type) *obj.LSym {
	...
	case types.TARRAY:
		...
		checkAll := func(unroll int64, last bool, eq func(pi, qi ir.INode) ir.INode) {
		...
		case types.TSTRING:
			...
			checkAll(3, false, func(pi, qi ir.INode) ir.INode {
				// Compare lengths.
	...
	case types.TSTRUCT:
		...
		var flatConds []ir.INode
		for _, c := range conds {
			isCall := func(n ir.INode) bool {
...
func eqinterface(s, t ir.INode) (eqtab, eqdata ir.INode) {
...
func eqmemfunc(size int64, t *types.Type) (fn ir.INode, needsize bool) {
	switch size {
	default:
		fn = syslook("memequal")
		needsize = true
...
func ispaddedfield(t *types.Type, i int) bool {
	if !t.IsStruct() {
		base.Fatal("ispaddedfield called non-struct %v", t)
	}
	end := t.Width
	if i+1 < t.NumFields() {
