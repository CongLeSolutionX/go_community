...
func genhash(t *types.Type) *obj.LSym {
	...
	case AMEM:
		...
		closure := typeLookup(fmt.Sprintf(".hashfunc%d", t.Size())).Linksym()
		...
		ot = duintptr(closure, ot, uint64(t.Size())) // size encoded in closure
...
func geneq(t *types.Type) *obj.LSym {
	...
	case AMEM:
		...
		closure := typeLookup(fmt.Sprintf(".eqfunc%d", t.Size())).Linksym()
		if len(closure.P) != 0 {
			return closure
		}
		...
		ot = duintptr(closure, ot, uint64(t.Size()))
		ggloblsym(closure, int32(ot), obj.DUPOK|obj.RODATA)
		return closure
	case ASPECIAL:
		break
