...
func makeWrapperInterface(wrapperType *types.Type) []*types.Field {
	wrapperField := types.NewField()
	wrapperField.Type = wrapperType
	unwrapMethod := types.NewField()
	unwrapMethod.Sym = lookup("Unwrap")
	unwrapMethod.Type = functypefield(fakeRecvField(), nil, []*types.Field{wrapperField})
	return []*types.Field{unwrapMethod}
}

func makeErrorInterface(wrapperType *types.Type) *types.Type {
...
func lexinit1() {
	// wrapper type
	wrapperSym := builtinpkg.Lookup("wrapper")

	types.Wrappertype = types.New(TINTER)
	types.Wrappertype.Sym = wrapperSym
	types.Wrappertype.SetInterface(makeWrapperInterface(types.Wrappertype))

	wrapperUnderlyingType := types.New(TINTER)
	wrapperUnderlyingType.SetInterface(makeWrapperInterface(types.Wrappertype))
	types.Wrappertype.Orig = wrapperUnderlyingType

	wrapperSym.Def = asTypesNode(typenod(types.Wrappertype))
	dowidth(types.Wrappertype)

	// error type
	s := builtinpkg.Lookup("error")
	types.Errortype = makeErrorInterface(types.Wrappertype)
	types.Errortype.Sym = s
	types.Errortype.Orig = makeErrorInterface(types.Wrappertype)
