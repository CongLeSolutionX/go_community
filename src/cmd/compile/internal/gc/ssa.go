...
func (s *state) expr(n *Node) *ssa.Value {
	...
	case ONAME:
		if n.Class == PFUNC {
			...
			return s.f.Entry.NewValue(ssa.OpGlobal, Ptrto(n.Type), funcsym(n.Sym))
		}
	...
	case OCALLFUNC:
		...
		if static {
			...
			entry := s.curBlock.NewValue2(ssa.OpLoad, s.config.Uintptr, nil, closure, s.mem())
			call = s.curBlock.NewValue3(ssa.OpClosureCall, ssa.TypeMem, nil, entry, closure, s.mem())
		}
		b := s.endBlock()
...
// canSSA returns true if n is an ONAME which is SSA-able
func canSSA(n *Node) bool {
	if n.Op != ONAME {
		log.Fatalf("canSSA passed a non-ONAME %v", n)
	}
	if n.Addrtaken {
		return false
	}
