...
func buildssa(fn *Node, worker int) *ssa.Func {
	...
	if printssa {
		ssaDF := ssaDumpFile
		if ssaDir != "" {
			ssaDF = filepath.Join(ssaDir, s.f.Name+".html")
		}
...
func (s *state) openDeferExit() {
	...
	for i := len(s.openDefers) - 1; i >= 0; i-- {
		...
		for j, argAddrVal := range r.argVals {
			...
			ACArgs = append(ACArgs, ssa.ArgOrResult{T: types.Types[TUINTPTR], Offset: int32(argStart + f.Offset)})
...
func (s *state) call(n *Node, k callKind, returnResultAddr bool) *ssa.Value {
	...
	if k == callNormal {
		...
		for i := 0; i < nf; i++ {
			fp := res.Field(i)
			ACResults = append(ACResults, ssa.ArgOrResult{T: fp.Type, Offset: int32(fp.Offset + Ctxt.FixedFrameSize())})
		}
	}
	...
	if k == callDeferStack {
		...
		ACArgs = append(ACArgs, ssa.ArgOrResult{T: types.Types[TUINTPTR], Offset: int32(Ctxt.FixedFrameSize())})
...
func (s *state) rtcall(fn *obj.LSym, returns bool, results []*types.Type, args ...*ssa.Value) []*ssa.Value {
	...
	for _, t := range results {
		...
		ACResults = append(ACResults, ssa.ArgOrResult{T: t, Offset: int32(offR)})
		offR += t.Size()
	}
