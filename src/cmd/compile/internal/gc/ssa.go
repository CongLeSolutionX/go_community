...
var ssaDump string // dump SSA phases for this function to ssaDumpFile
...
func dumpSourcesColumn(writer *ssa.HTMLWriter, fn *Node) {
	...
	for _, fi := range fn.Func.DebugInlined {
		var elno src.XPos
		if fi.Name.Defn == nil {
	...
	// Sort everything topologically: target function being on top,
	// followed by functions from the same file and then the rest.
	sort.Slice(inlFns, func(i, j int) bool {
		a := inlFns[i]
		b := inlFns[j]
		...
		if targetFn != nil {
			if a.Filename == targetFn.Filename {
				return true
			}
			if b.Filename == targetFn.Filename {
				return false
			}
		}
