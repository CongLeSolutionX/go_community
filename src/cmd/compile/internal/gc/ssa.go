...
func init() {
	...
	makeAtomicGuardedIntrinsicARM64 := func(op0 ssa.Op, op1 ssa.Op, typ types.EType, rtyp types.EType,
		opEmitter func(s *state, n *Node, args []*ssa.Value, op ssa.Op, typ types.EType)) func(s *state, n *Node, args []*ssa.Value) *ssa.Value {
		...
		return func(s *state, n *Node, args []*ssa.Value) *ssa.Value {
			...
			s.startBlock(bTrue)
			opEmitter(s, n, args, op1, typ)
			s.endBlock().AddEdgeTo(bEnd)

			// Use original instruction sequence.
			s.startBlock(bFalse)
			opEmitter(s, n, args, op0, typ)
			s.endBlock().AddEdgeTo(bEnd)
