...
type SSAGenState struct {
	...
	// within a block, the first instruction at a given line number.
	// Used to move statement marks to the beginning ofg such runs.
...
func (s *SSAGenState) Prog(as obj.As) *obj.Prog {
	p := s.pp.Prog(as)
	if as == obj.APCDATA { // these do not generate executable code.
		return p
	}
...
func genssa(f *ssa.Func, pp *Progs) {
	...
	// relax DefaultStmt into NotStmt
