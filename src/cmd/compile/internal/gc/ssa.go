...
func genValue(v *ssa.Value) {
	...
	case ssa.OpAMD64CMPQ, ssa.OpAMD64CMPL, ssa.OpAMD64CMPW, ssa.OpAMD64CMPB,
		...
		ssa.OpAMD64UCOMISS, ssa.OpAMD64UCOMISD:
...
func oneFPJump(b *ssa.Block, jumps *[2]floatingEQNEJump, i int, likely ssa.BranchPrediction, branches []branch) []branch {
	p := Prog(jumps[i].jump)
	p.To.Type = obj.TYPE_BRANCH
	branches = append(branches, branch{p, b.Succs[jumps[i].index]})
	if i == 1 {
		likely *= -1
	}
...
func genBlock(b, next *ssa.Block, branches []branch) []branch {
	...
	case ssa.BlockAMD64EQ, ssa.BlockAMD64NE,
		...
		// TODO need to get this right for floating point EQ/NE,
		// which requires second conditional for unordered case.
