...
// Call returns a new CALL instruction for the SSA value v. It uses
// PrepareCall to prepare the call.
...
// PrepareCall does preparations before a CALL can be emitted, for example
// it emits a PCDATA instruction and it updates s.maxarg to ensure that the
// call arguments can fit into the stack frame of the current function.
