...
func (s *state) emitOpenDeferInfo() {
	...
	off = dvarint(x, off, -s.deferBitsTemp.Offset_)
	...
	for i := len(s.openDefers) - 1; i >= 0; i-- {
		...
		off = dvarint(x, off, -r.closureNode.Offset_)
		...
		if r.rcvrNode != nil {
			off = dvarint(x, off, -r.rcvrNode.Offset_)
			off = dvarint(x, off, s.config.PtrSize)
			off = dvarint(x, off, 0)
		}
		for j, arg := range r.argNodes {
			f := getParam(r.n, j)
			off = dvarint(x, off, -arg.Offset_)
...
func buildssa(fn *ir.Func, worker int) *ssa.Func {
	...
	for _, n := range fn.Dcl {
		...
		case ir.PPARAM:
			...
			args = append(args, ssa.Param{Type: n.Type(), Offset: int32(n.Offset_)})
		case ir.PPARAMOUT:
			s.decladdrs[n] = s.entryNewValue2A(ssa.OpLocalAddr, types.NewPtr(n.Type()), n, s.sp, s.startmem)
			results = append(results, ssa.Param{Type: n.Type(), Offset: int32(n.Offset_)})
			if s.canSSA(n) {
...
func (s *state) stmt(n ir.Node) {
	...
	case ir.OSWITCH, ir.OSELECT:
		...
		// generate body code
		if n.Op() == ir.OSWITCH {
			s.stmtList(n.(*ir.SwitchStmt).Compiled)
		} else {
			s.stmtList(n.(*ir.SelectStmt).Compiled)
		}
...
func (s *state) addr(n ir.Node) *ssa.Value {
	...
	case ir.ONAME:
		...
		case ir.PEXTERN:
			...
			if n.Offset_ != 0 {
				v = s.entryNewValue1I(ssa.OpOffPtr, v.Type, n.Offset_, v)
			}
			return v
		case ir.PPARAM:
			// parameter slot
	...
	case ir.OCLOSUREREAD:
		...
		return s.newValue1I(ssa.OpOffPtr, t, n.Offset_,
...
func (s *state) addNamedValue(n *ir.Name, v *ssa.Value) {
	...
	if n.Class_ == ir.PAUTO && n.Offset_ != 0 {
		s.Fatalf("AUTO var with offset %v %d", n, n.Offset_)
	}
...
func (s byXoffset) Less(i, j int) bool { return s[i].Offset_ < s[j].Offset_ }
...
func emitStackObjects(e *ssafn, pp *Progs) {
	...
	for _, v := range vars {
		...
		off = duintptr(x, off, uint64(v.Offset_))
...
func defframe(s *SSAGenState, e *ssafn) {
	...
	for _, n := range e.curfn.Dcl {
		...
		if n.Type().Size()%int64(Widthptr) != 0 || n.Offset_%int64(Widthptr) != 0 || n.Type().Size() == 0 {
			e.Fatalf(n.Pos(), "var %L has size %d offset %d", n, n.Type().Size(), n.Offset_)
		}

		if lo != hi && n.Offset_+n.Type().Size() >= lo-int64(2*Widthreg) {
