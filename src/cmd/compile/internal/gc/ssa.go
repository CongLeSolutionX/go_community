...
func (s *state) ifaceType(n *Node, v *ssa.Value) *ssa.Value {
	...
	s.vars[&typNod] = tab
	isnonnil := s.newValue2(ssa.OpNeqPtr, Types[TBOOL], tab, s.constIntPtr(byteptr.PtrTo(), 0))
	b := s.endBlock()
	b.Kind = ssa.BlockIf
	b.Control = isnonnil
	b.Likely = ssa.BranchLikely
	...
	s.startBlock(bEnd)
	typ := s.variable(&typNod, byteptr)
	delete(s.vars, &typNod) // avoid map growth
	return typ
}
...
func (s *state) dottype(n *Node, commaok bool) (res, resok *ssa.Value) {
	...
	s.startBlock(bOk)
	s.vars[&idata] = s.newValue1(ssa.OpIData, n.Type, iface)
	if commaok {
	...
	s.startBlock(bFail)
	s.vars[&idata] = s.entryNewValue0(ssa.OpConstNil, byteptr)
	if commaok {
	...
	delete(s.vars, &idata) // avoid map growth
	delete(s.vars, &ok)
	return res, resok
}
