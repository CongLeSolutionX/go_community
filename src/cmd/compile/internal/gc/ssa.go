...
// interface call.
func getParam(n ir.INode, i int) *types.Field {
	t := n.Left().Type()
...
func buildssa(fn ir.INode, worker int) *ssa.Func {
	name := ir.FuncName(fn)
	printssa := false
	...
	s.labeledNodes = map[ir.INode]*ssaLabel{}
	s.fwdVars = map[ir.INode]*ssa.Value{}
	...
	s.decladdrs = map[ir.INode]*ssa.Value{}
	var args []ssa.Param
...
func dumpSourcesColumn(writer *ssa.HTMLWriter, fn ir.INode) {
...
type state struct {
	...
	labeledNodes map[ir.INode]*ssaLabel
	...
	vars map[ir.INode]*ssa.Value

	// fwdVars are variables that are used before they are defined in the current block.
	// This map exists just to coalesce multiple references into a single FwdRef op.
	// *Node is the unique identifier (an ONAME Node) for the variable.
	fwdVars map[ir.INode]*ssa.Value

	// all defined variables at the end of each block. Indexed by block ID.
	defvars []map[ir.INode]*ssa.Value

	// addresses of PPARAM and PPARAMOUT variables.
	decladdrs map[ir.INode]*ssa.Value
...
var (
	...
	ptrVar       = ssaDummy("ptr")
	lenVar       = ssaDummy("len")
	newlenVar    = ssaDummy("newlen")
	capVar       = ssaDummy("cap")
	typVar       = ssaDummy("typ")
	okVar        = ssaDummy("ok")
	deferBitsVar = ssaDummy("deferBits")
)
...
func (s *state) stmt(n ir.INode) {
	...
	case ir.OAS:
		...
		if rhs != nil && (rhs.Op() == ir.OSLICE || rhs.Op() == ir.OSLICE3 || rhs.Op() == ir.OSLICESTR) && samesafeexpr(rhs.Left(), n.Left()) {
			...
			if i != nil && (i.Op() == ir.OLITERAL && i.Val().Ctype() == ir.CTINT && i.Int64Val() == 0) {
				// [0:...] is the same as [:...]
				i = nil
			}
...
func floatForComplex(t *types.Type) *types.Type {
	...
	base.Fatal("unexpected type: %v", t)
	return nil
}
...
func (s *state) expr(n ir.INode) *ssa.Value {
...
func (s *state) append(n ir.INode, inplace bool) *ssa.Value {
...
func (s *state) condBranch(cond ir.INode, yes, no *ssa.Block, likely int8) {
	switch cond.Op() {
	case ir.OANDAND:
...
func (s *state) assign(left ir.INode, right *ssa.Value, deref bool, skip skipMask) {
	if left.Op() == ir.ONAME && left.IsBlank() {
		return
	}
	t := left.Type()
	dowidth(t)
...
type intrinsicBuilder func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value
...
func init() {
	...
	if !instrumenting {
		add("runtime", "slicebytetostringtmp",
			func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value {
	...
	type atomicOpEmitter func(s *state, n ir.INode, args []*ssa.Value, op ssa.Op, typ types.EType)

	makeAtomicGuardedIntrinsicARM64 := func(op0, op1 ssa.Op, typ, rtyp types.EType, emit atomicOpEmitter) intrinsicBuilder {

		return func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value {
	...
	atomicXchgXaddEmitterARM64 := func(s *state, n ir.INode, args []*ssa.Value, op ssa.Op, typ types.EType) {
	...
	atomicCasEmitterARM64 := func(s *state, n ir.INode, args []*ssa.Value, op ssa.Op, typ types.EType) {
	...
	atomicAndOrEmitterARM64 := func(s *state, n ir.INode, args []*ssa.Value, op ssa.Op, typ types.EType) {
	...
	makeRoundAMD64 := func(op ssa.Op) func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value {
		return func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value {
	...
	makeOnesCountAMD64 := func(op64 ssa.Op, op32 ssa.Op) func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value {
		return func(s *state, n ir.INode, args []*ssa.Value) *ssa.Value {
...
func isIntrinsicCall(n ir.INode) bool {
	if n == nil || n.Left() == nil {
		return false
	}
...
func (s *state) intrinsicCall(n ir.INode) *ssa.Value {
...
func (s *state) intrinsicArgs(n ir.INode) []*ssa.Value {
	// Construct map of temps; see comments in s.call about the structure of n.
	temps := map[ir.INode]*ssa.Value{}
...
func (s *state) openDeferRecord(n ir.INode) {
	...
	if n.Op() == ir.OCALLFUNC {
		...
		opendefer.closureNode = closure.Aux.(ir.INode)
		if !(fn.Op() == ir.ONAME && fn.Class() == ir.PFUNC) {
			opendefer.closure = closure
		}
	} else if n.Op() == ir.OCALLMETH {
		if fn.Op() != ir.ODOTMETH {
			base.Fatal("OCALLMETH: n.Left not an ODOTMETH: %v", fn)
		}
		...
		if fn.Op() != ir.ODOTINTER {
			base.Fatal("OCALLINTER: n.Left not an ODOTINTER: %v", fn.Op())
		}
		...
		opendefer.closureNode = opendefer.closure.Aux.(ir.INode)
		opendefer.rcvrNode = opendefer.rcvr.Aux.(ir.INode)
	}
	for _, argn := range n.Rlist().Slice() {
		...
		args = append(args, v)
		argNodes = append(argNodes, v.Aux.(ir.INode))
	}
...
func (s *state) openDeferSave(n ir.INode, t *types.Type, val *ssa.Value) *ssa.Value {
	canSSA := canSSAType(t)
	var pos src.XPos
	if canSSA {
		pos = val.Pos
...
func (s *state) callResult(n ir.INode, k callKind) *ssa.Value {
	return s.call(n, k, false)
}

func (s *state) callAddr(n ir.INode, k callKind) *ssa.Value {
	return s.call(n, k, true)
}

// Calls the function n using the specified call type.
// Returns the address of the return value (or nil if none).
func (s *state) call(n ir.INode, k callKind, returnResultAddr bool) *ssa.Value {
	s.prevCall = nil
...
func (s *state) getMethodClosure(fn ir.INode) *ssa.Value {
...
// interface call
func (s *state) getClosureAndRcvr(fn ir.INode) (*ssa.Value, *ssa.Value) {
	i := s.expr(fn.Left())
...
func (s *state) addr(n ir.INode) *ssa.Value {
	if n.Op() != ir.ONAME {
		s.pushLine(n.Pos())
		defer s.popLine()
	}
...
func (s *state) canSSA(n ir.INode) bool {
	if base.Flag.N != 0 {
		return false
	}
...
func (s *state) exprPtr(n ir.INode, bounded bool, lineno src.XPos) *ssa.Value {
	p := s.expr(n)
	if bounded || n.NonNil() {
...
func (s *state) intDivide(n ir.INode, a, b *ssa.Value) *ssa.Value {
	needcheck := true
	switch b.Op {
...
func (s *state) putArg(n ir.INode, t *types.Type, off int64, forLateExpandedCall bool) (ssa.Param, *ssa.Value) {
	var a *ssa.Value
...
func (s *state) storeArgWithBase(n ir.INode, t *types.Type, base *ssa.Value, off int64) {
	pt := types.NewPtr(t)
	var addr *ssa.Value
	if base == s.sp {
...
func (s *state) uint64Tofloat64(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.uint64Tofloat(&u64_f64, n, x, ft, tt)
}

func (s *state) uint64Tofloat32(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.uint64Tofloat(&u64_f32, n, x, ft, tt)
}

func (s *state) uint64Tofloat(cvttab *u642fcvtTab, n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	// if x >= 0 {
	//    result = (floatY) x
	// } else {
	// 	  y = uintX(x) ; y = x & 1
...
func (s *state) uint32Tofloat64(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.uint32Tofloat(&u32_f64, n, x, ft, tt)
}

func (s *state) uint32Tofloat32(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.uint32Tofloat(&u32_f32, n, x, ft, tt)
}

func (s *state) uint32Tofloat(cvttab *u322fcvtTab, n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	// if x >= 0 {
	// 	result = floatY(x)
	// } else {
...
func (s *state) referenceTypeBuiltin(n ir.INode, x *ssa.Value) *ssa.Value {
...
func (s *state) float32ToUint64(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.floatToUint(&f32_u64, n, x, ft, tt)
}
func (s *state) float64ToUint64(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.floatToUint(&f64_u64, n, x, ft, tt)
}

func (s *state) float32ToUint32(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.floatToUint(&f32_u32, n, x, ft, tt)
}

func (s *state) float64ToUint32(n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
	return s.floatToUint(&f64_u32, n, x, ft, tt)
}

func (s *state) floatToUint(cvttab *f2uCvtTab, n ir.INode, x *ssa.Value, ft, tt *types.Type) *ssa.Value {
...
func (s *state) dottype(n ir.INode, commaok bool) (res, resok *ssa.Value) {
	...
	var tmp ir.INode    // temporary for use with large types
...
func (s *state) variable(name ir.INode, t *types.Type) *ssa.Value {
	v := s.vars[name]
	if v != nil {
		return v
	}
...
func CheckLoweredGetClosurePtr(v *ssa.Value) {
	...
	if entry != v.Block || entry.Values[0] != v {
		base.Fatal("in %s, badly placed LoweredGetClosurePtr: %v %v", v.Block.Func.Name, v.Block, v)
	}
}

// AutoVar returns a *Node and int64 representing the auto variable and offset within it
// where v should be spilled.
func AutoVar(v *ssa.Value) (ir.INode, int64) {
	loc := v.Block.Func.RegAlloc[v.ID].(ssa.LocalSlot)
	if v.Type.Size() > loc.Type.Size() {
		v.Fatalf("spill/restore type %s doesn't fit in slot type %s", v.Type, loc.Type)
	}
	return loc.N.(ir.INode), loc.Off
}
...
func (s *SSAGenState) Call(v *ssa.Value) *obj.Prog {
	...
	if sym, ok := v.Aux.(*ssa.AuxCall); ok && sym.Fn != nil {
		...
		case sys.ARM, sys.ARM64, sys.MIPS, sys.MIPS64:
			p.To.Type = obj.TYPE_MEM
		default:
			base.Fatal("unknown indirect call family")
		}
...
func (s *SSAGenState) PrepareCall(v *ssa.Value) {
	...
	if !idx.StackMapValid() {
		...
		if sym, ok := v.Aux.(*ssa.AuxCall); !ok || !(sym.Fn == typedmemclr || sym.Fn == typedmemmove) {
			base.Fatal("missing stack map index for %v", v.LongString())
		}
	}
...
type ssafn struct {
	...
	scratchFpMem ir.INode             // temp for floating point register / memory moves on some architectures
...
func clobberBase(n ir.INode) ir.INode {
