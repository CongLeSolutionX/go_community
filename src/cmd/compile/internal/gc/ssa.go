	...
	case OAS:
		...
		if rhs != nil {
			...
			case OSTRUCTLIT, OARRAYLIT, OSLICELIT:
				...
				rhs = nil
			case OAPPEND:
				// Check if we're writing the result of an append back to the same slice.
				// Only in that case can we handle it specially to avoid write barriers on
				// the fast (non-growth) path.
