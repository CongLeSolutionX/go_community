...
func (s *state) uint32Tofloat(cvttab *u322fcvtTab, n *Node, x *ssa.Value, ft, tt *Type) *ssa.Value {
	...
	b.AddEdgeTo(bElse)
	s.startBlock(bElse)
	a1 := s.newValue1(cvttab.cvt2F, tt, x)
	oneOnTo32 := cvttab.value(s, tt, float64(1<<32))
	a2 := s.newValue2(cvttab.addf, tt, a1, oneOnTo32)
