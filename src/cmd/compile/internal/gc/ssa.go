...
func (s *state) call(n *Node, k callKind) (inreg bool, result *ssa.Value) {
	...
	case OCALLFUNC:
		if k == callNormal && fn.Op == ONAME && fn.Class == PFUNC {
			sym = fn.Sym
			registerArgs = sym.Def != nil && sym.Def.Name != nil && sym.Def.Name.Defn != nil && sym.Def.Name.Defn.Func.hasRegisterArgs()
			// TODO Why are pragmas landing in two (or more) provably different places?
			if sym.Def != nil && sym.Def.Func.hasRegisterArgs() {
				// Imported pragmas land here, but ONLY imported pragmas land here.
				if chatty {
					s.f.Config.Warnl(lineno, "Second path to registerArgs %s", sym.Name)
				}
				registerArgs = true
			}
			break
	...
	case OCALLMETH:
		...
		if k == callNormal {
			...
			registerArgs = sym.Def != nil && sym.Def.Name != nil && sym.Def.Name.Defn != nil && sym.Def.Name.Defn.Func.hasRegisterArgs()
			// TODO need to check to see if imported methods  go through here.
			if sym.Def != nil && sym.Def.Func.hasRegisterArgs() {
				// Imported pragmas land here, but ONLY imported pragmas land here.
				if chatty {
					s.f.Config.Warnl(lineno, "Second path to registerArgs %s", sym.Name)
...
func (s *state) rtcall(fn *Node, returns bool, results []*Type, args ...*ssa.Value) []*ssa.Value {
	...
	// registerArgs := fn.Name != nil && fn.Name.Defn != nil && fn.Name.Defn.Func.hasRegisterArgs()
	sym := fn.Sym
	registerArgs := sym.Def != nil && sym.Def.Name != nil && sym.Def.Name.Defn != nil && sym.Def.Name.Defn.Func.hasRegisterArgs()
	var ras regArgState
