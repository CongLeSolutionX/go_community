...
func buildssa(fn *Node) *ssa.Func {
	...
	if registerArgs && chatty {
		Warnl(lineno, "Saw register args def for %v", name)
	}
...
func (s *state) newValue3AI(op ssa.Op, t ssa.Type, aux interface{}, auxint int64, arg0, arg1, arg2 *ssa.Value) *ssa.Value {
	return s.curBlock.NewValue3AI(s.peekLine(), op, t, aux, auxint, arg0, arg1, arg2)
}
...
func (s *state) argList(registerArgs bool, l Nodes) {
	...
	for _, oas := range l.Slice() {
		...
		if addr.Op != ssa.OpOffPtr {
			panic("Expected address to be OffPtr")
		}
		s.vars[&memVar] = s.newValue3AI(ArgRegStores[reg], ssa.TypeMem, t, addr.AuxInt, addr.Args[0], r, s.mem())
	}
...
func (s *state) someRegisterArgs(fn *Node, sym *Sym) bool {
	registerArgs1 := fn.Name != nil && fn.Name.Defn != nil && fn.Name.Defn.Func.hasRegisterArgs()
	registerArgs2 := sym.Def != nil && sym.Def.Name != nil && sym.Def.Name.Defn != nil && sym.Def.Name.Defn.Func.hasRegisterArgs()
	registerArgs3 := sym.Def != nil && sym.Def.Func.hasRegisterArgs()
	registerArgs := registerArgs1 || registerArgs2 || registerArgs3
	...
	if registerArgs {
		...
		if chatty {
			s.f.Config.Warnl(lineno, "registerArgs %s, %v %v %v", sym.Name, registerArgs1, registerArgs2, registerArgs3)
		}
	}
	return registerArgs
}
...
func (s *state) call(n *Node, k callKind) (inreg bool, result *ssa.Value) {
	...
	case OCALLFUNC:
		if k == callNormal && fn.Op == ONAME && fn.Class == PFUNC {
			sym = fn.Sym
			registerArgs = s.someRegisterArgs(fn, sym)
			break
		}
		closure = s.expr(fn)
	case OCALLMETH:
...
func (s *state) resolveFwdRef(v *ssa.Value, dm *sparseDefState) {
	...
	if b == s.f.Entry {
		...
		if s.canSSA(name) {
			...
			v.Op = ArgRegArgOps[da.reg] // this picks up register args.
			v.Aux = name
			return
		}
		// Not SSAable. Load it.
		addr := da.addr // Not SSAable, guaranteed to be not in a register.
		if addr == nil {
