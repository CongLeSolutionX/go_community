...
func init() {
	...
	makeAtomicXchgARM64 := func(op0 ssa.Op, op1 ssa.Op, ty types.EType) func(s *state, n *Node, args []*ssa.Value) *ssa.Value {
	...
	addF("runtime/internal/atomic", "Xchg",
		makeAtomicXchgARM64(ssa.OpAtomicExchange32, ssa.OpAtomicExchange32Variant, TUINT32),
		sys.ARM64)
	addF("runtime/internal/atomic", "Xchg64",
		makeAtomicXchgARM64(ssa.OpAtomicExchange64, ssa.OpAtomicExchange64Variant, TUINT64),
		sys.ARM64)
	...
	makeAtomicCasARM64 := func(op0 ssa.Op, op1 ssa.Op, ty types.EType) func(s *state, n *Node, args []*ssa.Value) *ssa.Value {
		return func(s *state, n *Node, args []*ssa.Value) *ssa.Value {
			// Target Atomic feature is identified by dynamic detection
			addr := s.entryNewValue1A(ssa.OpAddr, types.Types[TBOOL].PtrTo(), arm64HasATOMICS, s.sb)
			v := s.load(types.Types[TBOOL], addr)
			...
			// We have atomic instructions - use it directly.
			s.startBlock(bTrue)
			v0 := s.newValue4(op1, types.NewTuple(types.Types[TBOOL], types.TypeMem), args[0], args[1], args[2], s.mem())
			s.vars[&memVar] = s.newValue1(ssa.OpSelect1, types.TypeMem, v0)
			s.vars[n] = s.newValue1(ssa.OpSelect0, types.Types[ty], v0)
			s.endBlock().AddEdgeTo(bEnd)
			...
			// Use original instruction sequence.
			s.startBlock(bFalse)
			v1 := s.newValue4(op0, types.NewTuple(types.Types[TBOOL], types.TypeMem), args[0], args[1], args[2], s.mem())
			s.vars[&memVar] = s.newValue1(ssa.OpSelect1, types.TypeMem, v1)
			s.vars[n] = s.newValue1(ssa.OpSelect0, types.Types[ty], v1)
			s.endBlock().AddEdgeTo(bEnd)
	...
	addF("runtime/internal/atomic", "Cas",
		makeAtomicCasARM64(ssa.OpAtomicCompareAndSwap32, ssa.OpAtomicCompareAndSwap32Variant, TUINT32),
		sys.ARM64)
	addF("runtime/internal/atomic", "Cas64",
		makeAtomicCasARM64(ssa.OpAtomicCompareAndSwap64, ssa.OpAtomicCompareAndSwap64Variant, TUINT64),
		sys.ARM64)
