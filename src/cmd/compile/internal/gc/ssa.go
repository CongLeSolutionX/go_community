	...
	case OCONV:
		...
		if ft.IsInteger() && tt.IsInteger() {
			...
			if tt.Size() == ft.Size() {
				...
				default:
					s.Fatalf("weird integer truncation %T -> %T", ft, tt)
				}
			} else if ft.IsSigned() {
				// sign extension
				switch 10*ft.Size() + tt.Size() {
				...
				case 48:
					op = ssa.OpSext32to64
				default:
					s.Fatalf("bad integer sign extension %T -> %T", ft, tt)
				}
			} else {
				// zero extension
				switch 10*ft.Size() + tt.Size() {
				...
				case 48:
					op = ssa.OpZext32to64
				default:
					s.Fatalf("weird integer sign extension %T -> %T", ft, tt)
				}
			}
			return s.newValue1(op, n.Type, x)
		}
		s.Unimplementedf("unhandled OCONV %T -> %T", n.Left.Type, n.Type)
		return nil
...
func (s *state) extendIndex(v *ssa.Value) *ssa.Value {
	...
	if v.Type.IsSigned() {
		...
		case 48:
			op = ssa.OpSext32to64
		default:
			s.Fatalf("bad signed index extension %T", v.Type)
		}
	} else {
		switch 10*size + s.config.PtrSize {
		case 14:
		...
		case 48:
			op = ssa.OpZext32to64
		default:
			s.Fatalf("bad unsigned index extension %T", v.Type)
		}
	}
