...
//   sink = &Node{x} --> level(x)=-1, x is accessible from sink via one "address of"
//   sink = &Node{&Node{x}} --> level(x)=-2, x is accessible from sink via two "address of"
//
// However, in the next example x's level value and suffixValue differ:
//   sink = &Node{&Node{x.left}} --> level(x).value=-1, level(x).suffixValue=1
...
// the value of x.left was copied into a field of an newly allocated
// Node).
//
