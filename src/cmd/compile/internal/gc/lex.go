...
func pragcgo(text string) {
	...
	case "cgo_export_static", "cgo_export_dynamic":
		...
		case 2:
			...
			pragcgobuf += fmt.Sprintln(verb, local)
		case 3:
			local := plan9quote(f[1])
			remote := plan9quote(f[2])
			pragcgobuf += fmt.Sprintln(verb, local, remote)
		default:
			Yyerror(`usage: //go:%s local [remote]`, verb)
		}
	case "cgo_import_dynamic":
		const usage = `usage: //go:cgo_import_dynamic local [remote ["library"]]`
		switch len(f) {
		case 2:
			local := plan9quote(f[1])
			pragcgobuf += fmt.Sprintln(verb, local)
		...
		case 4:
			...
			pragcgobuf += fmt.Sprintln(verb, local, remote, library)
		default:
			Yyerror(usage)
		}
	...
	case "cgo_dynamic_linker":
		...
		pragcgobuf += fmt.Sprintln(verb, path)
	case "cgo_ldflag":
		if len(f) != 2 || !isQuoted(f[1]) {
		...
		pragcgobuf += fmt.Sprintln(verb, arg)
	}
}

// pragmaFields is similar to strings.FieldsFunc(s, isSpace)
// but does not split when inside double quoted regions.
func pragmaFields(s string) []string {
	...
	if fieldStart >= 0 { // Last field might end at the end of the string.
