...
func typecheck1(n *Node, top int) (res *Node) {
	...
	case OASOP,
		...
		if !okfor[op][TFLOAT64] && (Isconst(l, CTFLT) || Isconst(r, CTFLT)) {
			yyerror("invalid operation: %v (operator %v not defined on untyped float)", n, op)
			n.Type = nil
			return n
		}

		if !okfor[op][TCOMPLEX128] && (Isconst(l, CTCPLX) || Isconst(r, CTCPLX)) {
			yyerror("invalid operation: %v (operator %v not defined on untyped complex)", n, op)
			n.Type = nil
			return n
		}
		n.Type = t
