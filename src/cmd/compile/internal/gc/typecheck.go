...
func typecheck1(n *ir.Node, top int) (res *ir.Node) {
	...
	case ir.OSLICE, ir.OSLICE3:
		...
		if l.Type.IsArray() {
			...
			n.Left = nod(ir.OADDR, n.Left, nil)
...
func typecheckargs(n *ir.Node) {
	...
	for _, f := range t.FieldSlice() {
		t := temp(f.Type)
		as.Ninit.Append(nod(ir.ODCL, t, nil))
		as.List.Append(t)
		n.List.Append(t)
	}
	if static {
	...
	if f1 != nil {
		...
		if t.IsInterface() {
			if n.Left.Type.IsPtr() {
				n.Left = nod(ir.ODEREF, n.Left, nil) // implicitstar
				n.Left.SetImplicit(true)
				n.Left = typecheck(n.Left, ctxExpr)
			}
	...
	if f2 != nil {
		...
		if !types.Identical(rcvr, tt) {
			if rcvr.IsPtr() && types.Identical(rcvr.Elem(), tt) {
				...
				n.Left = nod(ir.ODEREF, n.Left, nil)
				n.Left.SetImplicit(true)
				n.Left = typecheck(n.Left, ctxType|ctxExpr)
...
func deadcodefn(fn *ir.Node) {
	...
	fn.Nbody.Set([]*ir.Node{nod(ir.OEMPTY, nil, nil)})
}
