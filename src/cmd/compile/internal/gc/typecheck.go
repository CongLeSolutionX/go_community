...
func typecheck1(n *Node, top int) (res *Node) {
	...
	case OASOP,
		...
		if iscmp[n.Op] {
			...
			//if (op == OMOD || op == OOR || op == OAND || op == OANDNOT || op == OXOR) && (Isconst(r, CTFLT) || Isconst(l, CTFLT) || Isconst(r, CTCPLX) || Isconst(l, CTCPLX)) {
	...
	case OBITNOT, ONEG, ONOT, OPLUS:
		...
		if n.Op == OBITNOT && (Isconst(l, CTFLT) || Isconst(l, CTCPLX)) {
			yyerror("invalid operation: %v %v", n.Op, t)
			//yyerror("invalid operation: operator %v not defined on untyped float or complex", n.Op)
			n.Type = nil
			return n
		}
		n.Type = t
