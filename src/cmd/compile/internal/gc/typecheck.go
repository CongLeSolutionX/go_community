...
func typecheck1(n ir.Node, top int) (res ir.Node) {
	...
	case ir.OSLICE, ir.OSLICE3:
		...
		if l.Type().IsArray() {
			...
			addr := ir.NewAddrExpr(base.Pos, n.X)
	...
	case ir.OCALL:
		...
		if l.Op() == ir.ONAME && l.(*ir.Name).BuiltinOp != 0 {
			l := l.(*ir.Name)
			if n.DDD && l.BuiltinOp != ir.OAPPEND {
		...
		typecheckaste(ir.OCALL, n.X, n.DDD, t.Params(), n.Args, func() string { return fmt.Sprintf("argument to %v", n.X) })
		if t.NumResults() == 0 {
			return n
		}
		if t.NumResults() == 1 {
...
func typecheckMethodExpr(n *ir.SelectorExpr) (res ir.Node) {
	...
	me := ir.NewMethodExpr(n.Pos(), n.X, m)
...
func typecheckcomplit(n *ir.CompLitExpr) (res ir.Node) {
	...
	case types.TSLICE:
		...
		n.Ntype = ir.Node(nodintconst(length)).(ir.Ntype)
		...
		if len(n.List) != 0 && nokeys(n.List) {
			...
			for i, l := range ls {
				...
				if l.Op() == ir.OKEY {
					...
					sk := ir.NewStructKeyExpr(l.Pos(), nil, kv.Value)
					ls[i] = sk
					l = sk
					...
					if key.Sym() == nil || key.Op() == ir.OXDOT || key.Sym().IsBlank() {
						...
						sk.Value = typecheck(sk.Value, ctxExpr)
						continue
					}
