...
notenough:
	if n == nil || !n.Diag {
		mismatch := callOrOpMismatchMessage(nl, tstruct, isddd)
		if call != nil {
			// call is the expression being called, not the overall call.
			// Method expressions have the form T.M, and the compiler has
			// rewritten those to ONAME nodes but left T in Left.
			if call.Op == ONAME && call.Left != nil && call.Left.Op == OTYPE {
				yyerror("not enough arguments in call to method expression %v%s", call, mismatch)
			} else {
				yyerror("not enough arguments in call to %v%s", call, mismatch)
			}
		} else {
			yyerror("not enough arguments to %v%s", op, mismatch)
		}
		if n != nil {
			n.Diag = true
		}
...
toomany:
	mismatch := callOrOpMismatchMessage(nl, tstruct, isddd)
	if call != nil {
		yyerror("too many arguments in call to %v%s", call, mismatch)
	} else {
		yyerror("too many arguments to %v%s", op, mismatch)
	}
	goto out
}
