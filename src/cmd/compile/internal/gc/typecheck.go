	...
	if n == nil || !n.Diag {
		if call != nil {
			...
			if call.Op == ONAME && call.Left != nil && call.Left.Op == OTYPE {
				yyerror("not enough arguments in call to method expression %v%s", call, callOrOpMismatchMessage(nl, tstruct, isddd))
			} else {
				yyerror("not enough arguments in call to %v%s", call, callOrOpMismatchMessage(nl, tstruct, isddd))
			}
		} else {
			yyerror("not enough arguments to %v%s", op, callOrOpMismatchMessage(nl, tstruct, isddd))
		}
		if n != nil {
			n.Diag = true
		}
...
toomany:
	if call != nil {
		yyerror("too many arguments in call to %v%s", call, callOrOpMismatchMessage(nl, tstruct, isddd))
	} else {
		yyerror("too many arguments to %v%s", op, callOrOpMismatchMessage(nl, tstruct, isddd))
	}
	goto out
}

func callOrOpMismatchMessage(nl Nodes, tstruct *Type, isddd bool) string {
