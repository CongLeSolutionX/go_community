...
func typecheck1(n ir.Node, top int) (res ir.Node) {
	...
	case ir.OTARRAY:
		...
		n.Size = indexlit(typecheck(n.Size, ctxExpr))
		size := n.Size
	...
	case ir.OTFUNC:
		ok |= ctxType
		n := n.(*ir.FuncType)
		n.SetOTYPE(functype(n.Rcvr, n.Args, n.Results))
	...
	case ir.OCALL:
		...
		if l.Op() == ir.ONAME && l.SubOp() != 0 {
			...
			switch l.SubOp() {
			default:
				base.Errorf("unknown builtin %v", l)
				return n
...
func typecheckcomplit(n ir.Node) (res ir.Node) {
	...
	if array, ok := n.Right().(*ir.ArrayType); ok && array.Elem != nil && array.Size == nil {
