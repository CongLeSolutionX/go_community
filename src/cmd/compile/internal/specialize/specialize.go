...
// Package specialize performs call specialization of indirect calls based on
// profile information.
//
// This pass specializes interface calls for the hottest callee. That is, it
...
// SpecializePackage is the main entry point for code specialization using
// profile data.
func SpecializePackage(p *pgo.Profile) {
	if base.Debug.PGOSpecialize == 0 {
		return
	}
...
// shouldSpecialize checks if the target function should be specialized.
//
// Specialization is most valuable when the callee is inlined, so if it won't
// inline we can skip specializing.
func shouldSpecialize(fn *ir.Func) bool {
	var reason string
	if base.Flag.LowerM > 1 || logopt.Enabled() {
		defer func() {
			if reason != "" {
				if base.Flag.LowerM > 1 {
					fmt.Printf("%v: should not specialize %v: %s\n", ir.Line(fn), ir.FuncName(fn), reason)
				}
				if logopt.Enabled() {
					logopt.LogOpt(fn.Pos(), ": should not specialize function", "specialize", ir.FuncName(fn), reason)
				}
			}
		}()
	}
...
// logging. If the call is specialized, the specialization fields should be
// updated.
...
// specializeCalls specializes calls in fn using the profile information by
// walking over the body of fn.
func specializeCalls(fn *ir.Func, p *pgo.Profile) {
	...
	edit = func(n ir.Node) ir.Node {
		...
		if base.Debug.PGODebug >= 2 {
			fmt.Printf("%v: specialize considering call %v\n", ir.Line(call), call)
		}
		...
		if !shouldSpecialize(callee) {
			return n
		}

		if stat != nil {
			stat.Specialized = ir.LinkFuncName(callee)
			stat.SpecializedWeight = weight
		}

		return rewriteCall(n, call, fn, callee, ctyp)
	}

	ir.EditChildren(fn, edit)
}

// rewriteCall specializes the given call using a direct method call to
// concretetyp.
func rewriteCall(n ir.Node, call *ir.CallExpr, curfn, callee *ir.Func, concretetyp *types.Type) ir.Node {
	if base.Flag.LowerM != 0 {
		fmt.Printf("%v: specializing call to %v\n", ir.Line(call), callee)
	}
...
func findHotConcreteCallee(p *pgo.Profile, caller *ir.Func, call ir.Node) (*ir.Func, int64) {
	...
	for _, e := range callerNode.OutEdges {
		...
		if op == ir.OXXX {
			...
			// interface. It would still be OK to specialize a call
			// to this method.
			//
			...
			if base.Debug.PGODebug >= 2 {
				fmt.Printf("%v: edge %s:%d -> %s (weight %d): %v doesn't implement %v\n", ir.Line(call), callerName, callOffset, e.Dst.Name(), e.Weight, ctyp, inter)
			}
			continue
		}
