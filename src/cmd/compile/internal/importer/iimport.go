...
func iImportData(imports map[string]*types2.Package, data []byte, path string) (_ int, pkg *types2.Package, err error) {
	...
	p := iimporter{
		...
		typParamIndex: make(map[ident]types2.Type),
	}
...
type iimporter struct {
	...
	typParamIndex map[ident]types2.Type
...
func (r *importReader) obj(name string) {
	...
	case 'P':
		...
		// bound, save the partial type in typParamIndex before reading the bounds.
		id := ident{r.currPkg.Name(), name}
		r.p.typParamIndex[id] = t
...
func (r *importReader) doType(base *types2.Named) types2.Type {
	...
	case typeParamType:
		...
		t, ok := r.p.typParamIndex[id]
		if ok {
			// We're already in the process of importing this typeparam.
			return t
		}
		// Else import the definition of the typeparam now.
		r.p.doDecl(pkg, name)
		return r.p.typParamIndex[id]
