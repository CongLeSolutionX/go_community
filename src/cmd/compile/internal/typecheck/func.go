...
func tcCall(n *ir.CallExpr, top int) (res ir.Node) {
...
func tcAppend(n *ir.CallExpr) (res ir.Node) {
	typecheckargs(n)
	args := n.Args
	if len(args) == 0 {
...
func tcClose(n *ir.UnaryExpr) (res ir.Node) {
	n.X = Expr(n.X)
	n.X = DefaultLit(n.X, nil)
	l := n.X
	t := l.Type()
...
func tcComplex(n *ir.BinaryExpr) (res ir.Node) {
	l := Expr(n.X)
	r := Expr(n.Y)
...
func tcCopy(n *ir.BinaryExpr) (res ir.Node) {
...
func tcDelete(n *ir.CallExpr) (res ir.Node) {
	typecheckargs(n)
	args := n.Args
	if len(args) == 0 {
...
func tcMake(n *ir.CallExpr) (res ir.Node) {
	args := n.Args
	if len(args) == 0 {
...
func tcMakeSliceCopy(n *ir.MakeExpr) (res ir.Node) {
...
func tcNew(n *ir.UnaryExpr) (res ir.Node) {
	if n.X == nil {
...
func tcPanic(n *ir.UnaryExpr) (res ir.Node) {
	n.X = Expr(n.X)
...
func tcPrint(n *ir.CallExpr) (res ir.Node) {
	typecheckargs(n)
	ls := n.Args
	for i1, n1 := range ls {
...
func tcRealImag(n *ir.UnaryExpr) (res ir.Node) {
	n.X = Expr(n.X)
	l := n.X
	t := l.Type()
	if t == nil {
...
func tcRecover(n *ir.CallExpr) (res ir.Node) {
	if len(n.Args) != 0 {
