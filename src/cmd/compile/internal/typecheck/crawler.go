...
// markGeneric is similar to markType, but marking all methods not just
// exported methods. If a base generic type is written to export file
// (even if not explicitly marked for export), all of its methods need
// to be available for instantiation if needed.
