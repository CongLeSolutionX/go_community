...
func (p *crawler) markEmbed(t *types.Type) {
	...
	if t.Sym() != nil && t.Kind() != types.TINTER {
		for _, m := range t.Methods().Slice() {
			n := m.Nname.(*ir.Name)
			if base.Flag.G > 0 && n.Class == ir.Pxxx {
				// TODO(mdempsky): Broken method left behind by stenciling?
				// Why didn't t.IsInstantiatedGeneric() catch this above?
				// Removing this causes typeparam/mdempsky/15.go to fail with
				// -G=3.
				continue
			}
			p.markInlBody(n)
