...
func DeclVars(vl []ir.Node, t ir.Ntype, el []ir.Node) []ir.Node {
	var init []ir.Node
	doexpr := len(el) > 0
...
func StartFuncBody(fn *ir.Func) {
	...
	TODO_funcStack = append(TODO_funcStack, funcStackEnt{ir.CurFunc, DeclContext})
	ir.CurFunc = fn
...
func FinishFuncBody() {
	...
	types.Popdcl()
	var e funcStackEnt
	TODO_funcStack, e = TODO_funcStack[:len(TODO_funcStack)-1], TODO_funcStack[len(TODO_funcStack)-1]
	ir.CurFunc, DeclContext = e.curfn, e.dclcontext
}

var TODO_funcStack []funcStackEnt // stack of previous values of Curfn/dclcontext
