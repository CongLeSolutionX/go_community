...
// tparamList reads in type params into a field slice.
...
func (r *importReader) doDecl(sym *types.Sym) *ir.Name {
	...
	case 'F':
		...
		if r.p.exportVersion == IexportVersion {
...
// TODO(danscales) - share with noder versions
...
func Instantiate(pos src.XPos, baseType *types.Type, targs []*types.Type) *types.Type {
	...
	// importing it), but t's underlying type will be updated when baseType's
