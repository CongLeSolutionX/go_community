...
func Assignop1(src, dst *types.Type) (ir.Op, string) {
	...
	if dst.IsInterface() && src.Kind() != types.TNIL {
		...
		ok, why := Implements(src, dst, true)
		if ok {
			return ir.OCONVIFACE, ""
		}
		return ir.OXXX, ":\n\t" + why
	...
	if src.IsInterface() && dst.Kind() != types.TBLANK {
		var why string
		if ok, _ := Implements(dst, src, false); ok {
...
// an interface, a type parameter, or a concrete type. If Implements returns false and explain is true, then Implements returns a string explaining why t does not implement iface.
func Implements(t, iface *types.Type, explain bool) (bool, string) {
	...
	if explain {
		if isptrto(t, types.TINTER) {
			why = fmt.Sprintf("%v is pointer to interface, not interface", t)
		} else if have != nil && have.Sym == missing.Sym && have.Nointerface() {
			why = fmt.Sprintf("%v does not implement %v (%v method is marked 'nointerface')", t, iface, missing.Sym)
		} else if have != nil && have.Sym == missing.Sym {
			why = fmt.Sprintf("%v does not implement %v (wrong type for %v method)\n"+
			"\t\thave %v%S\n\t\twant %v%S", t, iface, missing.Sym, have.Sym, have.Type, missing.Sym, missing.Type)
		} else if ptr != 0 {
			why = fmt.Sprintf("%v does not implement %v (%v method has pointer receiver)", t, iface, missing.Sym)
		} else if have != nil {
			why = fmt.Sprintf("%v does not implement %v (missing %v method)\n"+
			"\t\thave %v%S\n\t\twant %v%S", t, iface, missing.Sym, have.Sym, have.Type, missing.Sym, missing.Type)
		} else {
			why = fmt.Sprintf("%v does not implement %v (missing %v method)", t, iface, missing.Sym)
		}
	}
	return false, why
}
