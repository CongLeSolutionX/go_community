...
func (s *Schedule) staticAssignInlinedCall(l *ir.Name, loff int64, call *ir.InlinedCallExpr, typ *types.Type) bool {
	...
	//
	// If the init values are side-effect-free and only appear
	// at most once each in the function body, then we record
	// the mapping from names to values in s.args and call
	// StaticAssign on the RHS of the inlined return argument.
	//
