...
type Param struct {
	Type   *types.Type
	Offset int32 // Offset of Param if not in a register, spill offset if it is in a register, types.BADWIDTH if there is none predefined
	Reg    []abi.RegIndex
...
func (a *AuxCall) Reg(i *regInfo, c *Config) *regInfo {
	...
	if a.pri.InRegistersUsed()+a.pri.OutRegistersUsed() == 0 {
...
func archRegForAbiReg(r abi.RegIndex, c *Config) uint8 {
	var m int8
	if int(r) < len(c.iParamRegs) {
		m = c.iParamRegs[r]
	} else {
		m = c.fParamRegs[int(r)-len(c.iParamRegs)]
	}
	return uint8(m)
}
...
func (a *AuxCall) OffsetOfArg(which int64) int64 {
	return int64(a.pri.InParam(int(which)).Offset())
}
...
func StaticAuxCall(sym *obj.LSym, args []Param, results []Param, paramResultInfo *abi.ABIParamResultInfo) *AuxCall {
	...
	return &AuxCall{Fn: sym, args: args, results: results, pri: paramResultInfo, reg: reg}
}

// InterfaceAuxCall returns an AuxCall for an interface call.
func InterfaceAuxCall(args []Param, results []Param, paramResultInfo *abi.ABIParamResultInfo) *AuxCall {
	return &AuxCall{Fn: nil, args: args, results: results, pri: paramResultInfo}
}
...
func OwnAuxCall(fn *obj.LSym, args []Param, results []Param, paramResultInfo *abi.ABIParamResultInfo) *AuxCall {
	...
	return &AuxCall{Fn: fn, args: args, results: results, pri: paramResultInfo}
}
