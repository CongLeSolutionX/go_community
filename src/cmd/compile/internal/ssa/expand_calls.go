...
func expandCalls(f *Func) {
	...
	// Accumulate lists of things to process, note "wide" selects and mem for each call.
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			case OpStore:
				if a := v.Args[1]; a.Op == OpSelectN && !x.canSSAType(a.Type) {
					if a.Uses > 1 {
	...
	for _, v := range args {
		...
		x.rewriteSelectOrArgNew(f.Entry.Pos, f.Entry, v, v, m0, v.Type, rc)
	}
	...
	for _, v := range selects {
		...
		if store := x.wideSelects[v]; store != nil {
			...
			if len(regs) > 0 {
				...
				mem = x.rewriteSelectOrArgNewDirect(call.Pos, call.Block, v, mem, v.Type, rc)
				store.copyOf(mem)
			} else {
		...
		var rc registerCursor
		rc.init(regs, aux.abiInfo, nil, auxBase, 0)
		x.rewriteSelectOrArgNew(call.Pos, call.Block, v, v, mem, v.Type, rc)
	}
	...
	for _, b := range exitBlocks {
		v := b.Controls[0]
		x.rewriteFuncResultsNew(v, b, f.OwnAux)
		b.SetControl(v)
	}

}

func (x *expandState) rewriteFuncResultsNew(v *Value, b *Block, aux *AuxCall) {
	// This is very similar to rewriteCallArgsNew
	// differences:
	// firstArg + preArgs
	// sp vs auxBase
	...
	for j, a := range v.Args[:len(v.Args)-1] {
		...
		mem = x.decomposeAsNecessary(v.Pos, b, a, mem, aux.TypeOfResult(i), rc)
	}
	v.resetArgs()
...
func (x *expandState) rewriteCallArgsNew(v *Value, firstArg int) {
	if x.debug > 1 {
		x.indent(3)
		defer x.indent(-3)
	...
	for i, a := range v.Args[firstArg : len(v.Args)-1] { // skip leading non-parameter SSA Args and trailing mem SSA Arg.
		...
		mem = x.decomposeAsNecessary(v.Pos, v.Block, a, mem, aType, rc)
	}
...
func (x *expandState) decomposePair(pos src.XPos, b *Block, a, mem *Value, t0, t1 *types.Type, o0, o1 Op, rc *registerCursor) *Value {
	...
	pos = pos.WithNotStmt()
	mem = x.decomposeAsNecessary(pos, b, e, mem, t0, rc.next(t0))
	e = b.NewValue1(pos, o1, t1, a)
	mem = x.decomposeAsNecessary(pos, b, e, mem, t1, rc.next(t1))
	return mem
}
...
// into the appropriate sequence of stores and register assignments to transmit that value in a given ABI.
// 'pos' is the source position all this is tied to
// 'b' is the enclosing block
// 'a' is the value to decompose
// 'm0' is the memory arg used for any stores
// 't' is the type of the destination for 'a' (TODO is this not a.Type?)
// 'rc' is a registerCursor which identifies the register/memory destination for the value
func (x *expandState) decomposeAsNecessary(pos src.XPos, b *Block, a, m0 *Value, t *types.Type, rc registerCursor) *Value {
	if x.debug > 1 {
		x.indent(3)
		defer x.indent(-3)
	}
	...
	case types.TARRAY:
		...
		for i := int64(0); i < at.NumElem(); i++ {
			...
			pos = pos.WithNotStmt()
			mem = x.decomposeAsNecessary(pos, b, e, mem, et, rc.next(et))
		}
		return mem
	...
	case types.TINTER:
		...
		if a.Op == OpIMake {
			...
			return x.decomposeAsNecessary(pos, b, data, mem, data.Type, rc.next(data.Type))
		}
...
func (x *expandState) rewriteSelectOrArgNew(pos src.XPos, b *Block, container, a, m0 *Value, at *types.Type, rc registerCursor) *Value {
	...
	makeOf := func(a *Value, b0 *Block, op Op, args []*Value) *Value {
		if a == nil {
			if b0 == nil {
				a = b.NewValue0(pos, op, at)
			} else {
				a = b0.NewValue0(pos, op, at)
			}
			a.AddArgs(args...)
		} else if b0 != nil && a.Block != b0 {
			a0 := b0.NewValue0(pos, op, at)
			a0.AddArgs(args...)
			a.copyOf(a0) // copyof, or reset?
			a = a0
		} else {
			a.resetArgs()
	...
	if at.Size() == 0 {
		...
		if at.IsArray() {
			return makeOf(a, nil, OpArrayMake0, nil)
		}
		if at.IsStruct() {
			return makeOf(a, nil, OpStructMake0, nil)
		}
		return a
	}
	...
	case types.TARRAY:
		et := at.Elem()
		for i := int64(0); i < at.NumElem(); i++ {
			e := x.rewriteSelectOrArgNew(pos, b, container, nil, m0, et, rc.next(et))
			addArg(e)
		}
		a = makeOf(a, nil, OpArrayMake1, args)
	...
	case types.TSTRUCT:
		...
		a = makeOf(a, nil, StructMakeOp(at.NumFields()), args)
		x.commonSelectors[sk] = a
		return a

	case types.TSLICE:
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.BytePtr, rc.next(x.typs.BytePtr)))
		pos = pos.WithNotStmt()
	...
	case types.TSTRING:
		...
		pos = pos.WithNotStmt()
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.Int, rc.next(x.typs.Int)))
		a = makeOf(a, nil, OpStringMake, args)
		x.commonSelectors[sk] = a
		return a

	case types.TINTER:
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.Uintptr, rc.next(x.typs.Uintptr)))
		pos = pos.WithNotStmt()
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.BytePtr, rc.next(x.typs.BytePtr)))
		a = makeOf(a, nil, OpIMake, args)
		x.commonSelectors[sk] = a
		return a

	case types.TCOMPLEX64:
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.Float32, rc.next(x.typs.Float32)))
		pos = pos.WithNotStmt()
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.Float32, rc.next(x.typs.Float32)))
		a = makeOf(a, nil, OpComplexMake, args)
		x.commonSelectors[sk] = a
		return a

	case types.TCOMPLEX128:
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.Float64, rc.next(x.typs.Float64)))
		pos = pos.WithNotStmt()
		addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.Float64, rc.next(x.typs.Float64)))
		a = makeOf(a, nil, OpComplexMake, args)
		x.commonSelectors[sk] = a
	...
	case types.TINT64:
		if at.Size() > x.regSize {
			addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.firstType, rc.next(x.firstType)))
			pos = pos.WithNotStmt()
			addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.secondType, rc.next(x.secondType)))
			if !x.f.Config.BigEndian {
				// Int64Make args are big, little
				args[0], args[1] = args[1], args[0]
			}
			a = makeOf(a, nil, OpInt64Make, args)
			x.commonSelectors[sk] = a
			return a
		}
	case types.TUINT64:
		if at.Size() > x.regSize {
			addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.UInt32, rc.next(x.typs.UInt32)))
			pos = pos.WithNotStmt()
			addArg(x.rewriteSelectOrArgNew(pos, b, container, nil, m0, x.typs.UInt32, rc.next(x.typs.UInt32)))
			if !x.f.Config.BigEndian {
				// Int64Make args are big, little
...
// rewriteSelectOrArgNewDirect handles the case of a SelectN'd result from a function call that is too large for SSA,
...
func (x *expandState) rewriteSelectOrArgNewDirect(pos src.XPos, b *Block, container, m0 *Value, at *types.Type, rc registerCursor) *Value {
	...
	case types.TARRAY:
		et := at.Elem()
		for i := int64(0); i < at.NumElem(); i++ {
			m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, et, rc.next(et))
		}
		return m0
	...
	case types.TSLICE:
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.BytePtr, rc.next(x.typs.BytePtr))
		pos = pos.WithNotStmt()
	...
	case types.TINTER:
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.Uintptr, rc.next(x.typs.Uintptr))
		pos = pos.WithNotStmt()
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.BytePtr, rc.next(x.typs.BytePtr))
		return m0

	case types.TCOMPLEX64:
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.Float32, rc.next(x.typs.Float32))
		pos = pos.WithNotStmt()
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.Float32, rc.next(x.typs.Float32))
		return m0

	case types.TCOMPLEX128:
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.Float64, rc.next(x.typs.Float64))
		pos = pos.WithNotStmt()
		m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.Float64, rc.next(x.typs.Float64))
		return m0
	...
	case types.TINT64:
		if at.Size() > x.regSize {
			m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.firstType, rc.next(x.firstType))
			pos = pos.WithNotStmt()
			m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.secondType, rc.next(x.secondType))
			return m0
		}
	case types.TUINT64:
		if at.Size() > x.regSize {
			m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.UInt32, rc.next(x.typs.UInt32))
			pos = pos.WithNotStmt()
			m0 = x.rewriteSelectOrArgNewDirect(pos, b, container, m0, x.typs.UInt32, rc.next(x.typs.UInt32))
			return m0
		}
...
func (rc *registerCursor) String() string {
	dest := "<none>"
	if rc.storeDest != nil {
		dest = fmt.Sprintf("%s+%d", rc.storeDest.String(), rc.storeOffset)
	}
	regs := "<none>"
	if rc.regValues != nil {
		regs = ""
...
func (c *registerCursor) next(t *types.Type) registerCursor {
	c.storeOffset = alignTo(c.storeOffset, t.Alignment())
	rc := *c
	c.storeOffset = alignTo(c.storeOffset+t.Size(), t.Alignment())
...
type expandState struct {
	...
	firstOp    Op // for 64-bit integers on 32
	secondOp   Op
