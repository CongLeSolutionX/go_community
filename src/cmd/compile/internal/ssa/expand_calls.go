...
type Abi1RO uint8 // the register offset in abi1.
...
type registerCursor struct {
	storeDest *Value // turns out this can do double duty.
	regsLen   int
	nextSlice Abi1RO
...
// prInfoForArg returns the ABIParamAssignment for v, assumed to be an OpArg.
	...
	case OpSelectN:
		...
		if which == aux.NResults() { // mem is after the results.
			...
			if call.Op == OpStaticLECall {
				...
				if aux.pri == nil {
					panic(badVal("aux.pri nil for call", call))
				}
				if existing := x.memForCall[call.ID]; existing == nil {
					selector.AuxInt = int64(aux.pri.OutRegistersUsed())
			...
			if x.canSSAType(leafType) {
				...
				if call.Op == OpStaticLECall { // TODO this is temporary until all calls are register-able
					...
					if mem := x.memForCall[call.ID]; mem != nil {
						...
						mem = call.Block.NewValue1I(call.Pos.WithNotStmt(), OpSelectN, types.TypeMem, int64(aux.pri.OutRegistersUsed()), call)
...
func (x *expandState) decomposeArgOrLoad(pos src.XPos, b *Block, source, mem *Value, t *types.Type, offset int64, loadRegOffset Abi1RO, storeRc registerCursor,
	decomposeOne func(x *expandState, pos src.XPos, b *Block, source, mem *Value, t1 *types.Type, offArg int64, offStore int64, loadRegOffset Abi1RO, storeRc registerCursor) *Value,
	decomposeTwo func(x *expandState, pos src.XPos, b *Block, source, mem *Value, t1 *types.Type, t2 *types.Type, offArg int64, offStore int64, loadRegOffset Abi1RO, storeRc registerCursor) *Value) *Value {
	u := source.Type
	switch u.Kind() {
	case types.TARRAY:
		elem := u.Elem()
...
// storeArgOrLoad converts stores of SSA-able potentially aggregate arguments (passed to a call) into a series of primitive-typed
...
func (x *expandState) storeArgOrLoad(pos src.XPos, b *Block, source, mem *Value, t *types.Type, offset int64, loadRegOffset Abi1RO, storeRc registerCursor) *Value {
	...
	case OpArrayMake0, OpStructMake0:
		// TODO is this correct for registers?
		return mem
...
func expandCalls(f *Func) {
			...
			for j, a := range v.Args {
				...
				if a.Op == OpDereference {
					...
					if a.Op == OpLoad && a.Args[0].Op == OpLocalAddr {
						addr := a.Args[0] // This is a self-move. // TODO do what here for registers?
	...
	// Step 4: rewrite the calls themselves, correcting the type.  Also convert OpArg into register variants.
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			switch v.Op {
			case OpArg:
...
func (x *expandState) rewriteArgToMemOrRegs(v *Value) *Value {
	...
	case 1:
		...
		v.Aux = &AuxNameInt{v.Aux.(*ir.Name), 0}
		v.AuxInt = i
...
func (x *expandState) newArgToMemOrRegs(a, toReplace *Value, offset int64, regOffset Abi1RO, t *types.Type, pos src.XPos) *Value {
	//if a.AuxInt != 0 {
	//	fmt.Printf("newArgToMemOrRegs, a.AuxInt=%d, offset=%d\n", a.AuxInt, offset)
	//}
	key := selKey{a, offset, t.Width, t}
	w := x.commonArgs[key]
	if w != nil {
		if toReplace != nil {
			toReplace.copyOf(w)
	...
	case 0:
		...
		if pa.Offset() != int32(frameOff) {
			panic(fmt.Errorf("Parameter assignment %d and OpArg.Aux frameOffset %d disagree, op=%s",
				pa.Offset(), frameOff, a.LongString()))
		}

		aux := a.Aux
		auxInt := a.AuxInt + offset
		if toReplace != nil && toReplace.Block == a.Block {
			...
			w := a.Block.NewValue0IA(pos, OpArg, t, auxInt, aux)
			x.commonArgs[key] = w
			if x.debug {
				fmt.Printf("\tnew %s\n", w.LongString())
			}
