...
// A registerCursor tracks which register is used for an Arg or result, or a piece of such.
// Also carries the base register for storing memory results, which otherwise is just clutter.
type registerCursor struct {
	...
	result    *[]*Value // values assigned to registers accumulate here
}
...
const (
	// Register offsets for fields of built-in aggregate types
	RO_complex_imag = 1
	RO_string_len   = 1
	RO_slice_len    = 1
	RO_slice_cap    = 2
