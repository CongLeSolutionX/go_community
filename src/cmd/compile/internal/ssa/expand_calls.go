...
func expandCalls(f *Func) {
	...
	// end in OpSelectN, it does nothing (this can happen depending on compiler phase ordsering).
	...
	rewriteSelect = func(leaf *Value, selector *Value, offset int64) []LocalSlot {
		...
		case OpInt64Hi:
			...
			locs = splitSlots(ls, ".ptr", hiOffset, leafType)

		case OpInt64Lo:
			w := selector.Args[0]
			ls := rewriteSelect(leaf, w, offset+lowOffset)
			locs = splitSlots(ls, ".ptr", lowOffset, leafType)
		...
		case OpComplexImag:
			...
			locs = splitSlots(ls, ".image", leafType.Width, leafType)
	...
	splitStore = func(dest, source, mem, v *Value, t *types.Type, offset int64, firstStorePos src.XPos) *Value {
		...
		case types.TSTRUCT:
			if t.NumFields() == 1 && t.Field(0).Type.Width == t.Width && t.Width == regSize {
				...
				// Other rewriting phases create minor glitches when transform IData, for instance the
				// interface-typed Arg "x" of ToFloat
				//   v6 (858) = Arg <Value> {x} (x[Value], x[Value])
				// is rewritten by decomposeArgs into
				//   v141 (858) = Arg <uintptr> {x}
				//   v139 (858) = Arg <*uint8> {x} [8]
				// because of a type case clause on line 862
				//  	case intVal:
				//		   return itof(x)
