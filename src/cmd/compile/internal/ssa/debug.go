...
func (f *FuncDebug) BlockString(b int) string {
	var vars []string

	for slot, list := range f.Blocks[b].Variables {
...
type VarLoc struct {
	...
	// Indicates whether this location lasts past the block's end. Without
	// this, there would be no way to distinguish between a range that ended
	// on the last Value of a block and one that didn't end at all.
	// In particular, if this is true, the variable is still live over the
...
func (v *VarLoc) String() string {
	...
	return fmt.Sprintf("%v-%v%s@%s", pos(v.Start, v.StartProg, v.StartPC), pos(v.End, v.EndProg, v.EndPC), surv, strings.Join(storage, ","))
}
