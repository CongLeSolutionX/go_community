...
// LocalSlot's in a type insensive way (e.g. taking into account the
// base name, offset, and width of the slot, but not ignoring the slot
...
func (sk *slotKey) String() string {
	var bb bytes.Buffer
	fmt.Fprintf(&bb, "name=%v offset=%d width=%d", sk.name, sk.offset, sk.width)
	if sk.splitOf != noSlot {
		fmt.Fprintf(&bb, " splitof:%d splitoffset:%d",
			sk.splitOf, sk.splitOffset)
	}
	return bb.String()
}

func (sc *slotCanonicalizer) String() string {
	if sc == nil {
		return "<nil *slotCanonicalizer>"
	}
	var bb bytes.Buffer
	for i := 0; i < len(sc.slkeys); i++ {
		fmt.Fprintf(&bb, "%d: %v\n", i, &sc.slkeys[i])
	}
	return bb.String()
}

// lookup looks up a LocalSlot in the slot canonicalizer "sc" to
// see if there is an entry for it. If "insert" is set, then new
// entries for the slot will be created if not already present.
// Return value is a pair <keyidx, bool> where the boolean indicates
// whether the LocalSlot was found, and keyidx is the entry for the
// slot if found or inserted.
func (sc *slotCanonicalizer) lookup(ls LocalSlot, insert bool) (SlKeyIdx, bool) {
	...
	if ls.SplitOf != nil {
		if !insert {
			...
			if !sfound {
				panic("misuse of slotCanonicalizer.find: no entry for splitOf")
			}
		} else {
...
//     v10 = ArgIntReg <int> {unused+0} [0] : DI
//
// This function examines the live OpArg{Int,Float}Reg values
// and sythesizes new (dead) values for the non-live params
// or the non-live pieces of partially live params.
//
func PopulateABIInRegArgOps(f *Func) {
	...
	// OpArg{Int,Float}Reg ops. Record the slots they use
	// to a map.
	...
	// Now make a pass through the ABI in-params.
