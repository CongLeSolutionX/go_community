...
func BuildFuncDebug(ctxt *obj.Link, f *Func, loggingLevel int, stackOffset func(LocalSlot) int32, rval *FuncDebug) {
	...
	blockLocs, hasABackEdge := state.liveness()
	state.buildLocationLists(blockLocs, hasABackEdge)
...
func (state *debugState) liveness() ([]*BlockDebug, bool) {
	...
	for i := len(po) - 1; i >= 0; i-- {
		...
		startState, hasBE, blockChanged := state.mergePredecessors(b, blockLocs, nil, false, false)
		locs.lastCheckedTime = counterTime
		counterTime++
		hasABackEdge = hasABackEdge || hasBE
	...
	return blockLocs, hasABackEdge
}

// mergePredecessors takes the end state of each of b's predecessors and
// intersects them to form the starting state for b. It puts that state in
// blockLocs, and fills state.currentState with it.  It returns the start
// state and the presence/absence of backedges to this block.
//
...
// predecessor.
//
// predecessorsUnchanged indicates that none of the predecessors have changed,
// and that complicated merges can be skipped; startState is already correct.
// A particular case of this is a function with no loops; a single pass computes
// everything.
//
...
// approximated by taking the smallest starting state of all the predecessors
...
func (state *debugState) mergePredecessors(b *Block, blockLocs []*BlockDebug, previousBlock *Block, predecessorsUnchanged, forLocationLists bool) (abt.T, bool, bool) {
	// Filter out back branches.
	var predsBuf [10]*Block
	var backedgeTarget bool
	...
	for _, pred := range b.Preds {
		if bl := blockLocs[pred.b.ID]; bl != nil && bl.everProcessed {
			...
			backedgeTarget = bl.poNum <= locs.poNum
		} else {
			backedgeTarget = true
		}
	}
	...
	if len(preds) == 0 {
		...
		return abt.T{}, false, blockChanged
	}
	...
	if len(preds) == 1 {
		if previousBlock != nil && preds[0].ID != previousBlock.ID {
			// Mark everything in previous block as changed because it is not a predecessor.
			// TODO implement abt.MapDifference, markChanged everything in the difference set.
		...
		return p0, backedgeTarget, l0.lastChangedTime > locs.lastCheckedTime
	}
	...
	if predecessorsUnchanged && locs.everProcessed {
		// we're done!
		if previousBlockIsNotPredecessor {
			// Mark everything in previous block as changed because it is not a predecessor.
			markChangedVars(blockLocs[previousBlock.ID].endState, locs.startState)
		}
		state.currentState.reset(locs.startState)
		locs.everProcessed = true
		return locs.startState, backedgeTarget, blockChanged
	}
	...
	if updating {
		...
		// Check for an early out; this should always hit if there are no cycles.
		if len(preds) == 0 {
			state.currentState.reset(locs.startState)
			if state.loggingLevel > 2 {
			...
			locs.everProcessed = true
			return locs.startState, backedgeTarget, false
		}
	}
	...
	for it := newState.Iterator(); !it.IsEmpty(); {
		...
		// Delete if necessary, but not otherwise.
		if x.absent() {
			if !x0.absent() {
				blockChanged = true
				newState.Delete(k)
	...
	return newState, backedgeTarget, blockChanged
}
...
func canMerge(pending, new VarLoc) bool {
	...
	if pending.onStack() {
		if pending.StackOffset != new.StackOffset {
			return false
		}
	}
...
func (state *debugState) buildLocationLists(blockLocs []*BlockDebug, hasABackEdge bool) {
	// Run through the function in program text order, building up location
	// lists as we go. The heavy lifting has mostly already been done.

	var prevBlock *Block
	for _, b := range state.f.Blocks {
		state.mergePredecessors(b, blockLocs, prevBlock, !hasABackEdge, true)
