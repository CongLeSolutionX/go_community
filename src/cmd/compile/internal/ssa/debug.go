...
func (l piecesList) String() string {
	...
	for i, p := range l {
		...
		if p.op != 0 {
			fmt.Fprintf(&sb, "(op:0x%x ", p.op)
			if p.operandType == signedOperand {
				fmt.Fprintf(&sb, "off:%d ", p.operand)
			} else if p.operandType == unsignedOperand {
				fmt.Fprintf(&sb, "off:%d ", uint64(p.operand))
			}
			fmt.Fprintf(&sb, "size:%d)", p.size)
		} else {
			fmt.Fprintf(&sb, "(padding:%d)", p.size)
		}
	}
	return sb.String()
}
...
// grow extends l's length by one. If there is extra capacity, it is used. It
// returns a pointer to the last element (after growing), which has not been
// zero-ed out; it is the caller's responsibility to overwrite it. Before
// overwriting, the caller can take advantage of the existing capacity in the
// pieces slice of the last element.
func (l *locList) grow() *locInfo {
	if cap(*l) >= len(*l)+1 {
		*l = (*l)[:len(*l)+1]
	} else {
		*l = append(*l, locInfo{})
...
func (debugInfo *FuncDebug) PutLocationList(list locList, ctxt *obj.Link, listSym, startPC *obj.LSym, stackOffset int32) {
	...
	for _, loc := range list {
		...
		var buf [10]byte // scratch space sufficient for a SLEB/ULEB128.
...
// the stack, or where it will be spilled in the case of in-reg argument.
//
// The stack growth trailer looks like this:
//
// ret
...
func (debugInfo *FuncDebug) fixupListForStackGrowthTrailer(list locList, stackOffset int32) locList {
	...
	// Assume that the variable was spilled into the location indicated
...
func BuildFuncDebugNoOptimized(ctxt *obj.Link, f *Func, loggingEnabled bool, stackOffset func(LocalSlot) int32, rval *FuncDebug) {
	...
	for _, inp := range pri.InParams() {
		...
		regPieces := make([]pieceInfo, 0, 2*len(inp.Registers))
		for k, r := range inp.Registers {
			...
			regPieces = append(regPieces, pInfo)
			if len(inp.Registers) > 1 {
				...
				if padding[k] > 0 {
					...
					regPieces = append(regPieces, pieceInfo{size: padding[k]})
				}
			}
			if loggingEnabled {
				state.logf("\n")
		...
		stackLoc := locInfo{
			...
			pieces:     make([]pieceInfo, 1),
		}
		pInfo := &stackLoc.pieces[0]
		soff := stackOffset(sl)
		if soff == 0 {
