	...
	for _, b := range state.f.Blocks {
		...
		for _, v := range b.Values {
			...
			for i, varID := range state.changedVars.contents() {
				state.updateVar(VarID(varID), b, v, i < len(isChangedArgOrPhi) && isChangedArgOrPhi[i])
			}
...
func (state *debugState) updateVar(varID VarID, b *Block, v *Value, beginAtStart bool) {
