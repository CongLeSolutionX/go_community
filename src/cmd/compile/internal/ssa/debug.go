...
func (loc liveSlot) absent() bool {
	return loc.Registers == 0 && loc.StackOffset == 0
}

func (loc liveSlot) onStack() bool {
	return loc.StackOffset != 0
}

func (loc liveSlot) StackOffsetValue() int32 {
	return loc.StackOffset >> 1
}
...
type VarLoc struct {
	...
	Registers RegisterSet
	// The stack offset, stored as a 31-bit integer and a presence flag at
	// the low-order bit.
	StackOffset int32
}
...
func (loc VarLoc) onStack() bool {
	return loc.StackOffset != 0
}

func (loc VarLoc) StackOffsetValue() int32 {
	return loc.StackOffset >> 1
}
...
func (state *debugState) processValue(v *Value, vSlots []SlotID, vReg *Register) bool {
	...
	case v.Op == OpArg:
		...
		for _, slot := range vSlots {
			...
			setSlot(slot, VarLoc{0, stackOffset})
		}
	...
	case v.Op == OpStoreReg:
		...
		for _, slot := range vSlots {
			...
			setSlot(slot, VarLoc{last.Registers, stackOffset})
			if state.loggingEnabled {
