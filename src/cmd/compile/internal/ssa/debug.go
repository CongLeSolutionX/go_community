...
type BlockDebug struct {
	...
	// This affects how the merge is done; the goal is to maximize sharing and avoid allocation.
	everProcessed bool
}
...
type debugState struct {
	...
	f             *Func
	loggingLevel  int
	convergeCount int // testing; iteration over block debug state out to converge
	registers     []Register
...
func BuildFuncDebug(ctxt *obj.Link, f *Func, loggingLevel int, stackOffset func(LocalSlot) int32, rval *FuncDebug) {
	...
	// A specific number demands exactly that many iterations.
	// Under particular circumstances it make require more than the total of
	// 2 passes implied by a single run through liveness and a single run through
...
func (state *debugState) liveness() []*BlockDebug {
	...
	converged := false
	// The iteration rule is that by default, run until converged,
	// but if a particular iteration count is specified, run that many
	// iterations, no more, no less.  A count is specified as the
	// thousands digit of the location lists debug flag, e.g.
	// -d=locationlists=4000
...
// blockLocs[b.ID].startState, and fills state.currentState with it.
// It returns the start state and whether this is changed from the previously
// approximated value of startState for this block.  After the first call,
// subsequent calls can only shrink startState.
//
// Passing forLocationLists=true enables additional side-effects that are
// necessary for building location lists but superflous while still iterating
// to an answer.
//
...
// predecessor.
//
// Note that mergePredecessors behaves slightly differently between first and
// subsequent calls for a block.  For the first call, the starting state is
// approximated by taking the state from the predecessor whose state is smallest,
// and removing any elements not in all the other predecessors; this makes the
...
//
// To improve performance, each block's state information is marked with lastChanged
// and lastChecked "times" so unchanged predecessors can be skipped on after-the-first
// iterations.  Doing this allows extra iterations by the caller to be almost free.
//
// It is important to know that the set representation used for startState,
// endState, and merges can share data for two sets where one is a small delta
// from the other.  Doing this does require a little care in how sets are updated,
// both in mergePredecessors, and using its result.
func (state *debugState) mergePredecessors(b *Block, blockLocs []*BlockDebug, previousBlock *Block, forLocationLists bool) (abt.T, bool) {
	...
	// If previous block is not a predecessor, its location information changes at the boundary with this block.
	...
	if updating {
		// After the first approximation, i.e., when updating, results can only
		// get smaller, because initially backedge predecessors do not participate
		// in the intersection.  This means that for the update, given the prior
		// approximation of startState, there is no need to re-intersect with
		// unchanged blocks.  Therefore remove unchanged blocks from the predecessor
		// list.
		...
		// Check for an early out; this should always hit for the update if there are no cycles.
		if len(preds) == 0 {
			blockChanged = false
	...
	// if this is a subsequent call, tweak the startState instead.
	// Note that these "set" values are values; there are no side
	// effects to other values as these are modified.
	newState := baseState
	if updating {
...
func canMerge(pending, new VarLoc) bool {
	...
	if pending.onStack() && pending.StackOffset != new.StackOffset {
		// if there is a stack mapping, it must remain equal (StackOffset also encodes presence)
		return false
	}
