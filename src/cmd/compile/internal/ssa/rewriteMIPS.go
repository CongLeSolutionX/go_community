...
func rewriteValueMIPS(v *Value, config *Config) bool {
	...
	case OpAdd32carry:
		return rewriteValueMIPS_OpAdd32carry(v, config)
	...
	case OpEq64:
		return rewriteValueMIPS_OpEq64(v, config)
	case OpEq64F:
	...
	case OpSub32carry:
		return rewriteValueMIPS_OpSub32carry(v, config)
...
func rewriteValueMIPS_OpAdd32carry(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Add32carry <t> x y)
	// cond:
	// result: (MakeResultCarryTuple (ADD <t.FieldType(0)> x y) (SGTU x (ADD <t.FieldType(0)> x y)))
	for {
		t := v.Type
		x := v.Args[0]
		y := v.Args[1]
		v.reset(OpMIPSMakeResultCarryTuple)
		v0 := b.NewValue0(v.Line, OpMIPSADD, t.FieldType(0))
		v0.AddArg(x)
		v0.AddArg(y)
		v.AddArg(v0)
		v1 := b.NewValue0(v.Line, OpMIPSSGTU, config.fe.TypeBool())
		v1.AddArg(x)
		v2 := b.NewValue0(v.Line, OpMIPSADD, t.FieldType(0))
		v2.AddArg(x)
		v2.AddArg(y)
		v1.AddArg(v2)
		v.AddArg(v1)
...
func rewriteValueMIPS_OpAtomicOr8(v *Value, config *Config) bool {
	...
	// cond: !config.BigEndian
	// result: (LoweredAtomicOr (AND <config.fe.TypeUInt32().PtrTo()> (MOVWconst [-4]) ptr) 		(SLL <config.fe.TypeUInt32()> (ZeroExt8to32 val) 			(SLLconst <config.fe.TypeUInt32()> [3] 				(ANDconst <config.fe.TypeUInt32()> [3] ptr))) mem)
	for {
		ptr := v.Args[0]
		val := v.Args[1]
		mem := v.Args[2]
	...
	// match: (AtomicOr8 ptr val mem)
	// cond: config.BigEndian
	// result: (LoweredAtomicOr (AND <config.fe.TypeUInt32().PtrTo()> (MOVWconst [-4]) ptr) 		(SLL <config.fe.TypeUInt32()> (ZeroExt8to32 val) 			(SLLconst <config.fe.TypeUInt32()> [3] 				(ANDconst <config.fe.TypeUInt32()> [3] 					(XORconst <config.fe.TypeUInt32()> [3] ptr)))) mem)
	for {
		ptr := v.Args[0]
		val := v.Args[1]
		mem := v.Args[2]
...
func rewriteValueMIPS_OpEq64(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Eq64 x y)
	// cond:
...
func rewriteValueMIPS_OpMIPSAND(v *Value, config *Config) bool {
	...
	// match: (AND (SGTUconst [1] x:(XOR _ _)) (SGTUconst [1] y:(XOR _ _)))
	// cond:
...
func rewriteValueMIPS_OpMIPSMOVBUreg(v *Value, config *Config) bool {
	...
	// match: (MOVBUreg x:(MOVBload [off] {sym} ptr mem))
	// cond: x.Uses == 1 && clobber(x)
	// result: @x.Block (MOVBUload <v.Type> [off] {sym} ptr mem)
	for {
		x := v.Args[0]
		...
		b = x.Block
		v0 := b.NewValue0(v.Line, OpMIPSMOVBUload, v.Type)
		v.reset(OpCopy)
		v.AddArg(v0)
		v0.AuxInt = off
		v0.Aux = sym
...
func rewriteValueMIPS_OpMIPSMOVBreg(v *Value, config *Config) bool {
	...
	// match: (MOVBreg x:(MOVBUload [off] {sym} ptr mem))
	// cond: x.Uses == 1 && clobber(x)
	...
	for {
		...
		b = x.Block
		v0 := b.NewValue0(v.Line, OpMIPSMOVBload, v.Type)
		v.reset(OpCopy)
		v.AddArg(v0)
		v0.AuxInt = off
		v0.Aux = sym
...
func rewriteValueMIPS_OpMIPSMOVHUreg(v *Value, config *Config) bool {
	...
	// match: (MOVHUreg x:(MOVHload [off] {sym} ptr mem))
	// cond: x.Uses == 1 && clobber(x)
	// result: @x.Block (MOVHUload <v.Type> [off] {sym} ptr mem)
	for {
		x := v.Args[0]
		...
		b = x.Block
		v0 := b.NewValue0(v.Line, OpMIPSMOVHUload, v.Type)
		v.reset(OpCopy)
		v.AddArg(v0)
		v0.AuxInt = off
		v0.Aux = sym
...
func rewriteValueMIPS_OpMIPSMOVHreg(v *Value, config *Config) bool {
	...
	// match: (MOVHreg x:(MOVHUload [off] {sym} ptr mem))
	// cond: x.Uses == 1 && clobber(x)
	...
	for {
		...
		b = x.Block
		v0 := b.NewValue0(v.Line, OpMIPSMOVHload, v.Type)
		v.reset(OpCopy)
		v.AddArg(v0)
		v0.AuxInt = off
		v0.Aux = sym
...
func rewriteValueMIPS_OpMIPSOR(v *Value, config *Config) bool {
	...
	// match: (OR (SGTUzero x:(XOR _ _)) (SGTUzero y:(XOR _ _)))
	// cond:
...
func rewriteValueMIPS_OpMIPSSGTUconst(v *Value, config *Config) bool {
	...
	// cond: 0 < d && d <= 31 && 1<<uint32(32-d) <= uint32(c)
	// result: (MOVWconst [1])
	for {
		c := v.AuxInt
		v_0 := v.Args[0]
		...
		d := v_0.AuxInt
		if !(0 < d && d <= 31 && 1<<uint32(32-d) <= uint32(c)) {
			break
		}
		v.reset(OpMIPSMOVWconst)
		v.AuxInt = 1
...
func rewriteValueMIPS_OpMove(v *Value, config *Config) bool {
	...
	for {
		...
		v.AuxInt = 2
		v.AddArg(dst)
		v0 := b.NewValue0(v.Line, OpMIPSMOVBload, config.fe.TypeInt8())
		v0.AuxInt = 2
		v0.AddArg(src)
		v0.AddArg(mem)
		v.AddArg(v0)
		...
		v1.AuxInt = 1
		v1.AddArg(dst)
		v2 := b.NewValue0(v.Line, OpMIPSMOVBload, config.fe.TypeInt8())
		v2.AuxInt = 1
		v2.AddArg(src)
		v2.AddArg(mem)
		v1.AddArg(v2)
		v3 := b.NewValue0(v.Line, OpMIPSMOVBstore, TypeMem)
		v3.AddArg(dst)
		v4 := b.NewValue0(v.Line, OpMIPSMOVBload, config.fe.TypeInt8())
		v4.AddArg(src)
		v4.AddArg(mem)
		v3.AddArg(v4)
		v3.AddArg(mem)
...
func rewriteValueMIPS_OpSelect0(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Select0 (MakeResultCarryTuple r _))
	// cond:
	// result: r
	for {
		v_0 := v.Args[0]
		if v_0.Op != OpMIPSMakeResultCarryTuple {
			break
		}
		r := v_0.Args[0]
		v.reset(OpCopy)
...
func rewriteValueMIPS_OpSelect1(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Select1 (MakeResultCarryTuple _ c))
	// cond:
	// result: c
	for {
		v_0 := v.Args[0]
...
func rewriteValueMIPS_OpSub32carry(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Sub32carry <t> x y)
	// cond:
	// result: (MakeResultCarryTuple (SUB <t.FieldType(0)> x y) (SGTU (SUB <t.FieldType(0)> x y) x))
	for {
		t := v.Type
		x := v.Args[0]
		y := v.Args[1]
		...
		v.AddArg(v0)
		v1 := b.NewValue0(v.Line, OpMIPSSGTU, config.fe.TypeBool())
		v2 := b.NewValue0(v.Line, OpMIPSSUB, t.FieldType(0))
		v2.AddArg(x)
		v2.AddArg(y)
		v1.AddArg(v2)
		v1.AddArg(x)
