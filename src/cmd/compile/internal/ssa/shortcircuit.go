...
func shortcircuitBlock(b *Block) bool {
	...
	if nOtherPhi != 0 {
		...
		for i := 0; i < len(b.Values); i++ {
			...
			// phi moved to a different block.
			...
			phielimValue(phi)
			i-- // reprocess new value at this index
		}
...
func shortcircuitPhiPlan(b *Block, ctl *Value, cidx int, ti int64) func(*Value) {
	// t is the "taken" branch: the successor we always go to when coming in from p.
	t := b.Succs[ti].b
	...
	if len(t.Preds) == 1 && len(t.Succs) == 1 &&
		...
		m := t.Succs[0].b
		return func(v *Value) {
			// Replace any uses of v in t and u with the value v must
			// have given that we have arrived at that block.
			// Then move v to m and adjust its value accordingly;
			// this handles all other uses of v.
			argI, argX := v.Args[cidx], v.Args[1^cidx]
			replaceUses(u, v, argX)
			if v.Uses == 0 {
				return
			}
			phi := t.Func.newValue(OpPhi, v.Type, t, v.Pos)
			phi.AddArg2(argX, argI)
			replaceUses(t, v, phi)
			if v.Uses == 0 {
				return
			}
			v.moveTo(m)
			// The phi in m belongs to whichever pred idx corresponds to the taken successor.
			if m.Preds[0].b == t {
				v.SetArgs2(phi, argX)
			} else {
				v.SetArgs2(argX, phi)
	...
	if len(t.Preds) == 1 && len(u.Preds) == 1 &&
		(len(t.Succs) == 0 ||
			(len(t.Succs) == 1 && len(t.Succs[0].b.Succs) == 0 && len(t.Succs[0].b.Preds) == 1)) {
		//   b
		//  / \
		// t   u
		// where t is an exit block
		//
		// OR
		//
		//   b
	...
	if len(u.Preds) == 1 && len(t.Preds) == 1 &&
		(len(u.Succs) == 0 ||
			(len(u.Succs) == 1 && len(u.Succs[0].b.Succs) == 0 && len(u.Succs[0].b.Preds) == 1)) {
		//   b
		//  / \
		// u   t
		// where u is Exit/Ret
		...
		return func(v *Value) {
			...
			replaceUses(u, v, argX)
			if len(u.Succs) == 1 {
				replaceUses(u.Succs[0].b, v, argX)
			}
			if v.Uses == 0 {
				return
			}
			v.moveTo(t)
			v.SetArgs2(argX, argI)
		}
	}
...
func replaceUses(b *Block, old, new *Value) {
	for _, v := range b.Values {
		for i, a := range v.Args {
			if a == old {
				v.SetArg(i, new)
