...
func rewriteValueAMD64_OpAMD64MOVBQZX(v *Value) bool {
	...
	// match: (MOVBQZX x)
	// cond: zeroUpper56Bits(x,3) && setType(x, v.Type)
	// result: x
	for {
		x := v_0
		if !(zeroUpper56Bits(x, 3) && setType(x, v.Type)) {
			break
		}
		v.copyOf(x)
		return true
...
func rewriteValueAMD64_OpAMD64MOVWQZX(v *Value) bool {
	...
	// match: (MOVWQZX x)
	// cond: zeroUpper48Bits(x,3) && setType(x, v.Type)
	// result: x
	for {
		x := v_0
		if !(zeroUpper48Bits(x, 3) && setType(x, v.Type)) {
			break
		}
		v.copyOf(x)
		return true
