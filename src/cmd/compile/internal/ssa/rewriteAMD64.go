...
func rewriteValueAMD64_OpAMD64MOVBstore(v *Value) bool {
	...
	// match: (MOVBstore [c3] {s} p3 (SHRQconst [56] w) x1:(MOVWstore [c2] {s} p2 (SHRQconst [40] w) x2:(MOVLstore [c1] {s} p1 (SHRQconst [8] w) x3:(MOVBstore {s} p0 w mem))))
	// cond: x1.Uses == 1 && x2.Uses == 1 && x3.Uses == 1 && sequentialAddresses(p0, p1, int64(1 - c1)) && sequentialAddresses(p0, p2, int64(5 - c2)) && sequentialAddresses(p0, p3, int64(7 - c3)) && clobber(x1, x2, x3)
	...
	for {
		...
		mem := x3.Args[2]
		p0 := x3.Args[0]
		if w != x3.Args[1] || !(x1.Uses == 1 && x2.Uses == 1 && x3.Uses == 1 && sequentialAddresses(p0, p1, int64(1-c1)) && sequentialAddresses(p0, p2, int64(5-c2)) && sequentialAddresses(p0, p3, int64(7-c3)) && clobber(x1, x2, x3)) {
			break
		}
		v.reset(OpAMD64MOVQstore)
		v.Aux = symToAux(s)
