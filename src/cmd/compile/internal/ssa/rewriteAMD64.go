...
func rewriteValueAMD64(v *Value, config *Config) bool {
	...
	end32ef1328af280ac18fa8045a3502dae9:
		...
		// result: (InvertFlags (CMPQconst <TypeFlags> x [c]))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAMD64CMPQconst, TypeInvalid, 0, nil)
			v0.Type = TypeFlags
			v0.AddArg(x)
			v0.AuxInt = c
			v.AddArg(v0)
	...
	case OpIsInBounds:
		...
		// result: (SETB (CMPQ <TypeFlags> idx len))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAMD64CMPQ, TypeInvalid, 0, nil)
			v0.Type = TypeFlags
			v0.AddArg(idx)
			v0.AddArg(len)
			v.AddArg(v0)
	...
	case OpIsNonNil:
		...
		// result: (SETNE (TESTQ <TypeFlags> p p))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAMD64TESTQ, TypeInvalid, 0, nil)
			v0.Type = TypeFlags
			v0.AddArg(p)
			v0.AddArg(p)
			v.AddArg(v0)
	...
	case OpLsh:
		...
		// result: (ANDQ (SHLQ <t> x y) (SBBQcarrymask <t> (CMPQconst <TypeFlags> [64] y)))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAMD64SHLQ, TypeInvalid, 0, nil)
			v0.Type = t
			v0.AddArg(x)
			v0.AddArg(y)
			v.AddArg(v0)
			v1 := v.Block.NewValue(v.Line, OpAMD64SBBQcarrymask, TypeInvalid, 0, nil)
			v1.Type = t
			v2 := v.Block.NewValue(v.Line, OpAMD64CMPQconst, TypeInvalid, 0, nil)
			v2.Type = TypeFlags
			v2.AuxInt = 64
			v2.AddArg(y)
			v1.AddArg(v2)
	...
	case OpMove:
		...
		// result: (REPMOVSB dst src (Const <TypeUInt64> [size]) mem)
		{
			...
			v.AddArg(dst)
			v.AddArg(src)
			v0 := v.Block.NewValue(v.Line, OpConst, TypeInvalid, 0, nil)
			v0.Type = TypeUInt64
			v0.AuxInt = size
			v.AddArg(v0)
			v.AddArg(mem)
	...
	case OpRsh:
		...
		// result: (ANDQ (SHRQ <t> x y) (SBBQcarrymask <t> (CMPQconst <TypeFlags> [64] y)))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAMD64SHRQ, TypeInvalid, 0, nil)
			v0.Type = t
			v0.AddArg(x)
			v0.AddArg(y)
			v.AddArg(v0)
	...
	ende3e068773b8e6def1eaedb4f404ca6e5:
		...
		// result: (SARQ <t> x (CMOVQCC <t> 			(CMPQconst <TypeFlags> [64] y) 			(Const <t> [63]) 			y))
		{
			...
			v.Type = t
			v.AddArg(x)
			v0 := v.Block.NewValue(v.Line, OpAMD64CMOVQCC, TypeInvalid, 0, nil)
			v0.Type = t
			v1 := v.Block.NewValue(v.Line, OpAMD64CMPQconst, TypeInvalid, 0, nil)
			v1.Type = TypeFlags
			v1.AuxInt = 64
			v1.AddArg(y)
			v0.AddArg(v1)
			v2 := v.Block.NewValue(v.Line, OpConst, TypeInvalid, 0, nil)
			v2.Type = t
			v2.AuxInt = 63
			v0.AddArg(v2)
			v0.AddArg(y)
	...
	end5a74a63bd9ad15437717c6df3b25eebb:
		...
		// result: (NEGQ (SUBQconst <t> x [c]))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAMD64SUBQconst, TypeInvalid, 0, nil)
			v0.Type = t
			v0.AddArg(x)
			v0.AuxInt = c
			v.AddArg(v0)
...
func rewriteBlockAMD64(b *Block) bool {
	...
	end04935012db9defeafceef8175f803ea2:
		...
		// result: (NE (TESTB <TypeFlags> cond cond) yes no)
		{
			...
			b.Kind = BlockAMD64NE
			v0 := v.Block.NewValue(v.Line, OpAMD64TESTB, TypeInvalid, 0, nil)
			v0.Type = TypeFlags
			v0.AddArg(cond)
			v0.AddArg(cond)
			b.Control = v0
