...
func rewriteValueAMD64_OpAMD64MOVLstore_10(v *Value) bool {
	...
	// match: (MOVLstore [off] {sym} ptr a:(ADDLconst [c] l:(MOVLload [off2] {sym2} ptr2 mem2)) mem)
	// cond: off == off2 && sym == sym2 && isSamePtr(ptr, ptr2) && mem == mem2 && a.Uses == 1 && l.Uses == 1 && validValAndOff(c,off)
	...
	for {
		...
		mem2 := l.Args[1]
		mem := v.Args[2]
		if !(off == off2 && sym == sym2 && isSamePtr(ptr, ptr2) && mem == mem2 && a.Uses == 1 && l.Uses == 1 && validValAndOff(c, off)) {
			break
		}
...
func rewriteValueAMD64_OpAMD64MOVQstore_0(v *Value) bool {
	...
	// match: (MOVQstore [off] {sym} ptr a:(ADDQconst [c] l:(MOVQload [off2] {sym2} ptr2 mem2)) mem)
	// cond: off == off2 && sym == sym2 && isSamePtr(ptr, ptr2) && mem == mem2 && a.Uses == 1 && l.Uses == 1 && validValAndOff(c,off)
