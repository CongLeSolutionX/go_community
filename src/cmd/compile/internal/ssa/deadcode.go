	...
	for _, b := range f.Blocks {
		...
		for j, v := range b.Values {
			if live[v.ID] {
				...
				if v.Pos.IsStmt() == src.LicoIsStmt {
...
func moveStmtMarkerForward(j int, b *Block, l uint, live []bool) {
	...
	for k := j + 1; k < len(b.Values); k++ {
		u := b.Values[k]
		if len(live) > 0 && live[u.ID] && u.Pos.IsStmt() != src.LicoNotStmt {
			if u.Pos.Line() == l {
				u.Pos = u.Pos.WithIsStmt()
			}
			stopped = true
			break // if line numbers don't match, then it is lost.
		}
	}
	// If ran to the end of the block with no improvement, try the control.
	if !stopped && b.Control != nil && b.Pos.IsStmt() != src.LicoNotStmt && b.Pos.Line() == l {
