	...
	for _, b := range f.Blocks { // range loop is safe since the blocks we added contain no WB stores
		for i, v := range b.Values {
			...
			case OpStoreWB, OpMoveWB, OpMoveWBVolatile:
				...
				// there may be a sequece of WB stores in the current block. find them.
				storeWBs = storeWBs[:0]
				others = others[:0]
				...
				b.Succs = append(b.Succs[:0], Edge{bThen, 0}, Edge{bElse, 0})
				bThen.Preds = append(bThen.Preds, Edge{b, 0})
				bElse.Preds = append(bElse.Preds, Edge{b, 1})

				bThen.Succs = append(bThen.Succs, Edge{bEnd, 0})
				bElse.Succs = append(bElse.Succs, Edge{bEnd, 1})
				bEnd.Preds = append(bEnd.Preds, Edge{bThen, 0}, Edge{bElse, 0})
				...
				for _, w := range storeWBs {
					...
					val := w.Args[1]
					siz := w.AuxInt
					typ := w.Aux // only non-nil for MoveWB
