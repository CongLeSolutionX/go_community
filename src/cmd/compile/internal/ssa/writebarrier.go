...
// IsSafeAddr returns whether v is known to be an address that doesn't need instrumentation.
func IsSafeAddr(v *Value) bool {
	for v.Op == OpOffPtr || v.Op == OpAddPtr || v.Op == OpPtrIndex || v.Op == OpCopy {
		v = v.Args[0]
	}
	switch v.Op {
	...
	case OpAddr:
		...
		case OpSB:
			sym := v.Aux.(*obj.LSym)
			// TODO(mdempsky): This is hacky.
			if strings.HasPrefix(sym.Name, "go.itab.") || strings.Contains(sym.Name, ".statictmp_") || strings.HasSuffix(sym.Name, "·f") {
				return true
			}
		}

	}
	if v.Type.IsKind(types.TFUNC) {
		return true
	}
	return false
}
