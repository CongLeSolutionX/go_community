...
//
// TODO(mdempsky): Generalize by returning an *ssa.Value instead so we
// can simulate reads of more data.
func loadLSymOffset(lsym *obj.LSym, offset int64) *obj.LSym {
	if lsym.Type != objabi.SRODATA {
		return nil
	}

	// TODO(mdempsky): Would it be worthwhile to use binary search here?
	// Are relocs guaranteed to be ordered by Off?
