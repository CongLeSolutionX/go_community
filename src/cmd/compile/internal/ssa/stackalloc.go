...
func (s *stackAllocState) init(f *Func, spillLive [][]ID) {
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			s.values[v.ID].isArg = hasArgOp(v)
...
func (s *stackAllocState) stackalloc() {
	...
	for _, v := range f.Entry.Values {
		...
		if f.pass.debug > stackDebug {
			fmt.Printf("stackalloc OpArg*Reg %s to %s\n", v, loc)
		}
		...
		for i := len(b.Values) - 1; i >= 0; i-- {
			...
			if s.values[v.ID].needSlot {
				...
				for _, id := range live.contents() {
					...
					if s.values[v.ID].typ.Compare(s.values[id].typ) == types.CMPeq || hasArgOp(v) || s.values[id].isArg {
			...
			if hasArgOp(v) && s.values[v.ID].needSlot {
				...
				// Unclear if this is necessary or wrong for register Args
				live.add(v.ID)
			}
		}
	}
...
func hasArgOp(v *Value) bool {
	return v.Op == OpArg || v.Op == OpArgFReg || v.Op == OpArgIReg
}
