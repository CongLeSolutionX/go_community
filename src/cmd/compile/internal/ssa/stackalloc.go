...
func isArgOp(x Op) bool {
	switch x {
	case OpArgIReg, OpArgFReg, OpArg:
		return true
	}
	return false
}
...
func (s *stackAllocState) init(f *Func, spillLive [][]ID) {
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			s.values[v.ID].isArg = isArgOp(v.Op)
...
func (s *stackAllocState) stackalloc() {
	...
	for _, v := range f.Entry.Values {
		...
		// TODO this is old experiment code
		// Problem the new ABI has spill locations for registers, not "local slots".
		// but "Location" wants a Local Slot.
		// Looking up int/float parameter index, combined with f's param memory size, provides offset from FP.
		...
		for i := len(b.Values) - 1; i >= 0; i-- {
			...
			if s.values[v.ID].needSlot {
				...
				for _, id := range live.contents() {
					...
					if s.values[v.ID].typ.Compare(s.values[id].typ) == types.CMPeq || isArgOp(v.Op) || s.values[id].isArg {
