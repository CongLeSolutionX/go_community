...
func (s *stackAllocState) buildInterferenceGraph() {
	...
	for _, b := range f.Blocks {
		// Simultaneously live -> interfere
		/*
			fmt.Printf("doing %s\n", b)
			for i, x := range s.live[b.ID] {
				for _, y := range s.live[b.ID][i+1:] {
					if s.values[x].typ.Equal(s.values[y].typ) {
						// TODO: need this?
						s.interfere[x] = append(s.interfere[x], y)
						s.interfere[y] = append(s.interfere[y], x)
					}
				}
			}
		*/
