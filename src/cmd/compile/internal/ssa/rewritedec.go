...
func rewriteValuedec_OpSliceCap(v *Value) bool {
	...
	// cond: t.IsSlice()
	// result: @x.Block (Load <typ.Int> (OffPtr <typ.IntPtr> [v.Block.Func.ABISelf.SliceCapIndex() * config.PtrSize] ptr) mem)
	for {
		x := v_0
		if x.Op != OpLoad {
			break
		...
		v1.AuxInt = int64ToAuxInt(v.Block.Func.ABISelf.SliceCapIndex() * config.PtrSize)
		v1.AddArg(ptr)
		v0.AddArg2(v1, mem)
		return true
	}
...
func rewriteValuedec_OpSliceLen(v *Value) bool {
	...
	// cond: t.IsSlice()
	// result: @x.Block (Load <typ.Int> (OffPtr <typ.IntPtr> [v.Block.Func.ABISelf.SliceLenIndex() * config.PtrSize] ptr) mem)
	for {
		x := v_0
		if x.Op != OpLoad {
			break
		...
		v.copyOf(v0)
		v1 := b.NewValue0(v.Pos, OpOffPtr, typ.IntPtr)
		v1.AuxInt = int64ToAuxInt(v.Block.Func.ABISelf.SliceLenIndex() * config.PtrSize)
		v1.AddArg(ptr)
		v0.AddArg2(v1, mem)
		return true
	}
