...
func nilcheckelim(f *Func) {
	...
	// allocate auxillary date structures for computing store order
	...
	for len(work) > 0 {
		...
		case Work:
			...
			b.Values = storeOrder(b.Values, sset, stack, storeNumber)
...
// Auxillary data structures are passed in as arguments, so
// that they can be allocated in the caller and be reused.
// This function takes care of reset them.
func storeOrder(values []*Value, sset *sparseSet, stack []*Value, storeNumber []int32) []*Value {
	// find all stores
	var stores []*Value
	hasNilCheck := false
	sset.clear()
