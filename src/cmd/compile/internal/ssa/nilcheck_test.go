...
func TestNilcheckAddrOffPtr(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Valu("ptr1", OpAddr, ptrType, 0, nil, "sp"),
			Valu("ptr2", OpOffPtr, ptrType, 8, nil, "ptr1"),
			Valu("nilcheck", OpNilCheck, c.config.Types.Bool, 0, nil, "ptr2", "mem"),
...
// TestIsNonNilcheckAddrOffPtr verifies that nilchecks of OpAddr+OffPtr constructed
// values are removed.
func TestIsNonNilcheckAddrOffPtr(t *testing.T) {
	c := testConfig(t)
...
// TestNilcheckAddrPtrOffPtr verifies that nilchecks of OpAddPtr+OffPtr constructed
// values are removed.
func TestNilcheckAddrPtrOffPtr(t *testing.T) {
	c := testConfig(t)
	ptrType := c.config.Types.BytePtr
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Valu("arg", OpArg, ptrType, 0, nil),
			Valu("off", OpConst64, c.config.Types.Int64, 20, nil),
			Valu("ptr1", OpAddPtr, ptrType, 0, nil, "sp", "off"),
			Valu("ptr2", OpOffPtr, ptrType, 8, nil, "ptr1"),
			Valu("nilcheck", OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr2"),
			Valu("store", OpStore, types.TypeMem, 0, ptrType, "nilcheck", "arg", "mem"),
			Exit("store")))
