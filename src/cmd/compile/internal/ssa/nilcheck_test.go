...
func benchmarkNilCheckDeep(b *testing.B, depth int) {
	...
	for i := 0; i < depth; i++ {
		blocs = append(blocs,
			Bloc(blockn(i),
				Valu(ptrn(i), OpAddr, ptrType, 0, nil, "sb", "mem"),
...
func TestNilcheckAddr(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Goto("checkPtr")),
		Bloc("checkPtr",
			Valu("ptr1", OpAddr, ptrType, 0, nil, "sb", "mem"),
			Valu("bool1", OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr1"),
			If("bool1", "extra", "exit")),
		Bloc("extra",
			Goto("exit")),
...
func TestNilcheckPhi(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Valu("baddr", OpAddr, c.config.Types.Bool, 0, "b", "sp", "mem"),
		...
		Bloc("b1",
			...
			Goto("checkPtr")),
		Bloc("b2",
			Valu("ptr2", OpAddr, ptrType, 0, nil, "sb", "mem"),
			Goto("checkPtr")),
