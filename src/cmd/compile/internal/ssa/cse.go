	...
	for _, e := range partition {
		slices.SortFunc(e, func(v, w *Value) int {
			...
			if _, _, ok := isMemUser(v); ok {
				...
				return cmp.Compare(vSkips, wSkips)
			}
			return 0
		})
...
// Return index of pointer argument, index of "memory" argument and true on such instructions, otherwise return (-1, -1, false).
...
// Return index of pointer argument, index of "memory" argument and true on such instructions, otherwise return (-1, -1, false).
// TODO: We may also need the written type for aliasing queries (but currently we only detect OpStore, so it is in the Aux field).
...
func getEffectiveMemoryArg(memTable []int32, v *Value) (ID, uint32) {
	...
	if idxPtr, idxMem, ok := isMemUser(v); ok {
		...
		mem, skips := skipDisjointMemDefs(v, idxPtr, idxMem)
		if mem.ID <= maxId {
			memId = mem.ID
		} else {
...
func skipDisjointMemDefs(user *Value, idxUserPtr, idxUserMem int) (*Value, uint32) {
