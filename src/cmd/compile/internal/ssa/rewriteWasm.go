...
func rewriteValueWasm_OpWasmI64LeU(v *Value) bool {
	...
	for {
		if v_0.Op != OpWasmI64Const || v_0.AuxInt != 1 {
			break
		}
		x := v_1
		v.reset(OpWasmI64Eqz)
...
func rewriteValueWasm_OpWasmI64Load(v *Value) bool {
	...
	for {
		...
		v.reset(OpWasmI64Const)
		v.AuxInt = int64(read64(sym, off+int64(off2), config.ctxt.Arch.ByteOrder))
		return true
	}
	return false
}
...
func rewriteValueWasm_OpWasmI64Load16U(v *Value) bool {
	...
	for {
		...
		v.reset(OpWasmI64Const)
		v.AuxInt = int64(read16(sym, off+int64(off2), config.ctxt.Arch.ByteOrder))
		return true
	}
	return false
}
...
func rewriteValueWasm_OpWasmI64Load32U(v *Value) bool {
	...
	for {
		...
		v.reset(OpWasmI64Const)
		v.AuxInt = int64(read32(sym, off+int64(off2), config.ctxt.Arch.ByteOrder))
		return true
	}
	return false
}
...
func rewriteValueWasm_OpWasmI64Load8U(v *Value) bool {
	...
	for {
		...
		v.reset(OpWasmI64Const)
		v.AuxInt = int64(read8(sym, off+int64(off2)))
		return true
	}
	return false
}
func rewriteValueWasm_OpWasmI64LtU(v *Value) bool {
	...
	// result: (I64Eqz (I64Eqz x))
	for {
		if v_0.Op != OpWasmI64Const || v_0.AuxInt != 0 {
			break
		}
		x := v_1
		v.reset(OpWasmI64Eqz)
	...
	for {
		x := v_0
		if v_1.Op != OpWasmI64Const || v_1.AuxInt != 1 {
			break
		}
		v.reset(OpWasmI64Eqz)
		v.AddArg(x)
