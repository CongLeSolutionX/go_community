...
func (po *poset) Ordered(n1, n2 *Value) bool {
	...
	if po.isInvalidConstant(n1) || po.isInvalidConstant(n2) {
		// We can't store relations about invalid constants, so for now let's
		// assume we know nothing.
		// TODO(rasky): we could do better here: eg: an unsigned poset
		// cannot store explicit relations with negative numbers, but surely
		// all negative numbers are outside of possible bounds, so it would
		// be possible to still return a correct value here.
		return false
	}
