...
// vconst returns an index that can be used as a SSA op in posetTestOp to refer
// to a SSA constant (OpConst64). For instance, vconst(-2) can be used to
// use a SSA constant with value -2 in the tests.
// Allowed values are in the range [-128, 127].
...
// vconst2 is exactly like vconst, but returns a different index (thus, a different
// fake SSA op) for the same numeric value. Can be used in tests to verify that
// poset implementation works correctly when different SSA constant instances are
// used with the same numeric value.
...
func TestPosetConst(t *testing.T) {
	...
	testPosetOps(t, false, []posetTestOp{
		{Checkpoint, 0, 0},
		{SetOrder, 1, vconst(15)},
		// An unsigned poset cannot record relations with negative numbers
		{SetOrderOrEqual, 100, vconst(-5)},
		{SetOrderOrEqual, 100, vconst(100)},

		{Ordered, 1, vconst(15)},
		{Ordered_Fail, 1, vconst(-5)},
		{Ordered, 1, vconst(100)},
		{OrderedOrEqual, 1, vconst(100)},
		{OrderedOrEqual_Fail, 100, vconst(-5)},
		{OrderedOrEqual, 100, vconst(100)},
		{Ordered_Fail, 100, vconst(15)},
		{Ordered_Fail, vconst(15), 100},

		// Check that we cannot record equality / non-equality with negative
		// numbers.
		{SetEqual, 2, vconst(-5)},
		{SetNonEqual, 3, vconst(-5)},
		{Equal_Fail, 2, vconst(-5)},
		{NonEqual_Fail, 3, vconst(-5)},
