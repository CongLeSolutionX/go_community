...
type regAllocState struct {
	...
	// a list of copies we generated to move things around.
	// they will be deleted at the end if not get used.
	copies []*Value
...
func (s *regAllocState) init(f *Func) {
	...
	s.copies = make([]*Value, 0, f.NumValues())
...
sinking:
	...
	for _, c := range s.copies {
		if c.Uses == 0 && s.f.Config.DebugTest {
			if s.f.pass.debug > regDebug {
				fmt.Printf("delete copied value %s : %s\n", c.LongString(), c.RegName())
			}
			c.Args[0].Uses--
			f.freeValue(c)
		}
...
func (e *edgeState) processDest(loc Location, vid ID, splice **Value, line int32) bool {
	...
	if occupant.vid == vid {
		...
		occupant.c.Uses++
		// Note: if splice==nil then c will have Uses > 0 but not appeared
		// to be used anywhere. This is non-SSA formed code, so be careful
		// after this pass not to run deadcode elimination.
		return true
	}
