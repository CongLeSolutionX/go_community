...
func (s *regAllocState) regalloc(f *Func) {
	...
	var nSpillsLost int           // # of sunk spills lost because of register use
...
sinking:
	for _, ts := range toSink {
		...
		// Pre-check to be sure that spilled value is still in expected register on all exits.
	...
	if f.pass.stats > 0 {
		f.logStat("spills_info",
			nSpills, "spills", nSpillsInner, "inner_spills_remaining", nSpillsSunk, "inner_spills_sunk", nSpillsSunkUnused, "inner_spills_unused", nSpillsNotSunkLateUse, "inner_spills_shuffled", nSpillsLost, "inner_spills_lost")
	}
}
