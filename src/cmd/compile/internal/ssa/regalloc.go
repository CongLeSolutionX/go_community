...
func (s *regAllocState) regspec(v *Value) regInfo {
	...
	if op == OpArgIntReg {
		...
		return regInfo{inputs: []inputInfo{}, outputs: []outputInfo{{regs: 1 << uint(reg)}}}
	}
	if op == OpArgFloatReg {
...
func (s *regAllocState) regalloc(f *Func) {
	...
	for _, b := range s.visitOrder {
		...
		ok:
			...
			{
				outRegs := [32]register{} // Bias registers by +1 so that zero is the no-value value; 0 minus 1 = noRegister
				maxOutIdx := -1
				var used regMask
