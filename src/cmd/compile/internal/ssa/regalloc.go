...
// at least one of its predecessors are scheduled.  The most recent such
// predecessor provides the starting register state for a block.
//
...
// If the spill (x2) is never used, it will be removed.
//
// Phi values are special, as always.  If the phi value starts out in a
// register, then it is spilled similarly to regular ops:
//     b1: y = ... : AX        b2: z = ... : AX
//         goto b3                 goto b3
//     b3: x = phi(y, z) : AX
//         x2 = StoreReg x
//
// If a phi value starts out on the stack, then it is spilled at the end
// of the predecessor blocks using StoreReg ops:
//     b1: y = ... : AX        b2: z = ... : BX
...
type valState struct {
	...
	uses       []int32 // places where Value is used
	usestorage [2]int32
...
type regAllocState struct {
	...
	// order of basic blocks, indexed by block ID
	blockOrder []int32

	// for each block, the index of primary predecessor in its Preds list
	primary []int32

	// live values on each edge.  live[b.ID][idx] is a list of value IDs
	// which are live on b's idx'th succecessor edge.
	live [][][]ID
	...
	// An ordered list of all uses in the function
	uses []use
...
func (s *regAllocState) allocReg(mask regMask) register {
	...
	for t := register(0); t < numRegs; t++ {
		...
		if s.values[v.ID].uses[0] > maxuse {
			r = t
			maxuse = s.values[v.ID].uses[0]
		}
	}
	if maxuse == -1 {
...
func (s *regAllocState) init(f *Func) {
	...
	// Compute primary predecessors.  A predecessor of b is primary if it is the
	// closest predecessor that appears before b in the layout order.
	// We record the index in the Preds list where the primary predecessor sits.
	s.primary = make([]int32, f.NumBlocks())
	for _, b := range f.Blocks {
		best := -1
		for i, p := range b.Preds {
			if s.blockOrder[p.ID] >= s.blockOrder[b.ID] {
				continue // backward edge
			}
			if best == -1 || s.blockOrder[p.ID] > s.blockOrder[b.Preds[best].ID] {
				best = i
			}
		}
	...
	for _, b := range f.Blocks {
		...
		if b.Control != nil {
			s.values[b.Control.ID].uses = append(s.values[b.Control.ID].uses, pc*2-1)
			s.uses = append(s.uses, use{pc*2 - 1, b.Control.ID})
		}
		// uses by backedges
		// Backedges are treated as uses so that the uses span the entire live
		// range of the value.
		for i, c := range b.Succs {
			if s.blockOrder[c.ID] > s.blockOrder[b.ID] {
				continue // forward edge
			}
			for _, vid := range s.live[b.ID][i] {
				s.values[vid].uses = append(s.values[vid].uses, pc*2-1)
				s.uses = append(s.uses, use{pc*2 - 1, vid})
			}
		}
	}
	if pc*2 < 0 {
...
func (s *regAllocState) regalloc(f *Func) {
	...
	for _, v := range f.Entry.Values {
		...
		case OpSP:
			sp = v
			s.assignReg(register(4), v, v) // TODO: arch-dependent
		case OpSB:
			sb = v
			s.assignReg(register(32), v, v) // TODO: arch-dependent
		}
	}
	_ = sb
	_ = sp
	...
	// start state (after phi ops) of registers for each merge block,
	// indexed by blockid/regnum.
	...
	for _, b := range f.Blocks {
		...
		if b == f.Entry {
			...
			c := b.Preds[idx]
			s.setState(endRegs[c.ID])

			// Drop anything not live on the c->b edge.
			var idx2 int
			for idx2 = 0; idx2 < len(c.Succs); idx2++ {
				if c.Succs[idx2] == b {
					break
				}
			}
			liveset.clear()
			liveset.addAll(s.live[c.ID][idx2])
			for r := register(0); r < numRegs; r++ {
				v := s.regs[r].v
				if v == nil {
					continue
			...
			for _, v := range oldSched {
				...
				m := s.values[v.Args[idx].ID].regs &^ used
				var r register
				if m != 0 {
					r = pickReg(m)
					used |= regMask(1) << r
		...
		for _, v := range oldSched {
			...
			for _, j := range regspec.inputOrder {
				a := v.Args[j]
				if a.Op == OpSP || a.Op == OpSB {
					continue
				}
				v.Args[j] = s.allocValToReg(a, regspec.inputs[j], true)
			}
		...
		// TODO: Load control value into reg (just for flags?)
	...
	for _, b := range f.Blocks {
		...
		for i, p := range b.Preds {
			...
			for _, v := range b.Values {
				...
				// If aleady in a register, use that.  If not, use register 0.
			...
			for _, v := range b.Values {
				...
				r := s.getHome(v)
				if r == noRegister {
					continue // stack-based phi handled above
				}
				if dst[r] != v {
...
// live returns a map from block ID and succecessor edge index to a list
