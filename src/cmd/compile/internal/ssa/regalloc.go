	...
	for _, b := range f.Blocks {
		...
		for idx, v := range oldSched {
				...
				if mask&wreg != 0 {
					...
					if len(w.Args) == 0 {
						...
						if w.Op == OpFP || w.Op == OpSP || w.Op == OpGlobal {
							c = b.NewValue1(w.Line, OpCopy, w.Type, 0, nil, w)
						} else {
							c = b.NewValue(w.Line, w.Op, w.Type, w.AuxInt, w.Aux)
						}
					} else if len(w.Args) == 1 && (w.Args[0].Op == OpFP || w.Args[0].Op == OpSP || w.Args[0].Op == OpGlobal) {
						// Materialize offsets from SP/FP/Global
						c = b.NewValue1(w.Line, w.Op, w.Type, w.AuxInt, w.Aux, w.Args[0])
					} else if wreg != 0 {
						...
						s := pickReg(wreg)
						// inv: s != r
						c = b.NewValue(w.Line, OpCopy, w.Type, 0, nil)
						c.AddArg(regs[s].c)
					} else {
						// Load from home location
						c = b.NewValue(w.Line, OpLoadReg8, w.Type, 0, nil)
						c.AddArg(w)
					}
			...
			if len(outputs) == 0 || outputs[0] == 0 {
				...
				c := b.NewValue(v.Line, v.Op, v.Type, v.AuxInt, v.Aux)
				c.AddArgs(v.Args...)
				home = setloc(home, c, &registers[r])
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			for i, w := range v.Args {
				c := b.Preds[i]
				cpy := c.NewValue1(w.Line, OpCopy, v.Type, 0, nil, w)
				v.Args[i] = cpy
			}
		}
	}
