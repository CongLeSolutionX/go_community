...
type regAllocState struct {
	...
	// register state at the end of each block
	endRegs [][]endReg

	// register state at the start of merge blocks
	// saved state does not include phi ops.
	startRegs [][]startReg

	// set of live spills at the end of each block
	spillLive [][]ID
}

type endReg struct {
	r register
	v *Value // pre-regalloc value held in this register (TODO: id?)
...
func (s *regAllocState) regalloc(f *Func) {
	...
	for _, b := range f.Blocks {
		...
		// isn't in a register, insert a pseudouse and mark
		// its spill location as used.  We need to do this here
		// to make the liveness analysis in stackalloc correct.
		// TODO: pass this info to stackalloc some other way?
		for _, e := range s.live[b.ID] {
