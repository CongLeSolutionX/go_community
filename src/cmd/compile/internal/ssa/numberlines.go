...
func nextGoodStatementIndex(v *Value, i int, b *Block) int {
	...
	// program execution order, which tends to be true directly after "build")
	if i >= len(b.Values)-1 {
		return i
	}
...
func notStmtBoundary(op Op) bool {
	...
	case OpCopy, OpPhi, OpVarKill, OpVarDef, OpUnknown, OpFwdRef, OpArg:
		// not OpCopy; rewrite overwrites ops w/ OpCopy, but their Pos can be a
		// statement boundary that needs to be preserved by movement to another Value.
		return true
	}
	return false
}
	...
	for j := len(po) - 1; j >= 0; j-- {
		...
		for i, v := range b.Values {
			if v.Pos.IsStmt() != src.PosNotStmt {
				note(v.Pos.Line())
				if i != nextGoodStatementIndex(v, i, b) {
					continue
				}
				firstPosIndex = i
				firstPos = v.Pos
