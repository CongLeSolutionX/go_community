...
func isPoorStatementStart(v *Value, i int, b *Block) bool {
	...
	switch v.Op {
	case OpAddr, OpStructSelect, OpConst8, OpConst16, OpConst32, OpConst64, OpConst32F, OpConst64F:
	default:
		return false
	}
	// The successor also has to be eligible to be a statement with the same line number.
	return b.Values[i+1].Pos.IsStmt() != src.LicoNotStmt && b.Values[i+1].Pos.Line() == v.Pos.Line()
}
