...
func tighten(f *Func) {
	...
	// Find loop depth of each block.
	// We use this to make sure we don't tighten a value into a (deeper) loop.
	loops := f.loopnest()
	loops.calculateDepths()
	depth := make([]int16, f.NumBlocks())
	for i := range depth {
		if loop := loops.b2l[i]; loop != nil {
			depth[i] = loop.depth
		}
	}
	...
	for changed {
		...
		// If the location we would move a value to is inside a loop, move
		// the location up to just before the loop head.
		for _, b := range f.Blocks {
			...
			for _, v := range b.Values {
				...
				for targetloop != nil && (origloop == nil || targetloop.depth > origloop.depth) {
					// Find a predecessor of the loop head not in the loop.
					for _, e := range targetloop.header.Preds {
						p := e.b
						if loops.b2l[p.ID] != targetloop {
							t = p
							target[v.ID] = t
							targetloop = loops.b2l[t.ID]
							break
