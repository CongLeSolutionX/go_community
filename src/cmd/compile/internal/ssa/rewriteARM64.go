...
func rewriteValueARM64_OpPopCount16_0(v *Value) bool {
	...
	// cond:
	// result: (FMOVDfpgp <t> (VUADDLV <typ.Float32> (VCNT <typ.Float32> (FMOVDgpfp <typ.UInt16> (ANDconst <t> [(1<<16)-1] x)))))
	for {
		t := v.Type
		x := v.Args[0]
...
func rewriteValueARM64_OpPopCount32_0(v *Value) bool {
	...
	// cond:
	// result: (FMOVDfpgp <t> (VUADDLV <typ.Float64> (VCNT <typ.Float64> (FMOVDgpfp <typ.UInt64> (ZeroExt32to64 x)))))
	for {
		t := v.Type
		x := v.Args[0]
		v.reset(OpARM64FMOVDfpgp)
...
func rewriteValueARM64_OpPopCount64_0(v *Value) bool {
	...
	// cond:
	// result: (FMOVDfpgp <t> (VUADDLV <typ.Float64> (VCNT <typ.Float64> (FMOVDgpfp <typ.UInt64> x))))
	for {
		t := v.Type
		x := v.Args[0]
		v.reset(OpARM64FMOVDfpgp)
