...
func rewriteValueARM64(v *Value) bool {
	...
	case OpARM64MOVQstore:
		return rewriteValueARM64_OpARM64MOVQstore_0(v)
...
func rewriteValueARM64_OpARM64MOVQstore_0(v *Value) bool {
	b := v.Block
	_ = b
	config := b.Func.Config
	_ = config
	// match: (MOVQstore [off1] {sym} (ADDconst [off2] ptr) val1 val2 mem)
	// cond: is32Bit(off1+off2) 	&& (ptr.Op != OpSB || !config.ctxt.Flag_shared)
	// result: (MOVQstore [off1+off2] {sym} ptr val1 val2 mem)
	for {
		off1 := v.AuxInt
		sym := v.Aux
		_ = v.Args[3]
	...
	// match: (MOVQstore [off1] {sym1} (MOVDaddr [off2] {sym2} ptr) val1 val2 mem)
	// cond: canMergeSym(sym1,sym2) && is32Bit(off1+off2) 	&& (ptr.Op != OpSB || !config.ctxt.Flag_shared)
	// result: (MOVQstore [off1+off2] {mergeSym(sym1,sym2)} ptr val1 val2 mem)
	for {
		off1 := v.AuxInt
		sym1 := v.Aux
		_ = v.Args[3]
	...
	// match: (MOVQstore [off] {sym} ptr (MOVDconst [0]) (MOVDconst [0]) mem)
	// cond:
...
func rewriteValueARM64_OpZero_10(v *Value) bool {
	...
	// cond:
	// result: (MOVQstore [0] ptr (MOVDconst [0]) (MOVDconst [0]) mem)
	for {
		if v.AuxInt != 16 {
			break
		}
	...
	// cond:
	// result: (MOVQstore [16] ptr (MOVDconst [0]) (MOVDconst [0]) 		(MOVQstore [0] ptr (MOVDconst [0]) (MOVDconst [0]) mem))
	for {
		if v.AuxInt != 32 {
			break
		}
		...
		v1.AuxInt = 0
		v.AddArg(v1)
		v2 := b.NewValue0(v.Pos, OpARM64MOVQstore, types.TypeMem)
		v2.AuxInt = 0
		v2.AddArg(ptr)
	...
	// cond:
	// result: (MOVQstore [32] ptr (MOVDconst [0]) (MOVDconst [0]) 		(MOVQstore [16] ptr (MOVDconst [0]) (MOVDconst [0]) 			(MOVQstore [0] ptr (MOVDconst [0]) (MOVDconst [0]) mem)))
	for {
		if v.AuxInt != 48 {
			break
		}
		...
		v4.AuxInt = 0
		v2.AddArg(v4)
		v5 := b.NewValue0(v.Pos, OpARM64MOVQstore, types.TypeMem)
		v5.AuxInt = 0
		v5.AddArg(ptr)
		v6 := b.NewValue0(v.Pos, OpARM64MOVDconst, typ.UInt64)
		v6.AuxInt = 0
	...
	// cond:
	// result: (MOVQstore [48] ptr (MOVDconst [0]) (MOVDconst [0]) 		(MOVQstore [32] ptr (MOVDconst [0]) (MOVDconst [0]) 			(MOVQstore [16] ptr (MOVDconst [0]) (MOVDconst [0]) 				(MOVQstore [0] ptr (MOVDconst [0]) (MOVDconst [0]) mem))))
	for {
		if v.AuxInt != 64 {
			break
		}
		...
		v7.AuxInt = 0
		v5.AddArg(v7)
		v8 := b.NewValue0(v.Pos, OpARM64MOVQstore, types.TypeMem)
		v8.AuxInt = 0
		v8.AddArg(ptr)
...
func rewriteValueARM64_OpZero_20(v *Value) bool {
	...
	// match: (Zero [s] ptr mem)
	// cond: s%16 == 0 && (s > 8*128 || config.noDuffDevice)
	...
	for {
		...
		ptr := v.Args[0]
		mem := v.Args[1]
		if !(s%16 == 0 && (s > 8*128 || config.noDuffDevice)) {
			break
		}
