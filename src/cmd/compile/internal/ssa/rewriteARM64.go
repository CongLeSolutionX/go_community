...
func rewriteValueARM64_OpARM64FMOVSstore_0(v *Value) bool {
	...
	config := b.Func.Config
	_ = config
	// match: (FMOVSstore ptr (FMOVSgpfp val) mem)
	// cond:
	// result: (MOVDstore ptr val mem)
	for {
		_ = v.Args[2]
...
func rewriteValueARM64_OpARM64MOVDstore_0(v *Value) bool {
	...
	// match: (MOVDstore ptr (FMOVSfpgp val) mem)
	// cond:
...
func rewriteValueARM64_OpPopCount16_0(v *Value) bool {
	...
	// cond:
	// result: (FMOVSfpgp <t> (VUADDLV <typ.Float64> (VCNT <typ.Float64> (FMOVDgpfp <typ.UInt16> (ANDconst <t> [(1<<16)-1] x)))))
	for {
		t := v.Type
		x := v.Args[0]
...
func rewriteValueARM64_OpPopCount32_0(v *Value) bool {
	...
	// cond:
	// result: (FMOVSfpgp <t> (VUADDLV <typ.Float64> (VCNT <typ.Float64> (FMOVDgpfp <typ.UInt64> (ZeroExt32to64 x)))))
	for {
		t := v.Type
		x := v.Args[0]
		v.reset(OpARM64FMOVSfpgp)
...
func rewriteValueARM64_OpPopCount64_0(v *Value) bool {
	...
	// cond:
	// result: (FMOVSfpgp <t> (VUADDLV <typ.Float64> (VCNT <typ.Float64> (FMOVDgpfp <typ.UInt64> x))))
	for {
		t := v.Type
		x := v.Args[0]
		v.reset(OpARM64FMOVSfpgp)
