...
func rewriteValueARM64_OpARM64ORshiftLL(v *Value) bool {
	...
	// cond: arm64REV16WConstMask(a, b)
	// result: (REV16W <typ.UInt32> w)
	for {
		if v.Type != typ.UInt32 || v.AuxInt != 8 || v_0.Op != OpARM64UBFX || v_0.Type != typ.UInt32 || v_0.AuxInt != armBFAuxInt(8, 24) {
			break
		}
		v_0_0 := v_0.Args[0]
		...
		b := v_1.AuxInt
		if w != v_1.Args[0] || !(arm64REV16WConstMask(a, b)) {
			break
		}
		v.reset(OpARM64REV16W)
		v.Type = typ.UInt32
	...
	for {
		...
		if w != v_1.Args[0] || !(arm64REV16DConstMask(a, b)) {
			break
		}
