...
func rewriteBlockARM64(b *Block) bool {
	...
	case BlockARM64EQ:
		...
		// match: (EQ (ANDSconst [c] x) yes no)
		// cond: oneBit(c)
		// result: (TBZ {ntz(c)} x yes no)
		for {
			v := b.Control
			if v.Op != OpARM64ANDSconst {
				break
			}
			c := v.AuxInt
			x := v.Args[0]
	...
	case BlockARM64NE:
		...
		// match: (NE (ANDSWconst [c] x) yes no)
		// cond: oneBit(int64(uint32(c)))
