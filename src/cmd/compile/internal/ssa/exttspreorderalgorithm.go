...
func reorderBasicBlocksUsingTSPReorderAlgorithm(f *Func) {
	N := len(f.Blocks)
	...
	for _, b := range f.Blocks {
		for _, e := range b.Succs {
			if e.b.BBFreq.RawCount != 0 {
				Weight[b.LayoutIndex][e.b.LayoutIndex] = (uint64)(e.b.BBFreq.RawCount)
			}
		}
	}
	DP := make([][]int64, 1<<N)
...
//
// ===----------------------------------------------------------------------=== //
func reorderBasicBlocksUsingExtTSPReorderAlgorithm(f *Func) {
	extTSP := ExtTSP{
		f:         f,
...
// reorderBasicBlocks uses profile to reorder the basic blocks in the given function.
func tspReorderBasicBlocks(f *Func) {
	if base.Flag.PgoBb != 2 {
		return
	}
	...
	if len(f.Blocks) <= TSPThreshold {
		reorderBasicBlocksUsingTSPReorderAlgorithm(f)
	} else {
		reorderBasicBlocksUsingExtTSPReorderAlgorithm(f)
	}
