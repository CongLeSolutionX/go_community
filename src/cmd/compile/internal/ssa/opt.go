...
func expandCalls(f *Func) {
	...
	val2Preds := make(map[*Value]int32)
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			// As long as we're here, clean out any copies that were inserted.
			// This code is stolen from rewrite.go, minus the line number updates,
			// which should be done above at the site where the copies are inserted, if needed.
