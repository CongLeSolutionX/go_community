...
func expandCalls(f *Func) {
	...
	isSSAAggregateType := func(t *types.Type) bool {
		if !canSSAType(t) {
			return false
		}
		et := t.Etype
	...
	storeArg = func(pos src.XPos, b *Block, a *Value, t *types.Type, offset int64, mem *Value) *Value {
		...
		case OpInt64Make:
			...
			mem = storeArg(pos, b, a.Args[0], tHi, hiOffset, mem)
			return storeArg(pos, b, a.Args[1], tLo, lowOffset, mem)
		}
	...
	splitStore = func(dst, src, mem, v *Value, t *types.Type, offset int64, firstStorePos src.XPos) *Value {
		...
		case types.TINT64, types.TUINT64:
			...
			mem = splitStore(dst, sel, mem, v, tHi, hiOffset, firstStorePos)
			firstStorePos = firstStorePos.WithNotStmt()
			sel = src.Block.NewValue1(pos, OpInt64Lo, tLo, src)
			mem = splitStore(dst, sel, mem, v, tLo, lowOffset, firstStorePos)
	...
	for _, b := range f.Blocks {
		for _, v := range b.Values {
			...
			case OpStaticLECall:
				...
				// DO NOT CHANGE THE TYPE, need it for converting aggregate args.
				v.Aux = aux
				v.AuxInt = auxInt
				v.SetArgs1(mem)
			}
