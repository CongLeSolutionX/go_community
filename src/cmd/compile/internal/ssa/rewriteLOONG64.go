...
func rewriteValueLOONG64_OpLOONG64ADDVconst(v *Value) bool {
	...
	for {
		...
		v.reset(OpLOONG64MOVVaddr)
		v.AuxInt = int64(int32(off1) + int32(off2))
		v.Aux = sym
		v.AddArg(ptr)
		return true
	}
...
func rewriteValueLOONG64_OpLOONG64DIVVU(v *Value) bool {
	...
	for {
		x := v_0
		if v_1.Op != OpLOONG64MOVVconst || v_1.AuxInt != 1 {
			break
		}
		v.copyOf(x)
		return true
...
func rewriteValueLOONG64_OpLOONG64LoweredAtomicStore32(v *Value) bool {
	...
	for {
		ptr := v_0
		if v_1.Op != OpLOONG64MOVVconst || v_1.AuxInt != 0 {
			break
		}
		mem := v_2
...
func rewriteValueLOONG64_OpLOONG64MASKEQZ(v *Value) bool {
	...
	// result: (MOVVconst [0])
	for {
		if v_0.Op != OpLOONG64MOVVconst || v_0.AuxInt != 0 {
			break
		}
		v.reset(OpLOONG64MOVVconst)
		return true
...
func rewriteValueLOONG64_OpLOONG64MOVBUload(v *Value) bool {
	...
	for {
		...
		v.AuxInt = int64(off1 + int32(off2))
		v.Aux = sym
		v.AddArg2(ptr, mem)
		return true
	}
...
func rewriteValueLOONG64_OpMove(v *Value) bool {
	...
	for {
		...
		v.AuxInt = t.Alignment()
		v0 := b.NewValue0(v.Pos, OpLOONG64ADDVconst, src.Type)
		v0.AuxInt = s - moveSize(t.Alignment(), config)
		v0.AddArg(src)
...
func rewriteBlockLOONG64(b *Block) bool {
	...
	case BlockLOONG64EQ:
		...
		for b.Controls[0].Op == OpLOONG64SGTU {
			...
			x := v_0.Args[0]
			v_0_1 := v_0.Args[1]
			if v_0_1.Op != OpLOONG64MOVVconst || v_0_1.AuxInt != 0 {
				break
			}
