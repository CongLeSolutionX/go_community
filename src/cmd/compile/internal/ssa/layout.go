...
func layout(f *Func) {
	f.Blocks = layoutOrder(f, f.pass.test&1 == 0, f.pass.test&2 == 0)
}
...
func layoutRegallocOrder(f *Func) []*Block {
	...
	switch f.pass.test {
	case 0: // layout order
		return layoutOrder(f, false, true)
	case 1: // existing block order
		return f.Blocks
	case 2: // layout order, reversed exits
		return layoutOrder(f, false, false)
	case 3: // reverse of postorder; legal, but usually not good.
		po := f.postorder()
...
func layoutOrder(f *Func, rotateLoops, exitsInOrder bool) []*Block {
	...
	for {
		...
		if rotateLoops && len(order) > 0 {
			...
			if bl, bendl := ln.b2l[b.ID], ln.b2l[order[len(order)-1].ID]; bl != nil &&
				...
				bstart := ln.goodtop(b, bl, b2PO, scheduled)
				// if f.pass.debug > 0 {
				// 	fmt.Printf("Goodtop %v returns %v\n", b, bstart)
				// }
				if bstart != nil && !scheduled[bstart.ID] {
					zerodegree.add(b.ID) // refile this one.
					b = bstart
					bid = b.ID
					didLoop[bl] = true
				}
