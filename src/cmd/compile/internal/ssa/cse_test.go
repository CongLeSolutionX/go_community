...
func TestCSEAuxPartitionBug(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Valu("raddr", OpAddr, c.config.Types.Int64.PtrTo(), 0, nil, "sp", "raddrdef"),
...
func TestZCSE(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Valu("addr1", OpAddr, c.config.Types.Int64.PtrTo(), 0, nil, "sb1", "start"),
			Valu("addr2", OpAddr, c.config.Types.Int64.PtrTo(), 0, nil, "sb2", "start"),
