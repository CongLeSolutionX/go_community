...
func (s *gdbState) start() {
	...
	s.ioState.readExpecting(-1, 5000, "Reading symbols from ")
	expect("Breakpoint [0-9]+ at", s.ioState.writeRead("b main.main\n"))
	run := "run"
...
func expect(want string, got tstring) {
	if want != "" {
		...
		if match {
			fmt.Printf("MATCHED(O) %s AS %s\n", got.o, want)
			return
		}
		match, err = regexp.MatchString(want, got.e)
		if match {
			fmt.Printf("MATCHED(E) %s AS %s\n", got.e, want)
			return
		}
		fmt.Printf("EXPECTED '%s', GOT '%s' AND '%s'\n", want, got.o, got.e)
	}
}
