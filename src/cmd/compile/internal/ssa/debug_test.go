...
func testNexting(t *testing.T, base string) {
	...
	if (runtime.GOOS != "linux" || runtime.GOARCH != "amd64") && !*delve && !*force {
		// Running gdb on OSX/darwin is very flaky.
		// It also probably requires an admin password typed into a dialog box.
		// Non-linux, non-amd64 tends to either have a different stepping history
		// and/or not have gdb installed on the test machines.
		return
	}
	...
	runGo(t, "testdata", "build", "-o", exe, base+".go")
	var h1 *nextHist
	if *delve {
		h1 = dlvTest(exe, 1000)
		nextlog = filepath.Join("testdata", base+".delve-nexts")
	} else {
		h1 = gdbTest(exe, 1000)
	}
	if *update {
