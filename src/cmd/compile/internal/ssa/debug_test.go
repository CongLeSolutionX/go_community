...
var repeats = flag.Bool("r", false, "detect repeats in debug steps (gdb only)")
...
//
// Because this test (combined with existing compiler deficiencies) is flaky, for gdb-based testing
// by default inlining is disabled (otherwise output depends on library internals)
// and by default repeated lines in the next stream are ignored (because this appears to be timing-dependent in gdb)
...
func TestNexting(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		t.Run(fmt.Sprintf("dbg-%d", i), func(t *testing.T) {
...
func (h *nextHist) add(file, line, text string) bool {
	// Do not record beyond the return from main.
	if strings.Contains(file, "src/runtime/proc.go") {
		return false
	}
	fi := h.f2i[file]
	if fi == 0 {
