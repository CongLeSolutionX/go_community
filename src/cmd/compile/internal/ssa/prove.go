	...
	if r == gt || r == gt|eq {
		if x, delta := isConstDelta(v); x != nil && d == signed {
			...
			if !w.isGenericIntConst() {
				// If w is non-constant, let's derive:  x-d > w  ⇒  x > w
				// This is useful for loops with bounds "len(slice)-d".
				...
				// With w,d constants, we want to derive: x+d > w  ⇒  x > w-d
				//
				...
				//
				// If the initial fact is x+d >= w instead, the derived conditions are:
				//    if min<max: min <= x AND x <= max
				//    if min>max: min <= x OR  x <= max
				//
