...
func prove(f *Func) {
	...
	for _, v := range findIndVar(f) {
		...
		// to use end on each iteration anymore. We compare against zero instead.
		...
		if !(start.Op == OpConst8 || start.Op == OpConst16 || start.Op == OpConst32 || start.Op == OpConst64) {
			// if the start is not a constant we would be winning nothing from inverting the loop
			continue
		}
