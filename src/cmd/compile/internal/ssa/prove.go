...
func prove(f *Func) {
	...
	for _, v := range findIndVar(f) {
		...
		if v.step < 0 {
			start, end = v.max, v.min
		}

		// rewrite to a downward counting loop checking against zero if the
		// loop body does not depends on ind or nxt.
		// This reduce pressure on the register allocator because we do not need
		// to use end on each iterations anymore. We compare against zero instead.
		...
		// while the original form keeps ind then nxt and max alive
		if nxt := v.nxt; ind.Uses == 2 && // used by comparison and next
			nxt.Uses == 1 { // used by induction
			if start.Op == OpConst8 || start.Op == OpConst16 || start.Op == OpConst32 || start.Op == OpConst64 {
				// only perform the rewrite if start is const else we are not really saving anything
				header := ind.Block
				...
				// replace end inside comparison by start
				...
				panic(fmt.Sprintf("unreachable, ind: %v, start: %v, end: %v, step: %d", ind, start, end, v.step))
			replacedEnd:

				// replace start inside induction by end
			...
			replacedStart:
				...
				// invert nxt
				if nxt.Args[0] != ind {
					// unlike additions substraction is not commutative so be sure we get it right
			...
			// given the loop values are not used but to loop there is not value to apply them as induction variables
			// let's not bog down the facts table
			continue
		}
