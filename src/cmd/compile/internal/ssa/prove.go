...
var (
	// TODO: handle reverse type. saves about 10k.
	reverse = [...]uint{0, 4, 2, 6, 1, 5, 3, 7}
		...
		case descend:
			...
			if parent != nil && parent.Kind == BlockIf && rangeMaskTable[parent.Control.Op] != 0 {
				...
				if ok { // comes from a restricting branch
					// parent modifies the range for (a0, a1).
					// saves the previous type mask.
			...
			if node.block.Kind == BlockIf {
				...
				if rm != 0 {
					...
					for t := typeMask(1); t <= tm; t <<= 1 {
						...
						m, has := mask[i]
						if has && rm&m == m {
							f.Logf("proved positive branch of %s in %s\n", node.block, f.Name)
							succ = 0
							break
						}
						if has && (full^rm)&m == m {
							f.Logf("proved negative branch of %s in %s\n", node.block, f.Name)
							succ = 1
							break
						}
					}
