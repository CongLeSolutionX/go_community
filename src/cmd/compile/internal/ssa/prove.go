...
// a pair of variables (v, w). Without a priori knowledge the
// mask is lt | eq | gt meaning v can be less, equal or
// greater than w. When the execution path branches the condition
// is `v op w` the set of relations is updated to exclude
// any relation not possible due to `v op w` being true (or false).
//
// E.g.
//
// r := relation(...)
...
// and restoring factsTable in order to undo a branch.
var checkpointFact = fact{}

func newFactsTable() *factsTable {
	ft := &factsTable{}
	ft.facts = make(map[pair]relation)
	ft.stack = make([]fact, 0)
	return ft
}
...
// Called called on backing up on a branch.
...
func updateRestrictions(ft *factsTable, t domain, v, w *Value, r relation, branch int) {
	if t == 0 || branch == -1 {
...
func simplifyBlock(ft *factsTable, b *Block) int {
	...
	tr, has := domainRelationTable[b.Control.Op]
	if !has {
		return -1
	}
	...
	c := b.Control
	if (c.Op == OpIsInBounds || c.Op == OpIsSliceInBounds) && isNonNegative(c.Args[0]) && isNonNegative(c.Args[1]) {
		m := ft.get(a0, a1, signed)
		if m != 0 && tr.r&m == m {
			if b.Func.pass.debug > 0 {
				b.Func.Config.Warnl(int(b.Line), "Proved constant bounds %s", c.Op)
			}
			return 0
		}
	}
