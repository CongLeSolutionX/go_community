...
func simplifyBlock(ft *factsTable, b *Block) branch {
	...
	// Assumes c.Args[1] is positive.
	// If c.Args[1] is negative then c must be the first slice bounds check
	// from arr[0:j], i.e. c tests for 0 <= 0 <= j. It's fine to drop c because
	// the second slice check, 0 <= j <= cap(arr) will fail.
	if c.Op == OpIsSliceInBounds && c.Args[0].Op == OpConst64 && c.Args[0].AuxInt == 0 {
		if b.Func.pass.debug > 0 {
