...
func rewriteValuePPC64_OpMove(v *Value, config *Config) bool {
	...
	// result: (MOVWstore [2] dst (MOVWZload [4] src mem) 		(MOVWstore dst (MOVWZload src mem) mem))
	for {
		s := v.AuxInt
		dst := v.Args[0]
		src := v.Args[1]
	...
	// result: (MOVHstore [3] dst (MOVHZload [6] src mem) 		(MOVHstore [2] dst (MOVHZload [4] src mem) 			(MOVHstore [1] dst (MOVHZload [2] src mem) 				(MOVHstore dst (MOVHZload src mem) mem))))
	for {
		s := v.AuxInt
		dst := v.Args[0]
		src := v.Args[1]
	...
	// result: (LoweredMove [SizeAndAlign(s).Align()] 		(ADDconst <src.Type> dst [-moveSize(SizeAndAlign(s).Align(), config)]) 		(ADDconst <src.Type> src [-moveSize(SizeAndAlign(s).Align(), config)]) 		(ADDconst <src.Type> src [SizeAndAlign(s).Size()-moveSize(SizeAndAlign(s).Align(), config)]) 		mem)
	for {
		s := v.AuxInt
		dst := v.Args[0]
		src := v.Args[1]
		...
		v0.AddArg(dst)
		v0.AuxInt = -moveSize(SizeAndAlign(s).Align(), config)
		v.AddArg(v0)
		v1 := b.NewValue0(v.Line, OpPPC64ADDconst, src.Type)
		v1.AddArg(src)
		v1.AuxInt = -moveSize(SizeAndAlign(s).Align(), config)
		v.AddArg(v1)
		v2 := b.NewValue0(v.Line, OpPPC64ADDconst, src.Type)
		v2.AddArg(src)
		v2.AuxInt = SizeAndAlign(s).Size() - moveSize(SizeAndAlign(s).Align(), config)
		v.AddArg(v2)
		v.AddArg(mem)
		return true
	}
...
func rewriteValuePPC64_OpZero(v *Value, config *Config) bool {
	...
	// cond: (SizeAndAlign(s).Size() > 512 || config.noDuffDevice) || SizeAndAlign(s).Align()%4 != 0
	// result: (LoweredZero [SizeAndAlign(s).Align()] 		(ADDconst <ptr.Type> ptr [-moveSize(SizeAndAlign(s).Align(), config)]) 		(ADDconst <ptr.Type> ptr [SizeAndAlign(s).Size()-moveSize(SizeAndAlign(s).Align(), config)]) 		mem)
	for {
		s := v.AuxInt
		ptr := v.Args[0]
		mem := v.Args[1]
		...
		v0.AuxInt = -moveSize(SizeAndAlign(s).Align(), config)
		v.AddArg(v0)
		v1 := b.NewValue0(v.Line, OpPPC64ADDconst, ptr.Type)
		v1.AddArg(ptr)
