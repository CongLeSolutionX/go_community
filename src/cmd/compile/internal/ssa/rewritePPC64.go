...
func rewriteValuePPC64_OpStore(v *Value) bool {
	...
	// match: (Store {t} ptr val mem)
	// cond: t.Size() == 8 && !is64BitFloat(val.Type) && val.Type.Alignment()%8 == 0
	...
	for {
		...
		val := v_1
		mem := v_2
		if !(t.Size() == 8 && !is64BitFloat(val.Type) && val.Type.Alignment()%8 == 0) {
			break
		}
		v.reset(OpPPC64MOVDstore)
		v.AddArg3(ptr, val, mem)
	...
	// match: (Store {t} ptr val mem)
	// cond: t.Size() == 4 && !is32BitFloat(val.Type) && val.Type.Alignment()%4 == 0
	...
	for {
		...
		val := v_1
		mem := v_2
		if !(t.Size() == 4 && !is32BitFloat(val.Type) && val.Type.Alignment()%4 == 0) {
			break
		}
		v.reset(OpPPC64MOVWstore)
		v.AddArg3(ptr, val, mem)
