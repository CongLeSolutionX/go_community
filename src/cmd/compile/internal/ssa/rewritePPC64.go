...
func rewriteValuePPC64_OpPPC64OR_50(v *Value) bool {
	...
	// result: @mergePoint(b,x0,x1,x2) (SLDconst <t> (MOVWBRload <t> {s} (MOVDaddr <typ.Uintptr> [i0] {s} p) mem) [32])
	for {
		t := v.Type
		_ = v.Args[1]
		s2 := v.Args[0]
...
func rewriteValuePPC64_OpPPC64OR_60(v *Value) bool {
	...
	// result: @mergePoint(b,x0,x1,x2) (SLDconst <t> (MOVWBRload <t> {s} p mem) [32])
	for {
		t := v.Type
		_ = v.Args[1]
		s2 := v.Args[0]
