...
func rewriteValuePPC64_OpPPC64MOVBstore_0(v *Value) bool {
	...
	// match: (MOVBstore [i3] {s} p (SRWconst w [24]) x0:(MOVBstore [i2] {s} p (SRWconst w [16]) x1:(MOVHstore [i0] {s} p w mem)))
	// cond: !config.BigEndian 	&& x0.Uses == 1 && x1.Uses == 1 	&& i2 == i0+2 && i3 == i0+3 	&& clobber(x0) && clobber(x1)
	// result: (MOVWstore [i0] {s} p w mem)
	for {
		i3 := v.AuxInt
		s := v.Aux
		...
		mem := x1.Args[2]
		if !(!config.BigEndian && x0.Uses == 1 && x1.Uses == 1 && i2 == i0+2 && i3 == i0+3 && clobber(x0) && clobber(x1)) {
			break
		}
		v.reset(OpPPC64MOVWstore)
		v.AuxInt = i0
