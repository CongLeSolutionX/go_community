...
func rewriteValueS390X_OpS390XRISBGZ(v *Value) bool {
	...
	// result: (RISBGZ x {(*r.InMerge(^uint64(0)<<c)).Rotate(c)})
	for {
		r := auxToS390xRotateParams(v.Aux)
		if v_0.Op != OpS390XSLDconst {
			break
		...
		v.reset(OpS390XRISBGZ)
		v.Aux = s390xRotateParamsToAux((*r.InMerge(^uint64(0) << c)).Rotate(c))
		v.AddArg(x)
		return true
	}
	...
	// result: (RISBGZ x {(*z.InMerge(y.OutMask())).Rotate(y.Amount)})
	for {
		...
		v.reset(OpS390XRISBGZ)
		v.Aux = s390xRotateParamsToAux((*z.InMerge(y.OutMask())).Rotate(y.Amount))
		v.AddArg(x)
		return true
	}
	// match: (RISBGZ x {r})
	...
	// match: (RISBGZ x {r})
	// cond: r.Start == 0 && r.End == 63-(r.Amount&63)
	// result: (SLDconst x [r.Amount&63])
	for {
		r := auxToS390xRotateParams(v.Aux)
		x := v_0
		if !(r.Start == 0 && r.End == 63-(r.Amount&63)) {
			break
		}
...
func rewriteValueS390X_OpS390XSLDconst(v *Value) bool {
	...
	// result: (RISBGZ x {(*s390x.NewRotateParams(0, 63-c, c).InMerge(r.OutMask())).Rotate(r.Amount)})
	for {
		...
		v.reset(OpS390XRISBGZ)
		v.Aux = s390xRotateParamsToAux((*s390x.NewRotateParams(0, 63-c, c).InMerge(r.OutMask())).Rotate(r.Amount))
		v.AddArg(x)
		return true
	}
	// match: (SLDconst x [0])
...
func rewriteValueS390X_OpS390XSRDconst(v *Value) bool {
	...
	// result: (RISBGZ x {(*s390x.NewRotateParams(c, 63, -c&63).InMerge(r.OutMask())).Rotate(r.Amount)})
	for {
		c := auxIntToInt8(v.AuxInt)
		if v_0.Op != OpS390XRISBGZ {
			break
		...
		v.reset(OpS390XRISBGZ)
		v.Aux = s390xRotateParamsToAux((*s390x.NewRotateParams(c, 63, -c&63).InMerge(r.OutMask())).Rotate(r.Amount))
		v.AddArg(x)
		return true
	}
	// match: (SRDconst x [0])
