...
func (f *Func) dominatorsLTOrig(entry *Block, predFn linkedBlocks, succFn linkedBlocks) []*Block {
	...
	fromID, n := f.dfsOrig(entry, succFn, semi, vertex, label, parent)
	...
	for i := maxBlockID - 1; i > 0; i-- {
		w := vertex[i]
		if w == 0 {
			// Some blocks might be unreachable.
			continue
		}
...
func (f *Func) dfsOrig(entry *Block, succFn linkedBlocks, semi, vertex, label, parent []ID) (fromID []*Block, n ID) {
	maxBlockID := entry.Func.NumBlocks()
