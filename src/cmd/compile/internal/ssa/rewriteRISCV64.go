...
func rewriteValueRISCV64_OpMove(v *Value) bool {
	...
	// cond: s%8 == 0 && s > 32 && s <= 8*128 && t.Alignment()%8 == 0 && !config.noDuffDevice && logLargeCopy(v, s)
	...
	for {
		...
		src := v_1
		mem := v_2
		if !(s%8 == 0 && s > 32 && s <= 8*128 && t.Alignment()%8 == 0 && !config.noDuffDevice && logLargeCopy(v, s)) {
			break
		}
...
func rewriteValueRISCV64_OpZero(v *Value) bool {
	...
	// cond: s%8 == 0 && s > 32 && s <= 8*128 && t.Alignment()%8 == 0 && !config.noDuffDevice
	...
	for {
		...
		ptr := v_0
		mem := v_1
		if !(s%8 == 0 && s > 32 && s <= 8*128 && t.Alignment()%8 == 0 && !config.noDuffDevice) {
			break
		}
