...
func rewriteValueMIPS64_OpMIPS64MOVDstore(v *Value) bool {
	...
	b := v.Block
	config := b.Func.Config
	// match: (MOVDstore [off] {sym} ptr (MOVDgpfp val) mem)
	// result: (MOVVstore [off] {sym} ptr val mem)
	for {
		off := auxIntToInt32(v.AuxInt)
		sym := auxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpMIPS64MOVDgpfp {
			break
		}
		val := v_1.Args[0]
		mem := v_2
...
func rewriteValueMIPS64_OpMIPS64MOVFstore(v *Value) bool {
	...
	b := v.Block
	config := b.Func.Config
	// match: (MOVFstore [off] {sym} ptr (MOVFgpfp val) mem)
	// result: (MOVWstore [off] {sym} ptr val mem)
	for {
		off := auxIntToInt32(v.AuxInt)
		sym := auxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpMIPS64MOVFgpfp {
			break
		}
		val := v_1.Args[0]
		mem := v_2
...
func rewriteValueMIPS64_OpMIPS64MOVVstore(v *Value) bool {
	...
	b := v.Block
	config := b.Func.Config
	// match: (MOVVstore [off] {sym} ptr (MOVDfpgp val) mem)
	// result: (MOVDstore [off] {sym} ptr val mem)
	for {
		off := auxIntToInt32(v.AuxInt)
		sym := auxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpMIPS64MOVDfpgp {
			break
		}
		val := v_1.Args[0]
		mem := v_2
...
func rewriteValueMIPS64_OpMIPS64MOVWstore(v *Value) bool {
	...
	b := v.Block
	config := b.Func.Config
	// match: (MOVWstore [off] {sym} ptr (MOVFfpgp val) mem)
	// result: (MOVFstore [off] {sym} ptr val mem)
	for {
		off := auxIntToInt32(v.AuxInt)
		sym := auxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpMIPS64MOVFfpgp {
			break
		}
		val := v_1.Args[0]
		mem := v_2
