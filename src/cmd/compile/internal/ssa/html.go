...
<p>
<b>CFG</b>: Solid/dashed edges represent likely/unlikely branches. Blue is for backward edges. Red edges connect laid out ordered blocks. Light red edge indicated the order of layout for disconnected blocks.
</p>
...
func (d *dotWriter) writeFuncSVG(w io.Writer, phase string, f *Func) {
	...
	if f.laidout {
		fmt.Fprintln(pipe, `edge[constraint=false,color="#ffc2b3",style=solid];`)
		for i := 1; i < len(f.Blocks); i++ {
			if layoutDrawn[f.Blocks[i].ID] {
				continue
			}
			fmt.Fprintf(pipe, `%s -> %s;`, f.Blocks[i-1], f.Blocks[i])
		}
	}
	fmt.Fprint(pipe, "}")
	pipe.Close()
