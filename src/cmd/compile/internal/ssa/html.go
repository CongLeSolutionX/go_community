...
<p>
<b>CFG</b>: Solid/dashed edges represent likely/unlikely branches. Red edges connect laid out ordered blocks.  Blue is for back edges.
</p>
...
func (d *dotWriter) writeFuncSVG(w io.Writer, phase string, f *Func) {
	...
	fmt.Fprint(pipe, `digraph "" { margin=0; size="4,40"; ranksep=.2; ratio=compress; `)
	id := strings.Replace(phase, " ", "-", -1)
	...
	for i, b := range f.Blocks {
		...
		fmt.Fprintf(pipe, `%v [label="%v%s\n%v",id="graph_node_%v_%v",tooltip="%v",style=filled,fillcolor=white,fontsize=16,fontname="Menlo,Times,serif",margin="0.01,0.03"];`, b, b, layout, b.Kind, id, b, b.LongString())
	}
	...
	for _, b := range f.Blocks {
		for i, s := range b.Succs {
			...
			fmt.Fprintf(pipe, `%v -> %v [label=" %d ",style="%s",color="%s",fontsize=16,fontname="Menlo,Times,serif"];`, b, s.b, i, style, color)
		}
	}
	if f.laidout {
		...
		for i := 1; i < len(f.Blocks); i++ {
			...
			fmt.Fprintf(pipe, `%s -> %s [color=blue];`, f.Blocks[i-1], f.Blocks[i])
		}
	}
	fmt.Fprint(pipe, "}")
	pipe.Close()
	...
	svgID := "svg_graph_" + id
	fmt.Fprintf(w, `<div class="zoom"><a href="#" onclick="graphReduce('%s');">-</a> <a href="#" onclick="graphEnlarge('%s');">+</a></div>`, svgID, svgID)
