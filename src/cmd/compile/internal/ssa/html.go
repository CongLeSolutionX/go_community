...
`)
	...
	svgID := "svg_graph_" + id
	io.WriteString(w, `<div class="zoom"><a href="#" onclick="graphReduce('`+svgID+`');">-</a> <a href="#" onclick="graphEnlarge('`+svgID+`');">+</a></div>`)
	d.copyAfter(w, buf, `<svg `)
	io.WriteString(w, `id="`+svgID+`" onload="makeDraggable(evt)" `)
