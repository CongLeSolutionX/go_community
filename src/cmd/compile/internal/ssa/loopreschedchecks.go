...
func insertLoopReschedChecks(f *Func) {
	...
	for _, emc := range tofixBackedges {
		...
		offset := test.NewValue0I(bb.Pos, constOp, pt, 2*pt.Size()) // g.stackguard0
		limaddr := test.NewValue2(bb.Pos, OpAddPtr, pt, g, offset)
		lim := test.NewValue2(bb.Pos, OpLoad, pt, limaddr, mem0)
		cmp := test.NewValue2(bb.Pos, cmpOp, f.Config.fe.TypeBool(), sp, lim)
		test.SetControl(cmp)

		test.AddEdgeTo(sched)                       // if true
		test.Succs = append(test.Succs, Edge{h, i}) // if false (rewrite h end of edge)
