...
var genericOps = []opData{
	...
	// Before lowering, LECalls receive their fixed inputs (first), memory (last), and a variable number of input values.
	// They produce a variable number of result values.
	// After lowering, Calls have the same fixed-middle-memory arrangement of inputsm, with the difference that the "middle" is only the register-resident inputs,
	// and the non-register inputs are instead stored at ABI-defined offsets from SP (and the stores thread through the memory that is ultimately an input to the call).
	// After lowering, Call results are transformed in a similar way, so that the variable number of outputs corresponds to registers, and those outputs that do not
	// fit in registers are instead stored to memory by the called function or method.
