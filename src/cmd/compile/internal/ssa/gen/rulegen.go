...
func genMatch(rr *RuleRewrite, arch arch, match string) (pos, checkOp string) {
	...
	// TODO: consider always populating cnt.
	// That would mean failing to match some commutative rules
	// in which the critical piece is not the match but the cond.
	// As of writing, there are currently no such instances,
	// at least that fire compiling std+cmd.
	if rr.cond == "" {
		cnt = varCount(rr.match)
	}
...
func genMatch0(rr *RuleRewrite, arch arch, match, v string, cnt map[string]int) (pos, checkOp string) {
	...
	if commutative {
		...
		if cnt != nil && cnt[args[0]] == 1 && cnt[args[1]] == 1 {
			commutative = false
		}
	}
