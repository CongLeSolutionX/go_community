...
func init() {
	...
	ops := []opData{
		...
		// arg0 = address of memory to zero, minus element size (in R3, changed as side effect)
		...
		//	CMP	R3, Rarg1
		//	BLE	-2(PC)
		// NOTE: It would be bad for the pre-adjusted address to get hoisted before a memory allocation,
		// because it is not a valid pointer.  Because it is clobbered a "good" optimizer ought not do that.
		{
			name:      "LoweredZero",
			aux:       "Int64",
			argLength: 3,
			reg: regInfo{
				inputs:   []regMask{buildReg("R3"), gp, gp},
				clobbers: buildReg("R3 CR"),
			},
		},

		// large or unaligned move
		// arg0 = address of dst memory, minus element size (in R3, changed as side effect)
		// arg1 = address of src memory, minus element size (in R4, changed as side effect)
		...
		//	CMP	R4, Rarg2
		//	BLT	-3(PC)
		// NOTE: It would be bad for the pre-adjusted address(es) to get hoisted before a memory allocation,
		// because it is not a valid pointer.  Because it is clobbered a "good" optimizer ought not do that.
		{
			name:      "LoweredMove",
			aux:       "Int64",
