...
func buildReg(s string) regMask {
	if len(AMD64Regname) > 64 {
		panic("too many registers")
	}
	num := map[string]int{}
	for i, name := range AMD64Regname {
		if name[0] != '.' {
...
func init() {
	...
	gpsp := buildReg("AX CX DX BX BP SI DI R8 R9 R10 R11 R12 R13 R14 R15 SP FP")
	...
	var AMD64ops = []opData{
		...
		g{name: "MOVBload", reg: gpload},         // load byte from arg0+aux.(int64). arg1=mem
