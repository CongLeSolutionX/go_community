...
func init() {
	...
	var AMD64ops = []opData{
		...
		// SSE 4.1 has instructions for these, but we only assume SSE 2 so we can't use them.
		// Instead, these are implemented as two moves, reg->mem->reg.
		// Hence the use of scratch space.
		// TODO: should we have generic versions of these?
		{name: "MOVQi2f", argLength: 1, reg: gpfp, usesScratch: true}, // move 64 bits from int to float reg
		{name: "MOVQf2i", argLength: 1, reg: fpgp, usesScratch: true}, // move 64 bits from float to int reg
		{name: "MOVLi2f", argLength: 1, reg: gpfp, usesScratch: true}, // move 32 bits from int to float reg
		{name: "MOVLf2i", argLength: 1, reg: fpgp, usesScratch: true}, // move 32 bits from float to int reg
