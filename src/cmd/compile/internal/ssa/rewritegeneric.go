...
func rewriteValuegeneric_OpCeil(v *Value) bool {
	...
	// cond: ceilF(c) == ceilF(c)
	// result: (Const64F [ceilF(c)])
	for {
		if v_0.Op != OpConst64F {
			break
		}
		...
		v.reset(OpConst64F)
		v.AuxInt = float64ToAuxInt(ceilF(c))
		return true
	}
	return false
}
...
func rewriteValuegeneric_OpFloor(v *Value) bool {
	...
	// cond: floorF(c) == floorF(c)
	// result: (Const64F [floorF(c)])
	for {
		if v_0.Op != OpConst64F {
			break
		}
		c := auxIntToFloat64(v_0.AuxInt)
		if !(floorF(c) == floorF(c)) {
			break
		}
		v.reset(OpConst64F)
		v.AuxInt = float64ToAuxInt(floorF(c))
		return true
	}
	return false
}
...
func rewriteValuegeneric_OpRoundToEven(v *Value) bool {
	...
	// cond: roundToEvenF(c) == roundToEvenF(c)
	// result: (Const64F [roundToEvenF(c)])
	for {
		if v_0.Op != OpConst64F {
			break
		}
		c := auxIntToFloat64(v_0.AuxInt)
		if !(roundToEvenF(c) == roundToEvenF(c)) {
			break
		}
		v.reset(OpConst64F)
		v.AuxInt = float64ToAuxInt(roundToEvenF(c))
		return true
	}
	return false
}
...
func rewriteValuegeneric_OpTrunc(v *Value) bool {
	...
	// cond: truncF(c) == truncF(c)
	// result: (Const64F [truncF(c)])
	for {
		if v_0.Op != OpConst64F {
			break
		}
		c := auxIntToFloat64(v_0.AuxInt)
		if !(truncF(c) == truncF(c)) {
			break
		}
		v.reset(OpConst64F)
		v.AuxInt = float64ToAuxInt(truncF(c))
		return true
	}
	return false
}
