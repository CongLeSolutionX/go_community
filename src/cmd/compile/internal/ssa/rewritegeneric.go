...
func rewriteValuegeneric(v *Value, config *Config) bool {
	...
	case OpArrayIndex:
		...
		// result: (Load (PtrIndex <ptr.Type.Elem().Elem().PtrTo()> ptr idx) mem)
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpPtrIndex, TypeInvalid, 0, nil)
	...
	case OpConst:
		...
		// result: (StringMake (OffPtr <TypeBytePtr> [2*config.ptrSize] (Global <TypeBytePtr> {config.fe.StringSym(s.(string))})) (Const <config.Uintptr> [int64(len(s.(string)))]))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpOffPtr, TypeInvalid, 0, nil)
			v0.Type = TypeBytePtr
			v0.AuxInt = 2 * config.ptrSize
			v1 := v.Block.NewValue(v.Line, OpGlobal, TypeInvalid, 0, nil)
			v1.Type = TypeBytePtr
	...
	case OpLoad:
		...
		// result: (StringMake (Load <TypeBytePtr> ptr mem) (Load <config.Uintptr> (OffPtr <TypeBytePtr> [config.ptrSize] ptr) mem))
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpLoad, TypeInvalid, 0, nil)
			v0.Type = TypeBytePtr
			v0.AddArg(ptr)
			v0.AddArg(mem)
			v.AddArg(v0)
			v1 := v.Block.NewValue(v.Line, OpLoad, TypeInvalid, 0, nil)
			v1.Type = config.Uintptr
			v2 := v.Block.NewValue(v.Line, OpOffPtr, TypeInvalid, 0, nil)
			v2.Type = TypeBytePtr
	...
	case OpPtrIndex:
		...
		// result: (Add ptr (Mul <config.Uintptr> idx (Const <config.Uintptr> [t.Elem().Size()])))
		{
			...
			v.resetArgs()
			v.AddArg(ptr)
			v0 := v.Block.NewValue(v.Line, OpMul, TypeInvalid, 0, nil)
			v0.Type = config.Uintptr
			v0.AddArg(idx)
			v1 := v.Block.NewValue(v.Line, OpConst, TypeInvalid, 0, nil)
			v1.Type = config.Uintptr
	...
	case OpSliceCap:
		...
		// result: (Load (Add <ptr.Type> ptr (Const <config.Uintptr> [config.ptrSize*2])) mem)
		{
			...
			v.Aux = nil
			v.resetArgs()
			v0 := v.Block.NewValue(v.Line, OpAdd, TypeInvalid, 0, nil)
			v0.Type = ptr.Type
			v0.AddArg(ptr)
			v1 := v.Block.NewValue(v.Line, OpConst, TypeInvalid, 0, nil)
			v1.Type = config.Uintptr
	...
	end324ffb6d2771808da4267f62c854e9c8:
		...
		// result: (Store (OffPtr <TypeBytePtr> [config.ptrSize] dst) (StringLen <config.Uintptr> str) (Store <TypeMem> dst (StringPtr <TypeBytePtr> str) mem))
		{
			...
			v0.AddArg(dst)
			v.AddArg(v0)
			v1 := v.Block.NewValue(v.Line, OpStringLen, TypeInvalid, 0, nil)
			v1.Type = config.Uintptr
			v1.AddArg(str)
			v.AddArg(v1)
			v2 := v.Block.NewValue(v.Line, OpStore, TypeInvalid, 0, nil)
			v2.Type = TypeMem
			v2.AddArg(dst)
			v3 := v.Block.NewValue(v.Line, OpStringPtr, TypeInvalid, 0, nil)
			v3.Type = TypeBytePtr
			v3.AddArg(str)
			v2.AddArg(v3)
			v2.AddArg(mem)
