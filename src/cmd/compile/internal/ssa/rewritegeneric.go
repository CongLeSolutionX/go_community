...
func rewriteValuegeneric_OpMod64u(v *Value) bool {
	...
	// cond: (config.RegSize == 4 || v.Block.Func.pass.name != "opt") && x.Op != OpConst64 && c > 0 && umagicOK64(c)
	...
	for {
		...
		c := auxIntToInt64(v_1.AuxInt)
		if !((config.RegSize == 4 || v.Block.Func.pass.name != "opt") && x.Op != OpConst64 && c > 0 && umagicOK64(c)) {
			break
		}
		v.reset(OpSub64)
		v0 := b.NewValue0(v.Pos, OpMul64, t)
