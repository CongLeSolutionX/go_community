...
func rewriteValuegeneric_OpLoad(v *Value) bool {
	...
	// match: (Load <t> _ _)
	// cond: t.IsStruct() && fe.CanSSA(t)
	// result: rewriteLoadStruct(v)
	for {
		t := v.Type
		if !(t.IsStruct() && fe.CanSSA(t)) {
			break
		}
		v.copyOf(rewriteLoadStruct(v))
		return true
	}
	// match: (Load <t> _ _)
...
func rewriteValuegeneric_OpStore(v *Value) bool {
	...
	// result: rewriteStoreStruct(v)
	for {
		if v_1.Op != OpStructMake {
			break
		}
		v.copyOf(rewriteStoreStruct(v))
		return true
	}
