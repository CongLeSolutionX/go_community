...
func rewriteValuegeneric_OpLeq16(v *Value) bool {
	...
	// match: (Leq16 c:(Const16 [n]) _)
	// cond: int64(n) == minValueForInt(c.Type)
	// result: (ConstBool [true])
	for {
		c := v_0
		if c.Op != OpConst16 {
		...
		if !(int64(n) == minValueForInt(c.Type)) {
			break
		}
		v.reset(OpConstBool)
		v.AuxInt = boolToAuxInt(true)
	...
	// match: (Leq16 _ c:(Const16 [n]))
	// cond: int64(n) == maxValueForInt(c.Type)
	// result: (ConstBool [true])
	for {
		c := v_1
		if c.Op != OpConst16 {
		...
		n := auxIntToInt16(c.AuxInt)
		if !(int64(n) == maxValueForInt(c.Type)) {
			break
		}
		v.reset(OpConstBool)
		v.AuxInt = boolToAuxInt(true)
