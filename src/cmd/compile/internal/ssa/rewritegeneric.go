...
func rewriteValuegeneric_OpSelectN(v *Value) bool {
	...
	// cond: sz >= 0 && call.Uses == 1 && symNamed(sym, "runtime.memmove") && dst.Type.IsPtr() && isInlinableMemmove(dst, src, int64(sz), config) && clobber(call)
	...
	for {
		...
		if !(sz >= 0 && call.Uses == 1 && symNamed(sym, "runtime.memmove") && dst.Type.IsPtr() && isInlinableMemmove(dst, src, int64(sz), config) && clobber(call)) {
			break
		}
		v.reset(OpMove)
...
func rewriteValuegeneric_OpStaticLECall(v *Value) bool {
	...
	// cond: isSameCall(callAux, "runtime.memequal") && symHasPrefix(scon, "go.string.")
	...
	for {
		...
		v_2 := v.Args[2]
		if v_2.Op != OpConst64 || auxIntToInt64(v_2.AuxInt) != 1 || !(isSameCall(callAux, "runtime.memequal") && symHasPrefix(scon, "go.string.")) {
			break
		}
