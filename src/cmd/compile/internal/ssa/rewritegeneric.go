...
func rewriteValuegeneric_OpNilCheck(v *Value, config *Config) bool {
	...
	// cond: mem.Op == OpStaticCall 	&& isSameSym(mem.Aux, "runtime.newobject") 	&& c == config.ctxt.FixedFrameSize() + config.PtrSize 	&& warnRule(config.Debug_checknil() && int(v.Line) > 1, v, "removed nil check")
	// result: (Invalid)
	for {
		v_0 := v.Args[0]
		if v_0.Op != OpLoad {
		...
		if !(mem.Op == OpStaticCall && isSameSym(mem.Aux, "runtime.newobject") && c == config.ctxt.FixedFrameSize()+config.PtrSize && warnRule(config.Debug_checknil() && int(v.Line) > 1, v, "removed nil check")) {
			break
		}
		v.reset(OpInvalid)
		return true
