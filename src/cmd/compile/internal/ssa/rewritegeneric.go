...
func rewriteValuegeneric_OpIsInBounds(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (IsInBounds (ZeroExt8to16 _) (Const64 [c]))
	// cond: math.MaxUint8 <= c
	// result: (ConstBool [1])
	for {
		if v.Args[0].Op != OpZeroExt8to16 {
			break
		}
		if v.Args[1].Op != OpConst64 {
			break
	...
	// match: (IsInBounds (ZeroExt8to32 _) (Const64 [c]))
	// cond: math.MaxUint8 <= c
	// result: (ConstBool [1])
	for {
	...
	// match: (IsInBounds (ZeroExt16to32 _) (Const64 [c]))
	// cond: math.MaxUint16 <= c
	// result: (ConstBool [1])
	for {
