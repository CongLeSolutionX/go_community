...
func rewriteValueARM(v *Value, config *Config) bool {
	...
	case OpARMSMULABB:
		return rewriteValueARM_OpARMSMULABB(v, config)
	case OpARMSMULBB:
		return rewriteValueARM_OpARMSMULBB(v, config)
	case OpARMSRA:
...
func rewriteValueARM_OpARMADD(v *Value, config *Config) bool {
	...
	// match: (ADD (SMULBB x y) a)
	// cond:
	// result: (SMULABB x y a)
	for {
		v_0 := v.Args[0]
		if v_0.Op != OpARMSMULBB {
			break
		}
		x := v_0.Args[0]
		y := v_0.Args[1]
		a := v.Args[1]
		v.reset(OpARMSMULABB)
		v.AddArg(x)
		v.AddArg(y)
		v.AddArg(a)
		return true
	}
	// match: (ADD a (SMULBB x y))
	// cond:
	// result: (SMULABB x y a)
	for {
		a := v.Args[0]
		v_1 := v.Args[1]
		if v_1.Op != OpARMSMULBB {
			break
		}
		x := v_1.Args[0]
		y := v_1.Args[1]
...
func rewriteValueARM_OpARMSMULABB(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (SMULABB x (MOVWconst [c]) a)
	// cond: int16(c) == -1
	// result: (SUB a x)
	for {
		x := v.Args[0]
...
func rewriteValueARM_OpARMSMULBB(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (SMULBB x (MOVWconst [c]))
	// cond: int16(c) == -1
	// result: (RSBconst [0] x)
	for {
		x := v.Args[0]
