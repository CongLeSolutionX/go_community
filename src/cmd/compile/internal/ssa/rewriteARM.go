...
func rewriteValueARM_OpMove(v *Value, config *Config) bool {
	...
	// match: (Move [s] dst src mem)
	// cond: SizeAndAlign(s).Size() > 4 && SizeAndAlign(s).Align()%4 != 0
	// result: (LoweredMove1 dst src (ADDconst <src.Type> src [SizeAndAlign(s).Size()]) mem)
	for {
		s := v.AuxInt
		dst := v.Args[0]
		src := v.Args[1]
...
func rewriteValueARM_OpZero(v *Value, config *Config) bool {
	...
	// match: (Zero [s] ptr mem)
	// cond: SizeAndAlign(s).Size() > 4 && SizeAndAlign(s).Align()%4 != 0
	// result: (LoweredZero1 ptr (ADDconst <ptr.Type> ptr [SizeAndAlign(s).Size()]) (MOVWconst [0]) mem)
	for {
		s := v.AuxInt
		ptr := v.Args[0]
		mem := v.Args[1]
