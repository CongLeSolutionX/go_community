...
func rewriteValueARM(v *Value, config *Config) bool {
	...
	case OpCarry:
		return rewriteValueARM_OpCarry(v, config)
	case OpClosureCall:
	...
	case OpHi32:
		return rewriteValueARM_OpHi32(v, config)
	case OpHmul16:
	...
	case OpLo32:
		return rewriteValueARM_OpLo32(v, config)
	case OpLoad:
	...
	case OpValue32:
		return rewriteValueARM_OpValue32(v, config)
	case OpXor16:
...
func rewriteValueARM_OpCarry(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Carry x)
	// cond:
	// result: (LoweredCarry x)
	for {
		x := v.Args[0]
		v.reset(OpARMLoweredCarry)
		v.AddArg(x)
		return true
	}
}
...
func rewriteValueARM_OpHi32(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Hi32 x)
	// cond:
	// result: (LoweredHi32 x)
	for {
		x := v.Args[0]
		v.reset(OpARMLoweredHi32)
		v.AddArg(x)
		return true
	}
}
...
func rewriteValueARM_OpLo32(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Lo32 x)
	// cond:
	// result: (LoweredValue32 x)
	for {
		x := v.Args[0]
		v.reset(OpARMLoweredValue32)
		v.AddArg(x)
		return true
	}
}
...
func rewriteValueARM_OpValue32(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (Value32 x)
	// cond:
