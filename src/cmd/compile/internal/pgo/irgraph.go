...
func New(profileFile string) *Profile {
	...
	g, _ := newGraph(profile, &Options{
		CallTree:    false,
		ObjNames:    true, // Include StartLine.
...
func (p *Profile) VisitIR(fn *ir.Func, recursive bool) {
	...
	// Create the key for the nodeMap.
	nodeinfo := nodeMapKey{
		CallerName:     name,
		CalleeName:     "",
		CallSiteOffset: 0,
...
func (p *Profile) EdgeWeight(caller, callee string, lineOffset int) (float64, bool) {
	k := nodeMapKey{
		CallerName:     caller,
		CalleeName:     callee,
		CallSiteOffset: lineOffset,
	}
	w, ok := p.nodeMap[k]
	if !ok {
		return 0, false
