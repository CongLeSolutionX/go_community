...
// Visit traverses the body of each ir.Func and use GlobalNodeMap to determine
// if we need to add an edge from ir.Func and any node in the ir.Func body.
...
// addEdge adds an edge between caller and new node that points to `callee`
// based on the profile-graph and GlobalNodeMap.
func (p *Profile) addEdge(caller *IRNode, callee *ir.Func, n *ir.Node, callername string, line int) {
	g := p.WeightedCG
...
func (p *Profile) createIRGraphEdge(fn *ir.Func, callernode *IRNode, name string) {
	...
	doNode = func(n ir.Node) bool {
		...
		case ir.OCALLFUNC:
			...
			f := inlCallee(call.X)
			if f != nil {
				p.addEdge(callernode, f, &n, name, line)
			}
		case ir.OCALLMETH:
			call := n.(*ir.CallExpr)
			...
			line := int(base.Ctxt.InnermostPos(n.Pos()).RelLine())
			p.addEdge(callernode, fn2, &n, name, line)
		}
		return false
	}
	doNode(fn)
