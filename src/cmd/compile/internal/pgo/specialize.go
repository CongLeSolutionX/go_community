...
func SpecializeIfaceCallSites(fn *ir.Func, p *Profile) {
	...
	edit = func(n ir.Node) ir.Node {
		...
		for _, edge := range p.WeightedCG.OutEdges[callerNode] {
		...
		default:
			// Don't care about builtin functions.
			// TODO: don't we?
			builtin = true
		}
		...
		f, weight := p.findHotConcreteCallee(fn, n)
		if f == nil {
		...
		stat.Specialized = ir.LinkFuncName(f)
