...
func EqStruct(t *types.Type, np, nq ir.Node) []ir.Node {
	...
	for i, fields := 0, t.FieldSlice(); i < len(fields); {
		...
		if size <= 16 {
			...
			byteptrx, byteptry := nodeUintPtr(nx), nodeUintPtr(ny)

			for offset := int64(0); offset < size; {
				offsetbyteptrx := ir.NewBinaryExpr(base.Pos, ir.OADD, byteptrx, ir.NewInt(offset))
				offsetbyteptry := ir.NewBinaryExpr(base.Pos, ir.OADD, byteptry, ir.NewInt(offset))
				...
				xptr := typecheck.ConvNop(offsetbyteptrx, types.NewPtr(types.Types[typ]))
				yptr := typecheck.ConvNop(offsetbyteptry, types.NewPtr(types.Types[typ]))

				and(ir.NewBinaryExpr(base.Pos, ir.OEQ, ir.NewStarExpr(base.Pos, xptr), ir.NewStarExpr(base.Pos, yptr)))
			}
		} else {
			// More than 16 bytes: use memequal.
...
func nodeUintPtr(n ir.Node) ir.Node {
	na := typecheck.NodAddr(n)
	return typecheck.ConvNop(na, types.Types[types.TUINTPTR])
}
