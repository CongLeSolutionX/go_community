...
func (s *state) emitOpenDeferInfo() {
	...
	for i := len(s.openDefers) - 1; i >= 0; i-- {
		...
		pri := s.f.Abi0.ABIAnalyzeFuncType(r.n.X.Type().FuncType())
		for j, arg := range r.argNodes {
			f := getParam(r.n, j)
			off = dvarint(x, off, -okOffset(arg.FrameOffset()))
			off = dvarint(x, off, f.Type.Size())
			off = dvarint(x, off, okOffset(pri.InParam(j).FrameOffset(pri)))
		}
	}
}
...
func (s *state) expr(n ir.Node) *ssa.Value {
	...
	case ir.ORESULT:
		...
		if s.prevCall == nil || s.prevCall.Op != ssa.OpStaticLECall && s.prevCall.Op != ssa.OpInterLECall && s.prevCall.Op != ssa.OpClosureLECall {
			// Do the old thing
			addr := s.constOffPtrSP(types.NewPtr(n.Type()), n.Index)
...
func (s *state) addr(n ir.Node) *ssa.Value {
	...
	case ir.ORESULT:
		...
		n := n.(*ir.ResultExpr)
		if s.prevCall == nil || s.prevCall.Op != ssa.OpStaticLECall && s.prevCall.Op != ssa.OpInterLECall && s.prevCall.Op != ssa.OpClosureLECall {
			return s.constOffPtrSP(t, n.Index)
		}
		which := n.Index
		if which == -1 {
...
func (s *state) rtcall(fn *obj.LSym, returns bool, results []*types.Type, args ...*ssa.Value) []*ssa.Value {
	...
	// Issue call
	var call *ssa.Value
	aux := ssa.StaticAuxCall(fn, ACArgs, ACResults, s.f.AbiDefault.ABIAnalyzeTypes(nil, callArgTypes, results)) // WILL NEED A TYPE FOR THIS.)
	callArgs = append(callArgs, s.mem())
	call = s.newValue0A(ssa.OpStaticLECall, aux.LateExpansionResultType(), aux)
	call.AddArgs(callArgs...)
	s.vars[memVar] = s.newValue1I(ssa.OpSelectN, types.TypeMem, int64(len(ACResults)), call)
