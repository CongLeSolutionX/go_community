...
func (s *state) checkPtrAlignment(n *ir.ConvExpr, v *ssa.Value, count *ssa.Value) {
	...
	if count.Type.Size() > s.config.PtrSize {
		s.Fatalf("expected count fit to a pointer size, have: %d, want: %d", count.Type.Size(), s.config.PtrSize)
	}
...
func (s *state) conv(n *ir.ConvExpr, v *ssa.Value, fromType, toType *types.Type) *ssa.Value {
	if fromType.IsBoolean() && toType.IsKind(types.TUINT8) {
		// Bool -> uint8 is generated internally when indexing into runtime.staticbyte.
		return s.newValue1(ssa.OpCopy, n.Type(), v)
	}
	if fromType.IsInteger() && toType.IsInteger() {
		var op ssa.Op
		if toType.Size() == fromType.Size() {
			op = ssa.OpCopy
		} else if toType.Size() < fromType.Size() {
			// truncation
			switch 10*fromType.Size() + toType.Size() {
			case 21:
				op = ssa.OpTrunc16to8
			case 41:
				op = ssa.OpTrunc32to8
			...
			default:
				s.Fatalf("weird integer truncation %v -> %v", fromType, toType)
			}
			...
			default:
				s.Fatalf("bad integer sign extension %v -> %v", fromType, toType)
			}
		} else {
			// zero extension
			switch 10*fromType.Size() + toType.Size() {
			...
			default:
				s.Fatalf("weird integer sign extension %v -> %v", fromType, toType)
			}
		}
	...
	if fromType.IsFloat() || toType.IsFloat() {
		conv, ok := fpConvOpToSSA[twoTypes{s.concreteEtype(fromType), s.concreteEtype(toType)}]
		if s.config.RegSize == 4 && Arch.LinkArch.Family != sys.MIPS && !s.softFloat {
			if conv1, ok1 := fpConvOpToSSA32[twoTypes{s.concreteEtype(fromType), s.concreteEtype(toType)}]; ok1 {
				conv = conv1
			}
		}
		if Arch.LinkArch.Family == sys.ARM64 || Arch.LinkArch.Family == sys.Wasm || Arch.LinkArch.Family == sys.S390X || s.softFloat {
			if conv1, ok1 := uint64fpConvOpToSSA[twoTypes{s.concreteEtype(fromType), s.concreteEtype(toType)}]; ok1 {
				conv = conv1
			}
		}

		if Arch.LinkArch.Family == sys.MIPS && !s.softFloat {
			if fromType.Size() == 4 && fromType.IsInteger() && !fromType.IsSigned() {
				// toType is float32 or float64, and fromType is also unsigned
				if toType.Size() == 4 {
					return s.uint32Tofloat32(n, v, fromType, toType)
				}
				if toType.Size() == 8 {
					return s.uint32Tofloat64(n, v, fromType, toType)
				}
			} else if toType.Size() == 4 && toType.IsInteger() && !toType.IsSigned() {
				// fromType is float32 or float64, and toType is unsigned integer
				if fromType.Size() == 4 {
					return s.float32ToUint32(n, v, fromType, toType)
				}
				if fromType.Size() == 8 {
					return s.float64ToUint32(n, v, fromType, toType)
				}
			}
		}

		if !ok {
			s.Fatalf("weird float conversion %v -> %v", fromType, toType)
		}
		...
		if op1 != ssa.OpInvalid && op2 != ssa.OpInvalid {
			...
			if op1 == ssa.OpCopy {
				...
				return s.newValueOrSfCall1(op2, n.Type(), v)
			}
			if op2 == ssa.OpCopy {
				return s.newValueOrSfCall1(op1, n.Type(), v)
			}
			return s.newValueOrSfCall1(op2, n.Type(), s.newValueOrSfCall1(op1, types.Types[it], v))
		}
		// Tricky 64-bit unsigned cases.
		if fromType.IsInteger() {
			// toType is float32 or float64, and fromType is also unsigned
			if toType.Size() == 4 {
				return s.uint64Tofloat32(n, v, fromType, toType)
			}
			if toType.Size() == 8 {
				return s.uint64Tofloat64(n, v, fromType, toType)
			}
			s.Fatalf("weird unsigned integer to float conversion %v -> %v", fromType, toType)
		}
		// fromType is float32 or float64, and toType is unsigned integer
		if fromType.Size() == 4 {
			return s.float32ToUint64(n, v, fromType, toType)
		}
		if fromType.Size() == 8 {
			return s.float64ToUint64(n, v, fromType, toType)
		}
		s.Fatalf("weird float to unsigned integer conversion %v -> %v", fromType, toType)
		return nil
	}

	if fromType.IsComplex() && toType.IsComplex() {
		var op ssa.Op
		if fromType.Size() == toType.Size() {
			...
			case 16:
				op = ssa.OpRound64F
			default:
				s.Fatalf("weird complex conversion %v -> %v", fromType, toType)
			}
		} else if fromType.Size() == 8 && toType.Size() == 16 {
			op = ssa.OpCvt32Fto64F
		} else if fromType.Size() == 16 && toType.Size() == 8 {
			op = ssa.OpCvt64Fto32F
		} else {
			s.Fatalf("weird complex conversion %v -> %v", fromType, toType)
		}
		ftp := types.FloatForComplex(fromType)
		ttp := types.FloatForComplex(toType)
		return s.newValue2(ssa.OpComplexMake, toType,
