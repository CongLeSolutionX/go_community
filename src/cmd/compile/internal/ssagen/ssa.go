...
func (s *state) conv(n ir.Node, v *ssa.Value, fromType, toType *types.Type) *ssa.Value {
	if fromType.IsBoolean() && toType.IsKind(types.TUINT8) {
		// Bool -> uint8 is generated internally when indexing into runtime.staticbyte.
		return s.newValue1(ssa.OpCopy, toType, v)
	}
	if fromType.IsInteger() && toType.IsInteger() {
		...
		return s.newValue1(op, toType, v)
	}
	...
	if fromType.IsFloat() || toType.IsFloat() {
		...
		if op1 != ssa.OpInvalid && op2 != ssa.OpInvalid {
			...
			if op1 == ssa.OpCopy {
				...
				return s.newValueOrSfCall1(op2, toType, v)
			}
			if op2 == ssa.OpCopy {
				return s.newValueOrSfCall1(op1, toType, v)
			}
			return s.newValueOrSfCall1(op2, toType, s.newValueOrSfCall1(op1, types.Types[it], v))
		}
	...
	s.Fatalf("unhandled OCONV %s -> %s", fromType.Kind(), toType.Kind())
	return nil
}
