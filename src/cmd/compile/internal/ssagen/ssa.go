...
func (s *state) newHeapaddr(n *ir.Name) {
	s.setHeapaddr(n, s.newObject(n.Type()))
}

// setHeapaddr allocates a new PAUTO variable to store ptr (which must be non-nil)
// and then sets it as n's heap address.
func (s *state) setHeapaddr(n *ir.Name, ptr *ssa.Value) {
	...
	addr := ir.NewNameAt(s.curfn.Pos(), &types.Sym{Name: "&" + n.Sym().Name, Pkg: types.LocalPkg})
