...
// +build ignore
//
// Rudimentary program for remastering ABU utilities unit test
// expected outputs, typically useful in situations where there are
// large numbers of golden outputs that have changed as a result of a
// difference in the output format.
//
// $ ABITEST_EMIT_REMASTER_SCRIPT=1 go test -test.v -test.run=TestABIUtils .
// <test output>
// ... emitting remaster inputs to file '/tmp/remaster-inputs.txt'
// $ go build remaster-abi-tests.go
// $ ./remaster-abi-tests /tmp/remaster-inputs.txt
//
...
func fixupFile(file string, fc FileChanges) {
	...
	// Process each change. In the unlikely (impossible?) case
	// that unit tests run more than once (meaning that we may
	// encounter multiple failures each at the same source line)
	// handle this by verifying that the new outputs are the same.
	delta := 0
