...
func TestConcatBytesAllocations(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"two empty elements":      {f: func() []byte { return []byte(empty + empty + empty) }, allocs: 0},
		"multiple empty elements": {f: func() []byte { return []byte(empty + empty + empty + empty + empty + empty + empty) }, allocs: 0},
