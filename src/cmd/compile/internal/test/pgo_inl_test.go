...
func TestPGOHash(t *testing.T) {
	...
	// buildPath is the source dir path that the compiler sees.
	buildPath, err := filepath.EvalSymlinks(dir)
	if err != nil {
		buildPath = dir
	}
	...
	gcflag0 := fmt.Sprintf("-pgoprofile=%s -trimpath %s=>%s -d=pgoinlinebudget=160,pgoinlinecdfthreshold=90,pgodebug=1", pprof, buildPath, pkg)
