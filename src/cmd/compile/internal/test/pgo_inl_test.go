...
func TestPGOIntendedInlining(t *testing.T) {
	...
	testPGOIntendedInlining(t, srcDir)
}
...
func TestPGOIntendedInliningShiftedLines(t *testing.T) {
	...
	// Copy most of the file unmodified.
	for _, file := range []string{"go.mod", "inline_hot_test.go", "inline_hot.pprof"} {
		if err := copyFile(filepath.Join(dir, file), filepath.Join(srcDir, file)); err != nil {
			t.Fatalf("error copying %s", file)
		}
	}
