...
var testInline3DumpPatterns = []*checkPair{
	// AST checks
	{regexp.MustCompile(`[1-9][0-9]* \..*INLMARK.*bb_test.go:144`), false},
	{regexp.MustCompile(`[1-9][0-9]* \..*INLMARK.*bb_test.go:147`), false},
}
...
func checkInline3Dump(t *testing.T, out []byte, funcName string) {
	...
	var res = []*checkPair{
		{regexp.MustCompile(`(\d*) \..*INLMARK.*bb_test.go:144`), 0},
		{regexp.MustCompile(`(\d*) \..*INLMARK.*bb_test.go:147`), 0},
	}
	var err error
	...
	if res[0].c < res[1].c || res[0].c == 0 || res[1].c == 0 {
		t.Logf("%s", string(out))
		t.Fatalf("Error evaluating counters for function %s: counter on line 144 (%d) should be greater than counter on line 147 (%d)\n", funcName, res[0].c, res[1].c)
	}
}
