...
func (n *miniType) posOr(pos src.XPos) src.XPos {
	if pos.IsKnown() {
		return pos
	}
	return n.pos
...
func (n *miniType) setType(t *types.Type, self Node) {
	if n.typ != nil {
...
func NewFuncType(pos src.XPos, rcvr *Field, args, results []*Field) *FuncType {
	n := &FuncType{Rcvr: rcvr, Args: args, Results: results}
	n.op = OTFUNC
	n.pos = pos
	return n
}
...
// An ArrayType represents a [Size]Elem type syntax.
// If Size is nil, the type is a [...]Elem in an array literal.
// Otherwise (Size == nil && !SizeDDD), the type is a []Elem slice.
...
func NewArrayType(pos src.XPos, size Node, elem Node) *ArrayType {
	n := &ArrayType{Size: size, Elem: elem}
	n.op = OTARRAY
	n.pos = pos
	return n
}
...
func (n *ArrayType) DeepCopy(pos src.XPos) Node {
	return NewArrayType(n.posOr(pos), DeepCopy(pos, n.Size), DeepCopy(pos, n.Elem))
}
