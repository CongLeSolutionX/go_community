...
// Init initializes the oracle based on the IR in function fn,
// in preparate
func (ro *ReassignOracle) Init(fn *Func) {
	...
	pruneIfNeeded := func(nn Node, asn Node) {
		if oname := outerName(nn); oname != nil {
			if defn, ok := ro.ok[oname]; ok {
				// any assignment to a param invalidates the entry.
				paramAssigned := oname.Class == PPARAM
				// assignment to local ok only if assignment is its original def.
	...
	// Prune away anything that looks assigned.
