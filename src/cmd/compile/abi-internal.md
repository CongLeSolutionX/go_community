...
It uses F8 – F15, F0 – F7 for floating-point arguments and results.
...
*Rationale*: These register meanings are compatible with Go’s
stack-based calling convention. X8 – X15, F8 – F15 is used for compressed
instruction (RVC) which will benefit code size in the future.

#### Stack layout

The stack pointer, X2, grows down and is always aligned to 8 bytes.
...
*Rationale*: Although riscv abi document stats that SP alaways aligned to 16 bytes,
but Go will adopt compressed instructions in the future which aligned to 8 bytes.
