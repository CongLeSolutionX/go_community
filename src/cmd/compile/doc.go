...
longer needed during the call. Stack growth does not understand these
arguments, so this directive must be used with //go:nosplit (in the marked
function and all transitive calls) to prevent stack growth. The conversion from
pointer to uintptr must appear in the argument list of any call to this
function. This directive is used for some low-level system call implementations
and should be avoided otherwise.
...
should be avoided otherwise. This directive is similar to
//go:uintptrkeepalive, but escapes arguments to the heap, making stack growth
safe.
