...
var (
	...
	foundFormats  = make(map[string]bool)        // set of all formats ("type format") found
	callSites     []*callSite                    // list of all calls using format strings
	filesUpdated  bool                           // if set, files were updated (only possible with -u)
)

func TestFormats(t *testing.T) {
	testenv.MustHaveGoBuild(t) // more restrictive than necessary, but that's ok

	// process each directories
	filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		if info.IsDir() {
			...
			if info.Name() == "ssa" {
				//return filepath.SkipDir // cannot handle indexed format
			}
			testFormats(t, filepath.Join("cmd/compile", path), path)
		}
		return nil
	})
	...
	if len(foundFormats) > 0 && testing.Verbose() {
		...
		fmt.Println("var knownFormats = map[string]string{")
		printList(list)
		fmt.Println("}")
	}

	// all format strings must in the formatStrings set (self-verification)
	for _, p := range callSites {
	...
	if len(formatStrings) > 0 && filesUpdated {
		...
		fmt.Println("WARNING: Potentially missed format strings")
		printList(list)
		t.Fail()
	}
}
...
func testFormats(t *testing.T, importPath, srcDir string) {
	// determine .go files
	buildPkg, err := build.Import(importPath, srcDir, 0)
	if err != nil {
	...
	filenames = append(filenames, buildPkg.GoFiles...)
	filenames = append(filenames, buildPkg.CgoFiles...)
	filenames = append(filenames, buildPkg.TestGoFiles...)

	// TODO(gri) verify _test files outside package
	for _, name := range buildPkg.XTestGoFiles {
	...
	for i, name := range filenames {
		filenames[i] = filepath.Join(srcDir, name)
	}
	...
	if _, err := conf.Check(importPath, fset, files, &types.Info{Types: etypes}); err != nil {
		t.Fatal(err)
	}
