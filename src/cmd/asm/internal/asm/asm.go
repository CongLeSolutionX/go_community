...
func (p *Parser) append(prog *obj.Prog, cond string, doLabel bool) {
	// Special case for ARM: instructions like ADD.P. TODO Move into append
	if p.arch.Thechar == '5' && cond != "" {
		bits, ok := arch.ARMCondition(cond)
		if !ok {
			p.errorf("invalid ARM condition code %q", cond)
		}
...
func (p *Parser) asmInstruction(op int, cond string, a []obj.Addr) {
	...
	case 6:
		...
		if p.arch.Thechar == '5' && arch.IsARMMRC(op) {
			...
			if cond != "" {
				p.errorf("TODO: can't handle ARM condition code for instruction")
			}
			cond = ""
