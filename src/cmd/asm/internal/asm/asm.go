...
func (p *Parser) asmJump(op obj.As, cond string, a []obj.Addr) {
	...
	case target.Type == obj.TYPE_MEM && target.Name == obj.NAME_NONE:
		...
		if p.arch.Char == sys.CharPPC64 && target.Offset == 0 {
...
func (p *Parser) asmInstruction(op obj.As, cond string, a []obj.Addr) {
	...
	case 1:
		...
		if p.arch.Char == sys.CharPPC64 && arch.IsPPC64NEG(op) {
	...
	case 2:
		...
		} else if p.arch.Char == sys.CharARM64 && arch.IsARM64CMP(op) {
			prog.From = a[0]
			prog.Reg = p.getRegister(prog, op, &a[1])
			break
		} else if p.arch.Char == sys.CharMIPS64 {
	...
	case 3:
		...
		case sys.CharARM64:
			...
			prog.Reg = p.getRegister(prog, op, &a[1])
			prog.To = a[2]
		case sys.Char386, sys.CharAMD64:
			prog.From = a[0]
			prog.From3 = newAddr(a[1])
			prog.To = a[2]
		case sys.CharPPC64:
	...
	case 4:
		if p.arch.Char == sys.CharARM && arch.IsARMMULA(op) {
		...
		if p.arch.Char == sys.CharPPC64 && arch.IsPPC64RLD(op) {
	...
	case 5:
		...
		return
	case 6:
		if p.arch.Char == sys.CharARM && arch.IsARMMRC(op) {
