	...
	for tok != '\n' && tok != ';' {
		...
		for {
			...
			if len(operands) == 0 && len(items) == 0 {
				if p.arch.HasChar(sys.CharARM, sys.CharARM64) && tok == '.' {
					// ARM conditionals.
					tok = p.lex.Next()
					str := p.lex.Text()
					if tok != scanner.Ident {
...
func (p *Parser) register(name string, prefix rune) (r1, r2 int16, scale int8, ok bool) {
	...
	if c == ':' || c == ',' || c == '+' {
		...
		case ',':
			if !p.arch.HasChar(sys.CharARM, sys.CharARM64) {
...
func (p *Parser) registerIndirect(a *obj.Addr, prefix rune) {
	...
	if r2 != 0 {
		...
		if p.arch.HasChar(sys.CharARM, sys.CharARM) {
			// Special form
...
func (p *Parser) registerNumber(name string) uint16 {
	if p.arch.Char == sys.CharARM && name == "g" {
		return 10
	}
	if name[0] != 'R' {
