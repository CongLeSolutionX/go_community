...
TEXT errors(SB),$0
	...
	MOVL	(AX)(SP*1), AX		// ERROR "can't use FP/SB/PC/SP as the index register in (AX)(SP*1)"
	...
	// TODO: fix invalid printing of pseudo regs. Currently printed as "R???-ID".
	CALL (AX)(PC*1)                 // ERROR "can't use FP/SB/PC/SP as the index register in (AX)(R???-4*1)"
	CALL (AX)(SB*1)                 // ERROR "can't use FP/SB/PC/SP as the index register in (AX)(R???-2*1)"
	CALL (AX)(FP*1)                 // ERROR "can't use FP/SB/PC/SP as the index register in (AX)(R???-1*1)"
