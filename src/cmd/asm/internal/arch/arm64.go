...
func ARM64RegisterExtension(a *obj.Addr, ext string, reg, num int16, isAmount, isIndex bool) error {
	...
	if reg <= arm64.REG_R31 && reg >= arm64.REG_R0 {
		...
		case "B":
			...
			a.Index = arm64.EncodeIndex(arm64.ARNG_B, arm64.RTYP_ARNG_INDEX, num)
		case "H":
			if !isIndex {
				return nil
			}
			a.Index = arm64.EncodeIndex(arm64.ARNG_H, arm64.RTYP_ARNG_INDEX, num)
		case "S":
			if !isIndex {
				return nil
			}
			a.Index = arm64.EncodeIndex(arm64.ARNG_S, arm64.RTYP_ARNG_INDEX, num)
		case "D":
			if !isIndex {
				return nil
			}
			a.Index = arm64.EncodeIndex(arm64.ARNG_D, arm64.RTYP_ARNG_INDEX, num)
		default:
