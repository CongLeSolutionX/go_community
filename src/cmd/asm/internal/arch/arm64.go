...
func ARM64RegisterExtension(a *obj.Addr, ext string, reg, num int16, isAmount, isIndex bool) error {
	...
	if reg <= arm64.REG_R31 && reg >= arm64.REG_R0 {
		...
		case "UXTB":
			...
			a.Reg = reg
			a.Index = arm64.RTYP_EXT_UXTB<<6 | num
		case "UXTH":
			if a.Type == obj.TYPE_MEM {
				return errors.New("invalid shift for the register offset addressing mode")
			}
			a.Reg = reg
			a.Index = arm64.RTYP_EXT_UXTH<<6 | num
		case "UXTW":
			// effective address of memory is a base register value and an offset register value.
			if a.Type == obj.TYPE_MEM {
				a.Offset = arm64.RTYP_EXT_UXTW<<6 | int64(num)
				a.Offset = a.Offset<<16 | int64(reg)
				a.Index |= arm64.RTYP_MEM_ROFF << 6
			} else {
				a.Reg = reg
				a.Index = arm64.RTYP_EXT_UXTW<<6 | num
			}
		case "UXTX":
			if a.Type == obj.TYPE_MEM {
				return errors.New("invalid shift for the register offset addressing mode")
			...
			a.Reg = reg
			a.Index = arm64.RTYP_EXT_UXTX<<6 | num
		case "SXTB":
			if a.Type == obj.TYPE_MEM {
				return errors.New("invalid shift for the register offset addressing mode")
			}
			a.Reg = reg
			a.Index = arm64.RTYP_EXT_SXTB<<6 | num
		case "SXTH":
			if a.Type == obj.TYPE_MEM {
				return errors.New("invalid shift for the register offset addressing mode")
			}
			a.Reg = reg
			a.Index = arm64.RTYP_EXT_SXTH<<6 | num
		case "SXTW":
			if a.Type == obj.TYPE_MEM {
				a.Offset = arm64.RTYP_EXT_SXTW<<6 | int64(num)
				a.Offset = a.Offset<<16 | int64(reg)
				a.Index |= arm64.RTYP_MEM_ROFF << 6
			} else {
				a.Reg = reg
				a.Index = arm64.RTYP_EXT_SXTW<<6 | num
			}
		case "SXTX":
			if a.Type == obj.TYPE_MEM {
				a.Offset = arm64.RTYP_EXT_SXTX<<6 | int64(num)
				a.Offset = a.Offset<<16 | int64(reg)
				a.Index |= arm64.RTYP_MEM_ROFF << 6
			} else {
				a.Reg = reg
				a.Index = arm64.RTYP_EXT_SXTX<<6 | num
			}
		case "LSL":
			a.Offset = arm64.RTYP_LSL<<6 | int64(num)
			a.Offset = a.Offset<<16 | int64(reg)
			a.Index |= arm64.RTYP_MEM_ROFF << 6
		default:
		...
		case "B8":
			...
			a.Index = arm64.ARNG_8B << 11
		case "B16":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_16B << 11
		case "H4":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_4H << 11
		case "H8":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_8H << 11
		case "S2":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_2S << 11
		case "S4":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_4S << 11
		case "D1":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_1D << 11
		case "D2":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_2D << 11
		case "Q1":
			if isIndex {
				return errors.New("invalid register extension")
			}
			a.Index = arm64.ARNG_1Q << 11
		case "B":
			if !isIndex {
				return nil
			}
			a.Index = arm64.ARNG_B<<11 | arm64.RTYP_INDEX<<6 | num
		case "H":
			if !isIndex {
				return nil
			}
			a.Index = arm64.ARNG_H<<11 | arm64.RTYP_INDEX<<6 | num
		case "S":
			if !isIndex {
				return nil
			}
			a.Index = arm64.ARNG_S<<11 | arm64.RTYP_INDEX<<6 | num
		case "D":
			if !isIndex {
				return nil
			}
			a.Index = arm64.ARNG_D<<11 | arm64.RTYP_INDEX<<6 | num
		default:
...
func ARM64RegisterListOffset(firstReg, regCnt, arrangement, index, scale int) (int64, error) {
	...
	//    | ... | 1 | ... | Scale | Index |   count  | arrangement |     number     |
	//    +-----+---+-----+-------+-------+----------+-------------+----------------+
	//    63    61  60    31    28 27   23 22      20 19         16 15              0
	//   arm64 uses the 60th bit to differentiate from other archs
	// For more details, refer to: obj/arm64/list7.go
	offset := 1<<60 | int64(scale<<28) | int64(index)<<23 | (int64(regCnt) << 20) | int64(arrangement)<<16 | int64(firstReg)
	return offset, nil
}
