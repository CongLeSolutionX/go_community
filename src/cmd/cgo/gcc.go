...
type typeConv struct {
	...
	// badStructs contains C structs that should be marked Incomplete.
	badStructs map[string]bool
...
func (c *typeConv) Init(ptrSize, intSize int64) {
	...
	c.badStructs = make(map[string]bool)
	c.bool = c.Ident("bool")
	c.byte = c.Ident("byte")
	c.int8 = c.Ident("int8")
		...
		if ptr, ok := dt.Type.(*dwarf.PtrType); ok {
			if strct, ok := ptr.Type.(*dwarf.StructType); ok {
				if c.badStructPointerTypedef(dt.Name, strct) {
					c.badStructs[strct.StructName] = true
					// Make sure we update any previously computed type.
					name := "_Ctype_struct_" + strct.StructName
					if oldType := typedef[name]; oldType != nil {
						oldType.Go = c.Ident("_Cgopkg.Incomplete")
					}
				}
			}
		}
