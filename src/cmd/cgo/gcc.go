...
// assignMangledNames mangles names for all the C references.
func (p *Package) assignMangledNames(f *File) {
...
func (p *Package) rewriteCall(f *File, call *Call, name *Name) bool {
	...
	// This is particular useful when passing additional arguments
	// to _cgoCheckPointer, as done in checkAddrArgs.
	//
	...
	//
	// When the call to C.f is deferred, we use an additional
	// function literal to evaluate the arguments at the right
	// time.
	//    defer func() func() {
	//            _cgo0 := p
	//            return func() {
	...
	if !call.Deferred {
		...
		if name.FuncType.Result != nil {
			...
			fmt.Fprintf(&sb, "%s", gofmtLine(rtype))
			result = true
		}
	...
	// Call _cgoCheckPointer as needed.
...
func (p *Package) checkAddr(sb, sbCheck *bytes.Buffer, arg ast.Expr, i int) bool {
	...
	*px = origX
	...
	fmt.Fprintf(sbCheck, "_cgoCheckPointer(_cgoBase%d, true); ", i)
...
func (p *Package) rewriteRef(f *File) {
	...
	for _, r := range f.Ref {
		...
		if (r.Context == ctxCall || r.Context == ctxCall2) && r.Name.Kind == "func" {
