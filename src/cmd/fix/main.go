...
func processFile(filename string, useStdin bool) error {
	...
	src := try(ioutil.ReadAll(f))

	file := try(parser.ParseFile(fset, filename, src, parserMode))
...
func diff(b1, b2 []byte) (data []byte, err error) {
	f1 := try(writeTempFile("", "go-fix", b1))
	defer os.Remove(f1)

	f2 := try(writeTempFile("", "go-fix", b2))
	defer os.Remove(f2)
