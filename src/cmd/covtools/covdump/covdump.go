...
var argsflag = flag.Bool("args", false, "Dump program args from counter data files")
...
var emitlegacyflag = flag.String("emitlegacy", "", "Convert to legacy format and emit to selected file.")
...
// package, drawing counter data from the map 'mm'. Return value is
// true if we emitted any output for the package, false otherwise.
func processPackage(mfname string, pd *decodemeta.CoverageMetaDataDecoder, pkgIdx uint32, mm map[pkfunc]decodecounter.FuncPayload) bool {
	if pkgre != nil {
...
func visitPod(p cov.Pod) {
	...
	for pkIdx := uint32(0); pkIdx < np; pkIdx++ {
		...
		if processPackage(p.MetaFile, pd, pkIdx, mm) {
			emitted = true
		}
	}
	if !emitted && pkgre != nil {
		warn("no matching output for package regular expression %q",
			*pkgreflag)
	}
}
...
func performdir(dirpath string) {
	...
	if len(pods) == 0 {
		warn("no applicable files found in dir %s", dirpath)
		return
	}
	for _, p := range pods {
		visitPod(p)
...
func main() {
	...
	if *emitlegacyflag != "" {
		...
		if err != nil {
			usage(fmt.Sprintf("unable to open legacy output file %q", *emitlegacyflag))
		}
	}
