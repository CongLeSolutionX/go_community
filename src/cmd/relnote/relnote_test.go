...
func TestLinks(t *testing.T) {
	...
	doc, err := relnote.Merge(docFS)
	if err != nil {
		t.Fatalf("relnote.Merge: %v", err)
	}
	aaa := markdown.ToHTML(doc)
	...
	testTree := func(dir, prefix string) {
		testFile := func(path string, _ os.FileInfo, err error) error {
			if err != nil {
				t.Fatal(err)
			}
		...
		testFile("/doc/next.html", nil, nil)
	}

	testTree("../../../doc/next", "https://go.dev")
}
