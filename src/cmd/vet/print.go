...
// getFormat returns the format string parameter and its' index within the given
// call expression.
//
// The first string literal or string constant is assumed as a format string.
//
// If it cannot find any format string parameter, it returns  ("", -1).
func getFormat(f *File, call *ast.CallExpr) (string, int) {
	for i, arg := range call.Args {
		switch x := arg.(type) {
		case *ast.BasicLit:
			if x.Kind == token.STRING {
				return x.Value[1 : len(x.Value)-1], i
			}
		case *ast.Ident:
			lit := f.pkg.types[arg].Value
			if lit != nil && lit.Kind() == constant.String {
				return constant.StringVal(lit), i
			}
		}
	}
	return "", -1
