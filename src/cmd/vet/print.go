...
// getFormatArg returns the format string argument and its index within
...
func getFormatArg(f *File, call *ast.CallExpr) (string, int) {
	...
	if typ != nil {
		if sig, ok := typ.(*types.Signature); ok {
			...
			if s, ok := getStringArg(f, call, idx); ok {
				return s, idx
			}
...
// getStringArg returns call's string argument at the index idx.
//
// ("", false) is returned if call's argument at the index idx isn't a string.
func getStringArg(f *File, call *ast.CallExpr, idx int) (string, bool) {
...
func (f *File) checkPrintf(call *ast.CallExpr, name string) {
	format, idx := getFormatArg(f, call)
	if idx < 0 {
		if *verbose {
