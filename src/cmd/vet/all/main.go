...
func (p platform) vet(ncpus int) {
	...
	if p != hostPlatform {
		...
		// Rob has indicated that these situations are rare enough
		// that he doesn't want to fix vet for it.
		// https://go-review.googlesource.com/c/32851/5/src/cmd/vet/print.go#200
		// So just skip the printf checks on non-host platforms.
