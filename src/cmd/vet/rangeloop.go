...
func checkLoop(f *File, node ast.Node) {
	...
	appendId := func(ids []*ast.Ident, expr ast.Expr) []*ast.Ident {
	...
	case *ast.RangeStmt:
		body = n.Body
		vars = appendId(vars, n.Key)
		vars = appendId(vars, n.Value)
	case *ast.ForStmt:
		body = n.Body
		...
		case *ast.AssignStmt:
			...
			for _, lhs := range post.Lhs {
				vars = appendId(vars, lhs)
			}
		case *ast.IncDecStmt:
			// e.g. for i := 0; i < n; i++
			vars = appendId(vars, post.X)
		}
	}
	if vars == nil {
		return
	...
	// Inspect the final statement of the loop body.
	if len(body.List) == 0 {
		return
	}
