...
// requirements into two separate blocks. If requirements are already present in
// mixed blocks without block-level comments, they are moved into direct-only
// and indirect-only blocks. Requirements are not moved out of blocks with
// block-level comments, nor are requirements added to such blocks. Existing
// requirements in commented blocks may be updated or deleted though.
func (f *File) SetRequireSeparateIndirect(req []*Require) {
	...
	// This may make some blocks empty.
	...
	for _, r := range f.Require {
		path := r.Mod.Path
		if need[path] == nil {
			// Requirement not needed. Delete.
			r.markRemoved()
			continue
		}
		if have[path] != nil {
			// Duplicate. Keep the first one.
			r.markRemoved()
			continue
		}
		have[r.Mod.Path] = r
	...
	// Find the last uncommented direct-only and indirect-only blocks.
	// Track any other require lines or blocks so we can insert new blocks
	// next to them.
	var (
	...
	// If the direct block is immediately after the indirect block,
	// swap them. This should get us into the same state whether we add
	// requirements one by one or all at once.
	if lastDirectBlockIndex == lastIndirectBlockIndex+1 {
		lastDirectBlockIndex, lastIndirectBlockIndex = lastIndirectBlockIndex, lastDirectBlockIndex
		f.Syntax.Stmt[lastDirectBlockIndex] = lastDirectBlock
		f.Syntax.Stmt[lastIndirectBlockIndex] = lastIndirectBlock
	}

	// Move or add requirements to the appropriate blocks.
	moveReq := func(r *Require) {
		var line *Line
		if r.Syntax == nil {
		...
		line.InBlock = true
		block := lastDirectBlock
		if r.Indirect {
			block = lastIndirectBlock
		}
		block.Line = append(block.Line, line)
	}

	for _, r := range have {
		if block := lineToBlock[r.Syntax]; block != nil && hasComments(block.Comments) {
			continue
		}
		moveReq(r)
	}
	for _, r := range need {
		if have[r.Mod.Path] != nil {
			continue
		}
		moveReq(r)
		f.Require = append(f.Require, r)
