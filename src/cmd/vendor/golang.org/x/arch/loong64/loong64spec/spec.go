...
// loong64spec reads the "LoongArch-Vol1-EN.pdf"
// to collect instruction encoding details.
// usage: loong64spec file.pdf
...
var fileNames = []string{"prologue.tmp", "op.tmp", "opstr.tmp", "instFormats.tmp"}

var files = map[string]string{
	"prologue.tmp":    "// Generated by Loong64 internal tool\n// DO NOT EDIT\n// Copyright 2022 The Go Authors. All rights reserved.\n// Use of this source code is governed by a BSD-style\n// license that can be found in the LICENSE file.\n\npackage loong64asm\n\n",
	"op.tmp":          "const (\n\t_ Op = iota\n",
...
func main() {
	...
	for pageNum := firstPage; pageNum <= n; pageNum++ {
		p := f.Page(pageNum)
		cPageOps, cPageOpstrs, cPageInstFormatComments, cPageInstFormats := parsePage(pageNum, p)
...
func findWords(chars []pdf.Text) (words []pdf.Text) {
	for i := 0; i < len(chars); {
		...
		// chars whose Y are same, check if the next char's X belong to the range of xRange, if ture, means it should
...
func parsePage(num int, p pdf.Page) (ops []string, opstrs map[string]string, instFormatComments map[string]string, instFormats map[string]string) {
	...
	for i := 0; i < len(text); {
		...
		opstr := fmt.Sprintf("%s:\t\"%s\",", strings.Replace(text[i].S, ".", "_", -1), text[i].S)
		instFormatComment := ""
		binValue := ""
		binMask := ""
		instArgs := ""
