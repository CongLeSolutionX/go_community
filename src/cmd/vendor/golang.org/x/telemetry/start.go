...
func Start(config Config) {
	...
	if _, err := os.Stat(telemetry.LocalDir); err != nil {
		// There was a problem statting LocalDir, which will be set
		// as the working dir of the child. Exit early.
		fmt.Fprintln(os.Stderr, "mode is ", mode)
		fmt.Fprintf(os.Stderr, "error statting %v: %v\n", telemetry.LocalDir, err)
