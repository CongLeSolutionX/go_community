...
var importer = NewImporter(build.Default)
...
func walkDir(t *testing.T, path string, endTime time.Time) (nimports int) {
	...
	for _, f := range list {
		if f.IsDir() {
			nimports += walkDir(t, filepath.Join(path, f.Name()), endTime)
...
func TestImportStdLib(t *testing.T) {
	...
	nimports := walkDir(t, "", time.Now().Add(dt)) // installed packages
