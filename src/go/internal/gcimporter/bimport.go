...
func (p *importer) obj(pkg *types.Package) bool {
	...
	case constTag:
		...
		typ := p.typ()
		val := p.value()
		obj = types.NewConst(token.NoPos, pkg, name, typ, val)
	...
	case varTag:
		name := p.string()
		typ := p.typ()
		obj = types.NewVar(token.NoPos, pkg, name, typ)
	...
	if alt := pkg.Scope().Insert(obj); alt != nil {
		// TODO(gri) check if gc exports the same object multiple times
...
func (p *importer) typ() types.Type {
	...
	case namedTag:
		...
		for i := p.int(); i > 0; i-- {
			...
			params := p.paramList()
			result := p.paramList()
			const isddd = false // TODO(gri) set this!
...
func (p *importer) fieldName() (*types.Package, string) {
	...
	pkg := p.pkgList[0] // anonymous and exported names assume local package
...
func (p *importer) paramList() *types.Tuple {
	n := p.int()
	if n == 0 {
		return nil
	}
	...
	for i := range params {
		params[i] = p.param(named)
	}
	return types.NewTuple(params...)
}

func (p *importer) param(named bool) *types.Var {
	typ := p.typ()
...
func (p *importer) float() constant.Value {
	...
	// TODO(gri) check this code (exp is incorrect?)

	// read mantissa bytes (big endian) and exponent
	x := constant.MakeFromBytes([]byte(p.string()))
	exp := p.int()
	switch {
	case exp < 0:
