...
func (p *importer) methodList(parent *types.Package, baseTypeName *types.Named) (methods []*types.Func) {
	if n := p.int(); n > 0 {
		methods = make([]*types.Func, n)
		for i := range methods {
			methods[i] = p.method(parent, baseTypeName)
		}
	}
	return
}

func (p *importer) method(parent *types.Package, baseTypeName *types.Named) *types.Func {
	pos := p.pos()
	pkg, name, _ := p.fieldName(parent)
	recv := types.NewVar(token.NoPos, parent, "", baseTypeName)
	params, isddd := p.paramList()
	result, _ := p.paramList()
