...
func (p *parser) parseTypeParameters() *ast.FieldList {
	...
	if p.tok != token.RBRACK {
		fields = p.parseParameterList(nil, nil, token.RBRACK)
	} else {
		p.error(p.pos, "empty type parameter list")
...
func (p *parser) parseParameters() *ast.FieldList {
	...
	return &ast.FieldList{Opening: lbrack, List: fields, Closing: rparen}
}
