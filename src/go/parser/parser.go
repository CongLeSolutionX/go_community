...
// advanceToClosingOrSemi advances to the right-hand delimiter closing the
// current position, taking into account delimiter balancing, and stopping on
// any semicolons.
//
...
//
// This method must only be called with '}', ')', or ']'.
func (p *parser) advanceToClosingOrSemi(right token.Token) {
	...
	case token.RBRACK:
		left = token.LBRACK
	default:
		panic(fmt.Sprintf("can't match delimiter %s", right))
	}
	for open := 1; ; p.next() {
		switch p.tok {
		case left:
...
L:
	...
	// technically be expecting an identifier, 'type', or a brace here.
	rbrace := p.expect(token.RBRACE)
