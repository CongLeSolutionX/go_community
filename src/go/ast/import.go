...
func sortSpecs(fset *token.FileSet, f *File, specs []Spec) []Spec {
	...
	for i, g := range f.Comments {
		// We ignore comments until they start at the same line as the first import spec.
		if g.Pos() < f.Package || fset.Position(g.Pos()).Line < fset.Position(pos[0].Start).Line {
			continue
		}
		if i < cstart {
			cstart = i
	...
	for _, g := range comments {
		...
		if specIndex == 0 && pos[specIndex].Start > g.Pos() {
			left = true
		} else if specIndex+1 < len(specs) && // Or it can also appear at the left of an import spec.
