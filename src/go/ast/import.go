...
func SortImports(fset *token.FileSet, f *File) {
	for _, d := range f.Decls {
		...
		if len(d.Specs) > 0 {
			...
			rParenLine := fset.PositionFor(d.Rparen, false).Line
...
func sortSpecs(fset *token.FileSet, f *File, specs []Spec) []Spec {
	...
	// Any comment from the line of pos[0].Start to the final line counts.
	begSpecs := pos[0].Start
	endSpecs := pos[len(pos)-1].End
	beg := fset.File(begSpecs).LineStart(fset.PositionFor(begSpecs, false).Line)
	end := fset.File(endSpecs).LineStart(fset.PositionFor(endSpecs, false).Line + 1) // beginning of next line
	...
	for _, g := range comments {
		...
		if specIndex == 0 && pos[specIndex].Start > g.Pos() {
			...
			fset.Position(pos[specIndex].Start).Line+1 == fset.Position(g.Pos()).Line {
			specIndex++
			left = true
		}
	...
	for i, s := range specs {
		if i == len(specs)-1 || !collapse(s, specs[i+1]) {
			...
			p := s.Pos()
			fset.File(p).MergeLine(fset.PositionFor(p, false).Line)
		}
	}
	specs = deduped
