...
func sortSpecs(fset *token.FileSet, f *File, specs []Spec) []Spec {
	...
	firstLine := fset.Position(pos[0].Start).Line
	cstart := len(f.Comments)
	cend := len(f.Comments)
	for i, g := range f.Comments {
		// We ignore comments until they start on the same line as the first import spec.
		if g.Pos() < f.Package || fset.Position(g.Pos()).Line < firstLine {
			continue
		}
		if i < cstart {
			cstart = i
