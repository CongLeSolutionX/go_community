...
//
// Roughly speaking, for a *Cursor c, the following invariants hold:
//
//   c.Parent().Name()          == c.Node()  if c.Index() <  0
//   c.Parent().Name()[Index()] == c.Node()  if c.Index() >= 0
//
...
// Index reports the index >= 0 of the current Node in the slice of Nodes that contains it
// or a value < 0 if the current Node is not part of a slice.
...
func (a *application) applyList(parent Node, name string) {
	// avoid heap-allocating a new iterator for each applyList call; reuse a.iterator instead
	saved := a.iter
	a.iter.index = 0
	for {
