...
type Scanner struct {
	...
	nlComment  token.Pos // position of first newline in preceding comment
...
scanAgain:
	if s.nlComment.IsValid() {
		// Return artificial ';' token after /*...*/ comment
		// containing newline, at position of first newline.
		pos, tok, lit = s.nlComment, token.SEMICOLON, "\n"
		s.nlComment = token.NoPos
		return
	}
		...
		case '/':
			if s.ch == '/' || s.ch == '*' {
				...
				if s.insertSemi && nlOffset != 0 {
					...
					s.nlComment = s.file.Pos(nlOffset)
					s.insertSemi = false
				} else {
