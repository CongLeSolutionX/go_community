...
type NoGoError struct {
	Dir        string
	HasIgnored bool // set when there are ignored Go files
}

func (e *NoGoError) Error() string {
	msg := "no buildable Go source files in " + e.Dir
	if e.HasIgnored {
		msg += ", package has ignored Go files due to mismatching build tags, etc"
	}
	return msg
}
...
Found:
	...
	if len(p.GoFiles)+len(p.CgoFiles)+len(p.TestGoFiles)+len(p.XTestGoFiles) == 0 {
		return p, &NoGoError{Dir: p.Dir, HasIgnored: len(p.IgnoredGoFiles) > 0}
	}
