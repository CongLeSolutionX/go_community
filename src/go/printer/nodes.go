...
// position) and fromOutCol. If the start position is on a different line from
// the current position (or either is unknown), the result is infinity.
func (p *printer) distanceFrom(startPos token.Pos, fromOutCol int) int {
	if startPos.IsValid() && p.pos.IsValid() {
		if startPosition := p.posFor(startPos); startPosition.Line == p.pos.Line {
			return p.out.Column - fromOutCol
		}
	}
	return infinity
}
