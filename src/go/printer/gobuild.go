...
func (p *printer) fixGoBuildLines() {
	...
	for pos := 0; ; {
		...
		if pos+3 < len(p.output) && p.output[pos] == tabwriter.Escape && p.output[pos+1] == '/' && p.output[pos+2] == '/' {
			blank = false
			pos += 3
			for pos < len(p.output) && p.output[pos] != '\n' {
				pos++
			}
		}
		// Skip over \n at end of line.
		if pos < len(p.output) && p.output[pos] != '\n' {
			break
		}
		pos++
		if blank {
	...
	case 0:
		...
		for _, pos := range p.plusBuild {
			y, err := constraint.Parse(p.commentTextAt(pos)) // TODO constraint.Parse
			if err != nil {
				x = nil
				break
			}
	...
	for _, end := range toDelete {
		...
		after = appendSafe(after, p.output[start:end])
		start = end + len(p.lineAt(end))
	}
	after = appendSafe(after, p.output[start:])
...
func appendSafe(x, y []byte) []byte {
	if (len(x) < 1 || isNL(x[len(x)-1])) && (len(x) < 2 || isNL(x[len(x)-2])) && len(y) > 0 && isNL(y[0]) {
		y = y[1:]
	}
	return append(x, y...)
}
