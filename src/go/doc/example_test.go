...
// This example illustrates how to use NewWithExamples
// to compute package documentation with examples.
func ExampleNewWithExamples() {
...
`
	...
	fset := token.NewFileSet()
	goFiles := map[string]*ast.File{"src.go": mustParse(fset, src)}
	testGoFiles := []*ast.File{mustParse(fset, test)}
