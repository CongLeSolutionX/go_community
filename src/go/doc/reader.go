...
// addMethod adds a selector for method m.
//
// If a field or method already exists at a higher level than m,
// m replaces it.
//
// If a field or method already exists at the same level as m,
// the selector is marked as having a conflict at that level.
//
// If a field or method already exists at a lower level than m,
// m is ignored.
func (ss selectorSet) addMethod(m *Func) {
	...
	case s.method.Level == m.Level:
		...
		// TODO(bcmills): Ideally, we shouldn't have to check
		// for equality here, but somehow we're ending up with
		// duplicated calls for the same method definition —
		// perhaps as a result of customizeRecv?
		if !reflect.DeepEqual(s.method.Decl, m.Decl) {
...
// addField adds a selector for field f.
//
// If a field or method already exists at a higher level than f,
// f replaces it.
//
// If a field or method already exists at the same level as f,
// the selector is marked as having a conflict at that level.
//
// If a field or method already exists at a lower level than f,
// f is ignored.
func (ss selectorSet) addField(f *_Field) {
	...
	case s.field.Level == f.Level:
		// TODO(bcmills): I'm not sure whether we actually need the
		// equality check here at all, but it is empirically needed
		// for addMethod and it seems less risky to keep them in sync.
		if !reflect.DeepEqual(s.field.Decl, f.Decl) {
			// The field declarations conflict.
			s.field.Decl = nil
		}
	default:
...
// readFunc processes a func or methodd eclaration.
...
func customizeRecv(f *Func, recvTypeName string, embeddedIsPtr bool, addLevel int) *Func {
	...
	newF.Level = f.Level + addLevel

	return &newF
}

// collectEmbedded collects the embedded methods and fields of typ in ss.
func (r *reader) collectEmbedded(ss selectorSet, typ *namedType, recvTypeName string, embeddedIsPtr bool, addLevel int, inPath embeddedSet) {
	inPath[typ] = true
	for embedded, isPtr := range typ.embedded {
		...
		for _, s := range embedded.selectors {
			if s.field != nil {
				ss.addField(&_Field{
					Name:  s.field.Name,
					Decl:  s.field.Decl,
					Level: addLevel + s.field.Level,
				})
			}

			if s.method != nil {
				var m *Func
				if s.method.Decl != nil {
					m = customizeRecv(s.method, recvTypeName, thisEmbeddedIsPtr, addLevel)
				}
				if m == nil {
					// s.method has a conflict or is invalid, but record its level for
					// conflict resolution anyway.
					m = &Func{
						Name:  s.method.Name,
						Level: addLevel + s.method.Level,
					}
				}
				ss.addMethod(m)
			}
		...
		if !inPath[embedded] {
			r.collectEmbedded(ss, embedded, recvTypeName, thisEmbeddedIsPtr, addLevel+1, inPath)
		}
	}
	delete(inPath, typ)
}
