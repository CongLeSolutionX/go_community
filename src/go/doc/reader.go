	...
	if fun.Type.Results.NumFields() >= 1 {
		...
		for i, res := range fun.Type.Results.List {
			...
			if n, imp := baseTypeName(factoryType); !imp && r.isVisible(n) {
				// Ignore pre-declared types for i > 0. We allow a predeclared type
		...
		// If there is exactly one result type, or there are only pre-declared types before a non-predeclared type,
