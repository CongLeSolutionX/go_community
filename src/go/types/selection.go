...
// previous (implicit) operations are all field selections.
// Each element of Indices specifies an implicit field (a, b, c)
// by its index in the struct type of the field selection operand.
//
// For a FieldVal operation, the final (explicit) selection refers to
// the field specified by Selection.Obj.
//
// For a MethodVal operation, the final (explicit) selection refers to
// a method. If the "pointerness" of the method's declared receiver
// does not match that of the effective receiver after implicit field
// selection, then an & or * operation is implicitly applied to the
// receiver variable or value. So, x.f. denotes (&x.a.b.c).f when f
// requires a pointer receiver but x.a.b.c is a non-pointer variable,
// or (*x.a.b.c).f when f requires a non-pointer receiver but x.a.b.c
// is a pointer value.
//
