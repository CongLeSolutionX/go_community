...
func (check *Checker) labels(body astBlockStmt) {
...
type block struct {
	...
	lstmt  astLabeledStmt            // labeled statement to which this block belongs, or nil
	labels map[string]astLabeledStmt // allocated lazily
}
...
func (b *block) insert(s astLabeledStmt) {
	...
	labels := b.labels
	if labels == nil {
		labels = make(map[string]astLabeledStmt)
		b.labels = labels
	}
	labels[name] = s
}
...
func (b *block) gotoTarget(name string) astLabeledStmt {
...
func (check *Checker) blockBranches(all *Scope, parent *block, lstmt astLabeledStmt, list astStmtList) []astBranchStmt {
	b := &block{parent: parent, lstmt: lstmt}

	var (
		// TODO: fix this placeholder.
		varDeclPos         astPosition = token.NoPos
		fwdJumps, badJumps []astBranchStmt
	)

	// All forward jumps jumping over a variable declaration are possibly
	// invalid (they may still jump out of the block and be ok).
	// recordVarDecl records them for the given position.
	recordVarDecl := func(pos astPosition) {
		varDeclPos = pos
		badJumps = append(badJumps[:0], fwdJumps...) // copy fwdJumps to badJumps
	}

	jumpsOverVarDecl := func(jmp astBranchStmt) bool {
	...
	blockBranches := func(lstmt astLabeledStmt, list astStmtList) {
