...
func (check *Checker) stmt(ctxt stmtContext, s ast.Stmt) {
	...
	case *ast.SwitchStmt:
		...
		if s.Tag != nil {
			check.expr(&x, s.Tag)
			if isUntyped(x.typ) {
				// spec: "If the switch expression evaluates to an untyped constant, it
				// is first converted to its default type; if it is an untyped boolean
				// value, it is first converted to type bool. The predeclared untyped
				// value nil cannot be used as switch expression."
