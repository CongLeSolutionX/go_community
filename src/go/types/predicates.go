...
func identical(x, y Type, cmpTags bool, p *ifacePair) bool {
	...
	case *Signature:
		...
		// Check variadic first as a minor optimization, to avoid potentially
		// substituting below.
		...
		if x.TypeParams().Len() > 0 {
			...
			for i, x := range xtparams {
				y := ytparams[i]
				ybound := check.subst(token.NoPos, y.bound, smap, nil)
				if !identical(x.bound, ybound, cmpTags, p) {
					return false
				}
			}
		...
		return identical(x.params, yparams, cmpTags, p) &&
