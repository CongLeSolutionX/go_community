...
// This file defines [HashMap], a mapping whose keys are Types, whose
...
type HashMap[V any] struct {
	table map[uint][]hashMapEntry[V] // maps hash to entries of that hash
	len   int                        // number of map entries
}

type hashMapEntry[V any] struct {
	key   Type // nil => unused entry
	value V
}
...
// It returns zero if the entry is not present.
...
func (m *HashMap[V]) Set(key Type, value V) (prev V) {
	...
	if m.table != nil {
		...
		if hole != nil {
			*hole = hashMapEntry[V]{key, value} // overwrite deleted entry
		} else {
			m.table[hash] = append(bucket, hashMapEntry[V]{key, value})
		}
	} else {
		m.table = map[uint][]hashMapEntry[V]{hash: {{key, value}}}
	}
...
// String returns a string representation of the map's entries in
// unspecified order. Values are printed as if by [fmt.Sprint].
func (m *HashMap[V]) String() string {
	return m.toString(true)
}

// KeysString returns a string representation of the map's key set in
// unspecified order.
func (m *HashMap[V]) KeysString() string {
	return m.toString(false)
}

func (m *HashMap[V]) toString(values bool) string {
...
func Hash(t Type) uint {
	...
	case *Struct:
		...
		for i, n := 0, t.NumFields(); i < n; i++ {
			f := t.Field(i)
			hash *= 36529 // field order is significant
			if f.Anonymous() {
				hash += 8861
			}
	...
	case *Signature:
		...
		for tparam := range t.TypeParams().TypeParams() {
			hash *= 34273 // parameter order is significant
	...
	case *Interface:
		// Interfaces are identical if they have the same set
		// of methods, with identical names and types, and
		// they have the same set of type restrictions.
		// See Identical for more detail.
		var hash uint = 9103
	...
	case *Named:
		...
		for targ := range t.TypeArgs().Types() {
			hash *= 31583 // type arg order is significant
			hash += 34469 * Hash(targ)
		}
		return hash
...
func hashTuple(tuple *Tuple) uint {
	...
	hash := 9137 + 2*uint(n)
	for i := 0; i < n; i++ {
		hash *= 34211 // var order is significant
...
func hashPtr(ptr any) uint {
	...
	// We could use maphash.Comparable (#54670, CL 609761),
...
func shallowHash(t Type) uint {
	...
	case *Tuple:
		...
		hash := 9137 + 2*uint(n)
		for i := 0; i < n; i++ {
			hash *= 31543 // var order is significant
