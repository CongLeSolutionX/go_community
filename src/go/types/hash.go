...
// HashMap[V] is a generic map from [Type] to values of type V.
//
// Keys (types) are considered equivalent if they are [Identical].
//
// Just as with map[K]V, a nil *HashMap is a valid empty map.
type HashMap[V any] struct {
	table  map[uint][]hashMapEntry[V] // maps hash to bucket; entry.key==nil means unused
...
func (m *HashMap[V]) Set(key Type, value V) (prev V) {
	if m.table != nil {
		...
		if hole != nil {
			*hole = hashMapEntry[V]{key, hash, value} // overwrite deleted entry
		} else {
			m.table[hash] = append(bucket, hashMapEntry[V]{key, hash, value})
		}
	} else {
		hash := Hash(key)
		m.table = map[uint][]hashMapEntry[V]{hash: {hashMapEntry[V]{key, hash, value}}}
	}
