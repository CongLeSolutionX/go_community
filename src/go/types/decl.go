...
func (check *Checker) objDecl(obj Object, def *Named, path []*TypeName) {
	...
	//
	// - an object whose type is not yet know is painted white (initial color)
	// - an object whose type is in the process of being inferred is painted grey
	// - an object whose type is fully inferred is painted black
	//
	// During type inference, an object's color changes from white to grey
	// to black (pre-declared objects are painted black from the start).
	// A black object (i.e., its type) can only depend (refer to) other black
	// ones. White and gray objects may depend on white and black objects.
	...
	// is such that the color value of a gray object indicates the index of
	// that object in the object path.

	// The existing code sets types and doesn't paint objects black in all
	// cases. Fix the color invariants for now.
	...
	if trace {
		check.trace(obj.Pos(), "-- checking %s (color = %s, path = %s)", obj, obj.color(), pathString(path))
		check.indent++
		defer func() {
			check.indent--
