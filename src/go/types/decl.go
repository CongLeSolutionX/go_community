...
func (check *Checker) typeDecl(obj *TypeName, tdecl *ast.TypeSpec, def *TypeName) {
	...
	check.later(func() {
		...
		_ = versionOk && check.isImportedConstraint(rhs) && check.verifyVersionf(tdecl.Type, go1_18, "using type constraint %s", rhs)
	...
	if tdecl.Assign.IsValid() {
		...
		if versionOk && tparam0 != nil && !check.verifyVersionf(tparam0, go1_23, "generic type alias") {
			versionOk = false
		}
		if versionOk && !check.verifyVersionf(atPos(tdecl.Assign), go1_9, "type alias") {
			versionOk = false
		}
	...
	if versionOk && tparam0 != nil && !check.verifyVersionf(tparam0, go1_18, "type parameter") {
		versionOk = false
	}
