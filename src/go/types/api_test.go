...
func TestInferredInfo(t *testing.T) {
	...
	for _, test := range tests {
		...
		inferred := make(map[*ast.Ident]Inferred)
		...
		makePkg := func(src string) *Package {
			...
			pkg, err := conf.Check("", fset, []*ast.File{f}, &Info{Inferred: inferred, Uses: uses})
			if err != nil {
				t.Fatal(err)
			}
		...
		for ident, inf := range inferred {
			if ExprString(ident) == test.fun {
				...
				if gensig, _ := uses[ident].Type().(*Signature); gensig == nil || gensig.TypeParams().Len() == 0 {
