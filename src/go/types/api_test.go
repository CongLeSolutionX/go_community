...
func TestTooNew(t *testing.T) {
	for _, test := range []struct {
		...
		{"", "go1.99", "p:2:1: file requires newer Go version go1.99"},
		...
		for id, obj := range info.Defs {
			if obj != nil {
				gotObjs = append(gotObjs, fmt.Sprintf("%s:%T", id.Name, obj))
			}
		}
		if !strings.Contains(fmt.Sprint(gotObjs), "f:*types.Func") {
			t.Errorf("%q: want Func for  unexpected error: got %q, want substring %q",
				src, gotObjs, "x")
		}
	}
}
