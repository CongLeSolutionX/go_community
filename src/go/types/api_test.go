...
`
		...
		for ident, inst := range instances {
			if ExprString(ident) == test.fun {
				...
				gen := uses[ident].Type()
				lister, _ := gen.(interface{ TypeParams() *TypeParamList })
				if lister == nil || lister.TypeParams().Len() == 0 {
					t.Errorf("package %s: info.Types[%v] = %v, want generic signature", pkg.Name(), ident, gen)
					continue
				}

				// Verify the invariant that re-instantiating the generic type with
				// TypeArgs results in an equivalent type.
				inst2, err := Instantiate(nil, gen, targs, true)
				if err != nil {
					t.Errorf("Instantiate(%v, %v) failed: %v", gen, targs, err)
				}
				if !Identical(inst.Type, inst2) {
					t.Errorf("Identical(%v, %v) = false", inst.Type, inst2)
				}
				break
			}
		}
		if targs == nil {
			t.Errorf("package %s: no instance information found for %s", pkg.Name(), test.fun)
			continue
		}
