	...
	for _, test := range tests {
		...
		*boolFieldAddr(&conf, "_EnableReverseTypeInference") = true
		...
		makePkg := func(src string) *Package {
			var pkg *Package
			if strings.Contains(src, "issue51803") {
				// allow error in this case
				pkg, _ = typecheck(src, &conf, &Info{Instances: instMap, Uses: useMap})
			} else {
				pkg = mustTypecheck(src, &conf, &Info{Instances: instMap, Uses: useMap})
			}
		...
		t.Run(pkg.Name(), func(t *testing.T) {
			...
			if got, want := len(instances), len(test.instances); got != want {
				for i, inst := range instances {
					t.Logf("%d. %s -> [%#v %s]", i, inst.Ident.Name, inst.Inst.TypeArgs, inst.Inst.Type)
				}
