...
func (err *error_) report() {
	...
	if multiError {
		for i := range err.desc {
			p := &err.desc[i]
			check.callErrorHandler(i, p.posn, err.code, p.msg, err.soft)
		}
	} else {
		check.callErrorHandler(0, err.posn(), err.code, err.msg(), err.soft)
	}
}

func (check *Checker) callErrorHandler(index int, posn positioner, code Code, msg string, soft bool) {
	assert(code != 0)

	if check.conf._Trace {
		check.trace(posn.Pos(), "ERROR: %s (code = %d)", msg, code)
	}
	...
	if index == 0 {
		...
		if index == 0 && check.conf._ErrorURL != "" {
		...
		// Explicitly reported sub-error: prepend position information and indent.
		if posn.Pos().IsValid() {
			msg = check.fset.Position(posn.Pos()).String() + ": " + msg
		}
		msg = "\t" + msg
	}
