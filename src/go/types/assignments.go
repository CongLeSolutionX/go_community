...
func (check *Checker) initVars(lhs []*Var, rhsExprs []ast.Expr, returnPos token.Pos) {
	rhs, commaOk := check.exprList(rhsExprs, len(lhs) == 2 && !returnPos.IsValid())
	...
	if commaOk {
		...
		check.recordCommaOkTypes(rhsExprs[0], a)
		return
	}
...
func (check *Checker) assignVars(lhs, rhsExprs []ast.Expr) {
	rhs, commaOk := check.exprList(rhsExprs, len(lhs) == 2)
