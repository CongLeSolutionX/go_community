...
`
	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, "celsius.go", data, 0)
	if err != nil {
		log.Fatal(err)
	}

	// Type-check the package containing a single source file.
	...
	// Print the method set of Celsius and of *Celsius.
...
// types.Info struct: definitions and references of each named object,
...
`
	...
	// Type-check the package.
	// We create an empty map for each kind of data
