...
func (check *Checker) builtin(x *operand, call *ast.CallExpr, id builtinId) (_ bool) {
	...
	switch id {
	case _Append:
		// append(s E, x ...E) S, where E is the element type of S
		...
		typeset(S, func(_, u Type) bool {
			...
			if E != nil && !Identical(E, s.elem) {
				check.errorf(x, InvalidAppend, "invalid append: mismatched element types %s and %s", E, s.elem)
				E = nil
				return false
			}
			E = s.elem
	...
	case _SliceData:
		...
		typeset(x.typ, func(_, u Type) bool {
			s, _ := u.(*Slice)
			if s == nil || slice != nil && !Identical(slice, s) {
				slice = nil
				return false
			}
			slice = s
