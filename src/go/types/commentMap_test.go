...
// []comment lists in a map indexed by line number. The position for each
// comment is the position of the token immediately preceding the comment
// and the Error message is the comment text, with all comments that are
// on the same line collected in a slice, in source order. If there is
// no preceding token (the matching comment appears at the beginning of
// the file), then the recorded position is unknown (line, col = 0, 0).
