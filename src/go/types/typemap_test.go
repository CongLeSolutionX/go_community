...
// TODO(adonovan): - test hashcodes are consistent with equals for a
// range of types (e.g. all types generated by type-checking some body
// of real code).
//
// TODO(adonovan): opt: do the impl trick to reduce duplicate
// generated code. But: gotta be careful with method delegation and nil recv.
...
func TestMap(t *testing.T) {
	...
	// Set of new key.
	if prev := tmap.Set(tPStr1, "*string"); prev != "" {
	...
	if prev := tmap.Set(tChanInt1, "<-chan int"); prev != "" {
...
func TestGeneric(t *testing.T) {
	...
	var m types.TypeMap[string]
	prev := m.Set(tPStr1, "tPStr1")
	t.Error(prev) // ""
	prev = m.Set(tPStr2, "tPStr2")
	t.Error(prev)    // "pPStr1"
	t.Error(m.Len()) // 1
	prev = m.Set(tStr, "string")
	t.Error(prev)    // ""
	t.Error(m.Len()) // 2
	for typ, v := range m.All() {
		t.Error(typ, v) // (*string "tPStr2"), (string "string")
	}
}
