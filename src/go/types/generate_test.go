...
func generate(t *testing.T, filename string, write bool) {
	...
	fmt.Fprintf(&buf, "// Code generated by \"go test -run=Generate -write=all\" from %s/%s; DO NOT EDIT.\n\n", filepath.ToSlash(rel), filename)
	...
	if d := diff.Diff(filename+" (on disk)", onDiskContent, filename+" (generated)", generatedContent); d != nil {
		if write {
			...
			t.Errorf("generated file (in %s) does not match source (in %s):\n%s",
				dstDir, srcDir, string(d))
		}
	}
}
