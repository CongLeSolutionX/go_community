...
func TestContextDeduplication(t *testing.T) {
	...
	// ...but verify we don't inst with different type arguments.
...
func FuzzContext(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if identical != (hash1 == hash2) {
			panic(fmt.Sprintf("bad types %s and %s (identical: %t, hash1: %q hash2: %q)", typ1, typ2, identical, hash1, hash2))
		}
	})
}
...
func (r *typeReader) typ() Type {
	...
	nComplex := int(unionType)
	if complexity > rem/nComplex { // modulus will exceed uint8
		panic(fmt.Sprintf("invalid complexity factor %d", complexity))
	}

	modulus := len(Typ) + complexity*int(unionType)
