...
func (check *Checker) collectObjects() {
	...
	for fileNo, file := range check.files {
		...
		for decli := 0; decli < file.Decls().Len(); decli++ {
			decl := file.Decls().Decl(decli)
			switch decl.Kind() {
			case badDeclKind:
				// ignore
					...
					case ValueSpecKind:
						...
						case token.VAR:
							...
							if s.Values().Len() == 1 {
								...
								// for a later phase.
								d1 = &declInfo{file: fileScope, lhs: lhs, typ: s.Type(), init: s.Values().Expr(0)}
							}
...
func (check *Checker) resolveBaseTypeName(typ astExpr) (ptr bool, base *TypeName) {
	...
	for {
		...
		if pexpr := typ.StarExpr(); pexpr != nil {
