...
type declInfo struct {
	...
	typ   astExpr     // type, or nil
	init  astExpr     // init/orig expression, or nil
	fdecl astFuncDecl // func declaration, or nil
...
func (check *Checker) arityMatch(s, init astValueSpec) {
	l := s.NamesLen()
	r := s.ValuesLen()
	if init != nil {
		r = init.ValuesLen()
...
func (check *Checker) declarePkgObj(ident astIdent, obj Object, d *declInfo) {
	assert(ident.IdentName() == obj.Name())

	// spec: "A package-scope or file-scope identifier with name init
	// may only be declared to be a function with this (func()) signature."
	if ident.IdentName() == "init" {
	...
	if ident.IdentName() == "main" && check.pkg.name == "main" {
...
func (check *Checker) importPackage(pos astPosition, path, dir string) *Package {
...
func (check *Checker) collectObjects() {
	...
	for fileNo, file := range check.files {
		...
		// Use the actual source file extent rather than astFile extent since the
		// latter doesn't include comments which appear at the start or end of the file.
		// Be conservative and use the astFile extent if we don't have a *token.File.
		...
		for decli := 0; decli < file.DeclsLen(); decli++ {
			...
			case astGenDecl:
				...
				for iota := 0; iota < d.SpecsLen(); iota++ {
					...
					case astImportSpec:
						// import package
						sPath := s.Path()
						path, err := validatedImportPath(sPath.LitValue())
						if err != nil {
					...
					case (astValueSpec):
						...
						case token.VAR:
							...
							for i := 0; i < s.NamesLen(); i++ {
								name := s.Name(i)
								obj := newVar(name.Pos(), pkg, name.IdentName(), nil)
								lhs[i] = obj
			...
			case astFuncDecl:
				name := d.Name()
				obj := newFunc(name.Pos(), pkg, name.IdentName(), nil)
				if d.Recv() == nil {
					// regular function
					if name.IdentName() == "init" {
...
func (check *Checker) resolveBaseTypeName(typ astExpr) (ptr bool, base *TypeName) {
	...
	for {
		...
		if pexpr, _ := typ.(astStarExpr); pexpr != nil {
			// if we've already seen a pointer, we're done
			if ptr {
				return false, nil
			}
			ptr = true
			typ = unparen(pexpr.InnerX()) // continue with pointer base type
		}
		...
		obj := check.pkg.scope.Lookup(name.IdentName())
		if obj == nil {
			return false, nil
		}
