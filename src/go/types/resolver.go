...
func (check *Checker) arityMatch(s, init itypes.ValueSpec) {
	...
	case l < r:
		if l < s.ValuesLen() {
			...
			check.errorf(s.Pos(), "extra init expr at %s", check.fset.Position(unwrapPos(init.Pos())))
			// TODO(gri) avoid declared but not used error here
		}
...
func (check *Checker) filename(fileNo int) string {
	...
	if pos := file.Pos(); pos.IsKnown() {
		// TODO: eliminate this type assertion
		return check.fset.File(unwrapPos(pos)).Name()
	}
...
func (check *Checker) collectObjects() {
	...
	for fileNo, file := range check.files {
		...
		// TODO: eliminate this type assertion
		if f := check.fset.File(unwrapPos(file.Pos())); f != nil {
			pos, end = wrapPos(token.Pos(f.Base())), wrapPos(token.Pos(f.Base()+f.Size()))
		}
		...
		// TODO: eliminate this type assertion
		fileDir := dir(check.fset.Position(unwrapPos(file.Name().Pos())).Filename)
			...
			case itypes.FuncDecl:
				...
				if d.Recv() == nil {
					...
					if name.Name() == "init" {
						...
						check.declare(pkg.scope, name, obj, noPos)
					}
				} else {
					// method
		...
		for _, obj := range scope.elems {
			if alt := pkg.scope.Lookup(obj.Name()); alt != nil {
				if pkg, ok := obj.(*PkgName); ok {
					check.errorf(wrapPos(alt.Pos()), "%s already declared through import of %s", alt.Name(), pkg.Imported())
					check.reportAltDecl(pkg)
				} else {
					check.errorf(wrapPos(alt.Pos()), "%s already declared through dot-import of %s", alt.Name(), obj.Pkg())
