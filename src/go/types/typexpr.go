...
func (check *Checker) structType(styp *Struct, e *ast.StructType) {
	...
	for _, f := range list.List {
		...
		if len(f.Names) > 0 {
			...
			// pointer type."
			// TODO(rFindley) should we accept parenthesized type names? (spec question)
			pos := f.Type.Pos()
			...
			check.atEnd(func() {
				...
				case *Basic:
					...
					if t.kind == UnsafePointer {
						check.errorf(errpos, _InvalidPtrEmbed, "embedded field type cannot be unsafe.Pointer")
					}
				case *Pointer:
					check.errorf(errpos, _InvalidPtrEmbed, "embedded field type cannot be a pointer")
				case *Interface:
					if isPtr {
						check.errorf(errpos, _InvalidPtrEmbed, "embedded field type cannot be a pointer to an interface")
					}
				}
			})
		}
...
func embeddedFieldIdent(e ast.Expr) *ast.Ident {
	...
	case *ast.CallExpr:
		...
		// TODO (rFindley) should we now accept ParenExprs here? (spec question)
		// case *ast.ParenExpr:
		// 	return embeddedFieldIdent(e.X)
	}
