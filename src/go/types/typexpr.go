...
//
func (check *Checker) ident(x *operand, e itypes.Ident, def *Named, wantType bool) {
	x.mode = invalid
	x.expr = e
...
func (check *Checker) typ(e itypes.Expr) Type {
...
//
func (check *Checker) definedType(e itypes.Expr, def *Named) (T Type) {
	if trace {
...
func (check *Checker) funcType(sig *Signature, recvPar itypes.FieldList, ftyp itypes.FuncType) {
	...
	if recvPar != nil {
		...
		default:
			...
			check.error(wrapPos(recvList[len(recvList)-1].Pos()), "method must have exactly one receiver")
...
//
func (check *Checker) typInternal(expr itypes.Expr, def *Named) Type {
	switch e := expr.(type) {
	case itypes.BadExpr:
	...
	case itypes.FuncType:
		typ := new(Signature)
		def.setUnderlying(typ)
		check.funcType(typ, nil, e)
		return typ

	case itypes.InterfaceType:
		typ := new(Interface)
		def.setUnderlying(typ)
...
//
func (check *Checker) typOrNil(e itypes.Expr) Type {
	x := operandPool.Get().(*operand)
...
func (check *Checker) arrayLength(e itypes.Expr) int64 {
	x := operandPool.Get().(*operand)
	defer operandPool.Put(x)
	check.expr(x, e)
	if x.mode != constant_ {
...
func (check *Checker) collectParams(scope *Scope, list itypes.FieldList, variadicOk bool) (params []*Var, variadic bool) {
	if list == nil {
		return
	}
	...
	for i := 0; i < list.Len(); i++ {
		field := list.Field(i)
		ftype := field.Type()
		if t, _ := ftype.(itypes.DotDotDot); t != nil {
			ftype = t.Elt()
	...
	if variadic {
		...
		check.recordTypeAndValue(list.Field(list.Len()-1).Type(), typexpr, last.typ, nil)
	}

	return
}

func (check *Checker) declareInSet(oset *objset, pos itypes.Pos, obj Object) bool {
...
func (check *Checker) interfaceType(ityp *Interface, iface itypes.InterfaceType, def *Named) {
...
func (check *Checker) completeInterface(ityp *Interface) {
	...
	if trace {
		check.trace(noPos, "complete %s", ityp)
		check.indent++
		defer func() {
			check.indent--
			check.trace(noPos, "=> %s", ityp)
		}()
	}
	...
	var seen objset
	var methods []*Func
	mpos := make(map[*Func]itypes.Pos) // method specification or method embedding position, for good error messages
	addMethod := func(pos itypes.Pos, m *Func, explicit bool) {
...
func (check *Checker) tag(t itypes.BasicLit) string {
	if t != nil {
...
func (check *Checker) structType(styp *Struct, e itypes.StructType) {
	list := e.Fields()
	if list == nil {
		return
	}
	...
	var typ Type
	var tag string
	add := func(ident itypes.Ident, embedded bool, pos itypes.Pos) {
		if tag != "" && tags == nil {
	...
	// (issue #25627).
	addInvalid := func(ident itypes.Ident, pos itypes.Pos) {
		typ = Typ[Invalid]
		tag = ""
		add(ident, true, pos)
	}
...
func embeddedFieldIdent(expr itypes.Expr) itypes.Ident {
	switch e := expr.(type) {
	case itypes.Ident:
		return e
	case itypes.StarExpr:
		// *T is valid, but **T is not
		if xx, _ := e.X().(itypes.StarExpr); xx == nil {
