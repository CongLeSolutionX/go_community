...
// type-checked.
func Eval(fset *token.FileSet, pkg *Package, pos srcPos, expr string) (_ TypeAndValue, err error) {
	// parse expressions
...
func CheckExpr(fset *token.FileSet, pkg *Package, pos srcPos, expr ast.Expr, info *Info) (err error) {
	// determine scope
	var scope *Scope
	if pkg == nil {
		scope = Universe
