...
func (check *Checker) funcInst(x *operand, ix *ast.IndexExpr) {
	args := unpackExpr(ix.Index)
	targs := check.typeList(args)
	if targs == nil {
		x.mode = invalid
		x.expr = ix
		return
	...
	poslist := make([]token.Pos, len(args))
	for i, arg := range args {
		poslist[i] = arg.Pos()
	}

	// If we have all type arguments now, eagerly instantiate the function.
	// Otherwise delay instantiation so that type arguments may be inferred.
	//
	// TODO(rFindley): It should be possible to simply return an instance in all
	//                 cases here, with the assumption that the expansion will be
	//                 typechecked when it is needed. However, this subtly fails
	//                 during inference (in fixedbugs/issue39976.go2). Need to
	//                 investigate further.
	var res Type
	if len(targs) == len(sig.tparams) {
		sig := check.instantiate(x.Pos(), sig, targs, poslist).(*Signature)
		res = sig
		assert(sig.tparams == nil) // signature is not generic anymore
	} else {
		inst := &instance{
			check:   check,
			pos:     x.Pos(),
		...
		// inst.value should be set during type inference.
		check.later(func() {
...
func (check *Checker) call(x *operand, call *ast.CallExpr) exprKind {
	...
	default:
		...
		var inst *instance
		if inst, _ = x.typ.(*instance); inst != nil {
			sig = asSignature(inst.base)
			targs = inst.targs
			poslist = inst.poslist
		} else {
		...
		if !ok {
			...
			x.expr = call
			if inst != nil {
				inst.value = Typ[Invalid] // avoid follow-on errors when type is not inferred
			}
			return expression
		}

		sig = check.arguments(call, sig, args, targs, poslist)
		if inst != nil {
			inst.value = sig
		}
...
func (check *Checker) arguments(call *ast.CallExpr, sig *Signature, args []*operand, targs []Type, poslist []token.Pos) (rsig *Signature) {
	rsig = sig
	...
	if len(sig.tparams) > 0 {
		...
		targs, failed := check.infer(sig.tparams, sigParams, args, targs)
		if targs == nil {
		...
		rsig = check.instantiate(call.Pos(), sig, targs, poslist).(*Signature)
...
func (check *Checker) selector(x *operand, e *ast.SelectorExpr) {
	...
	if m, _ := obj.(*Func); m != nil {
		...
		if len(sig.rparams) > 0 {
			...
			targs, failed := check.infer(sig.rparams, NewTuple(sig.recv), []*operand{&arg}, nil)
			if failed >= 0 {
