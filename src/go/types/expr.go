...
are generally of the form:
  ...
  func f(x *operand, e itypes.Expr, ...)
...
func (check *Checker) unary(x *operand, e itypes.UnaryExpr, op token.Token) {
	switch op {
	case token.AND:
		// spec: "As an exception to the addressability
		// requirement x may also be a composite literal."
		// if _, ok := unparen(x.expr).(itypes.CompositeLit); !ok && x.mode != variable {
		if compLit, _ := unparen(x.expr).(itypes.CompositeLit); compLit == nil && x.mode != variable {
...
//
func (check *Checker) updateExprType(x itypes.Expr, typ Type, final bool) {
	...
	case itypes.BadExpr,
		...
		itypes.IndexExpr,
		itypes.SliceExpr,
		itypes.TypeAssertExpr,
		itypes.StarExpr,
		itypes.KeyValueExpr,
		itypes.ArrayType,
		itypes.StructType,
		itypes.FuncType,
		itypes.InterfaceType,
		itypes.MapType,
		itypes.ChanType:
	...
	case itypes.CallExpr:
		// Resulting in an untyped constant (e.g., built-in complex).
		// The respective calls take care of calling updateExprType
		// for the arguments if necessary.

	case itypes.Ident, itypes.BasicLit, itypes.SelectorExpr:
	...
	case itypes.BinaryExpr:
		...
		if isComparison(e.Op()) {
			...
			check.updateExprType(e.X(), typ, final)
			check.updateExprType(e.Y(), typ, final)
		}
...
func (check *Checker) updateExprVal(x itypes.Expr, val constant.Value) {
...
func (check *Checker) shift(x, y *operand, e itypes.BinaryExpr, op token.Token) {
...
func (check *Checker) binary(x *operand, e itypes.BinaryExpr, lhs, rhs itypes.Expr, op token.Token) {
	// var y operand
...
func (check *Checker) index(index itypes.Expr, max int64) (typ Type, val int64) {
	typ = Typ[Invalid]
	val = -1
...
//
func (check *Checker) indexedElts(expr itypes.CompositeLit, typ Type, length int64) int64 {
	...
	for ei := 0; ei < expr.EltsLen(); ei++ {
		...
		validIndex := false
		eval := e
		if kv, _ := e.(itypes.KeyValueExpr); kv != nil {
...
//
func (check *Checker) rawExpr(x *operand, e itypes.Expr, hint Type) exprKind {
	if trace {
		check.trace(e.Pos(), "expr %s", e)
		check.indent++
		defer func() {
			check.indent--
			check.trace(e.Pos(), "=> expr %s", x)
		}()
	}
...
//
func (check *Checker) exprInternal(x *operand, expr itypes.Expr, hint Type) exprKind {
	...
	case itypes.CompositeLit:
		...
		case eType != nil:
			...
			if atyp, _ := eType.(itypes.ArrayType); atyp != nil && atyp.Len() != nil {
				if ellip, _ := atyp.Len().(itypes.DotDotDot); ellip != nil && ellip.Elt() == nil {
	...
	case itypes.SelectorExpr:
		check.selector(x, e)

	case itypes.IndexExpr:
		eIndex := e.Index()
		check.expr(x, e.X())
		if x.mode == invalid {
			check.use(eIndex)
	...
	case itypes.SliceExpr:
		...
		// check indices
		var ind [3]int64
		for i, expr := range []itypes.Expr{e.Low(), e.High(), e.Max()} {
			x := int64(-1)
			switch {
			case expr != nil:
	...
	case itypes.KeyValueExpr:
		// key:value expressions are handled in composite literals
		check.invalidAST(expr.Pos(), "no key:value expected")
		goto Error

	case itypes.ArrayType, itypes.StructType, itypes.FuncType,
		itypes.InterfaceType, itypes.MapType, itypes.ChanType:
		x.mode = typexpr
	...
	default:
		...
		// TODO:confirm this message is correct.
		panic(fmt.Sprintf("%s: unknown expression type %T", expr.Pos(), expr))
	}
...
func (check *Checker) typeAssertion(pos itypes.Pos, x *operand, xtyp *Interface, T Type) {
...
//
func (check *Checker) expr(x *operand, e itypes.Expr) {
	check.multiExpr(x, e)
	check.singleValue(x)
}

// multiExpr is like expr but the result may be a multi-value.
func (check *Checker) multiExpr(x *operand, e itypes.Expr) {
...
// If an error occurred, x.mode is set to invalid.
//
func (check *Checker) exprWithHint(x *operand, e itypes.Expr, hint Type) {
	assert(hint != nil)
...
// If an error occurred, x.mode is set to invalid.
//
func (check *Checker) exprOrType(x *operand, e itypes.Expr) {
	check.rawExpr(x, e, nil)
	check.singleValue(x)
	if x.mode == novalue {
