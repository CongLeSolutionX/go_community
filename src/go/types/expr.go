...
func (check *Checker) exprInternal(x *operand, e ast.Expr, hint Type) exprKind {
	...
	case *ast.IndexExpr:
		// TODO(rFindley) previous iterations of this code guarded on the
		// configuration check.UseBrackets. Test/confirm that no such guard is
		// necessary.
		...
		if x.mode == typexpr {
			// TODO(rFindley) is this guard necessary? types2 doesn't have it.
			if isGeneric(x.typ) {
				// type instantiation
				x.mode = invalid
			...
			check.errorf(x, 0, "%s is not a generic type", x.typ)
			goto Error
		}
