...
type Checker struct {
	...
	*Info
	...
	//
	// pkgPathMap is allocated and built lazily, so that we don't pay the price
	// of constructing it on the happy path. seenPkgMap tracks the packages that
	// we've already walked.
	pkgPathMap map[string]map[string]struct{}
	seenPkgMap map[*Package]struct{}
