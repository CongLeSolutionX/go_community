...
func (u *unifier) nify(x, y Type, p *ifacePair) (result bool) {
	...
	// EnableInterfaceInference is set and one or both types are interfaces,
	// rather than insisting on equivalent structure, we check if there are
	// any common methods that can be unified. This is obviously the same as
	// checking equivalent structure if they are equivalent, but also allows
	// us to infer more type arguments if the interfaces don't have the same
	// method set or one of the types is not an interface.
	if u.check.conf._EnableInterfaceInference {
		...
		// If one of x or y is an interface, it is in x.

		if xi != nil {
			// Each method present in both xi must also be in y and they must unify.
	...
	case *Named:
		...
		if y, ok := y.(*Named); ok {
			if u.check.conf._EnableInterfaceInference {
				// If one or both defined types are interfaces,
				// in assignments one type must implement the
				// other; i.e., at least the shared methods must
				// have matching signatures. Here we don't know if
				// we are in assignment context or in which direction
				// the assignment is going, but in the worst case we
				// may infer type parameters for an invalid assignment.
				// In the best case we infer type parameters that we
				// wouldn't infer if we were just looking at origin.
				// Keep unifying the underlying types in this case.
				xi := IsInterface(x)
				yi := IsInterface(y)
				switch {
				case xi && yi:
					return u.nify(x.under(), y.under(), p)
				case xi:
