...
func (u *unifier) nify(x, y Type, p *ifacePair) (result bool) {
	...
	// If EnableInterfaceInference is set and one or both types are interfaces,
	// check that common methods can be unified rather than requiring that the
	// types are structurally equivalent. If they are structurally equivalent,
	// common methods will unify as before. But if they are not equivalent,
	// unifying common methods may infer additional type arguments. Subsequent
	// instantiation and assignment checks will exclude any cases where type
	// inference succeeded incorrectly.
	if u.check.conf._EnableInterfaceInference {
		...
		if xi != nil && yi != nil {
			...
			// Methods that are present in both interfaces must unify.
			...
			for _, ym := range ymethods {
				id := ym.Id()
				if xm := xmap[id]; xm != nil {
					if !u.nify(xm.typ, ym.typ, q) {
						return false
					}
					delete(xmap, id)
					if len(xmap) == 0 {
						break // no more xmethods so we can stop
					}
				}
			}
			return true
		...
		// For them to unify, the must be structurally equivalent.
	}
