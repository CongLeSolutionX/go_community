	...
	case px != nil:
		...
		if x := u.at(px); x != nil {
			...
			if u.nify(x, y, mode, p) {
				...
				// The named or directional (channel) type is always the more narrow type choice;
				// if inexact unification succeeded, a matching unnamed type or bydirectional channel
				// will unify against the more narrow choice as well and avoid parameter order
				// dependencies and assignment issues (go.dev/issue/43056, go.dev/issue/62157).
				if mode&exact == 0 {
