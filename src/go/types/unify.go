...
// it is a non-type parameter interfaces. Otherwise it returns nil.
		...
		if x := u.at(px); x != nil {
			...
			if u.nify(x, y, mode, p) {
				...
				if xi != nil && yi != nil {
					...
					// Unification cannot chose the interface because that would
					// introduce an order dependency: if multiple types are matched
					// against the same type parameter, it matters when the interface
					// appears (go.dev/issue/60933).
					return false
				}
