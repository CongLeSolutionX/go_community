...
func dependencyGraph(objMap map[Object]*declInfo) []*graphNode {
	...
	// (We need to include all nodes, even the ones that have no dependencies
	// and are not dependencies of other nodes because they still need to be
	// scheduled for initialization in correct order relative to other nodes.)
	for obj, n := range M {
	...
	for obj, n := range M {
		if _, ok := obj.(*Func); ok {
			...
			delete(M, obj) // remove function from graph
		}
	}

	// build graph G without functions
