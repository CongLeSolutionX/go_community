...
func (check *Checker) validType0(typ Type, info map[*Named]typeInfo, path []Object) typeInfo {
	const (
	...
	switch t := typ.(type) {
	case *Array:
		return check.validType0(t.elem, info, path)

	case *Struct:
		for _, f := range t.fields {
			if check.validType0(f.typ, info, path) == invalid {
				return invalid
			}
		}
	...
	case *Union:
		for _, t := range t.terms {
			if check.validType0(t.typ, info, path) == invalid {
				return invalid
			}
		}
	...
	case *Interface:
		for _, etyp := range t.embeddeds {
			if check.validType0(etyp, info, path) == invalid {
				return invalid
			}
		}
	...
	case *Named:
		...
		case unknown:
			info[t] = marked
			info[t] = check.validType0(t.fromRHS, info, append(path, t.obj)) // only types of current package added to path
		case marked:
			// cycle detected
			for i, tn := range path {
				if t.obj.pkg != check.pkg {
