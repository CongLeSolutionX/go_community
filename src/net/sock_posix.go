...
func socket(ctx context.Context, network string, family, sotype, proto int, ipv6only bool, laddr, raddr sockaddr, ctrlFn func(string, string, syscall.RawConn) error) (fd *netFD, err error) {
	...
	if fd, err = newFD(s, family, sotype, network); err != nil {
		poll.CloseFunc(s)
		return nil, err
	}
