...
func socket(network string, family, sotype, proto int, ipv6only, fastopen bool, laddr, raddr sockaddr, deadline time.Time, cancel <-chan struct{}) (*netFD, error) {
	...
	if err := fd.dial(fastopen, laddr, raddr, deadline, cancel); err != nil {
		fd.Close()
		return nil, err
	}
	return fd, nil
...
func (fd *netFD) dial(fastopen bool, laddr, raddr sockaddr, deadline time.Time, cancel <-chan struct{}) error {
	var err error
