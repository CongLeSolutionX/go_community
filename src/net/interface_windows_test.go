...
type byRevAddrLen []IPNet

func (ps byRevAddrLen) Len() int { return len(ps) }

func (ps byRevAddrLen) Less(i, j int) bool {
...
func (ps byRevAddrLen) Swap(i, j int) { ps[i], ps[j] = ps[j], ps[i] }
...
func TestWindowsAddrPrefixLen(t *testing.T) {
	for i, tt := range windowsAddrPrefixLenTests {
		...
		sort.Sort(byRevAddrLen(tt.pfxs))
		l = addrPrefixLen(tt.pfxs, tt.ip)
		if l != tt.out {
			t.Errorf("#%d: got %d; want %d", i, l, tt.out)
		}
