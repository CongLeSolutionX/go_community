...
func Test_readMsg(t *testing.T) {
	...
	tempDir, err := ioutil.TempDir("", "Test_readMsg")
	if err != nil {
		t.Fatal(err)
	}
	...
	cmd := exec.Command(os.Args[0], "-test.run=^Test_readMsg$", "--", tempDir)
	...
	_, oobn, _, _, err := uc.readMsg(buf, oob)
	if err != nil {
...
// passFDChild is the child process used by Test_readMsg.
func passFDChild() {
	defer os.Exit(0)
