...
The method for resolving domain names, whether indirectly via functions like Dial
or directly via functions like LookupHost and LookupAddr, varies by operating system.

On Unix systems, the resolver has two options for resolving names.
It can use a pure Go resolver that sends DNS requests directly to the servers listed in /etc/resolv.conf, or
...
when /etc/resolv.conf or /etc/nsswitch.conf specify the use of features that
the Go resolver does not implement,
...
The decision can also be forced while building of the Go source tree
by setting the netgo or netcgo build tag.

A numeric netdns setting, as in GODEBUG=netdns=1, causes the resolver
to print debugging information about its decisions.
To both force a particular resolver and print debugging information,
