...
func TestUnixAndUnixpacketServer(t *testing.T) {
	...
	for i, tt := range unixAndUnixpacketServerTests {
		...
		for i := 0; i < N; i++ {
			...
			// solaris-amd64-oraclerel builder. The source of the nil panic is not
			// obvious because there are many nillable types involved, so we will
			// temporarily inspect all of them.
			checkNils := func() {
				if c == nil {
