...
// putCIDRMask will return m when it has at least l bytes of len, otherwise
// returns a new slice.
func putCIDRMask(m IPMask, ones, l int) IPMask {
	if len(m) < l {
		m = make(IPMask, l)
	}
...
func (ip IP) Mask(mask IPMask) IP {
	return putIPMask(nil, ip, mask)
}

// putIPMask will return out when it has sufficient len for the result
// of masking the IP address ip with mask, otherwise returning a new slice.
func putIPMask(out, ip IP, mask IPMask) []byte {
...
func ParseCIDR(s string) (IP, *IPNet, error) {
	...
	ip := IP(buf[:IPv6len:IPv6len])
	buf = buf[IPv6len:]

	m := putCIDRMask(buf[:iplen:iplen], nwlen, iplen)
	buf = buf[iplen:]

	nw := putIPMask(buf[:iplen:iplen], ip, m)
	net := &IPNet{IP: nw, Mask: m}
	return ip, net, nil
}
