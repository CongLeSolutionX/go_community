...
func cgoLookupPTRName(addr string) ([]string, error, bool) {
	...
	var b []byte
	var errno C.int
	for l := C.NI_MAXHOST; l < 2*C.NI_MAXHOST+1; l += C.NI_MAXHOST {
		b = make([]byte, l)
		errno = C.getnameinfo(sa, salen, (*C.char)(unsafe.Pointer(&b[0])), C.socklen_t(l), nil, 0, C.NI_NAMEREQD)
		if errno == 0 || errno != C.EAI_OVERFLOW {
			break
		}
	}
	if errno != 0 {
