...
// the public interface by constructing a *os.File from a file descriptor opened
// on a socket, but the WASI preview 1 specification is too limited to support
// this approach for UDP sockets. Instead, we test the internals that make it
// possible for WASI host runtimes and guest programs to integrate socket
// extensions with the net package using net.FileConn and net.FileListener.
//
// Note that the creation of net.Conn and net.Listener values for TCP sockets
// has a end-to-end test in src/runtime/internal/wasitest, here we are only
...
func TestWasip1NewFileConn(t *testing.T) {
	if c, ok := newFileConn(newFD("tcp", -1)).(*TCPConn); !ok {
		...
		assertTCPAddr(t, "LocalAddr", c.LocalAddr())
		assertTCPAddr(t, "RemoteAddr", c.RemoteAddr())
	}
	if c, ok := newFileConn(newFD("udp", -1)).(*UDPConn); !ok {
		t.Errorf("newFileConn: udp conn type mismatch: %T", c)
	} else {
		assertUDPAddr(t, "LocalAddr", c.LocalAddr())
		assertUDPAddr(t, "RemoteAddr", c.RemoteAddr())
	}
}

func assertTCPAddr(t *testing.T, method string, addr Addr) {
	if _, ok := addr.(*TCPAddr); !ok {
		t.Errorf("%s: returned address is not a *TCPAddr: %T", method, addr)
	}
}

func assertUDPAddr(t *testing.T, method string, addr Addr) {
