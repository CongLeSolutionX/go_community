...
func TestTCPFastOpenDialerTimeout(t *testing.T) {
	...
	for _, tmo := range []time.Duration{-3 * time.Second, 0} {
		...
		// timeout, deadline values and must ignore write
		// deadline values.
		c.SetDeadline(aLongTimeAgo)
		c.SetWriteDeadline(aLongTimeAgo)
		_, err = c.Write([]byte("TCP FATOPEN FIRST WRITE TIMEOUT TEST"))
		if err == nil {
			if d.Timeout != 0 {
				t.Fatal("should fail")
			}
...
func TestTCPFastOpenDialerCancel(t *testing.T) {
	if !supportsPassiveTCPFastOpen || !supportsActiveTCPFastOpen {
		t.Skip("TCP fast open is not supported")
	}
	...
	d := Dialer{FastOpen: true, Cancel: cancel}
	c, err := d.Dial(ln.Addr().Network(), ln.Addr().String())
	if err != nil {
		if perr := parseDialError(err); perr != nil {
			t.Error(perr)
