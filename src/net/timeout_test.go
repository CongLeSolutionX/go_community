...
func TestReadTimeoutFluctuation(t *testing.T) {
	...
	for {
		...
		if actual > 2*d {
			if t.Failed() || actual > maxDynamicTimeout || d >= maxDynamicTimeout/2 {
			...
			t.Logf("Read took longer than %v; trying with longer timeout", 2*d)
			d *= 2
			continue
		}
...
func TestReadFromTimeoutFluctuation(t *testing.T) {
	...
	for {
		...
		if actual > 2*d {
			...
			// Maybe this machine is too slow to reliably schedule goroutines within
			// the requested duration. Increase the timeout and try again.
			t.Logf("ReadFrom took longer than %v; trying with longer timeout", 2*d)
			d *= 2
			continue
		}
...
func TestWriteTimeoutFluctuation(t *testing.T) {
	...
	for {
		...
		if actual > 2*d {
			...
			// Maybe this machine is too slow to reliably schedule goroutines within
			// the requested duration. Increase the timeout and try again.
			t.Logf("Write took longer than %v; trying with longer timeout", 2*d)
			d *= 2
			continue
		}
