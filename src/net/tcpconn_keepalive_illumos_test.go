...
func verifyKeepAliveSettings(t *testing.T, fd fdType, oldCfg, cfg KeepAliveConfig) {
	const tcpKeepAliveAbortThreshold = 8 * time.Minute // default value on illumos
	...
	case cfg.Interval == -1 && cfg.Count > 0:
		cfg.Interval = tcpKeepAliveAbortThreshold / time.Duration(oldCfg.Count)
	case cfg.Count == -1 && cfg.Interval > 0:
		cfg.Count = int(tcpKeepAliveAbortThreshold / oldCfg.Interval)
	}
