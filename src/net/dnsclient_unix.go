...
// tryOneName does DNS lookups for name and returns a canonical name
// and a list of resource records.
// Name must be rooted.
// When err is not nil and completed is false, the caller may continue
// to do lookup with subsequent suffixed names.
func tryOneName(cfg *dnsConfig, name string, qtype uint16) (cname string, rrs []dnsRR, err error, completed bool) {
	if len(cfg.servers) == 0 {
		return "", nil, &DNSError{Err: "no DNS servers", Name: name}, true
	}
...
func lookupDNS(name string, qtype uint16) (cname string, rrs []dnsRR, err error) {
	...
	for _, name := range names {
		cname, rrs, err, completed = tryOneName(conf, name, qtype)
		if rooted || completed {
			break
		}
	}
	...
	for _, name := range names {
		for _, qtype := range qtypes {
			go func(qtype uint16) {
				_, rrs, err, completed := tryOneName(conf, name, qtype)
				lane <- racer{rrs, err, completed}
			}(qtype)
		}
		for range qtypes {
			racer := <-lane
