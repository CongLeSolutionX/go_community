...
func dialUnix(network string, laddr, raddr *UnixAddr, deadline time.Time) (*UnixConn, error) {
	return nil, &OpError{Op: "dial", Net: network, Source: laddr.opAddr(), Addr: raddr.opAddr(), Err: syscall.EPLAN9}
}
...
func listenUnix(network string, laddr *UnixAddr) (*UnixListener, error) {
	return nil, &OpError{Op: "listen", Net: network, Source: nil, Addr: laddr.opAddr(), Err: syscall.EPLAN9}
}
