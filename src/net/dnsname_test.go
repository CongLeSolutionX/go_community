...
func emitDNSNameTest(ch chan<- dnsNameTest) {
	...
	char64 := char63 + "a"
	long_domain := strings.Repeat(char63+".", 5) + "example"
	...
	// 253 is fine:
	ch <- dnsNameTest{long_domain[len(long_domain)-253:], true}
	// 254 is fine (for RFC 6762 Multicast DNS):
	ch <- dnsNameTest{long_domain[len(long_domain)-254:], true}
	// Terminal dots don't contribute to length
	ch <- dnsNameTest{long_domain[len(long_domain)-254:] + ".", true}
	// 255 is bad:
	ch <- dnsNameTest{long_domain[len(long_domain)-255:], false}
}
