	...
	if tc.limitReadSize == 0 || tc.downNet == "tcp" {
		...
		if err := rc.Control(func(fd uintptr) {
			if hook.called && hook.dstfd != int(fd) {
				t.Errorf("wrong destination file descriptor: got %d, want %d", hook.dstfd, int(fd))
			}
		}); err != nil {
			t.Fatalf("syscall.RawConn.Control error: %v", err)
		}

		sc, ok = serverUp.(syscall.Conn)
		if !ok {
			t.Fatalf("expected syscall.Conn")
		}
		rc, err = sc.SyscallConn()
		...
		if err := rc.Control(func(fd uintptr) {
			if hook.called && hook.srcfd != int(fd) {
				t.Errorf("wrong destination file descriptor: got %d, want %d", hook.srcfd, int(fd))
			}
		}); err != nil {
			t.Fatalf("syscall.RawConn.Control error: %v", err)
		}
