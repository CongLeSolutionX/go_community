...
// AddrFromSlice returns an IP from the standard library's IP type.
// If std is invalid, ok is false.
// AddrFromSlice not do an implicit Unmap if
// len(std) == 16 and contains an IPv6-mapped IPv4 address.
func AddrFromSlice(std []byte) (ip Addr, ok bool) {
	switch len(std) {
	case 4:
...
// For the zero value, it returns 0.
...
//
// Note that the Addr zero value is not an unspecified address.
...
func (ip Addr) Prefix(bits int) (Prefix, error) {
	effectiveBits := bits
	switch ip.z {
	case z0:
		return Prefix{}, nil
	case z4:
		if bits > 32 {
			return Prefix{}, errors.New("prefix length " + itoa.Itoa(bits) + " too large for IPv4")
		}
		effectiveBits += 96
	default:
		if bits > 128 {
			return Prefix{}, errors.New("prefix length " + itoa.Itoa(bits) + " too large for IPv6")
		}
	}
...
// PrefixFrom returns an Prefix with IP ip and bit prefix length.
//
// It does not allocate.
//
// If bits is less than zero or greater than 255, an invalid value 255
// is used instead.
...
func (p Prefix) IsValid() bool { return !p.ip.isZero() && int(p.bits) <= p.ip.BitLen() }
