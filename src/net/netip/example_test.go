...
func ExampleAddrFromSlice() {
	...
	ip, _ := netip.AddrFromSlice(b16)
	fmt.Println(ip)

	b4 := []byte{8, 8, 8, 8}
	ip, _ = netip.AddrFromSlice(b4)
	fmt.Println(ip)

	invalidIP := []byte{8, 8, 8, 8, 8}
	ip, _ = netip.AddrFromSlice(invalidIP)
	fmt.Println(ip)
	// Output:
	// fc00::
	// 8.8.8.8
...
func ExampleIPv6Unspecified() {
	fmt.Println(netip.IPv6Unspecified())
	// Output:
	// ::
}
...
func ExampleParseAddr() {
	...
	ip, _ = netip.ParseAddr("2001:0000:0000:0000:0000:0000:0000:1234")
	fmt.Println(ip)

	ip, _ = netip.ParseAddr("fe80::1cc0:3e8c:119f:c2e1%ens18")
	fmt.Println(ip)

	ip, _ = netip.ParseAddr("8.8.meow.8")
	fmt.Println(ip)
	// Output:
	// 8.8.8.8
	// 2001::1234
...
func ExampleAddr_WithZone() {
	...
	fmt.Println(ipv4.WithZone(zone))
	fmt.Println(ipv6.WithZone(zone))
	fmt.Println(ipv6WithZone.WithZone(zone))
	fmt.Println(ipv6WithZone.WithZone(emptyZone))
	// Output:
	// 8.8.8.8
	// 6::2%a%3
	// 6::4%a%3
