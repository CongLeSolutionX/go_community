...
func TestAddressParsingAndFormatting(t *testing.T) {
	...
	tests := []string{
		...
		//`<üñîçøðé@example.com>`, // unicode chars
	}

	for _, test := range tests {
		adr, err := ParseAddress(test)
		if err != nil {
			t.Errorf("Couldn't parse address %s: %s", test, err.Error())
			continue
		}
		str := adr.String()
		adr, err = ParseAddress(str)
		if err != nil {
			t.Errorf("Couldn't parse address %s (formatted as %s) again after formatting: %s", test, str, err.Error())
			continue
		}

		if adr.String() != test {
			t.Errorf("Addresses didn't match after parsing again. Expected: %s, Received: %s", test, adr.String())
			continue
		}
