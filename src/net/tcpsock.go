...
//
// Also note that on Solaris and OpenSolaris derivatives like illumos that support
// TCP Keep-Alive, setting only one of Idle and Interval to a non-negative value while
// setting the other to a negative value will result in the negative one being recalculated
// as the quotient of tcp_keepalive_abort_interval(eight minutes as default) and the
// non-negative one. Thus, you may as well set the other one to a non-negative value
// if you've already set one of Idle and Interval.
