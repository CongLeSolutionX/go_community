...
func internetSocket(network string, laddr, raddr sockaddr, sotype, proto int, mode string, fastopen bool, deadline time.Time, cancel <-chan struct{}) (fd *netFD, err error) {
	family, ipv6only := favoriteAddrFamily(network, laddr, raddr, mode)
	return socket(network, family, sotype, proto, ipv6only, fastopen, laddr, raddr, deadline, cancel)
}
