...
func internetSocket(ctx context.Context, network string, laddr, raddr sockaddr, sotype, proto int, mode string, ctrlFn func(string, string, syscall.RawConn) error) (fd *netFD, err error) {
	if (runtime.GOOS == "windows" || runtime.GOOS == "openbsd" || runtime.GOOS == "nacl") && mode == "dial" && raddr.isWildcard() {
		raddr = raddr.toLocal(network)
	}
	family, ipv6only := favoriteAddrFamily(network, laddr, raddr, mode)
	return socket(ctx, network, family, sotype, proto, ipv6only, laddr, raddr, ctrlFn)
}
