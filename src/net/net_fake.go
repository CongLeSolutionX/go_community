...
func resolvedAddrIsValid(sa sockaddr) bool {
	...
	case *UDPAddr:
		return (len(sa.IP) == 4 || len(sa.IP) == 16) && sa.Port > 0 && sa.Port < 1<<16
	case *UnixAddr:
		return sa.Name == "" || (filepath.IsAbs(sa.Name) && sa.Name == filepath.Clean(sa.Name))
	default:
		return true
	}
}
...
func fakeConnect(fd *netFD, laddr, raddr sockaddr) error {
	...
	case syscall.SOCK_DGRAM:
		if fd.family == syscall.AF_UNIX {
			name := fd.raddr.(*UnixAddr).Name
			if _, err := os.Stat(filepath.Dir(name)); err != nil {
				return os.NewSyscallError("connect", syscall.EACCES)
			}
		}
...
func (ffd *fakeNetFD) writeTo(p []byte, sa syscall.Sockaddr) (n int, err error) {
	...
	if peeri == nil {
		if strings.HasPrefix(ffd.fd.net, "udp") {
			return len(p), nil
		}
