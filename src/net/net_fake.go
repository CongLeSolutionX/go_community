...
type fakeNetFD struct {
	...
	// incoming holds incoming connections that have not yet been accepted.
	// It is nil if the FD is not listening for connections, and closed when the
	// FD's Listener is closed.
	incoming      chan []*netFD // 1-buffered
	incomingEmpty chan bool     // 1-buffered; holds true when the incoming queue is empty
}
...
func validateResolvedAddr(family int, sa sockaddr) error {
...
func assignFakeAddr(net string, family int, addr sockaddr, defaultPort int) (sockaddr, error) {
	validate := func(sa sockaddr) (sockaddr, error) {
		if err := validateResolvedAddr(family, sa); err != nil {
			return nil, err
		}
		return sa, nil
	}
...
func newFakeNetFD(fd *netFD) *fakeNetFD {
	...
	for {
		...
		if port >= 1<<16 {
			nextPort.CompareAndSwap(port, 1)
			continue
		}
		ffd.reservedPort = int(port)
...
func fakeListen(fd *netFD, laddr sockaddr) (err error) {
	...
	fd.laddr, err = assignFakeAddr(fd.net, fd.family, matchIPFamily(fd.family, laddr), ffd.reservedPort)
	if err != nil {
		return wrapErr(err)
	}
...
func fakeConnect(fd *netFD, laddr, raddr sockaddr) error {
	...
	if err := validateResolvedAddr(fd.family, fd.raddr.(sockaddr)); err != nil {
		return err
	}
