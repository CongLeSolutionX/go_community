...
type Dialer struct {
	...
	// necesserily the ones passed to Dial.
...
type ListenConfig struct {
	...
	//
	// Network and address parameters passed to Control method are not
	// necesserily the ones passed to Listen.
	Control func(network, address string, c syscall.RawConn) error
}
...
func (lc *ListenConfig) Listen(ctx context.Context, network, address string) (Listener, error) {
	...
	switch la.(type) {
	case *TCPAddr:
		l, err = sl.listenTCP(ctx, la.(*TCPAddr))
	case *UnixAddr:
		l, err = sl.listenUnix(ctx, la.(*UnixAddr))
	default:
...
func (lc *ListenConfig) ListenPacket(ctx context.Context, network, address string) (PacketConn, error) {
	...
	switch la.(type) {
	case *UDPAddr:
		c, err = sl.listenUDP(ctx, la.(*UDPAddr))
	case *IPAddr:
		c, err = sl.listenIP(ctx, la.(*IPAddr))
	case *UnixAddr:
		c, err = sl.listenUnixgram(ctx, la.(*UnixAddr))
	default:
		return nil, &OpError{Op: "listen", Net: sl.network, Source: nil, Addr: la, Err: &AddrError{Err: "unexpected address type", Addr: address}}
	}
	if err != nil {
