...
// The zero value for deadline is ready for use.
...
func (d *deadline) Waiter() chan struct{} {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.cancel == nil {
		d.cancel = make(chan struct{})
...
var (
	errDeadline = pipeError{errors.New("pipe: deadline exceeded"), true}
	errClosed   = pipeError{errors.New("pipe: closed connection"), false}
)
