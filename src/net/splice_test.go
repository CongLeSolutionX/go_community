...
func (bench spliceFileBench) benchSpliceFile(b *testing.B) {
	f, err := os.CreateTemp(b.TempDir(), "linux-splice-to-file")
	if err != nil {
		b.Fatal(err)
	}
	defer f.Close()

	// TODO(panjf2000): we should probably refactor this benchmark to use a fixed size of bytes?
	// This will generate a file with totalSize bytes on disk.
	// In general b.N can be arbitrarily large (based on -benchtime), so that seems like it will
	// cause the benchmark to fail if the benchmarks are run for a long time to reduce noise.
