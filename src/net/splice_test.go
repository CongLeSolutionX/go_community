...
func TestSplice(t *testing.T) {
	...
	t.Run("src at EOF", testSpliceHandlesSrcAtEOF)
}
...
func testSpliceHandlesSrcAtEOF(t *testing.T) {
	clientUp, serverUp, err := spliceTestSocketPair("tcp", ":0")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !handled {
		t.Errorf("got err = %v, handled = %t for exhausted LimitedReader, want handled = true", err, handled)
	}
}
