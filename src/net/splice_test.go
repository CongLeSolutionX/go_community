...
func (bench spliceFileBench) benchSpliceFile(b *testing.B) {
	...
	// This might generate a file with totalSize bytes, in general b.N can be
	// arbitrarily large (based on -benchtime), so that seems like it will cause
	// the benchmark to fail if the benchmarks are run for a long time to reduce noise.
