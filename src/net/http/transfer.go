...
func (t *transferReader) fixTransferEncoding() error {
	...
	//   "identity", "chunked" and "gzip"/"x-gzip"
	// encodings, the loop below is designed with foresight.
	//
	// As per RFC 7230 Section 3.3.1 Transfer-Encoding, one invariant
	// that must be maintained is that, if present, chunked encoding
	// must always come last and it MUST be applied only once.
	for i, encoding := range encodings {
		...
		if encoding == "identity" {
			...
			if len(encodings) != 1 {
				return fmt.Errorf(`http: "identity" when present must be the only transfer encoding; got %v`, encodings)
			}
		...
		case encoding == "chunked":
			...
			if i+1 != len(encodings) {
				return fmt.Errorf("http: chunked must be applied only once and as the last encoding; got %v", encodings)
			}
			// Supported otherwise.
