...
`,
			wantErr: "http: chunked must be applied only once and as the last encoding; got [chunked  gzip]",
		},
...
` + "0\r\n\r\n",
			wantErr: `http: "identity" when present must be the only transfer encoding; got [identity  identity]`,
		},
	}
