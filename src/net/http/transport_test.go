...
// A cancelTest is a test of request cancelation.
		...
		f(t, cancelTest{
			...
			checkErr: func(when string, err error) {
				if !errors.Is(err, ExportErrRequestCanceled) && !errors.Is(err, ExportErrRequestCanceledConn) {
					t.Errorf("%v error = %v, want errRequestCanceled", when, err)
				}
			},
		})
	})
...
// runCancelTestTransport uses Request.Cancel.
...
// runCancelTestTransport uses a request context.
