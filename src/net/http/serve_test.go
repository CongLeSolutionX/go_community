...
func TestStripPrefix(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if err != nil {
			t.Errorf("%v: %v", tc.reqPath, err)
			continue
		}
		res.Body.Close()
		if tc.path == "" {
			if res.StatusCode != StatusNotFound {
				t.Errorf("%v: got %q, want 404 Not Found", tc.reqPath, res.Status)
			}
			continue
		}
		if res.StatusCode != StatusOK {
			t.Errorf("%v: got %q, want 200 OK", tc.reqPath, res.Status)
			continue
		}
		if g, w := res.Header.Get("X-Path"), tc.path; g != w {
			t.Errorf("%v: got Path %q, want %q", tc.reqPath, g, w)
		}
		if g, w := res.Header.Get("X-RawPath"), tc.rawPath; g != w {
			t.Errorf("%v: got RawPath %q, want %q", tc.reqPath, g, w)
		}
	}
}
