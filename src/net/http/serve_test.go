...
func testServerWriteTimeout(t *testing.T, mode testMode) {
	for timeout := 1 * time.Millisecond; ; timeout *= 2 {
		...
		//
		// To avoid that problem, we add an explicit timeout to the request, but
		// make it much longer than the server's write timeout so that the server
		// should always time out first.
		cst.c.Timeout = timeout * 10
