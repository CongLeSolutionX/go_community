...
// previously-registered hooks in old and the new ones in new,
...
func compose(new, old *ClientTrace) *ClientTrace {
	...
	for i := 0; i < structType.NumField(); i++ {
		...
		if of.IsNil() && !nf.IsNil() {
			// Case 1: old does not have hook and new has
			// Then, take new hook
			lf.Set(nf)
		} else if !of.IsNil() && nf.IsNil() {
			// Case 2: old has hook and new does not
			// Then, take old hook
			lf.Set(of)
		} else if !of.IsNil() && !nf.IsNil() {
			// Case 3: old and new both have hook
			// Then, both hook functions have to be called.
