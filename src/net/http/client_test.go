...
func TestClientRedirectsContext(t *testing.T) {
	run(t, testClientRedirectsContext)
}
...
func TestClientRedirectUseResponse(t *testing.T) {
	run(t, testClientRedirectUseResponse)
}
...
func TestClientRedirect308NoGetBody(t *testing.T) {
	run(t, testClientRedirect308NoGetBody)
}
...
func testClientRedirect308NoGetBody(t *testing.T, mode testMode) {
	...
	req, err := NewRequest("POST", cst.ts.URL, strings.NewReader("some body"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRedirectCookiesJar(t *testing.T) {
	run(t, testRedirectCookiesJar)
}

func testRedirectCookiesJar(t *testing.T, mode testMode) {
	setParallel(t)
	cst := newClientServerTest(t, mode, echoCookiesRedirectHandler)
	c := cst.c
	c.Jar = new(TestJar)
...
func TestJarCalls(t *testing.T) {
	run(t, testJarCalls, []testMode{http1Mode})
}
...
func TestClientWrites(t *testing.T) {
	run(t, testClientWrites, []testMode{http1Mode})
}
...
func testClientWithCorrectTLSServerName(t *testing.T, mode testMode) {
	...
	if _, err := c.Get(cst.ts.URL); err != nil {
...
func TestResponseSetsTLSConnectionState(t *testing.T) {
	run(t, testResponseSetsTLSConnectionState, []testMode{https1Mode, http2Mode})
}

func testResponseSetsTLSConnectionState(t *testing.T, mode testMode) {
	if mode == http2Mode {
		// TODO(dneil): Is this a real failure?
		// Or does this test just not work with anything other than HTTP/1?
		t.Skip("fails in HTTP/2")
	}
	ts := newClientServerTest(t, mode, HandlerFunc(func(w ResponseWriter, r *Request) {
		w.Write([]byte("Hello"))
	})).ts
...
func TestClientHeadContentLength(t *testing.T) {
	run(t, testClientHeadContentLength)
}
...
func TestClientDoCanceledVsTimeout(t *testing.T) {
	run(t, testClientDoCanceledVsTimeout)
}
