...
//
// The path and host are used unchanged for CONNECT requests except redirection.
//
...
func (mux *ServeMux) Handler(r *Request) (h Handler, pattern string) {
	...
	n := len(path)
	if _, exist := mux.m[path]; !exist && n > 0 && path[n-1] != '/' && mux.m[path+"/"].explicit {
		path = path + "/"
		url := &url.URL{Path: path, RawQuery: r.URL.RawQuery}
		return RedirectHandler(url.String(), StatusMovedPermanently), path
	}

	// CONNECT requests are not canonicalized except redirection.
	if r.Method == "CONNECT" {
