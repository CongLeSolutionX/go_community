...
func (srv *Server) SetKeepAlivesEnabled(v bool) {
	...
	if !v {
		srv.closeIdleConns() // close http1 conns
	}

	srv.mu.Lock() // for modifying/closing doneChan
	defer srv.mu.Unlock()
	if v {
		// If we weren't in shutdown already (new value == 1),
		// then reset the done channel.
		if atomic.AddInt32(&srv.inShutdown, 1) == 1 {
			srv.doneChan = nil
		}
