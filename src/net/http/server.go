...
func (w *response) ReadFrom(src io.Reader) (n int64, err error) {
	...
	if !w.cw.chunking && w.bodyAllowed() && w.req.Method != MethodHead {
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	if w.req.Method == MethodHead || !bodyAllowedForStatus(code) || code == StatusNoContent {
...
func (sh serverHandler) ServeHTTP(rw ResponseWriter, req *Request) {
	...
	if !sh.srv.DisableGeneralOptionsHandler && req.RequestURI == "*" && req.Method == MethodOptions {
