...
type Server struct {
	...
	waitingShutdown chan struct{}
}

func init() {
	http2h1ServerShutdownChan = (*Server).getDoneChan
}
...
func (s *Server) Shutdown(ctx context.Context) error {
	defer s.SetKeepAlivesEnabled(s.doKeepAlives())
	s.SetKeepAlivesEnabled(false) // before s.mu is locked
...
// notifyMaybeShutdown is called by events which might cause Shutdown
// to wake up sooner.
func (s *Server) notifyMaybeShutdown() {
	s.mu.Lock()
	s.notifyMaybeShutdownLocked()
	s.mu.Unlock()
}

func (s *Server) notifyMaybeShutdownLocked() {
	ch := s.waitingShutdown
	if ch != nil {
		select {
		case ch <- struct{}{}:
