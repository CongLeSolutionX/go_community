...
func (root *routingNode) match(host, method, path string) (*routingNode, []string) {
	if host != "" {
		...
		// This implements the special case to our "more specific wins" precedence
		// that is required for backwards compatibility: patterns with a host take
		// precedence over those without one.
		if n, m := root.findChild(host).matchMethodAndPath(method, path); n != nil {
			return n, m
		}
	}
...
// matchMethodAndPath matches just the method and path.
...
// matchPath matches just the path.
// Its return values are the same as [routingNode.match].
