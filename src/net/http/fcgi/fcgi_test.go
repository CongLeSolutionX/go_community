...
func makeNameValuePair11(nameData, valueData []byte) []byte {
	return bytes.Join(
		[][]byte{
...
var streamBeginTypeStdin = bytes.Join([][]byte{
	...
	makeRecord(typeParams, 1,
		makeNameValuePair11([]byte("REQUEST_METHOD"), []byte("GET"))),
	makeRecord(typeParams, 1,
		makeNameValuePair11([]byte("SERVER_PROTOCOL"), []byte("HTTP/1.1"))),
