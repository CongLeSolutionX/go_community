...
func TestAcceptIgnoreBrokenConnRequest(t *testing.T) {
	...
	case "darwin":
		...
		case "arm", "arm64":
			t.Skip("not supported on %s/%s", runtime.GOOS, runtime.GOARCH)
		}
	}
	...
	// kill it because it's an easy way to make TCP SYN_RCVD state
	...
	// Give a chance to send TCP RESET segment to the kernel.
