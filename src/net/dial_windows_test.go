...
func TestDialContextCancelRace(t *testing.T) {
	oldConnectFunc := poll.ConnectExFunc
	oldTestHookCanceledDial := testHookCanceledDial
	defer func() {
		poll.ConnectExFunc = oldConnectFunc
	...
	poll.ConnectExFunc = func(s syscall.Handle, sa syscall.Sockaddr, b *byte, n uint32, nwr *uint32, o *syscall.Overlapped) error {
		err := oldConnectFunc(s, sa, b, n, nwr, o)
		t.Logf("connectex(...) = %v", err)
		cancelCtx()
