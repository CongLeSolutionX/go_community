...
func parseHost(host string) (string, error) {
	...
	if strings.HasPrefix(host, "[") {
		...
		// IPv6 address textual representation but in URI we
		// need to use "%25" as the encoding of "%".
		// See RFC 6874 for further information.
		i := strings.Index(host[1:], "]")
		if i < 0 {
			return "", errors.New("missing ']' in host")
		}
		// We need to parse a host subcomponent without a
		// ZoneID in RFC 6874 because the ZoneID is allowed to
