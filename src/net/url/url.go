...
//
// If the scheme is not provided, then the URL is typically parsed as starting
// with a path. Should the host be part of the URL, then the URL must be
// prefixed with //. For example:
//
//	"/pkg/net/url/"              =>  &URL{Path:"/pkg/net/url/"}
//	"//golang.org/pkg/net/url/"  =>  &URL{Host:"golang.org", Path:"/pkg/net/url/"}
//
// Note that the Path field is stored in decoded form: /%47%6f%2f becomes /Go/.
// A consequence is that it is impossible to tell which slashes in the Path were
// slashes in the raw URL and which were %2f. This distinction is rarely important,
// but when it is, code must not use Path directly. If URL decoding was
// performed while parsing, the original string is stored in RawPath.
// If the RawPath is populated and properly decodes to Path, then its value
// will be used by by the String method.
type URL struct {
	Scheme     string
	Opaque     string    // encoded opaque data; must not start with a "/"
	User       *Userinfo // username and password information
	Host       string    // host or host:port
	Path       string    // hierarchal path; must not start with a "//"
...
// and password.
//
// This should only be used with legacy web sites as RFC 2396 warns:
