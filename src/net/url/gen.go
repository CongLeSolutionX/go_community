...
const (
	...
	_count // used only inside this file
)

func main() {
	// 256 bytes (ishex) + (_count-1) * 256 bytes (shouldEscape) + 256 bytes (unhex)
	lookUpTable := make([]byte, (_count+1)*256)
	...
	// shouldEscape.
	for mode := encodePath; mode < _count; mode++ {
		for c := 0; c < 256; c++ {
	...
	// unhex.
	for c := 0; c < 256; c++ {
		lookUpTable[c+int(_count*256)] = unhex(byte(c))
	}

	w := new(bytes.Buffer)
	fmt.Fprintf(w, content, string(lookUpTable), _count*256)
...
func ishex(c byte) bool {
	return lookUpTable[uint(c)] != 0
}

func unhex(c byte) byte {
	return lookUpTable[%v:][uint(c)]
}
