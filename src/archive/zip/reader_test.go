...
type ZipTest struct {
	...
	Comment       string
	File          []ZipTestFile
	Base64Encoded bool  // needed for Apple notarization (golang.org/issue/34986)
...
var tests = []ZipTest{
	...
	},
	{
		...
		// Use base64-encoded file to avoid Apple’s notarization service
...
func readTestZip(t *testing.T, zt ZipTest) {
	...
	if zt.Source != nil {
		...
		if zt.Base64Encoded {
			tf, err := base64.DecodeToTempFile(path)
			if err != nil {
				t.Errorf("base64.DecodeToTempFile(%s): %v", path, err)
				return
			}
			defer os.Remove(tf)
			path = tf
