...
// decompression. If the file has a trailing data descriptor, as determined
// from the file's header, a DataDescriptor will be unmarshalled and returned.
func (f *File) OpenRaw() (io.Reader, *DataDescriptor, error) {
	bodyOffset, err := f.findBodyOffset()
	if err != nil {
		return nil, nil, err
	}
	...
	if f.hasDataDescriptor() {
		desr := io.NewSectionReader(f.zipr, f.headerOffset+bodyOffset+size, f.dataDescriptorLen())
		desc, err = readDataDescriptor(desr, f)
	}
	return r, desc, err
}
