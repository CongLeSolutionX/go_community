...
func TestHeaderAllowedFormats(t *testing.T) {
	...
	vectors := []struct {
		...
		header:  &Header{Uid: 1<<56 - 1},
		paxHdrs: map[string]string{paxUid: "72057594037927935"},
		formats: formatPAX | formatGNU,
	}, {
		header:  &Header{Uid: 1 << 56},
		paxHdrs: map[string]string{paxUid: "72057594037927936"},
		formats: formatPAX,
	}, {
	...
	for i, v := range vectors {
		...
		if formats != v.formats {
			t.Errorf("test %d, checkAllowedFormats(...): got %v, want %v", i, prettyFormat(formats), prettyFormat(v.formats))
		}
		if formats&formatPAX > 0 && !reflect.DeepEqual(paxHdrs, v.paxHdrs) && !(len(paxHdrs) == 0 && len(v.paxHdrs) == 0) {
			t.Errorf("test %d, checkAllowedFormats(...):\ngot  %v\nwant %s", i, paxHdrs, v.paxHdrs)
		}
	}
}
