...
// This algorithm has the desireable property that it moves each element exactly twice.
...
// overlaps reports whether &a[i] == &b[j] for any 0<=i<len(a) and 0<=j<len(b).
// a and b must be nonempty.
func overlaps[S ~[]E, E any](a, b S) bool {
	return uintptr(unsafe.Pointer(&a[0])) <= uintptr(unsafe.Pointer(&b[len(b)-1])) &&
		uintptr(unsafe.Pointer(&b[0])) <= uintptr(unsafe.Pointer(&a[len(a)-1]))
}
