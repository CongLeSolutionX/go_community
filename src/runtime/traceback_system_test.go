...
// This file defines a test of GOTRACEBACK=system.
// It is in its own file to minimize line number perturbation.

// TODO read the file to avoid the line_number problems.
...
// by GOTRACEBACK=system (see func traceback2) contains a complete,
// parseable list of program counters for the running goroutine that
// can (after suitable munging) be fed to runtime.CallersFrames to
// obtain accurate information about the logical call stack, even in
...
func TestTracebackSystem(t *testing.T) {
	...
	cmd.Env = append(os.Environ(), entrypointVar+"=crash")
	...
	got := formatStack(pcs)
	want := `runtime.gopanic:+59
runtime_test.grandchild:+1
runtime_test.child:+1
runtime_test.crash.func1:+1
runtime.goexit:+0`
	if got != want {
		t.Errorf("got:\n%swant:\n%s", got, want)
	}
