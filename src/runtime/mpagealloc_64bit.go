...
//
// With levelShift, one can discover the summary at level l related to apointer p
// by doing:
//   p >> levelShift[l]
...
func (s *pageAlloc) sysInit(sysStat *uint64) {
	...
	for l, shift := range levelShift {
		...
		b := alignUp(uintptr(entries)*mallocSumBytes, physPageSize)
		r := sysReserve(nil, b)
		if r == nil {
...
// for summaries. It is also responsible for updating length of
// the slices in s.summay.
//
...
// Both must be aligned to mallocChunkBytes.
//
// sysGrow must be called before s.start and s.end are updated.
func (s *pageAlloc) sysGrow(base, limit uintptr, sysStat *uint64) {
	...
	for l := len(s.summary) - 1; l >= 0; l-- {
		...
		extendMappedRegion(
			...
			uintptr(mlo)*mallocSumBytes,
			uintptr(mhi)*mallocSumBytes,
			uintptr(lo)*mallocSumBytes,
			uintptr(hi)*mallocSumBytes,
			sysStat,
		)
	}
}
