...
func (s *scavengeIndex) grow(base, limit uintptr, sysStat *sysMemStat) uintptr {
	...
	haveMax := s.max.Load()
	have := makeAddrRange(
		uintptr(unsafe.Pointer(&s.chunks[haveMin])),
		uintptr(unsafe.Pointer(&s.chunks[haveMax])),
	)
	...
	need := makeAddrRange(
		uintptr(unsafe.Pointer(&s.chunks[needMin])),
		uintptr(unsafe.Pointer(&s.chunks[needMax])),
	)
