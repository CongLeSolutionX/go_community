...
func (a *addrRanges) removeLastChunks(nchunks uint) addrRange {
	...
	chunksInLast := uint(alignUp(last.size(), pallocChunkBytes) / pallocChunkBytes)
	if chunksInLast > nchunks {
		...
		newEnd := alignDown(last.end()-1, pallocChunkBytes) - uintptr(nchunks-1)*pallocChunkBytes
		a.ranges[len(a.ranges)-1] = makeAddrRange(last.start(), newEnd)
		a.totalBytes -= last.end() - newEnd
		return makeAddrRange(newEnd, last.end())
	}
	a.ranges = a.ranges[:len(a.ranges)-1]
