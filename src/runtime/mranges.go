...
// offAddrBox is like offAddr, but operations on it are atomic.
...
type offAddrBox struct {
	// a contains the offset address, unlike offAddr.
	a atomic.Int64
}

// Clear attempts to store minOffAddr in offAddrBox. It may fail
// if a marked value is placed in the box in the meanwhile.
func (b *offAddrBox) Clear() {
	for {
		old := b.a.Load()
		if old < 0 {
			return
...
func (b *offAddrBox) StoreMin(addr uintptr) {
...
// StoreUnmark attempts to unmark the value in offAddrBox and
...
func (b *offAddrBox) StoreUnmark(markedAddr, newAddr uintptr) {
...
func (b *offAddrBox) StoreMarked(addr uintptr) {
...
func (b *offAddrBox) Load() (uintptr, bool) {
	v := b.a.Load()
	wasMarked := false
	if v < 0 {
		wasMarked = true
