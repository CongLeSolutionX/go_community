...
func (a addrRange) contains(addr uintptr) bool {
	...
	return !(offAddr{addr}).lessThan(a.base) && (offAddr{addr}).lessThan(a.limit)
}
...
// offAddrFromRaw takes an address in the offset space and
// places it into an offAddr.
func offAddrFromRaw(a uintptr) offAddr {
	return offAddr{a - arenaBaseOffset}
}

// value returns the address in the offset space as a uintptr.
func (l offAddr) value() uintptr {
	return l.a + arenaBaseOffset
}
...
// equals returns true if the two offAddr values are equal.
func (l1 offAddr) equals(l2 offAddr) bool {
	// No need to compare in the offset space, it
	// means the same thing.
	return l1 == l2
}

// addr returns the read address for this offset address.
...
func (a *addrRanges) add(r addrRange) {
	...
	coalescesDown := i > 0 && a.ranges[i-1].limit.equals(r.base)
	coalescesUp := i < len(a.ranges) && r.limit.equals(a.ranges[i].base)
...
func (a *addrRanges) removeAbove(addr uintptr) {
	...
	if r := a.ranges[pivot-1]; r.contains(addr) {
		removed += r.size()
		r = r.subtract(addrRange{offAddr{addr}, maxOffAddr})
		removed -= r.size()
		a.ranges[pivot-1] = r
	}
