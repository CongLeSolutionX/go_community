...
func init() {
	const cfOSXSAVE uint64 = 1 << 27
	const cfFMA uint64 = 1 << 12
	const cfMOVBE uint64 = 1 << 22
	const cfAVX2 uint64 = 1 << 5
	const cfAVX uint64 = 1 << 28
	const cfBMI1 uint64 = 1 << 3
	const cfBMI2 uint64 = 1 << 8
	const cfABM uint64 = 1 << 5
	...
	if core4thGeneration && enabledAVX {
...
func isIntel() bool {
	...
	if vendorStringBytes == intelSignature {
		return true
	}
	return false
}
