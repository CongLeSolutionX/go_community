...
func mapiterinit(t *maptype, h *hmap, it *hiter) {
	...
	if old := h.flags; old&(iterator|oldIterator) != iterator|oldIterator {
		atomic.Atomicor8(&h.flags, iterator|oldIterator)
	}
...
func mapzero(t *_type) {
	...
	cursize := uintptr(atomic.Atomicloadp(unsafe.Pointer(&zerosize)))
	if t.size <= cursize {
		return
	}
	...
	if cursize < t.size {
		...
		atomic.Atomicstorep1(unsafe.Pointer(&zeroptr), persistentalloc(cursize, 64, &memstats.other_sys))
		atomic.Atomicstorep1(unsafe.Pointer(&zerosize), unsafe.Pointer(zerosize))
	}
	unlock(&zerolock)
}
