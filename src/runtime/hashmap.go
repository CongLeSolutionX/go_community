...
// If h != nil and buckets != nil, the compiler has determined that the result
// does not escape the calling function, so the map can be created directly
// in h (on stack) and use buckets as the first bucket.
func makemap(t *maptype, hint int64, h *hmap, buckets unsafe.Pointer) *hmap {
