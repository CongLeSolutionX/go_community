...
TEXT sigtramp<>(SB),NOSPLIT,$0-0
	...
	CMPL	DI, $0
	JNE	callgo
	// Exception from non-Go thread.
	// Set dummy TLS so sigtrampgo does not crash
	// when calling getg().
	MOVL	CX, 0(SP)	// save CX
	MOVL	DI, 4(SP)	// set arg2. arg1 and arg3 are unused
	CALL	runtime·setldt(SB)	// clobbers CX and DX
	MOVL	0(SP), CX	// restore CX
