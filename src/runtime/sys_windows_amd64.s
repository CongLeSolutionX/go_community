...
// BX is the kind if sigtramp function.
TEXT sigtramp<>(SB),NOSPLIT|NOFRAME,$0-0
	// Switch from the host ABI to the Go ABI.
	PUSH_REGS_HOST_TO_ABI0()

	// Set up ABIInternal environment: g in R14, cleared X15.
	// TLS can be nil if called from a non-go thread,
	// clear R14 so that there's a bad value in there.
	XORPS	X15, X15
	XORQ    R14, R14
	get_tls(DX)
	CMPQ	DX, $0
...
TEXT runtime·exceptiontramp(SB),NOSPLIT|NOFRAME,$0
	MOVQ	CX, AX
	MOVQ	$const_callbackVEH, BX
	JMP	sigtramp<>(SB)

TEXT runtime·firstcontinuetramp(SB),NOSPLIT|NOFRAME,$0-0
	MOVQ	CX, AX
	MOVQ	$const_callbackFirstVCH, BX
	JMP	sigtramp<>(SB)

TEXT runtime·lastcontinuetramp(SB),NOSPLIT|NOFRAME,$0-0
	MOVQ	CX, AX
	MOVQ	$const_callbackLastVCH, BX
	JMP	sigtramp<>(SB)
