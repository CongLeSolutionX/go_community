...
func net_runtime_pollOpen(fd uintptr) (*_sched.PollDesc, int) {
	...
	if pd.Wg != 0 && pd.Wg != _sched.PdReady {
		_lock.Gothrow("netpollOpen: blocked write on free descriptor")
	}
	if pd.Rg != 0 && pd.Rg != _sched.PdReady {
		_lock.Gothrow("netpollOpen: blocked read on free descriptor")
	}
	pd.Fd = fd
	pd.Closing = false
	pd.Seq++
...
func net_runtime_pollClose(pd *_sched.PollDesc) {
	if !pd.Closing {
		_lock.Gothrow("netpollClose: close w/o unblock")
	}
	if pd.Wg != 0 && pd.Wg != _sched.PdReady {
		_lock.Gothrow("netpollClose: blocked write on closing descriptor")
	}
	if pd.Rg != 0 && pd.Rg != _sched.PdReady {
		_lock.Gothrow("netpollClose: blocked read on closing descriptor")
	}
...
func net_runtime_pollUnblock(pd *_sched.PollDesc) {
	_lock.Lock(&pd.Lock)
	if pd.Closing {
		_lock.Gothrow("netpollUnblock: already closing")
	}
	pd.Closing = true
	pd.Seq++
	var rg, wg *_core.G
