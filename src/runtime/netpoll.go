...
type pollDesc struct {
	...
	// atomicInfo holds bits from closing, everr, rd, and wd,
...
func (i pollInfo) noReadDeadline() bool  { return i&pollNoReadDeadline != 0 }
func (i pollInfo) noWriteDeadline() bool { return i&pollNoWriteDeadline != 0 }
...
func (pd *pollDesc) publishInfo() {
	...
	if pd.rd < 0 {
		info |= pollNoReadDeadline
	}
	if pd.wd < 0 {
		info |= pollNoWriteDeadline
	}
...
func (pd *pollDesc) setEventErr(b bool) {
	x := pd.atomicInfo.Load()
	for (x&uint32(pollEventErr) != 0) != b && !pd.atomicInfo.CompareAndSwap(x, x^uint32(pollEventErr)) {
		x = pd.atomicInfo.Load()
	}
}
...
func netpollcheckerr(pd *pollDesc, mode int32) int {
	...
	if (mode == 'r' && info.noReadDeadline()) || (mode == 'w' && info.noWriteDeadline()) {
...
func netpollblock(pd *pollDesc, mode int32, waitio bool) bool {
	...
	// do the opposite: store to closing/rd/wd, updatePollAtomic, load of rg/wg
