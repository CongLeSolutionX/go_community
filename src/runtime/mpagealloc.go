...
//
// The leaves of the radix tree are summaries that represent the smallest unit
...
func (s *pageAlloc) update(base, npages uintptr, contig, alloc bool) {
	...
	if sa == ea && sc == ec {
		...
		y := s.arenas(sa).pageAlloc[sc].summarize()
		if x == y {
			return
		}
		...
		summary[arenaToChunkIndex(sa, sc)] = s.arenas(sa).pageAlloc[sc].summarize()
		...
		summary[arenaToChunkIndex(ea, ec)] = s.arenas(ea).pageAlloc[ec].summarize()
	} else {
		...
		if sa == ea {
			...
			a := s.arenas(sa)
			for j := sc; j <= ec; j++ {
				summary[arenaToChunkIndex(sa, j)] = a.pageAlloc[j].summarize()
			}
		} else {
			// The range spans multiple arenas.
			for j := sc; j < mallocChunksPerArena; j++ {
				summary[arenaToChunkIndex(sa, j)] = s.arenas(sa).pageAlloc[j].summarize()
			}
			for i := sa + 1; i < ea; i++ {
				a := s.arenas(i)
				for j := 0; j < mallocChunksPerArena; j++ {
					summary[arenaToChunkIndex(i, j)] = a.pageAlloc[j].summarize()
				}
			}
			for j := 0; j <= ec; j++ {
				summary[arenaToChunkIndex(ea, j)] = s.arenas(ea).pageAlloc[j].summarize()
			}
		}
	}
...
func (s *pageAlloc) freeSlow(base, npages uintptr) {
	...
	if sa == ea && sc == ec {
		// The range doesn't cross any arena or chunk boundaries.
		s.arenas(sa).pageAlloc[sc].free(si, ei+1-si)
	} else if sa == ea && sc != ec {
		// The range doesn't cross any arena boundaries, but
		// does cross at least one chunk boundary.
		a := s.arenas(sa)
		a.pageAlloc[sc].free(si, mallocChunkPages-si)
		for c := sc + 1; c < ec; c++ {
			a.pageAlloc[c].freeAll()
		}
		a.pageAlloc[ec].free(0, ei+1)
	} else {
		// The range crosses at least one arena boundary.
		{
			// Alloc the first arena.
...
func (s *pageAlloc) free(base, npages uintptr) {
	...
	if npages == 1 {
		...
		s.arenas(arenaIndex(base)).pageAlloc[arenaChunkIndex(base)].free1(chunkPageIndex(base))
	} else {
