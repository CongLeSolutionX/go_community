...
// address space (16 GiB on 64-bit systems), with each higher tier
// representing successively smaller subsections until we reach the
// highest granularity in the highest tier: a chunk.
//
...
// new potential hint, and the number scavenged bytes in the newly-allocated
// region. The hint need not be better than s.hint.
//
...
func (s *pageAlloc) allocSlow(npages uintptr) (uintptr, uintptr, uintptr) {
	...
	// foundBestHint indicates whether it's been found. Note that 0 may
	// be a valid "best hint", especially on 32-bit architectures.
	hint := uintptr(0)
	foundBestHint := false
nextLevel:
