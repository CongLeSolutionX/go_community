...
func (s *pageAlloc) grow(base, size uintptr) {
	...
	s.inUse.add(addrRange{offAddr{base}, offAddr{limit}})
...
func (s *pageAlloc) find(npages uintptr) (uintptr, offAddr) {
	...
	foundFree := func(addr offAddr, size uintptr) {
		if !addr.lessThan(firstFree.base) && !firstFree.bound.lessThan(addr.add(size-1)) {
...
nextLevel:
	for l := 0; l < len(s.summary); l++ {
		...
		j0 := 0
		if searchIdx := int(s.searchAddr.value() >> levelShift[l]); searchIdx&^(entriesPerBlock-1) == i {
		...
		for j := j0; j < len(entries); j++ {
			...
			foundFree(offAddrFromRaw(uintptr(i+j)<<levelShift[l]), (uintptr(1)<<logMaxPages)*pageSize)
		...
		if size >= uint(npages) {
			...
			addr := offAddrFromRaw(uintptr(i) << levelShift[l]).add(uintptr(base) * pageSize).addr()
