...
// findAddr returns the next mapped offAddr for the given
// address. If offAddr refers to mapped memory, then it is
// returned. If offAddr is higher than any mapped region, then
// it returns maxOffAddr.
//
// s.mheapLock must be held.
func (s *pageAlloc) findAddr(addr offAddr) offAddr {
	// If we're not in a test, validate first by checking mheap_.arenas.
	// This is a fast path which is only safe to use outside of testing.
	ai := arenaIndex(addr.addr())
	if s.test || mheap_.arenas[ai.l1()] == nil || mheap_.arenas[ai.l1()][ai.l2()] == nil {
		vAddr, ok := s.inUse.findAddr(addr.addr())
		if ok {
...
nextLevel:
	for l := 0; l < len(s.summary); l++ {
		...
		if size >= uint(npages) {
			...
			return addr, s.findAddr(firstFree.base)
		}
		if l == 0 {
