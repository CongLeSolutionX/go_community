...
// view of the address space on systems with segmented address spaces. This
// linearized view is the same one generated by chunkIndex and arenaIndex,
// done by adding arenaBaseOffset.
//
// On systems without a segmented address space, it's just a normal comparison.
//
...
func (s *pageAlloc) grow(base, size uintptr, sysStat *uint64) {
	...
	for c := chunkIndex(base); c < chunkIndex(limit); c++ {
		s.chunks[c].scavenged.setRange(0, mallocChunkPages)
	}
...
func (s *pageAlloc) allocRange(base, npages uintptr) uintptr {
	...
	if sc == ec {
		...
		scav += s.chunks[sc].scavenged.popcntRange(si, mallocChunkPages-si)
		s.chunks[sc].allocRange(si, mallocChunkPages-si)
		for c := sc + 1; c < ec; c++ {
			scav += s.chunks[c].scavenged.popcntRange(0, mallocChunkPages)
			s.chunks[c].allocAll()
		}
...
//
// It uses s.searchAddr to prune it's search, assuming that no malloc chunks
...
nextLevel:
	for l := 0; l < len(s.summary); l++ {
		...
		if size >= uint(npages) {
			...
			// If at this point we still haven't found the first free page,
			// this must be it.
			if !foundSearchAddr {
...
func (s *pageAlloc) alloc(npages uintptr) (addr uintptr, scav uintptr) {
	...
	if mallocChunkPages-chunkPageIndex(s.searchAddr) >= uint(npages) {
		// npages is guaranteed to be no greater than mallocChunkPages here.
