...
nextLevel:
	for l := 0; l < len(s.summary); l++ {
		...
		for j := j0; j < len(level); j++ {
			if arenaBaseOffset != 0 && l == 0 && uintptr((i+j)<<levelShift[l]) == arenaBaseOffset {
				// arenaBaseOffset is != 0 and we're crossing the
				// boundary. Make sure we don't allocate across this
				// boundary by artificially saying we have no running
				// size from the previous iteration.
				// Note that this can only ever happen at level 0.
				//
				// TODO(mknyszek): It may be that this is never actually
				// possible because it would require the zero page to be
				// mapped. If it's not, then there's already a discontinuity
				// in the address space that prevents this case from ever
				// happening.
				size = 0
			}
			sum := level[j]
