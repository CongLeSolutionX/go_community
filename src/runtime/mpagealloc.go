...
// of entries for the given summary level. The range is then rounded
// to a 1 << levelBits[level] boundary. base must be the inclusive
// lower bound and limit must be the exclusive upper bound.
//
// addrsToSummaryRange assumes that limit is strictly larger than
// base.
//
...
func addrsToSummaryRange(level int, base, limit uintptr) (lo int, hi int) {
	if base >= limit {
		...
		throw("addrsToSummaryRange found base >= limit")
	}
	...
	//
	// We need to turn limit into an inclusive bound and add one after
	// the fact in order to get the final rounding right, otherwise
	// limit might get rounded into the same index as base.
	rlo := (base + arenaBaseOffset) >> sh
	rhi := (((limit - 1) + arenaBaseOffset) >> sh) + 1
...
func (s *pageAlloc) init(mheapLock *mutex, sysStat *uint64) {
	...
	s.mallocBitsAlloc.init(unsafe.Sizeof(mallocBits{}), nil, nil, sysStat)
