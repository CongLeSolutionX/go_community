...
func chunkIndex(p uintptr) chunkIdx {
	return chunkIdx(offsetAddress(p) / pallocChunkBytes)
}

// chunkIndex returns the base address of the palloc chunk at index ci.
func chunkBase(ci chunkIdx) uintptr {
	return (offAddr(ci) * pallocChunkBytes).addr()
}
...
func addrsToSummaryRange(level int, base, limit uintptr) (lo int, hi int) {
	...
	lo = int(offsetAddress(base) >> levelShift[level])
	hi = int(offsetAddress(limit-1)>>levelShift[level]) + 1
	return
}
...
type pageAlloc struct {
	...
	//
	// TODO(mknyszek): Consider making this an offAddr.
	searchAddr uintptr
...
func (s *pageAlloc) compareSearchAddrTo(addr uintptr) int {
	...
	lAddr := offsetAddress(addr)
	lSearchAddr := offsetAddress(s.searchAddr)
...
func (s *pageAlloc) find(npages uintptr) (uintptr, uintptr) {
	...
	firstFree := struct {
		...
		base:  offAddr(0),
		bound: offAddr(1<<heapAddrBits - 1),
	}
...
nextLevel:
	for l := 0; l < len(s.summary); l++ {
		...
		j0 := 0
		if searchIdx := int(offsetAddress(s.searchAddr) >> levelShift[l]); searchIdx&^(entriesPerBlock-1) == i {
		...
		for j := j0; j < len(entries); j++ {
			...
			foundFree(offAddr((i+j)<<levelShift[l]), (uintptr(1)<<logMaxPages)*pageSize)
		...
		if size >= uint(npages) {
			...
			addr := offAddr(i << levelShift[l]).add(uintptr(base) * pageSize).addr()
	...
	foundFree(offsetAddress(searchAddr), chunkBase(ci+1)-searchAddr)
	return addr, firstFree.base.addr()
}
...
func (s *pageAlloc) free(base, npages uintptr) {
	...
	limit := base + npages*pageSize - 1
	if offLimit := offsetAddress(limit); s.scav.freeHWM < offLimit {
