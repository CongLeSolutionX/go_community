...
func (p *pageAlloc) grow(base, size uintptr) {
	...
	// Mark all new memory as not huge page eligible.
	if !p.test {
...
func (p *pageAlloc) free(base, npages uintptr) {
	...
	if npages == 1 {
		...
		if sc == ec {
			...
			p.scav.index.free(sc, ei+1-si)
		} else {
			// The range crosses at least one chunk boundary.
			p.chunkOf(sc).free(si, pallocChunkPages-si)
			p.scav.index.free(sc, pallocChunkPages-si)
			for c := sc + 1; c < ec; c++ {
				p.chunkOf(c).freeAll()
				p.scav.index.free(c, pallocChunkPages)
			}
			p.chunkOf(ec).free(0, ei+1)
			p.scav.index.free(ec, ei+1)
		}
	}
