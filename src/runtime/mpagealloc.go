...
func chunkBase(ci chunkIdx) uintptr {
	return offAddr(uintptr(ci) * pallocChunkBytes).addr()
}
...
func addrsToSummaryRange(level int, base, limit uintptr) (lo int, hi int) {
	...
	hi = int(offsetAddress(limit)>>levelShift[level]) + 1
	return
}
...
func (s *pageAlloc) grow(base, size uintptr) {
	...
	// than chunks. Also, sysGrow expects an inclusive range, aligned
	// to a chunk.
...
nextLevel:
	for l := 0; l < len(s.summary); l++ {
		...
		if size >= uint(npages) {
			...
			addr := offAddr(uintptr(i<<levelShift[l]) + uintptr(base)*pageSize).addr()
