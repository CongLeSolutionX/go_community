...
func deferreturn() {
	runDefers(nil, getcallerpc(), unsafe.Pointer(getcallersp()))
}
...
func Goexit() {
	...
	runDefers(&p, getcallerpc(), unsafe.Pointer(getcallersp()))
...
// runDefers walks the stack starting from (pc, sp), and invokes any
// pending defers.
//
// If p is not nil, it's linked to the top of the gp._panic stack, so
// it can get recovered. If it's recovered, runDefers will instead
// non-locally return to the frame that pushed that deferred call.
// However, if that would cause runDefers to jump over another
// runDefers call with p.goexit, then it will non-locally return to
// resume that loop instead.
//
// If p is nil, then only the top-most stack frame is processed.
func runDefers(p *_panic, pc uintptr, sp unsafe.Pointer) {
	gp := getg()
	...
	for pc != 0 {
		...
		systemstack(func() {
			...
			for u.initAt(pc, uintptr(sp), 0, gp, 0); u.valid(); u.next() {
				// TODO(mdempsky): If we populate deferreturn for all
				// functions containing a defer, then we can simplify the loop
				// condition here (i.e., get rid of limit, and only call
				// funcdata if the function has a defer):
				//
				...
				if fd == nil && u.frame.sp != limit {
					continue
				}

				if fd != nil && u.frame.fn.deferreturn == 0 {
					throw("missing deferreturn")
				}

				retpc = u.frame.fn.entry() + uintptr(u.frame.fn.deferreturn)
				lr = u.frame.lr
				sp = unsafe.Pointer(u.frame.sp)
				fp = unsafe.Pointer(u.frame.fp)
				varp = unsafe.Pointer(u.frame.varp)
				return
			}
		})

		// Process any open-coded defers for this frame.
		if fd != nil {
			// TODO(mdempsky): The metadata for open-coded defers is
			// suboptimal. Instead of a sequence of varints, for each
			// individual closure; we should store in the frame a variable
			// like:
			//
			//	var defers struct {
			//		n  int         // count of how many defers are present
			//		fn [...]func() // array of that many defers
			//	}
			//
			// Then we only need a single integer to find all of the
			// relevant data within the stack frame.
			//
			// TODO(mdempsky): Also, if we store the offset relative to sp
			// instead of varp, then that's one fewer word of state we need
			// to maintain.

			offsetPtr := func(offset uintptr) unsafe.Pointer {
				return unsafe.Pointer(uintptr(varp) - offset)
			}

			deferBitsOffset, fd := readvarintUnsafe(fd)
			nDefers, fd := readvarintUnsafe(fd)
			deferBits := (*uint8)(offsetPtr(uintptr(deferBitsOffset)))
			...
			for i := int(nDefers) - 1; i >= 0; i-- {
				...
				*deferBits &^= 1 << i
				...
				fn := *(*func())(offsetPtr(uintptr(closureOffset)))

				callDefer(p, fn, retpc, sp)
			}
		}

		// Process any pending linked defers for this frame.
		for {
			d := gp._defer
			if d == nil || d.sp != uintptr(sp) {
				break
			}

			// TODO(mdempsky): Currently, linked defers are implemented
			// roughly as:
			//
			//	if deferproc(f) != 0 {
			//		deferreturn()
			...
			//	}
			//
			// where deferproc returns 0 the first time it's called, and
			// then returns non-zero a second time if f (or g, respectively)
			// recovered a panic.
			//
			// But for open-coded defers, we already add a single "CALL
			// deferreturn; RET" sled at the end of the function, which we
			// already found above while walking the stack. If we added that
			// same sled to all functions that contain a defer, we could
			// drop the d.pc field, and change the above deferproc calls
			// into just:
			//
			//	deferproc(f)
			//	deferproc(g)
...
// callDefer invokes deferred function fn, with pending panic p (if
// any).
//
// If the deferred function recovered the panic, we longjmp to the
// corresponding frame's deferreturn sled.
//
// Note: If p is goexit, callDefer might return after a nested panic
// has processed further defers up the call stack. runDefers needs to
// be written to account for this.
func callDefer(p *_panic, fn func(), retpc uintptr, sp unsafe.Pointer) {
	if p != nil {
...
// longjmp non-locally returns to (pc, sp), which should be a
// deferreturn sled for the function that recovered the pending panic.
//
...
// defers.
func longjmp(pc uintptr, sp unsafe.Pointer) {
	gp := getg()

	// Unwind the panic stack.
	p := gp._panic
	for ; p != nil && uintptr(p.sp) < uintptr(sp); p = p.link {
		// Don't allow jumping past a pending Goexit.
		// Instead, we'll jump back just outside Goexit's last callDefer.
		//
		...
		//
		// These are probably pretty rare occurences in practice, and they
		// don't seem any worse than the existing logic. But if we move
		// the unwinding state into _panic, we could detect when we run
		// into where the last panic started, and then just pick up where
		// it left off instead.
		//
	...
	// "CALL deferreturn" instuction; we can simulate it with something
	// like:
	//
	//	if usesLR {
	//		lr = pc
