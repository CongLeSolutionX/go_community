...
func throwreturn() {
	_lock.Gothrow("no return at end of a typed function - compiler is broken")
}

func throwinit() {
	_lock.Gothrow("recursive call during initialization - linker skew")
}
...
func deferproc(siz int32, fn *_core.Funcval) { // arguments of fn follow fn
	if _core.Getg().M.Curg != _core.Getg() {
		...
		_lock.Gothrow("defer on system stack")
	}
	...
	_lock.Systemstack(func() {
		d := newdefer(siz)
		if d.Panic != nil {
			_lock.Gothrow("deferproc: d.panic != nil after newdefer")
		}
		d.Fn = fn
		d.Pc = callerpc
		d.Sp = sp
...
func newdefer(siz int32) *_core.Defer {
	...
	if d == nil {
		...
		total := _schedinit.Goroundupsize(_schedinit.Totaldefersize(uintptr(siz)))
