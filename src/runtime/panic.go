	...
	for p.sp != nil {
		for p.openDefers > 0 {
			...
			*p.deferBitsPtr &^= bit
			...
			return *(*func())(p.offsetPtr(uintptr(closureOffset))), true
		}
		...
		// We've exhausted this frame.
		// Time to loop to the next one.

		if p.deferreturn {
			break
		}

		p.nextFrame(p.lr, p.fp)
	}

	return nil, false
}

// nextFrame finds the next frame that may contain deferred functions
// to call.
func (p *_panic) nextFrame(pc uintptr, sp unsafe.Pointer) {
	gp := getg()
	...
	systemstack(func() {
		...
		for u.initAt(pc, uintptr(sp), 0, gp, 0); u.valid(); u.next() {
			...
			if fd != nil {
				...
				var deferBitsOffset, openDefers uint32
				deferBitsOffset, fd = readvarintUnsafe(fd)
				openDefers, fd = readvarintUnsafe(fd)
				deferBitsPtr := (*uint8)(p.offsetPtr(uintptr(deferBitsOffset)))

				if *deferBitsPtr != 0 {
					p.openDefers = int(openDefers)
		...
		// Finished walking the stack. Let _panic.next know.
		p.sp = nil
	})
}

func (p *_panic) offsetPtr(offset uintptr) unsafe.Pointer {
	return unsafe.Pointer(uintptr(p.varp) - offset)
}
...
func longjmp(pc uintptr, sp unsafe.Pointer) {
	...
	p := gp._panic
	for ; p != nil && uintptr(p.startSP) < uintptr(sp); p = p.link {
