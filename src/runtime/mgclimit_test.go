...
func TestGCCPULimiter(t *testing.T) {
	...
	if l.Fill() != 0 {
		t.Fatalf("expected empty bucket from updating during a transition, got fill of %d cpu-ns", l.Fill())
	}
	l.FinishGCTransition(advance(1 * time.Microsecond))
	...
	// exactly 1 nanosecond.
	//
	// The window we need to drain the bucket is 1/(1-2*gcBackgroundUtilization) times the current fill:
	//
	//   fill + (window * gcBackgroundUtilization - window * (1-gcBackgroundUtilization)) = 0
	//   fill = window * ((1-gcBackgroundUtilization) - gcBackgroundUtilization)
	//   fill = window * (1-2*gcBackgroundUtilization)
	//   window = fill / (1-2*gcBackgroundUtilization)
	l.Update(0, advance(time.Duration((1/(1-2*GCBackgroundUtilization))*float64(2*time.Millisecond+1*time.Microsecond))))
	if l.Fill() != 0 {
		t.Fatalf("expected empty bucket from draining after a GC started, got fill of %d cpu-ns", l.Fill())
	}
