...
func traceInitReadCPU() {
	...
	profBuf := newProfBuf(3, profBufWordCount, profBufTagCount) // after the timestamp, header is [pp.id, gp.goid, mp.id]
...
func traceStartReadCPU() {
	...
	go func() {
		for traceEnabled() {
			...
			//
			// We can't do a blocking read here because of a bug on Darwin.
...
func traceStopReadCPU() {
	// We need to make sure we do this *after* we've already notified
	// every trace writer to stop. If we close the profBuf before doing
	// that, then that goroutine will have to spin until tracing is
	// disabled.
	//
	// Since we're closing it now, one of two things will happen:
	// - The logger goroutine has already exited because it observed
	//   that the trace is disabled.
	// - The logger goroutine is blocked on reading from the profBuf.
	//
	// Closing the profBuf now will ensure that the goroutine wakes up
	// and it will always check whether the trace is disabled.
	if traceEnabled() {
		throw("traceStopReadCPU called with trace enabled")
	}

	// Prevent writing to the old CPU profile buffer and flush it.
	trace.cpuLogWrite.Store(nil)
	trace.cpuLogRead.close()
	trace.cpuSleep.wake()

	// Wait until the CPU profile -> trace goroutine exits.
	<-trace.cpuLogDone
