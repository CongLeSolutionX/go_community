...
func GCPhys() {
	...
	const (
		// Currently, we target 32 MiB worth of memory for our test,
	...
	// scavenged immediately as part of the 2x size allocations.
	overuse := (float64(heapBacked) - float64(stats.NextGC)) / float64(stats.NextGC)
	if overuse <= 0.10 {
		fmt.Println("OK")
		return
	}
	// Physical memory utilization exceeds the threshold, so the scavenger did not
	// operate as expected.
	//
