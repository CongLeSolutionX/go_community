...
func ReadMemStatsSlow() (base, slow MemStats) {
	...
	systemstack(func() {
		...
		for i := mheap_.pages.start; i < mheap_.pages.end; i++ {
			pg := mheap_.pages.chunks[i].scavenged.popcntRange(0, mallocChunkPages)
...
func (d *MallocData) FindScavengeCandidate(searchIdx uint, max uintptr) (uint, uint) {
	return (*mallocData)(d).findScavengeCandidate(searchIdx, max)
}
func (d *MallocData) AllocRange(i, n uint) { (*mallocData)(d).allocRange(i, n) }
func (d *MallocData) ScavengedSetRange(i, n uint) {
	(*mallocData)(d).scavenged.setRange(i, n)
}
func (d *MallocData) MallocBits() *MallocBits {
	return (*MallocBits)(&(*mallocData)(d).mallocBits)
}
func (d *MallocData) Scavenged() *MallocBits {
	return (*MallocBits)(&(*mallocData)(d).scavenged)
}
...
func NewPageAlloc(chunks, scav map[ChunkIdx][]BitRange) *PageAlloc {
	...
	for i, init := range chunks {
		...
		p.update(addr, mallocChunkPages, false, false)
	}

	SetFinalizer(p, func(p *pageAlloc) {
		...
		if pageAlloc64Bit != 0 {
			...
			for _, s := range p.summary {
				resSize += uintptr(cap(s)) * mallocSumBytes
			}
...
// between the two.
var BaseChunkIdx = ChunkIdx(chunkIndex((0xc000*pageAlloc64Bit + 0x200*pageAlloc32Bit) * mallocChunkBytes))
