		...
		for i := arenaIdx(mheap_.pages.start / mallocChunksPerArena); i <= arenaIdx((mheap_.pages.end-1)/mallocChunksPerArena); i++ {
			...
			if a != nil {
				...
				for c := 0; c < mallocChunksPerArena; c++ {
					pg += a.pageAlloc[c].scavenged.popcntRange(0, mallocChunkPages)
				}
...
func GetTestPageAlloc(arenas map[int][]BitRange) *PageAlloc {
	...
	for aidx, init := range arenas {
		...
		for _, s := range init {
			si, ei := s.I/mallocChunkPages, (s.I+s.N-1)/mallocChunkPages
			if si == ei {
				ha.pageAlloc[si].allocRange(s.I%mallocChunkPages, s.N)
			} else {
				ha.pageAlloc[si].allocRange(s.I%mallocChunkPages, mallocChunkPages-s.I%mallocChunkPages)
				for i := si + 1; i < ei; i++ {
					ha.pageAlloc[i].allocRange(0, mallocChunkPages)
				}
				ha.pageAlloc[ei].allocRange(0, (s.I+s.N-1)%mallocChunkPages+1)
			}
		}
...
const BaseArenaIdx = 0xc00*pageAlloc64Bit + 0x200*pageAlloc32Bit
			...
			if a != nil {
				for c := 0; c < mallocChunksPerArena; c++ {
					for j := 0; j < mallocChunkPages/64; j++ {
						...
						want := a.pageAlloc[c].scavenged[j] &^ a.pageAlloc[c].mallocBits[j]
						got := a.pageAlloc[c].scavenged[j]
						if want != got {
							ok = false
							if n >= len(mismatches) {
								break arenaLoop
