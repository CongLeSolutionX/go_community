...
func ReadMemStatsSlow() (base, slow MemStats) {
	...
	systemstack(func() {
		...
		for _, p := range allp {
			...
			largeFree += uint64(c.localLargeFree)
			slow.Frees += uint64(c.localLargeFreeN)

			// Collect tiny allocation stats.
			tinyAllocs += uint64(c.localTinyAllocs)

			// Collect per-sizeclass stats.
			for i := 0; i < _NumSizeClasses; i++ {
				slow.Frees += uint64(c.localSmallFreeN[i])
				bySize[i].Frees += uint64(c.localSmallFreeN[i])
				bySize[i].Mallocs += uint64(c.localSmallFreeN[i])
