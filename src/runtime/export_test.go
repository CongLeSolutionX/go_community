...
// StringifyPallocBits gets the bit range from b, converts
// the bits to ASCII 0 and 1 characters, and places them in out.
...
func (d *PallocData) FindScavengeCandidate(searchIdx uint, max uintptr) (uint, uint) {
	return (*pallocData)(d).findScavengeCandidate(searchIdx, max)
}
...
func NewPageAlloc(chunks, scav map[ChunkIdx][]BitRange) *PageAlloc {
	...
	for i, init := range chunks {
		...
		p.grow(addr, pallocChunkBytes, nil)
