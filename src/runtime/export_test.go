...
func ReadMemStatsSlow() (base, slow MemStats) {
	...
	systemstack(func() {
		...
		// TODO(mknyszek): This could probably be faster.
		for i := mheap_.pages.start; i < mheap_.pages.end; i++ {
			a := mheap_.pages.arenas(i)
			if a != nil {
				for i := 0; i < len(a.pageAlloc.mallocBits); i++ {
					inuse := a.pageAlloc.mallocBits[i]
					scav := a.pageAlloc.scavenged[i]
					slow.HeapReleased += uint64(bits.OnesCount8(scav&^inuse)) * pageSize
				}
			}
		}
...
// Expose chunk8 for testing.
type Chunk8 chunk8

func UnsafeChunkFromSlice(b []uint8) *Chunk8 { return (*Chunk8)(unsafeChunkFromSlice(b)) }
...
func (d *MallocData) HasScavengeCandidate(hint int) bool {
	return (*mallocData)(d).hasScavengeCandidate(hint)
}
...
func (p *PageAlloc) InitScavState(arenas map[int][]BitRange) {
	...
	for highaddr, init := range arenas {
		...
		a.pageAlloc.scavenged.clearRange(0, pagesPerArena)
		for _, s := range init {
			a.pageAlloc.scavengeRange(s.I, s.N)
		}
	}
...
//
// It initializes the dummy pageAlloc using arenas, with the map
// key representing the non-base-offset'd arena index and the
...
// allocate any of pageAlloc's structures, because both pageAlloc
// and mheap are go:notinheap. Furthermore, pageAlloc consists of
// enormous memory mappings that, while probably not problematic
// in small multiples, could have adverse effects if we could create
// as many as we want on-demand. This mechanism thus also helps
// limit the amount of memory mappings we create.
func GetTestPageAlloc(arenas map[int][]BitRange) *PageAlloc {
	...
	entry.pageAlloc.init(&entry.mheap)
	...
	for aidx, init := range arenas {
		...
		ha.pageAlloc.scavengeRange(0, pagesPerArena)
		...
		// Update pageAlloc metadata.
		entry.pageAlloc.setArenas(addr, 1, nil)
		n++
	}
	return (*PageAlloc)(&entry.pageAlloc)
}

// PutTestPageAlloc returns a pageAlloc to the cache mechaism.
//
...
func PutTestPageAlloc(pa *PageAlloc) {
	...
	for l := 0; l < summaryLevels; l++ {
		sysFree(unsafe.Pointer(&p.summary[l][0]), uintptr(len(p.summary[l]))*mallocSumBytes, nil)
	}
