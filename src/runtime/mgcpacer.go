...
func (c *gcControllerState) heapGoalInternal() (goal, minTrigger uint64) {
	...
	if newGoal := c.memoryLimitGoal(); go119MemoryLimitSupport && newGoal < goal {
		goal = newGoal
	} else {
...
func (c *gcControllerState) memoryLimitGoal() uint64 {
	...
	for {
		...
		// Technically this is impossible, but these stats are inconsistent, so this
		// condition may occur. However, this condition is necessarily transient, so just
		// try again. In the case of a persistent accounting error, we'll deadlock here.
	}
	...
	//
	//    memoryLimit - ((mappedReady - heapFree - heapAlloc) + max(mappedReady - memoryLimit, 0)) - memoryLimitGoalHeadroom
	//                    ^1                                    ^2                                   ^3
	//
	...
	//
	// The third term (marker 3) subtracts an additional memoryLimitGoalHeadroom bytes from the
	...
	if goal < memoryLimitGoalHeadroom || goal-memoryLimitGoalHeadroom < memoryLimitGoalHeadroom {
		goal = memoryLimitGoalHeadroom
	} else {
		goal = goal - memoryLimitGoalHeadroom
	}
...
func (c *gcControllerState) commit(isSweepDone bool) {
	...
	if gcPercentGoal < c.heapMinimum {
		gcPercentGoal = c.heapMinimum
	}
