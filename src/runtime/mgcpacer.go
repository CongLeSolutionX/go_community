...
func (c *gcControllerState) heapGoalInternal() (goal, minTrigger uint64) {
	...
	// by a tiny bit.
	//
	// Only do this if our heap size is not determined by the memory limit.
	// In that case, the goal must not shift.
	if !goalLimited && c.triggered != ^uint64(0) && goal < c.triggered+64<<10 {
		goal = c.triggered + 64<<10
	}
	if goalLimited {
		// We definitely ignored the sweepDistTrigger above, so ignore it again.
		// It's much more important to maintain the memory limit than to try and
		// deal with transient performance issues from a pacing parameter change.
		// This also enforces the invariant that the trigger <= goal.
		return goal, 0
	}
