...
type gcControllerState struct {
	...
	// rate (NOT throughput) and the GC's scan rate (effectively, throughput).
...
func (c *gcControllerState) startCycle(markStartTime int64) {
	...
	c.stackScan = c.scannableStackSize
...
func (c *gcControllerState) endCycle(now int64, gomaxprocs int32, userForced bool) float64 {
	...
	if goexperiment.PacerRedesign {
		...
		//
		// The units we want for the numerator and denominator are B / cpu-ns.
		...
		//
		//    assistDuration * gomaxprocs * (1 - utilization)
		//
		...
		//
		//    assistDuration * gomaxprocs * (utilization + idleUtilization)
		//
		...
		//     (heapLive-trigger) / (assistDuration * gomaxprocs * (1-utilization)) /
		//         (scanWork) / (assistDuration * gomaxprocs * (utilization+idleUtilization)
		//
		// Note that because we only care about the ratio, assistDuration and gomaxprocs cancel out.
		currentConsMark := (float64(c.heapLive-c.trigger) / (1 - utilization)) /
			(float64(c.heapScanWork+c.stackScanWork+c.globalsScanWork) / (utilization + idleUtilization))
