...
TEXT	runtime·racereadrangepc1(SB), NOSPLIT, $0-24
	...
	MOVV	size+8(FP), R6
	MOVV	pc+16(FP), R7
	ADD	$4, R7	// pc is function start, tsan wants return address.
	// void __tsan_read_range(ThreadState *thr, void *addr, uintptr size, void *pc);
	MOVV	$__tsan_read_range(SB), R25
	JMP	racecalladdr<>(SB)
...
// If addr (R4) is out of range, do nothing.
...
TEXT	racecallatomic<>(SB), NOSPLIT, $0
	...
	MOVV	40(R29), R13	// 1st arg is addr. after two times JAL, get it at 40(R29)
	MOVV	(R13), R24	// segv here if addr is bad
	// Check that addr is within [arenastart, arenaend) or within [racedatastart, racedataend).
	MOVV	runtime·racearenastart(SB), R12
	SGTU	R12, R7, R13
...
racecallatomic_data:
	...
	MOVV	runtime·racedataend(SB), R12
	SGTU	R12, R7, R13
	BNE	R13, R0, racecallatomic_ignore
racecallatomic_ok:
...
GLOBL runtime·tls_g(SB), TLSBSS+DUPOK, $8
