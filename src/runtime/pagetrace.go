...
// 8 bytes follows this format for non-sync events:
//
//     [16 timestamp delta][35 base address][10 npages][1 isLarge][2 pageTraceEventType]
//
// If the "large" bit is set then the event is 16 bytes wide with
// the second 8 byte word containing the full npages value (the npages
// bitfield is 0).
//
...
// heap.
//
// The timestamp delta is computed from cuts off the bottom pageTraceTimeLostBits and keeps
// only the next pageTraceTimeDelta bits of the delta between the
//
// Sync events meanwhile have the following structure:
//
//    [61 timestamp or P ID][1 isPID][2 pageTraceSyncEvent]
//
// In essence, the "large" bit repurposed to indicate whether it's a timestamp (3 bits of
// precision lost) or a P ID (these are typically uint32).
...
//
// Does not handle flushing and always assumes there's space in the buffer.
// This is OK because there's typically just one P ID event in the buffer at
// any given time at the very beginning.
...
// writeSync writes a sync event, which is basically just a timestamp. Handles flushing.
func (tb pageTraceBuf) writeSync(pid int32) pageTraceBuf {
	...
	e := (uint64(tb.timeBase) &^ 0b111) | uint64(pageTraceSyncEvent)
	tb.buf.events[tb.len] = e
	tb.len++
	return tb
}
...
func (tb pageTraceBuf) writeEvent(pid int32, now int64, base, npages uintptr, typ pageTraceEventType) pageTraceBuf {
	...
	// Write the timestamp delta in the upper pageTraceTomeDeltaBits.
...
func (tb pageTraceBuf) flush(pid int32, now int64) pageTraceBuf {
	...
	if pageTrace.fd >= 0 {
		write(uintptr(pageTrace.fd), unsafe.Pointer(&tb.buf.events[0]), int32(tb.len*8))
	}
...
var pageTrace struct {
	...
	fdLock    mutex
	fd        int32
	exitingFd int32 // Used in finishPageTrace when exiting.
}
...
func initPageTrace(env string) {
	// bytealg hasn't been initialized yet.
	indexByte := func(s string, b byte) int {
		for i := 0; i < len(s); i++ {
			if s[i] == b {
				return i
			}
...
func finishPageTrace() {
	...
	systemstack(func() {
		// Grab the fd, and turn off tracing by zeroing the fd slot.
		//
		// We put the fd we get into pageTrace.exitingFd because we can't pass
		// a closure to forEachP.
		lock(&pageTrace.fdLock)
		pageTrace.exitingFd = pageTrace.fd
		pageTrace.fd = -1
		unlock(&pageTrace.fdLock)
		...
		forEachP(func(pp *p) {
			...
			tb := pp.pageTraceBuf
			if tb.len > 0 {
				write(uintptr(pageTrace.exitingFd), unsafe.Pointer(&tb.buf.events[0]), int32(tb.len*8))
			}
			unlock(&pageTrace.fdLock)
		})
