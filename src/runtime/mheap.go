...
//
// specialWeak is allocated from non-GC'd memory, so any heap
// pointers must be specially handled.
type specialWeakHandle struct {
	_       sys.NotInHeap
	special special
	handle  *atomic.Uint64 // May be a heap pointer.
}
...
const weakHandleDyingBit = 1 << 63

// Retrieves or creates a weak pointer handle for the object p.
func getOrAddWeakHandle(p unsafe.Pointer) *atomic.Uint64 {
	...
	handle := new(atomic.Uint64)
	s.special.kind = _KindSpecialWeakHandle
	s.handle = handle
	handle.Store(uint64(uintptr(p)))
	if addspecial(p, &s.special) {
		// This is responsible for maintaining the same
		// GC-related invariants as markrootSpans in any
		// situation where it's possible that markrootSpans
		...
		if gcphase != _GCoff {
			...
			gcw := &mp.p.ptr().gcw
			// Mark everything reachable from the object
			// so it's retained until the weak handle is
			// dead.
			if !span.spanclass.noscan() {
				scanobject(base, gcw)
			}
...
func getWeakHandle(p unsafe.Pointer) *atomic.Uint64 {
	span := spanOfHeap(uintptr(p))
	if span == nil {
