...
func (h *mheap) freeSpanLocked(s *mspan, acctinuse, acctidle bool, unusedsince int64) {
	...
	if before := spanOf(s.base() - 1); before != nil && before.state == mSpanFree {
		...
		// Now that we're done potentially scavenging s, we can adjust it.
...
func scavenge1(unscav *mTreap, t *treapNode, scav *mTreap, now, limit uint64) uintptr {
	...
	// node and add it to scav. Note that calling scavenge() may not
	// have actually scavenged s if it doesn't actually cover at
	// least one full page.
	if s.scavenged {
		successor := unscav.removeNode(t)
		scav.insert(s)
	}
	return released + scavenge1(unscav, successor, scav, now, limit)
}
...
func scavengeLargest(unscav *mTreap, scav *mTreap, npages uintptr) uintptr {
	...
	for released < npages {
		...
		r := s.scavenge()
		if r == 0 {
			// If we were unable to release anything from s, that probably
			// means s is too small to actually have any pages released from it.
