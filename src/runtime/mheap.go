...
func mlookup(v uintptr, base *uintptr, size *uintptr, sp **_base.Mspan) int32 {
	...
	if s.Sizeclass == 0 {
		...
		if size != nil {
			*size = s.Npages << _base.XPageShift
		}
		return 1
	}
...
func scavengelist(list *_base.Mspan, now, limit uint64) uintptr {
	if _base.PhysPageSize > _base.XPageSize {
	...
	for s := list.Next; s != list; s = s.Next {
		if (now-uint64(s.Unusedsince)) > limit && s.Npreleased != s.Npages {
			released := (s.Npages - s.Npreleased) << _base.XPageShift
			_base.Memstats.Heap_released += uint64(released)
			sumreleased += released
			s.Npreleased = s.Npages
			sysUnused((unsafe.Pointer)(s.Start<<_base.XPageShift), s.Npages<<_base.XPageShift)
		}
	}
	return sumreleased
}
...
func runtime_debug_freeOSMemory() {
	_iface.StartGC(_gc.GcForceBlockMode, false)
