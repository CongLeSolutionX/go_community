...
type mheap struct {
	...
	// scavengeGoal is the amount of retained idle memory (measured by heapRetained)
	// that the runtime will try to maintain by returning memory to the OS.
	scavengeGoal uint64
...
type heapArena struct {
	...
	// zero. Increases monotonically until it hits heapArenaBytes.
	//
...
// it acquires the heap lock. See mheap for details.
...
HaveBase:
	...
	if !manual {
		// This is not a heap span, so we should do some
...
func (h *mheap) grow(npage uintptr) bool {
	ask := alignUp(npage, mallocChunkPages) * pageSize

	totalGrowth := uintptr(0)
	nBase := alignUp(h.curArena.base+ask, physPageSize)
	nBase = alignUp(nBase, mallocChunkBytes)
	if nBase > h.curArena.end {
		...
		if uintptr(av) == h.curArena.end {
			...
			if size := h.curArena.end - h.curArena.base; size != 0 {
				h.pages.grow(h.curArena.base, size, &memstats.gc_sys)
				totalGrowth += size
			}
	...
	h.pages.grow(v, nBase-v, &memstats.gc_sys)
	totalGrowth += nBase - v
	...
	if retained := heapRetained(); retained > h.scavengeGoal {
		todo := totalGrowth
		if overage := uintptr(retained - h.scavengeGoal); todo > overage {
			todo = overage
		}
