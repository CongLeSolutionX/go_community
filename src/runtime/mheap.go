...
func scavenge1(unscav *mTreap, t *treapNode, scav *mTreap, now, limit uint64) uintptr {
	...
	for t != nil {
		...
		// If we didn't end up scavenging s, then scavenge its
		// children since we don't have to remove it.
		if r == 0 {
			return released +
				scavenge1(unscav, t.left, scav, now, limit) +
				scavenge1(unscav, t.right, scav, now, limit)
		}
		// If we did end up scavenging s, then remove it from unscav
		// and add it to scav. We may then take the span that is now
		// in its place and process it in this call. We could
		// alternatively recurse here, but that means we generate
		// a new stack frame per deletion which quickly leads to running
		// out of stack.
