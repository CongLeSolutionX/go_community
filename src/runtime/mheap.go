...
func (h *mheap) allocSpanLocked(npage uintptr) *mspan {
	...
	s = h.allocLarge(npage) // responsible for removing s from treap
	...
	if p > 0 {
		...
		if before != nil && before.state == _MSpanFree {
			...
			if h.isLargeSpan(before.npages) {
				...
				h.largeFreeTreap().removeSpan(before)
				if before.next != nil || before.prev != nil {
					// We just removed this t from it's place in the treap so it had better not be
					// part of a list.
					throw("why")
				}
			} else {
				...
				if before.next != nil || before.prev != nil {
					// We just removed this span from it's place in some freeList so it had better not be
					// part of a list.
					throw("why")
				}
			}
	...
	if (p + s.npages) < uintptr(len(h.spans)) {
		...
		if after != nil && after.state == _MSpanFree {
			...
			if h.isLargeSpan(after.npages) {
				h.largeFreeTreap().removeSpan(after)
			} else {
...
func (h *mheap) largeFreeTreap() *mTreap {
	return &h.freelarge
}
...
func (list *mSpanList) remove(span *mspan) {
	if span.list != list {
		println("runtime: failed MSpanList_Remove span.npages=", span.npages,
