...
func (s *mspan) scavenge() uintptr {
	...
	released := end - start
	hr := memstats.heap_released
	memstats.heap_released += uint64(released)
	if hr >= memstats.heap_released {
		println(hr, memstats.heap_released, released)
	}
	s.scavenged = true
...
HaveSpan:
	...
	if s.scavenged {
		...
		if hr < memstats.heap_released {
			println(hr, memstats.heap_released, end-start)
		}
...
func (h *mheap) freeSpanLocked(s *mspan, acctinuse, acctidle bool, unusedsince int64) {
	...
	if needsScavenge {
		...
		if hr < memstats.heap_released {
			println(hr, memstats.heap_released, prescavenged)
		}
		s.scavenge()
	}
