...
// newMarkBits returns a pointer to 8 byte aligned words
// to be used for this span's mark bits.
func newMarkBits(nelems uintptr) *uint8 {
	lock(&gcBitsArenas.lock)
	wordsNeeded := uintptr((nelems + 63) / 64)
	bytesNeeded := wordsNeeded * 8
	if gcBitsArenas.next == nil {
		gcBitsArenas.next = newArena()
	}
	if gcBitsArenas.next.free+bytesNeeded > uintptr(len(gcBits{}.bits)) {
...
// newAllocBits returns a pointer to 8 byte aligned words
...
func newArena() *gcBits {
	...
	if (uintptr(unsafe.Pointer(&result.bits[0])) & 7) == 0 {
		result.free = 0
	} else {
...
func growNextArena(minimum uintptr) {
	if minimum > gcBitsChunkBytes {
		throw("minumum greater that gcBitsChunkBytes")
	}
	fresh := newArena()
...
func growCurrentArena(minimum uintptr) {
	...
	fresh := newArena()
	fresh.next = gcBitsArenas.current
	gcBitsArenas.current = fresh
}
