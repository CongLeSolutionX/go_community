...
func (h *mheap) allocSpan(npages uintptr, manual bool, spanclass spanClass, sysStat *uint64) (s *mspan) {
	...
	if pp != nil && npages < pageCacheSize/4 {
		...
		if base != 0 {
			s = h.tryAllocMSpan()
			if s == nil || (!manual && spanclass.sizeclass() == 0) {
				// We either failed to acquire a mspan, or the
				// allocation is for a large object. This means we
...
func (h *mheap) grow(npage uintptr) bool {
	...
	if retained := heapRetained(); retained > h.scavengeRetainedGoal {
		todo := totalGrowth
		if overage := uintptr(retained - h.scavengeRetainedGoal); todo > overage {
			todo = overage
		}
