...
type mheap struct {
	...
	// spaced CacheLinePad size bytes apart, so that each MCentral.lock
	// gets its own cache line.
	// central is indexed by spanClass.
	central [numSpanClasses]struct {
		mcentral mcentral
		pad      [unsafe.Sizeof(cpu.CacheLinePad{}) - unsafe.Sizeof(mcentral{})%unsafe.Sizeof(cpu.CacheLinePad{})]byte
	}
