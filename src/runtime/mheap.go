...
// maxScavengeDebt is that maximum amount of bytes that we're willing
...
const maxScavengeDebt = heapArenaBytes
...
func (h *mheap) allocSpanLocked(npage uintptr, stat *uint64) *mspan {
	...
	if s != nil {
		h.scavengeDebt += int64(s.released())
		goto MaybeScavenge
	}
...
MaybeScavenge:
	if h.scavengeDebt >= maxScavengeDebt {
		...
		// with larger spans.
		scavenged := scavengeLargest(&h.free, &h.scav, uintptr(h.scavengeDebt))
		h.scavengeDebt -= int64(scavenged)
	}
...
HaveSpan:
	...
	if s.scavenged {
		start, end := s.physPageBounds()
		used := end - start
		sysUsed(unsafe.Pointer(start), used)
...
// returning how many bytes were actually added.
//
// h must be locked.
...
func scavenge(unscav *mTreap, scav *mTreap, now, limit uint64) uintptr {
	return scavenge1(unscav, unscav.treap, scav, now, limit)
}

// scavenge1 is a recursive helper for scavenge. t must always
// be either a valid node in unscav or nil.
func scavenge1(unscav *mTreap, t *treapNode, scav *mTreap, now, limit uint64) uintptr {
	if t == nil {
		return 0
	}
	released := uintptr(0)
	...
	if released == 0 {
		return scavenge1(unscav, t.left, scav, now, limit) +
			scavenge1(unscav, t.right, scav, now, limit)
	}
	next := unscav.removeNode(t)
	scav.insert(s)
	return released + scavenge1(unscav, next, scav, now, limit)
}

// scavengeLargest scavenges npages worth of physical pages from spans in unscav
// starting from the largest span and working down. It then takes those spans
// and places them in scav.
//
// Returns the amount of physical pages released.
func scavengeLargest(unscav *mTreap, scav *mTreap, npages uintptr) uintptr {
	t := unscav.treap
	released := uintptr(0)
	for released < npages {
		if t == nil {
		...
		r := s.scavenge()
		// r is in bytes, so just round up to the nearest number of pages
		// released.
