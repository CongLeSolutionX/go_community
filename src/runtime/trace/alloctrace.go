...
func runtime_startAllocTrace() bool
...
//
// This must only be called if the GOALLOCTRACEFROMINIT environment
// variable is set to a positive integral value, otherwise the
// runtime will crash.
//
// TODO(mknyszek): Support incomplete traces?
//
// GOALLOCTRACEFROMINIT is only available on AIX, Darwin, Dragonfly BSD,
...
func StartAllocTrace(w io.Writer, maxBytes int64) error {
	...
	fromInit := runtime_startAllocTrace()
	go func() {
		header := []byte{0, 0, 1, 14}
		if fromInit {
			header[0] |= 1 << 7
		}
		w.Write(header)
