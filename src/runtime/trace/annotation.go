...
// end mark, and provide the latency distribution per task type.
...
func WithSpan(ctx context.Context, name string, fn func(context.Context)) {
	...
	id := fromContext(ctx).id
	userSpan(id, spanStartCode, name)
	defer userSpan(id, spanEndCode, name)
	fn(ctx)
}
...
// same goroutine where the span was started. Spans started prior
// to this span must end after this span ends. This is a lower-level
// API than WithSpan, which should be used instead whenever possible.
func StartSpan(ctx context.Context, name string) func() {
	id := fromContext(ctx).id
	userSpan(id, spanStartCode, name)
	return func() { userSpan(id, spanEndCode, name) }
}
