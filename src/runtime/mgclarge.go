...
type treapNode struct {
	...
	baseKey  uintptr    // base address of the span, used as primary sort key
	spanKey  *mspan     // span at base address baseKey
...
func checkTreapNode(t *treapNode) {
	...
	if t.spanKey.base() != t.baseKey {
		println("runtime: checkTreapNode treapNode t=", t, "     t.baseKey=", t.baseKey,
			"t.spanKey.base()=", t.spanKey.base())
		throw("why does span.base() and treap.baseKey do not match?")
	}
	if t.left != nil && t.baseKey < t.left.baseKey {
		throw("found out-of-order spans in treap (left child has greater base address)")
	}
	if t.right != nil && t.baseKey > t.right.baseKey {
...
func (root *mTreap) insert(span *mspan) {
	...
	// That is, it is a binary tree ordered according to the baseKey,
	// but then among the space of possible binary trees respecting those
	// baseKeys, it is kept balanced on average by maintaining a heap ordering
	...
	*pt = t // t now at a leaf.
	...
	for t.parent != nil && t.parent.priority > t.priority {
		if t != nil && t.spanKey.base() != t.baseKey {
			println("runtime: insert t=", t, "t.baseKey=", t.baseKey)
			println("runtime:      t.spanKey=", t.spanKey, "t.spanKey.base()=", t.spanKey.base())
			throw("span and treap base addresses do not match?")
		}
		if t.parent.left == t {
...
// span with the smallest base address which is at least npages in size.
// If no span has at least npages it returns nil.
//
// This algorithm is as follows:
// * If there's a left child and it's subtree can satisfy this allocation,
...
func (root *mTreap) find(npages uintptr) treapIter {
	...
	for t != nil {
		...
		if t.left != nil && t.left.maxPages >= npages {
			t = t.left
		} else if t.spanKey.npages >= npages {
