...
// equalTo is used to order the treap.
// npagesKey and npages are the primary keys.
// spanKey and span are the secondary keys.
func (t *treapNode) equalTo(npages uintptr, s *mspan) bool {
...
func (t *treapNode) lessThan(npages uintptr, s *mspan) bool {
	if t != nil && t.spanKey.npages != t.npagesKey {
		throw("npages of span and treap do not match?")
	}
	if t.npagesKey < npages {
		return true
	}
	...
	// t.npagesKey == npages
	if uintptr(unsafe.Pointer(t.spanKey)) < uintptr(unsafe.Pointer(s)) {
		return true
	}
	return false
}

// greaterThan is used to order the treap.
// npagesKey and npages are the primary keys.
// spanKey and span are the secondary keys.
// span == nil (0) will always be lessThan all
// spans of the same size.
func (t *treapNode) greaterThan(npages uintptr, s *mspan) bool {
	if t != nil && t.spanKey.npages != t.npagesKey {
		throw("why does span and treap node not match?")
	}
	if t.npagesKey > npages {
		return true
	}
...
func (t *treapNode) isSpanInTreap(s *mspan) bool {
	...
	if t.left.isSpanInTreap(s) {
		return true
	}

	if t.right.isSpanInTreap(s) {
		return true
	}
	return false
}
...
func checkTreapNode(t *treapNode) {
	...
	if t.left != nil && t.lessThan(t.left.npagesKey, t.left.spanKey) {
		throw("t.lessThan(t.left.npagesKey, t.left.spanKey) is not false")
	}
	if t.right != nil && t.greaterThan(t.right.npagesKey, t.right.spanKey) {
		throw("t.lessThan(t.left.npagesKey, t.left.spanKey) is not false")
	}
}
...
func (root *mTreap) insert(span *mspan) {
	...
	pt := &root.treap
	for t := *pt; t != nil; t = *pt {
		// Check to see if span is already in treap
		if t.equalTo(npages, span) {
			throw("Inserting span already in treap")
		}
		last = t
		if t.greaterThan(npages, span) {
			pt = &t.left
		} else {
			pt = &t.right
		}
	...
	*pt = t // t now at a leaf.
	...
	for t.parent != nil && t.parent.priority > t.priority {
		if t != nil && t.spanKey.npages != t.npagesKey {
			println("runtime: equalTo:40 t=", t, "t.npagesKey=", t.npagesKey)
...
func (root *mTreap) remove(npages uintptr) *mspan {
	...
	t := *ps
	for ; t != nil; t = *ps {
		if t.equalTo(npages, nil) {
			throw("treap node with nilspan found")
			goto Found
		}
		if t.greaterThan(npages, nil) {
			if t.left == nil {
				goto Found
			}
			ps = &t.left
			...
			if t.right == nil {
				// look up the tree to find first node that fits.
				if t.parent == nil {
					return nil
				}
				for ; t.lessThan(npages, nil); t = t.parent {
					if t.parent == nil {
						// An all right traversal means there are no nodes with enough npages.
						// Detect this by following the path back to the root (parent == nil)
						return nil
					}
				}
				// s is the best fit in this tree.
				goto Found
			}
			ps = &t.right
		}
...
func (root *mTreap) removeSpan(span *mspan) {
	...
	s := *ps
	for ; s != nil; s = *ps {
		if s.equalTo(npages, span) {
			goto Found
		}
		if s.greaterThan(npages, span) {
			if s.left == nil {
				throw("span not found")
			}
			ps = &s.left
		} else {
			if !s.lessThan(npages, span) {
				throw("treap not valid")
			}
			if s.right == nil {
				throw("span not found")
			}
...
Found:
	// Rotate s down to be leaf of tree for removal, respecting priorities.
	for s.right != nil || s.left != nil {
		if s.right == nil || s.left != nil && s.left.priority < s.right.priority {
			root.rotateRight(s)
		} else {
			root.rotateLeft(s)
		}
	...
	s.spanKey = nil
	s.npagesKey = 0
	mheap_.treapalloc.free(unsafe.Pointer(s))
	return
}
...
func scavengetreap(treap *treapNode, now, limit uint64) uintptr {
	...
	result := scavengeTreapNode(treap, now, limit)
	if treap.left != nil {
		result += scavengetreap(treap.left, now, limit)
	}
	if treap.right != nil {
		result += scavengetreap(treap.right, now, limit)
	}
	return result
}
