...
// Trigger the concurrent GC when the allocated heap has reached
// triggerratio times the growth allowed by gcpercent.
...
func gcMark(start_time int64) {
	...
	memstats.next_gc = memstats.heap_live + uint64(float64(memstats.heap_live*uint64(gcpercent)/100)*triggerratio)
	...
	if debug.gctrace > 0 {
		...
		if heap1 != memstats.heap_live {
			print("runtime: mstats skew: heap=", heap1, "/", memstats.heap_live, "\n")
			throw("mstats skew")
		}
