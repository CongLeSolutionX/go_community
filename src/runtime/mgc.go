...
var work struct {
	...
	// usedForced indicates the current GC cycle was forced by an
	// explicit user call.
	userForced bool
...
func GC() {
	...
	//
	// Since we're now done with sweeping, we have everything we
	// need to present the snapshot consistent with mark
	// termination N+1.
	//
	// Prevent preemption so that we can consistently check that
	// we're still in cycle N+1.
	mp := acquirem()
	if work.cycles == n+1 {
		mProf_postSweep()
	}
