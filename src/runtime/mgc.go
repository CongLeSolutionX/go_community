...
func shouldtriggergc() bool {
	return memstats.heap_live >= memstats.next_gc && atomic.Atomicloaduint(&bggc.working) == 0
}
...
func gc(mode int) {
	...
	if mode == gcBackgroundMode { // Do as much work concurrently as possible
		...
		systemstack(func() {
			...
			// mutators.
			atomic.Atomicstore(&gcBlackenEnabled, 1)
	...
	atomic.Atomicstore(&gcBlackenEnabled, 0)
	...
	pauseNS += now - pauseStart
	atomic.Atomicstore64(&memstats.last_gc, uint64(unixNow)) // must be Unix time to make sense to user
...
func gcMarkWorkAvailable(p *p) bool {
	...
	if atomic.Atomicload64(&work.full) != 0 || atomic.Atomicload64(&work.partial) != 0 {
