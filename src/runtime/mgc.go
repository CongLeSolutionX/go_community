...
func gcSetTriggerRatio(triggerRatio float64) {
	...
	if gcpercent >= 0 {
		...
		if !gosweepdone() {
			...
			sweepMin := memstats.heap_live + sweepMinHeapDistance*uint64(gcpercent)/100
		...
		if int64(trigger) < 0 {
			print("next_gc=", memstats.next_gc, " heap_marked=", memstats.heap_marked, " heap_live=", memstats.heap_live, " initialHeapLive=", work.initialHeapLive, "triggerRatio=", triggerRatio, " minTrigger=", minTrigger, "\n")
	...
	if gosweepdone() {
		...
		heapDistance := int64(trigger) - int64(heapLive)
		...
		if sweepDistancePages <= 0 {
			...
			mheap_.sweepHeapLiveBasis = heapLive
