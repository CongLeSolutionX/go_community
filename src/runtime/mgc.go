...
func gcMark(start_time int64) {
	...
	// Flush localScan from each mcache since we're about to modify
	// heap_scan directly. If we were to flush this later, then localScan
	...
	for _, p := range allp {
		...
		memstats.heap_scan += uint64(c.localScan)
		c.localScan = 0
	}
