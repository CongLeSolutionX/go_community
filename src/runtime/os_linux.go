...
func syscall_runtime_doAllThreadsSyscall(trap, a1, a2, a3, a4, a5, a6 uintptr) (r1, r2, err uintptr) {
	...
	// STW is not required for correctness. However, without STW other
	// goroutines may observe thread state switching back and forth (e.g.,
	// new UID, old UID, new UID) if they migrate between Ms that are
	// transiently in different thread states.
	//
	// STW for a more consistent experience: goroutines never observe old
	// thread state after new thread state.
	...
	// required.
	//
	// For most syscalls (e.g., setuid), this would be fine, as they are
	// idempotent, but we take the conservative approach and avoid this in
	// case callers use an exotic syscall that is not idempotent.
