...
// useTimerCreateProfiler enables the per-thread CPU profiler
const useTimerCreateProfiler = true

// ignoreSIGPROF compares this signal delivery's code to the best available
// SIGPROF source for this thread. It returns whether the delivery should be
// ignored, either because there's a better source available or because the
// current thread cannot make guarantees about which sources are available (such
// as when the thread isn't managed by the Go runtime).
//
//go:nosplit
...
// destroyThreadTimer cleans up any valid profiling timer for the thread mp. It
// clears both the valid flag and the id of the timer, and destroys the timer.
// It does not modify the shutdown flag.
