...
// doAllThreadsSyscall.
//
// perThreadSyscall may only be written while mp.needPerThreadSyscall == 0.
...
func syscall_runtime_doAllThreadsSyscall(trap, a1, a2, a3, a4, a5, a6 uintptr) (r1, r2, err uintptr) {
	...
	// N.B. Locking disables preemption, preventing us from changing Ms.
	...
	for {
		...
		for mp := allm; done && mp != nil; mp = mp.alllink {
			...
			done = mp.needPerThreadSyscall.Load() == 0
		}
		if done {
			break
		}
...
func runPerThreadSyscall() {
	...
	r1, r2, errno := syscall_RawSyscall6(args.trap, args.a1, args.a2, args.a3, args.a4, args.a5, args.a6)
	if GOARCH == "ppc64" || GOARCH == "ppc64le" {
		// ppc64 doesn't use r2.
		r2 = 0
	}
