...
func memAlloc(n uintptr) unsafe.Pointer {
	...
	for {
		if p.size >= n {
			...
			memFreelist = prevp
			return unsafe.Pointer((uintptr(unsafe.Pointer(p))) + unsafe.Sizeof(memHdr{}))
		}
		if p == memFreelist {
			p = morecore(n)
			if p == nil {
...
func morecore(n uintptr) *memHdr {
	...
	up := (*memHdr)(cp)
	up.size = n
	memFree(unsafe.Pointer((uintptr(cp))+unsafe.Sizeof(memHdr{})), n)
	return memFreelist
}

func memFree(ap unsafe.Pointer, n uintptr) {
	bp := (*memHdr)(unsafe.Pointer(uintptr(ap) - unsafe.Sizeof(memHdr{})))
	bpn := uintptr(unsafe.Pointer(bp))
	var p *memHdr
	pn := uintptr(unsafe.Pointer(p))
	for p = memFreelist; !(bpn > pn && bpn < uintptr(unsafe.Pointer(p.next))); p = p.next {
		bpn = uintptr(unsafe.Pointer(bp))
		pn = uintptr(unsafe.Pointer(p))
