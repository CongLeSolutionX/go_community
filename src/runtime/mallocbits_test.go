	...
	tests := map[string]struct {
		...
		"None": {
			hits: []hit{
				...
				{5, 3, 0},
				{2, 11, 0},
				{PagesPerArena/4 + 1, PagesPerArena / 2, 0},
				{0, PagesPerArena, 0},
			},
		},
		"All": {
			s: []BitRange{{0, PagesPerArena}},
			hits: []hit{
				{0, 1, 1},
				{5, 3, 3},
				{2, 11, 11},
				{PagesPerArena/4 + 1, PagesPerArena / 2, PagesPerArena / 2},
				{0, PagesPerArena, PagesPerArena},
			},
		},
		"Half": {
			s: []BitRange{{PagesPerArena / 2, PagesPerArena / 2}},
			hits: []hit{
				{0, 1, 0},
				{5, 3, 0},
				{2, 11, 0},
				{PagesPerArena/2 - 1, 1, 0},
				{PagesPerArena / 2, 1, 1},
				{PagesPerArena/2 + 10, 1, 1},
				{PagesPerArena/2 - 1, 2, 1},
				{PagesPerArena / 4, PagesPerArena / 4, 0},
				{PagesPerArena / 4, PagesPerArena/4 + 1, 1},
				{PagesPerArena/4 + 1, PagesPerArena / 2, PagesPerArena/4 + 1},
				{0, PagesPerArena, PagesPerArena / 2},
			},
		},
		"OddBound": {
			s: []BitRange{{0, 111}},
			hits: []hit{
				...
				{99, 1, 1},
				{120, 1, 0},
				{PagesPerArena / 2, PagesPerArena / 2, 0},
				{0, PagesPerArena, 111},
			},
		},
		"Scattered": {
...
func TestMallocBitsSummarize(t *testing.T) {
	tests := map[string]struct {
		...
		"StartAndEnd": {
			free:  []BitRange{{0, 11}, {PagesPerArena - 23, 23}},
			start: 11,
			max:   23,
			end:   23,
		},
		"StartMaxEnd": {
			free:  []BitRange{{0, 4}, {50, 100}, {PagesPerArena - 4, 4}},
			start: 4,
			max:   100,
			end:   4,
		},
		"OnlyMax": {
			free: []BitRange{{1, 20}, {35, 241}, {PagesPerArena - 50, 30}},
			max:  241,
		},
		"MultiMax": {
