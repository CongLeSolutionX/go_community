...
func TestMallocBitsAllocRange(t *testing.T) {
	test := func(t *testing.T, i, n uint, want *MallocBits) {
...
func TestMallocBitsPopcntRange(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"All": {
			init: []BitRange{{0, MallocChunkPages}},
			hits: []hit{
				{0, 1, 1},
				{5, 3, 3},
				{2, 11, 11},
		...
		"Half": {
			init: []BitRange{{MallocChunkPages / 2, MallocChunkPages / 2}},
			hits: []hit{
				{0, 1, 0},
				{5, 3, 0},
				{2, 11, 0},
...
func TestMallocBitsSummarizeRandom(t *testing.T) {
	b := new(MallocBits)
	for i := 0; i < 1000; i++ {
		// Randomize bitmap.
		for i := range b {
		...
		checkMallocSum(t, b.Summarize(), SummarizeSlow(b))
	}
}
...
func TestMallocBitsAlloc(t *testing.T) {
	tests := map[string]struct {
		...
		"ExactFit1": {
			...
			npages: 1,
			hits:   []uint{MallocChunkPages/2 - 3, ^uint(0)},
			after:  []BitRange{{0, MallocChunkPages}},
		},
		"ExactFit2": {
		...
		"ExactFit65": {
			...
			npages: 65,
			hits:   []uint{MallocChunkPages/2 - 31, ^uint(0)},
			after:  []BitRange{{0, MallocChunkPages}},
		},
		"SomeFree161": {
