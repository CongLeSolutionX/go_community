...
// word. Most other Ms must attempt to spend their time sleeping to reduce
// traffic on the cache line. This is the "spin bit" for which the
// implementation is named. (The anti-starvation mechanism also grants temporary
// permission for an M to spin.)
//
...
type mWaitList struct {
	...
	// bits of muintptr values available for flags.
	_ [700 * (2 - goarch.PtrSize/4)]byte
}
...
func unlock2wake(l *mutex) {
	...
	if !antiStarve && v&mutexSpinning != 0 {
		return
	}
