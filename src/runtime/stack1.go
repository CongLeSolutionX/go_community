...
func newstack() {
	...
	if thisg.M.Morebuf.G.Stackguard0 == _lock.StackFork {
		_lock.Gothrow("stack growth after fork")
	}
	if thisg.M.Morebuf.G != thisg.M.Curg {
		...
		_lock.Gothrow("runtime: wrong goroutine in newstack")
	}
	if thisg.M.Curg.Throwsplit {
		...
		_lock.Gothrow("runtime: stack split at bad time")
	}
	...
	if gp.Stack.Lo == 0 {
		_lock.Gothrow("missing stack in newstack")
	}
	sp := gp.Sched.Sp
	...
	if sp < gp.Stack.Lo {
		...
		_lock.Gothrow("runtime: split stack overflow")
	}
	...
	if gp.Stackguard0 == _lock.StackPreempt {
		if gp == thisg.M.G0 {
			_lock.Gothrow("runtime: preempt g0")
		}
		if thisg.M.P == nil && thisg.M.Locks == 0 {
			_lock.Gothrow("runtime: g is running but p is not")
		}
		if gp.Preemptscan {
...
func morestackc() {
	_lock.Systemstack(func() {
		_lock.Gothrow("attempt to execute C code on Go stack")
	})
}
