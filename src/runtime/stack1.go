...
func newstack() {
	...
	if thisg.M.Morebuf.G.Ptr().Stackguard0 == _lock.StackFork {
		_lock.Throw("stack growth after fork")
	}
	if thisg.M.Morebuf.G.Ptr() != thisg.M.Curg {
		print("runtime: newstack called from g=", thisg.M.Morebuf.G, "\n"+"\tm=", thisg.M, " m->curg=", thisg.M.Curg, " m->g0=", thisg.M.G0, " m->gsignal=", thisg.M.Gsignal, "\n")
		morebuf := thisg.M.Morebuf
		_lock.Traceback(morebuf.Pc, morebuf.Sp, morebuf.Lr, morebuf.G.Ptr())
	...
	if thisg.M.Curg.Throwsplit {
		...
		_lock.Traceback(morebuf.Pc, morebuf.Sp, morebuf.Lr, gp)
	...
	// value now and below.
	preempt := _core.Atomicloaduintptr(&gp.Stackguard0) == _lock.StackPreempt
	...
	if preempt {
		if thisg.M.Locks != 0 || thisg.M.Mallocing != 0 || thisg.M.Preemptoff != "" || thisg.M.P.Status != _lock.Prunning {
	...
	if gp.Sched.Ctxt != nil {
		...
		_sched.Writebarrierptr_nostore((*uintptr)(unsafe.Pointer(&gp.Sched.Ctxt)), uintptr(gp.Sched.Ctxt))
	}
