...
func ready(gp *g, traceskip int) {
	...
	if atomic.Atomicload(&sched.npidle) != 0 && atomic.Atomicload(&sched.nmspinning) == 0 { // TODO: fast atomic
		wakep()
	}
	_g_.m.locks--
...
func freezetheworld() {
	...
	for i := 0; i < 5; i++ {
		...
		atomic.Atomicstore(&sched.gcwaiting, 1)
...
func readgstatus(gp *g) uint32 {
	return atomic.Atomicload(&gp.atomicstatus)
}
		...
		case _Grunnable, _Gsyscall, _Gwaiting:
			...
			if castogscanstatus(gp, s, s|_Gscan) {
				if !gp.gcscandone {
					...
					scanstack(gp)
					atomic.Atomicstore(&gp.stackLock, 0)
					gp.gcscandone = true
				}
				restartg(gp)
			}
...
func startTheWorldWithSema() {
	...
	if atomic.Atomicload(&sched.npidle) != 0 && atomic.Atomicload(&sched.nmspinning) == 0 {
		wakep()
	}
...
func forEachP(fn func(*p)) {
	...
	for _, p := range allp[:gomaxprocs] {
		if p != _p_ {
			atomic.Atomicstore(&p.runSafePointFn, 1)
		}
	}
	preemptall()
...
func lockextra(nilokay bool) *m {
	...
	for {
		old := atomic.Atomicloaduintptr(&extram)
		if old == locked {
			yield := osyield
			yield()
			continue
...
func unlockextra(mp *m) {
	atomic.Atomicstoreuintptr(&extram, uintptr(unsafe.Pointer(mp)))
}
...
func handoffp(_p_ *p) {
	...
	if atomic.Atomicload(&sched.nmspinning)+atomic.Atomicload(&sched.npidle) == 0 && atomic.Cas(&sched.nmspinning, 0, 1) { // TODO: fast atomic
		startm(_p_, true)
		return
	}
	lock(&sched.lock)
	...
	if sched.npidle == uint32(gomaxprocs-1) && atomic.Atomicload64(&sched.lastpoll) != 0 {
		unlock(&sched.lock)
		startm(_p_, false)
		return
	}
...
top:
	...
	if !_g_.m.spinning && 2*atomic.Atomicload(&sched.nmspinning) >= uint32(gomaxprocs)-atomic.Atomicload(&sched.npidle) { // TODO: fast atomic
		goto stop
	}
	if !_g_.m.spinning {
		_g_.m.spinning = true
...
stop:
	...
	if netpollinited() && atomic.Xchg64(&sched.lastpoll, 0) != 0 {
		...
		atomic.Atomicstore64(&sched.lastpoll, uint64(nanotime()))
		if gp != nil {
			lock(&sched.lock)
			_p_ = pidleget()
			unlock(&sched.lock)
...
func resetspinning() {
	...
	if _g_.m.spinning {
		...
		nmspinning = atomic.Atomicload(&sched.nmspinning)
	}

	// M wakeup policy is deliberately somewhat conservative (see nmspinning handling),
	// so see if we need to wakeup another P here.
	if nmspinning == 0 && atomic.Atomicload(&sched.npidle) > 0 {
		wakep()
	}
}
...
func reentersyscall(pc, sp uintptr) {
	...
	if atomic.Atomicload(&sched.sysmonwait) != 0 { // TODO: fast atomic
	...
	_g_.m.mcache = nil
	_g_.m.p.ptr().m = 0
	atomic.Atomicstore(&_g_.m.p.ptr().status, _Psyscall)
	if sched.gcwaiting != 0 {
...
func entersyscall_sysmon() {
	lock(&sched.lock)
	if atomic.Atomicload(&sched.sysmonwait) != 0 {
		atomic.Atomicstore(&sched.sysmonwait, 0)
		notewakeup(&sched.sysmonnote)
	}
	unlock(&sched.lock)
}
...
func exitsyscallfast_pidle() bool {
	lock(&sched.lock)
	_p_ := pidleget()
	if _p_ != nil && atomic.Atomicload(&sched.sysmonwait) != 0 {
		atomic.Atomicstore(&sched.sysmonwait, 0)
		notewakeup(&sched.sysmonnote)
	}
	unlock(&sched.lock)
...
func exitsyscall0(gp *g) {
	...
	if _p_ == nil {
		globrunqput(gp)
	} else if atomic.Atomicload(&sched.sysmonwait) != 0 {
		atomic.Atomicstore(&sched.sysmonwait, 0)
		notewakeup(&sched.sysmonnote)
	}
	unlock(&sched.lock)
...
func newproc1(fn *funcval, argp *uint8, narg int32, nret int32, callerpc uintptr) *g {
	...
	if atomic.Atomicload(&sched.npidle) != 0 && atomic.Atomicload(&sched.nmspinning) == 0 && unsafe.Pointer(fn.fn) != unsafe.Pointer(funcPC(main)) { // TODO: fast atomic
		wakep()
	}
	_g_.m.locks--
	if _g_.m.locks == 0 && _g_.preempt { // restore the preemption request in case we've cleared it in newstack
...
func sigprof(pc, sp, lr uintptr, gp *g, mp *m) {
	...
	if prof.hz != 0 {
		...
		atomic.Atomicstore(&prof.lock, 0)
	}
	mp.mallocing--
}
...
func procresize(nprocs int32) *p {
	...
	atomic.Atomicstore((*uint32)(unsafe.Pointer(int32p)), uint32(nprocs))
	return runnablePs
}
...
func sysmon() {
	...
	for {
		...
		usleep(delay)
		if debug.schedtrace <= 0 && (sched.gcwaiting != 0 || atomic.Atomicload(&sched.npidle) == uint32(gomaxprocs)) { // TODO: fast atomic
			lock(&sched.lock)
			if atomic.Atomicload(&sched.gcwaiting) != 0 || atomic.Atomicload(&sched.npidle) == uint32(gomaxprocs) {
				atomic.Atomicstore(&sched.sysmonwait, 1)
				unlock(&sched.lock)
		...
		lastpoll := int64(atomic.Atomicload64(&sched.lastpoll))
		now := nanotime()
		...
		lastgc := int64(atomic.Atomicload64(&memstats.last_gc))
		if lastgc != 0 && unixnow-lastgc > forcegcperiod && atomic.Atomicload(&forcegc.idle) != 0 && atomic.Atomicloaduint(&bggc.working) == 0 {
			lock(&forcegc.lock)
			forcegc.idle = 0
...
func retake(now int64) uint32 {
	...
	for i := int32(0); i < gomaxprocs; i++ {
		...
		if s == _Psyscall {
			...
			if runqempty(_p_) && atomic.Atomicload(&sched.nmspinning)+atomic.Atomicload(&sched.npidle) > 0 && pd.syscallwhen+10*1000*1000 > now {
				continue
			}
			// Need to decrement number of idle locked M's
...
func schedtrace(detailed bool) {
	...
	for i := int32(0); i < gomaxprocs; i++ {
		...
		mp := _p_.m.ptr()
		h := atomic.Atomicload(&_p_.runqhead)
		t := atomic.Atomicload(&_p_.runqtail)
		if detailed {
			id := int32(-1)
			if mp != nil {
				id = mp.id
...
retry:
	h := atomic.Atomicload(&_p_.runqhead) // load-acquire, synchronize with consumers
	t := _p_.runqtail
	if t-h < uint32(len(_p_.runq)) {
		_p_.runq[t%uint32(len(_p_.runq))] = gp
		atomic.Atomicstore(&_p_.runqtail, t+1) // store-release, makes the item available for consumption
		return
	}
...
func runqget(_p_ *p) (gp *g, inheritTime bool) {
	...
	for {
		h := atomic.Atomicload(&_p_.runqhead) // load-acquire, synchronize with other consumers
		t := _p_.runqtail
		if t == h {
			return nil, false
		}
...
func runqgrab(_p_ *p, batch *[256]*g, batchHead uint32, stealRunNextG bool) uint32 {
	for {
		h := atomic.Atomicload(&_p_.runqhead) // load-acquire, synchronize with other consumers
		t := atomic.Atomicload(&_p_.runqtail) // load-acquire, synchronize with the producer
		n := t - h
		n = n - n/2
		if n == 0 {
			if stealRunNextG {
...
func runqsteal(_p_, p2 *p, stealRunNextG bool) *g {
	...
	atomic.Atomicstore(&_p_.runqtail, t+n) // store-release, makes the item available for consumption
	return gp
}
