...
func isscanstatus(status uint32) bool {
	if status == _lock.Gscan {
		_lock.Gothrow("isscanstatus: Bad status Gscan")
	}
...
func stopscanstart(gp *_core.G) {
	_g_ := _core.Getg()
	if _g_ == gp {
		_lock.Gothrow("GC not moved to G0")
	}
	if _gc.Stopg(gp) {
		if !isscanstatus(_lock.Readgstatus(gp)) {
			_sched.Dumpgstatus(gp)
			_lock.Gothrow("GC not in scan state")
		}
		_gc.Restartg(gp)
	}
}
...
func park(unlockf func(*_core.G, unsafe.Pointer) bool, lock unsafe.Pointer, reason string) {
	_g_ := _core.Getg()
...
func parkunlock(lock *_core.Mutex, reason string) {
	park(_sched.Parkunlock_c, unsafe.Pointer(lock), reason)
}
...
func goexit0(gp *_core.G) {
	...
	if _g_.M.Locked&^_cgo.LockExternal != 0 {
		...
		_lock.Gothrow("internal lockOSThread error")
	}
	_g_.M.Locked = 0
	gfput(_g_.M.P, gp)
	_sched.Schedule()
...
func newproc1(fn *_core.Funcval, argp *uint8, narg int32, nret int32, callerpc uintptr) *_core.G {
	...
	if fn == nil {
		...
		_lock.Gothrow("go of nil func value")
	}
	...
	if siz >= _core.StackMin-4*_lock.RegSize-_lock.RegSize {
		_lock.Gothrow("newproc: function arguments too large for new goroutine")
	}
	...
	if newg.Stack.Hi == 0 {
		_lock.Gothrow("newproc1: newg missing stack")
	}

	if _lock.Readgstatus(newg) != _lock.Gdead {
		_lock.Gothrow("newproc1: new g is not Gdead")
	}
...
func gfput(_p_ *_core.P, gp *_core.G) {
	if _lock.Readgstatus(gp) != _lock.Gdead {
		_lock.Gothrow("gfput: bad status (not Gdead)")
	}
...
func testSchedLocalQueue() {
	...
	for i := 0; i < len(_p_.Runq); i++ {
		if _sched.Runqget(_p_) != nil {
			_lock.Gothrow("runq is not empty initially")
		}
		for j := 0; j < i; j++ {
		...
		for j := 0; j < i; j++ {
			if _sched.Runqget(_p_) != &gs[i] {
				...
				_lock.Gothrow("bad element")
			}
		}
		if _sched.Runqget(_p_) != nil {
			_lock.Gothrow("runq is not empty afterwards")
		}
	}
}
