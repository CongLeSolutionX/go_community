...
func mstart1() {
	...
	if unsafe.Pointer(_g_.m.mstartfn) != nil {
...
func newm(fn func(), _p_ *p) {
	mp := allocm(_p_)
	// procresize make _p_ reachable through allp, which never changes, so WB can be eliminated
	...
	if iscgo {
		...
		// mp is reachable via allm and mp.g0 is never changes, so WB can be eliminated.
...
func exitsyscall(dummy int32) {
	...
	if trace.enabled {
		...
		// TODO: Figure out how to get tracGoSysExit into the trace log or
