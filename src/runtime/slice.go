...
//
// Note, there are specialized versions of this function, which should be
// kept in sync with this function.
...
// growslicebyte is a specialized function of growslice for []byte.
func growslicebyte(oldPtr unsafe.Pointer, newLen, oldCap, num int) slice {
	oldLen := newLen - num
	if raceenabled {
		callerpc := getcallerpc()
		racereadrangepc(oldPtr, uintptr(oldLen), callerpc, abi.FuncPCABIInternal(growslicebyte))
	}
	if msanenabled {
		msanread(oldPtr, uintptr(oldLen))
	}
	if asanenabled {
		asanread(oldPtr, uintptr(oldLen))
	}

	if newLen < 0 {
		panic(errorString("growslicebyte: len out of range"))
	}
...
// growslice32 is a specialized function of growslice for types with 4 bytes and no pointers.
func growslice32(oldPtr unsafe.Pointer, newLen, oldCap, num int) slice {
	const et_size = 4

	oldLen := newLen - num
	if raceenabled {
		callerpc := getcallerpc()
		racereadrangepc(oldPtr, uintptr(oldLen*int(et_size)), callerpc, abi.FuncPCABIInternal(growslice32))
	}
	if msanenabled {
		msanread(oldPtr, uintptr(oldLen*int(et_size)))
	}
	...
	if newLen < 0 {
		panic(errorString("growslice32: len out of range"))
	}
...
// growslice64 is a specialized function of growslice for types with 8 bytes and no pointers.
func growslice64(oldPtr unsafe.Pointer, newLen, oldCap, num int) slice {
	const et_size = 8

	oldLen := newLen - num
	if raceenabled {
		callerpc := getcallerpc()
		racereadrangepc(oldPtr, uintptr(oldLen*int(et_size)), callerpc, abi.FuncPCABIInternal(growslice64))
	}
	if msanenabled {
		msanread(oldPtr, uintptr(oldLen*int(et_size)))
	}
	...
	if newLen < 0 {
		panic(errorString("growslice64: len out of range"))
	}
