...
func (w traceWriter) refill() traceWriter {
	...
	w.varint(uint64(mID))
	w.varint(uint64(ts))
	w.traceBuf.lenPos, w.traceBuf.lenSize = w.varintReserve()
	return w
}
...
type traceBufHeader struct {
	...
	lenSize  int       // encoded size of batch length value
}
...
func (buf *traceBuf) varintReserve() (pos, encodeSize int) {
	// TODO: This could reserve for some maximum value, to avoid
	// taking the 10 byte hit every time.
	p := buf.pos
	buf.pos += traceBytesPerNumber
	return p, traceBytesPerNumber
}
...
// consumes encodeSize bytes. This is intended for when the caller
// needs to reserve space for a varint but can't populate it until
// later.
func (buf *traceBuf) varintAt(pos int, v uint64, encodeSize int) {
	...
	if v != 0 {
		throw("v could not fit in encodeSize")
	}
}
...
func traceBufFlush(buf *traceBuf, gen uintptr) {
	...
	// Write out the buffer's length in the header.
	buf.varintAt(buf.lenPos, uint64(buf.pos-(buf.lenPos+buf.lenSize)), buf.lenSize)
