...
// If it is not provided, the barrier will still behave as expected.
// However if it is provided, it *must* fully describe the layout
// of the space between [dst, dst+size) and [src, src+size). That is,
// if a type is provided, this function assumes that the first word
// at dst corresponds to the first bit of typ.GCData (if it is not
// a gcprog) and so on. Callers must use this optimization very
// carefully. When in doubt, passing nil is always safe.
//
