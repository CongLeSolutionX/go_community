...
func scavengeSleep(ns int64) int64 {
	...
	// If we're not being asked to sleep for very long,
	// just spin and deschedule. We should not go around this
...
func bgscavenge(c chan int) {
	...
	// Exponentially-weighted moving average of the percent of time this goroutine spends
	// scavenging. It represents a measure of scheduling overheads which might extend the sleep
...
func (s *pageAlloc) scavengeone(max uintptr, locked bool) uintptr {
	...
	ai := arenaIndex(s.scavAddr)
	a := s.arenas(ai)
	if a != nil {
		ci := arenaChunkIndex(s.scavAddr)
		base, npages := a.pageAlloc[ci].findScavengeCandidate(chunkPageIndex(s.scavAddr), maxPages)
...
nextChunk:
	for i := top - 1; i >= s.start; i-- {
		...
		if a == nil {
			...
			throw("found non-zero summary for nil heapArena")
		}
		base, npages := a.pageAlloc[i%mallocChunksPerArena].findScavengeCandidate(mallocChunkPages-1, maxPages)
		if npages == 0 {
...
//
// s.mheap must be locked.
func (s *pageAlloc) scavengeRangeLocked(i arenaIdx, a *heapArena, chunk, base, npages int) {
	a.pageAlloc[chunk].scavengeRange(base, npages)

	// Compute the full address for the start of the range.
	addr := arenaBase(i) + uintptr(chunk)*mallocChunkBytes + uintptr(base)*pageSize
