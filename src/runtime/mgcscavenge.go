...
func gcPaceScavenger(memoryLimit int64, heapGoal, lastHeapGoal uint64) {
	...
	// As described at the top of this file, there are two worlds here: one in which
	// we do not have a memory limit, and one in which we do. Let's handle the latter
	// first because it's simpler.
	if memoryLimit != maxInt64 {
		// We want to target retaining (100-reduceExtraPercent)% of the heap.
		retainedGoal := uint64(float64(memoryLimit) * (100.0 - reduceExtraPercent))

		// mappedReady is comparable to memoryLimit, and represents how much total memory
		// the Go runtime has committed now (estimated).
		retainedNow := gcController.mappedReady.Load()

		// If we're below the goal already, or within one page of it, just disable
		// the background scavenger. This may seems worrisome at first, but note that
		// the allocator will assist the background scavenger in the face of a memory
		...
		if retainedNow <= retainedGoal || retainedNow-retainedGoal < uint64(physPageSize) {
			mheap_.scavengeGoal.Store(^uint64(0))
			return
		}
		// Mark the bottom bit as us being in the memory limit case.
		mheap_.scavengeGoal.Store(retainedGoal | 1)
		return
	}
	...
	// Mark the bottom bit as us not being in the memory limit case.
	mheap_.scavengeGoal.Store(retainedGoal &^ 1)
}
...
func (s *scavengerState) init() {
	...
	if s.shouldStop == nil {
		s.shouldStop = func() bool {
			...
			goal := mheap_.scavengeGoal.Load()
			if goal&1 == 0 {
				return heapRetained() <= goal
			}
			return gcController.mappedReady.Load() <= goal
		}
	}
	if s.gomaxprocs == nil {
...
type scavengeIndex struct {
	...
	//
	// searchAddr is the maximum address containing memory available to scavenge. It is a
	// hint to the find operation to avoid O(n^2) behavior in repeated lookups.
	//
	...
	// scavenge. However, mark, when marking a new chunk at an indiex greater than the current
	...
	// iteration, but alas we don't currently support AND and OR operations on larger
	// atomics.
	searchAddr atomic.Int64
	chunks     []atomic.Uint8
...
func (s *scavengeIndex) find() (chunkIdx, uint) {
	...
	var increased bool
	if searchAddr < 0 {
		// The searchAddr is negative, which means something increased it.
		// Mark this because later we're going to try to take ownership of
		// reducing searchAddr the first time after the increase.
		increased = true
	...
	for i := start; i >= min; i-- {
		...
		// Try to reduce searchAddr.
		newSearchAddr := int64(chunkBase(ci) + pallocChunkBytes - pageSize)
		...
		if increased {
			s.searchAddr.CompareAndSwap(-searchAddr, newSearchAddr)
			return ci, pallocChunkPages - 1
		}
		for {
			// Decrease searchAddr.
			old := s.searchAddr.Load()
			if old <= newSearchAddr {
				break
			}
			if s.searchAddr.CompareAndSwap(old, newSearchAddr) {
				break
			}
		}
		return ci, pallocChunkPages - 1
	...
	for {
		...
		if s.searchAddr.CompareAndSwap(old, 0) {
			break
		}
	}
	return 0, 0
...
func (s *scavengeIndex) mark(base, limit uintptr) {
	...
	searchAddr := s.searchAddr.Load()
	if searchAddr < 0 {
		// We've read an update from a mark that hasn't been consumed yet.
		searchAddr = -searchAddr
	}
	...
	if int64(newSearchAddr) > searchAddr {
		s.searchAddr.Store(-int64(newSearchAddr))
	}
}
