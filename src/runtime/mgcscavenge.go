...
func gcPaceScavenger() {
	...
	// If we're already below our goal, then disable the background scavenger.
...
func scavengeSleep(ns int64) int64 {
	...
	// If we're being asked to sleep for longer, let's park
	// in earnest.
	lock(&scavenge.lock)
...
func bgscavenge(c chan int) {
	...
	// Exponentially-weighted moving average of the ratio of time this
	// goroutine spends scavenging (that is, percent of a single CPU) to
	// time it spends sleeping. It represents a measure of scheduling
	// overheads which might extend the sleep or the critical time beyond
	// what's expected. Assume no overhead to begin with.
	//
	...
	const idealRatio = scavengePercent / (100.0 - scavengePercent)
	scavengeEWMA := float64(idealRatio)
	...
	for {
		...
		// much, then scavengeEMWA < idealRatio, so we'll adjust the sleep time
		// down.
		adjust := scavengeEWMA / idealRatio
		...
		const alpha = 0.5
		scavengeEWMA = alpha*ratio + (1-alpha)*scavengeEWMA
	}
}
...
func (s *pageAlloc) scavengeOne(max uintptr, locked bool) uintptr {
	...
	for i := top - 1; i >= s.start; i-- {
		...
		// Check if there are any free pages at all. It's imperative that we
		...
		// optimistic.
		if !s.chunks[i].hasScavengeCandidate() {
			continue
		}
		...
		// all the way up to where we searched as scavenged for future calls
		// and keep iterating.
...
func (m *mallocData) hasScavengeCandidate() bool {
...
// note that findScavengeCandidate searches backwards through the mallocData. As a
...
func (m *mallocData) findScavengeCandidate(searchIdx uint, max uintptr) (uint, uint) {
	i := int(searchIdx / 64)
	...
	if x<<z1 != 0 {
		...
		run = 64 - z1
		for j := i - 1; j >= 0; j-- {
			x := m.scavenged[j] | m.mallocBits[j]
	...
	// Split the run we found if it's larger than maxm but hold on to
