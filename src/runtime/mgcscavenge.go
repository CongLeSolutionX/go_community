...
func scavengeSleep(ns int64) bool {
	...
	gopark(func(_ *g, mtx unsafe.Pointer) bool {
		// If there was an update, just abort the sleep.
		// It's safe to acquire mheap_.lock here because
		// we're running on the system stack.
		lock(&mheap_.lock)
		hasUpdate := mheap_.scavengeGen != scavenge.gen
		unlock(&mheap_.lock)

		if hasUpdate {
			// Stop the timer since we're not actually
		...
		unlock((*mutex)(mtx))
		return true
	}, unsafe.Pointer(&scavenge.lock), waitReasonSleep, traceEvGoSleep, 2)
...
func bgscavenge(c chan int) {
	...
	// Exponentially-weighted moving average of the ratio of actual time to
	// wake-up vs. the requested sleep time. It represents a measure of
	// scheduling overheads which might extend the sleep beyond the requested
	// time. Assume no overhead to begin with.
	overheadEWMA := float64(1.0)
	...
	for {
		...
		if crit < 100e6 {
			// There isn't much time until we'll be ready to go again, just yield
			// for something else and we'll be back here soon enough.
			Gosched()
			continue
		}
		...
		expect := int64(float64(crit) / (scavengePercent / 100.0) / overheadEWMA)

		// Go to sleep, measuring how long we actually sleep for.
		start := nanotime()
		scavengeSleep(expect)
		actual := start - nanotime()

		// Update the overhead average.
		overheadEWMA /= 2
		overheadEWMA += (float64(actual) / float64(expect)) / 2
	}
}
