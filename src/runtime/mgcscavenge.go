...
func (s *pageAlloc) scavenge(nbytes uintptr, mayUnlock bool) uintptr {
	...
	for released < nbytes && addrs.size() != 0 {
		r, a := s.scavengeOne(addrs, nbytes-released, mayUnlock)
		released += r
		addrs = a
		if released < nbytes && addrs.size() == 0 {
			addrs, gen = s.scavengeReserve()
			continue
		}
	}
...
func (s *pageAlloc) scavengeReserve() (addrRange, uint32) {
	size := alignUp(s.inUse.totalSpace, pallocChunkBytes) / scavengeReservationShards
...
func (s *pageAlloc) scavengeOne(iter addrRange, max uintptr, mayUnlock bool) (uintptr, addrRange) {
	...
	for i := chunkIndex(iter.end() - 1); i >= chunkIndex(iter.start()); i-- {
		...
		// We were fooled, so let's continue on to the next chunk.
		unlockHeap()
	}
	lockHeap()
