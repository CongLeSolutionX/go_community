...
func (s *pageAlloc) scavengeOne(max uintptr, locked bool) uintptr {
	...
	for i := top - 1; i >= s.start; i-- {
		...
		chunk := &s.chunks[i]
		base, npages := chunk.findScavengeCandidate(pallocChunkPages-1, maxPages)
		if npages > 0 {
...
// in the region represented by this pallocData.
func (m *pallocData) hasScavengeCandidate() bool {
	...
	for i := len(m.scavenged) - 1; i >= 0; i-- {
		...
		x := m.scavenged[i] | m.pallocBits[i]
...
//
// max is a searchIdx for how big of a region is desired. If max >= pallocChunkPages, then
...
func (m *pallocData) findScavengeCandidate(searchIdx uint, max uintptr) (uint, uint) {
	i := int(searchIdx / 64)
	...
	if x<<z1 != 0 {
		...
		run = 64 - z1
		for j := i - 1; j >= 0; j-- {
			x := m.scavenged[j] | m.pallocBits[j]
