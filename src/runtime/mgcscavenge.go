...
func (s *pageAlloc) scavengeReserve() (addrRange, uint32) {
	// If we have a large, contiguous heap, we want to split up the
	// heap into shards, but each shard must have a minimum size of
	// pallocChunkBytes because this is the unit of work the scavenger
	// operates on. This means for small heaps we may divide the heap
	// into many fewer (larger) shards, so we may hide a larger relative
	// portion of the heap from e.g. the synchronous scavenger.
	chunksTotal := alignUp(s.inUse.totalBytes, pallocChunkBytes) / pallocChunkBytes
	chunksPerShard := alignUp(chunksTotal, scavengeReservationShards) / scavengeReservationShards

	// removeLastChunks requires that scav.inUse's addrRanges are always
	// aligned to pallocChunkBytes. We maintain this requirement by:
	// 1) Ensuring scavengeOne always returns an aligned addrRange.
	// 2) Mapping heap memory in pallocChunkBytes at a minimum.
	// 3) Only manipulating scav.inUse in scavengeReserve and scavengeUnreserve.
	return s.scav.inUse.removeLastChunks(uint(chunksPerShard)), s.scav.gen
}
