...
func (s *pageAlloc) resetScavengeAddr() {
	s.scavAddr = chunkBase(s.end+1) - 1
}
...
func (s *pageAlloc) scavengeone(max uintptr, locked bool) uintptr {
	...
	lockHeap()
	if chunkIndex(s.scavAddr) < s.start {
...
nextArena:
	for i := top - 1; i >= arenaIdx(int(s.start)/mallocChunksPerArena); i-- {
		for j := mallocChunksPerArena - 1; j >= 0; j-- {
			...
			if s.summary[len(s.summary)-1][arenaToChunkIndex(i, j)] == 0 {
				continue
			}
			...
			a := s.arenas(i)
			if a == nil {
				sum := s.summary[len(s.summary)-1][arenaToChunkIndex(i, j)]
				print("runtime: arenaIndex = ", i, ", arenaChunkIndex = ", j, "\n")
			...
			if npages == 0 {
				...
				s.scavAddr = arenaBase(i-1) + pagesPerArena*pageSize - 1
				unlockHeap()
				continue nextArena
			}
...
func (s *pageAlloc) scavengeRangeLocked(i arenaIdx, a *heapArena, base, npages int) {
	...
	addr := arenaBase(i) + uintptr(base)*pageSize
