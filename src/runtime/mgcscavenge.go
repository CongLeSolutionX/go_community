...
// max == min.
func (m *pallocData) findScavengeCandidate(searchIdx uint, _min, max uintptr) (uint, uint) {
	if _min&(_min-1) != 0 || _min == 0 {
		print("runtime: min = ", _min, "\n")
		throw("min must be a non-zero power of 2")
	} else if _min > maxPagesPerPhysPage {
		print("runtime: min = ", _min, "\n")
		throw("min too large")
	}
	...
	for ; i >= 0; i-- {
		...
		x := fillAligned(m.scavenged[i]|m.pallocBits[i], uint(_min))
		if x != ^uint64(0) {
			break
		}
	}
	...
	if x<<z1 != 0 {
		...
		run = 64 - z1
		for j := i - 1; j >= 0; j-- {
			x := fillAligned(m.scavenged[j]|m.pallocBits[j], uint(_min))
