...
type scavengerState struct {
	...
	// timeNow is a function that gets the current monotonic time
	// in nanoseconds. Usually nanotime, but stubbed out for testing.
	timeNow func() int64

	// scavenge is a function that scavenges n bytes of memory.
	// Usually mheap.pages.scavenger, but stubbed out for testing.
	scavenge func(n uintptr) uintptr
...
func (s *scavengerState) init() {
	...
	if s.timeNow == nil {
		throw("did not find stub for timeNow")
	}
	if s.scavenge == nil {
		throw("did not find stub for scavenge")
	}
	if s.shouldStop == nil {
		throw("did not find stub for shouldStop")
	}
}
...
func (s *scavengerState) sleep(worked float64) {
	...
	cpuFraction := worked / ((float64(slept) + worked) * float64(gomaxprocs))
...
func bgscavenge(c chan int) {
	scavenger.timeNow = func() int64 {
		return nanotime()
	}
	scavenger.scavenge = func(n uintptr) uintptr {
		return mheap_.pages.scavenge(n)
	}
	scavenger.shouldStop = func() bool {
