...
func traceStackID(mp *m, pcBuf []uintptr, skip int) uint64 {
	...
	curgp := mp.curg
	nstk := 1
	if tracefpunwindoff() || (curgp != nil && curgp.m != nil && curgp.m.incgocallback) {
		...
		if curgp == gp {
			nstk += fpTracebackPCs(getcallerfp(), skip, pcBuf[1:])
		} else if curgp != nil {
			// We might be called from mcall(fn) here. In that
			// sched.bp is the bp of mcall's caller, so we need capture sched.pc
			// manually here to avoid to get mcall's caller pc.
			pcBuf[1] = curgp.sched.pc
			nstk += 1 + fpTracebackPCs(curgp.sched.bp, skip, pcBuf[2:])
		}
	}
	if nstk > 0 {
...
//
//go:nosplit otherwise fp could point to the old stack
func fpTracebackPCs(fp uintptr, skip int, pcBuf []uintptr) (i int) {
	for i = 0; i < len(pcBuf) && fp != 0; i++ {
