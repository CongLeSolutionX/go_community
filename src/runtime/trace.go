...
func traceStackID(mp *m, buf []uintptr, skip int) uint64 {
	...
	// logical/inlined PCs and then to full frames in allFrames(). This makes
	// tracing much quicker, mainly because we are only doing lookups needed
	// to deal with inlining AFTER we have deduplicated stack traces.
	// physicalCallers is even faster when we can use frame pointers, since it
	// avoids the overhead of gentraceback().
	if gp == _g_ {
