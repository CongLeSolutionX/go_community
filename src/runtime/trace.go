...
func StartTrace() error {
	...
	for _, gp := range allgs {
		...
		if status != _Gdead {
			...
			// All stacks are written before events.
			var goStackID uint64
			goStackID, buf = traceWriteStack(buf, []uintptr{gp.startpc + sys.PCQuantum})
			traceEvent(traceEvGoCreate, -1, uint64(gp.goid), uint64(goStackID), stackID)
		}
		if status == _Gwaiting {
...
func (ct *traceCacheTable) findString(s string) *traceCacheEntry {
	hash := memhash(stringStructOf(&s).str, 0, uintptr(stringStructOf(&s).len))
	return ct.find(hash)
}
