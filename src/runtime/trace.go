...
func traceStackID(mp *m, buf []uintptr, skip int) uint64 {
	...
	if fpunwindoff() {
		...
		if curgp == gp {
			nstk = fpTracebackPCs(getcallerfp(), skip, buf)
		} else if curgp != nil {
			...
			nstk = fpTracebackPCs(getcallerfp(), skip+4, buf)
		}
	}
	if nstk > 0 {
...
// fpunwindoff returns false if frame pointer unwinding is disabled via GODEBUG
// or not supported by the architecture.
...
// fpunwindExpand.
//
// TODO: This does not yet include PCs generated by the cgo unwinder, if one is
// registered.
//
// go:nosplit otherwise fp could point to old stack
func fpTracebackPCs(fp uintptr, skip int, pcBuf []uintptr) int {
	if skip < 0 {
		skip = 0 // avoid -1 being interpreted as sentinel value
	}
...
// fpunwindExpand checks if pcBuf contains logical or physical frames (produced
// by frame pointer unwinding) using a sentinal value in pcBuf[0]. Logical
// frames are simply returned. Physical frames are turned into logical frames
// via inline unwinding and by applying the skip value that's stored in
// pcBuf[0].
func fpunwindExpand(pcBuf []uintptr) []uintptr {
	if len(pcBuf) > 0 && pcBuf[0] == ^uintptr(0) {
	...
	// TODO: Use new inline unwinder after CL 466099 lands
	for _, pc := range pcBuf[1:] {
		f := findfunc(pc)
		if !f.valid() {
			break
		}
