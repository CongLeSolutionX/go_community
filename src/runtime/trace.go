...
const (
	...
	traceEvGoStartLocal   = 15 // goroutine starts running on the same P as the last event [timestamp, goroutine id]
	traceEvGoEnd          = 16 // goroutine ends [timestamp]
	traceEvGoStop         = 17 // goroutine stops (like in select{}) [timestamp, stack]
	traceEvGoSched        = 18 // goroutine calls Gosched [timestamp, stack]
	traceEvGoPreempt      = 19 // goroutine is preempted [timestamp, stack]
	traceEvGoSleep        = 20 // goroutine calls Sleep [timestamp, stack]
	traceEvGoBlock        = 21 // goroutine blocks [timestamp, stack]
	traceEvGoUnblock      = 22 // goroutine is unblocked [timestamp, goroutine id, seq, stack]
	traceEvGoUnblockLocal = 23 // goroutine is unblocked on the same P as the last event [timestamp, goroutine id, stack]
	traceEvGoBlockSend    = 24 // goroutine blocks on chan send [timestamp, stack]
	traceEvGoBlockRecv    = 25 // goroutine blocks on chan recv [timestamp, stack]
	traceEvGoBlockSelect  = 26 // goroutine blocks on select [timestamp, stack]
	traceEvGoBlockSync    = 27 // goroutine blocks on Mutex/RWMutex [timestamp, stack]
	traceEvGoBlockCond    = 28 // goroutine blocks on Cond [timestamp, stack]
	traceEvGoBlockNet     = 29 // goroutine blocks on network [timestamp, stack]
	traceEvGoSysCall      = 30 // syscall enter [timestamp, stack]
	traceEvGoSysExit      = 31 // syscall exit [timestamp, goroutine id, seq, real timestamp]
	traceEvGoSysExitLocal = 32 // syscall exit on the same P as the last event [timestamp, goroutine id, real timestamp]
	traceEvGoSysBlock     = 33 // syscall blocks [timestamp]
	traceEvGoWaiting      = 34 // denotes that goroutine is blocked when tracing starts [timestamp, goroutine id]
	traceEvGoInSyscall    = 35 // denotes that goroutine is in syscall when tracing starts [timestamp, goroutine id]
	traceEvHeapAlloc      = 36 // memstats.heap_live change [timestamp, heap_alloc]
	traceEvNextGC         = 37 // memstats.next_gc change [timestamp, next_gc]
	traceEvTimerGoroutine = 38 // denotes timer goroutine [timer goroutine id]
	traceEvFutileWakeup   = 39 // denotes that the previous wakeup of this goroutine was futile [timestamp]
	traceEvCount          = 40
)
