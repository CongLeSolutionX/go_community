...
func loadOptionalSyscalls() {
	lib := stdcall1(_LoadLibraryA, uintptr(unsafe.Pointer(&kernel32dll[0])))
	if lib == 0 {
		return
	}
	_AddDllDirectory = windowsFindfunc(addDllDirectory, lib)
	_AddVectoredContinueHandler = windowsFindfunc(addVectoredContinueHandler, lib)
	_GetQueuedCompletionStatusEx = windowsFindfunc(getQueuedCompletionStatusEx, lib)
	_LoadLibraryExW = windowsFindfunc(loadLibraryExW, lib)
}
