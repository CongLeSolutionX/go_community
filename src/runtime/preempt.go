...
// satisfied, it injects a call to asyncPreempt and resumes the
// thread. This spills all registers and enters the scheduler.
...
type suspendGState struct {
	...
	// owned indicates that this suspendG transitioned the G to
	// _Gwaiting and thus is responsible for readying it when
	// done.
	owned bool
}

// suspendG suspends goroutine gp at a safe-point and returns the
// state of the suspended goroutine.
//
// It is safe for multiple callers to suspend the same goroutine at
// the same time. It will be resumed once all callers resume it.
//
...
func suspendG(gp *g) suspendGState {
	...
	for i := 0; ; i++ {
		...
		case _Gpreempted:
			...
			// We now own the G in its waiting state.
			owned = true
		...
		case _Grunnable, _Gsyscall, _Gwaiting:
			...
			return suspendGState{g: gp, owned: owned}
...
func isAsyncSafePoint(gp *g, pc, sp uintptr) bool {
	...
	if hasPrefix(funcname(f), "runtime.") {
		// For now we never async preempt the runtime. Known
		// issues include: various points in the scheduler
		// ("don't preempt between here and here"), much of
		// the defer implementation (untyped info on stack),
		// bulk write barriers (write barrier check).
		//
