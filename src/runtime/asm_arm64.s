...
chunk_loop:
	...
	VADDP	V6.B16, V6.B16, V6.B16 // 16B->8B
	VADDP	V6.B16, V6.B16, V6.B16 // 8B->4B
	VADDP	V6.B16, V6.B16, V6.B16 // 4B->2B
	VADDP	V6.B16, V6.B16, V6.B16 // 2B->1B
	VMOV	V6.B[0], R6
	ADD	R6, R11, R11
	BNE	chunk_loop
	CBZ	R2, done
	// Work with not 32-byte aligned tail
tail:
	MOVBU.P	1(R0), R5
	SUB	$1, R2, R2
	CMP	R5, R1
...
TEXT bytes·indexShortStr(SB), NOSPLIT, $0-56
	...
	SUB	R3, R1, R4
	ADD	R0, R4, R4	// R4 contains start of the last substring
	ADD	$1, R0, R27
	TBZ	$3, R3, len_2_7
len_8:
	MOVD	(R2), R5	// R5 contains sep
loop_8:
	MOVD.P	1(R0), R6
	CMP	R5, R6
	BEQ	found
