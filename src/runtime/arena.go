...
func checkArenaSetToFault() {
	...
	for ch := tempChunkList; ch != nil; ch = next {
		...
		//
		// It's OK to set it here because (1) a GC isn't in-progress, so nothing should
		// actually read this field and (2) we're currently on the system stack, so a
		// GC is blocked from starting. The STW at the start of the GC acts as a barrier
		// for this update.
