...
func TestMemmoveOverlapLarge0x110000(t *testing.T) {
	testOverlap(t, 0x110000)
}
...
func testOverlap(t *testing.T, size int) {
	...
	for n := size - 50; n > 1; n >>= 2 {
		for x := 0; x <= size-n; x = x*9 + 1 { // offset in src
			for y := 0; y <= size-n; y = y*11 + 1 { // offset in dst
				// Reset input
				copyref(test, src)
				copyref(ref, src)
