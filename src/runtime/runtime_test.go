...
func testProfileRecordNullPadding[T profileRecord](t *testing.T, name string, fn func([]T) (int, bool)) {
	...
	t.Run(name, func(t *testing.T) {
		...
		for _, sr := range p {
			for i, v := range stack0(&sr) {
				found = found || (i == 0 && v != 0)
				if v == ^uintptr(0) {
					t.Fatal("StackRecord not null zero padded")
				}
			}
		}

		if !found {
			t.Fatalf("did not find any non-empty Stack0 entries in %d records", len(p))
		}
	})
}

type profileRecord = interface {
	StackRecord | MemProfileRecord | BlockProfileRecord
}
