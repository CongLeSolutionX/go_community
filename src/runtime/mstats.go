...
type mstats struct {
	...
	// localMemStatsGen is a generation counter for stats that are
	// sharded. See mcache.go for details.
	localMemStatsGen uint32
}
...
// memStatsDelta contains a bunch of deltas of runtime
// memory statistics for how these values were changed
// on an individual P. Note that this value need not always
// wholly represent the delta to each stat made by one P.
type memStatsDelta struct {
	committed       int64 // byte delta of memory committed by a P
	released        int64 // byte delta of released memory generated by a P
	inHeap          int64 // byte delta of memory placed in the heap by a P
	inStacks        int64 // byte delta of memory reserved for stacks by a P
	inWorkBufs      int64 // byte delta of memory reserved for work bufs by a P
	inPtrScalarBits int64 // byte delta of memory reserved for unrolled GC prog bits
}

// merge adds in the deltas from b into a.
func (a *memStatsDelta) merge(b *memStatsDelta) {
...
// memStatsShard represents a shard of various memory statistics.
// One of these structures should be allocated to each live P.
//
// To write updates to memory stats use the acquire and release
// methods. To obtain a consistent global snapshot of these statistics,
// use readLocalMemStats.
type memStatsShard struct {
	// seq is a sequence number for writers to synchronize
	// with a reader. An odd value indicates that a writer
	// is currently writing to stats[localMemStatsGen%3]
	// for a potentially stale value of localMemStatsGen.
	// An even value indicates that no writer is currently
	// writing to shard. Note that this also implies that
	// future writers will notice any updates to
	// localMemStatsGen, so it's safe to read at indicies
	// (localMemStatsGen-1)%3 and (localMemStatsGen-2)%3.
	seq uint32

	// stats is a ring buffer of memStatsDelta values.
	// The one that is being written to is always determined
	// by localMemStatsGen. If a reader increases localMemStatsGen
	// and synchronizes with writers via seq, it takes
	...
	// at index (localMemStatsGen-2)%3.
	stats [3]memStatsDelta
}

// acquire returns a memStatsDelta to be updated. In effect,
...
// at a time.
func (m *memStatsShard) acquire() *memStatsDelta {
	seq := atomic.Xadd(&m.seq, 1)
	if seq%2 == 0 {
		// Should have been incremented to odd.
		print("runtime: seq=", seq, "\n")
		throw("bad sequence number")
	}
	gen := atomic.Load(&memstats.localMemStatsGen) % 3
	return &m.stats[gen]
}
...
func (m *memStatsShard) unsafeRead(out *memStatsDelta) {
	for i := range m.stats {
		out.merge(&m.stats[i])
	}
}
...
func (m *memStatsShard) unsafeClear() {
	for i := range m.stats {
		m.stats[i] = memStatsDelta{}
	}
}

// readLocalMemStats takes a globally consistent snapshot
// of sharded memory statistics and puts the aggregated value
// in out. Even though out is a memStatsDelta, the resulting
// values should be complete and valid statistic values.
//
// Not safe to call concurrently. The world must be stopped.
func readLocalMemStats(out *memStatsDelta) {
	...
	// Rotate localMemStatsGen, effectively taking
	// a snapshot of the state of these statistics at the
	// point of the exchange.
	//
	// This exchange is safe to do because we won't race
	// with anyone else trying to update this value.
	currGen := atomic.Load(&memstats.localMemStatsGen)
	atomic.Xchg(&memstats.localMemStatsGen, (currGen+1)%3)
	prevGen := currGen - 1
	if currGen == 0 {
		prevGen = 2
	}
	for _, p := range allp {
		...
		for {
			seq := atomic.Load(&m.seq)
			if seq%2 == 0 {
				break
			}
		}
		// At this point we've observed that the sequence
		// number is even, so any future writers will observe
		// the new localMemStatsGen value. That means it's
		// safe to read from the other deltas in the stats buffer.

		// Perform our responsibilities and free up
		// stats[(readGen-1)%3] for the next time we want to take
		// a snapshot.
		m.stats[currGen].merge(&m.stats[prevGen])
		m.stats[prevGen] = memStatsDelta{}

		// Finally, aggregate this P's delta into out.
		out.merge(&m.stats[currGen])
	}
	releasem(mp)
}
