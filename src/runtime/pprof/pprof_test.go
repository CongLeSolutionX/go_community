...
func TestCPUProfileInlining(t *testing.T) {
	...
	p := testCPUProfile(t, stackContains, []string{"runtime/pprof.inlinedCallee", "runtime/pprof.inlinedCaller"}, func(dur time.Duration) {
...
func TestCPUProfileRecursion(t *testing.T) {
	p := testCPUProfile(t, stackContains, []string{"runtime/pprof.inlinedCallee", "runtime/pprof.recursionCallee", "runtime/pprof.recursionCaller"}, func(dur time.Duration) {
...
func testCPUProfile(t *testing.T, matches matchFunc, need []string, f func(dur time.Duration)) *profile.Profile {
	switch runtime.GOOS {
	case "darwin":
	...
	for duration <= maxDuration {
		...
		if p, ok := profileOk(t, matches, need, prof, duration); ok {
			return p
		}
...
func profileOk(t *testing.T, matches matchFunc, need []string, prof bytes.Buffer, duration time.Duration) (_ *profile.Profile, ok bool) {
	ok = true
