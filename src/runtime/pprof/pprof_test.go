...
func TestCPUProfileMultithreadMagnitude(t *testing.T) {
	...
	// kernel.
	_, builder := os.LookupEnv("GO_BUILDER_NAME")
	if builder {
		major, minor, _, err := linuxKernelVersion()
		if err != nil {
	...
	defer func() {
		if t.Failed() {
			t.Logf("Failure of this test may indicate that your system suffers from the a known Linux kernel bug fixed on newer kernels. See https://golang.org/issue/49065.")
		}
	}()
