...
func TranslateCPUProfile(b []byte, startTime time.Time) (*profile.Profile, error) {
	...
	const minRawProfile = 5 * wordSize // Need a minimum of 5 64-bit words.
	if len(b) < minRawProfile {
		return nil, fmt.Errorf("truncated profile")
	}
	n := len(b) / wordSize
	data := ((*[1 << 24]uint)(unsafe.Pointer(&b[0])))[:n:n]
	period := data[3]
	...
	for len(b) > 0 {
		if len(data) < 2 {
			return nil, fmt.Errorf("truncated profile/1")
		}
		count := data[0]
		nstk := data[1]
		fmt.Printf("count:%v nstk: %v\n", count, nstk)
		if len(data) < int(2+nstk) {
			return nil, fmt.Errorf("truncated profile/2 %v %v", len(data), 2+nstk)
		}
		stk := data[2 : 2+nstk]
		data = data[2+nstk:]
