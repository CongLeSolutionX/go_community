...
// translateGoroutineProfile parses a goroutine profile in
// runtime/proto format into a profile.proto.
func translateGoroutineProfile(b []byte, startTime time.Time) (*profile.Profile, error) {
	p := &profile.Profile{
		Period:     1,
	...
	// Parse samples from the gorutine profile.
	locs := make(map[uint64]*profile.Location)
	r := bytes.NewBuffer(b)
	for ln, err := r.ReadString('\n'); ln != ""; ln, err = r.ReadString('\n') {
		...
		// Parse profile lines, of the form:
		// <value> @ address1 address2 address3 ...
		fields := strings.Split(strings.TrimPrefix(ln, "\n"), " ")
		if len(fields) <= 2 || fields[1] != "@" {
			// Skip lines with too few addresses.
			continue
		}
		value, err := strconv.ParseInt(fields[0], 0, 64)
		if err != nil {
			continue
		}
		sloc := make([]*profile.Location, 0, len(fields)-2)
...
// symbolize adds names to locations, where they can be found.
