...
func printCountCycleProfile(w io.Writer, countName, cycleName string, records []runtime.BlockProfileRecord) error {
	...
	for _, r := range records {
		...
		locs = b.appendLocsForStack(locs[:0], runtime_stackRecordStack(&r.StackRecord, pcbuf))
...
func (p *runtimeProfile) Stack(dst []uintptr, i int) []uintptr {
	return runtime_stackRecordStack(&p.stk[i], dst)
}
...
func runtime_memProfileRecordStack(r *runtime.MemProfileRecord, stk []uintptr) []uintptr
func runtime_stackRecordStack(r *runtime.StackRecord, stk []uintptr) []uintptr
