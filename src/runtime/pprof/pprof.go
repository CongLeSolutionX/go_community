...
// Linknamed from runtime
func runtime_fpunwindExpand(dst, src []uintptr) int
...
func printCountCycleProfile(w io.Writer, countName, cycleName string, records []profilerecord.BlockProfileRecord) error {
	...
	values := []int64{0, 0}
	var locs []uint64
	expandedStack := make([]uintptr, 128) // TODO: not hard-coded
	for _, r := range records {
		values[0] = r.Count
		values[1] = int64(float64(r.Cycles) / cpuGHz)
		// For count profiles, all stack addresses are
		// return PCs, which is what appendLocsForStack expects.
		n := runtime_fpunwindExpand(expandedStack[:], r.Stack)
...
func writeProfileInternal(w io.Writer, debug int, name string, runtimeProfile func([]profilerecord.BlockProfileRecord) (int, bool)) error {
	...
	expandedStack := make([]uintptr, 128) // TODO: don't hard code
	for i := range p {
		r := &p[i]
