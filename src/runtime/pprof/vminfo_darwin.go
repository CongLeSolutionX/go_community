...
func machVMInfo(addMapping func(lo, hi, offset uint64, file, buildID string)) bool {
	...
	for {
		...
		if isExecutable(info.Protection) {
			addMapping(addr, addr+memRegionSize, uint64(info.Offset), regionFilename(addr), "")
			added = true
		}
		addr += memRegionSize
	}
...
func mach_vm_region(address, region_size *uint64, info uintptr) int32
func proc_regionfilename(address uint64, buf uintptr, buflen int64) int32
