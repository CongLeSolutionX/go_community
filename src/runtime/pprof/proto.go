...
func (b *profileBuilder) locForPC(addr uintptr) uint64 {
	loc, ok := b.findCachedLoc([]uintptr{addr})
	if ok {
		return loc.id
	}
	...
	b.locs[addr] = append(b.locs[addr], locInfo{id: id, pcs: []uintptr{addr}})
	start := b.pb.startMessage()
...
func (b *profileBuilder) build() {
	...
	for e := b.m.all; e != nil; e = e.nextAll {
		...
		for stk := e.stk; len(stk) > 0; {
			if l, ok := b.findCachedLoc(stk); ok {
			...
			if added := deck.add(addr, frames, symbolizeResult); added {
				stk = stk[1:]
				continue
			}
			...
			if l, ok := b.findCachedLoc(stk); ok {
				...
				deck.add(addr, frames, symbolizeResult) // must succeed.
				stk = stk[1:]
			}
		}
...
func (d *pcDeck) add(pc uintptr, frames []runtime.Frame, symbolizeResult symbolizeFlag) (success bool) {
...
// findCachedLoc returns the locInfo matching with the prefix of the stk trace.
func (b *profileBuilder) findCachedLoc(stk []uintptr) (locInfo, bool) {
	// b.locs[pc] is the cached pc sequences starting with the given pc.
	locInfos := b.locs[stk[0]]
outer:
	for _, l := range locInfos {
		// skip because l.pcs is longer than the stk, so can't be a prefix of the stk.
...
func (b *profileBuilder) emitLocation(deck *pcDeck) uint64 {
	...
	b.lastLocID++
	id := b.lastLocID
	b.locs[addr] = append(b.locs[addr], locInfo{
		id:  id,
