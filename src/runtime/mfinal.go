...
func SetFinalizer(obj any, finalizer any) {
	...
	if etyp.Kind_&kindMask != kindPtr {
		throw("runtime.SetFinalizer: first argument is " + toType(etyp).string() + ", not pointer")
	}
	...
	if ftyp.Kind_&kindMask != kindFunc {
		throw("runtime.SetFinalizer: second argument is " + toType(ftyp).string() + ", not a function")
	}
	ft := (*functype)(unsafe.Pointer(ftyp))
	if ft.dotdotdot() {
		throw("runtime.SetFinalizer: cannot pass " + toType(etyp).string() + " to finalizer " + toType(ftyp).string() + " because dotdotdot")
	}
	if ft.inCount != 1 {
		throw("runtime.SetFinalizer: cannot pass " + toType(etyp).string() + " to finalizer " + toType(ftyp).string())
	}
	fint := ft.in()[0]
	switch {
	case fint == etyp:
