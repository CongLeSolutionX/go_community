...
func wakefing() *g {
	for {
		currStatus := fingStatus.Load()
		if currStatus&(fingWait|fingWake) != fingWait|fingWake {
			break
		}
		if ok := fingStatus.CompareAndSwap(currStatus, currStatus&^(fingWait|fingWake)); ok {
			return fing
		}
	}
	return nil
...
func finalizercommit(gp *g, lock unsafe.Pointer) bool {
	unlock((*mutex)(lock))
	// fingStatus should be modified on g0 to avoid waking fing in running state, even if it is about to be parked.
	...
	for {
		...
		for fb != nil {
			for i := fb.cnt; i > 0; i-- {
				...
				for {
					currStatus := fingStatus.Load()
					if ok := fingStatus.CompareAndSwap(currStatus, currStatus&^fingRunningFinalizer); ok {
						break
					}
				}
