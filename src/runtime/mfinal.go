...
// 	// Check that err == nil.
// 	p := &S{d}
// 	runtime.SetFinalizer(p, func(p *S) { syscall.Close(p.d) })
// 	var buf [10]byte
// 	n, err := syscall.Read(p.d, buf[:])
//      runtime.KeepAlive(p)
// 	// No more uses of p after this point.
//
// If the KeepAlive call were not there the finalizer could run at the
// start of syscall.Read, closing the file descriptor before
// syscall.Read makes the actual system call.
