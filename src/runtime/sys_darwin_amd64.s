...
#define	v12_gtod_generation	0x6c  // obsolete since High Sierra
#define	v12_gtod_ns_base	0x70  // obsolete since High Sierra
#define	v12_gtod_sec_base	0x78  // obsolete since High Sierra
...
TEXT time·now(SB), NOSPLIT, $32-24
	...
	// This is the new code, for macOS High Sierra and newer
v13:
	// Loop trying to take a consistent snapshot
	// of the time parameters.
timeloop13:
	...
	// Now compute wall time
	TESTQ	R12, R12
	JZ		systime
	// curr=mono-ns_base
	SUBQ	R12, AX
	// if curr > tickspersec: use syscall
	CMPQ	AX, R11
	JAE		systime
	// curr *= scale (128bit in DX:AX)
	MULQ 	R10
	// curr += hiofs/loofs
	ADDQ	R9, AX
	ADCQ	R8, DX
	...
	// convert fraction of seconds to nanoseconds
	MOVQ 	$1000000000, CX
	MULQ	CX
	MOVQ	DX, nsec+8(FP)
	RET

	// This is the legacy code needed for macOS Sierra and older
v12:
	// Loop trying to take a consistent snapshot
	// of the time parameters.
timeloop:
