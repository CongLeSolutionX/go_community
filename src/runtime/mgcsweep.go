...
type sweepLocker struct {
	...
	sweepGen uint32
	// blocked indicates that this tracker is blocking sweep
...
func (t *sweepLocker) tryAcquire(s *mspan) (sweepLocked, bool) {
	// Check before attempting to CAS.
	if atomic.Load(&s.sweepgen) != t.sweepGen-2 {
	...
	if !atomic.Cas(&s.sweepgen, t.sweepGen-2, t.sweepGen-1) {
		return sweepLocked{}, false
	}
...
// specific spans.
func (t *sweepLocker) blockCompletion() {
	if !t.blocked {
...
func (t *sweepLocker) sweepIsDone() {
...
func sweepone() uintptr {
	...
	npages := ^uintptr(0)
	var finalSweeper bool
	for {
		s := mheap_.nextSpanForSweep()
		if s == nil {
			finalSweeper = atomic.Cas(&mheap_.sweepDrained, 0, 1)
			break
		}
