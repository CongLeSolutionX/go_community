...
func mSpan_EnsureSwept(s *mspan) {
	...
	sg := mheap_.sweepgen
	if atomic.Atomicload(&s.sweepgen) == sg {
		return
	}
	...
	for atomic.Atomicload(&s.sweepgen) != sg {
		osyield()
	}
}
...
func mSpan_Sweep(s *mspan, preserve bool) bool {
	...
	heapBitsSweepSpan(s.base(), size, n, func(p uintptr) {
		...
		if cl == 0 {
			...
			atomic.Atomicstore(&s.sweepgen, sweepgen)
...
func deductSweepCredit(spanBytes uintptr, callerSweepPages uintptr) {
	...
	for pagesOwed-int64(atomic.Atomicload64(&mheap_.pagesSwept)) > int64(callerSweepPages) {
