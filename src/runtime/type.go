...
func typesEqual(t, v *_type) bool {
	...
	case kindInterface:
		...
		for i := range it.mhdr {
			...
			tname := resolveNameOff(unsafe.Pointer(&it.mhdr[0]), tm.name)
			vname := resolveNameOff(unsafe.Pointer(&iv.mhdr[0]), vm.name)
			...
			tityp := resolveTypeOff(unsafe.Pointer(&it.mhdr[0]), tm.ityp)
			vityp := resolveTypeOff(unsafe.Pointer(&iv.mhdr[0]), vm.ityp)
