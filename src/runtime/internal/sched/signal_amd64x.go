...
func Sighandler(sig uint32, info *siginfo, ctxt unsafe.Pointer, gp *_core.G) {
	...
	print("PC=", _core.Hex(c.rip()), " m=", _g_.M.Id, "\n")
	...
	if _lock.Gotraceback(&docrash) > 0 {
		...
		if crashing > 0 && gp != _g_.M.Curg && _g_.M.Curg != nil && _lock.Readgstatus(_g_.M.Curg)&^_lock.Gscan == _lock.Grunning {
			// tracebackothers on original m skipped this one; trace it now.
			_lock.Goroutineheader(_g_.M.Curg)
			_lock.Traceback(^uintptr(0), ^uintptr(0), 0, gp)
		} else if crashing == 0 {
	...
	if docrash {
		...
		if _lock.GOOS == "darwin" || _lock.GOOS == "linux" {
			crashing++
			if crashing < _core.Sched.Mcount {
				...
				print("\n-----\n\n")
				raiseproc(_lock.SIGQUIT)
				_core.Usleep(5 * 1000 * 1000)
			}
		}
		_lock.Crash()
	}
