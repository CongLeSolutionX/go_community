...
func Ready(gp *_core.G) {
	...
	if status&^_lock.Gscan != _lock.Gwaiting {
		Dumpgstatus(gp)
		_lock.Throw("bad g->status in ready")
	}
...
func Casgstatus(gp *_core.G, oldval, newval uint32) {
	if (oldval&_lock.Gscan != 0) || (newval&_lock.Gscan != 0) || oldval == newval {
		_lock.Systemstack(func() {
			print("runtime: casgstatus: oldval=", _core.Hex(oldval), " newval=", _core.Hex(newval), "\n")
			_lock.Throw("casgstatus: bad incoming values")
		})
	}
	...
	for !Cas(&gp.Atomicstatus, oldval, newval) {
		if oldval == _lock.Gwaiting && gp.Atomicstatus == _lock.Grunnable {
			_lock.Systemstack(func() {
				_lock.Throw("casgstatus: waiting for Gwaiting but is Grunnable")
			})
		}
		// Help GC if needed.
...
func mstart1() {
	...
	if _g_ != _g_.M.G0 {
		_lock.Throw("bad runtime·mstart")
	}
...
func stopm() {
	...
	if _g_.M.Locks != 0 {
		_lock.Throw("stopm holding locks")
	}
	if _g_.M.P != nil {
		_lock.Throw("stopm holding p")
	}
	if _g_.M.Spinning {
...
func startm(_p_ *_core.P, spinning bool) {
	...
	if mp.Spinning {
		_lock.Throw("startm: m is spinning")
	}
	if mp.Nextp != nil {
		_lock.Throw("startm: m has p")
	}
...
func stoplockedm() {
	...
	if _g_.M.Lockedg == nil || _g_.M.Lockedg.Lockedm != _g_.M {
		_lock.Throw("stoplockedm: inconsistent locking")
	}
	if _g_.M.P != nil {
	...
	if status&^_lock.Gscan != _lock.Grunnable {
		...
		Dumpgstatus(_g_)
		_lock.Throw("stoplockedm: not runnable")
	}
	Acquirep(_g_.M.Nextp)
	_g_.M.Nextp = nil
}
...
func startlockedm(gp *_core.G) {
	...
	mp := gp.Lockedm
	if mp == _g_.M {
		_lock.Throw("startlockedm: locked to me")
	}
	if mp.Nextp != nil {
		_lock.Throw("startlockedm: m has p")
	}
...
func gcstopm() {
	...
	if _core.Sched.Gcwaiting == 0 {
		_lock.Throw("gcstopm: not waiting for gc")
	}
	if _g_.M.Spinning {
		_g_.M.Spinning = false
		_lock.Xadd(&_core.Sched.Nmspinning, -1)
...
stop:
	...
	// poll network
	if netpollinited() && Xchg64(&_core.Sched.Lastpoll, 0) != 0 {
		if _g_.M.P != nil {
			_lock.Throw("findrunnable: netpoll with p")
		}
		if _g_.M.Spinning {
			_lock.Throw("findrunnable: netpoll with spinning")
		}
...
func resetspinning() {
	...
	if _g_.M.Spinning {
		...
		if nmspinning < 0 {
			_lock.Throw("findrunnable: negative nmspinning")
		}
	} else {
...
func Schedule() {
	...
	if _g_.M.Locks != 0 {
		_lock.Throw("schedule: holding locks")
	}
...
top:
	...
	if gp == nil {
		...
		if _g_.M.P.Schedtick%61 == 0 && _core.Sched.Runqsize > 0 {
			_lock.Lock(&_core.Sched.Lock)
	...
	if gp == nil {
		...
		if gp != nil && _g_.M.Spinning {
			_lock.Throw("schedule: spinning with local work")
		}
	}
	if gp == nil {
...
func GoschedImpl(gp *_core.G) {
	status := _lock.Readgstatus(gp)
	if status&^_lock.Gscan != _lock.Grunning {
		Dumpgstatus(gp)
		_lock.Throw("bad g status")
	}
...
func Save(pc, sp uintptr) {
	...
	_g_.Sched.Ret = 0
	_g_.Sched.Ctxt = nil
	_g_.Sched.G = _core.Guintptr(unsafe.Pointer(_g_))
}
...
func entersyscallblock_handoff() {
	if Trace.Enabled {
		TraceGoSysCall()
		TraceGoSysBlock(_core.Getg().M.P)
	}
	Handoffp(releasep())
}
...
func Exitsyscall(dummy int32) {
	...
	if _lock.Getcallersp(unsafe.Pointer(&dummy)) > _g_.Syscallsp {
		_lock.Throw("exitsyscall: syscall frame is no longer valid")
	}
	...
	if exitsyscallfast() {
		if _g_.M.Mcache == nil {
			_lock.Throw("lost mcache")
		}
		if Trace.Enabled {
			if oldp != _g_.M.P || _g_.M.Syscalltick != _g_.M.P.Syscalltick {
				_lock.Systemstack(TraceGoStart)
			}
		}
	...
	if Trace.Enabled {
		...
		_lock.Systemstack(traceGoSysExit)
	}
...
func Acquirep(_p_ *_core.P) {
	...
	if _g_.M.P != nil || _g_.M.Mcache != nil {
		_lock.Throw("acquirep: already in go")
	}
	if _p_.M != nil || _p_.Status != _lock.Pidle {
		...
		_lock.Throw("acquirep: invalid p state")
	}
...
func releasep() *_core.P {
	...
	if _g_.M.P == nil || _g_.M.Mcache == nil {
		_lock.Throw("releasep: invalid arg")
	}
	_p_ := _g_.M.P
	if _p_.M != _g_.M || _p_.Mcache != _g_.M.Mcache || _p_.Status != _lock.Prunning {
		...
		_lock.Throw("releasep: invalid p state")
	}
	if Trace.Enabled {
...
func checkdead() {
	...
	if run < 0 {
		...
		_lock.Throw("checkdead: inconsistent counts")
	}
	...
	for i := 0; i < len(_lock.Allgs); i++ {
		...
		case _lock.Grunnable,
			...
			_lock.Throw("checkdead: runnable g")
		}
	}
	_lock.Unlock(&_lock.Allglock)
	if grunning == 0 { // possible if main goroutine calls runtime·Goexit()
		_lock.Throw("no goroutines (main called runtime.Goexit) - deadlock!")
	}
	...
	if gp != nil {
		...
		_p_ := Pidleget()
		if _p_ == nil {
			_lock.Throw("checkdead: no p for timer")
		}
		mp := Mget()
		if mp == nil {
			Newm(nil, _p_)
	...
	_lock.Throw("all goroutines are asleep - deadlock!")
}
...
func runqputslow(_p_ *_core.P, gp *_core.G, h, t uint32) bool {
	...
	if n != uint32(len(_p_.Runq)/2) {
		_lock.Throw("runqputslow: queue is not full")
	}
...
func Runqsteal(_p_, p2 *_core.P) *_core.G {
	...
	if t-h+n >= uint32(len(_p_.Runq)) {
		_lock.Throw("runqsteal: runq overflow")
	}
	for i := uint32(0); i < n; i++ {
