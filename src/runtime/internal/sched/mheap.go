...
func mHeap_AllocStack(h *_lock.Mheap, npage uintptr) *_core.Mspan {
	_g_ := _core.Getg()
	if _g_ != _g_.M.G0 {
		_lock.Throw("mheap_allocstack not on g0 stack")
	}
	_lock.Lock(&h.Lock)
...
HaveSpan:
	...
	if s.State != MSpanFree {
		_lock.Throw("MHeap_AllocLocked - MSpan not free")
	}
	if s.Npages < npage {
		_lock.Throw("MHeap_AllocLocked - bad npages")
	}
	MSpanList_Remove(s)
	if s.Next != nil || s.Prev != nil {
		_lock.Throw("still in list")
	}
	if s.Npreleased > 0 {
...
func MHeap_FreeSpanLocked(h *_lock.Mheap, s *_core.Mspan, acctinuse, acctidle bool) {
	...
	case MSpanStack:
		if s.Ref != 0 {
			_lock.Throw("MHeap_FreeSpanLocked - invalid stack free")
		}
	case XMSpanInUse:
		if s.Ref != 0 || s.Sweepgen != h.Sweepgen {
			...
			_lock.Throw("MHeap_FreeSpanLocked - invalid free")
		}
	default:
		_lock.Throw("MHeap_FreeSpanLocked - invalid span state")
	}
	...
	if p > 0 {
		t := H_spans[p-1]
		if t != nil && t.State != XMSpanInUse && t.State != MSpanStack {
			s.Start = t.Start
			s.Npages += t.Npages
