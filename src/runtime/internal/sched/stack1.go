...
func stackpoolalloc(order uint8) _core.Gclinkptr {
	...
	if s == list {
		...
		if s.Ref != 0 {
			_lock.Gothrow("bad ref")
		}
		if s.Freelist.Ptr() != nil {
			_lock.Gothrow("bad freelist")
		}
	...
	x := s.Freelist
	if x.Ptr() == nil {
		_lock.Gothrow("span has no free stacks")
	}
...
func Stackalloc(n uint32) _core.Stack {
	...
	thisg := _core.Getg()
	if thisg != thisg.M.G0 {
		_lock.Gothrow("stackalloc not on scheduler stack")
	}
	if n&(n-1) != 0 {
		_lock.Gothrow("stack size not a power of 2")
	}
	if StackDebug >= 1 {
		print("stackalloc ", n, "\n")
	}
	...
	if _lock.Debug.Efence != 0 || StackFromSystem != 0 {
		...
		if v == nil {
			_lock.Gothrow("out of memory (stackalloc)")
		}
