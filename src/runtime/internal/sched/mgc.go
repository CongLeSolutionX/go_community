...
func ismarked(mbits *Markbits) bool {
	if mbits.Bits&BitBoundary != BitBoundary {
		_lock.Throw("ismarked: bits should have boundary bit set")
	}
...
func Ischeckmarked(mbits *Markbits) bool {
	if mbits.Bits&BitBoundary != BitBoundary {
		_lock.Throw("ischeckmarked: bits should have boundary bit set")
	}
...
func Greyobject(obj uintptr, mbits *Markbits, wbuf *Workbuf) *Workbuf {
	...
	if Checkmark {
		...
		if !Ischeckmarked(mbits) {
			...
			_lock.Throw("docheckmark and ischeckmarked disagree")
		}
	} else {
	...
	for i := uintptr(0); i < n; i += _core.PtrSize {
		...
		if ptrmask != nil {
			...
			if WordsPerBitmapByte != 2 {
				_lock.Throw("alg doesn't work for wordsPerBitmapByte != 2")
			}
...
func Scanblock(b, n uintptr, ptrmask *uint8) {
	...
	if Gcphase == GCscan {
		_lock.Throw("scanblock: In GCscan phase but no b passed in.")
	}
