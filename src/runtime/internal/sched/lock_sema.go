...
func Notewakeup(n *_core.Note) {
	...
	case v == _lock.Locked:
		...
		_lock.Gothrow("notewakeup - double wakeup")
	default:
...
func Notesleep(n *_core.Note) {
	gp := _core.Getg()
	if gp != gp.M.G0 {
		_lock.Gothrow("notesleep not on g0")
	}
	if gp.M.Waitsema == 0 {
	...
	if !_core.Casuintptr(&n.Key, 0, uintptr(unsafe.Pointer(gp.M))) {
		...
		if n.Key != _lock.Locked {
			_lock.Gothrow("notesleep - waitm out of sync")
		}
		return
	}
	// Queued.  Sleep.
...
func Notetsleep_internal(n *_core.Note, ns int64, gp *_core.G, deadline int64) bool {
	...
	if !_core.Casuintptr(&n.Key, 0, uintptr(unsafe.Pointer(gp.M))) {
		// Must be locked (got wakeup).
		if n.Key != _lock.Locked {
			_lock.Gothrow("notetsleep - waitm out of sync")
		}
		return true
	}
	if ns < 0 {
	...
	for {
		...
		case _lock.Locked:
			...
			if _lock.Semasleep(-1) < 0 {
				_lock.Gothrow("runtime: unable to acquire - semaphore out of sync")
			}
			gp.M.Blocked = false
			return true
		default:
			_lock.Gothrow("runtime: unexpected waitm - semaphore out of sync")
		}
	}
}
...
func Notetsleepg(n *_core.Note, ns int64) bool {
	gp := _core.Getg()
	if gp == gp.M.G0 {
		_lock.Gothrow("notetsleepg on g0")
	}
	if gp.M.Waitsema == 0 {
		gp.M.Waitsema = _lock.Semacreate()
	}
