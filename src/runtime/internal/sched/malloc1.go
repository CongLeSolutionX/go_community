...
func mHeap_SysAlloc(h *_lock.Mheap, n uintptr) unsafe.Pointer {
	if n > uintptr(h.Arena_end)-uintptr(h.Arena_used) {
		...
		p_size := _lock.Round(n+_core.PageSize, 256<<20)
	...
	if n <= uintptr(h.Arena_end)-uintptr(h.Arena_used) {
		...
		if _lock.Mheap_.Shadow_enabled {
			SysMap(unsafe.Pointer(p+_lock.Mheap_.Shadow_heap), n, h.Shadow_reserved, &_lock.Memstats.Other_sys)
		}

		if uintptr(p)&(_core.PageSize-1) != 0 {
			_lock.Throw("misrounded allocation in MHeap_SysAlloc")
		}
	...
	p_size := _lock.Round(n, _core.PageSize) + _core.PageSize
