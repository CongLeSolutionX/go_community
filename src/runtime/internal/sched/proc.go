...
func Gopark(unlockf func(*_core.G, unsafe.Pointer) bool, lock unsafe.Pointer, reason string) {
	mp := Acquirem()
	gp := mp.Curg
	status := _lock.Readgstatus(gp)
	if status != _lock.Grunning && status != _lock.Gscanrunning {
		_lock.Gothrow("gopark: bad g status")
	}
	mp.Waitlock = lock
...
func Goparkunlock(lock *_core.Mutex, reason string) {
