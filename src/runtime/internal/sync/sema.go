...
func syncsemacquire(s *syncSema) {
	...
	if s.head != nil && s.head.Nrelease > 0 {
		...
		s.tail = w
		_sched.Goparkunlock(&s.lock, "semacquire", _sched.TraceEvGoBlockCond)
		if t0 != 0 {
...
func syncsemrelease(s *syncSema, n uint32) {
	...
	if n > 0 {
		...
		s.tail = w
		_sched.Goparkunlock(&s.lock, "semarelease", _sched.TraceEvGoBlockCond)
		_sem.ReleaseSudog(w)
	} else {
		_lock.Unlock(&s.lock)
	}
