...
func (s *Mspan) Base() uintptr {
	return uintptr(s.Start << XPageShift)
}

func (s *Mspan) Layout() (size, n, total uintptr) {
	total = s.Npages << XPageShift
	size = s.Elemsize
	if size > 0 {
		n = total / size
	}
...
func Inheap(b uintptr) bool {
	...
	k := b >> XPageShift
	x := k
	x -= Mheap_.Arena_start >> XPageShift
	s := H_spans[x]
...
func SpanOfUnchecked(p uintptr) *Mspan {
	return H_spans[(p-Mheap_.Arena_start)>>XPageShift]
}
...
func mHeap_AllocStack(h *Mheap, npage uintptr) *Mspan {
	...
	if s != nil {
		...
		s.Freelist = 0
		s.Ref = 0
		Memstats.Stacks_inuse += uint64(s.Npages << XPageShift)
	}
...
HaveSpan:
	...
	if s.Npreleased > 0 {
		sysUsed((unsafe.Pointer)(s.Start<<XPageShift), s.Npages<<XPageShift)
		Memstats.Heap_released -= uint64(s.Npreleased << XPageShift)
		s.Npreleased = 0
	}
	...
	if s.Npages > npage {
		...
		s.Npages = npage
		p := uintptr(t.Start)
		p -= (uintptr(unsafe.Pointer(h.Arena_start)) >> XPageShift)
		if p > 0 {
			H_spans[p-1] = s
		}
		H_spans[p] = t
	...
	Memstats.Heap_inuse += uint64(npage << XPageShift)
	Memstats.Heap_idle -= uint64(npage << XPageShift)
...
func mHeap_Grow(h *Mheap, npage uintptr) bool {
	...
	npage = Round(npage, (64<<10)/XPageSize)
	ask := npage << XPageShift
	...
	mSpan_Init(s, pageID(uintptr(v)>>XPageShift), ask>>XPageShift)
	p := uintptr(s.Start)
	p -= (uintptr(unsafe.Pointer(h.Arena_start)) >> XPageShift)
...
func MHeap_FreeSpanLocked(h *Mheap, s *Mspan, acctinuse, acctidle bool, unusedsince int64) {
	...
	case XMSpanInUse:
		if s.Ref != 0 || s.Sweepgen != h.Sweepgen {
			print("MHeap_FreeSpanLocked - span ", s, " ptr ", Hex(s.Start<<XPageShift), " ref ", s.Ref, " sweepgen ", s.Sweepgen, "/", h.Sweepgen, "\n")
	...
	if acctinuse {
		Memstats.Heap_inuse -= uint64(s.Npages << XPageShift)
	}
	if acctidle {
		Memstats.Heap_idle += uint64(s.Npages << XPageShift)
	}
	s.State = MSpanFree
	MSpanList_Remove(s)
