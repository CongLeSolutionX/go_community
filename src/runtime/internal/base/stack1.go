...
func stackpoolalloc(order uint8) Gclinkptr {
	...
	if s == list {
		...
		s = mHeap_AllocStack(&Mheap_, StackCacheSize>>XPageShift)
		if s == nil {
			Throw("out of memory")
		}
		if s.Ref != 0 {
		...
		for i := uintptr(0); i < StackCacheSize; i += FixedStack << order {
			x := Gclinkptr(uintptr(s.Start)<<XPageShift + i)
...
func Stackalloc(n uint32) (Stack, []Stkbar) {
	...
	if Debug.Efence != 0 || StackFromSystem != 0 {
		v := SysAlloc(Round(uintptr(n), XPageSize), &Memstats.Stacks_sys)
		if v == nil {
	...
	if StackCache != 0 && n < FixedStack<<NumStackOrders && n < StackCacheSize {
		...
		s := mHeap_AllocStack(&Mheap_, Round(uintptr(n), XPageSize)>>XPageShift)
		if s == nil {
			Throw("out of memory")
		}
		v = (unsafe.Pointer)(s.Start << XPageShift)
	}
