...
const (
	...
	TinySizeClass = XTinySizeClass
	MaxSmallSize  = XMaxSmallSize

	PageShift = XPageShift
	PageSize  = XPageSize
	PageMask  = XPageMask
...
const (
	XPageShift = 13
	XPageSize  = 1 << XPageShift
	XPageMask  = XPageSize - 1
)
...
const (
	...
	MaxMHeapList   = 1 << (20 - XPageShift) // Maximum page length for fixed-size list in MHeap.
	...
	MHeapMap_Bits      = MHeapMap_TotalBits - XPageShift
...
func mHeap_SysAlloc(h *Mheap, n uintptr) unsafe.Pointer {
	if n > uintptr(h.Arena_end)-uintptr(h.Arena_used) {
		...
		p_size := Round(n+XPageSize, 256<<20)
	...
	p_size := Round(n, XPageSize) + XPageSize
