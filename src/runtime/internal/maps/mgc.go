...
func gcmarknewobject_m(obj uintptr) {
	...
	if _sched.Checkmarkphase { // The world should be stopped so this should not happen.
		_lock.Throw("gcmarknewobject called while doing checkmark")
	}

	_sched.HeapBitsForAddr(obj).SetMarked()
}

// Scan count objects starting with those in wbuf.
//go:nowritebarrier
func drainobjects(wbuf *_sched.Workbuf, count uintptr) {
	for i := uintptr(0); i < count; i++ {
		if wbuf.Nobj == 0 {
			_sched.Putempty(wbuf)
			wbuf = trygetfull()
			if wbuf == nil {
				return
			}
		...
		wbuf = _sched.Scanobject(b, 0, nil, wbuf)
	}
...
//go:nowritebarrier
func trygetfull() *_sched.Workbuf {
	wbuf := (*_sched.Workbuf)(_sched.Lfstackpop(&_sched.Work.Full))
	if wbuf == nil {
		wbuf = (*_sched.Workbuf)(_sched.Lfstackpop(&_sched.Work.Partial))
	}
	return wbuf
}
...
func gchelpwork() {
	...
	case _sched.GCoff, _sched.GCquiesce, _sched.GCstw:
		// No work.
	case _sched.GCsweep:
