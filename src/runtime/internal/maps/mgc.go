...
func gcmarknewobject_m(obj uintptr) {
	if _sched.Gcphase != _sched.GCmarktermination {
		_lock.Throw("marking new object while not in mark termination phase")
	}
	if _sched.Checkmark { // The world should be stopped so this should not happen.
		_lock.Throw("gcmarknewobject called while doing checkmark")
	}
...
func Unrollgcprog1(maskp *byte, prog *byte, ppos *uintptr, inplace, sparse bool) *byte {
	...
	for {
		...
		default:
			if *prog == 0 {
				_lock.Throw("unrollgcprog: *prog is 0")
			}
...
func markspan(v unsafe.Pointer, size uintptr, n uintptr, leftover bool) {
	...
	if off%_sched.WordsPerBitmapByte != 0 {
		_lock.Throw("markspan: unaligned length")
	}
