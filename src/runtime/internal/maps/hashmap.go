...
func Makemap(t *Maptype, hint int64) *Hmap {
	...
	if !Ismapkey(t.key) {
		_lock.Gothrow("runtime.makemap: unsupported map key type")
	}

	// check compiler's and reflect's math
	if t.key.Size > MaxKeySize && (!t.indirectkey || t.keysize != uint8(_core.PtrSize)) ||
		t.key.Size <= MaxKeySize && (t.indirectkey || t.keysize != uint8(t.key.Size)) {
		_lock.Gothrow("key size wrong")
	}
	if t.elem.Size > MaxValueSize && (!t.indirectvalue || t.valuesize != uint8(_core.PtrSize)) ||
		t.elem.Size <= MaxValueSize && (t.indirectvalue || t.valuesize != uint8(t.elem.Size)) {
		_lock.Gothrow("value size wrong")
	}

	// invariants we depend on.  We should probably check these at compile time
	// somewhere, but for now we'll do it here.
	if t.key.Align > BucketCnt {
		_lock.Gothrow("key align too big")
	}
	if t.elem.Align > BucketCnt {
		_lock.Gothrow("value align too big")
	}
	if uintptr(t.key.Size)%uintptr(t.key.Align) != 0 {
		_lock.Gothrow("key size not a multiple of key align")
	}
	if uintptr(t.elem.Size)%uintptr(t.elem.Align) != 0 {
		_lock.Gothrow("value size not a multiple of value align")
	}
	if BucketCnt < 8 {
		_lock.Gothrow("bucketsize too small for proper alignment")
	}
	if DataOffset%uintptr(t.key.Align) != 0 {
		_lock.Gothrow("need padding in bucket (key)")
	}
	if DataOffset%uintptr(t.elem.Align) != 0 {
		_lock.Gothrow("need padding in bucket (value)")
	}
...
again:
	...
	for {
		for i := uintptr(0); i < BucketCnt; i++ {
			...
			_sched.Memmove(k2, key, uintptr(t.key.Size))
			v := _core.Add(unsafe.Pointer(b), DataOffset+BucketCnt*uintptr(t.keysize)+i*uintptr(t.valuesize))
			v2 := v
			if t.indirectvalue {
				v2 = *((*unsafe.Pointer)(v2))
			}
			_sched.Memmove(v2, val, uintptr(t.elem.Size))
			return
		}
		ovf := b.overflow(t)
		if ovf == nil {
	...
	_sched.Memmove(insertk, key, uintptr(t.key.Size))
	_sched.Memmove(insertv, val, uintptr(t.elem.Size))
	*inserti = top
	h.Count++
}
...
func Mapiterinit(t *Maptype, h *Hmap, it *Hiter) {
	...
	if unsafe.Sizeof(Hiter{})/_core.PtrSize != 10 {
		_lock.Gothrow("hash_iter size incorrect") // see ../../cmd/gc/reflect.c
	}
	it.t = t
	it.h = h
	...
	for {
		...
		if _sched.Cas(&h.flags, old, old|Iterator|OldIterator) {
			break
		}
	}
...
func hashGrow(t *Maptype, h *Hmap) {
	if h.oldbuckets != nil {
		_lock.Gothrow("evacuation not done in time")
	}
				...
				if (hash & newbit) == 0 {
					...
					if t.indirectkey {
						*(*unsafe.Pointer)(xk) = k2 // copy pointer
						...
						_sched.Memmove(xk, k, uintptr(t.key.Size)) // copy value
					}
					if t.indirectvalue {
						*(*unsafe.Pointer)(xv) = *(*unsafe.Pointer)(v)
					} else {
						_sched.Memmove(xv, v, uintptr(t.elem.Size))
					}
					xi++
					...
					if t.indirectkey {
						*(*unsafe.Pointer)(yk) = k2
						...
						_sched.Memmove(yk, k, uintptr(t.key.Size))
					}
					if t.indirectvalue {
						*(*unsafe.Pointer)(yv) = *(*unsafe.Pointer)(v)
					} else {
						_sched.Memmove(yv, v, uintptr(t.elem.Size))
					}
					yi++
...
func Ismapkey(t *_core.Type) bool {
	return _hash.Goalg(t.Alg).Hash != nil
}
