...
func mHeap_Alloc_m(h *_lock.Mheap, npage uintptr, sizeclass int32, large bool) *_core.Mspan {
	_g_ := _core.Getg()
	if _g_ != _g_.M.G0 {
		_lock.Throw("_mheap_alloc not on g0 stack")
	}
	_lock.Lock(&h.Lock)
...
func MHeap_LookupMaybe(h *_lock.Mheap, v unsafe.Pointer) *_core.Mspan {
	...
	s := _sched.H_spans[q]
	if s == nil || p < uintptr(s.Start) || uintptr(v) >= uintptr(unsafe.Pointer(s.Limit)) || s.State != _sched.XMSpanInUse {
		return nil
	}
	return s
}
...
func mSpanList_InsertBack(list *_core.Mspan, span *_core.Mspan) {
	if span.Next != nil || span.Prev != nil {
		...
		_lock.Throw("MSpanList_InsertBack")
	}
	span.Next = list
	span.Prev = list.Prev
	span.Next.Prev = span
...
func Addspecial(p unsafe.Pointer, s *_core.Special) bool {
	...
	if span == nil {
		_lock.Throw("addspecial on invalid pointer")
	}
...
func setprofilebucket(p unsafe.Pointer, b *_sem.Bucket) {
	...
	if !Addspecial(p, &s.Special) {
		_lock.Throw("setprofilebucket: profile already set")
	}
}
