...
func mapaccess1_fast32(t *Maptype, h *Hmap, key uint32) unsafe.Pointer {
	...
	if h.B == 0 {
		...
		hash := _hash.Goalg(t.key.Alg).Hash(_core.Noescape(unsafe.Pointer(&key)), 4, uintptr(h.hash0))
		m := uintptr(1)<<h.B - 1
...
func mapaccess1_fast64(t *Maptype, h *Hmap, key uint64) unsafe.Pointer {
	...
	if h.B == 0 {
		...
		hash := _hash.Goalg(t.key.Alg).Hash(_core.Noescape(unsafe.Pointer(&key)), 8, uintptr(h.hash0))
		m := uintptr(1)<<h.B - 1
		b = (*bmap)(_core.Add(h.buckets, (hash&m)*uintptr(t.bucketsize)))
		if c := h.oldbuckets; c != nil {
			oldb := (*bmap)(_core.Add(c, (hash&(m>>1))*uintptr(t.bucketsize)))
...
dohash:
	hash := _hash.Goalg(t.key.Alg).Hash(_core.Noescape(unsafe.Pointer(&ky)), 2*_core.PtrSize, uintptr(h.hash0))
	m := uintptr(1)<<h.B - 1
	b := (*bmap)(_core.Add(h.buckets, (hash&m)*uintptr(t.bucketsize)))
	if c := h.oldbuckets; c != nil {
		oldb := (*bmap)(_core.Add(c, (hash&(m>>1))*uintptr(t.bucketsize)))
