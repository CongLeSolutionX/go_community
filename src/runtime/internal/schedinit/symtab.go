...
func symtabinit() {
	...
	if pcln32[0] != 0xfffffffb || pcln[4] != 0 || pcln[5] != 0 || pcln[6] != _lock.PCQuantum || pcln[7] != _core.PtrSize {
		...
		_lock.Gothrow("invalid function symbol table\n")
	}
	...
	for i := 0; i < nftab; i++ {
		...
		if _lock.Ftab[i].Entry > _lock.Ftab[i+1].Entry {
			...
			f2name := "end"
			if i+1 < nftab {
				f2name = _lock.Gofuncname(f2)
			}
			println("function symbol table not sorted by program counter:", _core.Hex(_lock.Ftab[i].Entry), _lock.Gofuncname(f1), ">", _core.Hex(_lock.Ftab[i+1].Entry), f2name)
			for j := 0; j <= i; j++ {
				print("\t", _core.Hex(_lock.Ftab[j].Entry), " ", _lock.Gofuncname((*_lock.Func)(unsafe.Pointer(&_lock.Pclntable[_lock.Ftab[j].Funcoff]))), "\n")
			}
			_lock.Gothrow("invalid runtime symbol table")
		}
	}
