...
func mallocinit() {
	...
	if _gc.Class_to_size[_core.TinySizeClass] != _core.TinySize {
		_lock.Gothrow("bad TinySizeClass")
	}
	...
	if _core.PtrSize == 8 && (limit == 0 || limit > 1<<30) {
		...
		arenaSize := _sched.Round(_core.MaxMem, _core.PageSize)
		bitmapSize = arenaSize / (_core.PtrSize * 8 / 4)
		spansSize = arenaSize / _core.PageSize * _core.PtrSize
		spansSize = _sched.Round(spansSize, _core.PageSize)
		for i := 0; i <= 0x7f; i++ {
	...
	if p == 0 {
		...
		for _, arenaSize := range arenaSizes {
			...
			// to a MB boundary.
			p = _sched.Round(uintptr(unsafe.Pointer(&end))+(1<<18), 1<<20)
			pSize = bitmapSize + spansSize + arenaSize + _core.PageSize
			p = uintptr(_sched.SysReserve(unsafe.Pointer(p), pSize, &reserved))
			if p != 0 {
				break
		...
		if p == 0 {
			_lock.Gothrow("runtime: cannot reserve arena virtual address space")
		}
	}
	...
	p1 := _sched.Round(p, _core.PageSize)
	...
	if _lock.Mheap_.Arena_start&(_core.PageSize-1) != 0 {
		...
		_lock.Gothrow("misrounded allocation in mallocinit")
	}
