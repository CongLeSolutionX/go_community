...
func memhash(p unsafe.Pointer, s, seed uintptr) uintptr {
	if _lock.GOARCH == "amd64" && _lock.GOOS != "nacl" && useAeshash {
...
tail:
	...
	case s < 4:
		w := uint64(*(*byte)(p))
		w += uint64(*(*byte)(_core.Add(p, s>>1))) << 8
		w += uint64(*(*byte)(_core.Add(p, s-1))) << 16
		h ^= w * m1
	case s <= 8:
		w := uint64(readUnaligned32(p))
		w += uint64(readUnaligned32(_core.Add(p, s-4))) << 32
		h ^= w * m1
	case s <= 16:
		h ^= readUnaligned64(p) * m1
		h = rotl_31(h) * m2
		h = rotl_27(h)
		h ^= readUnaligned64(_core.Add(p, s-8)) * m1
	case s <= 32:
		h ^= readUnaligned64(p) * m1
		h = rotl_31(h) * m2
		h = rotl_27(h)
		h ^= readUnaligned64(_core.Add(p, 8)) * m1
		h = rotl_31(h) * m2
		h = rotl_27(h)
		h ^= readUnaligned64(_core.Add(p, s-16)) * m1
		h = rotl_31(h) * m2
		h = rotl_27(h)
		h ^= readUnaligned64(_core.Add(p, s-8)) * m1
	default:
