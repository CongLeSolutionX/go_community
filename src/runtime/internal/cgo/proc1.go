...
func newextram() {
	...
	gp.Sched.Lr = 0
	gp.Sched.G = _core.Guintptr(unsafe.Pointer(gp))
...
func reentersyscall(pc, sp uintptr) {
	...
	if _sched.Trace.Enabled {
		_lock.Systemstack(_sched.TraceGoSysCall)
	}
...
func entersyscall_gcwait() {
	...
	_lock.Lock(&_core.Sched.Lock)
	if _core.Sched.Stopwait > 0 && _sched.Cas(&_p_.Status, _lock.Psyscall, _lock.Pgcstop) {
		if _sched.Trace.Enabled {
...
func badunlockosthread() {
	_lock.Throw("runtime: internal error: misuse of lockOSThread/unlockOSThread")
}
