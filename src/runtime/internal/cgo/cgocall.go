...
func cgocall_errno(fn, arg unsafe.Pointer) int32 {
	if !_sched.Iscgo && _lock.GOOS != "solaris" && _lock.GOOS != "windows" {
		_lock.Gothrow("cgocall unavailable")
	}

	if fn == nil {
		_lock.Gothrow("cgocall nil")
	}
...
func cgocallbackg1() {
	...
	switch _lock.GOARCH {
	default:
		_lock.Gothrow("cgocallbackg is unimplemented on arch")
	case "arm":
	...
	// Invoke callback.
	_finalize.Reflectcall(unsafe.Pointer(cb.fn), unsafe.Pointer(cb.arg), uint32(cb.argsize), 0)
...
func unwindm(restore *bool) {
	...
	switch _lock.GOARCH {
	default:
		_lock.Gothrow("unwindm not implemented")
	case "386", "amd64":
