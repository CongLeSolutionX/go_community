...
func freedeferpanic() {
	...
	_lock.Gothrow("freedefer with d._panic != nil")
}

func freedeferfn() {
	// fn must be cleared before d is unlinked from gp.
	_lock.Gothrow("freedefer with d.fn != nil")
}
...
func Goexit() {
	...
	for {
		...
		d.Started = true
		_finalize.Reflectcall(unsafe.Pointer(d.Fn), _gc.DeferArgs(d), uint32(d.Siz), uint32(d.Siz))
		if gp.Defer != d {
			_lock.Gothrow("bad defer entry in Goexit")
		}
		d.Panic = nil
		d.Fn = nil
		gp.Defer = d.Link
...
func gopanic(e interface{}) {
	...
	if gp.M.Curg != gp {
		...
		printany(e)
		print("\n")
		_lock.Gothrow("panic on system stack")
	}
	...
	if gp.M.Softfloat != 0 {
		gp.M.Locks--
		gp.M.Softfloat = 0
		_lock.Gothrow("panic during softfloat")
	}
	if gp.M.Mallocing != 0 {
		print("panic: ")
		printany(e)
		print("\n")
		_lock.Gothrow("panic during malloc")
	}
	if gp.M.Gcing != 0 {
		print("panic: ")
		printany(e)
		print("\n")
		_lock.Gothrow("panic during gc")
	}
	if gp.M.Locks != 0 {
		print("panic: ")
		printany(e)
		print("\n")
		_lock.Gothrow("panic holding locks")
	}
	...
	for {
		...
		if gp.Defer != d {
			_lock.Gothrow("bad defer entry in panic")
		}
		d.Panic = nil
		d.Fn = nil
		gp.Defer = d.Link
		...
		if p.Recovered {
			...
			_lock.Gothrow("recovery failed") // mcall should not return
		}
	}
