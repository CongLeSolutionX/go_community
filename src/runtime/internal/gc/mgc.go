...
func Gc(mode int) {
	...
	_lock.Semacquire(&Worldsema, false)
	...
	if mode == GcBackgroundMode { // Do as much work concurrently as possible
		...
		if _base.Atomicload64(&_base.Work.Full) != 0 || _base.Atomicload64(&_base.Work.Partial) != 0 {
			if _base.Work.BgMark2.Done != 0 {
				_base.Throw("work.bgMark2.done != 0")
			}
	...
	if _base.Debug.Gctrace > 0 {
		...
		totalCpu := _base.Sched.Totaltime + (tEnd-_base.Sched.Procresizetime)*int64(_base.Gomaxprocs)
		util := _base.Work.Totaltime * 100 / totalCpu
...
func gcSweep(mode int) {
	...
	if !_base.XConcurrentSweep || mode == GcForceBlockMode {
	...
	if heapDistance < _base.XPageSize {
		// Avoid setting the sweep ratio extremely high
		heapDistance = _base.XPageSize
	}
	_base.Lock(&_base.Mheap_.Lock)
