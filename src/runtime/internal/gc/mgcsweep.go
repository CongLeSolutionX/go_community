...
func MSpan_Sweep(s *_base.Mspan, preserve bool) bool {
	...
	sstart := uintptr(s.Start << _base.XPageShift)
	...
	for special != nil {
		...
		p := uintptr(s.Start<<_base.XPageShift) + uintptr(special.Offset)/size*size
		hbits := _base.HeapBitsForAddr(p)
		if !hbits.IsMarked() {
			// Find the exact byte for which the special was setup
			// (as opposed to object beginning).
			p := uintptr(s.Start<<_base.XPageShift) + uintptr(special.Offset)
	...
	if nfree > 0 {
		...
		if _base.Debug.Efence > 0 {
			...
			sysFault(unsafe.Pointer(uintptr(s.Start<<_base.XPageShift)), size)
		} else {
...
func dumpFreeList(s *_base.Mspan) {
	...
	sstart := uintptr(s.Start << _base.XPageShift)
	link := s.Freelist
	for i := 0; i < int(s.Npages*_base.XPageSize/s.Elemsize); i++ {
		if i != 0 {
			print(" -> ")
		}
