...
func helpgc(nproc int32) {
	...
	for n := int32(1); n < nproc; n++ { // one M is currently running
		...
		mp := _sched.Mget()
		if mp == nil {
			_lock.Throw("gcprocs inconsistency")
		}
		mp.Helpgc = n
		mp.P = _lock.Allp[pos]
...
func Casfrom_Gscanstatus(gp *_core.G, oldval, newval uint32) {
	...
	default:
		...
		_lock.Throw("casfrom_Gscanstatus:top gp->status is not in scan state")
	...
	if !success {
		...
		_sched.Dumpgstatus(gp)
		_lock.Throw("casfrom_Gscanstatus: gp->status is not in scan state")
	}
...
func Castogscanstatus(gp *_core.G, oldval, newval uint32) bool {
	...
	case _lock.Grunning:
		if gp.Gcscanvalid {
			print("runtime: castogscanstatus _Grunning and gp.gcscanvalid is true, newval=", _core.Hex(newval), "\n")
			_lock.Throw("castogscanstatus")
		}
...
func casgcopystack(gp *_core.G) uint32 {
	for {
		...
		if oldstatus != _lock.Gwaiting && oldstatus != _lock.Grunnable {
			_lock.Throw("copystack: bad status, not Gwaiting or Grunnable")
		}
...
func Stopg(gp *_core.G) bool {
	for {
		...
		default:
			_sched.Dumpgstatus(gp)
			_lock.Throw("stopg: gp->atomicstatus is not valid")
...
func Restartg(gp *_core.G) {
	...
	default:
		_sched.Dumpgstatus(gp)
		_lock.Throw("restartg: unexpected status")
	...
	case _lock.Gscanenqueue:
		...
		if gp != _core.Getg().M.Curg {
			_lock.Throw("processing Gscanenqueue on wrong m")
		}
		_sched.Dropg()
		_sched.Ready(gp)
	}
...
func Stoptheworld() {
	...
	if _g_.M.Locks > 0 {
		_lock.Throw("stoptheworld: holding locks")
	}
	...
	for i := 0; i < int(_lock.Gomaxprocs); i++ {
		...
		if s == _lock.Psyscall && _sched.Cas(&p.Status, s, _lock.Pgcstop) {
			if _sched.Trace.Enabled {
				_sched.TraceGoSysBlock(p)
				_sched.TraceProcStop(p)
			}
			p.Syscalltick++
			_core.Sched.Stopwait--
		}
	...
	if _core.Sched.Stopwait != 0 {
		_lock.Throw("stoptheworld: not stopped")
	}
	for i := 0; i < int(_lock.Gomaxprocs); i++ {
		p := _lock.Allp[i]
...
func Starttheworld() {
	...
	for p1 != nil {
		...
		if p.M != nil {
			...
			p.M = nil
			if mp.Nextp != nil {
				_lock.Throw("starttheworld: inconsistent mp->nextp")
			}
			mp.Nextp = p
			_sched.Notewakeup(&mp.Park)
		} else {
	...
	if _lock.Atomicload(&_core.Sched.Npidle) != 0 && _lock.Atomicload(&_core.Sched.Nmspinning) == 0 {
		_sched.Wakep()
	}
...
func Procresize(nprocs int32) *_core.P {
	old := _lock.Gomaxprocs
	if old < 0 || old > _lock.MaxGomaxprocs || nprocs <= 0 || nprocs > _lock.MaxGomaxprocs {
		_lock.Throw("procresize: invalid arg")
	}
	if _sched.Trace.Enabled {
	...
	for i := int32(0); i < nprocs; i++ {
		...
		if pp == nil {
			pp = new(_core.P)
			pp.Id = i
			pp.Status = _lock.Pgcstop
			_sched.Atomicstorep(unsafe.Pointer(&_lock.Allp[i]), unsafe.Pointer(pp))
		}
		if pp.Mcache == nil {
			if old == 0 && i == 0 {
				if _core.Getg().M.Mcache == nil {
					_lock.Throw("missing mcache?")
				}
				pp.Mcache = _core.Getg().M.Mcache // bootstrap
			} else {
				pp.Mcache = _lock.Allocmcache()
			}
		}
	}
	...
	_lock.Atomicstore((*uint32)(unsafe.Pointer(int32p)), uint32(nprocs))
	return runnablePs
}
