...
func stackpoolfree(x _core.Gclinkptr, order uint8) {
	...
	if s.State != _sched.MSpanStack {
		_lock.Throw("freeing stack not in a stack span")
	}
...
func Stackfree(stk _core.Stack) {
	...
	if n&(n-1) != 0 {
		_lock.Throw("stack not a power of 2")
	}
	if _sched.StackDebug >= 1 {
...
var mapnames = []string{
	_sched.TypeDead:    "---",
	_sched.TypeScalar:  "scalar",
	_sched.TypePointer: "ptr",
}
...
func adjustpointers(scanp unsafe.Pointer, cbv *_lock.Bitvector, adjinfo *adjustinfo, f *_lock.Func) {
	...
	maxp := adjinfo.old.Hi
	delta := adjinfo.delta
	num := uintptr(bv.N) / _sched.TypeBitsWidth
	for i := uintptr(0); i < num; i++ {
		...
		case _sched.TypePointer:
			...
			up := uintptr(p)
			if f != nil && 0 < up && up < _core.PageSize && _lock.Debug.Invalidptr != 0 || up == _lock.PoisonStack {
				// Looks like a junk value in a pointer slot.
				// Live analysis wrong?
				_core.Getg().M.Traceback = 2
				print("runtime: bad pointer in frame ", _lock.Funcname(f), " at ", _core.Add(scanp, i*_core.PtrSize), ": ", p, "\n")
				_lock.Throw("invalid stack pointer")
			}
			if minp <= up && up < maxp {
				if _sched.StackDebug >= 3 {
					print("adjust ptr ", p, " ", _lock.Funcname(f), "\n")
				}
...
func adjustframe(frame *_lock.Stkframe, arg unsafe.Pointer) bool {
	...
	if _lock.Thechar == '6' && frame.Argp-frame.Varp == 2*_lock.RegSize {
		if !_lock.Framepointer_enabled {
			print("runtime: found space for saved base pointer, but no framepointer experiment\n")
			print("argp=", _core.Hex(frame.Argp), " varp=", _core.Hex(frame.Varp), "\n")
			_lock.Throw("bad frame layout")
		}
		if _sched.StackDebug >= 3 {
			print("      saved bp\n")
		}
...
func Copystack(gp *_core.G, newsize uintptr) {
	if gp.Syscallsp != 0 {
		_lock.Throw("stack growth not allowed in system call")
	}
	old := gp.Stack
	if old.Lo == 0 {
		_lock.Throw("nil stackbase")
	}
...
func shrinkstack(gp *_core.G) {
	...
	if gp.Stack.Lo == 0 {
		_lock.Throw("missing stack in shrinkstack")
	}
