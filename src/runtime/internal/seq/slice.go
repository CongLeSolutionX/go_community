...
func growslice(t *slicetype, old _sched.SliceStruct, n int64) _sched.SliceStruct {
	...
	if _sched.Raceenabled {
		...
		racereadrangepc(old.Array, uintptr(old.Len*int(t.elem.Size)), callerpc, _lock.FuncPC(growslice))
	}
	...
	capmem := _schedinit.Goroundupsize(uintptr(newcap) * uintptr(et.Size))
	newcap = int(capmem / uintptr(et.Size))
	var p unsafe.Pointer
	if et.Kind&_hash.KindNoPointers != 0 {
		p = rawmem(capmem)
