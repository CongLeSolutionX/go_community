...
func slicebytetostring(buf *_strings.TmpBuf, b []byte) string {
	l := len(b)
	if l == 0 {
	...
	if _sched.Raceenabled && l > 0 {
		_strings.Racereadrangepc(unsafe.Pointer(&b[0]),
			uintptr(l),
			_lock.Getcallerpc(unsafe.Pointer(&b)),
			_lock.FuncPC(slicebytetostring))
	}
	s, c := _strings.Rawstringtmp(buf, l)
	copy(c, b)
	return s
}
...
func slicerunetostring(a []rune) string {
	if _sched.Raceenabled && len(a) > 0 {
		_strings.Racereadrangepc(unsafe.Pointer(&a[0]),
...
func intstring(buf *[4]byte, v int64) string {
	...
	if buf != nil {
		b = buf[:]
		s = _strings.Slicebytetostringtmp(b)
	} else {
...
func Rawruneslice(size int) (b []rune) {
	...
	mem := _schedinit.Roundupsize(uintptr(size) * 4)
	p := _maps.Mallocgc(mem, nil, _sched.XFlagNoScan|_sched.XFlagNoZero)
