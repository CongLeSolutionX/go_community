...
func Netpollblock(pd *_sched.PollDesc, mode int32, waitio bool) bool {
	...
	for {
		...
		if old != 0 {
			_lock.Gothrow("netpollblock: double wait")
		}
	...
	if old > _sched.PdWait {
		_lock.Gothrow("netpollblock: corrupted state")
	}
...
func netpolldeadlineimpl(pd *_sched.PollDesc, seq uintptr, read, write bool) {
	...
	if read {
		if pd.Rd <= 0 || pd.Rt.F == nil {
			_lock.Gothrow("netpolldeadlineimpl: inconsistent read deadline")
		}
		pd.Rd = -1
	...
	if write {
		if pd.Wd <= 0 || pd.Wt.F == nil && !read {
			_lock.Gothrow("netpolldeadlineimpl: inconsistent write deadline")
		}
		pd.Wd = -1
