...
TEXT runtime∕internal∕core·Exit(SB),NOSPLIT,$0
	MOVL code+0(FP), DI
...
TEXT runtime∕internal∕hash·open(SB),NOSPLIT,$0
	MOVL name+0(FP), DI
	MOVL mode+4(FP), SI
	MOVL perm+8(FP), DX
...
TEXT runtime∕internal∕hash·close(SB),NOSPLIT,$0
	MOVL fd+0(FP), DI
	NACL_SYSCALL(SYS_close)
	MOVL AX, ret+8(FP)
	RET

TEXT runtime∕internal∕hash·read(SB),NOSPLIT,$0
	MOVL fd+0(FP), DI
	MOVL p+4(FP), SI
	MOVL n+8(FP), DX
...
TEXT syscall·naclWrite(SB), NOSPLIT, $24-20
	...
	MOVL SI, 4(SP)
	MOVL DX, 8(SP)
	CALL runtime∕internal∕core·Write(SB)
	MOVL 16(SP), AX
	MOVL AX, ret+16(FP)
	RET

TEXT runtime∕internal∕core·Write(SB),NOSPLIT,$16-20
	// If using fake time and writing to stdout or stderr,
	// emit playback header before actual Data.
	MOVQ runtime∕internal∕sched·faketime(SB), AX
	CMPQ AX, $0
	JEQ Write
	MOVL fd+0(FP), DI
	CMPL DI, $1
...
TEXT runtime·mstart_nacl(SB),NOSPLIT,$0
	...
	SUBL	$8, AX
	MOVL	AX, TLS
	JMP runtime∕internal∕sched·Mstart(SB)
...
TEXT runtime∕internal∕core·Osyield(SB),NOSPLIT,$0
...
TEXT runtime∕internal∕lock·sigtramp(SB),NOSPLIT,$80
...
debughandler:
	...
	MOVQ	0(BX), BX
	MOVQ	BX, 64(SP)
	CALL	runtime∕internal∕printf·printf(SB)
	...
	MOVL	g(CX), BX
	MOVL	BX, 12(SP)	// gp
	CALL	runtime∕internal∕lock·Traceback(SB)
