...
TEXT runtime∕internal∕lock·Raise(SB),NOSPLIT,$0
	...
	JMP	runtime∕internal∕sched·raiseproc(SB)

TEXT runtime∕internal∕sched·raiseproc(SB),NOSPLIT,$16
	MOVL	$20, AX // getpid
	INT	$0x80
	MOVL	AX, 4(SP)	// pid
	MOVL	Sig+0(FP), AX
...
TEXT runtime∕internal∕lock·Nanotime(SB),NOSPLIT,$0
	CALL	runtime·now(SB)
	MOVL	AX, ret_lo+0(FP)
	MOVL	DX, ret_hi+4(FP)
	RET
...
//	16(FP)	siginfo
//	20(FP)	context
TEXT runtime∕internal∕lock·sigtramp(SB),NOSPLIT,$40
	get_tls(CX)
...
TEXT runtime∕internal∕core·Usleep(SB),NOSPLIT,$32
	MOVL	$0, DX
	MOVL	usec+0(FP), AX
	MOVL	$1000000, CX
	DIVL	CX
...
TEXT runtime∕internal∕sched·bsdthread_create(SB),NOSPLIT,$32
	MOVL	$360, AX
...
TEXT runtime∕internal∕schedinit·bsdthread_register(SB),NOSPLIT,$40
	MOVL	$366, AX
