...
func TestProflabelChildgoroutine(t *testing.T) {
	...
	if !reflect.DeepEqual(labels, gotParentLabels) {
		t.Errorf("label set on goroutine: got %v, want %v", gotParentLabels, labels)
	}

	// child labels should be the same
	gotChildLabels := <-ch
	if !reflect.DeepEqual(labels, gotChildLabels) {
		t.Errorf("label set on goroutine: got %v, want %v", gotChildLabels, labels)
	}
	...
	gotParentLabels = proflabel.Get()
	if !reflect.DeepEqual(nil, gotParentLabels) {
		t.Errorf("label set on goroutine: got %v, want %v", gotParentLabels, nil)
	}
}
