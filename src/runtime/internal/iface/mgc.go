...
func shouldtriggergc() bool {
	return _base.Memstats.Heap_live >= _base.Memstats.Next_gc && Atomicloaduint(&Bggc.Working) == 0
}
...
// program.
func GC() {
	StartGC(_gc.GcForceBlockMode, false)
}
...
func StartGC(mode int, forceTrigger bool) {
	...
	mp := _base.Acquirem()
	if gp := _base.Getg(); gp == mp.G0 || mp.Locks > 1 || mp.Preemptoff != "" || !_base.Memstats.Enablegc || _base.Panicking != 0 || _base.Gcpercent < 0 {
		_base.Releasem(mp)
		return
	}
	_base.Releasem(mp)
	...
	if _base.Debug.Gcstoptheworld == 1 {
		mode = _gc.GcForceMode
	} else if _base.Debug.Gcstoptheworld == 2 {
		mode = _gc.GcForceBlockMode
	}

	if mode != _gc.GcBackgroundMode {
...
func Backgroundgc() {
	Bggc.g = _base.Getg()
	for {
		_gc.Gc(_gc.GcBackgroundMode)
		_base.Lock(&Bggc.lock)
		Bggc.Working = 0
		_base.Goparkunlock(&Bggc.lock, "Concurrent GC wait", _base.TraceEvGoBlock, 1)
	}
}
