...
func mHeap_Alloc_m(h *_base.Mheap, npage uintptr, sizeclass int32, large bool) *_base.Mspan {
	...
	if s != nil {
		...
		if sizeclass == 0 {
			s.Elemsize = s.Npages << _base.XPageShift
			s.DivShift = 0
			s.DivMul = 0
			s.DivShift2 = 0
			s.BaseMask = 0
		...
		if large {
			...
			_base.Memstats.Heap_live += uint64(npage << _base.XPageShift)
...
func mHeap_Alloc(h *_base.Mheap, npage uintptr, sizeclass int32, large bool, needzero bool) *_base.Mspan {
	...
	if s != nil {
		if needzero && s.Needzero != 0 {
			_base.Memclr(unsafe.Pointer(s.Start<<_base.XPageShift), s.Npages<<_base.XPageShift)
		}
		s.Needzero = 0
	}
	return s
...
func MHeap_LookupMaybe(h *_base.Mheap, v unsafe.Pointer) *_base.Mspan {
	...
	q := p
	q -= uintptr(unsafe.Pointer(h.Arena_start)) >> _base.XPageShift
	s := _base.H_spans[q]
...
func Addspecial(p unsafe.Pointer, s *_base.Special) bool {
	...
	offset := uintptr(p) - uintptr(span.Start<<_base.XPageShift)
	kind := s.Kind
...
func setprofilebucket(p unsafe.Pointer, b *_lock.Bucket) {
