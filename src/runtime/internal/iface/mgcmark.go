...
func gcAssistAlloc(size uintptr, allowAssist bool) {
	...
	if _base.Getg() == gp.M.G0 {
		return
	}
	if mp := _base.Getg().M; mp.Locks > 0 || mp.Preemptoff != "" {
		return
	}
...
retry:
	...
	if bgScanCredit > 0 {
		...
		_base.Xaddint64(&_base.GcController.BgScanCredit, -stolen)
...
func gcDrainN(gcw *_base.GcWork, scanWork int64) {
	if !_base.WriteBarrierEnabled {
		_base.Throw("gcDrainN phase incorrect")
	}
