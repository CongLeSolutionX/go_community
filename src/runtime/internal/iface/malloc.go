...
func Mallocgc(size uintptr, typ *_base.Type, flags uint32) unsafe.Pointer {
	...
	if flags&_base.FlagNoScan == 0 && typ == nil {
	...
	if size <= _base.MaxSmallSize {
		if flags&_base.FlagNoScan != 0 && size < _base.MaxTinySize {
			// Tiny allocator.
			//
			...
			s = c.Alloc[_base.TinySizeClass]
			v := s.Freelist
			if v.Ptr() == nil {
				_base.Systemstack(func() {
					mCache_Refill(c, _base.TinySizeClass)
				})
				shouldhelpgc = true
				s = c.Alloc[_base.TinySizeClass]
				v = s.Freelist
		...
		x = unsafe.Pointer(uintptr(s.Start << _base.PageShift))
		size = uintptr(s.Elemsize)
	}

	if flags&_base.FlagNoScan != 0 {
...
func largeAlloc(size uintptr, flag uint32) *_base.Mspan {
	...
	npages := size >> _base.XPageShift
	if size&_base.XPageMask != 0 {
		npages++
	}
	s := mHeap_Alloc(&_base.Mheap_, npages, 0, true, flag&_base.XFlagNoZero == 0)
	if s == nil {
		_base.Throw("out of memory")
	}
	s.Limit = uintptr(s.Start)<<_base.XPageShift + size
