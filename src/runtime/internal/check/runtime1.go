...
func check() {
	...
	if unsafe.Sizeof(x1) != 1 {
		_lock.Gothrow("bad unsafe.Sizeof x1")
	}
	if unsafe.Offsetof(y1.y) != 1 {
		_lock.Gothrow("bad offsetof y1.y")
	}
	if unsafe.Sizeof(y1) != 2 {
		_lock.Gothrow("bad unsafe.Sizeof y1")
	}

	if _lock.Timediv(12345*1000000000+54321, 1000000000, &e) != 12345 || e != 54321 {
		_lock.Gothrow("bad timediv")
	}
	...
	if m[0] != 1 || m[1] != 0xf1 || m[2] != 1 || m[3] != 1 {
		_lock.Gothrow("atomicor8")
	}

	*(*uint64)(unsafe.Pointer(&j)) = ^uint64(0)
	if j == j {
		_lock.Gothrow("float64nan")
	}
	if !(j != j) {
		_lock.Gothrow("float64nan1")
	}

	*(*uint64)(unsafe.Pointer(&j1)) = ^uint64(1)
	if j == j1 {
		_lock.Gothrow("float64nan2")
	}
	if !(j != j1) {
		_lock.Gothrow("float64nan3")
	}

	*(*uint32)(unsafe.Pointer(&i)) = ^uint32(0)
	if i == i {
		_lock.Gothrow("float32nan")
	}
	if i == i {
		_lock.Gothrow("float32nan1")
	}

	*(*uint32)(unsafe.Pointer(&i1)) = ^uint32(1)
	if i == i1 {
		_lock.Gothrow("float32nan2")
	}
	if i == i1 {
		_lock.Gothrow("float32nan3")
	}

	testAtomic64()

	if _core.FixedStack != _sched.Round2(_core.FixedStack) {
		_lock.Gothrow("FixedStack is not power-of-2")
	}
}
