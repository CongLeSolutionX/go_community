...
func testAtomic64() {
	...
	if _sched.Cas64(&test_z64, test_x64, 1) {
	...
	if _sched.Atomicload64(&test_z64) != 1 {
		_lock.Throw("load64 failed")
	}
	_sched.Atomicstore64(&test_z64, (1<<40)+1)
	if _sched.Atomicload64(&test_z64) != (1<<40)+1 {
		_lock.Throw("store64 failed")
	}
	if _lock.Xadd64(&test_z64, (1<<40)+1) != (2<<40)+2 {
		_lock.Throw("xadd64 failed")
	}
	if _sched.Atomicload64(&test_z64) != (2<<40)+2 {
		_lock.Throw("xadd64 failed")
	}
	if _sched.Xchg64(&test_z64, (3<<40)+3) != (2<<40)+2 {
		_lock.Throw("xchg64 failed")
	}
	if _sched.Atomicload64(&test_z64) != (3<<40)+3 {
		_lock.Throw("xchg64 failed")
	}
}
