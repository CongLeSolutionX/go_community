...
func Selectsize(size uintptr) uintptr {
	...
	return _sched.Round(selsize, _lock.Int64Align)
}

func Newselect(sel *Select, selsize int64, size int32) {
	if selsize != int64(Selectsize(uintptr(size))) {
		print("runtime: bad select size ", selsize, ", want ", Selectsize(uintptr(size)), "\n")
		_lock.Gothrow("bad select size")
	}
...
func SelectrecvImpl(sel *Select, c *Hchan, pc uintptr, elem unsafe.Pointer, received *bool, so uintptr) {
	i := sel.Ncase
	if i >= sel.Tcase {
		_lock.Gothrow("selectrecv: too many cases")
	}
	sel.Ncase = i + 1
...
loop:
	...
	gp.Param = nil
	_sched.Gopark(selparkcommit, unsafe.Pointer(sel), "select")
	...
	if c.Dataqsiz > 0 {
		_lock.Gothrow("selectgo: shouldn't happen")
	}
...
asyncrecv:
	...
	if cas.Elem != nil {
		_sched.Memmove(cas.Elem, chanbuf(c, c.recvx), uintptr(c.Elemsize))
	}
...
asyncsend:
	...
	_sched.Memmove(chanbuf(c, c.sendx), cas.Elem, uintptr(c.Elemsize))
	c.sendx++
...
syncrecv:
	...
	if cas.Elem != nil {
		_sched.Memmove(cas.Elem, sg.Elem, uintptr(c.Elemsize))
	}
	sg.Elem = nil
	gp = sg.G
...
syncsend:
	...
	if sg.Elem != nil {
		_sched.Memmove(sg.Elem, cas.Elem, uintptr(c.Elemsize))
	}
	sg.Elem = nil
	gp = sg.G
	gp.Param = unsafe.Pointer(sg)
