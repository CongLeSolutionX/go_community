...
func init() {
	if os.Getenv("TEST_CRASH_WHILE_TRACING") == "1" {
		...
		trace.Log(context.Background(), logCategory, logMessage)
		panic("yzzyx")
	}
}
...
loop:
	...
	if !seen {
		t.Errorf("expected one log event matching {category=%q message=%q}, but none of the %d received trace events match", logCategory, logMessage, i)
	}
