...
func TestStackOverflowTopAndBottomTraces(t *testing.T) {
	...
	tl, bl := len(topHalfStackTraces), len(bottomHalfStackTraces)
	if tl == 0 || bl == 0 {
		t.Fatal("Both lengths of stack-halves should be non-zero")
	}
	if tl != bl {
		t.Fatalf("len(topHalfStackTraces)=%d len(bottomHalfStackTraces)=%d yet must be equal", tl, bl)
	}
	t.Logf("tl: %d bl: %d", tl, bl)
	...
	if len(topHalf) != 0 && len(bottomHalf) != 0 {
