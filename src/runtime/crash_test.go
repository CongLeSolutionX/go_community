...
func TestStackOverflowTopAndBottomTraces(t *testing.T) {
	...
	//    runtime: goroutine stack exceeds 1000-byte limit
	...
	if nTopHalf == 0 || nBottomHalf == 0 {
        ...
        // The bottomo half will always have the frame for "runtime.goexit", hence we need to decrement 1 counted line.
	if nTopHalf != nBottomHalf-1 {
		t.Errorf("len(topHalfStackTraces)=%d len(bottomHalfStackTraces)+1=%d yet must be equal", nTopHalf, nBottomHalf-1)
	}
