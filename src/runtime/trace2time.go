...
// traceTimeSample is a point in time of the two possible trace clocks.
//
// Used for computing EvFrequency events.
type traceTimeSample struct {
	nanos int64
	ticks int64
}

// traceTimeSampleNow produces a traceTimeSample that represents the current
// time.
func traceTimeSampleNow() traceTimeSample {
	return traceTimeSample{
		nanos: nanotime(),
		ticks: cputicks(),
	}
}

// traceComputeFrequency estimates the number of trace clock units per
// second that elapse.
func traceComputeFrequency(ref traceTimeSample) uint64 {
	if !osHasLowResClock {
		// (trace clock units / nanoseconds) * (1e9 nanoseconds / 1 second)
		return uint64(1.0 / float64(traceTimeDiv) * 1e9)
	}
	start, end := ref, traceTimeSampleNow()
	for end.nanos <= start.nanos || end.ticks <= start.ticks {
		// Sleep for 5ms at a time until we can compute a reasonable
		// estimate.
		timeSleep(5_000_000)
		end = traceTimeSampleNow()
	}
	// (trace clock units / nanoseconds) * (1e9 nanoseconds / 1 second)
	freq := (float64(end.ticks-start.ticks) / traceTimeDiv / float64(end.nanos-start.nanos)) * 1e9
	return uint64(freq)
}
...
func traceFrequency(gen uintptr, freq uint64) {
