...
var trace struct {
	...
	shutdown      atomic.Bool // set when we are waiting for trace reader to finish after setting enabled to false
	...
	readerGen  uintptr // the generation the reader is currently reading for
	flushedGen uintptr // the last completed generation
	doneNote   [2]note // notifies traceAdvance when the trace reader has finished a generation
	...
	gen            atomic.Uintptr
	lastNonZeroGen uintptr // last non-zero generation

	// Number of goroutines in syscall exiting slow path.
	exitingSyscall atomic.Int32

	// genStartTime is the time the last generation started.
	genStartTime traceTimeSample

	// frequency is the number of trace clock ticks per second.
	frequency uint64
}
...
func StartTrace() error {
	...
	// Record the time tracing started.
	trace.genStartTime = traceTimeSampleNow()
	...
	for _, pp := range allp {
		tl.writer().writeProcStatusForP(pp).end()
	}
	traceRelease(tl)
...
func traceAdvance(stopTrace bool) {
	...
	// Compute trace.frequency if we haven't before.
	if trace.frequency == 0 {
		trace.frequency = traceComputeFrequency(trace.genStartTime)
	}
	...
	traceFrequency(gen, trace.frequency)
	...
	forEachGRace(func(gp *g) {
		...
		systemstack(func() {
			...
			if !s.dead {
				...
				ug.inMarkAssist = s.g.trace.inMarkAssist
			}
			resumeG(s)
	...
	// We're about to enter the new generation, so write down the time.
	// This gives a hard lower-bound on the next generation's timestamps.
	trace.genStartTime = traceTimeSampleNow()
	...
	noteclear(&trace.doneNote[1-(gen%2)])
	notetsleepg(&trace.doneNote[gen%2], -1)
	...
	systemstack(func() {
		...
		// Clear the doneNote.
		noteclear(&trace.doneNote[gen%2])
		if stopTrace {
	...
	if trace.full[gen%2].empty() {
		if trace.flushedGen == gen {
			if trace.shutdown.Load() {
				...
				notewakeup(&trace.doneNote[gen%2])
			...
			trace.readerGen = trace.gen.Load()
...
func traceReaderAvailable() *g {
	...
	// work available.
	if trace.flushedGen == trace.readerGen || !trace.full[trace.readerGen%2].empty() || trace.shutdown.Load() {
