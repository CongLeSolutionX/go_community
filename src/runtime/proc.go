...
func badmorestackg0() {
	if GOARCH != "amd64" { // TODO: implement crashstack
		ss := stringStructOf(&badmorestackg0Msg)
		write(2, ss.str, int32(ss.len))
		return
	}
	...
	switchToCrashStack(func() {
		print("runtime: morestack on g0, stack [", hex(g.stack.lo), " ", hex(g.stack.hi), "], sp=", hex(sp), ", called from\n")
		traceback1(pc, sp, 0, g, _TraceBadStack)
		print("\n")
...
var gcrash = g{
	...
	stackguard0: uintptr(unsafe.Pointer(&crashstack)) + unsafe.Sizeof(&crashstack) - 1000,
	stackguard1: uintptr(unsafe.Pointer(&crashstack)) + unsafe.Sizeof(&crashstack) - 1000,
}

var crashingG *g

var recursiveCrashStackMsg = "fatal: recursive switchToCrashStack\n"
...
func switchToCrashStack(fn func()) {
	me := getg()
	if atomic.Casp1((*unsafe.Pointer)(unsafe.Pointer(&crashingG)), nil, unsafe.Pointer(me)) {
		switchToCrashStack0(fn) // should never return
		abort()
	}
	if crashingG == me {
		// recursive crashing. too bad.
		sp := stringStructOf(&recursiveCrashStackMsg)
		write(2, sp.str, int32(sp.len))
		abort()
	}
	// Another g is crashing. Spin some time, hopefully it will finish traceback.
	for i := 0; i < 10000; i++ {
	}
	abort()
}
