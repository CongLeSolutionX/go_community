...
// P. startm m.spinning in the newly started M.
//
...
func startm(_p_ *p, spinning bool) {
	...
	if _p_ == nil {
		if spinning {
			// TODO(prattmic): All remaining _p_ == nil calls could
			// be cleaned up to find a P before calling startm.
...
func handoffp(_p_ *p) {
	...
	if atomic.Load(&sched.nmspinning)+atomic.Load(&sched.npidle) == 0 && atomic.Cas(&sched.nmspinning, 0, 1) { // TODO: fast atomic
		atomic.Store(&sched.needspinning, 0)
		startm(_p_, true)
		return
	}
	lock(&sched.lock)
...
func wakep() {
	...
	// See preemption comment in startm.
	mp := acquirem()
	...
	if pp == nil {
		...
		//
		// Normally this is a non-issue. If all of the Ps are running
		// then they will eventually enter the scheduler find the work.
		//
		// But we could race with a P going idle. It is running in
		// findrunnable and has already decided not to become spinning,
		// found no work, and is preparing to go idle (but has not yet
		// take sched.lock).
		//
		// If we do nothing, that P will go idle and we will fail to
		// start a new spinning M, resulting in a loss of work
		// conservation (idle P while there is runnable work).
		//
		// We use sched.needspinning to synchronize with non-spinning
		// Ms going idle. If needspinning is set when they are about to
		// drop their P, they abort the drop and instead become a new
		// spinning M on our behalf.
		//
		// See "Delicate dance" comment in findrunnable.
		if int32(atomic.Xadd(&sched.nmspinning, -1)) < 0 {
			throw("wakep: negative nmspinning")
		}
		atomic.Store(&sched.needspinning, 1)
		unlock(&sched.lock)
		releasem(mp)
		return
	}
...
top:
	...
	if !_g_.m.spinning && atomic.Load(&sched.needspinning) == 1 {
		// See comment below.
		atomic.Xadd(&sched.nmspinning, 1)
		atomic.Store(&sched.needspinning, 0)
		_g_.m.spinning = true
	...
	if _g_.m.spinning {
		...
		// transitions to be conservative as there are still some
		// chases of transient increments of nmspinning that are
		// decremented without going through this path. See
		// golang.org/issue/43997.
...
func schedtrace(detailed bool) {
	...
	lock(&sched.lock)
	print("SCHED ", (now-starttime)/1e6, "ms: gomaxprocs=", gomaxprocs, " idleprocs=", sched.npidle, " threads=", mcount(), " spinningthreads=", sched.nmspinning, " needspinng=", sched.needspinning, " idlethreads=", sched.nmidle, " runqueue=", sched.runqsize)
	if detailed {
