...
func forEachP(fn func(*p)) {
	...
	for i := 0; i < int(gomaxprocs); i++ {
		...
		if s == _Psyscall && p.runSafePointFn == 1 && atomic.Cas(&p.status, s, _Pidle) {
			...
			p.syscalltick++
			if debug.gcroc >= 1 {
				// ROC has associated this mcache and the related goroutine. This handoff
				// will remove that associations so the ROC epoch need to be published.
				// The assumption is that this is rare action.
				//
				p.mcache.publishMCache(true) // Perhaps do this in handoffp???
				p.mcache.rocEpoch++
			}
			// Handshake with the associated goroutine to alloc it to continue knowing
			// that the private objects in this mcache are now marked public.
...
func park_m(gp *g) {
	...
	recycleDone := false
	if debug.gcroc >= 1 && gcphase == _GCoff {
		publishStack(gp)
		if _g_.m.mcache != nil {
		...
		systemstack(func() {
			if _g_.rocMCache != nil {
				for atomic.Load64(&_g_.rocMCache.rocEpoch) == _g_.rocEpoch {
					// spin
					if cnt > 1000*1000*1000 {
						//						dlog().string("BUG BUG BAD At throw:2935: _g_.rocMCache.rocEpoch= ").int64(int64(_g_.rocMCache.rocEpoch)).
						//							string(" _g_.rocMCache= ").hex(uintptr(unsafe.Pointer(_g_.rocMCache))).
						//							string(" _g_.m.p.ptr().status= ").int64(int64(_g_.m.p.ptr().status)).
						//							string(" _g_.rocEpoch= ").int64(int64(_g_.rocEpoch)).string(" _g_.goid= ").int64(_g_.goid).end()
						//break
						throw("why big loop")
					}
					cnt++
...
//const forcePreemptNS = 10 * 1000 * 1000 * 10000 // 10ms * 10000 = 100 seconds basically turns preemption off.
	...
	for i := int32(0); i < gomaxprocs; i++ {
		...
		if s == _Psyscall {
			...
			if atomic.Cas(&_p_.status, s, _Pidle) {
				...
				if debug.gcroc >= 1 {
					if _p_.m != 0 && _p_.m.ptr().curg != nil {
						println("runtime: retake about to handoff: proc.go:4199", "_p_.m.ptr().curg.goid=", _p_.m.ptr().curg.goid,
							"_p_.mcache.rocGoid=", _p_.mcache.rocGoid)
						throw("why? this might be fine.")
					}
				}
				handoffp(_p_)
			}
