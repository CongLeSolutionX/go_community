...
// mDoFixupFromSignal confirms there is a pending fixup that is
// expected to be invoked from a signal handler and executes it.
//go:nosplit
func mDoFixupFromSignal() {
	_g_ := getg()
	if atomic.Load(&_g_.m.mFixup.used) != mFixupViaSignal {
		return
	}
	mDoFixup()
}
