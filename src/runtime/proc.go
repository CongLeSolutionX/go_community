...
retry:
	...
	// We've inverted the order in which it gets G's from the local runnable queue and then advances the head pointer
	// because we don't want to mess up the statuses of G's when runqdrain() and runqsteal() runs in parallel,
	// thus we should advance the head pointer before draining the local p into a gQueue, ensuring that we update
	// any gp.schedlink only after we take the full ownership of G.
