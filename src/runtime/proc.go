...
top:
	...
	if _g_.m.spinning || 2*atomic.Load(&sched.nmspinning) < procs-atomic.Load(&sched.npidle) {
		...
		gp, inheritTime, w, newWork := stealWork(now)
		if gp != nil {
			// Successfully stole.
			return gp, inheritTime
		}
...
func stealWork(now int64) (gp *g, inheritTime bool, pollUntil int64, newWork bool) {
	pp := getg().m.p.ptr()
	...
	for i := 0; i < stealTries; i++ {
		...
		for enum := stealOrder.start(fastrand()); !enum.done(); enum.next() {
			if sched.gcwaiting != 0 {
				...
				return nil, false, pollUntil, true
			}
			...
			if stealTimersOrRunNextG && timerpMask.read(enum.position()) {
				// now is never zero, so the returned now is
				// always identical.
				_, w, ran := checkTimers(p2, now)
				if w != 0 && (pollUntil == 0 || w < pollUntil) {
					pollUntil = w
				}
				if ran {
					...
					if gp, inheritTime := runqget(pp); gp != nil {
						return gp, inheritTime, pollUntil, ranTimer
					}
					ranTimer = true
				}
			}
			...
			if !idlepMask.read(enum.position()) {
				if gp := runqsteal(pp, p2, stealTimersOrRunNextG); gp != nil {
					return gp, false, pollUntil, ranTimer
				}
			}
		}
	}
	...
	// wait for.
	return nil, false, pollUntil, ranTimer
}
...
//
// If work, a P, and a worker G are all available, the P and G will be
