...
top:
	...
	for i := 0; i < 4; i++ {
		for enum := stealOrder.start(fastrand()); !enum.done(); enum.next() {
			...
			stealTimersOrRunNextG := i > 2 // only check timers or runnext on last pass of stealing
...
stop:
	...
	// checkTimers here because it calls adjusttimers because it may need to
	// allocate memory, which isn't allowed when we don't have an active P.
	for _, _p_ := range allpSnapshot {
