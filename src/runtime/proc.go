...
//
// This approach applies to two primary sources of potential work: readying a
// goroutine, new/modified-earlier timers, and idle-priority GC. See below for
// additional details.
//
// We unpark an additional thread when we submit work if (1) there is an idle P
// and there are no "spinning" worker threads (this is wakep()). A worker
// thread is considered spinning if it is out of local work and did not find
// work in global run queue/ netpoller; the spinning state is denoted in
// m.spinning and in sched.nmspinning. Threads unparked this way are also
// considered spinning; we don't do goroutine handoff so such threads are out
// of work initially. Spinning threads do some spinning looking for work in
// per-P run queues and timers before parking. If a spinning thread finds work
// it takes itself out of the spinning state and proceeds to execution. If it
// does not find work it takes itself out of the spinning state and then parks.
//
...
// semi-persistent CPU underutilization. The general pattern for submission is:
// submit work to local run/timer queue, #StoreLoad-style memory barrier, check
// sched.nmspinning. The general pattern for spinning->non-spinning transition
// is: decrement nmspinning, #StoreLoad-style memory barrier, check all per-P
// work queues for new work.
