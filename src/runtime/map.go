...
// they are iterating through has been moved to the new table.
...
const (
	...
	allEmpty           uint64 = 0xffff_ffff_ffff_ffff
	...
	// Use for loop to traverse values in the buckets if mask <= loopaccessMask.
	loopaccessMask = 1
)

// isFull returns true if the slot is full.
func isFull(v uint8) bool {
	return v < deletedSlot
}
...
type hmap struct {
	...
	buckets    unsafe.Pointer // array of (bucketmask+1) Buckets. may be nil if count==0.
	growthLeft int            // number of items that can be inserted before we need to grow the table
	bucketmask uintptr        // (number of buckets) - 1, can hold up to loadFactor * (bucketmask+1) items
}
...
// bucketMaskDouble returns the mask after doubling the capacity.
func bucketMaskDouble(mask uintptr) uintptr {
	return mask*2 + 1
}

func bucketNum(mask uintptr) uintptr {
	return mask + 1
}
...
func makemap(t *maptype, hint int, h *hmap) *hmap {
	...
	// Find the bucketNum which will hold the requested # of elements.
	// For hint < 0 isOverLoadFactor returns false since hint < bucketCnt.
	bucketNum := uintptr(1)
	for isOverLoadFactor(hint, bucketNum) {
		bucketNum *= 2
	}
	h.bucketmask = bucketNum - 1
	h.growthLeft = bucketCnt * int(bucketNum)

	// allocate initial hash table
	// if bucketNum == 1, the buckets field is allocated lazily later (in mapassign)
	// If hint is large zeroing this memory could take a while.
	if bucketNum != 1 {
		h.buckets = makeBucketArray(t, bucketNum)
	}
...
// nbuckets is the number of buckets to allocate.
func makeBucketArray(t *maptype, nbuckets uintptr) unsafe.Pointer {
	buckets := newarray(t.bucket, int(nbuckets))

	base := buckets
	for i := uintptr(0); i < nbuckets; i++ {
		*(*uint64)(base) = allEmpty
...
func mapiterinit(t *maptype, h *hmap, it *hiter) {
	...
	if unsafe.Sizeof(hiter{})/goarch.PtrSize != 11 {
	...
	var r uintptr
	r = uintptr(fastrandu())
	it.startBucket = r & mask
	it.offset = uint8(r >> mask & (bucketCnt - 1))
...
nextbucket:
	...
	bucket++
	if bucket == bucketNum(it.bucketmask) {
		bucket = 0
		it.wrapped = true
	}
bucketloop:
	if matchFull(b.tophash) == 0 {
	...
	for ; i < bucketCnt; i++ {
		...
		if h.bucketmask == it.bucketmask || !(t.reflexivekey() || t.key.equal(k, k)) {
...
func mapclear(t *maptype, h *hmap) {
	...
	// Clear bucket.
	nbuckets := bucketNum(h.bucketmask)
	...
	bptr := h.buckets
	for i := uintptr(0); i < nbuckets; i++ {
		*(*uint64)(bptr) = allEmpty
		bptr = add(bptr, uintptr(t.bucketsize))
	}
...
func isOverLoadFactor(count int, bucketNum uintptr) bool {
	return count > bucketCnt && uintptr(count) > bucketNum*maxItemInBucket
}

func (h *hmap) needGrow() bool {
	threshold := int(bucketNum(h.bucketmask) * emptyItemInBucket) // (capcity - capcity*loadFactor)
	return h.growthLeft <= threshold
}

func grow(h *hmap, t *maptype) {
	cap := bucketNum(h.bucketmask) * bucketCnt
...
func growbig(h *hmap, t *maptype) {
	oldMask := h.bucketmask
	newMask := bucketMaskDouble(oldMask)
	oldBucketnum := bucketNum(oldMask)
	newBucketnum := bucketNum(newMask)
	newCap := newBucketnum * bucketCnt

	newBucketArray := makeBucketArray(t, newBucketnum)
