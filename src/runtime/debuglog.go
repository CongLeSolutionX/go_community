...
// cannot be used without a P (the message will be lost).
//
...
func (l *debugLog) hex(x uintptr) *debugLog {
	...
	if x >= mheap_.arena_start {
		// Hex is often used for pointers, so we save space by
		// encoding these relative to the heap.
		l.rawByte(debugLogHexArena)
		l.rawVarint(uint64(x - mheap_.arena_start))
		return l
	}
...
func (l *debugLog) callers(skip, limit int) *debugLog {
	...
	sp := getcallersp(unsafe.Pointer(&l))
	pc := getcallerpc(unsafe.Pointer(&l))
	gp := getg()
	var nstk int
	systemstack(func() {
...
func dlog() *debugLog {
	...
	// Allocate log memory if necessary.
	if lr.data == nil {
...
func printDebugLog() {
	...
	fields:
		for {
			...
			case debugLogHexArena:
				u := uintptr(getVarint(l)) + mheap_.arena_start
				print(hex(u))
