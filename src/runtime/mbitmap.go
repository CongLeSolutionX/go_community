...
// refillsLeft is the remaining number of times refill will need to be called
// to complete the iterator.
func (tp typePointers) refillsLeft(limit uintptr) uintptr {
	if tp.typ == nil {
		return 1
	}
	const bytesInPtrDataWord = ptrBits * goarch.PtrSize
	nElems := ((limit - tp.elem) + tp.typ.Size_ - 1) / tp.typ.Size_
	nWordsPerElem := (tp.typ.PtrBytes + bytesInPtrDataWord - 1) / bytesInPtrDataWord
	leftover := ((limit - tp.elem + (nElems-1)*tp.typ.Size_) + bytesInPtrDataWord - 1) / bytesInPtrDataWord
	return nWordsPerElem*(nElems-1) + leftover
}
