...
func (s *mspan) divideByElemSize(n uintptr) uintptr {
	...
	q := uintptr((uint64(n) * uint64(divMul)) >> 32)
...
func (s *mSpanCache) objIndex(p uintptr) uintptr {
	const doubleCheck = false
...
func findObject(p, refBase, refOff uintptr) (base uintptr, spanCache *mSpanCache, objIndex uintptr) {
	...
	if !ok {
		if (GOARCH == "amd64" || GOARCH == "arm64") && p == clobberdeadPtr && debug.invalidptr != 0 {
			...
			badPointer(span, p, refBase, refOff)
		}
		return
	}
	// If p is a bad pointer, it may not be in s's bounds.
	//
	// Note that this doesn't race with span initialization because spanCache is valid, so all the
	// fields are guaranteed to be populated with accurate span information. However, this might
	// race with a span getting freed and reallocated in the sweep phase if we're looking at an
	// invalid pointer. There's nothing much we can really do about that; findObject assumes that
	// the pointer its looking at points to *something* Go code is referencing, and therefore it
	// should be kept alive. If that's not the case, this function is only *likely* to fail.
	//
	// See also spanOfHeap.
	...
	// See explanation in mksizeclasses.go's computeDivMagic.
	elemsize := uintptr(spanCache.elemsize)
	if elemsize == 0 {
		base = spanBase
		return
	}
	objIndex = uintptr((uint64(p-spanBase) * uint64(spanCache.npagesOrDivMul)) >> 32)
	base = spanBase + objIndex*elemsize
	return
}
...
func bulkBarrierPreWrite(dst, src, size uintptr) {
	...
	if _, ok := spanCache.valid(); !ok {
		...
		return
	} else if spanCache.state() != mSpanInUse || dst < spanCache.base() || spanCache.base()+spanCache.len() <= dst {
