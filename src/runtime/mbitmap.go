...
func typeBitsBulkBarrier(typ *_type, dst, src, size uintptr) {
	...
	if typ.Size_ != size {
		println("runtime: typeBitsBulkBarrier with type ", toType(typ).string(), " of size ", typ.Size_, " but memory size", size)
		throw("runtime: invalid typeBitsBulkBarrier")
	}
	if typ.Kind_&kindGCProg != 0 {
		println("runtime: typeBitsBulkBarrier with type ", toType(typ).string(), " with GC prog")
		throw("runtime: invalid typeBitsBulkBarrier")
	}
	if !writeBarrier.needed {
		return
