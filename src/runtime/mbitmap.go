...
func (h heapBits) next() heapBits {
	if h.shift < 4-1 {
		return heapBits{h.bitp, h.shift + 1}
	}
...
func (h heapBits) initCheckmarkSpan(size, n, total uintptr) {
	...
	if ptrSize == 8 && size == ptrSize {
		...
		for i := uintptr(0); i < n; i += 4 {
			*bitp &^= bitPointer | bitPointer<<heapBitsShift | bitPointer<<(2*heapBitsShift) | bitPointer<<(3*heapBitsShift)
...
func heapBitsSetType(x, size, dataSize uintptr, typ *_type) {
	...
	for {
		...
		hb = b & 0xf
		hb |= bitMarked | bitMarked<<heapBitsShift | bitMarked<<(2*heapBitsShift) | bitMarked<<(3*heapBitsShift)
		if w += 4; w >= nw {
			break
		}
		*hbitp = uint8(hb)
