...
// Returns the index of the next unset mark bit or s.nelems if
// there are none. Does not update the span s in any way.
...
func (s *mspan) nextFreeIndex(index uintptr) uintptr {
	...
	tz := ctz(theByte)
	if tz != 8 {
		result := uintptr(tz) + whichByte*bitsPerByte
		if result >= s.nelems {
			return s.nelems
		}
		return result
	...
	whichByte++
	index = (whichByte) * bitsPerByte
	for ; index < s.nelems; index += bitsPerByte {
		theByte = ^s.allocBits[whichByte]
		tz = ctz(theByte)
		if tz != 8 {
			result := uintptr(tz) + whichByte*bitsPerByte
...
func heapBitsSweep8BitPtrs(h heapBits, s *mspan, base, n uintptr, cl uint8, doCall bool, f func(uintptr)) (nfree int) {
	...
	for i := uintptr(0); i < n; i += 4 {
		...
		// during allocation.
		if !(mbits.isMarked() || mbits.index >= s.freeindex && s.allocBits[mbits.index/bitsPerByte]&mbits.mask == 0) {
			if doCall {
