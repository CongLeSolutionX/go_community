...
func (h heapBits) setMarked() {
	...
	atomic.Atomicor8(h.bitp, bitMarked<<h.shift)
}
...
func (h heapBits) setCheckmarked(size uintptr) {
	if size == ptrSize {
		atomic.Atomicor8(h.bitp, bitPointer<<h.shift)
		return
	}
	atomic.Atomicor8(h.bitp, bitMarked<<(heapBitsShift+h.shift))
}
	...
	if size == 2*ptrSize {
			...
			if ptrSize == 4 && dataSize == ptrSize {
				...
				if gcphase == _GCoff {
					*h.bitp |= (bitPointer | bitPointer<<heapBitsShift) << h.shift
					...
					atomic.Atomicor8(h.bitp, (bitPointer|bitPointer<<heapBitsShift)<<h.shift)
				}
			}
			return
		}
		...
		if gcphase == _GCoff {
			*h.bitp |= uint8(hb << h.shift)
			...
			atomic.Atomicor8(h.bitp, uint8(hb<<h.shift))
		}
		return
	}
	...
	case ptrSize == 8 && h.shift == 2:
		...
		if gcphase == _GCoff {
			*hbitp |= uint8(hb)
			...
			atomic.Atomicor8(hbitp, uint8(hb))
		}
		hbitp = subtract1(hbitp)
		if w += 2; w >= nw {
	...
	if w == nw+2 {
		if gcphase == _GCoff {
			*hbitp = *hbitp&^(bitPointer|bitMarked|(bitPointer|bitMarked)<<heapBitsShift) | uint8(hb)
			...
			atomic.Atomicand8(hbitp, ^uint8(bitPointer|bitMarked|(bitPointer|bitMarked)<<heapBitsShift))
			atomic.Atomicor8(hbitp, uint8(hb))
		}
	}
