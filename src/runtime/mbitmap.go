...
// being described. If a bit pair with a high bit 0 is encountered, the low bit
// can also be assumed to be 0, and the object description is over.
...
func (h heapBits) setCheckmarked(size uintptr) {
	...
	atomicor8(h.bitp, 1<<(heapBitsWidth+h.shift))
}
...
func (h heapBits) initCheckmarkSpan(size, n, total uintptr) {
	...
	case size%(4*ptrSize) == 0:
		// Checkmark bit is fourth bit (second mark bit) of every entry.
		// Each object starts at bit 0 of a heap bitmap byte.
		bitp := h.bitp
		step := size / heapBitmapScale
		for i := uintptr(0); i < n; i++ {
			*bitp &^= bitMarked << heapBitsWidth
	...
	case size%(4*ptrSize) == 2*ptrSize:
		...
		bitp := h.bitp
		step := size / heapBitmapScale
		n-- // if n is odd, stop loop from writing past final word
		for i := uintptr(0); i < n; i += 2 {
			*bitp &^= bitMarked << heapBitsWidth
			bitp = subtractb(bitp, step)
			*bitp &^= bitMarked << (4 + heapBitsWidth)
			bitp = subtractb(bitp, step+1)
		}
		n++
		if n&1 == 1 {
			// if n is odd, write final word
			*bitp &^= bitMarked << heapBitsWidth
		}
	}
}
...
// It sets all the checkmark bits to 1, their default value when not
// in checkmark mode.
func (h heapBits) clearCheckmarkSpan(size, n, total uintptr) {
	switch {
	default:
		throw("clearCheckmarkSpan")
...
func heapBitsSweepSpan(base, size, n uintptr, f func(uintptr)) {
	...
	case size%(4*ptrSize) == 2*ptrSize:
		...
		n++
		if n&1 == 1 {
			// if n is odd, process final object
			x := uint32(*bitp)
			if x&bitMarked != 0 {
				x &^= bitMarked
			} else {
