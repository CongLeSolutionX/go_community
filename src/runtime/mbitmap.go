...
// noMorePointers returns true if this word and all remaining words in this object
// are scalars.
// h must not describe the first or second word of the object.
func (h heapBits) noMorePointers() bool {
...
// far into the bitmap.
// h must describe the initial word of the object.
// nosplit because it is used by the TOC write barrier publish code.
//go:nosplit
...
// For each newly freed object in the span, heapBitsSweepSpan calls
// f(p), where p is the object's base address.
// It also sets the newly freed object to noscan indicating that
// the object should not be scanned for pointers.
...
func heapBitsSetType(x, size, dataSize uintptr, typ *_type) {
	...
	if size == 2*sys.PtrSize {
		...
		hb := b & 3
		if gcphase == _GCoff {
			// bitPointer == 1, bitMarked is 1 << 4, heapBitsShift is 1. 110011 is shifted h.shift and complemented
			// Basically this clears out the bits that are about to be ored into *h.hbitp in the next instructions.
			*h.bitp = *h.bitp &^ ((bitPointer | bitMarked | ((bitPointer | bitMarked) << heapBitsShift)) << h.shift)
			*h.bitp |= uint8(hb << h.shift)
		} else {
			// TODO:(rlh) since the GC is not concurrenlty setting the mark bits in the heap map anymore and malloc
			// owns the span we are allocating in why does this have to be atomic?
...
func heapBitsSetTypeNoScan(x, size uintptr) {
	...
	if sys.PtrSize == 8 && size == sys.PtrSize {
		...
		throw("noscan boject is too small")
...
func getgcmask(ep interface{}) (mask []byte) {
	...
	if mlookup(uintptr(p), &base, &n, nil) != 0 {
		...
		for i := uintptr(0); i < n; i += sys.PtrSize {
			...
			if i >= 2*sys.PtrSize && !hbits.noMorePointers() {
