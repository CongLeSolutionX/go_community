...
func TestCoverageApis(t *testing.T) {
	...
	harnessPath := buildHarness(t, bdir, []string{"-cover", "-coverpkg=all"})
	...
	// This test stubbed out for now, needs investigation. Works
	// reliably when I run it by hand, but on trybot runs it fails:
	// writing to an unwritable dir (directory created with 0555
	// protection) weirdly seems to work just fine.
	const stubbedOut = `
	t.Run("emitToUnwritableDir", func(t *testing.T) {
		t.Parallel()
		testEmitToUnwritableDir(t, harnessPath, dir)
	})`
...
func testEmitToDir(t *testing.T, harnessPath string, dir string) {
	withAndWithoutRunner(func(setGoCoverDir bool, tag string) {
		rdir := mkdir(t, filepath.Join(dir, "emitToDir-rdir-"+tag))
		edir := mkdir(t, filepath.Join(dir, "emitToDir-edir-"+tag))

		output, err := runHarness(t, harnessPath, "emitToDir",
...
func testEmitToWriter(t *testing.T, harnessPath string, dir string) {
	withAndWithoutRunner(func(setGoCoverDir bool, tag string) {
		...
		rdir := mkdir(t, filepath.Join(dir, "emitToWriter-rdir-"+tag))
		edir := mkdir(t, filepath.Join(dir, "emitToWriter-edir-"+tag))
...
func testEmitToNonexistentDir(t *testing.T, harnessPath string, dir string) {
	withAndWithoutRunner(func(setGoCoverDir bool, tag string) {
		...
		rdir := mkdir(t, filepath.Join(dir, "emitToNonex-rdir-"+tag))
		edir := mkdir(t, filepath.Join(dir, "emitToNonex-edir-"+tag))
...
func testEmitToUnwritableDir(t *testing.T, harnessPath string, dir string) {
	withAndWithoutRunner(func(setGoCoverDir bool, tag string) {
		...
		// Create an unwritable dir here.
		edir := filepath.Join(dir, "emitToUnwr-edir-"+tag)
		if err := os.Mkdir(edir, 0555); err != nil {
			t.Fatalf("mkdir failed: %v", err)
		}
		os.Chmod(edir, 0555)
		defer os.Chmod(edir, 0777)

		rdir := mkdir(t, filepath.Join(dir, "emitToUnwr-rdir-"+tag))
...
func testEmitToNilWriter(t *testing.T, harnessPath string, dir string) {
	withAndWithoutRunner(func(setGoCoverDir bool, tag string) {
		...
		rdir := mkdir(t, filepath.Join(dir, "emitToNilWriter-rdir-"+tag))
		edir := mkdir(t, filepath.Join(dir, "emitToNilWriter-edir-"+tag))
...
func testEmitToFailingWriter(t *testing.T, harnessPath string, dir string) {
	withAndWithoutRunner(func(setGoCoverDir bool, tag string) {
		...
		rdir := mkdir(t, filepath.Join(dir, "emitToFailingWriter-rdir-"+tag))
		edir := mkdir(t, filepath.Join(dir, "emitToFailingWriter-edir-"+tag))
