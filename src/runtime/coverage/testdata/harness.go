...
func emitToNonexistentDir() {
	...
	if err := coverage.EmitMetaDataToDir(mangled); err == nil {
		...
		if !strings.Contains(got, want) {
			log.Fatalf("meta-data emit to bad dir: wanted error containing %q got %q", want, got)
		}
	}
	...
	if err := coverage.EmitCounterDataToDir(mangled); err == nil {
		...
		got := fmt.Sprintf("%v", err)
		if !strings.Contains(got, want) {
			log.Fatalf("emitting counter data to bad dir: wanted error containing %q got %q", want, got)
		}
	}
}
