...
// generated code.
func processCoverTestDir(dir string, cfile string, cm string) error {
	...
	// Collect pods from test run. For the majority of cases
	// we would expect to see a single pod here, but allow
	// for multiple pods in case the test harness is doing
	// extra work to collect data files from builds that it
	// kicks off as part of the testing.
	...
	// Emit percent.
	if err := ts.cf.EmitPercent(os.Stdout); err != nil {
		return err
	}

	// Emit text output.
	if tf != nil {
		if err := ts.cf.EmitTextual(tf, cmode); err != nil {
			return err
		}
		tfClosed = true
	...
	for pkIdx := uint32(0); pkIdx < np; pkIdx++ {
		...
		for fnIdx := uint32(0); fnIdx < nf; fnIdx++ {
			...
			for i := 0; i < len(fd.Units); i++ {
				...
				ts.cf.AddUnit(fd.Srcfile, u, count)
			}
		}
	}
	return nil
