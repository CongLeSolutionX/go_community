...
type covCounterBlob struct {
	...
	// the runtime/covercounter.go, since that code expects a
...
//go:linkname runtime_reportErrorInHardcodedList runtime.reportErrorInHardcodedList
func runtime_reportErrorInHardcodedList(slot int32, pkgId int32)
...
func (s *emitState) VisitFuncs(f encodecounter.CounterVisitorFcn) error {
	...
	for _, c := range s.counterlist {
		...
		for i < len(sd) {
			...
			if ipk == 0 {
				fmt.Fprintf(os.Stderr, "\n")
				runtime_reportErrorInHardcodedList(int32(i), ipk)
			} else if ipk < 0 {
