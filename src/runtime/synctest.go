...
type synctestGroup struct {
	...
	running int   // non-idle goroutines
	active  int   // group will not idle when active > 0
}
...
// A group does not become idle while the active-count is non-zero.
...
// maybeWakeLocked returns a g to wake if the group is idle.
func (sg *synctestGroup) maybeWakeLocked() *g {
	...
	// All goroutines in the group are idle, and nothing has called Wait.
	// active > 0 while root runs, so we know it isn't running now.
...
func synctestWait() {
	...
	// Establish a happens-before relationship on the activity of the now-idle
