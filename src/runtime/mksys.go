	...
	for s.Scan() {
		...
		if strings.HasPrefix(line, "//sys") {
			...
			case strings.HasPrefix(line, "//sysfatal"):
				mode = "fatal"
				line = strings.TrimSpace(line[10:])
			case strings.HasPrefix(line, "//syscommon"):
				mode = "common"
			...
			fmt.Fprintf(outFile, "TEXT runtime·%s(SB),NOSPLIT,$0-%d\n", fn.Name, argSize)
...
type Func struct {
	...
	Results []*Param
	Sysname string
	Mode    string // "noret", "fatal", "common" or blank
}
...
type Compiler struct {
	SysReg     string
	RetReg     string
	ArgReg     func(num int) string
	Mov        func(size int) string
	Syscall    Instruction
	CheckError []Instruction
	Crash      []Instruction

	NormError []Instruction

	// CISC
	CheckAndCrashError  []Instruction
	CheckAndCommonError []Instruction
	CheckAndNormError   []Instruction
...
func (c *Compiler) Compile(fn *Func) (argSize int, insts []Instruction, err error) {
	if c.SysBase != "" {
		insts = append(insts, Instruction{
			Op: c.Mov(ptrSize),
			Args: []string{
				fmt.Sprintf("$(%s+%s)", c.SysBase, fn.Sysname),
				c.SysReg,
			},
		})
	} else {
	...
	case "fatal":
		if len(c.CheckAndCrashError) != 0 {
			insts = append(insts, c.CheckAndCrashError...)
		} else {
	...
	case "common":
		if len(fn.Results) == 0 {
			return 0, nil, errors.New("should return values in common mode")
		}

		if len(c.CheckAndCommonError) != 0 {
			insts = append(insts, c.CheckAndCommonError...)
		} else {
			insts = append(insts, c.CheckError...)
			insts = append(insts, Instruction{
				Op: c.Mov(ptrSize),
	...
	default:
		if len(fn.Results) > 0 && (len(c.NormError) != 0 || len(c.CheckAndNormError) != 0) {
			if len(c.CheckAndNormError) != 0 {
				insts = append(insts, c.CheckAndNormError...)
			} else {
				insts = append(insts, c.CheckError...)
				insts = append(insts, c.NormError...)
			}
		}
	}
