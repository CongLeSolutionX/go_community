...
func selectsetpc(cas *scase) {
	cas.pc = getcallerpc(unsafe.Pointer(&cas))
}
...
func selectgo(cas0 *scase, order0 *uint16, ncases int) (int, bool) {
	...
	scaseslice := slice{unsafe.Pointer(cas0), ncases, ncases}
	...
	pollslice := slice{unsafe.Pointer(order0), ncases, ncases}
	...
	lockslice := slice{add(unsafe.Pointer(order0), uintptr(ncases)*unsafe.Sizeof(*order0)), ncases, ncases}
