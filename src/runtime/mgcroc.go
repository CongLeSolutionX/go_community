...
// Publish the spans associated with macache c. This can be
// called without an associated g, for example with a syscall
// is blocked and the p is being retaken.
func (c *mcache) publishMCache(rollbackStat bool) {
	for i := range c.alloc {
		...
		if s != &emptymspan {
			...
			// are released. The span in c.alloc[i] that is still being used in not
			// released.
			next := s.nextUsedSpan
			s.nextUsedSpan = nil
			...
			for s = next; s != nil; s = next {
				...
				if s.allocated() != s.nelems {
					// Only the (skipped) first span (c.alloc[i]) should have free objects.
