...
//
// One might be tempted to ignore the write barrier if slot points in
// to the stack. Don't do it! Mark termination only re-scans frames
// that have potentially been active since the concurrent scan, so it
// depends on write barriers to track changes to pointers in stack
// frames that have not been active.
//go:nowritebarrier
