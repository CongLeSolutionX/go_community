...
type mOS struct {
	...
	// wait for the preemption. preemptM CASes this from 0 to 2.
	// If this fails, the preemption fails (as it would if the PC
	// weren't in Go code). The value is reset to 0 when returning
	// from external code or after a preemption is complete.
	//
...
func preemptM(mp *m) {
	...
	if !atomic.Cas(&mp.preemptExtLock, 0, 2) {
