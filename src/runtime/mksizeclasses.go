...
func computeDivMagic(c *class) {
	...
	// Small k allow us to fit the math required into 32 bits
...
nextk:
	for k := uint(0); ; k++ {
		...
		if uint64(mul)*uint64(max-1) >= 1<<32 {
			panic("mul*(max-1) too big")
		}
		c.mul = mul
		c.shift2 = k
		break
...
func printClasses(w io.Writer, classes []class) {
	...
	for _, c := range classes {
		b := "0"
		if powerOfTwo(c.size) {
			// If the size is a power of two, heapBitsForObject can divide even faster by masking.
			// Compute this mask.
			b = fmt.Sprintf("1<<16 - %d", c.size)
		}
		fmt.Fprintf(w, "{%d,%d,%d,%s},", c.shift, c.shift2, c.mul, b)
	}
	fmt.Fprintln(w, "}")
