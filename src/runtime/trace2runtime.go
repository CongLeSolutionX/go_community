...
type gTraceState struct {
	...
	sysExitTime traceTime // Unused; solely for backwards compatibility.

	// inMarkAssist indicates that the goroutine is in mark assist.
	inMarkAssist bool
}
...
func (s *gTraceState) reset() {
	s.seq = [2]uint64{}
	// N.B. inMarkAssist and s.statusTraced are managed and cleared separately.
}
...
//
// TODO(mknyszek): Rename this to traceGoDestroy.
...
func (tl traceLocker) GoUnpark(gp *g, skip int) {
	...
	if !gp.trace.statusWasTraced(tl.gen) && gp.trace.acquireStatus(tl.gen) {
		...
		w.w = w.w.writeGoStatus(gp.goid, -1, traceGoWaiting, gp.trace.inMarkAssist)
	}
...
func (tl traceLocker) GoSysCall() {
	...
	tl.mp.p.ptr().trace.mSyscallID = tl.mp.id
	tl.eventWriter(traceGoRunning, traceProcRunning).commit(traceEvGoSyscallBegin, tl.stack(skip))
}

// GoSysCall emits a GoSyscallEnd event, possibly along with a GoSyscallBlocked event
// if lostP is true.
//
...
func (tl traceLocker) ProcSteal(pp *p) {
	...
	// Emit a ProcRunning event for the P we just stole if necessary.
	goStatus := traceGoBad
	if tl.mp.curg != nil {
		goStatus = goStatusToTraceGoStatus(readgstatus(tl.mp.curg), tl.mp.curg.waitreason)
	}
	w := tl.eventWriter(goStatus, traceProcRunning)
...
func (tl traceLocker) OneNewExtraM(gp *g) {
	// TODO(mknyszek): This is probably wrong.
	//
	// Trigger two trace events for the locked g in the extra m,
	// since the next event of the g will be traceEvGoSyscallEnd in exitsyscall,
	// while calling from C thread to Go.
	w := tl.eventWriter(traceGoBad, traceProcBad)
	w.w.writeGoStatus(uint64(gp.goid), -1, traceGoSyscall, false)
	w.end()
}
