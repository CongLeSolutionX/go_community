...
type timers struct {
	...
	// len is the number of timers in the set.
	len atomic.Uint32

	// zombies is the number of deleted timers
	// remaining in P's heap.
...
// If ts != nil, then t must be ts.heap[0], and updateWhen takes care of
...
// The caller must have locked ts.lock.
func (ts *timers) add(t *timer) {
	...
	if t.ts != nil {
		throw("timers.add: P already set in timer")
	}
	t.ts = ts
...
func (t *timer) modify(when, period int64, f func(any, uintptr), arg any, seq uintptr) bool {
	...
	pending := state&timerHeaped != 0 && (state&timerNextWhen == 0 || t.nextWhen != 0)
	if !pending {
		t.ts.zombies.Add(-1)
	}
...
// adjust looks through the timers in the current P's heap for
...
// check runs any timers for the P that are ready.
...
// run examines the first timer in timers. If it is ready based on now,
...
func (ts *timers) unlockAndRun(t *timer, now int64, state uint32, mp *m) {
	...
	if raceenabled {
		...
		gp := getg()
		if gp.racectx != 0 {
			throw("timers.run: unexpected racectx")
		}
