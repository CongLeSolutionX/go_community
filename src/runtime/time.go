...
// It returns true if t was stopped, false if t was already run.
	...
	loop:
		for {
			...
			case timerModifying:
				...
				osyield()
			case timerNoStatus, timerRemoved:
				// We should not see these status in a timers heap.
				badTimer()
			case timerRunning, timerRemoving, timerMoving:
...
func runtimer(pp *p, now int64) int64 {
	for {
		...
		case timerNoStatus, timerRemoved:
			...
			badTimer()
		case timerRunning, timerRemoving, timerMoving:
			// Only this P should set these statuses.
			badTimer()
		default:
			badTimer()
		}
...
// The caller must have acquired the timers for pp.
func runOneTimer(pp *p, t *timer, delta int64) {
	f := t.f
	arg := t.arg
	seq := t.seq
