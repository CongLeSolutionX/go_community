...
func SetBlockProfileRate(rate int) {
	...
	_base.Atomicstore64(&_gc.Blockprofilerate, uint64(r))
}
...
func MemProfile(p []MemProfileRecord, inuseZero bool) (n int, ok bool) {
	_base.Lock(&_gc.Proflock)
	clear := true
	for b := _gc.Mbuckets; b != nil; b = b.Allnext {
		mp := b.Mp()
	...
	_base.Unlock(&_gc.Proflock)
	return
}

// Write b's data to r.
func record(r *MemProfileRecord, b *_gc.Bucket) {
	mp := b.Mp()
...
func iterate_memprof(fn func(*_gc.Bucket, uintptr, *uintptr, uintptr, uintptr, uintptr)) {
	_base.Lock(&_gc.Proflock)
	for b := _gc.Mbuckets; b != nil; b = b.Allnext {
		mp := b.Mp()
...
func BlockProfile(p []BlockProfileRecord) (n int, ok bool) {
	_base.Lock(&_gc.Proflock)
	for b := _gc.Bbuckets; b != nil; b = b.Allnext {
		n++
	}
	if n <= len(p) {
		ok = true
