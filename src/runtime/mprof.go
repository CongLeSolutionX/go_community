...
func persistentallocpcbuf(n int) []uintptr {
	if n == 0 {
		return nil
	}
	size := unsafe.Sizeof(uintptr(0)) * uintptr(n)
	return unsafe.Slice((*uintptr)(persistentalloc(size, 0, &memstats.buckhash_sys)), n)
}
