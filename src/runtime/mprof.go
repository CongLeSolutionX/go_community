...
func tryRecordGoroutineProfile(gp1 *g) {
	...
	for {
		...
		// While we have gp1.goroutineProfiled set to 1, gp1 appears _Grunnable
		// but is not actually able to run. Disable preemption for ourselves, to
		// make sure we finish profiling gp1 right away instead of leaving it
		// stuck in this limbo.
		mp.preemptoff = "goroutine profiling"
...
func doRecordGoroutineProfile(gp1 *g) {
	...
	// set gp1.goroutineProfiled to 1 and so are still preventing it from being
	// truly _Grunnable. So we'll use the system stack to avoid schedule delays.
