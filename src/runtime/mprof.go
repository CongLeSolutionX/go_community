...
func record(r *MemProfileRecord, b *bucket) {
	...
	applyprofstack(b.stk(), &r.Stack0, &r.stack1)
}
...
// frames. Callers must not modify the contents of stk after calling
// applyprofstack.
func applyprofstack(stk []uintptr, stack0 *[32]uintptr, stack1 *stack1) {
	i := copy(stack0[:], stk)
	clear(stack0[i:])
...
func saveg(pc, sp uintptr, gp *g, r *StackRecord) {
	// We can't use mp.profstack here because applyprofstack requires that the
	// stk we pass to it is immutable.
	pcbuf := make([]uintptr, debug.profstackdepth)
	var u unwinder
	u.initAt(pc, sp, 0, gp, unwindSilentErrors)
	n := tracebackPCs(&u, 0, pcbuf)
	applyprofstack(pcbuf[0:n], &r.Stack0, &r.stack1)
}
