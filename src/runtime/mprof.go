...
// a particular mutex.
//
// It takes as an argument the pointer bits of the mutex's key. It returns the
// new pointer value. (The caller is responsible for clearing and restoring the
// flag values.)
//
...
// acquire.
func fixMutexWaitList(head muintptr) muintptr {
	if head == 0 {
		return 0
	}
	hp := head.ptr()
...
//
// It modifies the nodes that are to remain in the list, but it does not modify
// mp's fields. It returns the value to assign as the head of the list, with the
// caller responsible for ensuring that the (atomic, contended) head assignment
// worked and subsequently clearing the list-related fields of mp.
//
// The caller must hold the mutex that the Ms of the list are waiting to
// acquire.
func removeMutexWaitList(head muintptr, mp *m) muintptr {
	...
	if head.ptr() == mp {
		// mp is the head
		if verifyMutexWaitList && mp.mWaitList.prev.ptr() != nil {
		...
		if mp.mWaitList.prev.ptr() == nil {
			// mp is not in the list at all
			return head
		}
		mp.mWaitList.prev.ptr().mWaitList.next = mp.mWaitList.next
		if tail.ptr() == mp {
			// mp is the tail
			if verifyMutexWaitList && mp.mWaitList.next.ptr() != nil {
...
func claimMutexWaitTime(now timePair, head muintptr) timePair {
	...
	if waiters > 1 {
		dt.nanotime = int64(waiters) * (dt.nanotime + now.nanotime - tailTimes.nanotime)
		dt.cputicks = int64(waiters) * (dt.cputicks + now.cputicks - tailTimes.cputicks)
	}
