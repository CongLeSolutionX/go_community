...
func (b *mallocBits) find1(searchIdx uint) uint {
	for i := searchIdx / 64; i < uint(len(b)); i++ {
		x := b[i]
		if x == ^uint64(0) {
			continue
		}
		return uint(i)*64 + uint(bits.TrailingZeros64(^x))
	}
	return ^uint(0)
}
...
func (b *mallocBits) findSmallN(npages uintptr, searchIdx uint) (uint, uint) {
	...
	for i := searchIdx / 64; i < uint(len(b)); i++ {
		...
		if newSearchIdx == ^uint(0) {
			...
			newSearchIdx = uint(i)*64 + uint(bits.TrailingZeros64(^bi))
		}
...
func (b *mallocBits) findLargeN(npages uintptr, searchIdx uint) (uint, uint) {
	...
	for i := searchIdx / 64; i < uint(len(b)); i++ {
		...
		if newSearchIdx == ^uint(0) {
			// The new searchIdx is going to be at these 64 bits after any
			// 1s we file, so count trailing 1s.
			newSearchIdx = uint(i)*64 + uint(bits.TrailingZeros64(^x))
		}
		if size == 0 {
...
func (b *mallocBits) allocRange(i, n uint) {
...
func (b *mallocBits) free1(i uint) {
	(*pageBits)(b).clear(i)
}

// free frees the range [i, i+n) of pages in the mallocBits.
func (b *mallocBits) free(i, n uint) {
