...
func convT2E16(t *_type, val uint16) (e eface) {
	if raceenabled {
		raceReadObjectPC(t, unsafe.Pointer(&val), getcallerpc(), funcPC(convT2E16))
	}
	if msanenabled {
		msanread(unsafe.Pointer(&val), t.size)
	}
...
func convT2E32(t *_type, val uint32) (e eface) {
	if raceenabled {
		raceReadObjectPC(t, unsafe.Pointer(&val), getcallerpc(), funcPC(convT2E32))
	}
	if msanenabled {
		msanread(unsafe.Pointer(&val), t.size)
	}
...
func convT2E64(t *_type, val uint64) (e eface) {
	if raceenabled {
		raceReadObjectPC(t, unsafe.Pointer(&val), getcallerpc(), funcPC(convT2E64))
	}
	if msanenabled {
		msanread(unsafe.Pointer(&val), t.size)
	}
