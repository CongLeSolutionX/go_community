...
func (b *pallocBits) summarize() pallocSum {
	...
	for i := 0; i < len(b); i++ {
		...
		// 000..l..000 1xxxxx1 000..t..000
		// We need to look inside the 1xxxxx1 for any contiguous
		// region of zeros.

		// n = number of bits we're examining. This count includes
		// the 1 bit we know is at either end (but it may be the
		// same 1 bit, so n could be as small as 1).
		n := 64 - t - l

		// Remove trailing zeros, so the part we're interested in is
		// at the bottom of x.
		x >>= t

		// The main loop below takes a number of iterations
		// equal to the number of internal zero runs. If we
		// have lots of zero runs, that loop will take too
		// long. Fortunately, if there are lots of zero runs
		// then there must be short ones. It is pretty easy to
		// get rid of short zero runs; just shrink each zero
		// run by a single zero, and try again. z keeps track
		// of the number of shrinks we did.
		z := uint(0)
	shrinkRuns:
		// Count the number of internal zero runs. We detect these
		// as a 1 with a zero just above it. That overcounts by 1,
		// which comes from the top 1.
		c := uint(sys.OnesCount64(x & ^(x>>1))) - 1
		if c > 4 {
			x |= x >> 1
			z++
			goto shrinkRuns
		...
		for ; c > 0; c-- {
			if max >= z+n-2*c {
				// The maximum length of a zero run at this point is n-2*c.
				//   1000..000001     one run of zeros, max length n-2.
				//   1000..000101     two runs of zeros, max length n-4.
				//   1000..010101     two runs of zeros, max length n-6.
				// If we already have a max run at least that long,
				// don't bother looking further.
				break
			}
			j := uint(sys.TrailingZeros64(^x)) // count contiguous trailing ones
			x >>= j                            // remove trailing ones
			n -= j
			k := uint(sys.TrailingZeros64(x)) // count next group of zeros
			x >>= k                           // remove zeros
			n -= k
			if z+k > max {
				max = z + k
