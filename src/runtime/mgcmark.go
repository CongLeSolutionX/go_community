...
func scanobject(b uintptr, gcw *gcWork) {
	...
	if n > maxObletBytes {
		...
		if b == spanBase {
			...
			// scan those.
			for oblet := b + maxObletBytes; oblet < spanBase+n; oblet += maxObletBytes {
		...
		// must be a large object, spanBase is the beginning
		// of the object.
		n = spanBase + n - b
		if n > maxObletBytes {
			n = maxObletBytes
...
func scanConservative(b, n uintptr, ptrmask *uint8, gcw *gcWork, state *stackScanState) {
	...
	for i := uintptr(0); i < n; i += goarch.PtrSize {
		...
		spanCache, arena := spanAndArenaOfHeap(val)
		if spanCache == nil {
			continue
		}
		span := spanCache.span()
