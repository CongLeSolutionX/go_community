...
func markroot(desc *parfor, i uint32) {
	...
	default:
		if _RootSpans0 <= i && i < _RootSpans0+_RootSpansShards {
			// mark MSpan.specials
			markrootSpans(&gcw, uint32(i-_RootSpans0))
			break
		}
...
// markrootSpans marks roots for spans (N mod _RootSpansShards == shard).
//
//go:nowritebarrier
func markrootSpans(gcw *gcWork, shard uint32) {
	sg := mheap_.sweepgen
	for spanidx := shard; spanidx < uint32(len(work.spans)); spanidx += _RootSpansShards {
