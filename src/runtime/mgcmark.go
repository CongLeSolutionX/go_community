				...
				if p != 0 {
					if obj, span, objIndex := findObject(p, b, i); obj != 0 {
						...
						if p >= stk.stack.lo && p < stk.stack.hi {
							if debug.invalidptr >= 2 && stk.sp >= stk.stack.lo && stk.sp < stk.stack.hi {
								// TODO: this could be only for debug.invalidptr >= 2 or similar?
								// TODO: what should the precise check here be?
								// TODO: investigate how this triggers on Windows in TestCallbackPanicLoop with just p+goarch.PtrSize.
								if p+(2*goarch.PtrSize) < stk.sp && p+(2*goarch.PtrSize) > p {
					...
					} else if debug.invalidptr >= 2 && span != nil && span.state.get() == mSpanManual {
