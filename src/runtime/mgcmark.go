...
func scanCard(c uintptr, gcw *gcWork) {
	...
	for idx, b := startIdx, s.base()+startIdx*n; b < c+cardSize && b+n <= endOfData; idx, b = idx+1, b+n {
		...
		// Note that we only check the ptrTarget bits in this card.
		// If ptrTarget bits were set for this object in another card,
		// they will be handled when that other card is processed.
		// (Note those ptrTarget bits may be in different heap arenas,
		// so it is nontrivial to access them.)
		// TODO: this check is unnecessary if the card is completely
		// contained in the object. There must be at least one ptrTarget
		// bit set in every dirty card.
		// TODO: read all the ptrTarget bits from the card initially, then
		// just do masks here.
		x, y := max(c, b), min(c+cardSize, b+n)
		if !bitSet(ha.ptrTarget[:]).hasInRange(x/objectQuantum%quantaPerArena, y/objectQuantum%quantaPerArena) {
		...
		//bitSet(ha.ptrTarget[:]).atomicSetInRange(x/objectQuantum%quantaPerArena, y/objectQuantum%quantaPerArena)
	...
	if c+cardSize > endOfData {
		// The card includes a tail that is either padding or
		// ptr/nonptr bits (or both). If any pointers target here, throw.
		x, y := max(c, endOfData), c+cardSize
		if bitSet(ha.ptrTarget[:]).hasInRange(x/objectQuantum%quantaPerArena, y/objectQuantum%quantaPerArena) {
			throw("found pointer to junk")
		}
	}
}
