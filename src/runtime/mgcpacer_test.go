	...
	for _, e := range []*gcExecTest{
		{
			...
			growthRate:    constant(2.0).mix(ramp(-1.0, 12)),
		...
		},
		{
			...
			globalsBytes:  32 << 10,
			nCores:        8,
			allocRate:     constant(33.0).mix(ramp(66.0, 1).delay(50)),
			scanRate:      constant(1024.0),
			growthRate:    constant(2.0).mix(ramp(-1.0, 12)),
			scannableFrac: constant(1.0),
			stackBytes:    constant(8192),
		...
		},
		{
			...
			globalsBytes:  32 << 10,
			nCores:        8,
			allocRate:     constant(33).mix(ramp(330, 1).delay(50)),
			scanRate:      constant(1024.0),
			growthRate:    constant(2.0).mix(ramp(-1.0, 12)),
			scannableFrac: constant(1.0),
			stackBytes:    constant(8192),
		...
		},
		{
			...
			scanRate:      constant(1024.0),
			growthRate:    constant(2.0).mix(ramp(-1.0, 12)),
			scannableFrac: constant(0.2).mix(unit(0.5).delay(50)),
			stackBytes:    constant(8192),
			length:        100,
			checker: func(t *testing.T, c []gcCycleResult) {
				n := len(c)
		...
		},
		{
			...
			scanRate:      constant(1024.0),
			growthRate:    constant(2.0).mix(ramp(-1.0, 12), random(0.01, 0x1), unit(14).delay(25)),
			scannableFrac: constant(1.0),
			stackBytes:    constant(8192),
			length:        50,
			checker: func(t *testing.T, c []gcCycleResult) {
		...
		},
		{
			...
			scanRate:      constant(1024.0),
			growthRate:    constant(2.0).mix(ramp(-1.0, 12), random(0.01, 0xe)),
			scannableFrac: constant(1.0),
			stackBytes:    constant(8192),
			length:        50,
			checker: func(t *testing.T, c []gcCycleResult) {
		...
		},
		{
			...
			scanRate:      constant(1024.0),
			growthRate:    constant(2.0).mix(ramp(-1.0, 12), random(0.01, 0x152)),
			scannableFrac: constant(1.0),
			stackBytes:    constant(8192),
			length:        50,
			checker: func(t *testing.T, c []gcCycleResult) {
		...
		t.Run(e.name, func(t *testing.T) {
			...
			for i := 0; i < e.length; i++ {
				cycle := e.next()
				c.StartCycle(cycle.stackBytes, e.globalsBytes, cycle.scannableFrac)
...
// vga returns a new stream that is f attenuated by the values
// produced by gain at each step.
//
// VGA stands for variable gain attenuation.
func (f float64Stream) vga(gain float64Stream) float64Stream {
	return func() float64 {
		return f() * gain()
	}
}
...
// mix returns a new stream that is the sum of all input streams
// at each step.
func (f float64Stream) mix(fs ...float64Stream) float64Stream {
	return func() float64 {
		sum := f()
		for _, s := range fs {
			sum += s()
