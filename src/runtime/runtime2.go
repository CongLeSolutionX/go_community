...
type schedt struct {
	...
	_ uint32 // Align timeToRun on 32-bit platforms.
...
func (t *atomicCPUTime) update(procs uint32, now int64) {
	...
	if now > oldStamp {
		t.total.Add((now - oldStamp) * int64(t.procs.Load()))
		t.stamp.Store(now)
	}
	// Even if we're using a stale "now" value, the procs value
	// is always going to be the most up-to-date, because this
