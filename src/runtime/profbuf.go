...
func countSub(x, y uint32) int {
	// x-y is 30-bit signed; sign-extend to 32 bits and convert to int.
...
func (x profIndex) addCountsAndClearFlags(data, tag int) profIndex {
	return profIndex((uint64(x)>>34+uint64(uint32(tag)<<2>>2))<<34 | uint64(uint32(x)+uint32(data)<<2>>2))
}
...
Read:
	...
	if len(data) > numData {
		...
		numData -= len(data) // available in case of rewind
	}
	skip := 0
	if data[0] == 0 {
		skip = len(data)
