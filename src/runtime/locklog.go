...
var lockNames []string = []string{
	...
	"sched", // below scavenge, forcegc, sweepWaiters, cpuprof, assistQueue
	"deadlock",
	"panic", // below deadlock*
	"allg",  // below tb?, sched, panic
	"allp",  // below sched
	"pollDesc",
	...
	"mspanSpecial", // below scavenge, cpuprof, sched, allg, timers, itab, reflectOffs, hchan, notifyList, traceBuf, traceStrings
	"prof",         // below scavenge, cpuprof, sched, allg, allp, timers, itab, reflectOffs, notifyList, traceBuf, traceStrings, hchan
	"gcBitsArenas", // below scavenge, sched, allg, timers, itab, reflectOffs, notifyList, traceBuf, traceStrings, hchan
	"root",
	"sweep",
	"trace",      // below scavenge, assistQueue, sched, hchan, traceBuf, traceStrings, root, sweep
	"stackpool",  // below scavenge, assistQueue, cpuprof, sched, timers, itab, reflectOffs, notifyList, traceBuf, traceStrings, hchan, prof, root, gcBitsArenas?, traceTab?
	"stackLarge", // below sched, itab, hchan, prof
	"defer",
	"wbufSpans", // below sched, allg, timers, itab, notifyList, hchan, mspanSpecial, root, defer
	...
	"mheapSpecial", // below scavenge, cpuprof, sched, allg, timers, itab, reflectOffs, notifyList, traceBuf, traceStrings,, hchan
	...
	"traceTab", // below scavenge, sweepWaiters, assistQueue, sched, allg, timers, fin, notifyList, traceBuf, traceStrings, hchan, root, trace, sweep, mheap
	...
	// Leaf locks with no dependencies
	"globalAlloc",
	"newmHandoff",
	"debugPtrmask",
	"faketimeState",
...
func lockLabeled(l *mutex, rank int) {
	...
	systemstack(func() {
		...
		if i > 1 && gp.m.locksHeld[i-1].rank < gp.m.locksHeld[i-2].rank {
			println(gp.m.procid, " ======")
			for j := 0; j < i; j++ {
				println(j, ":", lockNames[gp.m.locksHeld[j].rank], " ", gp.m.locksHeld[j].rank, " ", unsafe.Pointer(gp.m.locksHeld[j].l))
			}
