...
// Memory allocator.
//
// This is originally based on tcmalloc, but has diverged quite a bit.
...
//
//	2. If the mspan has no free slots, obtain a new mspan of
...
// hierarchy:
//
//	1. If the mspan still has allocated objects in it, place it on
//	   the mcentral free list for the mspan's size class.
//
//	2. Otherwise, if all objects in the mspan are free, the mspan
//	   if now "idle", so return it to the mheap.
//	   This may coalesce it with adjacent idle mspans.
//
//	3. If an mspan remains idle for long enough, return its pages
...
//
//	1. stack frames allocation can avoid zeroing altogether.
//	2. it exhibits better temporal locality, since the memory
//	   is likely about to be written to by the application.
