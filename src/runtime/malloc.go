...
const (
	...
	// On other 64-bit platforms, we limit the arena to 512GB, or 38 bits.
...
func mallocinit() {
	...
	if ptrSize == 8 && (limit == 0 || limit > 1<<30) {
		...
		// Allocating a 256 GB region takes away 39 bits, and the amd64
...
func mHeap_SysAlloc(h *mheap, n uintptr) unsafe.Pointer {
	if n > uintptr(h.arena_end)-uintptr(h.arena_used) {
		...
		if new_end <= h.arena_start+_MaxArena32 {
			...
			if p == h.arena_end {
				...
				atomicstoreuintptr(&h.arena_used, p + (-uintptr(p) & (_PageSize - 1)))
				h.arena_reserved = reserved
			} else {
				var stat uint64
	...
	if n <= uintptr(h.arena_end)-uintptr(h.arena_used) {
		...
		atomicstoreuintptr(&h.arena_used, p+n)
		if raceenabled {
