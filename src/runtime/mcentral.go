...
func (c *mcentral) releaseROCSpan(s *mspan) {
	...
	// 3 possibilities are split out even though two do the same thing.
	if s.allocCount == s.nelems {
		// 1. Free list is empty so put on empty freelist list.
		c.empty.remove(s)
		c.empty.insert(s)
	} else if s.allocCount > 0 {
	...
	if memstats.heap_live < 0 {
		println("memstats.heap_live=", memstats.heap_live,
