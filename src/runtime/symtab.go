...
func (ci *Frames) Next() (frame Frame, more bool) {
	for len(ci.frames) < 2 {
		...
		u := newInlineUnwinder(funcInfo, pc, nil)
		sf := u.srcFunc()
		if u.isInlined() {
...
func runtime_expandFinalInlineFrame(stk []uintptr) []uintptr {
	...
	u := newInlineUnwinder(f, tracepc, &cache)
	if !u.isInlined() {
	...
	for ; u.valid(); u.next() {
		funcID := u.srcFunc().funcID
		if funcID == funcID_wrapper && elideWrapperCalling(lastFuncID) {
			// ignore wrappers
		} else {
			stk = append(stk, u.pc+1)
		}
		lastFuncID = funcID
	}
