...
type Frames struct {
	...
	// frames expands callers into a sequence of Frames, tracking
	// the necessary state across PCs.
	frames frameExpander
}
...
// frameExpander expands a sequence of PCs into a sequence of Frames.
// It tracks the small amount of state across PCs necessary to perform
// this expansion.
//
...
func CallersFrames(callers []uintptr) *Frames {
	ci := &Frames{}
	ci.callers = ci.frames.init(callers)
	return ci
}

func (fe *frameExpander) init(callers []uintptr) []uintptr {
	if len(callers) >= 1 {
		pc := callers[0]
		s := pc - skipPC
...
func (ci *Frames) Next() (frame Frame, more bool) {
	ci.callers, frame, more = ci.frames.next(ci.callers)
	return
}

func (fe *frameExpander) next(callers []uintptr) (ncallers []uintptr, frame Frame, more bool) {
	ncallers = callers
	if !fe.expander.more {
		// Expand the next PC.
		if len(ncallers) == 0 {
		...
		fe.expander.init(ncallers[0], fe.wasPanic)
		ncallers = ncallers[1:]
		fe.wasPanic = fe.expander.funcInfo.valid() && fe.expander.funcInfo.entry == sigpanicPC
		if fe.skip > 0 {
			for ; fe.skip > 0; fe.skip-- {
				fe.expander.next()
			}
			fe.skip = 0
	...
	frame = fe.expander.next()
	return ncallers, frame, fe.expander.more || len(ncallers) > 0
}
