...
func TestPageAllocAllocToCache(t *testing.T) {
	tests := map[string]struct {
		...
		"AllFree": {
			...
			hits: []PageCache{
				NewPageCache(PageBase(0xc00, 0), ^uint64(0), 0x2),
				NewPageCache(PageBase(0xc00, 64), ^uint64(0), ^uint64(0)),
				NewPageCache(PageBase(0xc00, 128), ^uint64(0), 0),
				NewPageCache(PageBase(0xc00, 192), ^uint64(0), 0),
			},
			after: map[int][]BitRange{
				0xc00: {{0, PagesPerArena / 4}},
			},
		},
		"ManyArena": {
			before: map[int][]BitRange{
			...
			hits: []PageCache{
				NewPageCache(PageBase(0xc02, PagesPerArena-64), ^uint64(0), 0),
			},
			after: map[int][]BitRange{
				0xc00: {{0, PagesPerArena}},
				0xc01: {{0, PagesPerArena}},
		...
		"NotContiguous": {
			...
			hits: []PageCache{
				NewPageCache(PageBase(0xcff, 0), ^uint64(0), SetConsecBits64(0, 31, 33)),
			},
			after: map[int][]BitRange{
				0xc00: {{0, PagesPerArena}},
				0xcff: {{0, 64}},
		...
		"First": {
			before: map[int][]BitRange{
				0xc00: {{0, 32}, {33, 31}, {96, 32}},
			},
			scav: map[int][]BitRange{
				0xc00: {{1, 4}, {31, 5}, {66, 2}},
			},
			hits: []PageCache{
				NewPageCache(PageBase(0xc00, 0), 1<<32, 1<<32),
				NewPageCache(PageBase(0xc00, 64), SetConsecBits64(0, 0, 32), SetConsecBits64(0, 2, 2)),
			},
			after: map[int][]BitRange{
				0xc00: {{0, 128}},
			},
