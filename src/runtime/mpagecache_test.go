...
func TestPageCacheAlloc(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Hi53": {
			cache: NewPageCache(base, SetConsecBits64(0, 10, 53), SetConsecBits64(0, 10, 16)),
			hits: []hit{
		...
		"Full53": {
			cache: NewPageCache(base, ^uint64(0), SetConsecBits64(0, 10, 16)),
			hits: []hit{
				{53, base, 16 * PageSize},
				{53, 0, 0},
...
func TestPageAllocAllocToCache(t *testing.T) {
	tests := map[string]struct {
		...
		"AllFree": {
			...
			scav: map[int][]BitRange{
				BaseArenaIdx: {{1, 1}, {64, 64}},
			},
			hits: []PageCache{
				NewPageCache(PageBase(BaseArenaIdx, 0), ^uint64(0), 0x2),
				NewPageCache(PageBase(BaseArenaIdx, 64), ^uint64(0), ^uint64(0)),
				NewPageCache(PageBase(BaseArenaIdx, 128), ^uint64(0), 0),
				NewPageCache(PageBase(BaseArenaIdx, 192), ^uint64(0), 0),
			},
			after: map[int][]BitRange{
				BaseArenaIdx: {{0, 256}},
			},
		...
		"ManyArena": {
			before: map[int][]BitRange{
				...
				BaseArenaIdx + 2: {{0, PagesPerArena - 64}},
			},
			scav: map[int][]BitRange{
				BaseArenaIdx:     {{0, PagesPerArena}},
				BaseArenaIdx + 1: {{0, PagesPerArena}},
			...
			hits: []PageCache{
				NewPageCache(PageBase(BaseArenaIdx+2, PagesPerArena-64), ^uint64(0), 0),
			},
			after: map[int][]BitRange{
				BaseArenaIdx:     {{0, PagesPerArena}},
				BaseArenaIdx + 1: {{0, PagesPerArena}},
		...
		"NotContiguous": {
			...
			scav: map[int][]BitRange{
				BaseArenaIdx:        {{0, PagesPerArena}},
				BaseArenaIdx + 0xff: {{31, 67}},
			},
			hits: []PageCache{
				NewPageCache(PageBase(BaseArenaIdx+0xff, 0), ^uint64(0), SetConsecBits64(0, 31, 33)),
			},
			after: map[int][]BitRange{
				BaseArenaIdx:        {{0, PagesPerArena}},
				BaseArenaIdx + 0xff: {{0, 64}},
			},
		},
		"First": {
			before: map[int][]BitRange{
				BaseArenaIdx: {{0, 32}, {33, 31}, {96, 32}},
			},
			scav: map[int][]BitRange{
				BaseArenaIdx: {{1, 4}, {31, 5}, {66, 2}},
			},
			hits: []PageCache{
				NewPageCache(PageBase(BaseArenaIdx, 0), 1<<32, 1<<32),
				NewPageCache(PageBase(BaseArenaIdx, 64), SetConsecBits64(0, 0, 32), SetConsecBits64(0, 2, 2)),
			},
			after: map[int][]BitRange{
				BaseArenaIdx: {{0, 128}},
			},
