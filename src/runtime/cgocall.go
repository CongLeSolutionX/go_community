...
func cgoCheckPointer(ptr interface{}, args ...interface{}) interface{} {
	...
	if len(args) > 0 && t.kind&kindMask == kindPtr {
		...
		default:
			...
			ep = (*eface)(unsafe.Pointer(&args[0]))
			t = ep._type
		}
	}

	cgoCheckArg(t, ep.data, t.kind&kindDirectIface == 0, true)
	return ptr
}
...
func cgoCheckArg(t *_type, p unsafe.Pointer, indir, top bool) {
	...
	case kindInterface:
		...
		// A type known at compile time is OK since it won't
		// be in the heap.  A type not known at compile time
		// will be in the heap and will not be OK.
	...
	case kindPtr, kindUnsafePointer:
		...
		// Check the entire heap object that p points to,
		// looking for a Go pointer.
		...
		for i := uintptr(0); i < n; i += ptrSize {
			...
			if bits&bitPointer != 0 {
				if inheap(*(*uintptr)(unsafe.Pointer(base + i))) {
					panic(errorString(cgoCheckPointerFail))
				}
			}
			hbits = hbits.next()
