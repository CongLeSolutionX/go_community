...
func (b *profBuf) write(tag uint64, now int64, hdr []uint64, stk []uintptr) {
	...
	available := atomicloaduintptr(&b.r) + uintptr(len(b.data)) - b.w
	...
	xadduintptr(&b.w, n) // commit write
}
...
func enableProfCPU(hz, bufsize int) {
	...
	atomicstore(&bgprof.hz, uint32(hz))
