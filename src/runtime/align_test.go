...
func TestAtomicAlignment(t *testing.T) {
	for i, d := range runtime.AtomicFields {
		if d%8 != 0 {
			t.Errorf("align field test %d failed: offset is %d", i, d)
		}
	}
	for i, p := range runtime.AtomicVariables {
		if uintptr(p)%8 != 0 {
			t.Errorf("align variable test %d failed: address is %x", i, p)
		}
	}
	...
	x, err := ioutil.ReadFile("./align_runtime_test.go")
	if err != nil {
		panic(err)
	}
...
func (v *Visitor) Visit(n ast.Node) ast.Visitor {
	...
	// Uncomment below to see what they are.
	//v.t.Errorf("unchecked atomic operation %s %v", v.fset.Position(n.Pos()), v.print(n))
