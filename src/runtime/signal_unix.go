...
func sighandler(sig uint32, info *siginfo, ctxt unsafe.Pointer, gp *g) {
	...
	// TSAN intercepts signals and calls the signal handler at a later time.
	// When the signal handler is called, the memory may have changed, but
	// the signal context remains old. The unmatched signal context and memory
	// makes it unsafe to unwind or inspect the stack. So we ignore delayed
	// non-fatal signals that will cause a stack inspection (profiling signal
	// and preemption signal).
