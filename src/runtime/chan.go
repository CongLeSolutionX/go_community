...
func makechan(t *_channels.Chantype, size int64) *_channels.Hchan {
	...
	if elem.Size >= 1<<16 {
		_lock.Gothrow("makechan: invalid channel element type")
	}
	if _channels.HchanSize%_channels.MaxAlign != 0 || elem.Align > _channels.MaxAlign {
		_lock.Gothrow("makechan: bad alignment")
	}
	...
	if elem.Kind&_hash.KindNoPointers != 0 || size == 0 {
		// Allocate memory in one call.
		// Hchan does not contain pointers interesting for GC in this case:
		// buf points into the same allocation, elemtype is persistent.
		// SudoG's are referenced from their owning thread so they can't be collected.
		// TODO(dvyukov,rlh): Rethink when collector can move allocated objects.
		c = (*_channels.Hchan)(_maps.Mallocgc(_channels.HchanSize+uintptr(size)*uintptr(elem.Size), nil, _sched.FlagNoScan))
