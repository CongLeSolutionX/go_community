...
// R1 is the kind if sigtramp function.
TEXT sigtramp<>(SB),NOSPLIT|NOFRAME,$0
	MOVM.DB.W [R0, R4-R11, R14], (R13)	// push {r0, r4-r11, lr} (SP-=40)
	SUB	$(8+16), R13		// reserve space for g, sp, and
					// parameters/retval to go call

	MOVW	$0, R4
	MOVW	R4, 0(R13)	// No saved link register.
	MOVW	R0, 4(R13)	// Move arg0 (ExceptionPointers) into position
	MOVW	R4, 8(R13)	// Move arg1 (kind) into position
	BL	(R7)		// Call the goroutine

	ADD	$(8 + 16), R13			// free locals
