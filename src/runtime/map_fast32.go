...
again:
	...
	top := tophash(hash)
	for b := h.getbucket(t, bucket); b != nil; b = b.overflow(t) {
		i := -1
		for ind := bytealg.IndexByte(b.tophash[:], top) + 1; ind != 0; ind = bytealg.IndexByte(b.tophash[i+1:], top) + 1 {
			i += ind
