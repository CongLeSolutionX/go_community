...
TEXT runtime·rt0_go(SB),NOSPLIT,$0
	...
	SUB	$(FIXED_STACK+16), R1
	MOVW	R3, FIXED_STACK+0(R1) // argc
	MOVD	R4, FIXED_STACK+8(R1) // argv
...
loop:						\
	...
	MOVWZ	n+24(FP), R4;			\
	MOVWZ	retoffset+28(FP), R6;		\
	MOVD	R7, FIXED_STACK+0(R1);			\
	MOVD	R3, FIXED_STACK+8(R1);			\
	MOVD	R4, FIXED_STACK+16(R1);			\
	MOVD	R6, FIXED_STACK+24(R1);			\
...
TEXT runtime·cgocallback(SB),NOSPLIT,$24-24
	MOVD	$fn+0(FP), R3
	MOVD	R3, FIXED_STACK+0(R1)
	MOVD	frame+8(FP), R3
	MOVD	R3, FIXED_STACK+8(R1)
	MOVD	framesize+16(FP), R3
	MOVD	R3, FIXED_STACK+16(R1)
...
havem:
	...
	MOVD	R5, -(FIXED_STACK+16)(R4)
	MOVD	$-(FIXED_STACK+16)(R4), R1
	BL	runtime·cgocallbackg(SB)

	// Restore g->sched (== m->curg->sched) from saved values.
	MOVD	0(R1), R5
	MOVD	R5, (g_sched+gobuf_pc)(g)
	MOVD	$(FIXED_STACK+16)(R1), R4
...
TEXT runtime·getcallerpc(SB),NOSPLIT,$8-16
	MOVD	FIXED_STACK+8(R1), R3		// LR saved by caller
	MOVD	runtime·stackBarrierPC(SB), R4
	CMP	R3, R4
	BNE	nobar
	// Get original return PC.
	BL	runtime·nextBarrierPC(SB)
	MOVD	FIXED_STACK+0(R1), R3
nobar:
	MOVD	R3, ret+8(FP)
	RET
...
TEXT runtime·setcallerpc(SB),NOSPLIT,$8-16
	MOVD	pc+8(FP), R3
	MOVD	FIXED_STACK+8(R1), R4
	MOVD	runtime·stackBarrierPC(SB), R5
	CMP	R4, R5
	BEQ	setbar
	MOVD	R3, FIXED_STACK+8(R1)		// set LR in caller
	RET
setbar:
