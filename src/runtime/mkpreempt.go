...
func genARM64() {
	...
	// and not saved here.
	var l = layout{sp: "RSP", stack: 8} // add LR slot
	for i := 0; i <= 26; i++ {
		if i == 18 {
	...
	p("MOVD %d(RSP), R30", l.stack) // sigctxt.pushCall pushes LR on stack, restore it
	p("#ifdef GOOS_linux")
...
func genMIPS(_64bit bool) {
	...
	var l = layout{sp: "R29", stack: regsize} // add LR slot
	for i := 1; i <= 25; i++ {
		if i == 23 {
	...
	p(mov+" %d(R29), R31", l.stack)     // sigctxt.pushCall pushes LR on stack, restore it
...
func genPPC64() {
	...
	p("MOVD LR, R31")
	p("MOVD R31, -%d(R1)", l.stack) // save LR
	p("ADD $-%d, R1", l.stack)      // allocate frame

	l.save()
	p("CALL ·asyncPreempt2(SB)")
	l.restore()

	p("MOVD %d(R1), R31", l.stack) // sigctxt.pushCall pushes LR, R2, R12 on stack, restore them
	p("MOVD R31, LR")
...
func genS390X() {
	// Add integer registers R0-R9, R11, R12
	// R10 (REGTMP), R13 (g), R14 (LR), R15 (SP) are special, and not saved here.
	var l = layout{sp: "R15", stack: 16} // add LR slot and flags slot
	for i := 0; i <= 12; i++ {
		if i == 10 {
			continue // R10 is REGTMP
		}
		reg := fmt.Sprintf("R%d", i)
		l.add("MOVD", reg, 8)
	}
	...
	// allocate frame, save LR and flags (condition code)
	p("IPM R10") // save flags upfront, as ADD will clobber flags
	p("MOVD R14, -%d(R15)", l.stack)
	p("ADD $-%d, R15", l.stack)
	p("MOVD R10, 8(R15)") // save flags

	l.save()
	p("CALL ·asyncPreempt2(SB)")
	l.restore()

	p("MOVD %d(R15), R14", l.stack)    // sigctxt.pushCall pushes LR on stack, restore it
	p("ADD $%d, R15", l.stack+8)       // pop frame (including the space pushed by sigctxt.pushCall)
	p("MOVD -%d(R15), R10", l.stack)   // load flags to REGTMP
	p("SPM R10")                       // restore flags
