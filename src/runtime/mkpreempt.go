...
func genARM64() {
	...
	p("#endif")
	// On darwin, save the LR after decrementing SP. We run the signal
	// handler on the G stack (as it doesn't support SA_ONSTACK), so
	// any writes below SP may be clobbered.
	p("#ifdef GOOS_darwin")
	p("MOVD R30, (RSP)")
	p("#endif")
