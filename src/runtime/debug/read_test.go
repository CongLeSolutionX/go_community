...
// TestReadBuildInfoFromFile confirms that ReadBuildInfoFromFile can read
// build information from binaries on supported target platforms. It builds
// a trivial binary on the current platforms (or all platforms if -all is set)
// in various configurations and checks that build information can or cannot
// be read.
func TestReadBuildInfoFromFile(t *testing.T) {
	if testing.Short() {
		...
		t.Run(p.goos+"_"+p.goarch, func(t *testing.T) {
			for _, tc := range cases {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					if info, err := debug.ReadBuildInfoFromFile(name); err != nil {
						if tc.wantErr == "" {
