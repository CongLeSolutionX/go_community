...
// Example_monitor shows an example of using debug.SetCrashOutput to
// direct crashes to a "monitor" process, for automated crash
// reporting. The monitor is the same executable, invoked in a
// special mode indicated by an environment variable.
func Example_monitor() {
	...
	// commented out.
	//
	// To observe the monitor in action, uncomment the Output
	// comment below and run this command:
	//
	//    $ go test -run=Example_monitor runtime/debug
	//    panic: oops
	//    ...stack...
...
func monitor() {
	...
	cmd := exec.Command(exe, "-test.run=Example_monitor")
