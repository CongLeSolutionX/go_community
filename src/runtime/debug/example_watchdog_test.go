...
// reporting. The watchdog is the same executable, invoked in a
...
// return.
func watchdog() {
	const watchdogVar = "WATCHDOG"
	if os.Getenv(watchdogVar) != "" {
	...
	// Fork+exec the same executable in watchdog mode.
	exe, err := os.Executable()
	if err != nil {
		log.Fatal(err)
	}
	cmd := exec.Command(exe, "-test.run=Example_watchdog")
	cmd.Env = append(os.Environ(), watchdogVar+"=1")
