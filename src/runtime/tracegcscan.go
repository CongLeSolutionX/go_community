		...
		if s.spanclass.noscan() {
			...
			for i := uintptr(0); i < uintptr(s.nelems); i++ {
				if abits.index < uintptr(s.freeindex) || abits.isMarked() {
					...
					// It's possible for hdr to be nil if this is a large object
					// using delayed zeroing and we preempted between allocation
					// and zeroing.
					id := theTypeBitsTable.put(hdr)
					if hdr == nil {
						if s.spanclass.sizeclass() != 0 {
							throw("nil header on non-large span")
						}
					} else if id == 0 {
...
func (t *traceTypeBitsTable) dump(w traceWriter, gen uint32) {
	...
	for id, typ := range t.tab.all {
		...
		for maskOffset := 0; maskOffset < len(ptrMask); maskOffset += ptrMaskBlock {
			...
			var flushed bool
			w, flushed = w.ensure(4*traceBytesPerNumber + 8*len(ptrMaskPart))
			if flushed {
				// Write the header
