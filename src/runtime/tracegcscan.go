...
func (tl traceLocker) GCScanGCStart() {
	...
	for cls := range class_to_size {
		w.varint(uint64(class_to_size[cls]))
	...
	tl.eventWriter(traceGoRunning, traceProcRunning).commit(traceEvGCScanGCStart, traceArg(memstats.numgc), traceArg(gomaxprocs))
}
