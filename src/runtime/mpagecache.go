...
func (c *pageCache) flush(s *pageAlloc) {
	...
	ci := arenaChunkIndex(c.base)
	pi := chunkPageIndex(c.base)
	for i := 0; i < 64; i++ {
		if c.cache&(1<<i) != 0 {
			a.pageAlloc[ci].free1(pi + i)
			if c.scav&(1<<i) != 0 {
				a.pageAlloc[ci].scavengeRange(pi+i, 1)
			}
		}
	}
...
nextLevel:
	...
	j, c, scav := s.arenas(ai).pageAlloc[i%mallocChunksPerArena].allocToCache(0)
	if j < 0 {
		throw("bad summary data")
	}
...
func (s *pageAlloc) allocToCache() pageCache {
	...
	if s.summary[len(s.summary)-1][ci] != 0 {
		...
		j, cv, scav := s.arenas(ai).pageAlloc[ci%mallocChunksPerArena].allocToCache(chunkPageIndex(s.searchAddr))
		if j < 0 {
			throw("bad summary data")
		}
