...
func (c *pageCache) flush(s *pageAlloc) {
	...
	// we update the hint.
	if c.base+arenaBaseOffset < s.hint+arenaBaseOffset {
		s.hint = c.base
	}
...
func (m *mallocData) allocToCache(hint int) (int, uint64, uint64) {
	base, cache := m.mallocBits.allocToCache(hint)
	if base < 0 {
...
func (s *pageAlloc) allocToCache() pageCache {
	...
	c := pageCache{}
	ci := chunkIndex(s.hint) // chunk index
	if s.summary[len(s.summary)-1][ci] != 0 {
		// Fast path: there's free pages at or near the hint address.
		ai := arenaIndex(s.hint)
		j, cv, scav := s.arenas(ai).pageAlloc.allocToCache(arenaPageIndex(s.hint))
		if j < 0 {
			throw("bad summary data")
		}
		addr := arenaBase(ai) + uintptr(j)*pageSize
	...
	s.hint = c.base + pageSize*pageCacheSize
	return c
}
