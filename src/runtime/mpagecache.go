...
func (c *pageCache) flush(s *pageAlloc) {
	...
	s.update(c.base, pageCacheSize, false)
	*c = pageCache{}
}
...
func (m *mallocData) allocToCache(hint int) (int, uint64, uint64) {
	...
	scav := m.scavenged.load64(base) & cache
...
nextLevel:
	...
	j, c, scav := s.arenas(arenaIdx(i)).pageAlloc.allocToCache(0)
	if j < 0 {
		throw("bad summary data")
	}
...
func (s *pageAlloc) allocToCache() pageCache {
	...
	if s.summary[len(s.summary)-1][i] != 0 {
		...
		j, cv, scav := s.arenas(i).pageAlloc.allocToCache(arenaPageIndex(s.hint))
		if j < 0 {
			throw("bad summary data")
		}
