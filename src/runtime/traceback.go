...
func gentraceback(pc0 uintptr, sp0 uintptr, lr0 uintptr, gp *g, skip int, pcbuf *uintptr, max int, callback func(*stkframe, unsafe.Pointer) bool, v unsafe.Pointer, flags uint) int {
	...
	for _defer != nil && uintptr(_defer.frame) == _NoArgs {
		_defer = _defer.link
	}
	...
	for n < max {
		...
		frame.continpc = frame.pc
		if waspanic {
			if _defer != nil && _defer.frame == frame.sp {
		...
		for _defer != nil && (_defer.frame == frame.sp || _defer.frame == _NoArgs) {
			_defer = _defer.link
		}
	...
	if callback != nil && n < max && _defer != nil {
		if _defer != nil {
			print("runtime: g", gp.goid, ": leftover defer frame=", hex(_defer.frame), " pc=", hex(_defer.pc), "\n")
		}
		for _defer = gp._defer; _defer != nil; _defer = _defer.link {
			print("\tdefer ", _defer, " frame=", hex(_defer.frame), " pc=", hex(_defer.pc), "\n")
		}
