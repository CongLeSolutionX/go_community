...
func gentraceback(pc0, sp0, lr0 uintptr, gp *g, skip int, pcbuf *uintptr, max int, callback func(*stkframe, unsafe.Pointer) bool, v unsafe.Pointer, flags uint) int {
	...
	n, _, _ := ggentraceback(pc0, sp0, lr0, gp, skip, pcbuf, max, op, callback, v, flags)
	return n
}
...
// n always returns the number of total frames < max. nregularframes and nruntimeframes are only valid if op == countingframes.
func ggentraceback(pc0, sp0, lr0 uintptr, gp *g, skip int, pcbuf *uintptr, max int, op operation, callback func(*stkframe, unsafe.Pointer) bool, v unsafe.Pointer, flags uint) (ntotalframes, nregularframes, nruntimeframes int) {
...
func countframes(pc0, sp0, lr0 uintptr, gp *g, flags uint) (ntotalframes, nregularframes, nruntimeframes int) {
	return ggentraceback(pc0, sp0, lr0, gp, 0, nil, 1<<31-1, countingframes, nil, nil, flags)
}

func printtraceback(pc, sp, lr uintptr, gp *g, flags uint) {
	printtraceback1(pc, sp, lr, gp, flags, 0)
}

func printtraceback1(pc, sp, lr uintptr, gp *g, flags uint, nrecursions int) {
	// We'd like to print:
	...
	if nTop < nMaxFramesPerPrint {
		...
		if nTop == 0 && (flags&_TraceRuntimeFrames) == 0 && nrecursions <= 2 {
			// Try again to print the frames, but this time with _TraceRuntimeFrames.
			// Increase nrecursions to prevent infinite recursions.
			printtraceback1(pc, sp, lr, gp, flags|_TraceRuntimeFrames, nrecursions+1)
		}
		return
	}
	...
	totalframes, nregularframes, _ := countframes(pc, sp, lr, gp, flags)

	if nregularframes <= _TracebackMaxFrames {
		// In this case, we'll just print out from where we left off until the end.
		n := gentraceback(pc, sp, lr, gp, nMaxFramesPerPrint /* skip */, nil, 1<<31-1, nil, nil, flags)
		if n == 0 && (flags&_TraceRuntimeFrames) == 0 {
			_ = gentraceback(pc, sp, lr, gp, nMaxFramesPerPrint /* skip */, nil, 1<<31-1, nil, nil, flags|_TraceRuntimeFrames)
		}
		return
	}
	...
	if elide := totalframes - _TracebackMaxFrames; elide > 0 {
		print("\n... (")
		println(elide, "stack frames omitted)\n")
	}
	skip := totalframes - nMaxFramesPerPrint
