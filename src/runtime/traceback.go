...
type unwinder struct {
	...
	// cgoCtxt is the index into g.cgoCtxt of the next cgo frame.
	cgoCtxt int
...
func tracebackPCs(u *unwinder, skip int, pcBuf []uintptr) int {
	...
	for ; n < len(pcBuf) && u.valid(); u.next() {
		...
		// Ideally we would pass &u.cache for the cache, but this understandably
		// causes u to escape.
		//
		// TODO: Why does passing &cache, where cache is local, cause cache to
		// escape to the heap?
		for iu := newInlineUnwinder(f, u.symPC(), nil); n < len(pcBuf) && iu.valid(); iu.next() {
			sf := iu.srcFunc()
...
func traceback2(u *unwinder, showRuntime bool) int {
	...
	for ; n < max && u.valid(); u.next() {
		f := u.frame.fn
		for iu := newInlineUnwinder(f, u.symPC(), nil); n < max && iu.valid(); iu.next() {
			sf := iu.srcFunc()
			if !(showRuntime || showframe(sf, gp, n == 0, u.callerFuncID)) {
				continue
			}
...
func showframe(sf srcFunc, gp *g, firstFrame bool, callerID funcID) bool {
	mp := getg().m
	if mp.throwing >= throwTypeRuntime && gp != nil && (gp == mp.curg || gp == mp.caughtsig.ptr()) {
		return true
	}
	return showfuncinfo(sf, firstFrame, callerID)
}

// showfuncinfo reports whether a function with the given characteristics should
// be printed during a traceback.
func showfuncinfo(sf srcFunc, firstFrame bool, callerID funcID) bool {
	level, _, _ := gotraceback()
	if level > 1 {
		// Show all frames.
		return true
	...
	if sf.funcID == funcID_wrapper && elideWrapperCalling(callerID) {
		return false
	}
