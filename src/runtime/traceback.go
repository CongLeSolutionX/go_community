...
func tracebackPCs(u *unwinder, skip int, pcBuf []uintptr) int {
	...
	for ; n < len(pcBuf) && u.valid(); u.next() {
		...
		for iu := newInlineUnwinder(f, u.symPC(), noEscapePtr(&u.cache)); n < len(pcBuf) && iu.valid(); iu.next() {
			sf := iu.srcFunc()
...
func traceback2(u *unwinder, showRuntime bool) int {
	...
	for ; n < max && u.valid(); u.next() {
		f := u.frame.fn
		for iu := newInlineUnwinder(f, u.symPC(), noEscapePtr(&u.cache)); n < max && iu.valid(); iu.next() {
			sf := iu.srcFunc()
