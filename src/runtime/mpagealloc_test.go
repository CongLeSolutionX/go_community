...
func TestPageAllocGrow(t *testing.T) {
	...
	tests := map[string]test{
		"One": {
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+1, 0) - 1},
			},
		},
		"Contiguous2": {
			chunks: []ChunkIdx{
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+2, 0) - 1},
			},
		},
		"Contiguous5": {
			chunks: []ChunkIdx{
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+5, 0) - 1},
			},
		},
		"Discontiguous": {
			chunks: []ChunkIdx{
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+1, 0) - 1},
				{PageBase(BaseChunkIdx+2, 0), PageBase(BaseChunkIdx+3, 0) - 1},
				{PageBase(BaseChunkIdx+4, 0), PageBase(BaseChunkIdx+5, 0) - 1},
			},
		},
		"Mixed": {
			chunks: []ChunkIdx{
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+3, 0) - 1},
				{PageBase(BaseChunkIdx+4, 0), PageBase(BaseChunkIdx+5, 0) - 1},
			},
		},
		"WildlyDiscontiguous": {
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+2, 0) - 1},
				{PageBase(BaseChunkIdx+0x10, 0), PageBase(BaseChunkIdx+0x11, 0) - 1},
				{PageBase(BaseChunkIdx+0x21, 0), PageBase(BaseChunkIdx+0x22, 0) - 1},
			},
		},
		"ManyDiscontiguous": {
			...
			inUse: []AddrRange{
				...
				{PageBase(BaseChunkIdx+2, 0), PageBase(BaseChunkIdx+3, 0) - 1},
				{PageBase(BaseChunkIdx+4, 0), PageBase(BaseChunkIdx+5, 0) - 1},
				{PageBase(BaseChunkIdx+6, 0), PageBase(BaseChunkIdx+7, 0) - 1},
				{PageBase(BaseChunkIdx+8, 0), PageBase(BaseChunkIdx+9, 0) - 1},
				{PageBase(BaseChunkIdx+10, 0), PageBase(BaseChunkIdx+11, 0) - 1},
				{PageBase(BaseChunkIdx+12, 0), PageBase(BaseChunkIdx+13, 0) - 1},
				{PageBase(BaseChunkIdx+14, 0), PageBase(BaseChunkIdx+15, 0) - 1},
				{PageBase(BaseChunkIdx+16, 0), PageBase(BaseChunkIdx+17, 0) - 1},
				{PageBase(BaseChunkIdx+18, 0), PageBase(BaseChunkIdx+19, 0) - 1},
				{PageBase(BaseChunkIdx+20, 0), PageBase(BaseChunkIdx+21, 0) - 1},
				{PageBase(BaseChunkIdx+22, 0), PageBase(BaseChunkIdx+23, 0) - 1},
				{PageBase(BaseChunkIdx+24, 0), PageBase(BaseChunkIdx+25, 0) - 1},
				{PageBase(BaseChunkIdx+26, 0), PageBase(BaseChunkIdx+27, 0) - 1},
				{PageBase(BaseChunkIdx+28, 0), PageBase(BaseChunkIdx+29, 0) - 1},
				{PageBase(BaseChunkIdx+30, 0), PageBase(BaseChunkIdx+31, 0) - 1},
				{PageBase(BaseChunkIdx+32, 0), PageBase(BaseChunkIdx+33, 0) - 1},
				{PageBase(BaseChunkIdx+34, 0), PageBase(BaseChunkIdx+35, 0) - 1},
				{PageBase(BaseChunkIdx+36, 0), PageBase(BaseChunkIdx+37, 0) - 1},
				{PageBase(BaseChunkIdx+38, 0), PageBase(BaseChunkIdx+39, 0) - 1},
				{PageBase(BaseChunkIdx+40, 0), PageBase(BaseChunkIdx+41, 0) - 1},
				{PageBase(BaseChunkIdx+42, 0), PageBase(BaseChunkIdx+43, 0) - 1},
				{PageBase(BaseChunkIdx+44, 0), PageBase(BaseChunkIdx+45, 0) - 1},
				{PageBase(BaseChunkIdx+46, 0), PageBase(BaseChunkIdx+47, 0) - 1},
				{PageBase(BaseChunkIdx+48, 0), PageBase(BaseChunkIdx+49, 0) - 1},
				{PageBase(BaseChunkIdx+50, 0), PageBase(BaseChunkIdx+51, 0) - 1},
				{PageBase(BaseChunkIdx+52, 0), PageBase(BaseChunkIdx+53, 0) - 1},
				{PageBase(BaseChunkIdx+54, 0), PageBase(BaseChunkIdx+55, 0) - 1},
				{PageBase(BaseChunkIdx+56, 0), PageBase(BaseChunkIdx+57, 0) - 1},
				{PageBase(BaseChunkIdx+58, 0), PageBase(BaseChunkIdx+59, 0) - 1},
				{PageBase(BaseChunkIdx+60, 0), PageBase(BaseChunkIdx+61, 0) - 1},
				{PageBase(BaseChunkIdx+62, 0), PageBase(BaseChunkIdx+63, 0) - 1},
				{PageBase(BaseChunkIdx+64, 0), PageBase(BaseChunkIdx+65, 0) - 1},
			},
		},
	}
	if PageAlloc64Bit != 0 {
		tests["ExtremelyDiscontiguous"] = test{
			...
			inUse: []AddrRange{
				{PageBase(BaseChunkIdx, 0), PageBase(BaseChunkIdx+1, 0) - 1},
				{PageBase(BaseChunkIdx+0x100000, 0), PageBase(BaseChunkIdx+0x100001, 0) - 1},
			},
		}
	} else {
		lastChunkIdx := ChunkIndex(^uintptr(0) - PallocChunkPages*PageSize + 1)
		tests["TopOfAddressSpace"] = test{
			chunks: []ChunkIdx{
				lastChunkIdx,
			},
			inUse: []AddrRange{
				{PageBase(lastChunkIdx, 0), PageBase(lastChunkIdx+1, 0) - 1},
			},
		}
	}
...
func TestPageAllocAlloc(t *testing.T) {
	...
	if PageAlloc64Bit != 0 {
		...
		lastChunkIdx := ChunkIndex(^uintptr(0) - PallocChunkPages*PageSize + 1)
		tests["TopOfAddressSpace1"] = test{
			before: map[ChunkIdx][]BitRange{
				lastChunkIdx: {{0, PallocChunkPages - 3}},
			},
			scav: map[ChunkIdx][]BitRange{
				lastChunkIdx: {{PallocChunkPages - 1, 1}},
			},
			hits: []hit{
				{1, PageBase(lastChunkIdx, PallocChunkPages-3), 0},
				{1, PageBase(lastChunkIdx, PallocChunkPages-2), 0},
				{1, PageBase(lastChunkIdx, PallocChunkPages-1), PageSize},
				{1, 0, 0},
			},
			after: map[ChunkIdx][]BitRange{
				lastChunkIdx: {{0, PallocChunkPages}},
			},
		}
		tests["TopOfAddressSpacePallocChunkPages"] = test{
			before: map[ChunkIdx][]BitRange{
				lastChunkIdx: {},
			},
			scav: map[ChunkIdx][]BitRange{
			...
			hits: []hit{
				{PallocChunkPages, PageBase(lastChunkIdx, 0), PageSize},
				{1, 0, 0},
			},
			after: map[ChunkIdx][]BitRange{
				lastChunkIdx: {{0, PallocChunkPages}},
