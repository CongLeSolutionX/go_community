...
func TestScavenger(t *testing.T) {
	...
	totalSlept := new(atomic.Int64)
	totalWorked := new(atomic.Int64)
	availableWork := new(atomic.Uint64)
	stopAt := new(atomic.Uint64) // How much available work to stop at.
	...
	verifyScavengerState := func(t *testing.T, expWork uint64) {
		...
		if workDone := uint64(s.Released()); workDone != expWork {
			t.Errorf("expected %d bytes of work done, got %d", expWork, workDone)
		}
		...
		if cpuFraction < idealFraction-0.005 || cpuFraction > idealFraction+0.005 {
			t.Errorf("expected %f CPU fraction, got %f", idealFraction, cpuFraction)
		}
	}
	...
	// This time, let's stop after a certian amount of work.
	//
