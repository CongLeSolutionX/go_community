...
func TestFillAligned(t *testing.T) {
	...
	for m := uint(1); m <= 64; m *= 2 {
		check(0x0000000000000000, m)
		check(0xffffffffffffffff, m)
		check(0x0000000000000001, m)
		check(0x0000000000000002, m)
		check(0x0000000000000008, m)
		check(uint64(1)<<(m-1), m)
		check(uint64(1)<<m, m)
		for i := 0; i < 1000; i++ {
			check(rand.Uint64(), m)
		}
