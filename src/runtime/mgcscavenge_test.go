...
func TestMallocDataHasScavengeCandidate(t *testing.T) {
	tests := map[string]struct {
		...
		defaultHasCandidate bool
		exceptChunks        []int
	}{
		"AllFree": {
			defaultHasCandidate: true,
			exceptChunks:        []int{},
		},
		"AllScavenged": {
			scavenged:           []BitRange{{0, PagesPerArena}},
			defaultHasCandidate: false,
			exceptChunks:        []int{},
		},
		"NoneFree": {
		...
		"EndFree": {
			...
			defaultHasCandidate: false,
			exceptChunks:        []int{MallocChunksPerArena - 1},
		},
	}
	for name, v := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			for j := 0; j < MallocChunksPerArena; j++ {
				expect := v.defaultHasCandidate
				for _, c := range v.exceptChunks {
					if j == c {
						expect = !expect
						break
					}
				}
				if has := b.HasScavengeCandidate(j); has != expect {
					t.Fatalf("hasCandidate mismatch: got %t, want %t", has, expect)
				}
			}
		})
	}
...
func TestMallocDataFindScavengeCandidate(t *testing.T) {
	...
	if PhysHugePageSize > uintptr(PageSize) {
		...
		tests["PreserveHugePageMiddle"] = test{
			alloc: []BitRange{{0, bits - 10}, {2*bits + 10, PagesPerArena - (2*bits + 10)}},
		...
		tests["PreserveHugePageTop"] = test{
			alloc: []BitRange{{0, PagesPerArena - bits}},
			max:   1, // Even one page would break a huge page in this case.
			hit:   BitRange{PagesPerArena - bits, bits},
		}
	}
	for name, v := range tests {
		v := v
