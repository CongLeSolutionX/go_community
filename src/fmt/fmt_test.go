...
func TestParsenum(t *testing.T) {
	...
	for i, params := range testCases {
		...
		if num != params.Expect.Num {
			t.Errorf("%d: num: got %d, expected %d", i, num, params.Expect.Num)
		}
		if isnum != params.Expect.Isnum {
			t.Errorf("%d: isnum: got %t, expected %t", i, isnum, params.Expect.Isnum)
		}
		if newi != params.Expect.Newi {
			t.Errorf("%d: newi: got %d, expected %d", i, newi, params.Expect.Newi)
		}
	}
}
