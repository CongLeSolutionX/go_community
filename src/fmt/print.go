...
func (p *pp) fmtInteger(v uint64, signedness bool, verb rune) {
	switch verb {
	case 'v':
		if p.fmt.sharpV && signedness == unsigned {
			p.fmt0x64(v, true)
		} else {
			p.fmt.fmt_integer(v, 10, signedness, ldigits)
		}
	case 'd':
		p.fmt.fmt_integer(v, 10, signedness, ldigits)
	case 'b':
		p.fmt.fmt_integer(v, 2, signedness, ldigits)
	case 'o':
		p.fmt.fmt_integer(v, 8, signedness, ldigits)
	case 'x':
		p.fmt.fmt_integer(v, 16, signedness, ldigits)
	case 'X':
		p.fmt.fmt_integer(v, 16, signedness, udigits)
	case 'c':
		p.fmt.fmt_c(v)
	case 'q':
		if v <= utf8.MaxRune {
