...
func TestWriterPaletted(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("%d", tc.plen), func(t *testing.T) {
			...
			for i < len(data)-chunkFieldsLength {
				...
				case "IDAT":
					...
					dl := 0
					var ignored [528]byte
					for {
						n, err := r.Read(ignored[:])
						dl += n
						if err != nil {
							break
						}
					...
					if dl != tc.datalen {
						t.Errorf("got uncompressed data length %d, want %d", dl, tc.datalen)
					}
				}
