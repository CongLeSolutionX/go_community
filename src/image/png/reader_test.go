...
func TestDimensionOverflow(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if tc.unsupportedConfig {
			if err == nil {
				t.Errorf("i=%d: DecodeConfig: got nil, want non-nil", i)
		...
		if nPixels := int64(cfg.Width) * int64(cfg.Height); nPixels > 0x7f000000 {
			...
			// these. See also http://golang.org/issue/5050 "decoding... images
			// can cause huge memory allocations".
			continue
		}

		if _, err := Decode(bytes.NewReader(tc.src)); err == nil {
			t.Errorf("i=%d: Decode: got nil, want non-nil", i)
		}
	}
