...
func YCbCrToRGB(y, cb, cr uint8) (uint8, uint8, uint8) {
	...
	// round-to-nearest when dividing by 65536, i.e. shifting right by 16.
	...
	//
	// As mentioned above, a constant adjustment of 1<<15 is a natural choice,
	// but there is an additional constraint: if c0 := YCbCr{Y: y, Cb: 0x80,
	// Cr: 0x80} and c1 := Gray{Y: y} then c0.RGBA() should equal c1.RGBA().
	// Specifically, if y == 0 then "R = etc >> 8" should yield 0x0000 and if y
	// == 0xff then "R = etc >> 8" should yield 0xffff. If we used a constant
	// rounding adjustment of 1<<15, then it would yield 0x0080 and 0xff80
	// respectively.
	//
