...
func DedupSorted(data Interface) (n int) {
	size := data.Len()
	neq := func(i int) bool { return data.Less(n, i) || data.Less(i, n) }
	eq := func(i int) bool { return !data.Less(i, n) && !data.Less(n, i) }
	var l, r int
	for i := 0; i < size; n++ {
		if i != n {
			data.Swap(i, n)
		...
		l++
		i = l + Search(r-l, func(j int) bool {
			return neq(l + j)
		})
	}
	return n
