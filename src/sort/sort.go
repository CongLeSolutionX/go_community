...
// DedupSorted rearranges the pre-sorted underlying data structure
...
func DedupSorted(data Interface) (n int) {
	...
	for i := 0; i < size; n++ {
		...
		for off := 1; !data.Less(n, j); off *= 2 {
			i = j
			j = i + off
			if j >= size {
				if !data.Less(n, size-1) {
					return n + 1
				}
				j = size
				break
		...
		i++
		i += Search(j-i, func(k int) bool {
			return data.Less(n, k+i)
		})
	}
	return n
}
