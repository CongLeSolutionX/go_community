...
// searchPivots sorts nine points and puts pivots to lo and hi-1
// returns true if chosen pivots are equal
func searchPivots(data Interface, lo, hi int) (equal bool) {
	...
	// sanity check against skewed distribution
	equal = true
	if !data.Less(ix[1], ix[4]) {
	} else if !data.Less(ix[0], ix[1]) {
		ix[4] = ix[0]
	} else if !data.Less(ix[4], ix[8]) {
		ix[1] = ix[8]
	} else {
		equal = false
	}
...
// quickSortBig does sort of big arrays using two-pivot algorithm
func quickSortBig(data Interface, a, b, maxDepth int) {
	for b-a > threshold {
		if maxDepth == 0 {
			heapSort(data, a, b)
			return
		...
		if !equal {
			if mlo-mhi > bigsize {
				quickSortBig(data, a, mlo, maxDepth)
				quickSortBig(data, mhi, b, maxDepth)
				a, b = mlo, mhi // i.e., quickSort(data, mlo, mhi)
				continue
			}
			quickSortBig(data, mlo, mhi, maxDepth)
		}
		if b-mhi == bigsize {
			quickSortBig(data, a, mlo, maxDepth)
...
func Sort(data Interface) {
	...
	maxDepth *= 2
	if n > threshold {
		quickSortBig(data, 0, n, maxDepth)
	} else {
