...
// This example shows off different capabilities of the Search function
// also at https://play.golang.org/p/eAj5gZs3QE
func Example_search() {
	a := []int{1, 3, 6, 10, 15, 21, 28, 36, 45, 55}
	x := 5

	// lower bound in go: the position of the smallest integer greater than or equal to an element
	...
	// upper bound in go: the position of the smallest integer greater than an element
	x = 6
	...
	// lower bound: the first number before a number definitely less than an element
	lb = sort.Search(len(a), func(i int) bool {
		return a[i] < x
	}) - 1
	...
	// upper bound: the first number before a number less than or equal to an element
	ub = sort.Search(len(a), func(i int) bool {
		return a[i] <= 6
	}) - 1
	...
	// as seen above, the return type of the Search functions in go is int not bool,
