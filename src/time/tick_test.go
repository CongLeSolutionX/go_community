...
func TestLongAdjustTimers(t *testing.T) {
	...
	// Give ourselves 15 seconds to complete.
	// This used to reliably fail on an x86 laptop.
	// This should take less than a second to run now.
	done := make(chan bool)
	AfterFunc(15*Second, func() { close(done) })
