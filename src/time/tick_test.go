...
func TestTicker(t *testing.T) {
	...
	if runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" {
		// In the following ticker will run count/2 and reset the ticker
		// with doubled the duration for the rest of count/2. Since tick
		// frequency is limited on Darwin ARM64, do not use odd number
		// to give the ticks more time to let the test pass. See CL 220638.
		count = 6
