...
func parseRFC3339(value string, local *Location, nanos bool) (Time, error) {
	...
	// Parse year.
	if len(value) < 4 || !isDigit(value, 0) {
		return Time{}, &ParseError{layout, avalue, "2006", value, ""}
	}
	...
	if len(value) < 1 || value[0] != 'T' {
		return Time{}, &ParseError{layout, avalue, "T", value, ""}
	}
	value = value[1:]

	// Parse hour.
	hour, value, err := getnum(value, true)
	if hour < 0 || 24 <= hour {
	...
	sign, hourStr, minuteStr, value := value[0:1], value[1:3], value[4:6], value[6:]
