...
// is valid (ok).
func readFloat(s string) (man uint64, exp int, neg, trunc, hex bool, i int, ok bool) {
	...
	lim := uint64(0x1999999999999999) // man*10+ 9 fits in uint64 before lim.
	if ui+2 < uint(len(s)) && s[ui] == '0' && lower(s[ui+1]) == 'x' {
		hex, lim = true, uint64(0x1000000000000000) // man*16+15 fits in uint64 before lim.
		ui += 2
	}
