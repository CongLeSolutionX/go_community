...
// Otherwise, if dst is not sufficiently large, encoder implementations may panic.
...
type prefixedBytesEncoder struct {
	c byte
	b []byte
}

func (p *prefixedBytesEncoder) Len() int {
	return len(p.b) + 1
}

func (p *prefixedBytesEncoder) Encode(dst []byte) {
	dst[0] = p.c
	if copy(dst[1:], p.b) != len(p.b) {
		panic("internal error")
	}
}
