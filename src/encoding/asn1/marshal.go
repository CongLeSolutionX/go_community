...
// encoder is a lazy-encodable chunk.
// Before calling e.Encode(dst), you must preallocate buffer like dst := make([]byte, e.Len()).
// Otherwise, it panics.
...
type stringsEncoder string

func (s stringsEncoder) Len() int {
	return len(s)
}

func (s stringsEncoder) Encode(dst []byte) {
	copy(dst, s)
}
...
type taggedEncoder struct {
	scratch [8]byte // scrach for tags
	tags    encoder
	body    encoder
}

func (t *taggedEncoder) Len() int {
	return t.tags.Len() + t.body.Len()
}

func (t *taggedEncoder) Encode(dst []byte) {
	t.tags.Encode(dst)
	t.body.Encode(dst[t.tags.Len():])
}
...
func makeBigInt(n *big.Int) encoder {
	// TODO it panics. see https://github.com/golang/go/issues/16603
	if n.Sign() < 0 {
		...
		if len(bytes) == 0 || bytes[0]&0x80 == 0 {
			return multiEncoder([]encoder{byteEncoder(0xff), bytesEncoder(bytes)})
		}
		return bytesEncoder(bytes)
	} else if n.Sign() == 0 {
		// Zero is written as a single 0 zero rather than no bytes.
		return byteEncoder(0x00)
	} else {
		bytes := n.Bytes()
		if len(bytes) > 0 && bytes[0]&0x80 != 0 {
			// We'll have to pad this with 0x00 in order to stop it
			// looking like a negative number.
			return multiEncoder([]encoder{byteEncoder(0x00), bytesEncoder(bytes)})
		}
		return bytesEncoder(bytes)
	}
}
...
func makeBitString(b BitString) encoder {
	paddingBits := byte((8 - b.BitLength%8) % 8)

	return multiEncoder([]encoder{byteEncoder(paddingBits), bytesEncoder(b.Bytes)})
}
...
func makeBody(value reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	case bitStringType:
		return makeBitString(value.Interface().(BitString)), nil
	...
	case reflect.Bool:
		if v.Bool() {
			return byteEncoder(0xff), nil
		}
		return byteEncoder(0x00), nil
...
func makeField(v reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	if v.Type() == rawValueType {
		...
		t.tags = bytesEncoder(appendTagAndLength(t.scratch[:0], tagAndLength{rv.Class, rv.Tag, len(rv.Bytes), rv.IsCompound}))
	...
	if params.explicit {
		t.tags = bytesEncoder(appendTagAndLength(t.scratch[:0], tagAndLength{class, tag, bodyLen, isCompound}))

		tt := new(taggedEncoder)

		tt.body = t

		tt.tags = bytesEncoder(appendTagAndLength(tt.scratch[:0], tagAndLength{
			class:      ClassContextSpecific,
			tag:        *params.tag,
			length:     bodyLen + t.tags.Len(),
			isCompound: true,
		}))
