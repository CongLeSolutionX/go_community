...
// option represents an extension-option that may be passed to NewEncoding().
type option struct {
	// hidden fields may be added later to support multiple options.
}

// Raw is an option to NewEncoding yielding an unpadded encoding,
// which omits trailing '=' pading characters.
var Raw = option{}

// NewEncoding returns a new padded Encoding defined by the given alphabet,
// which must be a 64-byte string.
func NewEncoding(encoder string, options ...option) *Encoding {
	e := new(Encoding)
	e.encode = encoder
	e.pad = true
	for i := range(options) {
		...
		case Raw:
			e.pad = false
		default:
			panic("unrecognized NewEncoding option")
		}
	}
...
var RawStdEncoding = NewEncoding(encodeStd, Raw)

// URLEncoding is the unpadded alternate base64 encoding defined in RFC 4648.
// It is typically used in URLs and file names.
// This is the same as URLEncoding but omits '=' padding characters.
var RawURLEncoding = NewEncoding(encodeURL, Raw)
