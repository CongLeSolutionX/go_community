...
// source buffer si.
// It returns the number of bytes read from src, the number of bytes writen
// to dst, an 'end' value, which indicates if end-of-message padding or a
// partial quantum was encountered and thus any additional data is an error,
// and an error, if any.
func (enc *Encoding) decodeQuantum(dst, src []byte, si int) (nsi, n int, end bool, err error) {
	...
	for j := 0; j < len(dbuf); j++ {
		if len(src) == si {
			...
			case j == 1, enc.padChar != NoPadding:
				return si, 0, false, CorruptInputError(si - j)
			}
		...
		case 2:
			...
			if si == len(src) {
				// not enough padding
				return si, 0, false, CorruptInputError(len(src))
			}
	...
	case 3:
		...
		if enc.strict && dbuf[2] != 0 {
			return si, 0, end, CorruptInputError(si - 1)
		}
		dbuf[1] = 0
		fallthrough
	case 2:
		...
		if enc.strict && (dbuf[1] != 0 || dbuf[2] != 0) {
			return si, 0, end, CorruptInputError(si - 2)
		}
	}
	dst = dst[dinc:]

	return si, dlen - 1, end, err
}
...
func (enc *Encoding) decode(dst, src []byte) (n int, end bool, err error) {
	...
	for strconv.IntSize >= 64 && ilen-si >= 8 && olen-n >= 8 {
		...
		si, ninc, end, err = enc.decodeQuantum(dst[n:], src, si)
		n += ninc
		if err != nil {
			return n, end, err
		}
...
func (enc *Encoding) decode32(dst, src []byte) bool {
	...
	if n = uint32(enc.decodeMap[(sn>>24)&0xff]); n > 63 {
		return false
	}
	dn |= n << 26
	if n = uint32(enc.decodeMap[(sn>>16)&0xff]); n > 63 {
		return false
	}
	dn |= n << 20
	if n = uint32(enc.decodeMap[(sn>>8)&0xff]); n > 63 {
		return false
	}
	dn |= n << 14
	if n = uint32(enc.decodeMap[(sn>>0)&0xff]); n > 63 {
		return false
	}
	dn |= n << 8
...
func (enc *Encoding) decode64(dst, src []byte) bool {
	...
	sn := binary.BigEndian.Uint64(src)

	if n = uint64(enc.decodeMap[(sn>>56)&0xff]); n > 63 {
		return false
	}
	dn |= n << 58
	if n = uint64(enc.decodeMap[(sn>>48)&0xff]); n > 63 {
		return false
	}
	dn |= n << 52
	if n = uint64(enc.decodeMap[(sn>>40)&0xff]); n > 63 {
		return false
	}
	dn |= n << 46
	if n = uint64(enc.decodeMap[(sn>>32)&0xff]); n > 63 {
		return false
	}
	dn |= n << 40
	if n = uint64(enc.decodeMap[(sn>>24)&0xff]); n > 63 {
		return false
	}
	dn |= n << 34
	if n = uint64(enc.decodeMap[(sn>>16)&0xff]); n > 63 {
		return false
	}
	dn |= n << 28
	if n = uint64(enc.decodeMap[(sn>>8)&0xff]); n > 63 {
		return false
	}
	dn |= n << 22
	if n = uint64(enc.decodeMap[(sn>>0)&0xff]); n > 63 {
		return false
	}
	dn |= n << 16
