...
func TestEncodedLen(t *testing.T) {
	...
	case 32:
		tests = append(tests, test{StdEncoding, (math.MaxInt32-5)/8 + 1, 357913942})
		tests = append(tests, test{StdEncoding, math.MaxInt32/4*3 + 2, 2147483647})
	case 64:
		tests = append(tests, test{StdEncoding, (math.MaxInt64-5)/8 + 1, 1537228672809129302})
		tests = append(tests, test{StdEncoding, math.MaxInt64/4*3 + 2, 9223372036854775807})
	}
...
func TestDecodedLen(t *testing.T) {
	...
	switch strconv.IntSize {
	case 32:
		tests = append(tests, test{StdEncoding, math.MaxInt32/6 + 1, 268435456})
		tests = append(tests, test{StdEncoding, math.MaxInt32, 1610612735})
	case 64:
		tests = append(tests, test{StdEncoding, math.MaxInt64/6 + 1, 1152921504606846976})
		tests = append(tests, test{StdEncoding, math.MaxInt64, 6917529027641081855})
	}
	for _, tt := range tests {
