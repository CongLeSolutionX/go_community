...
func TestByteOrder(t *testing.T) {
	...
	for _, order := range []byteOrder{LittleEndian, BigEndian} {
		for _, value := range []uint64{
			...
			buf = order.AppendUint16(buf[:3], want16)
			if got := order.Uint16(buf[3:]); got != want16 {
			...
			buf = order.AppendUint32(buf[:3], want32)
			if got := order.Uint32(buf[3:]); got != want32 {
			...
			buf = order.AppendUint64(buf[:3], want64)
			if got := order.Uint64(buf[3:]); got != want64 {
