...
func getTypeInfo(ut *userTypeInfo) (*typeInfo, error) {
	rt := ut.base
	if ut.externalEnc != 0 || isNamedPtrTypeTo(ut.user, rt) {
...
// Returns true only if the types are declared the following way:
// type sometype ...
// type sometypePtr *sometype
func isNamedPtrTypeTo(tPtr, t reflect.Type) bool {
	return len(tPtr.Name()) > 0 && t != tPtr && tPtr.Kind() == reflect.Ptr
}
