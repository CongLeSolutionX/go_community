...
var unmarshalTests = []unmarshalTest{
	...
	{in: `{"x": 1}`, ptr: new(tx), out: tx{}, allFields: boolPtr(false)},
	{in: `{"x": 1}`, ptr: new(tx), err: &UnmarshalUnknownFieldsError{Keys: []string{"x"}}, allFields: boolPtr(true)},
	...
	// Z has a "-" tag.
	{in: `{"Y": 1, "Z": 2}`, ptr: new(T), out: T{Y: 1}, allFields: boolPtr(false)},
	{in: `{"Y": 1, "Z": 2}`, ptr: new(T), err: &UnmarshalUnknownFieldsError{Keys: []string{"Z"}}, allFields: boolPtr(true)},

	{in: `{"alpha": "abc", "alphabet": "xyz"}`, ptr: new(U), out: U{Alphabet: "abc"}, allFields: boolPtr(false)},
	{in: `{"alpha": "abc"}`, ptr: new(U), out: U{Alphabet: "abc"}, allFields: boolPtr(false)},
	{in: `{"alphabet": "xyz"}`, ptr: new(U), out: U{}, allFields: boolPtr(false)},
	{in: `{"alpha": "abc", "alphabet": "xyz"}`, ptr: new(U), err: &UnmarshalUnknownFieldsError{Keys: []string{"alphabet"}}, allFields: boolPtr(true)},
	{in: `{"alpha": "abc"}`, ptr: new(U), out: U{Alphabet: "abc"}, allFields: boolPtr(true)},
	{in: `{"alphabet": "xyz"}`, ptr: new(U), err: &UnmarshalUnknownFieldsError{Keys: []string{"alphabet"}}, allFields: boolPtr(true)},

	// duplicate key names
	{in: `{"alpha": "abc", "ALPHA": "ijk", "ALPHA": "xyz"}`, ptr: new(U), out: U{Alphabet: "xyz"}, allFields: boolPtr(false)},
	{in: `{"alpha": "abc", "ALPHA": "ijk", "ALPHA": "xyz"}`, ptr: new(U), out: U{Alphabet: "xyz"}, err: &UnmarshalUnknownFieldsError{Keys: []string{"alpha", "ALPHA"}}, allFields: boolPtr(true)},
	...
	},
	{
		...
		ptr:       new(Top),
		allFields: boolPtr(true),
		err:       &UnmarshalUnknownFieldsError{Keys: []string{"x"}},
	},
	{
		in:  `{"hello": 1}`,
		ptr: new(Ambig),
...
func testUnmarshalDisallowUnknownFields(t *testing.T, allFields bool) {
	for i, tt := range unmarshalTests {
		var scan scanner
		if tt.allFields != nil && *tt.allFields != allFields {
			continue
		}
		in := []byte(tt.in)
		...
		if err := dec.Decode(v.Interface()); !reflect.DeepEqual(err, tt.err) {
			...
			t.Errorf("#%d DisallowUnknownFields() %s: %v, want %v", i, s, err, tt.err)
			continue
		} else if err != nil {
			continue
		}
...
func TestUnmarshal(t *testing.T) {
	testUnmarshalDisallowUnknownFields(t, true)
	testUnmarshalDisallowUnknownFields(t, false)
}
...
func TestUnexportedDisallowUnknownFields(t *testing.T) {
	input := `{"Name": "Bob", "m": {"x": 123}, "m2": {"y": 456}, "abcd": {"z": 789}}`
	want := &UnmarshalUnknownFieldsError{Keys: []string{"m", "m2", "abcd"}}
