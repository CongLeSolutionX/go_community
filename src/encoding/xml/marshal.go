...
func (p *printer) EncodeToken(t Token) error {
	...
	case RawXML:
		if p.TokenWriter != nil {
			p.TokenWriter.EncodeToken(RawXML(t))
		}
		p.Write(t)
	...
	if val.Kind() == reflect.Struct {
		...
		if err1 != nil {
			...
			if p.TokenWriter == nil {
				...
				buf := &bytes.Buffer{}
				EscapeText(buf, b)
				p.TokenWriter.EncodeToken(RawXML(buf.Bytes()))
			}
		} else {
			if p.TokenWriter == nil {
				p.EscapeString(s)
...
func (p *printer) marshalTextInterface(val encoding.TextMarshaler, start StartElement) error {
	...
	if p.TokenWriter != nil {
		buf := &bytes.Buffer{}
		EscapeText(buf, text)
		p.TokenWriter.EncodeToken(RawXML(text))
	} else {
		EscapeText(p, text)
	}
...
func (p *printer) marshalStruct(tinfo *typeInfo, val reflect.Value) error {
	...
	for i := range tinfo.fields {
		...
		case fCDATA, fCharData:
			...
			if p.TokenWriter != nil {
				err := p.TokenWriter.EncodeToken(RawXML(buf.Bytes()))
				if err != nil {
					return err
				}
			}
		...
		case fInnerXml:
			...
			case []byte:
				if p.TokenWriter == nil {
					...
					p.TokenWriter.EncodeToken(RawXML(raw))
				}
				continue
			case string:
				if p.TokenWriter == nil {
