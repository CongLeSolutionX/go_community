...
func TestIssue16497(t *testing.T) {
	...
	if resp.Type != respEmbed.IQ.Type {
		t.Errorf("%s %s", resp.IQ.Type, respEmbed.IQ.Type)
	}
	if resp.XMLName != respEmbed.IQ.XMLName {
		t.Skipf("Skipping as roundtrip inside interface is https://go.dev/issue/16497")
		t.Errorf("%s %s", resp.IQ.XMLName, respEmbed.IQ.XMLName)
	}
}
