...
func TestParseErrors(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.err == "" {
			if err != io.EOF {
				t.Errorf("parse %s: expected no errors, got %q", test.src, err.Error())
			}
			continue
		}
		if err == nil || err == io.EOF {
			t.Errorf("parse %s: expected an error, got none", test.src)
			continue
		}
		if !strings.Contains(err.Error(), test.err) {
			t.Errorf("parse %s: can't find %q error substring\nerror: %q", test.src, test.err, err.Error())
			continue
		}
	}
}
