...
func (fsys MapFS) Open(name string) (fs.File, error) {
	...
	realName := fsys.resolveSymlinks(name)
...
func (fsys MapFS) resolveSymlinks(name string) string {
...
func (fsys MapFS) lstat(name string) (*mapFileInfo, error) {
	...
	realDir := fsys.resolveSymlinks(path.Dir(name))
	if !fs.ValidPath(realDir) {
		return nil, fs.ErrNotExist
	}
	elem := path.Base(name)
	...
	if realName == "." {
		return &mapFileInfo{elem, &MapFile{Mode: fs.ModeDir}}, nil
	}
	// Maybe a directory.
	prefix := realName + "/"
