...
// StatLink returns a FileInfo describing the named file.
// If the file is a symbolic link, the returned FileInfo describes the symbolic link.
// StatLink makes no attempt to follow the link.
func (fsys MapFS) StatLink(name string) (fs.FileInfo, error) {
	info, err := fsys.lstat(name)
	if err != nil {
