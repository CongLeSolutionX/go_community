...
func testFS(fsys fs.FS, expected ...string) error {
	...
	return fsTesterError{t.errors}
}

// fsTesterError wraps the list of errors encountered while testing with TestFS.
type fsTesterError struct {
	errors []error
}

// Unwrap implements the same unwrapping as the error returned by errors.Join.
func (e fsTesterError) Unwrap() []error {
	return e.errors
}

func (e fsTesterError) Error() string {
	errText := make([]string, 1, 1+len(e.errors))
	errText[0] = "TestFS found errors:"
