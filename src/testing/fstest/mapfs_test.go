...
func TestMapFSSymlink(t *testing.T) {
	...
	if err := TestFS(m, "fortune/k/ken.txt", "dirlink", "dirlink/ken.txt", "ken.txt", "linklink"); err != nil {
		t.Error(err)
	}
	...
	gotInfo, err := fs.StatLink(m, "dirlink")
	if err != nil {
		t.Errorf("fs.StatLink(m, \"dirlink\") = _, %v; want _, <nil>", err)
	} else {
		if got, want := gotInfo.Name(), "dirlink"; got != want {
			t.Errorf("fs.StatLink(m, \"dirlink\").Name() = %q; want %q", got, want)
		}
		if got, want := gotInfo.Mode(), fs.ModeSymlink; got != want {
			t.Errorf("fs.StatLink(m, \"dirlink\").Mode() = %v; want %v", got, want)
		}
	}
	...
	gotInfo, err = fs.StatLink(m, "linklink")
	if err != nil {
		t.Errorf("fs.StatLink(m, \"linklink\") = _, %v; want _, <nil>", err)
	} else {
		if got, want := gotInfo.Name(), "linklink"; got != want {
			t.Errorf("fs.StatLink(m, \"linklink\").Name() = %q; want %q", got, want)
		}
		if got, want := gotInfo.Mode(), fs.ModeSymlink; got != want {
			t.Errorf("fs.StatLink(m, \"linklink\").Mode() = %v; want %v", got, want)
		}
	}
