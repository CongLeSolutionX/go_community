...
--- FAIL: skipping after error (0.00s)
	...
	for _, tc := range testCases {
		...
		got := sanatizeLog(buf.String())
		want := sanatizeLog(tc.output)
		if got != want {
			t.Errorf("%s:ouput:\ngot:\n%s\nwant:\n%s", tc.desc, got, want)
		}
	}
...
// sanatizeLog removes line numbers from log entries.
func sanatizeLog(s string) string {
