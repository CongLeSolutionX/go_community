...
// See NewConverter and markFraming in cmd/internal/test2json/test2json.go for details
const (
...
func (p *chattyPrinter) escape(w stringWriter) stringWriter {
	if p != nil && p.json {
...
func (w escapeWriter) Write(p []byte) (total int, err error) {
	marks := string([]byte{markFraming, markErrBegin, markErrEnd, markEscape})
	escaped := [2]byte{markEscape}
	for len(p) > 0 {
		// Are there any remaining escapes?
		var n int
		i := bytes.IndexAny(p, marks)
		if i < 0 {
...
func (c *common) decorate(s string, skip int, isErr bool) string {
	...
	buf.WriteString("    ")
	esc := c.chatty.escape(buf)
	fmt.Fprintf(esc, "%s:%d: ", file, line) // escaped
