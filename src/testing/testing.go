...
// Context returns a context for this test that is active until the
// test has either failed or completed. Goroutines started by a test
// can wait for the Context's Done channel to become readable as a
// signal that they might exit. The first call to Fail (including via
// calls such as Errorf or Fatalf) causes the context's Done channel
// to close.
