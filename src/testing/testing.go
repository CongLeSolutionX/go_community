...
// Context returns a context for this test that is done either at the
// first call to Fail (including via calls such as Errorf or Fatalf),
// or when the test function returns.
...
func runTests(matchString func(pat, str string) (bool, error), tests []InternalTest) (ran, ok bool) {
	...
	for _, procs := range cpuList {
		...
		tctx := newTestContext(*parallel, newMatcher(matchString, *match, "-test.run"))
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
