...
func (m *M) Run() int {
	...
	if len(*matchList) != 0 {
		results, err := listTests(m.deps.MatchString, m.tests, m.benchmarks, m.examples)
		if err != nil {
...
func listTests(matchString func(pat, str string) (bool, error), tests []InternalTest, benchmarks []InternalBenchmark, examples []InternalExample) ([]string, error) {
	...
	for _, test := range tests {
		if ok, _ := matchString(*matchList, test.Name); ok {
			results = append(results, test.Name)
		}
	}
	for _, bench := range benchmarks {
		if ok, _ := matchString(*matchList, bench.Name); ok {
			results = append(results, bench.Name)
			fmt.Println(bench.Name)
		}
	}
	for _, example := range examples {
		if ok, _ := matchString(*matchList, example.Name); ok {
			results = append(results, example.Name)
		}
	}
	return results, nil
}
