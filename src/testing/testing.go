...
// Printf prints a message generated by testName that does not necessarily
// mention testName itself.
...
func (c *common) flushToParent(testName, format string, args ...interface{}) {
	...
	if c.chatty != nil && p.w == c.chatty.w {
		// We're flushing to the actual output, so update the test name.
		c.chatty.Updatef(testName, format, args...)
	} else {
		// We're flushing to the output buffer of the parent test, which will
		// already follow a test-name header when it is finally flushed to stdout.
		fmt.Fprintf(p.w, format, args...)
	}
}
...
func (t *T) Parallel() {
	...
	if t.chatty != nil {
		// Print to the root writer to avoid extra indentation.
		//
...
func (t *T) Run(name string, f func(t *T)) bool {
	...
	if t.chatty != nil {
		// Print directly to stdout so there is no delay.
