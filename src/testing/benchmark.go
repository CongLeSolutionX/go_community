...
func (b *B) launch() {
	...
	timePerInterval := b.benchTime / time.Duration(intervals)
	// run1 already reached time limit.
	if b.duration >= timePerInterval && intervals == 1 {
		b.result = b.toResult()
		return
	}
	...
	for i := 0; i < intervals; i++ {
		prevN := 0
		for n := 1; n < 1e9; {
			n = predictN(prevN, ops, elapsed.Nanoseconds(), timePerInterval.Nanoseconds())
			b.runN(n)
			elapsed += b.duration
			ops += int64(b.N)
			if b.failed {
			...
			if b.duration >= timePerInterval || n >= 1e9 {
				intervalResults = append(intervalResults, b.toResult())
				break
			}
			prevN = n
...
func predictN(prevN int, opsCompleted, nsElapsed, nsPerInterval int64) int {
	n := nsPerInterval
	if nsElapsed > 0 {
		n *= opsCompleted
		n /= nsElapsed
...
func (ctx *benchContext) processBench(b *B) {
	for i, procs := range cpuList {
		...
		if len(r.split) == 0 {
			...
			fmt.Fprintln(b.w, r.String())
			if *benchmarkMemory || b.showAllocResult {
				fmt.Fprintln(b.w, "\t"+r.MemString())
			}
		}
