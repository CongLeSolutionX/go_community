...
// ReportMetric will panic if unit is the empty string or if unit
// contains any whitespace.
// If unit is a unit normally reported by the benchmark framework itself
// (such as "ns/op"), ReportMetric will override that metric.
// For built-in units, setting n to 0 will cause the metric not to be reported.
...
// golang.org/design/14313-benchmark-format, not including the
// benchmark name.
// It includes ns/op and MB/s metrics if they are non-zero
// and all metrics from Extra.
// Extra metrics always override built-in metrics with the same name.
// It does not include allocs/op or B/op, since those are reported by
// MemString.
...
func (r BenchmarkResult) MemString() string {
	buf := new(bytes.Buffer)
	// If user metrics are 0, we hide the built-in metrics.
	if v, ok := r.Extra["B/op"]; !ok || v != 0 {
		fmt.Fprintf(buf, "%8d B/op", r.AllocedBytesPerOp())
	}
	if v, ok := r.Extra["allocs/op"]; !ok || v != 0 {
		if buf.Len() != 0 {
			fmt.Fprintf(buf, "\t")
		}
		fmt.Fprintf(buf, "%8d allocs/op", r.AllocsPerOp())
	}
	return buf.String()
}
