...
func runFuzzTargets(deps testDeps, fuzzTargets []InternalFuzzTarget, deadline time.Time) (ran, ok bool) {
	...
	var tctxFuzz *testContext
	if *matchFuzz != "" {
		mFuzz := newMatcher(deps.MatchString, *matchFuzz, "-test.fuzz")
		tctxFuzz = newTestContext(*parallel, mFuzz)
		tctxFuzz.deadline = deadline
	}
	...
	for _, ft := range fuzzTargets {
		...
		if tctxFuzz != nil {
			if _, fuzzMatched, _ := tctxFuzz.match.fullName(nil, ft.Name); fuzzMatched {
