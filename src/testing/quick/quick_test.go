...
func TestValue(t *testing.T) {
	...
	for i := 0; i < *defaultMaxCount; i++ {
		...
		samples := map[reflect.Kind]*struct {
			...
			reflect.String:     {reflect.TypeOf(string("")), reflect.ValueOf(randString(fakeRand, complexSize))},
		}

		// Test of creating simple types
		for _, sample := range samples {
			if v, ok := Value(sample.t, fakeRand); ok {
		...
		if v, ok := sizedValue(reflect.TypeOf(filledStruct{}), fakeRand, 1000); ok {
