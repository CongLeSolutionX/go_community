...
func TestReader(t *testing.T) {
	...
	errFuncs := map[string]func(error) bool{
		"IsUnexpectedEOF": func(err error) bool {
			return err == io.ErrUnexpectedEOF
		},
		"IsCorrupted": func(err error) bool {
			_, ok := err.(CorruptInputError)
			return ok
		},
	}
	vectors := []struct {
		name string // Sub-test name
		in   []byte // Test input string
		want []byte // Expected output string
		...
		`),
		want: []byte("Hel\x90\x90\x90\x90\x90"),
		errf: "IsUnexpectedEOF",
	}, {
	...
	for _, v := range vectors {
		t.Run(v.name, func(t *testing.T) {
			rd := NewReader(bytes.NewReader(v.in))
