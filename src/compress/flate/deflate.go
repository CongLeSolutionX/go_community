...
func (d *compressor) writeBlock(tok tokens, index int) error {
	if index > 0 {
		var window []byte
		...
		d.blockStart = index
		d.w.writeBlock(tok.tokens[:tok.n], false, window)
		return d.w.err
	}
	return nil
}
...
// hash4 is the hash function used when no native crc32 calculation
