...
func TestStreams(t *testing.T) {
	...
	testCases := []struct {
		...
		expect string // Expected result. Use "fail" to expect failure
	}{
		{
	...
	for idx, tc := range testCases {
		...
		if tc.expect == "fail" { // Expect failure
			if err == nil {
				t.Errorf("Failed case %d - '%s': expected failure, but got success", idx, tc.desc)
			}
		} else { // Expect success
			if err != nil {
				t.Errorf("Failed case %d - '%s': %v", idx, tc.desc, err)
			}
			if out := hex.EncodeToString(data); out != tc.expect {
				t.Errorf("Failed case %d - '%s': expected '%s', but got '%s'", idx, tc.desc, tc.expect, out)
			}
