...
// This encoding algorithm, which prioritizes speed over encoding size,
// is based on Snappy LZ77 matcher: github.com/golang/snappy
...
// and a copy of the previous block.
type encState struct {
	// table contains a hash table with the position of the previous
	// 4 byte match with a given hash.
	table [tableSize]uint32

	// block is storage for the previous block.
	block [maxStoreBlockSize]byte

	// prev contains a slice of the previously encoded block.
	// If prev is nil, no matches to the previous block is allowed.
	prev []byte

	// cur is the current encoding offset, incremented on every block.
	// This allows us to reuse the hash table between blocks.
	// Whenever a block has been encoded the size of the block is added.
	// To avoid overflows on large encodes, we reset this offset when it
	// becomes too big for 32 bits systems.
	// The encoder must take into account that a reset might have occurred.
	cur int
}

// Encode encodes a block of bytes, and outputs a token
...
func (e *encState) encode(dst []token, src []byte) []token {
	...
	// Ensure that e.cur doesn't wrap, mainly an issue on 32 bit systems.
	if e.cur > 1<<30 {
		e.cur = 1
	}
...
// Reset resets the encoding state.
