...
func TestDecodeBitGen(t *testing.T) {
	var vectors = []struct {
		...
		in:    `<<< < 1011001110001`,
		want:  []byte{0x71, 0x16}, // 0b01110001 0b00010110
		valid: true,
	}, {
		in:    `<<< > 1011001110001`,
		want:  []byte{0xcd, 0x11}, // 0b11001101 0b00010001
		valid: true,
	}, {
		in:    `>>> < 1011001110001`,
		want:  []byte{0x8e, 0x68}, // 0b10001110 0b01101000
		valid: true,
	}, {
		in:    `>>> > 1011001110001`,
		want:  []byte{0xb3, 0x88}, // 0b10110011 0b10001000
		valid: true,
	}, {
		in:    `>>> > 1011001110001 <<<`,
		valid: false,
		...
		in:    `<<< < H12:0f1b H12:0f1b`,
		want:  []byte{0x1b, 0xbf, 0xf1},
		valid: true,
	}, {
		in:    `<<< < H12:0f1b H11:0f1b`,
		valid: false,
	}, {
		in: `>>>
		...
		in: `<<< < 1111111011011100101110101001100001110110010101000011001000010000`,
		want: []byte{
		...
		in:    `<<< < 11111111011011100101110101001100001110110010101000011001000010000`,
		valid: false,
	}, {
		in:    `>>> < D0:0`,
		valid: true,
	}, {
		in:    `>>> < D63:18364758544493064720`,
		valid: false,
	}, {
		in:    `>>> > D63:18364758544493064720`,
		valid: false,
	}, {
		in:    `<<< < 01101 D11:1337 > 1011`,
		want:  []byte{0x2d, 0xa7, 0x0d}, // 0b00101101 0b10100111 0b00001101
		valid: true,
	}, {
		in: `
			<<<   	 # Comment 10110 < > D3:3
		...
		in: `<<< < D12:1337*4 10101111*2 X:aBcD*3`,
		want: []byte{
		...
		in:    `<<< < D12:1337*0 10101111*0 X:aBcD*0`,
		valid: true,
	}, {
		in:    `<<< < D12:1337*9999999999999999999999999999999999999999999999`,
		valid: false,
	}, {
		in:    `<<< X:abcd <"The " "quick "*5 "brown \"fox\"\n\n \\njumped" >"" # HA`,
		want:  []byte("\xab\xcdThe quick quick quick quick quick brown \"fox\"\n\n \\njumped"),
		valid: true,
	}}

	for i, v := range vectors {
		got, err := DecodeBitGen(v.in)
		if (err == nil) != v.valid {
			if err != nil {
