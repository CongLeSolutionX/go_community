...
const defaultMaxFieldSize = 32 * (1 << 10)
...
// a value of zero configures Reader to use the default max size (32 KiB), and
// a negative value configures Reader to ignore these strings.
//
// The effect of MaxFieldSize only applies to the next call to Reset,
// after which the default value is used for subsequent calls to Reset.
// If a GZIP stream has a header with string fields longer than n,
// then Reset will return an error.
...
// If maxSize is negative, then Reader skips over the string.
...
func (z *Reader) readString(maxSize int) (string, error) {
	...
	for {
		...
		if b > 0 {
			buf = append(buf, b)
			if maxSize < 0 {
				// Update the CRC even when skipping the string.
				if len(buf) == cap(buf) {
					z.digest = crc32.Update(z.digest, crc32.IEEETable, buf)
				}
				buf = buf[:0]
				continue
			}
