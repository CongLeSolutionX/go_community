...
func TestLongHeaders(t *testing.T) {
	...
	for _, tt := range tests {
		...
		// Check that MaxFieldSize only affects the next call to Reset.
		if err := r.Reset(bytes.NewReader(fileEdge)); err != nil {
			t.Errorf("Reset(edge) = %v, want nil", err)
		}
		if err := r.Reset(bytes.NewReader(fileLong)); err != errHeaderSize {
			t.Errorf("Reset(long) = %v, want %v", err, errHeaderSize)
		}
	}
}
