...
func TestNoBoundary(t *testing.T) {
	...
	if err.Error() != "reader : boundary is empty" {
		t.Error("reader : empty boundary did not return an error")
	}
	if part != nil {
		t.Error("reader : empty boundary did not return nil for part value")
	}
}
