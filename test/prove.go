...
func mod64sPositiveWithSmallerDividendMax(a, b int64, ensureBothBranchesCouldHappen bool) int {
	if a < 0 || b < 0 {
		return 42
	}
	a &= 0xff
	...
	// see go.dev/issue/68857 for bits.Len64
	z := bits.Len64(uint64(a % b)) // ERROR "Proved Mod64 does not need fix-up$"
...
func mod64sPositiveWithSmallerDivisorMax(a, b int64, ensureBothBranchesCouldHappen bool) int {
	if a < 0 || b < 0 {
		return 42
	}
	a &= 0xfff
...
func mod64sPositiveWithIdenticalMax(a, b int64, ensureBothBranchesCouldHappen bool) int {
	if a < 0 || b < 0 {
		return 42
	}
	a &= 0x10
...
func div64u(a, b uint64, ensureBothBranchesCouldHappen bool) uint64 {
	a &= 0xffff
	a |= 0xfff
	b &= 0xff
	b |= 0xf
	...
	if ensureBothBranchesCouldHappen {
		if z > 0xffff/0xf { // ERROR "Disproved Less64U$"
			return 42
		}
	} else {
		if z < 0xfff/0xff { // ERROR "Disproved Less64U$"
			return 1337
		}
	}
	return z
}
func div64s(a, b int64, ensureBothBranchesCouldHappen bool) int64 {
	if a < 0 || b < 0 {
		return 42
	}
	a &= 0xffff
	...
	if ensureBothBranchesCouldHappen {
		if z > 0xffff/0xf { // ERROR "Disproved Less64$"
			return 42
		}
	} else {
		if z < 0xfff/0xff { // ERROR "Disproved Less64$"
			return 1337
		}
	}
	return z
