...
func f10(a string) int {
	n := len(a)
	if a[:n>>1] == "aaa" { // ERROR "Proved 0 <= non-negative IsSliceInBounds$"
		return 0
	}
	return 1
}
...
func f11c(a []int, i int) {
	useSlice(a[:i]) // ERROR "Proved 0 <= non-negative IsSliceInBounds$"
	useSlice(a[:i]) // ERROR "Proved boolean IsSliceInBounds$"
}
...
func f12(a []int, b int) {
	useSlice(a[:b]) // ERROR "Proved 0 <= non-negative IsSliceInBounds$"
}
