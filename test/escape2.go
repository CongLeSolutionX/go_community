...
func foo70(mv1 *MV, m2 M) { // ERROR "leaking param: m2$" "leaking param: mv1$"
	m2 = mv1
	foo64(m2)
}
...
func foo75(z *int) { // ERROR "foo75 z does not escape$"
	myprint(z, 1, 2, 3) // ERROR "foo75 ... argument does not escape$"
}

func foo75a(z *int) { // ERROR "foo75a z does not escape$"
	myprint1(z, 1, 2, 3) // ERROR "foo75a ... argument does not escape$"
}

func foo75esc(z *int) { // ERROR "leaking param: z$"
	gxx = myprint(z, 1, 2, 3) // ERROR "foo75esc ... argument does not escape$"
}
...
func foo76b() {
	myprint(nil, 1, 2, 3) // ERROR "foo76b ... argument does not escape$"
}

func foo76c() {
	myprint1(nil, 1, 2, 3) // ERROR "foo76c ... argument does not escape$"
}

func foo76d() {
	defer myprint(nil, 1, 2, 3) // ERROR "foo76d ... argument does not escape$"
}

func foo76e() {
	defer myprint1(nil, 1, 2, 3) // ERROR "foo76e ... argument does not escape$"
}
...
func makemap0() {
	m := make(map[int]int) // ERROR "makemap0 make\(map\[int\]int, 0\) does not escape$"
	m[0] = 0
	m[1]++
	delete(m, 1)
	sink = m[0]
