...
func foo3(x int) *int { // ERROR "leaking param: x to result ~r1 level=-1$" "moved to heap: x$"
...
func foo23c(x int) func() int { // ERROR "moved to heap: x$" "leaking param: x to result ~r1 level=-2"
	return func() int { // ERROR "func literal escapes to heap$"
		x++ // ERROR "&x escapes to heap$"
		return x
	}
...
func indaddr1(x int) *int { // ERROR "leaking param: x to result ~r1 level=-1$" "moved to heap: x$"
	return &x // ERROR "&x escapes to heap$"
}
...
// contrast with
func float64bitsptr(f float64) *uint64 { // ERROR "leaking param: f to result ~r1 level=-1$" "moved to heap: f$"
...
func foo78(z int) *int { // ERROR "leaking param: z to result ~r1 level=-1$" "moved to heap: z$"
	return &z // ERROR "&z escapes to heap$"
}

func foo78a(z int) *int { // ERROR "leaking param: z to result ~r1 level=-1$" "moved to heap: z$"
...
func NewV(u U) *V { // ERROR "moved to heap: u$" "leaking param: u to result ~r1 level=-1"
	return &V{u.String()} // ERROR "&V literal escapes to heap$" "u escapes to heap$"
}
