...
var tests = []struct {
	...
	{"make b", append([]struct{}{}, make([]struct{}, 2)...), make([]struct{}, 2)},

	{"make c", append([]int{0, 1}, make([]int, 0)...), []int{0, 1}},
	{"make d", append([]int{0, 1}, make([]int, 2)...), []int{0, 1, 0, 0}},

	{"make e", append([]*int{&zero, &one}, make([]*int, 0)...), []*int{&zero, &one}},
	{"make f", append([]*int{&zero, &one}, make([]*int, 2)...), []*int{&zero, &one, nil, nil}},
}
