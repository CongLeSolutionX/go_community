// test -v -gcflags=-l escape_struct_return.go
...
func TestCallInputFunctions(t *testing.T) {
	tests := []struct {
		...
		{name: "A A", f: func() { s := gs; ps := &s; s2 := gs; ps2 := &s2; pps2 := &ps2; A(ps, pps2) }},
		{name: "A B", f: func() { s := gs; ps := &s; s2 := gs; ps2 := &s2; pps2 := &ps2; esc(ps); A(ps, pps2) }},
		{name: "A C", f: func() { s := gs; ps := &s; s2 := gs; ps2 := &s2; pps2 := &ps2; esc(pps2); A(ps, pps2) }},
		{name: "A D", f: func() { s := gs; ps := &s; s2 := gs; ps2 := &s2; pps2 := &ps2; esc(ps); esc(pps2); A(ps, pps2) }},
		{name: "B A", f: func() { s := gs; ps := &s; pps := &ps; B(pps) }},
		{name: "B B", f: func() { s := gs; ps := &s; pps := &ps; esc(pps); B(pps) }},
		{name: "tA1", f: func() { tA1() }},
		{name: "tA2", f: func() { tA2() }},
		{name: "tA3", f: func() { tA3() }},
		{name: "tB1", f: func() { tB1() }},
