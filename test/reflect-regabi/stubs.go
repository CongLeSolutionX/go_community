...
// Struct3 is Struct2 but with an anonymous arry-typed field
// that will allow it to be passed in registers identically to
// Struct1.
type Struct3 struct {
	a, b, c uint
	_       [2]uint32
...
// Struct8 is large struct type that can still be passed
// entirely in registers, despite having an nested array-typed
// field because that field is anonymous.
type Struct8 struct {
	Struct5
	Struct3
}
...
// Struct10 is a struct type that can be passed
// in registers despite an anonymous array field,
// but has so many fields that it'll be spilled
// anyway.
