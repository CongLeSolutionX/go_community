...
func tbff2() *string { // "BAD": need fine-grained analysis to track u[0] and u[1] differently.
...
func fum(x *U, y **string) *string { // ERROR "leaking param: x to result ~r2 level=1$" "leaking param: y$"
	x[0] = *y
	return x[1]
}

func fuo(x *U, y *U) *string { // ERROR "leaking param: x to result ~r2 level=1$" "leaking param: y$"
	x[0] = y[0]
	return x[1]
}
