...
func (t *test) wantedAsmOpcodes(fn string) (map[string]map[string][]wantedAsmOpcode, []string) {
	...
	for i, line := range strings.Split(string(src), "\n") {
		...
		// we find a line contains some code.
		comment += " " + matches[2]
		if matches[1] == "" {
			continue
		}

		// Parse and extract any opcode architecture opcode checks
		// from comments.
		archchecks := rxAsmPlatform.FindAllStringSubmatch(comment, -1)
		...
		for _, ac := range archchecks {
			...
			for _, m := range rxAsmCheck.FindAllString(allchecks, -1) {
				...
				// consider checks as matching from the beginning of the line
				// to avoid trivial bugs such as "ADD" matching "FADD". This
