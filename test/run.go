...
var (
	...
	generics       = flag.String("generics", "both", "whether to run compiler tests with generics enabled")
...
func main() {
	...
	switch *generics {
	case "true", "false", "both":
		// ok
	default:
		log.Fatalf("unsupported -generics value; use 'true', 'false', or 'both'")
	}
	...
	if flag.NArg() > 0 {
		for _, arg := range flag.Args() {
			...
			if fi, err := os.Stat(arg); err == nil && fi.IsDir() {
				for _, baseGoFile := range goFiles(arg) {
					tests = append(tests, startTests(arg, baseGoFile)...)
				}
			} else if strings.HasSuffix(arg, ".go") {
				dir, file := filepath.Split(arg)
				tests = append(tests, startTests(dir, file)...)
			} else {
		...
		for _, dir := range dirs {
			for _, baseGoFile := range goFiles(dir) {
				tests = append(tests, startTests(dir, baseGoFile)...)
			}
		}
	}
	...
	for _, test := range tests {
		...
		if status == "FAIL" {
			fmt.Printf("# go run run.go -generics=%v %s\n%s\nFAIL\t%s\t%s\n",
				test.generics,
...
type test struct {
	...
	dt          time.Duration
	generics    bool // whether this individual test run should use generics
...
// startTests
func startTests(dir, gofile string) []*test {
	var tests []*test
	if *generics == "both" || *generics == "false" {
		tests = append(tests, startTest(dir, gofile, false))
	}
	if *generics == "both" || *generics == "true" {
		tests = append(tests, startTest(dir, gofile, true))
	}
	return tests
}

func startTest(dir, gofile string, generics bool) *test {
	t := &test{
		dir:      dir,
		gofile:   gofile,
		generics: generics,
...
func (t *test) goGcflagsIsEmpty() bool {
	return "" == os.Getenv("GO_GCFLAGS") && !t.generics
}
...
func (t *test) run() {
	...
	// skipExcludedGcflags reports whether the current test
	// configuration should be skipped because flags (which should be an
	// arguments list for "go tool compile", not "go build") contains an
	// excluded flag.
...
var excludedFiles = map[string]bool{
	...
	"printbig.go":     true, // types2 doesn't convert large untyped ints to int64 when passed to print on 32-bit
