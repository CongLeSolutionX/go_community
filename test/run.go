...
// shouldTest looks for +test tags in a source file and returns
...
func checkShouldTest() {
	...
	// A file with no +test tags will always be tested.
	assert(shouldTest("// This is a test.", "os", "arch"))

	// +test tags separated by a space are OR-ed together.
	assertNot(shouldTest("// +test arm 386", "linux", "amd64"))

	// +test tags separated by a comma are AND-ed together.
	assertNot(shouldTest("// +test !windows,!plan9", "windows", "amd64"))
	assertNot(shouldTest("// +test !windows,!plan9", "plan9", "386"))

	// +test tags on multiple lines are AND-ed together.
