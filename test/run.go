...
var (
	...
	dirs = []string{".", "ken", "chan", "interface", "syntax", "dwarf", "fixedbugs", "codegen", "escestimator"}
	...
	for len(args) > 0 && strings.HasPrefix(args[0], "-") {
		...
		default:
			...
			if strings.Contains(a, "$PWD") {
				a = strings.Replace(a, "$PWD", getenv("PWD", ".")+"/"+t.dir, -1)
			}
	...
	for _, we := range want {
		...
		for _, errmsg := range errmsgs {
			...
			if we.re.MatchString(text) {
				matched = true
			} else if we.any && we.re.MatchString(errmsg) { // "ERRORANY" lacks leading file:line, use errmsg, not text
				matched = true
			} else {
...
type wantedError struct {
	...
	any     bool // match error line w/ no file:line prefix
	file    string
	prefix  string
}
...
var (
	...
	errAnyRx    = regexp.MustCompile(`// (?:GC_)?ERRORANY (.*)`)
...
func (t *test) wantedErrors(file, short string) (errs []wantedError) {
	...
	for i, line := range strings.Split(string(src), "\n") {
		...
		if m != nil {
			...
			m = errAnyRx.FindStringSubmatch(line)
			if m != nil {
				any = true
			} else {
