...
func (t *test) run() {
	...
	if pos == -1 {
		t.err = errors.New(fmt.Sprintf("double newline ending execution recipe not found in %s", t.goFileName()))
		return
	}
	action := t.src[:pos]
...
func (t *test) checkExpectedOutput(gotBytes []byte) {
	...
	if got != string(b) {
		if err == nil {
			...
			t.err = fmt.Errorf("because expected output file %s was not present, output should be empty. Instead saw\n%s", filename, got)
		}
	}
}
