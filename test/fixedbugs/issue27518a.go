...
func main() {
	...
	defer func() {
		// Ignore the panic.
		recover()
		// Force a stack walk. This will fail because x is now
	...
	// don't have a liveness map here. Traceback will instead use
	// the liveness map as of the defer, but in that liveness map,
	// x will be live again even though it points to dead memory.
	*nilp = 0
}
