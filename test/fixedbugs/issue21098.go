...
// The relevant changes for this bug are to give "empty"
// plain blocks (which may contain jumps) the line number
// of their successors, and to give rematerializations
// the line number of their uses, not their definitions.
...
func main() {
	...
	target := os.Getenv("TESTFILE")
	begin := "\"\".openPE" // Text at beginning of relevant dissassembly.

	if target == "" {
		target = "issue21098b"
	}

	cmd := exec.Command("go", "build", "-gcflags", "-S -N -l", "fixedbugs/"+target+".go")
	...
	beforeLineNumber := target + ".go:" // Text preceding line number in each line.
	...
	for scanner.Scan() {
		...
		if i < 0 {
			// Done reading lines
			if scannedCount < 100 {
				fmt.Printf("Scanned only %3.0f lines, was expecting more than 100\n", scannedCount)
				return
			}
		...
		scannedCount++
		i += lbln
		lineVal, err := strconv.Atoi(line[i : i+2])
		if err != nil {
			fmt.Printf("Expected 2-digit line number after %s in %s\n", beforeLineNumber, line)
		}
		if prevVal == 0 {
			prevVal = lineVal
		}
